...
A seguinte terminologia é utilizada nos esquemas do Sentinel:
...
| **Algoritmo de haxixe** | Cadeia | São suportadas as seguintes colunas de haxixe:<ul><li>MD5</li><li>SHA1</li><li>SHA256</li><li>SHA512</li></ul> |
| **Tipo de Arquivo** | Cadeia | O tipo do tipo de ficheiro:<ul><li>Extensão</li><li>Classe</li><li>ChamadoType</li></ul> |
| 
...
| DvcMacAddr | Cadeia | 06:10:9f:eb:8f:14 | O endereço MAC da interface de rede do dispositivo de reporte a partir do qual o evento foi enviado. | Dispositivo,<br>Mac |
...
| Produto de Eventos | Cadeia | OfficeSharepoint | O produto que gera o evento. | Evento |
...
| EventReportUrl | Cadeia | https://192.168.1.1/repoerts/ae3-56.htm | Um link para o relatório completo criado pelo dispositivo de reporte | Evento |
| EventVendor | Cadeia | Microsoft | O fornecedor do produto que gera o evento. | Evento |
| EventoResult | Multivalue: Sucesso, Parcial, Fracasso, [Vazio] (Cadeia) | Success | O resultado reportado para a atividade. Valor vazio quando não aplicável. | Evento |
| EventResultDetails | Cadeia | Senha errada | Razão ou detalhes para o resultado relatado no EventResult | Evento |
| EventoSsAversão | Real | 0.1 | Versão Azure Sentinel Schema. Atualmente 0.1. | Evento |
| EventSeverity | Cadeia | Baixo | Se a atividade reportada tiver um impacto de segurança, denota a gravidade do impacto. | Evento |
| EventoOriginaluid | Cadeia | af6ae8fe-ff43-4a4c-b537-8635976a2b51 | A identificação do registo do dispositivo de reporte. | Evento |
...
| EventUid | Guid (Corda) | 516a64e3-8360-4f1e-a67c-d96b3d52df54 | Identificador único usado pelo Sentinel para marcar uma linha. | Evento |
| NetworkApplicationProtocol | Cadeia | HTTPS | O protocolo de camada de aplicação utilizado pela ligação ou sessão. | Rede |
...
| DstGeoCity | Cadeia | Burlington | A cidade associada ao endereço IP de destino | Destino,<br>Área Geográfica |
...
| DstDvcFqdn | Cadeia | victim_pc.contoso.local | O nome de domínio totalmente qualificado do anfitrião onde o log foi criado | Destino,<br>Dispositivo |
...
| DstUserSid | SID do utilizador |  S-12-1445 | Identificação do Utilizador da identidade associada ao destino da sessão. Tipicamente, a identidade usada para autenticar um servidor. Consulte "tipos de dados" para obter mais detalhes. | Destino,<br>Utilizador |
| DstUserAadId | Corda (guia) | ae92b0b4-cfba-4b42-85a0-fbd862f4df54 | O ID do objeto de conta Azure AD do utilizador no final da sessão | Destino,<br>Utilizador |
| Nome DstUser | Nome de utilizador (String) | johnd | O nome de utilizador da identidade associada ao destino da sessão.  | Destino,<br>Utilizador |
| DstUserUpn | string | johnd@anon.com | A UPN da identidade associada ao destino da sessão. | Destino,<br>Utilizador |
| DstUserDomain | string | GRUPO DE TRABALHO | O domínio ou nome do computador da conta no destino da sessão | Destino,<br>Utilizador |
| DstZone | Cadeia | Dmz | A zona de rede do destino, tal como definida pelo dispositivo de reporte. | Destino |
...
| SrcGeoCity | Cadeia | Burlington | A cidade associada com o endereço IP de origem | Fonte,<br>Área Geográfica |
...
| SrcUserUpn | string | bob@alice.com | UPN da conta que inicia a sessão | Fonte,<br>Utilizador |
| SrcUserDomain | string | DESKTOP | O domínio da conta que inicia a sessão | Fonte,<br>Utilizador |
| SrcZone | Cadeia | Toque | A zona de rede da fonte, tal como definida pelo dispositivo de reporte. | Origem |
| NetworkProtocol | Cadeia | TCP | O protocolo IP utilizado pela ligação ou sessão. Tipicamente, TCP, UDP ou ICMP | Rede |
| CloudAppName | Cadeia | Facebook | O nome do pedido de destino para um pedido HTTP identificado por um representante. | Cloud |
| CloudAppId | Cadeia | 124 | O ID do pedido de destino para uma aplicação HTTP identificada por um representante. Este valor é geralmente específico do proxy utilizado. | Cloud |
| CloudAppOperation | Cadeia | DeleteFile | A operação que o utilizador realizou no contexto do pedido de destino para uma aplicação HTTP identificada por um representante. Este valor é geralmente específico do proxy utilizado. | Cloud |
| CloudAppRiskLevel | Cadeia | 3 | O nível de risco associado a uma aplicação HTTP identificada por um representante. Este valor é geralmente específico do proxy utilizado. | Cloud |
| Nome de arquivo | Cadeia | ImNotMalicious.exe | O nome de ficheiro transmitido através das ligações de rede para protocolos como FTP e HTTP que fornecem a informação sobre o nome do ficheiro. | Ficheiro |
| FilePath | Cadeia | C:\Malicious\ImNotMalicious.exe | O caminho completo, incluindo o nome do ficheiro, do ficheiro | Ficheiro |
| FileHashMd5 | Cadeia | 51BC68715FC7C109DCEA406B42D9D78F | O valor hash MD5 do ficheiro transmitido através das ligações de rede para protocolos. | Ficheiro |
...
| Filehashsha256 | Cadeia | 9B8F8EDB... C129976F03 | O valor hash sha256 do ficheiro transmitido através das ligações de rede para protocolos. | Ficheiro |
...
| HttpContentType | Cadeia | multiparte/formulários; limite=algo | O cabeçalho do tipo de conteúdo http response para sessões de rede HTTP/HTTPS. | Http |
| HttpReferrerOriginal | Cadeia | https://developer.mozilla.org/en-US/docs/Web/JavaScript | O cabeçalho de remetente http para sessões de rede HTTP/HTTPS. | Http |
| HttpUserAgentOriginal | Cadeia | Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, como Gecko) Chrome/83.0.4103.97 Safari/537.36 | O cabeçalho do agente http para sessões de rede HTTP/HTTPS. | Http |
| HttpRequestXff | Cadeia | 120.12.41.1 | O cabeçalho http-x-forward-for para sessões de rede HTTP/HTTPS. | Http |
| UrlCategoria | Cadeia | Motores de busca | O agrupamento definido de um URL (ou pode ser apenas baseado no domínio no URL) relacionado com o que é (por assim dizer: adulto, notícias, publicidade, domínios estacionados, etc.) | url |
| UrlOriginal | Cadeia | https:// contoso.com/fo/?k=v&q=u#f | O URL de pedido http para sessões de rede HTTP/HTTPS. | Url |
| UrlHostname | Cadeia | contoso.com | A parte de domínio de um URL de pedido HTTP para sessões de rede HTTP/HTTPS. | Url |
| AmeaçaCategoria | Cadeia | Troia | A categoria de uma ameaça identificada por um sistema de segurança como o Portal de Segurança Web de um IPS e está associada a esta sessão de rede. | Ameaça |
| ThreatId | Cadeia | Tr.124 | O ID de uma ameaça identificada por um sistema de segurança como o Portal de Segurança Web de um IPS e está associada a esta sessão de rede. | Ameaça |
| Nome de ameaça | Cadeia | Ficheiro de Teste EICAR | O nome da ameaça ou malware identificado | Ameaça |
