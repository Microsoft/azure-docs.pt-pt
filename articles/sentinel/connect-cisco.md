...
Você precisa implantar um agente em um computador Linux dedicado (VM ou local) para dar suporte à comunicação entre o Cisco ASA e o Azure Sentinel. O diagrama a seguir descreve a configuração no caso de uma VM do Linux no Azure.
...
## <a name="step-2-forward-cisco-asa-logs-to-the-syslog-agent"></a>ETAPA 2: encaminhar os logs do Cisco ASA para o agente de syslog
...
Para usar o esquema relevante em Log Analytics para os eventos Cisco, procure por `CommonSecurityLog`.
