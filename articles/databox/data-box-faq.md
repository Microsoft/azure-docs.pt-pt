...
R. O Data Box tem uma capacidade bruta de 100 TB e uma capacidade utilizável de 80 TB. Pode transferir até 80 TB de dados com a Data Box. Para transferir mais dados, tem de encomendar mais dispositivos.
...
R.  Para obter informações sobre quais os países/regiões disponíveis a Caixa de Dados, aceda à [disponibilidade da região.](data-box-overview.md#region-availability)  

### <a name="q-which-regions-can-i-store-data-in-with-data-box"></a>P. Em que regiões posso armazenar dados com o Data Box?
R. A Data Box é apoiada para todas as regiões dos EUA, Europa Ocidental, Norte da Europa, França, Reino Unido, Japão, Austrália e Canadá. Para mais informações, aceda à [disponibilidade da Região.](data-box-overview.md#region-availability)
...
R. Sim. Há uma carga do dispositivo perdida ou danificada. Esta taxa está abrangida pela [página de preços,](https://azure.microsoft.com/pricing/details/storage/databox/) bem como nos [Termos de Serviço do Produto.](https://www.microsoft.com/licensing/product-licensing/products)
...
R. Sim. Ambas as interfaces de 10 GbE podem ser ligadas na Caixa de Dados para copiar dados ao mesmo tempo. Para obter mais informações sobre como copiar dados, aceda ao [Tutorial: Copie os dados para a Caixa de Dados Azure](data-box-deploy-copy-data.md).
...
R. Não suportamos a configuração do canal portuário, a configuração do IO Multipath (MPIO) ou a configuração vLAN na Data Box.
...
R. Se o seu dispositivo não estiver a arrancar, vá ao seu pedido no portal Azure. Faça o download de uma etiqueta de envio e afixe-a no dispositivo. Para obter mais informações, aceda a [Download shipping label](data-box-portal-admin.md#download-shipping-label) (Transferir a guia de remessa).
...
R.  Uma vez que o estado da encomenda **da Cópia de Dados** apareça como **Completo,** deverá poder aceder imediatamente aos seus dados.
...
R.  Quando copia os dados para a Data Box, dependendo se os dados são blob de bloco ou blob de página ou ficheiros Azure, os dados são enviados para um dos seguintes caminhos na sua conta de Armazenamento Azure.
