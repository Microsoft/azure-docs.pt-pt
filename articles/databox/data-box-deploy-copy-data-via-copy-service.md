...
Este tutorial descreve como ingerir dados utilizando o serviço de cópia de dados sem um hospedeiro intermédio. O serviço de cópia de dados funciona localmente na Microsoft Azure Data Box, conecta-se ao seu dispositivo de armazenamento ligado à rede (NAS) via SMB e copia dados para Data Box.
...
1. Completou este tutorial: Instale a Caixa de [Dados Azure.](data-box-deploy-set-up.md)
2. Recebeu o seu dispositivo Data Box e o estado de encomenda no portal é **Entregue.**
3. Tem as credenciais do dispositivo NAS de origem a que se ligará para cópia de dados.
4. Estáligado a uma rede de alta velocidade. Recomendamos vivamente que tenha pelo menos uma ligação 10 Gigabit Ethernet (GbE). Se não estiver disponível uma ligação de 10 GbE, pode utilizar uma ligação de dados de 1-GbE, mas as velocidades de cópia serão afetadas.
...
* Se os dados enviados pela Data Box forem simultaneamente carregados por outras aplicações fora da Data Box, podem resultar falhas no trabalho de upload e corrupção de dados.
...
1. Na Web UI local do seu dispositivo Data Box, vá para **gerir** > **os dados da Cópia**.
