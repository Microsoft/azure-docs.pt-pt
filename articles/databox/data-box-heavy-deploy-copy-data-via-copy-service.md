---
title: 'Tutorial: Copiar dados para Azure Data Box Heavy via serviço de cópia de dados'
...
Este tutorial descreve como ingerir dados utilizando o serviço de cópia de dados sem um hospedeiro intermédio. O serviço de cópia de dados funciona localmente na Azure Data Box Heavy, conecta-se ao seu dispositivo de armazenamento (NAS) ligado à rede via SMB e copia dados para Data Box Heavy.
...
1. Completou este tutorial: [Configurar a Azure Data Box Heavy](data-box-heavy-deploy-set-up.md).
...
4. Está ligado a uma rede de alta velocidade. Para velocidades de cópia mais rápidas, podem ser utilizadas em paralelo duas ligações de 40 GbE (uma por nó). Se não tiver uma ligação de 40 GbE disponível, recomendamos que tenha pelo menos duas ligações de 10 GbE (uma por nó). 
...
- Ao copiar dados, certifique-se de que o tamanho dos dados está em conformidade com os limites de tamanho descritos no artigo [Azure storage and Data Box Heavy limits](data-box-heavy-limits.md).
- Se os dados enviados por Data Box Heavy forem enviados simultaneamente por outras aplicações fora da Data Box Heavy, podem resultar falhas no trabalho de upload e corrupção de dados.
...
1. Na UI web local do seu dispositivo Data Box Heavy, vá para **gerir**  >  **os dados da Cópia.**
...
> [!IMPORTANT]
> Repita as mesmas instruções para copiar os dados para o segundo nó na Caixa de Dados Pesada.
...
> [Envie o seu dispositivo Azure Data Box Heavy para a Microsoft](./data-box-heavy-deploy-picked-up.md)
