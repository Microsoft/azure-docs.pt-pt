...
A. Para obter informações sobre o preço dos Discos de Caixa de Dados, vá à [página de Preços](https://azure.microsoft.com/pricing/details/databox/disk/).

### <a name="q-how-do-i-get-data-box-disks"></a>P. Como recebo os discos do Data Box Disk? 
A.  Para obter discos de caixa de dados Azure, inicie sessão no portal Azure e crie uma ordem data box para discos. Indique as suas informações de contacto e os seus detalhes de notificação. Depois de fazer a encomenda, os discos são-lhe enviados num prazo de dez dias, com base na disponibilidade.
...
A.  Para ver onde estão atualmente disponíveis os Discos da Caixa de Dados, vá à disponibilidade da [Região.](data-box-disk-overview.md#region-availability)  
...
A. Data Box Disk é apoiado para todas as regiões dos EUA, Canadá, Austrália, Europa Ocidental e Norte da Europa, Coreia e Japão. Só são suportadas as regiões de cloud pública do Azure. O Azure Government e as outras clouds soberanas não são suportadas.
...
A. O Data Box Disk é enviado de dentro do mesmo país que o seu destino e não atravessará quaisquer fronteiras internacionais. A única exceção é para as encomendas na União Europeia (UE), onde os discos podem ser enviados de e para qualquer país da UE.
...
A. Se encontrar algum problema com discos de caixa de dados, [contacte o Suporte da Microsoft](https://docs.microsoft.com/azure/databox/data-box-disk-contact-microsoft-support).
...
A.  Só pode cancelar a encomenda depois de os discos terem sido encomendados e antes de serem enviados. Após o envio, já não pode cancelar a encomenda. No entanto, pode devolver os seus discos a uma carga. 
...
A. Sim. A Microsoft também oferece envios autogeridos apenas na região de Us Gov. Ao colocar a encomenda do Disco da Caixa de Dados, pode escolher a opção de envio autogerida. Para recolher a sua encomenda de Disco de Caixa de Dados, tome os seguintes passos:
...
A. O conjunto de ferramentas disponível com o Disco da Caixa de Dados contém três ferramentas:
...
A. Os Ficheiros Azure são suportados com o Disco da Caixa de Dados, mas não funcionarão bem com o Azure File Sync. Os metadados não são retidos se os dados do ficheiro forem utilizados com o Azure File Sync.
...
A. Sim. Os seus dados foram enviados para o Azure, mas se não vir nenhum disco gerido com os grupos de recursos especificados, é provável que os dados não sejam válidos. Se as bolhas de página, blocos, Ficheiros Azure e discos geridos não fossem válidos, estas iriam para as seguintes pastas:
