...
A associação de uma subscrição Azure no Open a um EA fará com que quaisquer créditos não consumidos do Azure no Open sejam perdidos. Para evitar qualquer perda potencial de créditos, é recomendado que os clientes consumam todos os créditos numa subscrição Azure no Open antes de adicionar a conta ao EA.  
...
1. No Azure EA Portal, na área de navegação esquerda, clique em **Relatórios** e veja o separador **Resumo de Utilização**.  
  ![Criar e ver o resumo da utilização e transferir os relatórios](./media/billing-ea-portal-get-started/create-ea-view-usage-summary-and-download-reports.png)
...
3. Alterne entre **M** (Mensal) e **P** (Personalizado) na parte superior direita da página para ver o **Resumo de Utilização** com datas de início e de fim.  
  ![Criar e ver o resumo da utilização e transferir os relatórios na vista personalizada](./media/billing-ea-portal-get-started/create-ea-view-usage-summary-and-download-reports-custom-view.png)
...
- **Conta**: uma unidade organizacional no Azure EA Portal utilizada para administrar subscrições e para relatórios.
- **Proprietário da conta**: a pessoa identificada para gerir as subscrições e os administradores de serviços no Microsoft Azure. Podem ver os dados de utilização nessa conta e as subscrições.
- **Subscrição de emenda**: uma subscrição única de um ano ou consubstancial ao abrigo da emenda da inscrição.
- **Alocação**: alocação de um montante monetário anual para os serviços do Microsoft Azure a uma tarifa de alocação com desconto para utilização em relação a este pré-pagamento.
- **Administrador de departamento**: a(s) pessoa(s) identificada(s) para gerir departamentos, criar novas contas e proprietários de contas, ver detalhes de utilização dos departamentos que gerem e ver os custos quando forem concedidas permissões.
...
- **Administrador do Enterprise**: a(s) pessoa(s) identificada(s) para gerir departamentos e proprietários de departamentos, bem como contas e proprietários de contas no Microsoft Azure. Têm a capacidade de gerir administradores do Enterprise e de ver os dados de utilização, as quantidades faturadas e os custos não faturados em todas as contas e subscrições associadas à inscrição Enterprise.
...
- **Inscrição do contrato Enterprise**: uma inscrição no programa de contrato Enterprise que fornece produtos da Microsoft em volume a tarifas com desconto.
...
- **Emenda da inscrição Enterprise do Microsoft Azure (emenda da inscrição)** : uma emenda assinada por uma empresa, que lhes fornece acesso ao Microsoft Azure como parte da inscrição Enterprise.
- **Azure EA Portal**: o portal utilizado pelos nossos clientes Enterprise para gerir as contas do Microsoft Azure e as subscrições relacionadas.
- **Quantidade de recursos consumida**: a quantidade de um serviço do Microsoft Azure individual que foi utilizada num mês.
- **Administrador de serviços**: a pessoa identificada para aceder e gerir subscrições e projetos de desenvolvimento no Azure EA Portal.
- **Subscrição**: representa uma subscrição do Azure EA Portal e é um contentor de serviços do Microsoft Azure geridos pelo mesmo administrador de serviços.
...
- **Pendente**: o administrador da inscrição tem de iniciar sessão no Azure EA Portal. Depois de a sessão ter sido iniciada, a inscrição mudará para o estado Ativo.
- **Ativo**: a inscrição está Ativa e podem ser criadas contas e subscrições no Azure EA Portal. A inscrição permanecerá ativa até à data limite do contrato Enterprise.
- **Período alargado indefinidamente**: o estado de período alargado indefinidamente ocorre depois de chegar à data limite do contrato Enterprise. Permite que os clientes EA que optaram pelo período alargado continuem a utilizar o Azure indefinidamente no final do contrato Enterprise. Antes de a inscrição EA atingir a data limite do contrato Enterprise, o administrador da inscrição deve decidir se vai renovar a inscrição através da adição de alocação monetária adicional, transferir para uma nova inscrição, migrar para o Programa de Subscrição Online da Microsoft (MOSP) ou confirmar a desativação de todos os serviços associados à inscrição.
- **Expirado**: o cliente EA não optou pelo período alargado e a inscrição EA atingiu a data limite do contrato enterprise, a inscrição vai expirar e todos os serviços associados serão desativados.
- **Transferido**: as inscrições onde todas as contas e serviços associados foram transferidos para uma nova inscrição serão apresentadas com o estado transferido. Tenha em consideração que as inscrições não serão transferidas automaticamente se for gerado um novo número de inscrição na renovação. O número de inscrição anterior tem de ser incluído na documentação de renovação do cliente para facilitar a transferência automática.
