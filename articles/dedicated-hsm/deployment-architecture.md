...
Cada uma dessas regiões tem racks HSM implementados em dois datacenters independentes ou, pelo menos, duas zonas de disponibilidade independentes. Sudeste asiático tem três zonas de disponibilidade e E.U.A. Leste 2 tem dois. Há um total de oito regiões na Europa, Ásia e EUA que oferecem o serviço de HSM dedicados. Para obter mais informações sobre as regiões de Azure, consulte as regiões oficiais de [Azure.](https://azure.microsoft.com/global-infrastructure/regions/)
...
Depois de arquitetura de implantação é determinada, a maioria das atividades de configuração para implementar essa arquitetura serão fornecidas pela Gemalto. Isto inclui a configuração do dispositivo, bem como a aplicação de cenários de integração. Para mais informações, utilize o portal de apoio ao [cliente Gemalto](https://supportportal.gemalto.com/csm/) e descarregue guias de administração e configuração. O site de parceiro da Microsoft tem uma variedade de guias de integração.
