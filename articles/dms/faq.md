...
Outros cenários de migração estão em versão prévia e exigem o envio de uma indicação por meio do site de visualização do DMS. Para uma listagem completa dos cenários em pré-visualização e para se inscrever para participar numa destas ofertas, consulte o [site de pré-visualização dMS](https://aka.ms/dms-preview/).
...
Você pode usar o serviço de migração de banco de dados do Azure para realizar migrações online e offline. Com uma migração *offline,* o tempo de inatividade da aplicação começa quando a migração começa. Com uma migração *online,* o tempo de inatividade limita-se ao tempo de redução no final da migração. Sugerimos que teste uma migração offline para determinar se o período de inatividade é aceitável; se não for, faça uma migração online.
