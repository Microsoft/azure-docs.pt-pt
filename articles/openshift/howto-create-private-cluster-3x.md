...
Um cluster privado pode ser implantado usando os scripts de amostra fornecidos abaixo. Uma vez que o cluster é implantado, execute o `cluster get` comando e veja a propriedade para determinar o endereço IP privado do servidor `properties.FQDN` OpenShift API.
...
Se alterar os servidores de nomes DNS na rede virtual do cluster, terá de emitir uma atualização no cluster com a `properties.RefreshCluster` propriedade definida para que os `true` VMs possam ser reimagemdos. Esta atualização permitir-lhes-á recolher os novos servidores de nomes.
...
> [!NOTE]
> A localização deve ser definida, uma `eastus2` vez que esta é atualmente a única localização suportada para clusters privados.
