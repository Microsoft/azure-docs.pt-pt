...
Se estiver a executar o Azure CLI localmente, abra uma concha de comando Bash e faça `az login` para iniciar sessão no Azure.
...
Dete`APPID` ao valor que guardou no passo 5 da Create a AD registo de [aplicações.](howto-aad-app-configuration.md#create-an-azure-ad-app-registration)
...
Detete `SECRET` ao valor que guardou no passo 8 da [Create a client secret](howto-aad-app-configuration.md#create-a-client-secret).

```bash
SECRET=<secret value>
```

Detete `TENANT` para o valor de identificação do inquilino que poupou no passo 7 da [Create um novo inquilino](howto-create-tenant.md#create-a-new-azure-ad-tenant)
...
Primeiro, pegue o identificador do VNET existente. O identificador será da forma: `/subscriptions/{subscription id}/resourceGroups/{resource group of VNET}/providers/Microsoft.Network/virtualNetworks/{VNET name}`.
...
Se **estiver** a espreitar o seu cluster para uma rede virtual, utilize o seguinte comando que adiciona a bandeira `--vnet-peer`:

```bash
az openshift create --resource-group $CLUSTER_NAME --name $CLUSTER_NAME -l $LOCATION --aad-client-app-id $APPID --aad-client-app-secret $SECRET --aad-tenant-id $TENANT --customer-admin-group-id $GROUPID --vnet-peer $VNET_ID
```

Se **pretender** monitorização Azure com o seu cluster, utilize o seguinte comando que adiciona a bandeira `--workspace-id`:
...
Depois de alguns minutos, `az openshift create` completa.
...
O sinal em URL para o seu cluster será `https://` seguido pelo valor `publicHostName`.  Por exemplo: `https://openshift.xxxxxxxxxxxxxxxxxxxx.eastus.azmosa.io`.  Utilizará este URI no próximo passo como parte do redirecionamento uri do registo da aplicação.
...
A página **Ferramentas** da Linha de Comando fornece um comando do formulário `oc login https://<your cluster name>.<azure region>.cloudapp.azure.com --token=<token value>`.  Clique na *Cópia para botão de clipboard* para copiar este comando.  Numa janela terminal, [delicie o seu caminho](https://docs.okd.io/latest/cli_reference/get_started_cli.html#installing-the-cli) para incluir a instalação local das ferramentas oc. Em seguida, inscreva-se no cluster utilizando o comando oc CLI que copiou.
