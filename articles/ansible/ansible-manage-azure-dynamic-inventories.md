...
1. Use o comando GNU `wget` para recuperar o script `azure_rm.py`:
...
1. Ao executar o comando anterior, você pode receber o seguinte erro:
    ...
    Se você receber o erro "verificação de chave de host", adicione a seguinte linha ao arquivo de configuração Ansible. O arquivo de configuração Ansible está localizado em `/etc/ansible/ansible.cfg`.
...
- Se você estiver usando Ansible > = 2,8, execute o seguinte comando:
