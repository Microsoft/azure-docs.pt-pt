...
```
> [!NOTE]
> O comando acima permite a conectividade de saída para os recursos no pool de backend do equilibrador de carga. Para uma configuração avançada de conectividade de saída, consulte **[as ligações de saída em Azure](load-balancer-outbound-connections.md)** e **[configurar regras de equilíbrio e saída de carga no Balancer de Carga Padrão utilizando o Azure CLI](configure-load-balancer-outbound-cli.md)**.
