...
description: Este artigo mostra-lhe como atualizar o Equilíbrio de Carga Interna Básica azure para o Equilíbrio de Carga Pública Padrão
...
* O script suporta a atualização do Balancer de Carga Interna onde é necessária a ligação de saída. Se a ligação de saída não for necessária para qualquer um dos VMs, consulte [esta página](upgrade-basicInternal-standard.md) para obter as melhores práticas.
* O Standard Load Balancer tem um novo endereço público. É impossível mover os endereços IP associados ao equilíbrio de carga interna básico existente sem problemas para o Standard Public Load Balancer, uma vez que têm SKUs diferentes.
...
1. Examinar os parâmetros necessários:
   ...
   * **newlocation: [String]: Required** – Este é o local em que o Balancer de Carga Padrão será criado. Recomendamos herdar a mesma localização do Equilibrador básico de carga escolhido para o Equilíbrio de Carga Padrão para uma melhor associação com outros recursos existentes.
