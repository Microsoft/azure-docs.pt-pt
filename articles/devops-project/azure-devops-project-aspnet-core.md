---
title: 'Quickstart: Criar um pipeline CI/CD para .NET com Arranque de DevOps Azure'
description: O Arranque azure DevOps facilita o arranque do Azure. Ajuda-o a iniciar uma aplicação .NET num serviço do Azure à sua escolha com alguns passos rápidos.
ms.prod: devops
...
---
# <a name="create-a-cicd-pipeline-for-net-with-azure-devops-starter"></a>Crie um pipeline CI/CD para .NET com Arranque de DevOps Azure

Configure a integração contínua (CI) e a entrega contínua (CD) para o seu núcleo .NET ou ASP.NET aplicação com devOps Starter. DevOps Starter simplifica a configuração inicial de um oleoduto de construção e libertação em Pipelines Azure.
...
DevOps Starter cria um oleoduto CI/CD em Azure DevOps. Você pode criar uma nova organização Azure DevOps ou usar uma organização existente. DevOps Starter também cria recursos Azure na subscrição Azure à sua escolha.
...
3. Selecione a sua **Subscrição Azure,** introduza um nome para a sua **aplicação Web** ou pegue o predefinido e, em seguida, selecione **Done**. Após alguns minutos, a visão geral de implantação de arranque de DevOps é exibida no portal Azure. 

4. Selecione **Recorrer para** ver o painel de arranque de DevOps. No canto superior direito, coloque o **Projeto** no seu painel para um acesso rápido. Uma aplicação de amostra é configurada num repo na sua **Organização Azure DevOps**. Uma construção é executada, e a sua aplicação está implantada para o Azure.
...
DevOps Starter criou um repositório Git em Azure Repos ou GitHub. Para visualizar o repositório e fazer alterações de código na sua aplicação, faça o seguinte:
...
4. **Selecione Editar**e, em seguida, fazer uma alteração na rubrica h2. Por exemplo, tipo Começar imediatamente com o Arranque de **DevOps Azure** ou fazer alguma sonorcultura.
...
6. No seu navegador, vá ao painel de iniciação Azure DevOps Starter.  Deverá agora ver que está em curso uma compilação. As alterações efetuadas são automaticamente construídas e implantadas através de um gasoduto CI/CD.

## <a name="examine-the-cicd-pipeline"></a>Examinar o gasoduto CI/CD

No passo anterior, o Arranque a Partida azure DevOps configurava automaticamente um gasoduto CI/CD completo. Explore e personalize o pipeline, conforme necessário. Tome os seguintes passos para se familiarizar com os oleodutos Azure DevOps.

1. Na parte superior do painel de arranque de DevOps, selecione **Build Pipelines**. Este link abre um separador de navegador e o Azure DevOps constrói um pipeline para o seu novo projeto.
...
1. Neste painel, pode examinar as várias tarefas para o seu pipeline de construção. A construção executa várias tarefas, tais como a busca de fontes do repositório Git, restaurar dependências e publicar saídas usadas que são usadas para implantações.
...
Pode eliminar o Serviço de Aplicações Azure e outros recursos relacionados que criou quando já não precisa deles. Utilize a funcionalidade **Eliminar** no painel de arranque de DevOps.
