---
title: Garantir a continuidade do negócio & recuperação de desastres usando as Regiões Emparelhadas de Azure
...
ms.locfileid: "82980053"
---
# <a name="business-continuity-and-disaster-recovery-bcdr-azure-paired-regions"></a>Continuidade empresarial e recuperação após desastre (BCDR): Regiões Emparelhadas do Azure
...
![AzureGeografia](./media/best-practices-availability-paired-regions/GeoRegionDataCenter.png)
...
4. **Azure Resource Manager** – o Resource Manager proporciona inerentemente o isolamento lógico dos componentes entre regiões. Isto significa que as falhas lógicas numa região são menos propensas a ter impacto noutra.
...
7. Ordem de recuperação da **região** – Em caso de paralisação alargada, a recuperação de uma região é priorizada em cada par. As aplicações implementadas entre regiões emparelhadas têm a garantia de ter uma das regiões recuperadas com prioridade. Se uma aplicação for implementada em regiões que não sejam emparelhadas, a recuperação poderá ser adiada – no pior dos casos, as regiões escolhidas podem ser as duas últimas a serem recuperadas.
