---
title: APIs e ferramentas para desenvolvedores - Lote Azure / Microsoft Docs
...
O processamento de cargas de trabalho paralelas com o Lote Azure é normalmente feito programáticamente utilizando uma das APIs do Lote. A aplicação ou serviço de cliente pode utilizar APIs do Batch para comunicar com o serviço Batch. Com as APIs do Batch, pode criar e gerir conjuntos de nós de computação, máquinas virtuais ou serviços cloud. Pode, então, agendar tarefas para serem executadas nesses nós. 
...
* [Cmdlets De Lote PowerShell][batch_ps]: Os cmdlets do lote Azure no módulo [PowerShell Azure](/powershell/azure/overview) permitem-lhe gerir os recursos do Lote com a PowerShell.
...
* [Portal Azure:][portal]Pode criar, monitorizar e eliminar piscinas, empregos e tarefas do Portal Azure. Pode ver as informações de estado para estes e outros recursos ao executar os trabalhos, e até mesmo transferir os ficheiros de nós de computação nos seus conjuntos. Por exemplo, pode transferir o `stderr.txt` de uma tarefa falhada, durante a resolução de problemas. Também pode transferir ficheiros de Ambiente de Trabalho Remoto (RDP) que pode utilizar para iniciar sessão em nós de computação.
* [Azure Batch Explorer][batch_labs]: Batch Explorer (anteriormente chamado BatchLabs) é uma ferramenta de cliente gratuita, rica e autónoma para ajudar a criar, depurar e monitorizar aplicações do Lote Azure. Transfira um [pacote de instalação](https://azure.github.io/BatchExplorer/) para Mac, Linux ou Windows.
...
* [Explorador de Armazenamento Azure][storage_explorer]: Embora não seja estritamente uma ferramenta de Lote Azure, o Explorador de Armazenamento é outra ferramenta valiosa para ter enquanto está a desenvolver e a depurar as suas soluções de Lote.
...
* Descarregue as amostras de código C# no [GitHub][github_samples] para ver como ambos e Python podem interagir com Batch para agendar e processar cargas de trabalho de amostra.
