...
Para obter o valor atual de `cmd.exe` uma variável ambiental, `/bin/sh` lance num nó de computação Windows ou num nó Linux:
...
As linhas de comando executadas por tarefas em nódeos computacionais não são executadas sob uma concha. Portanto, estas linhas de comando não podem aproveitar de forma nativa `PATH`características de concha, tais como a expansão variável ambiental (isto inclui o ). Para tirar partido de tais características, deve **invocar a concha** na linha de comando. Por exemplo, `cmd.exe` lançamento em nós de `/bin/sh` computação Windows ou em nós Linux:
...
| AZ_BATCH_HOST_LIST              | A lista de nódosos que são atribuídos a `nodeIP,nodeIP`uma tarefa em [várias instâncias][multi_instance] no formato . | Tarefas primárias e subtarefas em várias instâncias. | `10.0.0.4,10.0.0.5` |
| AZ_BATCH_IS_CURRENT_NODE_MASTER | Especifica se o nó atual é o nó principal para uma [tarefa em várias instâncias][multi_instance]. Os valores possíveis são `true` e. `false`| Tarefas primárias e subtarefas em várias instâncias. | `true` |
...
| AZ_BATCH_NODE_LIST              | A lista de nódosos que são atribuídos a `nodeIP;nodeIP`uma tarefa em [várias instâncias][multi_instance] no formato . | Tarefas primárias e subtarefas em várias instâncias. | `10.0.0.4;10.0.0.5` |
...
| AZ_BATCH_TASK_DIR               | O caminho completo do diretório de [tarefas][files_dirs] no nó. Este diretório `stdout.txt` contém `stderr.txt` o e para a tarefa, e o AZ_BATCH_TASK_WORKING_DIR. | Todas as tarefas. | C:\user\tasks\workitems\batchjob001\job-1\task001 |
