...
- '[execuçãoInfo](https://docs.microsoft.com/rest/api/batchservice/job/get#jobexecutioninformation)':
  - O `terminateReason` imóvel pode ter valores que indicam que `maxWallClockTime` o, especificado nas restrições de trabalho, foi excedido e, portanto, o trabalho foi encerrado. Também pode ser definido para indicar que uma tarefa falhou se a propriedade de trabalho `onTaskFailure` foi definida adequadamente.
...
- A tarefa passará sempre para o `completed` [Estado,](https://docs.microsoft.com/rest/api/batchservice/task/get#taskstate)independentemente de ter sido bem sucedida ou falhada.
