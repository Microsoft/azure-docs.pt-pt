...
description: Erros para verificar e resolver problemas de empregos e tarefas
author: mscurrell
ms.topic: how-to
ms.date: 03/10/2019
ms.author: markscu
...
Este artigo cobre os erros que podem ocorrer após a apresentação de trabalhos e tarefas. Ele enumera e explica os erros que precisam de ser verificados e tratados.

## <a name="jobs"></a>Tarefas

Um trabalho é um agrupamento de uma ou mais tarefas, as tarefas realmente especificando as linhas de comando a executar.
...
Se uma tarefa de libertação de emprego for especificada para um trabalho, então quando um trabalho está sendo encerrado, uma instância da tarefa de libertação de emprego é executada em cada um dos nós da piscina onde uma tarefa de preparação de emprego foi executada.  As instâncias de tarefa de libertação de emprego devem ser verificadas para determinar se houve erros:
...
- Há `resourceFiles` especificado para uma tarefa, mas houve uma falha que significava que um ou mais ficheiros não descarregava.
...
Devido às piscinas e aos nós de piscina frequentemente sendo efémeros, com os nós sendo continuamente adicionados e apagados, então recomenda-se que os ficheiros de registo sejam persistentes. [Os ficheiros de saída de tarefas](./batch-task-output-files.md) são uma forma conveniente de guardar ficheiros de registo para o Azure Storage.
