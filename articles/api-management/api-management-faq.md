...
* Utilize a autenticação mútua TLS conforme descrito em Como garantir serviços back-end utilizando a autenticação de certificado de [cliente na Gestão da API Azure](api-management-howto-mutual-certificates.md).
...
1. Use `Connect-AzAccount` o cmdlet para iniciar sessão.
2. Desdefinir o contexto da subscrição `Set-AzContext -SubscriptionID <subscriptionGUID>`que tem o serviço utilizando .
3. Obtenha um único URL de `Get-AzApiManagementSsoToken -ResourceGroupName <rgName> -Name <serviceName>`inscrição utilizando .
...
Utilize [`New-AzApiManagementBackend`](https://docs.microsoft.com/powershell/module/az.apimanagement/new-azapimanagementbackend) os cmdlets [`Set-AzApiManagementBackend`](https://docs.microsoft.com/powershell/module/az.apimanagement/set-azapimanagementbackend) PowerShell (para a nova extremidade traseira) ou `-SkipCertificateChainValidation` (para `True`a extremidade traseira existente) e coloque o parâmetro para .
