...
Coloque `daemon.json` no `/etc/docker` diretório no seu dispositivo.

Se a localização já tiver um `daemon.json` ficheiro, adicione a chave **DNS** ao mesmo e guarde o ficheiro.
