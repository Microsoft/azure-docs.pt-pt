...
O agente de IoT Edge e o Hub de IoT Edge são dois módulos que compõem o tempo de execução do IoT Edge. Para obter mais informações sobre as responsabilidades de cada módulo de tempo de execução, consulte [entender o tempo de execução de Azure IOT Edge e sua arquitetura](iot-edge-runtime.md).
...
| runtime.settings.registryCredentials<br>. .username {registryId} | O nome de utilizador do registo de contentor. No Azure Container Registry, o nome de utilizador é normalmente o nome do registo.<br><br> As credenciais do registro são necessárias para qualquer imagem de módulo particular. | Não |
