...
Para monitorizar a sua implantação, consulte [as implementações do Monitor IoT Edge](how-to-monitor-iot-edge-deployments.md).

## <a name="modify-a-deployment"></a>Modificar uma implantação

Quando modifica uma implementação, as alterações replicam-se imediatamente em todos os dispositivos visados. Pode modificar as seguintes definições e funcionalidades para uma implementação existente:
...
### <a name="modify-target-conditions-custom-metrics-and-labels"></a>Modificar as condições de destino, métricas personalizadas e etiquetas

1. No seu hub IoT, selecione **IoT Edge** do menu do painel esquerdo.
1. Selecione o separador de **implementações IoT Edge** e, em seguida, selecione a implementação que pretende configurar.
1. Selecione o separador **Condição alvo.** Altere a Condição de **Destino** para direcionar os dispositivos pretendidos. Também pode ajustar a **Prioridade**.  Selecione **Guardar**.
...
1. Selecione o separador **Métricas** e clique no botão Métricas de **Edição.** Adicione ou modifique métricas personalizadas, utilizando a sintaxe de exemplo como guia. Selecione **Guardar**.

    ![Editar métricas personalizadas numa implementação](./media/how-to-deploy-monitor/metric-list.png)

1. Selecione o separador **Etiquetas** e faça as alterações desejadas e selecione **Guardar**.
