...
Você pode exibir as mensagens que passam pelo hub de IoT Edge e coletar informações de logs detalhados dos contêineres de tempo de execução. Para ativar os logs detalhados nesses contêineres, defina `RuntimeLogLevel` no arquivo de configuração do YAML. Para abrir o ficheiro:
...
Por padrão, o elemento `agent` se parecerá com o exemplo a seguir:
...
2. Selecione **Configurar** em nome DNS. Se a sua máquina virtual já tem um nome DNS configurado, não precisa de configurar um novo. 
...
Se você receber um EventLogexception ao usar `Get-WinEvent` no Windows, verifique as entradas do registro.

**Causa raiz**

O comando `Get-WinEvent` PowerShell se baseia em uma entrada de registro a ser presente para localizar logs por um `ProviderName`específico.

**Resolução**

Defina uma entrada de registo para o daemon de IoT Edge. Crie um arquivo **iotedge. reg** com o seguinte conteúdo e importe-o para o registro do Windows clicando duas vezes nele ou usando o comando `reg import iotedge.reg`:
...
Um módulo de IoT Edge personalizado falha ao enviar uma mensagem para o edgeHub com um erro de `Module not found` 404. O daemon de IoT Edge imprime a mensagem seguinte nos registos: 
...
Encontrou um erro na plataforma do IoT Edge? [Envie um problema](https://github.com/Azure/iotedge/issues) para que possamos continuar a melhorar. 

Se você tiver mais perguntas, crie um [solicitação de suporte](https://portal.azure.com/#create/Microsoft.Support) para obter ajuda. 
