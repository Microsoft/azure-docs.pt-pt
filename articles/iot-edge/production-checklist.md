...
* O segundo guião cria atribuições de funções para conceder ao diretor de serviço, que pode ser executado posteriormente, se necessário. Recomendamos a aplicação da função `role` de utilizador **acrPull** para o parâmetro. Para obter uma lista de funções, consulte [as funções e permissões](../container-registry/container-registry-roles.md)do Registo de Contentores de Azure .

Para autenticar utilizando um diretor de serviço, forneça o ID principal de serviço e a palavra-passe que obteve a partir do primeiro script. Especifique estas credenciais no manifesto de implantação.
...
> [!NOTE]
> Após a implementação de uma autenticação de segurança melhorada, desative a definição do **utilizador do Administrador** para que o acesso padrão de utilizador/palavra-passe não esteja mais disponível. No registo de contentores no portal Azure, a partir do menu do painel esquerdo em **Definições,** selecione **Access Keys**.
