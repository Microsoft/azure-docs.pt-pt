...
| - | ------------------ | ------------------ |
| **AMD64 do Linux** | ![Utilize o Código VS para módulos Java em Linux AMD64](./media/tutorial-c-module/green-check.png) |  |
| **ARM32 do Linux** | ![Utilize o Código VS para módulos Java no Linux ARM32](./media/tutorial-c-module/green-check.png) |  |
...
1. No explorador do VS Code, clique com o botão direito do rato no ficheiro **deployment.template.json** e selecione **Criar e Emitir solução do IoT Edge**.
   ...
   O comando Build e Push inicia três operações. Em primeiro lugar, cria uma nova pasta na solução chamada **config** que detém o manifesto de implantação completo, que é construído a partir de informação no modelo de implementação e outros ficheiros de solução. Em segundo lugar, ele executa `docker build` para criar a imagem de contêiner com base no dockerfile apropriado para sua arquitetura de destino. Em seguida, ele executa `docker push` para enviar por push o repositório de imagens para o registro de contêiner.
...
Usamos o módulo JavaModule twin no manifesto de implantação para definir o limiar de temperatura em 25 graus. Você pode usar o módulo "atualizar" para alterar a funcionalidade sem precisar atualizar o código do módulo.
