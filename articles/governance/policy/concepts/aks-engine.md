...
1. Criar uma atribuição de funções para o diretor de serviço de cluster
   ...
   - Atribuir a atribuição de funções de "Policy Insights Data Writer (Preview)" ao ID da app principal do serviço de cluster (valor _aadClientID_ de etapa anterior) com o Azure CLI. Substitua-o `<subscriptionId>` pelo `<aks engine cluster resource group>` seu ID de subscrição e pelo grupo de recursos em que o cluster Kubernetes autogerido do Motor AKS está.
...
- Instalar em cluster existente com gráficos de leme
  ...
  1. Instale o complemento com um Gráfico de Leme. Substitua-o `<subscriptionId>` pelo `<aks engine cluster resource group>` seu ID de subscrição e pelo grupo de recursos em que o cluster Kubernetes autogerido do Motor AKS está.
...
Para visualizar os registos de add-on da política Azure, utilize: `kubectl`
...
O gatekeeper pod, _gatekeeper-controller-manager-0,_ `gatekeeper-system` está `kube-system` geralmente no espaço ou no espaço de nome, mas pode estar em um espaço de nome diferente dependendo de como é implantado.

Para ver os registos `kubectl`do Gatekeeper, utilize:
