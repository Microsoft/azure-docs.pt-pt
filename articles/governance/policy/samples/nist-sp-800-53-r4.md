...
O artigo seguinte detalha como a definição de iniciativa integrada da Azure Policy Regulatory Compliance mapeia para **domínios** de conformidade e **controlos** para NIST SP 800-53 R4.
...
Em seguida, encontre e selecione os **controlos [Preview]: Audit NIST SP 800-53 R4 e implementar extensões VM específicas para apoiar os requisitos** de auditoria Definição de iniciativa integrada de conformidade regulamentar.
...
### <a name="account-management--account-monitoring--atypical-usage"></a>Gestão de Contas Monitorização de Conta / Utilização Atípica
...
### <a name="account-management--automated-audit-actions"></a>Gestão de Contas Ações de Auditoria Automatizadas
...
### <a name="account-management--automated-system-account-management"></a>Gestão de Contas Gestão automatizada de conta de sistema
...
### <a name="account-management--inactivity-logout"></a>Gestão de Contas Logout de inatividade
...
### <a name="account-management--removal-of-temporary--emergency-accounts"></a>Gestão de Contas Remoção de Contas Temporárias/De Emergência
...
### <a name="account-management--shared--group-account-credential-termination"></a>Gestão de Contas Rescisão credencial de conta partilhada / grupo
...
### <a name="information-flow-enforcement--physical--logical-separation-of-information-flows"></a>Aplicação do Fluxo de Informação [ Separação Física/Lógica dos Fluxos de Informação
...
### <a name="information-flow-enforcement--security-policy-filters"></a>Aplicação do Fluxo de Informação [ Filtros de política de segurança
...
### <a name="least-privilege--auditing-use-of-privileged-functions"></a>Menos Privilégio / Utilização auditada de funções privilegiadas
...
### <a name="least-privilege--authorize-access-to-security-functions"></a>Menos Privilégio / Autorizar o Acesso a Funções de Segurança
...
### <a name="least-privilege--network-access-to-privileged-commands"></a>Menos Privilégio / Acesso à Rede a Comandos Privilegiados
...
### <a name="least-privilege--non-privileged-access-for-nonsecurity-functions"></a>Menos Privilégio / Acesso não privilegiado para funções de não-segurança
...
### <a name="least-privilege--privilege-levels-for-code-execution"></a>Menos Privilégio / Níveis de privilégio para execução de código
...
### <a name="least-privilege--privileged-accounts"></a>Menos Privilégio / Contas Privilegiadas
...
### <a name="least-privilege--prohibit-non-privileged-users-from-executing-privileged-functions"></a>Menos Privilégio / Proibir utilizadores não privilegiados de executar funções privilegiadas
...
### <a name="least-privilege--review-of-user-privileges"></a>Menos Privilégio / Revisão dos Privilégios de Utilização
...
|[Implementar pré-requisitos para auditar VMs do Windows em que o grupo de administradores contém qualquer um dos membros especificados](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F144f1397-32f9-4598-8c88-118decc3ccba) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais do Windows nas quais o grupo de Administradores contém qualquer um dos membros especificados. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.1.0](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_AdministratorsGroupMembersToExclude_Deploy.json) |
|[Implementar pré-requisitos para auditar VMs do Windows em que o grupo de administradores não contém todos os membros especificados](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F93507a81-10a4-4af0-9ee2-34cf25a96e98) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais do Windows nas quais o grupo de Administradores não contém todos os membros especificados. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.1.0](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_AdministratorsGroupMembersToInclude_Deploy.json) |
...
### <a name="remote-access--automated-monitoring--control"></a>Acesso Remoto / Monitorização automatizada / Controlo

**ID**: NIST SP 800-53 R4 AC-17 (1) **Propriedade**: Compartilhada

|Name<br /><sub>(Portal Azure)</sup> |Descrição |Efeito(s) |Versão<br /><sub>(GitHub)</sub> |
|---|---|---|---|---|
|[Implementar pré-requisitos para auditar VMs Linux que permitem ligações remotas a partir de contas sem senhas](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2Fec49586f-4939-402d-a29e-6ff502b20592) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais Linux que permitem ligações remotas a partir de contas sem senhas. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.2.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_LinuxPassword110_Deploy.json) |
...
### <a name="remote-access--disconnect--disable-access"></a>Acesso Remoto / Desconexão / Desativar o Acesso
...
### <a name="remote-access--managed-access-control-points"></a>Acesso Remoto / Pontos de controlo de acesso geridos
...
### <a name="remote-access--privileged-commands--access"></a>Acesso Remoto / Comandos Privilegiados / Acesso
...
### <a name="remote-access--protection-of-confidentiality--integrity-using-encryption"></a>Acesso Remoto / Proteção da Confidencialidade / Integridade Usando encriptação
...
### <a name="wireless-access--antennas--transmission-power-levels"></a>Acesso Sem Fios Antenas / Níveis de Potência de Transmissão
...
### <a name="wireless-access--authentication-and-encryption"></a>Acesso Sem Fios Autenticação e Encriptação
...
### <a name="wireless-access--disable-wireless-networking"></a>Acesso Sem Fios Desativar a rede sem fios
...
### <a name="wireless-access--restrict-configurations-by-users"></a>Acesso Sem Fios Restringir configurações pelos utilizadores
...
### <a name="audit-events--reviews-and-updates"></a>Eventos de Auditoria ! Comentários e atualizações
...
### <a name="audit-generation--changes-by-authorized-individuals"></a>Geração de Auditoria Alterações por Indivíduos Autorizados
...
### <a name="audit-reduction-and-report-generation--automatic-processing"></a>Redução de Auditoria e Geração de Relatórios Processamento Automático
...
### <a name="content-of-audit-records--additional-audit-information"></a>Conteúdo dos Registos de Auditoria ! Informação Adicional de Auditoria
...
### <a name="content-of-audit-records--centralized-management-of-planned-audit-record-content"></a>Conteúdo dos Registos de Auditoria ! Gestão Centralizada do Conteúdo Recorde de Auditoria Planeada
...
### <a name="non-repudiation"></a>Não-Repúdio
...
### <a name="protection-of-audit-information--access-by-subset-of-privileged-users"></a>Proteção de Informação de Auditoria / Acesso por Subconjunto de Utilizadores Privilegiados
...
### <a name="protection-of-audit-information--audit-backup-on-separate-physical-systems--components"></a>Proteção de Informação de Auditoria / Cópia de segurança da auditoria em sistemas/componentes físicos separados
...
### <a name="protection-of-audit-information--cryptographic-protection"></a>Proteção de Informação de Auditoria / Proteção Criptográfica
...
### <a name="response-to-audit-processing-failures--audit-storage-capacity"></a>Resposta a Falhas no Processamento de Auditoria / Capacidade de Armazenamento de Auditoria
...
### <a name="time-stamps--synchronization-with-authoritative-time-source"></a>Selos temporais ! Sincronização com fonte de tempo autoritária
...
### <a name="security-awareness-training--insider-threat"></a>Formação de Sensibilização para a Segurança Ameaça de Insider
...
### <a name="baseline-configuration--automation-support-for-accuracy--currency"></a>Configuração de base / Suporte de automatização para precisão / moeda
...
### <a name="baseline-configuration--retention-of-previous-configurations"></a>Configuração de base / Retenção de Configurações Anteriores
...
### <a name="baseline-configuration--reviews-and-updates"></a>Configuração de base / Comentários e atualizações
...
### <a name="configuration-change-control--automated-document--notification--prohibition-of-changes"></a>Controlo de Alterações de Configuração / Documento Automatizado / Notificação / Proibição de Alterações
...
### <a name="configuration-change-control--cryptography-management"></a>Controlo de Alterações de Configuração / Gestão da Criptografia
...
### <a name="configuration-change-control--test--validate--document-changes"></a>Controlo de Alterações de Configuração / Testar / Validar / Alterars de Documentos
...
### <a name="information-system-component-inventory--accountability-information"></a>Inventário de Componentes do Sistema de Informação / Informação sobre Prestação de Contas
...
### <a name="information-system-component-inventory--automated-maintenance"></a>Inventário de Componentes do Sistema de Informação / Manutenção Automatizada
...
### <a name="information-system-component-inventory--automated-unauthorized-component-detection"></a>Inventário de Componentes do Sistema de Informação / Deteção automatizada de componentes não autorizados
...
### <a name="information-system-component-inventory--no-duplicate-accounting-of-components"></a>Inventário de Componentes do Sistema de Informação / Sem contabilidade duplicada de componentes
...
### <a name="information-system-component-inventory--updates-during-installations--removals"></a>Inventário de Componentes do Sistema de Informação / Atualizações durante instalações / remoção
...
### <a name="alternate-processing-site--accessibility"></a>Local de Processamento Alternativo / Acessibilidade
...
### <a name="alternate-processing-site--preparation-for-use"></a>Local de Processamento Alternativo / Preparação para utilização
...
### <a name="alternate-processing-site--priority-of-service"></a>Local de Processamento Alternativo / Prioridade do Serviço
...
### <a name="alternate-processing-site--separation-from-primary-site"></a>Local de Processamento Alternativo / Separação do local primário
...
### <a name="alternate-storage-site--accessibility"></a>Local de Armazenamento Alternativo / Acessibilidade
...
### <a name="alternate-storage-site--recovery-time--point-objectives"></a>Local de Armazenamento Alternativo / Tempo de recuperação / Objetivos de Ponto
...
### <a name="alternate-storage-site--separation-from-primary-site"></a>Local de Armazenamento Alternativo / Separação do local primário
...
### <a name="contingency-plan-testing--alternate-processing-site"></a>Teste de Plano de Contingência / Local de processamento alternativo
...
### <a name="contingency-plan-testing--coordinate-with-related-plans"></a>Teste de Plano de Contingência / Coordenar com planos relacionados
...
### <a name="contingency-training--simulated-events"></a>Formação de Contingência / Eventos Simulados
...
### <a name="information-system-backup--separate-storage-for-critical-information"></a>Backup do Sistema de Informação Armazenamento separado para informações críticas
...
### <a name="information-system-backup--test-restoration-using-sampling"></a>Backup do Sistema de Informação Restauro de testes usando amostragem
...
### <a name="information-system-backup--testing-for-reliability--integrity"></a>Backup do Sistema de Informação Teste de Fiabilidade / Integridade
...
### <a name="information-system-backup--transfer-to-alternate-storage-site"></a>Backup do Sistema de Informação Transferência para Local de Armazenamento Alternativo
...
### <a name="information-system-recovery-and-reconstitution--restore-within-time-period"></a>Recuperação e Reconstituição do Sistema de Informação Restaurar dentro do período de tempo
...
### <a name="information-system-recovery-and-reconstitution--transaction-recovery"></a>Recuperação e Reconstituição do Sistema de Informação Recuperação de Transações
...
|Name<br /><sub>(Portal Azure)</sup> |Descrição |Efeito(s) |Versão<br /><sub>(GitHub)</sub> |
|---|---|---|---|---|
|[Implementar pré-requisitos para auditar Os VMs do Linux que não têm as permissões de ficheiros passwd definidas para 0644](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2Ff19aa1c1-6b91-4c27-ae6a-970279f03db9) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais Linux que não têm as permissões de ficheiros passwd definidas para 0644. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.2.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_LinuxPassword121_Deploy.json) |
|[Implementar pré-requisitos para auditar Os VMs do Linux que têm contas sem senhas](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F3470477a-b35a-49db-aca5-1073d04524fe) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais Linux que têm contas sem senhas. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.2.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_LinuxPassword232_Deploy.json) |
|[Implementar pré-requisitos para auditar VMs do Windows que não armazenam palavras-passe usando encriptação reversível](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F8ff0b18b-262e-4512-857a-48ad0aeb9a78) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais do Windows que não armazenam palavras-passe usando encriptação reversível. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.1.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_WindowsPasswordEncryption_Deploy.json) |
...
|[Mostre os resultados da auditoria dos VMs Linux que não têm as permissões de ficheiros passwd definidas para 0644](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2Fb18175dd-c599-4c64-83ba-bb018a06d35b) |Esta política só deve ser utilizada, juntamente com a correspondente política de implantação, numa iniciativa. Esta definição permite que a Azure Policy processe os resultados da auditoria às máquinas virtuais Linux que não têm as permissões de ficheiros passwd definidas para 0644. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |auditIfNotExists |[1.1.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_LinuxPassword121_Audit.json) |
|[Mostrar resultados de auditoria de VMs Linux que têm contas sem senhas](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2Fc40c9087-1981-4e73-9f53-39743eda9d05) |Esta política só deve ser utilizada, juntamente com a correspondente política de implantação, numa iniciativa. Esta definição permite que a Azure Policy processe os resultados da auditoria às máquinas virtuais Linux que têm contas sem senhas. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |auditIfNotExists |[1.1.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_LinuxPassword232_Audit.json) |
|[Mostrar resultados de auditoria de VMs do Windows que não armazenam senhas usando encriptação reversível](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F2d60d3b7-aa10-454c-88a8-de39d99d17c6) |Esta política só deve ser utilizada, juntamente com a correspondente política de implantação, numa iniciativa. Esta definição permite que a Azure Policy processe os resultados da auditoria a máquinas virtuais do Windows que não armazenam palavras-passe utilizando encriptação reversível. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |auditIfNotExists |[1.0.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_WindowsPasswordEncryption_Audit.json) |
...
### <a name="authenticator-management--multiple-information-system-accounts"></a>Gestão de Autenticadores / Contas múltiplas do sistema de informação
...
### <a name="authenticator-management--no-embedded-unencrypted-static-authenticators"></a>Gestão de Autenticadores / Nenhum autenticador estático incorporado
...
|Name<br /><sub>(Portal Azure)</sup> |Descrição |Efeito(s) |Versão<br /><sub>(GitHub)</sub> |
|---|---|---|---|---|
|[Implementar pré-requisitos para auditar VMs do Windows que permitam reutilizar as 24 palavras-passe anteriores](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F726671ac-c4de-4908-8c7d-6043ae62e3b6) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais do Windows que permitem a reutilização das 24 palavras-passe anteriores. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.1.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_WindowsPasswordEnforce_Deploy.json) |
|[Implementar pré-requisitos para auditar VMs do Windows que não tenham uma idade máxima de senha de 70 dias](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F356a906e-05e5-4625-8729-90771e0ee934) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais do Windows que não têm uma idade máxima de senha de 70 dias. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.1.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_WindowsMaximumPassword_Deploy.json) |
|[Implementar pré-requisitos para auditar VMs do Windows que não tenham uma idade mínima de senha de 1 dia](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F16390df4-2f73-4b42-af13-c801066763df) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais do Windows que não têm uma idade mínima de senha de 1 dia. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.1.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_WindowsMinimumPassword_Deploy.json) |
|[Implementar pré-requisitos para auditar VMs do Windows que não tenham a definição de complexidade da palavra-passe ativada](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F7ed40801-8a0f-4ceb-85c0-9fd25c1d61a8) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais do Windows que não têm a definição de complexidade da palavra-passe ativada. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.1.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_WindowsPasswordComplexity_Deploy.json) |
|[Implementar pré-requisitos para auditar VMs do Windows que não restringem o comprimento mínimo da palavra-passe a 14 caracteres](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F23020aa6-1135-4be2-bae2-149982b06eca) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais do Windows que não restringem o comprimento mínimo da palavra-passe a 14 caracteres. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.1.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_WindowsPasswordLength_Deploy.json) |
|[Implementar pré-requisitos para auditar VMs do Windows que não armazenam palavras-passe usando encriptação reversível](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2F8ff0b18b-262e-4512-857a-48ad0aeb9a78) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar máquinas virtuais do Windows que não armazenam palavras-passe usando encriptação reversível. Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.1.0-pré-visualização](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_WindowsPasswordEncryption_Deploy.json) |
...
### <a name="authenticator-management--protection-of-authenticators"></a>Gestão de Autenticadores / Proteção de Autenticadores
...
### <a name="identifier-management--identify-user-status"></a>Gestão de Identificadors Identificar o estado do utilizador
...
### <a name="incident-handling--automated-incident-handling-processes"></a>Tratamento de Incidentes / Processos automatizados de manuseamento de incidentes
...
### <a name="incident-handling--information-correlation"></a>Tratamento de Incidentes / Correlação de informação
...
### <a name="incident-monitoring--automated-tracking--data-collection--analysis"></a>Monitorização de Incidentes Rastreio automatizado / Recolha de Dados / Análise
...
### <a name="incident-reporting--automated-reporting"></a>Reportagem de incidentes [ Relatório Automatizado
...
### <a name="incident-response-assistance--automation-support-for-availability-of-information--support"></a>Assistência de Resposta a Incidentes / Suporte de Automação para Disponibilidade de Informação/ Suporte
...
### <a name="incident-response-assistance--coordination-with-external-providers"></a>Assistência de Resposta a Incidentes / Coordenação com fornecedores externos
...
### <a name="information-spillage-response--exposure-to-unauthorized-personnel"></a>Resposta ao derrame de informação ! Exposição a Pessoal Não Autorizado
...
### <a name="information-spillage-response--post-spill-operations"></a>Resposta ao derrame de informação ! Operações pós-derrame
...
### <a name="information-spillage-response--responsible-personnel"></a>Resposta ao derrame de informação ! Pessoal Responsável
...
### <a name="information-spillage-response--training"></a>Resposta ao derrame de informação ! Formação
...
### <a name="maintenance-personnel--individuals-without-appropriate-access"></a>Pessoal de Manutenção Indivíduos sem acesso adequado
...
### <a name="nonlocal-maintenance--comparable-security--sanitization"></a>Manutenção Não Local Segurança Comparável / Saneamento
...
### <a name="nonlocal-maintenance--document-nonlocal-maintenance"></a>Manutenção Não Local Documento Manutenção Não Local
...
### <a name="media-sanitization--equipment-testing"></a>Saneamento dos media Teste de Equipamentos
...
### <a name="media-sanitization--nondestructive-techniques"></a>Saneamento dos media Técnicas Não Destrutivas
...
### <a name="media-sanitization--review--approve--track--document--verify"></a>Saneamento dos media Revisão / Aprovação / Faixa / Documento / Verificação
...
### <a name="media-transport--cryptographic-protection"></a>Transporte de Meios de Comunicação Social Proteção Criptográfica
...
### <a name="personnel-screening--information-with-special-protection-measures"></a>Rastreio de Pessoal Informação com Medidas Especiais de Proteção
...
### <a name="personnel-termination--automated-notification"></a>Rescisão de Pessoal / Notificação automatizada
...
### <a name="fire-protection--automatic-fire-suppression"></a>Proteção contra incêndios Supressão automática de incêndios
...
### <a name="fire-protection--detection-devices--systems"></a>Proteção contra incêndios Dispositivos de Deteção / Sistemas
...
### <a name="fire-protection--suppression-devices--systems"></a>Proteção contra incêndios Dispositivos /Sistemas de Supressão
...
### <a name="physical-access-control--information-system-access"></a>Controlo de Acesso Físico / Acesso ao Sistema de Informação
...
### <a name="water-damage-protection--automation-support"></a>Proteção contra danos na água Suporte à Automação
...
### <a name="rules-of-behavior--social-media-and-networking-restrictions"></a>Regras de Comportamento Restrições de mídia social e networking
...
### <a name="vulnerability-scanning--automated-trend-analyses"></a>Digitalização de Vulnerabilidades Análises automatizadas de tendências
...
### <a name="vulnerability-scanning--breadth--depth-of-coverage"></a>Digitalização de Vulnerabilidades Largura / Profundidade de Cobertura
...
### <a name="vulnerability-scanning--correlate-scanning-information"></a>Digitalização de Vulnerabilidades Correlacionar informações de digitalização
...
### <a name="vulnerability-scanning--discoverable-information"></a>Digitalização de Vulnerabilidades Informação Descoberta
...
### <a name="vulnerability-scanning--privileged-access"></a>Digitalização de Vulnerabilidades Acesso Privilegiado
...
### <a name="vulnerability-scanning--review-historic-audit-logs"></a>Digitalização de Vulnerabilidades Rever registos de auditoria histórica
...
### <a name="vulnerability-scanning--update-by-frequency--prior-to-new-scan--when-identified"></a>Digitalização de Vulnerabilidades Atualização por Frequência / Antes de Nova Digitalização / Quando Identificado
...
### <a name="vulnerability-scanning--update-tool-capability"></a>Digitalização de Vulnerabilidades Atualizar capacidade de ferramenta
...
### <a name="continuous-monitoring--trend-analyses"></a>Monitorização Contínua Análises de Tendências
...
### <a name="penetration-testing--independent-penetration-agent-or-team"></a>Teste de Penetração Agente de Penetração Independente ou Equipa
...
### <a name="system-interconnections--restrictions-on-external-system-connections"></a>Interligações do sistema Restrições às ligações externas do sistema
...
### <a name="system-interconnections--unclassified-non-national-security-system-connections"></a>Interligações do sistema Ligações não classificadas do sistema de segurança não-nacional
...
### <a name="boundary-protection--access-points"></a>Proteção de Fronteiras Pontos de Acesso
...
### <a name="boundary-protection--deny-by-default--allow-by-exception"></a>Proteção de Fronteiras Negar por Padrão / Permitir por Exceção
...
### <a name="boundary-protection--dynamic-isolation--segregation"></a>Proteção de Fronteiras Isolamento Dinâmico / Segregação
...
### <a name="boundary-protection--external-telecommunications-services"></a>Proteção de Fronteiras Serviços externos de Telecomunicações
...
### <a name="boundary-protection--fail-secure"></a>Proteção de Fronteiras Falha segura
...
### <a name="boundary-protection--isolation-of-information-system-components"></a>Proteção de Fronteiras Isolamento dos Componentes do Sistema de Informação
...
### <a name="boundary-protection--prevent-split-tunneling-for-remote-devices"></a>Proteção de Fronteiras Evitar a separação de túneis para dispositivos remotos
...
### <a name="boundary-protection--prevent-unauthorized-exfiltration"></a>Proteção de Fronteiras Impedir a exfiltração não autorizada
...
### <a name="boundary-protection--route-traffic-to-authenticated-proxy-servers"></a>Proteção de Fronteiras Tráfego de rota para servidores de procuração autenticados
...
|Name<br /><sub>(Portal Azure)</sup> |Descrição |Efeito(s) |Versão<br /><sub>(GitHub)</sub> |
|---|---|---|---|---|
|[A Norma de Proteção DDoS deve ser ativada](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2Fa7aca53f-2ed4-4466-a25e-0b45ade68efd) |A norma de proteção DDoS deve ser ativada para todas as redes virtuais com uma sub-rede que faça parte de um gateway de aplicações com um IP público. |AuditIfNotExists, Desativado |[1.0.0](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Security%20Center/ASC_EnableDDoSProtection_Audit.json) |
...
### <a name="protection-of-information-at-rest--cryptographic-protection"></a>Proteção de Informação em Repouso / Proteção Criptográfica
...
### <a name="session-authenticity--invalidate-session-identifiers-at-logout"></a>Autenticidade da sessão Identificadores de sessão invalidados em Logout
...
### <a name="transmission-confidentiality-and-integrity--cryptographic-or-alternate-physical-protection"></a>Confidencialidade e Integridade da Transmissão Proteção Física Criptográfica ou Alternativa
...
|[Implementar pré-requisitos para auditar servidores web do Windows que não estão a utilizar protocolos de comunicação seguros](https://portal.azure.com/#blade/Microsoft_Azure_Policy/PolicyDetailBlade/definitionId/%2Fproviders%2FMicrosoft.Authorization%2FpolicyDefinitions%2Fb2fc8f91-866d-4434-9089-5ebfe38d6fd8) |Esta política cria uma atribuição de Configuração de Hóspedes para auditar servidores web do Windows que não estão a utilizar protocolos de comunicação seguros (TLS 1.1 ou TLS 1.2). Também cria uma identidade gerida atribuída ao sistema e implementa a extensão VM para configuração de convidados. Esta política só deve ser utilizada, juntamente com a correspondente política de auditoria, numa iniciativa. Para mais informações sobre as políticas de Configuração de Hóspedes, visite[https://aka.ms/gcpol](https://aka.ms/gcpol) |implementarIfNotExists |[1.1.0](https://github.com/Azure/azure-policy/blob/master/built-in-policies/policyDefinitions/Guest%20Configuration/GuestConfiguration_WindowsTLS_Deploy.json) |
...
### <a name="flaw-remediation--automated-flaw-remediation-status"></a>Remediação de falhas Estado de remediação de falhas automatizadas
...
### <a name="flaw-remediation--central-management"></a>Remediação de falhas Gestão Central
...
### <a name="flaw-remediation--time-to-remediate-flaws--benchmarks-for-corrective-actions"></a>Remediação de falhas Tempo para corrigir falhas / benchmarks para ações corretivas
...
### <a name="information-system-monitoring--analyze-traffic--covert-exfiltration"></a>Monitorização do Sistema de Informação Analisar Tráfego / Exfiltração Secreta
...
### <a name="information-system-monitoring--correlate-monitoring-information"></a>Monitorização do Sistema de Informação Correlacionar informações de monitorização
...
### <a name="information-system-monitoring--individuals-posing-greater-risk"></a>Monitorização do Sistema de Informação Indivíduos que representam maior risco
...
### <a name="information-system-monitoring--privileged-users"></a>Monitorização do Sistema de Informação Utilizadores Privilegiados
...
### <a name="information-system-monitoring--unauthorized-network-services"></a>Monitorização do Sistema de Informação Serviços de Rede Não Autorizados
...
### <a name="information-system-monitoring--wireless-intrusion-detection"></a>Monitorização do Sistema de Informação Deteção de intrusões sem fios
...
### <a name="malicious-code-protection--central-management"></a>Proteção de Código Malicioso Gestão Central
...
### <a name="security-alerts-advisories-and-directives--automated-alerts-and-advisories"></a>Alertas de Segurança, Avisos e Diretivas Alertas e Avisos Automatizados
...
### <a name="spam-protection--central-management"></a>Proteção de spam / Gestão Central
...
### <a name="developer-configuration-management--software--firmware-integrity-verification"></a>Gestão de Configuração de Desenvolvedores / Verificação de Integridade do Software / Firmware
...
### <a name="developer-security-testing-and-evaluation--dynamic-code-analysis"></a>Testes e Avaliação de Segurança do Programador Análise dinâmica do código
...
### <a name="developer-security-testing-and-evaluation--static-code-analysis"></a>Testes e Avaliação de Segurança do Programador Análise de código estático
...
### <a name="developer-security-testing-and-evaluation--threat-and-vulnerability-analyses"></a>Testes e Avaliação de Segurança do Programador Análises de Ameaças e Vulnerabilidades
...
### <a name="external-information-system-services--consistent-interests-of-consumers-and-providers"></a>Serviços de Sistema de Informação Externa Interesses consistentes dos consumidores e fornecedores
...
### <a name="external-information-system-services--identification-of-functions--ports--protocols--services"></a>Serviços de Sistema de Informação Externa Identificação de Funções / Portos / Protocolos / Serviços
...
### <a name="external-information-system-services--processing-storage-and-service-location"></a>Serviços de Sistema de Informação Externa Processamento, Armazenamento e Localização de Serviço
...
### <a name="external-information-system-services--risk-assessments--organizational-approvals"></a>Serviços de Sistema de Informação Externa Avaliações de Riscos / Aprovações Organizacionais
