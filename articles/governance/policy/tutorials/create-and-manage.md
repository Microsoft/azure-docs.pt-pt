...
O primeiro passo para impor a conformidade com o Azure Policy consiste em atribuir uma definição de política. Uma definição de política define sob que condição é imposta uma política e qual o efeito a ter. Neste exemplo, atribuir a definição de política incorporada chamada _Herdar uma etiqueta do grupo de recursos se faltar_ para adicionar a etiqueta especificada com o seu valor do grupo de recursos-mãe a recursos novos ou atualizados que faltam à etiqueta.

1. Vá ao portal Azure para atribuir políticas. Procure e selecione **Política**.

   :::image type="content" source="../media/create-and-manage/search-policy.png" alt-text="Pesquisar por Política na barra de pesquisa" border="false":::
...
1. Selecione **Atribuições** no lado esquerdo da página Azure Policy. Uma atribuição é uma política que foi atribuída para ter lugar num âmbito específico.
   ...
   :::image type="content" source="../media/create-and-manage/select-assignments.png" alt-text="Selecione Atribuições da página de visão geral da política" border="false":::

1. Selecione **Atribuir Política** na parte superior da página **Política - Atribuições**.

   :::image type="content" source="../media/create-and-manage/select-assign-policy.png" alt-text="Atribuir uma definição de política a partir da página de Atribuições" border="false":::
...
1. Selecione **Herdar uma etiqueta do grupo de recursos se faltar**. Se não conseguir encontrá-la imediatamente, escreva **uma etiqueta** na caixa de pesquisa e, em seguida, prima ENTER ou selecione fora da caixa de pesquisa.
   **Selecione Selecione** na parte inferior da página **Definições Disponíveis** uma vez encontrada e selecionada a definição de política.
   ...
   :::image type="content" source="../media/create-and-manage/select-available-definition.png" alt-text="Utilize filtro de pesquisa para localizar uma política":::
...
1. **Atribuído por é** automaticamente preenchido com base em quem está registado. Este campo é opcional e, por isso, podem ser introduzidos valores personalizados.

1. Selecione o separador **Parâmetros** na parte superior do assistente.

1. Para **etiquetas,** insira _Ambiente_.
...
1. Introduza as seguintes informações:
   ...
   Para ver mais amostras da Política Azure, consulte [amostras da Política Azure.](../samples/index.md)
...
Pode criar uma definição de política `az policy definition` utilizando o Azure CLI com o comando. Para criar uma definição de política com uma regra na mesma linha, utilize o seguinte exemplo:
...
1. Selecione **Definições** ao abrigo **da autoria** no lado esquerdo da página Política Azure.
   ...
   :::image type="content" source="../media/create-and-manage/definition-under-authoring.png" alt-text="Selecione definição a partir da página Definições" border="false":::

1. Selecione **+ Definição de Iniciativa** na parte superior da página para abrir a página **Definição de iniciativa**.

   :::image type="content" source="../media/create-and-manage/initiative-definition.png" alt-text="Rever página de definição de iniciativa" border="false":::
...
1. Introduza o **Nome** e a **Descrição** da iniciativa.
   ...
   Este exemplo valida que os recursos estão em conformidade com as definições políticas de segurança. O nome da iniciativa deverá ser **Proteger-se** e a definição da descrição: **Esta iniciativa foi criada para processar todas as definições de política associadas à proteção de recursos**.

1. Em **Categoria**, escolha de entre as opções existentes ou crie uma nova categoria.

1. Procure na lista de **Definições Disponíveis** (lado direito da página **Definição de iniciativa**) e selecione as definições de política que gostaria de adicionar a esta iniciativa. Para a iniciativa **Get Secure,** adicione as seguintes definições políticas incorporadas selecionando a **+** seguinte informação de definição de política ou selecionando uma linha de definição de política e, em seguida, a opção + **Adicionar** na página de detalhes:
   ...
   :::image type="content" source="../media/create-and-manage/initiative-definition-2.png" alt-text="Rever os parâmetros de definição de iniciativa" border="false":::
...
1. Se uma definição de política adicionada à iniciativa tiver parâmetros, eles são mostrados sob o nome de política na área na área de **categoria.** O _valor_ pode ser definido como "Definir o valor' (hard-coded para todas as atribuições dessa iniciativa) ou "Usar parâmetro de iniciativa" (definido durante cada atribuição de iniciativa). Se for selecionado o 'Valor definido', a descida ao direito de _Valor_ permite entrar ou selecionar o(s valor). Se “Usar Parâmetro de Iniciativa” estiver selecionado, será apresentada uma nova secção **Parâmetros de iniciativa** que lhe permite definir o parâmetro que será definido durante a atribuição de iniciativa. Os valores permitidos neste parâmetro de iniciativa podem restringir mais o que pode ser definido durante a atribuição de iniciativa.
   ...
   :::image type="content" source="../media/create-and-manage/initiative-definition-3.png" alt-text="Alterar parâmetros de definição de iniciativa dos valores permitidos" border="false":::

   > [!NOTE]
   > No caso de alguns parâmetros `strongType`, a lista de valores não pode ser determinada automaticamente. Nestes casos, um botão de reticências será apresentado à direita da linha do parâmetro. Selecionando-o abre a página&lt;'Parâmetro&gt;(nome do parâmetro)'. Nesta página, selecione a subscrição a utilizar para fornecer as opções de valor. Este âmbito de parâmetro só é utilizado durante a criação da definição de iniciativa e não tem qualquer impacto na avaliação de políticas nem no âmbito da iniciativa quando atribuído.
...
Pode criar uma definição de iniciativa política `az policy set-definition` utilizando o Azure CLI com o comando. Para criar uma definição de iniciativa política com uma definição de política existente, use o seguinte exemplo:
...
Pode criar uma definição de iniciativa política `New-AzPolicySetDefinition` utilizando o Azure PowerShell com o cmdlet. Para criar uma definição de iniciativa política com uma definição `VMPolicySet.json`de política existente, utilize o seguinte ficheiro de definição de iniciativa política como:
...
1. Localize a definição de iniciativa **Proteger-se** que criou anteriormente e selecione-a. Selecione **Atribuir** na parte superior da página para abrir a página **Proteger-se: Atribuir iniciativa**.
   ...
   :::image type="content" source="../media/create-and-manage/assign-definition.png" alt-text="Atribuir uma definição da página de definição de Iniciativa" border="false":::
...
1. Localize a iniciativa **Get Secure.** Provavelmente ainda está em _conformidade estado_ de **não começou.**
   ...
   :::image type="content" source="../media/create-and-manage/compliance-status-not-started.png" alt-text="Página de conformidade de iniciativa - avaliações não iniciadas" border="false":::
...
1. Depois de concluir a atribuição da iniciativa, a página de conformidade é atualizada com o _Estado de conformidade_ de **Conforme**.
   ...
   :::image type="content" source="../media/create-and-manage/compliance-status-compliant.png" alt-text="Página de conformidade de iniciativa- recursos conformes" border="false":::
...
:::image type="content" source="../media/create-and-manage/rg-deployment-denied.png" alt-text="Implementação negada pela atribuição de política" border="false":::
...
:::image type="content" source="../media/create-and-manage/compliance-overview.png" alt-text="Descrição geral de conformidade de uma política atribuída" border="false":::
...
1. Defino a **Exclusão** selecionando a elipse e selecionando o grupo de recursos para excluir, _Localizações Excluídas_ neste exemplo. **Selecione Adicionar ao Âmbito Selecionado** e, em seguida, selecione **Save**.
   ...
   :::image type="content" source="../media/create-and-manage/request-exclusion.png" alt-text="Adicione um grupo de recursos excluído sairá à atribuição de políticas" border="false":::
