...
description: Mapeamento de controle da amostra do CSCF v2020 Blueprint do SWIFT CSP para Azure Policy.
author: DCtheGeek
ms.author: dacoulte
ms.date: 09/24/2019
ms.topic: sample
...
O artigo a seguir fornece detalhes sobre como o exemplo do Azure Blueprints SWIFT CSP – CSCF v2020 Blueprint mapeia para o CSP SWIFT-CSCF v2020 Controls. Para obter mais informações sobre os controles, consulte [Swift CSP-CSCF v2020](https://www.swift.com/myswift/customer-security-programme-csp).

Os seguintes mapeamentos são para os controles **CSCF v2020 do CSP Swift** . Use a navegação à direita para ir diretamente para um mapeamento de controle específico. Muitos dos controles mapeados são implementados com uma iniciativa de [Azure Policy](../../../policy/overview.md) . Para examinar a iniciativa completa, abra a **política** no portal do Azure e selecione a página **definições** . Em seguida, localize e selecione o ** @ no__t-1Preview @ no__t-2: Audit SWIFT CSP – CSCF v2020 Controls e implante extensões de VM específicas para dar suporte aos requisitos de auditoria @ no__t-0 iniciativa de política interna.
...
## <a name="12-and-51-account-management"></a>Gerenciamento de conta de 1,2 e 5,1
...
## <a name="26-51-64-and-65a-account-management--role-based-schemes"></a>2,6, 5,1, 6,4 e 6.5 um gerenciamento de conta | Esquemas baseados em função
...
## <a name="29a--account-management--account-monitoring--atypical-usage"></a>2.9 a gerenciamento de contas | Monitoramento de conta/uso de atípicos
...
## <a name="13-51-and-64-separation-of-duties"></a>1,3, 5,1 e 6,4 separação de tarefas
...
- Mostrar resultados de auditoria de VMs do Windows nas quais o grupo de administradores não contém todos os membros especificados
- Implantar pré-requisitos para auditar VMs do Windows nas quais o grupo de administradores não contém todos os membros especificados
- Deve haver mais de um proprietário atribuído à sua assinatura

## <a name="13-51-and-64-least-privilege--review-of-user-privileges"></a>1,3, 5,1 e 6,4 privilégios mínimos | Revisão de privilégios de usuário
...
## <a name="22-27-41-and-61-remote-access--automated-monitoring--control"></a>2,2, 2,7, 4,1 e 6,1 acesso remoto | Monitoramento/controle automatizado
...
- \[Preview @ no__t-1: Mostrar resultados de auditoria de VMs Linux que permitem conexões remotas de contas sem senhas
- \[Preview @ no__t-1: Implantar pré-requisitos para auditar VMs Linux que permitem conexões remotas de contas sem senhas
...
## <a name="13-and-64-content-of-audit-records--centralized-management-of-planned-audit-record-content"></a>Conteúdo de 1,3 e 6,4 de registros de auditoria | Gerenciamento centralizado de conteúdo de registro de auditoria planejado
...
- \[Preview @ no__t-1: Implantar o agente de Log Analytics para conjuntos de dimensionamento de VM Linux (VMSS)
- \[Preview @ no__t-1: Implantar o agente de Log Analytics para VMs Linux
- \[Preview @ no__t-1: Implantar o agente de Log Analytics para VMSS (conjuntos de dimensionamento de VMs) do Windows
...
## <a name="13-and-64-audit-review-analysis-and-reporting--central-review-and-analysis"></a>Revisão, análise e relatórios de auditoria 1,3 e 6,4 | Revisão e análise central
...
## <a name="11-least-functionality--prevent-program-execution"></a>Funcionalidade mínima de 1,1 | Impedir a execução do programa
...
## <a name="11-least-functionality--authorized-software--whitelisting"></a>Funcionalidade mínima de 1,1 | Software autorizado/lista de permissões
...
## <a name="23-and-41-authenticator-management"></a>Gerenciamento de autenticador 2,3 e 4,1
...
- \[Preview @ no__t-1: Mostrar resultados de auditoria de VMs Linux que não têm as permissões de arquivo passwd definidas como 0644
...
- \[Preview @ no__t-1: Mostrar resultados de auditoria de VMs do Linux que têm contas sem senhas
- \[Preview @ no__t-1: Implantar requisitos para auditar VMs do Linux que têm contas sem senhas
- \[Preview @ no__t-1: Mostrar resultados de auditoria de VMs do Windows que não armazenam senhas usando criptografia reversível
- \[Preview @ no__t-1: Implantar requisitos para auditar VMs do Windows que não armazenam senhas usando criptografia reversível

## <a name="23-and-41-authenticator-management--password-based-authentication"></a>Gerenciamento de autenticador 2,3 e 4,1 | Autenticação baseada em senha
...
- \[Preview @ no__t-1: Mostrar resultados de auditoria de VMs do Windows que permitem reutilização das 24 senhas anteriores
- \[Preview @ no__t-1: Mostrar resultados de auditoria de VMs do Windows que não têm uma duração máxima de senha de 70 dias
- \[Preview @ no__t-1: Mostrar resultados de auditoria de VMs do Windows que não têm uma duração mínima de senha de 1 dia
- \[Preview @ no__t-1: Mostrar resultados de auditoria de VMs do Windows que não têm a configuração de complexidade de senha habilitada
- \[Preview @ no__t-1: Mostrar resultados de auditoria de VMs do Windows que não restringem o comprimento mínimo da senha para 14 caracteres
- \[Preview @ no__t-1: Mostrar resultados de auditoria de VMs do Windows que não armazenam senhas usando criptografia reversível
- \[Preview @ no__t-1: Implantar pré-requisitos para auditar VMs do Windows que permitem o reuso das 24 senhas anteriores
- \[Preview @ no__t-1: Implantar pré-requisitos para auditar VMs do Windows que não têm uma duração máxima de senha de 70 dias
- \[Preview @ no__t-1: Implantar pré-requisitos para auditar VMs do Windows que não têm uma duração mínima de senha de 1 dia
- \[Preview @ no__t-1: Implantar pré-requisitos para auditar VMs do Windows que não têm a configuração de complexidade de senha habilitada
- \[Preview @ no__t-1: Implantar pré-requisitos para auditar VMs do Windows que não restringem o comprimento mínimo da senha a 14 caracteres
- \[Preview @ no__t-1: Implantar pré-requisitos para auditar VMs do Windows que não armazenam senhas usando criptografia reversível

## <a name="22-and-27-vulnerability-scanning"></a>Verificação de vulnerabilidades 2,2 e 2,7
...
## <a name="11-and-61-boundary-protection"></a>Proteção de limite 1,1 e 6,1
...
## <a name="29a-boundary-protection--access-points"></a>2.9 uma proteção de limite | Pontos de acesso
...
## <a name="29a-boundary-protection--external-telecommunications-services"></a>2.9 uma proteção de limite | Serviços de telecomunicações externos
...
## <a name="21-24-24a-25a-and-26-transmission-confidentiality-and-integrity--cryptographic-or-alternate-physical-protection"></a>2,1, 2,4, 2.4 a, 2,5 A e 2,6 integridade e confidencialidade de transmissão | Proteção física criptográfica ou alternativa
...
## <a name="13-22-and-27-flaw-remediation"></a>1,3, 2,2 e 2,7 falha na correção
...
## <a name="22-and-28-information-system-monitoring--analyze-traffic--covert-exfiltration"></a>Monitoramento do sistema de informações 2,2 e 2,8 | Analisar o tráfego/cobrir vazamento
...
> O [CSP do Swift-CSCF v2020 Blueprint-visão geral](./index.md)
> [Swift CSP-CSCF V2020 Blueprint – implantar etapas](./deploy.md)
