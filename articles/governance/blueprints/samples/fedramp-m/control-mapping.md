...
O artigo a seguir fornece detalhes sobre como o exemplo do plano gráfico moderado do FedRAMP do Azure é mapeado para os controles moderados do FedRAMP. Para obter mais informações sobre os controles, consulte [linha de base de controles de segurança FedRAMP](https://www.fedramp.gov/).

Os mapeamentos a seguir são para os controles **moderados do FedRAMP** . Use a navegação à direita para ir diretamente para um mapeamento de controle específico. Muitos dos controles mapeados são implementados com uma iniciativa de [Azure Policy](../../../policy/overview.md) . Para examinar a iniciativa completa, abra a **política** no portal do Azure e selecione a página **definições** . Em seguida, localize e selecione a **\[visualização\]: auditar controles moderados do FedRAMP e implantar extensões de VM específicas para dar suporte à** iniciativa de política interna de requisitos de auditoria.
