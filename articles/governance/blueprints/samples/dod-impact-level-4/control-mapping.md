...
> Para ver a história da mudança, consulte o [GitHub Commit History](https://github.com/MicrosoftDocs/azure-docs/commits/master/articles/governance/blueprints/samples/dod-impact-level-4/control-mapping.md).
...
Este plano fornece definições de políticas que o ajudam a garantir que as notificações de segurança de dados estão devidamente ativadas. Além disso, esta planta garante que a auditoria e a segurança avançada dos dados estão configuradas nos Servidores SQL.
...
## <a name="ir-6-2-incident-reporting--vulnerabilities-related-to-incidents"></a>IR-6 (2) Reportagem de Incidentes [ Vulnerabilidades Relacionadas com Incidentes

Esta planta fornece definições de políticas que auditam registos com análise de avaliação de vulnerabilidade em máquinas virtuais, conjuntos de escala de máquinas virtuais e servidores SQL. Estes insights fornecem informações em tempo real sobre o estado de segurança dos seus recursos implantados e podem ajudá-lo a priorizar ações de reparação.
...
## <a name="si-02-06-flaw-remediation--removal-of-previous-versions-of-software--firmware"></a>SI-02 (06) Reparação de Falhas / Remoção de versões anteriores de Software / Firmware

Este projeto atribui definições de políticas que o ajudam a garantir que as aplicações estão a usar a versão mais recente do .NET Framework, HTTP, Java, PHP, Python e TLS. Este projeto também atribui uma definição de política que garante que os Serviços Kubernetes são atualizados para a sua versão não vulnerável.
...
- \[Pré-visualização\]: Os serviços da Kubernetes devem ser atualizados para uma versão Kubernetes não vulnerável
...
Este plano fornece definições de políticas que o ajudam a garantir que as notificações de segurança de dados estão devidamente ativadas. Além disso, esta planta garante que o nível de preços padrão está ativado para o Azure Security Center. Note que o nível padrão de preços permite a deteção de ameaças para redes e máquinas virtuais, fornecendo inteligência de ameaça, deteção de anomalias e análise de comportamento no Azure Security Center.
...
> [do plano de impacto do DoD Nível 4 - Desloque passos](./deploy.md)
