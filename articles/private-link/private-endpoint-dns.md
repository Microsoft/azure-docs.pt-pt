...
| Tipo de recurso de ligação privada / Subrecurso |Nome da Zona DNS Privada | Nome da zona DNS pública |
|---|---|---|---|
...
| Cofre de Chaves Azure (Microsoft.KeyVault/cofres) / cofre | privatelink.vaultcore.azure.net | vault.azure.net |
...
- [Nas instalações, a carga de trabalho utilizando um avançado do DNS](#on-premises-workloads-using-a-dns-forwarder)
...
## <a name="on-premises-workloads-using-a-dns-forwarder"></a>Nas instalações, a carga de trabalho utilizando um avançado do DNS
 
Para que, no local, as cargas de trabalho possam resolver um FQDN de um ponto final privado no endereço IP privado, deve utilizar um reencaminhador DNS para efetuar a resolução da zona pública de [DNS](#azure-services-dns-zone-configuration) de serviço Azure implantada em Azure.


O seguinte cenário é apropriado para uma rede de instalações que tenha um avançado dNS em Azure, que por sua vez é responsável pela resolução de todas as consultas de DNS através de um forwardr de nível de servidor para o Azure fornecido DNS [168.63.129.16](../virtual-network/what-is-ip-address-168-63-129-16.md) 
...
O diagrama seguinte ilustra a sequência de resolução dNS de uma rede de instalações que utiliza um reencaminhador DNS implantado em Azure, onde a resolução é feita por uma zona privada de DNS ligada a uma rede virtual.

:::image type="content" source="media/private-endpoint-dns/on-premise-using-azure-dns.png" alt-text="No local usando DNS Azure":::

Esta configuração pode ser estendida para uma rede de instalações que já tem uma solução DNS no lugar. 
...
- Na rede de instalações com uma solução DNS personalizada no lugar 
- Rede virtual [ligada às instalações](https://docs.microsoft.com/azure/architecture/reference-architectures/hybrid-networking/)
- Avançado do DNS implantado em Azure
- Zonas privadas de DNS [privatelink.database.windows.net](../dns/private-dns-privatednszone.md)  com [registo tipo A](../dns/dns-zones-records.md#record-types)
- Informações de ponto final privado (nome de registo FQDN e endereço IP privado)

O diagrama seguinte ilustra a sequência de resolução dNS de uma rede de premissa que encaminha condicionalmente o tráfego de DNS para Azure, onde a resolução é feita por uma zona privada de DNS ligada a uma rede virtual

> [!IMPORTANT]
> O encaminhamento condicional deve ser feito para `database.windows.net` a [zona](#azure-services-dns-zone-configuration) pública do DNS Ex: , em vez de **privatelink**.database.windows.net

:::image type="content" source="media/private-endpoint-dns/on-premise-forwarding-to-azure.png" alt-text="No local encaminhamento para O DNS Azure":::
