...
2. Se você não usar uma conta de domínio, desabilite o controle de acesso de usuário remoto no computador local da seguinte maneira:
    - Em chave do registro HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System, adicione um novo DWORD: **LocalAccountTokenFilterPolicy**. Defina o valor como **1**.
...
5. Abra **cspsconfigtool.exe**. Ele está disponível como um atalho na área de trabalho e na pasta%ProgramData%\home\svsystems\bin.
...
4. Habilite a autenticação de subsistema e senha do SFTP no arquivo sshd_config. Para fazer isso, entre como **raiz**.
