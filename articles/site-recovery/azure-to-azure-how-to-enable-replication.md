...
Habilite a replicação. Este procedimento pressupõe que a região primária do Azure está Ásia Oriental e a região secundária é Ásia Oriental do Sul.

1. No cofre, clique em **+ replicar**.
2. Observe os seguintes campos:
   - **Origem**: O ponto de origem das VMs, que neste caso é o **Azure**.
   - **Local de origem**: A região do Azure da qual você deseja proteger suas VMs. Para esta ilustração, o local de origem é ' Ásia Oriental '
   ...
   - **Grupo de recursos**: O grupo de recursos ao qual as máquinas virtuais de origem pertencem. Todas as VMs no grupo de recursos selecionado são listadas para proteção na próxima etapa.
     ...
     ![Ativar replicação](./media/site-recovery-replicate-azure-to-azure/enabledrwizard1.png)
...
4. Em **configurações**, você pode opcionalmente definir as configurações de site de destino:
   ...
   - **Assinatura de destino**: A assinatura de destino usada para recuperação de desastre. Por predefinição, a subscrição de destino será igual à subscrição de origem.
...
2. Clique em **Personalizar:** para modificar as configurações padrão:
    ...
    - Em **contas de armazenamento de destino**, selecione a conta que você deseja usar.
        ...
        ![Ativar replicação](./media/site-recovery-replicate-azure-to-azure/customize.PNG)
