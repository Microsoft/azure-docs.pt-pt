...
2. Na tela **failover** , selecione um **ponto de recuperação** para o qual fazer failover. Pode utilizar uma das opções seguintes:
   ...
   1. **Várias VMs processadas mais recentemente**: Essa opção só está disponível para planos de recuperação que têm pelo menos uma máquina virtual com consistência de várias VMs no. Máquinas virtuais que fazem parte de um failover de grupo de replicação para o ponto de recuperação consistente de várias VMs comum mais recente. Outros failover de máquinas virtuais para o último ponto de recuperação processado.  
   1. **Várias VMs mais recentes consistentes com o aplicativo**: Essa opção só está disponível para planos de recuperação que têm pelo menos uma máquina virtual com consistência de várias VMs no. Máquinas virtuais que fazem parte de um failover de grupo de replicação para o ponto de recuperação mais recente consistente com o aplicativo de várias VMs comum. Outros failover de máquinas virtuais para seu ponto de recuperação consistente com o aplicativo mais recente.
   1. **Personalizado**: Se você estiver fazendo failover de teste de uma máquina virtual, poderá usar essa opção para fazer failover em um ponto de recuperação específico.
...
1. Pode seguir o progresso da ativação pós-falha na página **Trabalhos**. Mesmo se ocorrerem erros durante um failover não planejado, o plano de recuperação será executado até ser concluído.
...
1. Verificação de pré-requisitos: Esta etapa garante que todas as condições necessárias para o failover sejam atendidas
1. Pós-falha Essa etapa processa os dados e os torna prontos para que uma máquina virtual do Azure possa ser criada fora dele. Se você tiver escolhido o ponto de recuperação **mais recente** , essa etapa criará um ponto de recuperação a partir dos dados que foram enviados para o serviço.
1. Iniciar: Esta etapa cria uma máquina virtual do Azure usando os dados processados na etapa anterior.

> [!WARNING]
> **Não cancelar um failover em andamento**: Antes do failover ser iniciado, a replicação da máquina virtual é interrompida. Se você **Cancelar** um trabalho em andamento, o failover será interrompido, mas a máquina virtual não começará a ser replicada. A replicação não pode ser iniciada novamente.
>
>
