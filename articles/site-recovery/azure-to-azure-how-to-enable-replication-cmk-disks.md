...
4. Em **configurações**, você pode definir as seguintes configurações de site de destino.
    ...
    - **Grupo de recursos de destino**: o grupo de recursos ao qual todas as máquinas virtuais replicadas pertencem. Por padrão, Site Recovery cria um novo grupo de recursos na região de destino. O nome Obtém o sufixo de `asr`. Se já existir um grupo de recursos que foi criado por Azure Site Recovery, ele será reutilizado. Você também pode optar por personalizá-lo, conforme mostrado na seção a seguir. O local do grupo de recursos de destino pode ser qualquer região do Azure, exceto a região onde as máquinas virtuais de origem estão hospedadas.
