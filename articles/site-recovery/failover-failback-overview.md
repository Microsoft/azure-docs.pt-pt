...
- **Estágio 1: fazer failover do local**: depois de configurar a replicação para o Azure para computadores locais, quando o site local ficar inativo, você falhará nessas máquinas no Azure. Após o failover, as VMs do Azure são criadas a partir de dados replicados.
...
**Ativação pós-falha** | **Detalhes** | **Automatiza** | **Modelo**
--- | --- | --- | ---
...
- Durante o failover planejado, você pode selecionar opções para sincronizar os dados antes do failback:
    ...
    - **Sincronizar dados somente durante o failover**: essa opção é mais rápida porque esperamos que a maior parte do disco tenha sido alterada e, portanto, não execute cálculos de soma de verificação. Ele realiza um download do disco. Recomendamos que você use essa opção se a VM estiver em execução no Azure por um tempo (um mês ou mais) ou se a VM local tiver sido excluída.  
