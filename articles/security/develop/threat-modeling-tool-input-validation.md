...
| **Passos** | XSLT suporta scripting dentro de folhas de estilo usando o `<msxml:script>` elemento. Isto permite que as funções personalizadas sejam usadas numa transformação XSLT. O guião é executado no contexto do processo que realiza a transformação. O script XSLT deve ser desativado quando num ambiente não confiável para impedir a execução de código não confiável. *Se utilizar .NET:* A escrita XSLT é desativada por padrão; no entanto, deve garantir que não foi explicitamente ativado através da `XsltSettings.EnableScript` propriedade.|
...
XmlReader reader = XmlReader.Create(stream, settings);
```
Note que o valor padrão de `ProhibitDtd` in `XmlReaderSettings` é verdadeiro, mas nele é `XmlTextReader` falso. Se estiver a utilizar o XmlReaderSettings, não precisa de configurar o ProhibitDtd de forma verdadeira mente, mas é recomendado por razões de segurança que o faça. Note também que a classe XmlDocument permite a resolução da entidade por defeito. 

### <a name="example"></a>Exemplo
Para desativar a resolução da entidade para a XmlDocuments, utilize a `XmlDocument.Load(XmlReader)` sobrecarga do método load e detete as propriedades apropriadas no argumento XmlReader para desativar a resolução, como ilustrado no seguinte código: 
...
| **Tecnologias Aplicáveis** | Genérica |
| **Atributos**              | N/D  |
| **Referências**              | Upload de [ficheiro sem restrições,](https://owasp.org/www-community/vulnerabilities/Unrestricted_File_Upload) [tabela de assinatura de ficheiros](https://www.garykessler.net/library/file_sigs.html) |
...
$('body').append(resHTML);   
```
Não `innerHtml` utilize; em vez disso, use `innerText` . Da mesma forma, em vez de `$("#elm").html()` , usar`$("#elm").text()` 
...
| **Atributos**              | N/D  |
| **Referências**              | N/D  |
| Passo | ASP.NET WebPages (Razor) executam a codificação automática de HTML. Todas as cordas impressas por nuggets de código incorporados (@ blocos) são automaticamente codificadas por HTML. No entanto, quando o `HtmlHelper.Raw` Método é invocado, devolve a marcação que não é codificada por HTML. Se for utilizado o método do `Html.Raw()` ajudante, contorna a proteção automática de codificação que a Razor fornece.|
