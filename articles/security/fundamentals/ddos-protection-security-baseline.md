...
Esta linha de base de segurança aplica orientações da [versão 1.0](../benchmarks/overview-v1.md) do Benchmark de Segurança Azure para a norma Azure DDoS Protection. A Referência de Segurança do Azure disponibiliza recomendações para proteger as suas soluções cloud no Azure. O conteúdo é agrupado pelos controlos de **segurança definidos** pelo Azure Security Benchmark e pela orientação conexa aplicável à norma Azure DDoS Protection. **Foram excluídos os controlos** não aplicáveis à norma Azure DDoS Protection, ou para os quais a responsabilidade é da Microsoft.

Para ver como o Azure DDoS Protection padrão mapeia completamente para o Azure Security Benchmark, consulte o [ficheiro de mapeamento de base de segurança padrão da Proteção Azure DDoS](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
- [Informações sobre como aceder à telemetria, registos e análise de ataque para o serviço DDoS Protection Standard](/azure/ddos-protection/telemetry-monitoring-alerting#configure-alerts-for-ddos-protection-metrics)
...
Além disso, o Azure Ative Directory (Azure AD) tem funções incorporadas que devem ser explicitamente atribuídas e são questionáveis. Utilize o módulo Azure AD PowerShell para realizar consultas ad hoc para descobrir contas que são membros de grupos administrativos.
...
- [Como permitir a autenticação multi-factor do Diretório Ativo Azure](../../active-directory/authentication/howto-mfa-getstarted.md)
...
**Orientação**: Utilize uma estação de trabalho segura gerida pelo Azure com a autenticação multi-factor do Diretório Ativo Azure, habilitada a iniciar sessão e configurar os seus pedidos de Bloqueio de ClienteS Azure.

- [Implementar uma estação de trabalho segura e gerida pelo Azure](/security/compass/overview)

- [Planejando uma implementação de autenticação multi-factor do Diretório Ativo Azure baseada na nuvem](../../active-directory/authentication/howto-mfa-getstarted.md)
...
**Orientação**: Utilize localizações nomeadas a Azure Ative (Azure AD) para permitir o acesso ao portal Azure a partir de agrupamentos lógicos específicos de intervalos de endereços IP ou países/regiões.
...
- [Gerir a Azure RBAC em Azure DDoS Protection](/azure/virtual-network/manage-ddos-protection)
