...
ms.locfileid: "68610577"
---
# <a name="quickstart-encrypt-a-windows-iaas-vm-with-azure-powershell"></a>Início rápido: Criptografar uma VM IaaS do Windows com Azure PowerShell
...
- Instalar ou atualizar para a [versão mais recente do módulo Azure PowerShell](/powershell/azure/install-az-ps)
    - A versão do módulo AZ precisa ser a 1.0.0 ou superior. Use `Get-Module Az -ListAvailable | Select-Object -Property Name,Version,Path` para verificar a versão.
...
1. **No administrador: ISE do Windows PowerShell** janela, clique em **Exibir** e, em seguida, clique em **Mostrar painel de script**.
...
1. **No administrador: ISE do Windows PowerShell** janela, clique em **arquivo** e em **abrir**. Navegue para o ficheiro **ADEPrereqScript.ps1** e faça duplo clique no mesmo. O script irá abrir num painel de scripts.
...
![Script de pré-requisitos do Azure Disk Encryption em execução no ISE do PowerShell](./media/quick-encrypt-vm-powershell/ade-prereq-script.PNG)
...
1. Copie o script no **administrador: ISE do Windows PowerShell** painel de script e altere as três principais variáveis. O grupo de recursos e a localização têm de ser os mesmos que utilizou no [script de pré-requisitos](#bkmk_PrereqScript).  
...
Agora que você criou e configurou um cofre de chaves e uma VM, você pode criptografar o disco com o cmdlet **set-AzVmDiskEncryptionExtension** . 
...
1. Quando a encriptação estiver concluída, pode verificar que o disco está encriptado com o seguinte cmdlet: 
     ...
     ```
    ![Saída Get-AzVmDiskEncryptionStatus](./media/quick-encrypt-vm-powershell/ade-get-encryption-status.PNG)
