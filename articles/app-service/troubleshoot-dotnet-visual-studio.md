...
1. [Coloque um ponto de rutura](https://docs.microsoft.com/visualstudio/debugger/) na `ViewBag.Message` linha.
...
1. No **Server Explorer,** clique com o botão direito na sua aplicação e, em seguida, clique **em Anexar Debugger**.
    ...
    ![Anexar depurar](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-attachdebugger.png)
...
3. [Desaponte um ponto de rutura](https://docs.microsoft.com/visualstudio/debugger/) na primeira declaração do `GnerateThumbnail` método.
...
5. Na **Profile** lista de drop-down profile, selecione o mesmo perfil que usou em [Get Started com o Azure WebJobs SDK](https://github.com/Azure/azure-webjobs-sdk/wiki).
...
8. Clique **em Anexar Debugger**.
    ...
    ![Anexar depurar](./media/web-sites-dotnet-troubleshoot-visual-studio/wjattach.png)
...
* Certifique-se de que `debug` o atributo do elemento no ficheiro `compilation` *Web.config* está definido como verdadeiro. É definido como verdadeiro por padrão quando publica uma configuração de construção de depuração.
    ...
    ```
* Se descobrir que o depurador não entra no código que pretende depurar, poderá ter de alterar a definição Just My Code.  Para obter mais informações, consulte [Especificar se deve depurar apenas o código do utilizador utilizando o Just My Code in Visual Studio](https://docs.microsoft.com/visualstudio/debugger/just-my-code).
...
1. *Abra os controladores\HomeController.cs*, e substitua os `Index` `About` métodos , e `Contact` métodos pelo seguinte código, a fim de adicionar `Trace` declarações e uma declaração `using` `System.Diagnostics` para:
...
1. Adicione um <a href="https://msdn.microsoft.com/library/vstudio/6915t83k(v=vs.100).aspx">elemento de rastreio</a> no ficheiro `<system.web>` Web.config, tal como o seguinte exemplo:
...
![Regista botões de separador](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-icons.png)
...
Quaisquer registos que possa monitorizar na janela **Saída** também podem ser descarregados como ficheiro *.zip.*

1. Na janela **saída,** clique em **Baixar Registos de Streaming**.

    ![Regista botões de separador](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-downloadicon.png)
...
Pode ativar o rastreio de pedidos falhados utilizando o Visual Studio, mas não pode vê-los no Visual Studio. Estes registos são ficheiros XML. O serviço de registo de streaming apenas monitoriza ficheiros que são considerados legíveis em modo de texto simples: *.txt*, *.html*e *ficheiros .log.*
