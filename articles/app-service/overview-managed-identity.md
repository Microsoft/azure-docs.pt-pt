...
1. Se estiver a utilizar a CLI do Azure numa consola local, primeiro inicie sessão no Azure com [az login](/cli/azure/reference-index#az_login). Utilize uma conta associada à subscrição Azure sob a qual pretende implementar a aplicação:
...
2. Crie uma aplicação de função utilizando o Azure PowerShell. Para obter mais exemplos de como utilizar a Azure PowerShell com Funções Azure, consulte a [referência Az.Functions](/powershell/module/az.functions/#functions):
...
- [Aceda a Azure Storage de forma segura usando uma identidade gerida](scenario-secure-app-access-storage.md)
- [Ligue para o Microsoft Graph de forma segura usando uma identidade gerida](scenario-secure-app-access-microsoft-graph-as-app.md)

[Microsoft.Azure.Services.AppAuthentication referência]: /dotnet/api/overview/azure/service-to-service-authentication