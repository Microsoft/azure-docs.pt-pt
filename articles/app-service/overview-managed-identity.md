...
> |-----|-----|-----|
> |recurso|Consulta|O recurso AAD URI do recurso para o qual deve ser obtido um símbolo. Este pode ser um dos [serviços Azure que suportam a autenticação Azure AD](../active-directory/managed-identities-azure-resources/services-support-managed-identities.md#azure-services-that-support-azure-ad-authentication) ou qualquer outro recurso URI.|
...
> [!IMPORTANT]
> Se estiver a tentar obter fichas para identidades atribuídas ao `clientid` utilizador, deve incluir a propriedade. Caso contrário, o serviço simbólico tentará obter um símbolo para uma identidade atribuída ao sistema, que pode ou não existir.
...
> [!NOTE]
> As variáveis ambientais são configuradas quando o processo começa pela primeira vez, por isso, depois de permitir uma `MSI_ENDPOINT` `MSI_SECRET` identidade gerida para a sua aplicação, poderá ter de reiniciar a sua aplicação, ou recolocar o seu código, antes e estar disponível para o seu código.
