---
title: 'Quickstart: Criar um servidor - Azure PowerShell - Base de Dados Azure para MariaDB'
...
ms.locfileid: "84054240"
---
# <a name="quickstart-create-an-azure-database-for-mariadb-server-using-powershell"></a>Quickstart: Criar uma Base de Dados Azure para servidor MariaDB usando powerShell

Este quickstart descreve como usar o PowerShell para criar uma Base de Dados Azure para servidor MariaDB num grupo de recursos Azure. Pode utilizar o PowerShell para criar e gerir os recursos do Azure interactivamente ou em scripts.
...
A tabela seguinte contém uma lista de parâmetros geralmente utilizados e valores de amostra para o `New-AzMariaDbServer` cmdlet.

|        **Definição**         | **Valor de exemplo** |                                                                                                                                                             **Descrição**                                                                                                                                                              |
| -------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Name                       | mydemoserver     | Escolha um nome globalmente único em Azure que identifique a sua Base de Dados Azure para o servidor MariaDB. O nome do servidor só pode conter letras, números e o caráter do hífen (-). Quaisquer caracteres maiúsculos especificados são automaticamente convertidos para minúsculas durante o processo de criação. Tem de conter entre 3 e 63 carateres. |
| ResourceGroupName          | myResourceGroup  | Indique o nome do grupo de recursos do Azure.                                                                                                                                                                                                                                                                                            |
| Sku                        | GP_Gen5_2        | O nome do SKU. Segue em abreviatura os ** \_ \_ vCores** de geração de calculadores de nível de preços da convenção. Para mais informações sobre o parâmetro Sku, consulte a informação que se segue a esta tabela.                                                                                                                                           |
...
O exemplo seguinte desativa o SSL na sua Base de Dados Azure para o servidor MariaDB.
...
1. Ligue-se ao servidor utilizando a `mysql` ferramenta de linha de comando.
...
## <a name="connect-to-the-server-using-mariadb-workbench"></a>Ligue-se ao servidor usando a Bancada de Trabalho MariaDB
...
1. Na caixa de diálogo **Configurar Ligação Nova**, introduza as informações seguintes no separador **Parâmetros**:
   ...
   ![configurar ligação nova](./media/quickstart-create-mariadb-server-database-using-azure-powershell/setup-new-connection.png)
...
Para eliminar apenas o servidor criado neste quickstart sem eliminar o grupo de recursos, utilize o `Remove-AzMariaDbServer` cmdlet.
...
> [Desenhe uma Base de Dados Azure para MariaDB usando powerShell](tutorial-design-database-using-powershell.md)
