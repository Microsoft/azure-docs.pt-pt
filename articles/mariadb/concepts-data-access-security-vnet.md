---
title: Visão geral do ponto de extremidade dos serviços de VNet do banco de dados do Azure para MariaDB Microsoft Docs
description: Descreve como os pontos de extremidade do serviço de VNet funcionam para o servidor do banco de dados do Azure para MariaDB.
author: ajlam
ms.author: andrela
ms.service: mariadb
ms.topic: conceptual
...
*As regras de rede virtual* são um recurso de segurança de firewall que controla se o banco de dados do Azure para o servidor MariaDB aceita comunicações enviadas de sub-redes específicas em redes virtuais. Este artigo explica por que o recurso de regra de rede virtual às vezes é a melhor opção para permitir a comunicação com segurança com o servidor do banco de dados do Azure para MariaDB.
...
> [!NOTE]
> Esse recurso está disponível em todas as regiões do Azure em que o banco de dados do Azure para MariaDB é implantado para servidores Uso Geral e com otimização de memória.
...
**Ponto de extremidade de serviço de rede virtual:** Um [ponto de extremidade de serviço de rede virtual][vm-virtual-network-service-endpoints-overview-649d] é uma sub-rede cujos valores de propriedade incluem um ou mais nomes formais de tipo de serviço do Azure. Neste artigo, estamos interessados no nome do tipo do **Microsoft. SQL**, que se refere ao serviço do Azure chamado banco de dados SQL. Essa marca de serviço também se aplica ao banco de dados do Azure para serviços MariaDB, MySQL e PostgreSQL. É importante observar ao aplicar a marca de serviço **Microsoft. SQL** a um ponto de extremidade de serviço VNet que configurará o tráfego de ponto de extremidade de serviço para todo o banco de dados SQL do Azure, banco de dados do Azure para MariaDB, banco de dados do Azure para MySQL e banco de dados do Azure servidores na sub-rede.

**Regra de rede virtual:** Uma regra de rede virtual para o banco de dados do Azure para MariaDB Server é uma sub-rede listada na lista de controle de acesso (ACL) do banco de dados do Azure para o servidor MariaDB. Para estar na ACL do banco de dados do Azure para o servidor MariaDB, a sub-rede deve conter o nome do tipo **Microsoft. SQL** .

Uma regra de rede virtual instrui o servidor do banco de dados do Azure para MariaDB a aceitar comunicações de cada nó que está na sub-rede.
...
Se seu **Microsoft. SQL** Server era um nó em uma sub-rede em sua rede virtual, todos os nós na rede virtual podem se comunicar com o banco de dados do Azure para o servidor MariaDB. Nesse caso, suas VMs podem se comunicar com o banco de dados do Azure para MariaDB sem a necessidade de nenhuma regra de rede virtual ou regras de IP.

No entanto, a partir de agosto de 2018, o banco de dados do Azure para o serviço MariaDB ainda não está entre os serviços que podem ser atribuídos diretamente a uma sub-rede.
...
Cada regra de rede virtual se aplica a todo o banco de dados do Azure para MariaDB Server, não apenas a um banco de dados específico no servidor. Em outras palavras, a regra de rede virtual se aplica no nível do servidor, não no nível do banco de dados.
...
- **Administrador do banco de dados:** &nbsp;Atualize a lista de controle de acesso (ACL) para adicionar a sub-rede fornecida ao banco de dados do Azure para o servidor MariaDB.
...
> [!NOTE]
> Em alguns casos, o banco de dados do Azure para MariaDB e a rede virtual estão em assinaturas diferentes. Nesses casos, você deve garantir as seguintes configurações:
...
- No firewall do banco de dados do Azure para MariaDB, cada regra de rede virtual faz referência a uma sub-rede. Todas essas sub-redes referenciadas devem ser hospedadas na mesma região geográfica que hospeda o banco de dados do Azure para MariaDB.

- Cada banco de dados do Azure para MariaDB Server pode ter até 128 entradas de ACL para qualquer rede virtual específica.
...
- Ativar pontos de extremidade de serviço de rede virtual no banco de dados do Azure para MariaDB usando a marca de serviço **Microsoft. SQL** também habilita os pontos de extremidade para todos os serviços de banco de dados do Azure: Banco de dados do Azure para MariaDB, banco de dados do Azure para MySQL, banco de dados do Azure para PostgreSQL, banco de dados SQL do Azure e Azure SQL Data Warehouse.
...
- [Criar e gerenciar o banco de dados do Azure para regras de VNet MariaDB usando o portal do Azure](howto-manage-vnet-portal.md)
