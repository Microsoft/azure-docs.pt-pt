...
| --------- | ---- | ----------- |
|name|string|Necessário. O nome para o papel de trabalhador. O nome do papel deve ser único.|
...
|protocolo|string|Necessário. O protocolo de transporte para o ponto final externo. Para um papel de trabalhador, os valores possíveis `HTTP` `HTTPS` `UDP` são, ou `TCP` .|
...
|setReadyOnProcessStart|boolean|Necessário. Especifica se a instância de função espera que o programa da linha de comando indique que foi iniciado. Este valor deve ser definido `true` neste momento. A definição do valor `false` é reservada para utilização futura.|
