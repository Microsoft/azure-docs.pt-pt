---
title: Linha de segurança Azure para ExpressRoute
description: A linha de base de segurança ExpressRoute fornece orientações processuais e recursos para a implementação das recomendações de segurança especificadas no Benchmark de Segurança Azure.
author: msmbaldwin
...
ms.locfileid: "105558215"
---
# <a name="azure-security-baseline-for-expressroute"></a>Linha de segurança Azure para ExpressRoute

Esta linha de base de segurança aplica orientações da [versão 1.0 do Azure Security Benchmark](../security/benchmarks/overview-v1.md) para o ExpressRoute. A Referência de Segurança do Azure disponibiliza recomendações para proteger as suas soluções cloud no Azure.
O conteúdo é agrupado pelos **controlos de segurança definidos** pelo Azure Security Benchmark e pelas orientações conexos aplicáveis ao ExpressRoute. Foram excluídos **os controlos** não aplicáveis ao ExpressRoute.

 
Para ver como o ExpressRoute mapeia completamente para o Azure Security Benchmark, consulte o [ficheiro de mapeamento de base de segurança ExpressRoute completo](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Orientação**: Não aplicável; o sign-on único (SSO) adiciona segurança e comodidade quando os utilizadores insinam-se em aplicações personalizadas no Azure Ative Directory (Azure AD). O acesso ao avião de controlo Azure ExpressRoute (por exemplo, portal Azure) já está integrado com a Azure AD e é acedido através do portal Azure, bem como do API gestor de recursos Azure.
...
**Orientação**: Utilize uma estação de trabalho de acesso privilegiada (PAW) com a autenticação multi-factor Azure Ative (Azure AD) habilitada a iniciar sessão e configurar os seus recursos relacionados com o Azure Sentinel. 
...
**Orientação**: Utilize o Azure Ative Directory (Azure AD) como o sistema central de autenticação e autorização dos seus recursos Azure ExpressRoute. A Azure AD protege os dados utilizando uma encriptação forte para dados em repouso e em trânsito. A Azure AD também sai, hashes e armazena seguramente as credenciais dos utilizadores.
