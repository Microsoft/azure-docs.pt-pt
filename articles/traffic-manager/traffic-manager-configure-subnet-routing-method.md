---
title: Encaminhamento de tráfego de sub-redes configurada - Gestor de Tráfego Azure
...
No cenário discutido neste artigo, utilizando o encaminhamento de sub-redes, dependendo do endereço IP da consulta do utilizador, o tráfego é encaminhado para um website interno ou um website de produção.
...
Nesta secção, você cria dois VMs *myEndpointVMEastUS* e *myEndpointVMWEurope* nas regiões **Azure dos EUA** e Da Europa **Ocidental.**

1. No canto superior esquerdo do portal Azure, selecione **Criar um**  >  **VM Compute**  >  **Windows Server 2016**.
...
Nesta secção, instale o servidor IIS nos dois VMs - *myIISVMEastUS*   &  *myIISVMWEurope*- e, em seguida, atualize a página do site predefinido. Quando visitar o site de um browser, a página personalizada do site mostra o nome da VM à qual se está a ligar.
...
9. Repita os passos 1-6 com a criação de uma ligação RDP com o *VM myIISVMWEurope* dentro do grupo de recursos *myResourceGroupTM2* para instalar o IIS e personalizar a sua página web padrão.
10. Lançar o Windows PowerShell no *myIISVMWEurope* e utilizar os seguintes comandos para instalar o servidor IIS e atualizar o ficheiro htm predefinido.
    ```powershell-interactive
    # Install IIS
    Install-WindowsFeature -name Web-Server -IncludeManagementTools
...
Nesta secção, você cria um VM *(mVMEastUS* e *myVMWestEurope*) em cada região de Azure (**Europa Oriental** e **Oeste.** Irá utilizar estas VMs para testar a forma como o Gestor de Tráfego encaminha o tráfego para o servidor do IIS mais próximo quando navega até ao site.
...
2. Veja o Gestor de Tráfego em ação da forma que se segue:
    ...
    - A partir do teste VM (*myVMEastUS)* que está localizado na região da **Europa Ocidental,** num navegador web, navegue até ao nome DNS do seu perfil de Gestor de Tráfego.
...
4. Selecione **OK**.
5. Poderá receber um aviso de certificado durante o processo de início de sessão. Se receber o aviso, selecione **Sim** ou **Continuar** para prosseguir com a ligação.
1. Num browser, na VM *myVMEastUS*, introduza o nome DNS do perfil do Gestor de Tráfego para ver o seu site. Uma vez que o endereço IP *do myVMEastUS* VM está associado ao ponto final *myIISVMEastUS,* o navegador web lança o servidor do site test - *myIISVMEastUS*.
