...
Lançámos uma nova versão do Azure Monitor para VMs em janeiro de 2020 antes do anúncio da AG. Os clientes que permitem o Azure Monitor para VMs passarão a receber a versão GA, mas os clientes existentes que utilizam a versão do Azure Monitor para VMs a partir do 4º trimestre de 2019 e mais cedo serão solicitados a fazer upgrade. Este FAQ oferece orientação para realizar um upgrade em escala se você tiver grandes implementações em vários espaços de trabalho.


Com esta atualização, o Azure Monitor para os dados de desempenho dos VMs é armazenado na mesma tabela *InsightsMetrics* que o [Azure Monitor para contentores](container-insights-overview.md), o que facilita a consulta dos dois conjuntos de dados. Além disso, é capaz de armazenar conjuntos de dados mais diversos que não conseguimos armazenar na tabela anteriormente utilizada. 
...
No passado, viabilámos a solução ServiceMap no seu espaço de trabalho e balcões de desempenho de configuração no seu espaço de trabalho Log Analytics para enviar os dados para a tabela *Perf.* Esta nova solução envia os dados para uma tabela chamada *InsightsMetrics* que também é utilizada pelo Azure Monitor para contentores. Este esquema de tabela permite-nos armazenar métricas adicionais e conjuntos de dados de serviço que não são compatíveis com o formato da tabela *Perf.*
...
Quando um espaço de trabalho do Log Analytics for atualizado para a versão mais recente do Azure Monitor para VMs, irá atualizar o agente de dependência de cada um dos VMs ligados a esse espaço de trabalho. Cada VM que necessitar de atualização será identificado no **separador Get Started** no Azure Monitor para VMs no portal Azure. Quando optar por atualizar um VM, irá atualizar o espaço de trabalho para esse VM juntamente com quaisquer outros VM ligados a esse espaço de trabalho. Pode selecionar um único VM ou vários VMs, grupos de recursos ou subscrições. 
...
O método anterior de permitir o Azure Monitor para VMs utilizou contadores de desempenho no seu espaço de trabalho. A versão atual armazena estes dados numa tabela chamada `InsightsMetrics` . Pode optar por desativar estes contadores de desempenho no seu espaço de trabalho se já não precisar de os utilizar. 
...
Não faz mal. Verá as indicações no portal Azure ao visualizar o Azure Monitor para VMs sobre a próxima atualização. Uma vez lançado, receberá um pedido de solicitação para atualizar para a nova versão. Se preferir utilizar apenas a funcionalidade [Maps,](vminsights-maps.md) pode optar por não fazer upgrade e continuar a utilizar a funcionalidade Maps no Azure Monitor para VMs e a solução 'Mapa de Serviço' acedida a partir do seu espaço de trabalho ou azulejo do painel de instrumentos.

Se optar por ativar manualmente os contadores de desempenho no seu espaço de trabalho, poderá ver dados em alguns dos nossos gráficos de desempenho visualizados pelo Azure Monitor. Assim que a nova solução for lançada, atualizaremos os nossos gráficos de desempenho para consultar os dados armazenados na `InsightsMetrics` tabela. Se quiser ver dados dessa tabela nestes gráficos, terá de atualizar para a nova versão do Azure Monitor para VMs.

As alterações para mover dados de e que afetarão tanto o Mapa de `ServiceMapComputer_CL` Serviço como o Monitor `ServiceMapProcess_CL` Azure para VMs, pelo que ainda é necessário planear esta atualização.
...
A `InsightsMetrics` tabela armazenará conjuntos de dados de VM, processo e serviço que recolhemos e só serão povoados se estiver a utilizar o Azure Monitor para VMs e a solução VM Insights. A solução Mapa de Serviço não recolherá nem armazenará dados na `InsightsMetrics` tabela.
...
Planeamos relançar esta funcionalidade de Saúde em 2020, depois de o Azure Monitor para VMs estar em GA.
