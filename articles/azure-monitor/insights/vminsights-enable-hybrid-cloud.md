...
Este artigo explica como ativar o Azure Monitor para VMs (pré-visualização) para máquinas virtuais ou computadores físicos alojados no seu datacenter ou outro ambiente de cloud. No final deste processo, será com êxito começaram a suas máquinas virtuais no seu ambiente de monitorização e saiba se ocorrerem problemas de desempenho ou disponibilidade. 

Antes de começar, certifique-se de que reveja os [pré-requisitos](vminsights-enable-overview.md) e certifique-se de que a sua subscrição e os recursos cumprem os requisitos. Reveja os requisitos e métodos de implementação para o [agente Linux do Log Analytics e o Windows](../../log-analytics/log-analytics-agent-overview.md).

[!INCLUDE [log-analytics-agent-note](../../../includes/log-analytics-agent-note.md)]

>[!NOTE]
>O Azure Monitor para agente de dependência de mapa de VMs não transmitir todos os dados em si, e ele não requer alterações às firewalls ou portas. Os dados de mapa sempre são transmitidos pelo agente do Log Analytics para o serviço Azure Monitor, diretamente ou através da [gateway do Operations Management Suite](../../azure-monitor/platform/gateway.md) se as políticas de segurança de TI não permitir que os computadores na rede ligar à internet.

Os passos para concluir esta tarefa são resumidos da seguinte forma:
...
Por exemplo, para executar o programa de instalação com o `-help` parâmetro, introduza **InstallDependencyAgent Linux64.bin-ajudar**.

Instalar o agente de dependência de Linux como raiz, executando o comando `sh InstallDependencyAgent-Linux64.bin`.
