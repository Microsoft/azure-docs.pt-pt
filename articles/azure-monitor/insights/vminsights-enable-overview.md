...
> [!IMPORTANT]
> O Azure Monitor para a funcionalidade de saúde de hóspedes VMs tem um suporte mais limitado do sistema operativo enquanto está em pré-visualização pública. Consulte [Enable Azure Monitor para saúde de hóspedes VMs (pré-visualização)](vminsights-health-enable.md) para uma lista detalhada.
...
O Azure Monitor para VMs requer que os dois agentes seguintes sejam instalados em cada máquina virtual ou escala de máquina virtual definida para ser monitorizada. Para embarcar no recurso, instale estes agentes e ligue-os ao espaço de trabalho.  
...
| [Modelos do Resource Manager](vminsights-enable-resource-manager.md) | Instale ambos os agentes utilizando qualquer um dos métodos suportados para implementar um modelo de Gestor de Recursos, incluindo CLI e PowerShell. |
