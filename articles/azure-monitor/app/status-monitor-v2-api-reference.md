...
> [!NOTE]
> O nome do AppFilter neste contexto pode ser confuso, `AppFilter` define o filtro regex do nome da aplicação (HostingEnvironment.SiteName no caso de .Net no IIS). `VirtualPathFilter` define o filtro regex do caminho virtual (HostingEnvironment.ApplicationVirtualPath no caso de .Net on IIS). Para instrumentar uma única aplicação, utilizaria o VirtualPathFilter da seguinte forma: `Enable-ApplicationInsightsMonitoring -InstrumentationKeyMap @(@{VirtualPathFilter="^/MyAppName$"; InstrumentationSettings=@{InstrumentationKey='<your ikey>'}})`
