...
> [!NOTE]
> Se uma exceção for passada para o madeirão, então a mensagem de registo (e exceção) aparecerá no portal Azure debaixo da `exceptions` mesa em vez da `traces` tabela.
