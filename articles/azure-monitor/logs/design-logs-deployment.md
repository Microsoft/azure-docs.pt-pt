...
Os clientes que ingerem menos de 4TB/dia geralmente não cumprem estes limites. Os clientes que ingerem volumes mais elevados ou que tenham picos como parte das suas operações normais devem considerar a possibilidade de se deslocarem para [agrupamentos dedicados](./logs-dedicated-clusters.md) onde o limite da taxa de ingestão possa ser aumentado.
...
Todos os recursos, soluções de monitorização e Insights como Application Insights e Insights VM, infraestruturas de apoio e aplicações mantidas pelas diferentes equipas estão configurados para encaminhar os seus dados de registo recolhidos para o espaço de trabalho partilhado centralizado da organização de TI. Aos utilizadores de cada equipa é-lhes concedido acesso a registos de recursos a que lhes foi dado acesso.
...
* Ativar e configurar quaisquer soluções de monitorização, insights como insights de contentores e/ou Monitor Azure para VMs, a sua conta de Automação e soluções de gestão como a Update Management, Start/Stop VMs, etc., que foram implantadas no espaço de trabalho original.
