...
| ID |Yes |ID do gestor de recursos da definição de autoescala |
| name |Yes |O nome da definição de autoescala |
| detalhes |Yes |Explicação da ação que o serviço de autoescala tomou e a alteração na contagem de exemplos |
| subscriptionId |Yes |ID de subscrição do recurso-alvo que está a ser dimensionado |
| resourceGroupName |Yes |Nome do Grupo de Recursos do recurso-alvo que está a ser dimensionado |
| resourceName |Yes |Nome do recurso-alvo que está a ser dimensionado |
| resourceType |Yes |Os três valores suportados: "microsoft.classiccompute/domainnames/slots/roles" - Funções cloud service, "microsoft.compute/virtualmachinescalesets" - Conjuntos de escala de máquina virtual e "Microsoft.Web/serverfarms" - Web App |
| resourceId |Yes |ID do gestor de recursos do recurso-alvo que está a ser dimensionado |
| portalLink |Yes |Ligação do portal Azure à página sumária do recurso-alvo |
| oldCapacity |Yes |A contagem de instâncias (antiga) atual quando a Autoscale tomou uma ação de escala |
| nova capacidade |Yes |A nova contagem de exemplos que Autoscale dimensionou o recurso para |
| propriedades |No |Opcional. Conjunto de <Tecla, Pares de valor> (por exemplo, Dicionário <String, String>). O campo de propriedades é opcional. Numa interface personalizada de utilizador ou fluxo de trabalho baseado em aplicações lógicas, pode introduzir teclas e valores que podem ser passados usando a carga útil. Uma forma alternativa de passar propriedades personalizadas de volta para a chamada webhook de saída é usar o webhook URI em si (como parâmetros de consulta) |
