...
1. Ativar a solução de Mapa de Serviços a partir do [Mercado Azure](https://azuremarketplace.microsoft.com/marketplace/apps/Microsoft.ServiceMapOMS?tab=Overview) ou utilizando o processo descrito nas [soluções de monitorização Add da Galeria de Soluções.](../insights/solutions.md)
1. [Instale o agente Desac dependência no Windows](../vm/vminsights-enable-hybrid.md#install-the-dependency-agent-on-windows) ou [instale o agente Desadependição no Linux](../vm/vminsights-enable-hybrid.md#install-the-dependency-agent-on-linux) em cada computador onde pretende obter dados. O Agente de Dependência pode monitorizar as ligações dos vizinhos imediatos, pelo que poderá não precisar de um agente em cada computador.
...
Para ver os dados de desempenho, poderá ser necessário [ativar os contadores de desempenho apropriados do Log Analytics](../agents/data-sources-performance-counters.md).  Os contadores que pretende ativar:
