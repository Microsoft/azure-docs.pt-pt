...
O Azure Monitor para VMs suporta qualquer sistema operativo que suporte o agente Log Analytics e o agente De dependência. Consulte [a visão geral dos agentes do Monitor Azure ](../agents/agents-overview.md#supported-operating-systems) para obter uma lista completa.

> [!IMPORTANT]
> O Azure Monitor para a funcionalidade de saúde de hóspedes VMs tem um suporte mais limitado do sistema operativo enquanto está em pré-visualização pública. Consulte [Enable Azure Monitor para saúde de hóspedes VMs (pré-visualização)](../vm/vminsights-health-enable.md) para uma lista detalhada.
...
O Azure Monitor para VMs requer que os dois agentes seguintes sejam instalados em cada máquina virtual ou escala de máquina virtual definida para ser monitorizada. Para embarcar no recurso, instale estes agentes e ligue-os ao espaço de trabalho.  Consulte [os requisitos de rede](../agents/log-analytics-agent.md#network-requirements) para os requisitos de rede para estes agentes.
...
- Consulte [os requisitos de rede](../platform/log-analytics-agent.md#network-requirements) para os requisitos de rede para o agente Log Analytics.
...
Para aprender a utilizar a função de monitorização de desempenho, consulte [o View Azure Monitor para o desempenho dos VMs](../vm/vminsights-performance.md). Para ver as dependências de aplicações descobertas, consulte [o Monitor do Azure para o Mapa de VMs](../vm/vminsights-maps.md).
