...
Ao utilizar a extensão Azure Monitor [Windows Azure Diagnostics (WAD),](../agents/diagnostics-extension-overview.md)pode recolher métricas e registos do sistema operativo convidado (OS convidado) que funciona como parte de uma máquina virtual, serviço em nuvem ou cluster Azure Service Fabric. A extensão pode enviar telemetria para vários locais diferentes listados no artigo anteriormente ligado.  
