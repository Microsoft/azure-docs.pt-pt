...
* As métricas personalizadas só estão disponíveis num subconjunto de regiões de Azure. Uma lista de regiões apoiadas está documentada nas [regiões apoiadas.](../essentials/metrics-custom-overview.md#supported-regions)
...
* As regras de alerta não têm um grupo de ação atribuído por padrão. Pode adicionar um [grupo de ação](../alerts/action-groups.md) ao alerta, selecionando um grupo de ação existente ou criando um novo grupo de ação enquanto edita a regra de alerta.
