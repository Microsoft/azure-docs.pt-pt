...
1. Escolha o intervalo de tempo sobre o qual para avaliar a condição especificada, utilizando a opção [**Period.**](../platform/alerts-unified-log.md#query-time-range)

1. Os alertas de registo podem basear-se em dois tipos de [**medidas:**](../platform/alerts-unified-log.md#measure)
...
1. Para a lógica de alerta de medições métricas, pode especificar opcionalmente como [dividir os alertas por dimensões](../platform/alerts-unified-log.md#split-by-alert-dimensions) utilizando a opção **Agregada.** A expressão de agrupamento de linha deve ser única e ordenada.
...
1. Em seguida, com base nos dados de pré-visualização definidos [ **o Operador,** **Valor Limiar**](../platform/alerts-unified-log.md#threshold-and-operator)e [**Frequência**](../platform/alerts-unified-log.md#frequency).

1. Também pode configurar opcionalmente o [número de violações para desencadear um alerta](../platform/alerts-unified-log.md#number-of-violations-to-trigger-alert) utilizando **Violações Totais ou Consecutivas.**
...
1. Escolha se pretende suprimir as ações de regras durante um tempo após o disparo de um alerta, utilize a opção [**Alertas de Supressão.**](../platform/alerts-unified-log.md#state-and-resolving-alerts) A regra ainda vai funcionar e criar alertas, mas as ações não serão desencadeadas para evitar o ruído. O valor das ações de silenciamento deve ser maior do que a frequência de alerta para ser eficaz.

    ![Alertas de supressão para alertas de registo](media/alerts-log/AlertsPreviewSuppress.png)

1. Especifique se a regra de alerta deve desencadear um ou mais [**grupos de ação**](../platform/action-groups.md#webhook) quando a condição de alerta for cumprida.
...
1. Comece pelo **separador Condição:**
    ...
    1. Verifique se a [**medida**](../platform/alerts-unified-log.md#measure), [**tipo de agregação**](../platform/alerts-unified-log.md#aggregation-type)e [**granularidade de agregação e granularidade**](../platform/alerts-unified-log.md#aggregation-granularity) estão corretas. 
    ...
    1. Em seguida, com base nos dados de pré-visualização, desateie a **lógica de Alerta;** [ **Operador,** **Valor Limiar**](../platform/alerts-unified-log.md#threshold-and-operator)e [**Frequência.**](../platform/alerts-unified-log.md#frequency)

        ![Gráfico de pré-visualização com lógica de limiar e alerta](media/alerts-log/chart-and-alert-logic.png)

    1. Pode configurar opcionalmente [**número de violações para desencadear o alerta**](../platform/alerts-unified-log.md#number-of-violations-to-trigger-alert) na secção de **opções Avançadas.**
    
        ![Opções avançadas](media/alerts-log/advanced-options.png)

1. No separador **Ações,** selecione ou crie os [grupos de ação necessários.](../platform/action-groups.md)

    ![Separador de ações](media/alerts-log/actions-tab.png)

1. No separador **Detalhes,** defina os detalhes da **regra alerta** e detalhes **do Projeto**. Pode configurar opcionalmente se não **deve começar** a funcionar agora , ou [**ações de silenciamento**](../platform/alerts-unified-log.md#state-and-resolving-alerts) por um período após o incêndio da regra de alerta.
...
1. A gestão de alertas mostra todos os alertas que dispararam. [Saiba mais sobre a gestão de alertas.](alerts-managing-alert-instances.md)
    ...
    > [!NOTE]
    > As regras de alerta de registo são atualmente [apátridas e não resolvem](../platform/alerts-unified-log.md#state-and-resolving-alerts).
...
* Saiba mais sobre [os alertas de registo.](../platform/alerts-unified-log.md)
