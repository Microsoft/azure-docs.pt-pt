...
> [!NOTE]
> A Azure Monitor tem atualmente vários agentes devido à recente consolidação do Azure Monitor e do Log Analytics. Embora possa haver sobreposição nas suas características, cada uma tem capacidades únicas. Dependendo dos seus requisitos, poderá precisar de um ou mais agentes nas suas máquinas. 

Você pode ter um conjunto específico de requisitos que não podem ser completamente cumpridos com um único agente para uma determinada máquina. Por exemplo, pode querer utilizar alertas métricos que requerem extensão de diagnóstico Azure, mas também quer aproveitar a funcionalidade do Azure Monitor para VMs que requer o agente Log Analytics e o agente Dependency. Em casos como este, você pode usar vários agentes, e este é um cenário comum para clientes que requerem funcionalidade de cada um.
...
- Enviar dados para ferramentas de terceiros utilizando [hubs de eventos Azure](../platform/diagnostics-extension-stream-event-hubs.md).
- Gerencie a segurança das suas máquinas utilizando o [Azure Security Center](../../security-center/security-center-introduction.md)  ou [o Azure Sentinel](../../sentinel/overview.md). (Não disponível em pré-visualização.)
...
O [agente Do Log Analytics](../platform/log-analytics-agent.md) recolhe dados de monitorização do sistema operativo dos hóspedes e cargas de trabalho de máquinas virtuais em Azure, outros fornecedores de nuvem e máquinas no local. Envia dados para um espaço de trabalho log analytics. O agente Log Analytics é o mesmo agente utilizado pelo Gestor de Operações do System Center, e pode comunicar com o seu grupo de gestão e o Azure Monitor simultaneamente. Este agente também é exigido por determinadas informações no Azure Monitor e outros serviços em Azure.
...
* Recolher registos e dados de desempenho de máquinas virtuais Azure ou máquinas híbridas hospedadas fora de Azure.
* Envie dados para um espaço de trabalho log analytics para tirar partido das funcionalidades suportadas por [Registos do Monitor Azure,](../platform/data-platform-logs.md) tais como [consultas de registo .](../log-query/log-query-overview.md)
* Utilize [o Azure Monitor para VMs](../insights/vminsights-overview.md) que lhe permite monitorizar as suas máquinas em escala e monitorizar os seus processos e dependências de outros recursos e processos externos..  
* Gerencie a segurança das suas máquinas utilizando o [Azure Security Center](../../security-center/security-center-introduction.md)  ou [o Azure Sentinel](../../sentinel/overview.md).
* Utilize [a Azure Automation Update Management](../../automation/update-management/overview.md), [Azure Automation State Configuration](../../automation/automation-dsc-overview.md), ou [Azure Automation Change Tracking and Inventory](../../automation/change-tracking/overview.md) para fornecer uma gestão abrangente das suas máquinas Azure e não-Azure.
...
- Envie dados para [a Azure Monitor Metrics](../platform/data-platform-metrics.md) para analisá-lo com o explorador de [métricas](../platform/metrics-getting-started.md) e para tirar partido de funcionalidades como [alertas métricos próximos](./../platform/alerts-metric-overview.md) em tempo real e [autoescalação](../platform/autoscale-overview.md) (apenas Windows).
- Enviar dados para ferramentas de terceiros utilizando [hubs de eventos Azure](../platform/diagnostics-extension-stream-event-hubs.md).
...
O [agente InfluxData Telegraf](../platform/collect-custom-metrics-linux-telegraf.md) é utilizado para recolher dados de desempenho dos computadores Linux para as Métricas do Monitor Azure.
...
O agente de Dependência recolhe dados descobertos sobre processos em execução na máquina e dependências de processos externos. 
...
- O agente Desadependido requer que o agente Log Analytics seja instalado na mesma máquina.
- Nos computadores Linux, o agente Log Analytics tem de ser instalado antes da extensão de diagnóstico do Azure.
...
Em máquinas híbridas, utilize [os servidores Azure Arc habilitados](../../azure-arc/servers/manage-vm-extensions.md) para implementar as extensões VM de Dependência do Monitor de Log e Azure.
...
- [Visão geral do agente Log Analytics](../platform/log-analytics-agent.md)
- [Descrição geral da extensão do Diagnóstico do Azure](../platform/diagnostics-extension-overview.md)
- [Colete métricas personalizadas para um Linux VM com o agente InfluxData Telegraf](../platform/collect-custom-metrics-linux-telegraf.md)
