...
Você pode ter um conjunto específico de requisitos que não podem ser completamente cumpridos com um único agente para uma determinada máquina. Por exemplo, pode querer utilizar alertas métricos que requerem extensão de diagnóstico Azure, mas também pretende aproveitar a funcionalidade de insights VM que requerem o agente Log Analytics e o agente Dependency. Em casos como este, você pode usar vários agentes, e este é um cenário comum para clientes que requerem funcionalidade de cada um.
...
* Envie dados para um espaço de trabalho log analytics para tirar partido das funcionalidades suportadas por [Registos do Monitor Azure,](../logs/data-platform-logs.md) tais como [consultas de registo .](../logs/log-query-overview.md)
* Utilize [insights VM](../vm/vminsights-overview.md) que lhe permitam monitorizar as suas máquinas em escala e monitorizar os seus processos e dependências de outros recursos e processos externos..  
...
- Envie dados para [a Azure Monitor Metrics](../essentials/data-platform-metrics.md) para analisá-lo com o explorador de [métricas](../essentials/metrics-getting-started.md) e para tirar partido de funcionalidades como [alertas métricos próximos](../alerts/alerts-metric-overview.md) em tempo real e [autoescalação](../autoscale/autoscale-overview.md) (apenas Windows).
- Enviar dados para ferramentas de terceiros utilizando [hubs de eventos Azure](./diagnostics-extension-stream-event-hubs.md).
- Colete [o Boot Diagnostics](/troubleshoot/azure/virtual-machines/boot-diagnostics) para investigar problemas de arranque VM.
...
O [agente InfluxData Telegraf](../essentials/collect-custom-metrics-linux-telegraf.md) é utilizado para recolher dados de desempenho dos computadores Linux para as Métricas do Monitor Azure.
...
* Utilize os [insights VM](../vm/vminsights-overview.md) da funcionalidade Mapa ou a solução [de Mapa de Serviço.](../vm/service-map.md)
...
| Sistema operativo | Agente Azure Monitor <sup>1</sup> | Log Analytics agente <sup>1</sup> | Agente de Dependência | Extensão de diagnóstico <sup>2</sup>| 
|:---|:---:|:---:|:---:|:---:
...
<sup>1</sup> Requer que o Python (2 ou 3) seja instalado na máquina.

<sup>2</sup> Requer que o Python 2 seja instalado na máquina.

<sup>3</sup> Edição conhecida a recolher eventos Syslog. Apenas os dados de desempenho são suportados atualmente.
...
- [Colete métricas personalizadas para um Linux VM com o agente InfluxData Telegraf](../essentials/collect-custom-metrics-linux-telegraf.md)