---
title: Utilize o armazenamento de bolhas para iis e armazenamento de mesa para eventos no Azure Monitor Microsoft Docs
...
> [!NOTE]
> O agente Log Analytics no Azure Monitor é tipicamente o método preferido para recolher dados do sistema operativo do hóspede em Registos monitores Azure. Consulte [a visão geral dos agentes do Monitor Azure](agents-overview.md) para uma comparação detalhada dos agentes.
...
Consulte [instalar e configurar a extensão de diagnóstico do Windows Azure (WAD)](diagnostics-extension-windows-install.md) ou [utilizar a extensão de diagnóstico do Linux para monitorizar métricas e registos](../../virtual-machines/extensions/diagnostics-linux.md) para obter detalhes sobre a instalação e configuração da extensão de diagnóstico. Isto irá permitir-lhe especificar a conta de armazenamento e configurar a recolha dos dados que pretende encaminhar para os Registos do Monitor Azure.
...
* [Recolher registos e métricas para serviços Azure](collect-azure-metrics-logs.md) para serviços Azure suportados.
