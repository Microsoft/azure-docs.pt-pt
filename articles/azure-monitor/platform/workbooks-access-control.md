...
* Acesso necessário para salvar livros
    ...
    - Salvar livros privados `("My")` não requer privilégios adicionais. Todos os utilizadores podem guardar livros privados, e só eles podem ver esses livros.
...
[O Colaborador de Monitorização](https://docs.microsoft.com/azure/role-based-access-control/built-in-roles#monitoring-contributor) inclui privilégios gerais `/write` utilizados por várias ferramentas de monitorização para guardar itens (incluindo `workbooks/write` privilégio de salvar livros partilhados).
