...
Pode criar propriedades de e-mail e webhook utilizando `New-AzAlertRuleEmail` `New-AzAlertRuleWebhook` e, respectivamente. Na regra de Alerta, atribua estes imóveis como ações à propriedade **das Ações** da Regra de Alerta.
...
Uma lista completa de opções disponíveis `Get-AzMetricDefinition` está disponível no [Get-MetricDefinitions](https://msdn.microsoft.com/library/mt282458.aspx).
