...
3. Introduza ou selecione valores para as seguintes definições e, em seguida, **selecione Criar**:
    ...
    |Subrede|Yes|Selecione uma sub-rede dentro da rede virtual selecionada. Pode alterar a sub-rede a que a interface de rede é atribuída após a sua criação.|
...
3. Os seguintes itens estão listados para a interface de rede que selecionou:
   ...
   - **Regras de segurança eficazes:**  As regras de segurança estão listadas se a interface de rede estiver ligada a uma máquina virtual em execução, e um NSG estiver associado à interface de rede, à sub-rede a que é atribuída, ou ambos. Para saber mais sobre o que é exibido, consulte [as regras de segurança eficazes.](#view-effective-security-rules) Para saber mais sobre os NSGs, consulte [os grupos de segurança da Rede.](security-overview.md)
...
Só é possível adicionar uma interface de rede ou remover uma interface de rede de um grupo de segurança de aplicações que utilize o portal se a interface de rede estiver ligada a uma máquina virtual. Pode utilizar o PowerShell ou o CLI Azure para adicionar uma interface de rede ou remover uma interface de rede de um grupo de segurança de aplicações, quer a interface da rede esteja ou não ligada a uma máquina virtual. Saiba mais sobre [os grupos de segurança de aplicações](security-overview.md#application-security-groups) e como [criar um grupo de segurança de aplicações.](manage-network-security-group.md)
...
5. Reveja a lista de regras de segurança eficazes para determinar se existem as regras corretas para a sua comunicação de entrada e saída necessária. Saiba mais sobre o que vê na lista na [visão geral do grupo de segurança network](security-overview.md).
