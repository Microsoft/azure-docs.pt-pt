---
title: 'Tutorial: Integrar porta de entrada NAT com um equilibrador de carga interno - Portal Azure'
titleSuffix: Virtual Network NAT
description: Neste tutorial, aprenda a integrar um gateway NAT de Rede Virtual com um Balancer de carga interna utilizando o portal Azure.
author: asudbring
ms.author: allensu
...
ms.locfileid: "104670865"
---
# <a name="tutorial-integrate-a-nat-gateway-with-an-internal-load-balancer-using-the-azure-portal"></a>Tutorial: Integrar uma porta DE ENTRADA NAT com um equilibrador de carga interno utilizando o portal Azure

Neste tutorial, você vai aprender a integrar um gateway NAT com um equilibrador de carga interno.

Por predefinição, um Balançador de Carga Padrão Azure está seguro. A conectividade de saída é explicitamente definida permitindo o SNAT de saída (Tradução de Endereços de Rede de Origem). 

O SNAT está ativado para um pool interno de backend através de outro equilibrador de carga pública, encaminhamento de rede ou um IP público definido numa máquina virtual.

A integração do gateway NAT substitui a necessidade de implantação de um equilibrador de carga pública, encaminhamento de rede ou um IP público definido numa máquina virtual no pool de backend.
...
4. Na página do **balanceador de carga,** ou selecione as seguintes informações: 
    ...
    | Zona de disponibilidade | Selecione **Zona redundante** para criar um equilibrador de carga resistente. </br> Para criar um equilibrador de carga zonal, selecione uma zona específica a partir de 1, 2 ou 3 |
...
3. Na página **de pool de backend,** para nome, insira **myBackendPool,** como o nome da sua piscina de backend e, em seguida, selecione **Add**.
...
2. Tome nota do endereço IP público:
    ...
    :::image type="content" source="./media/tutorial-nat-gateway-load-balancer-internal-portal/find-public-ip.png" alt-text="Screenshot de descobrir endereço IP público do gateway NAT." border="true":::

3. Selecione **Todos os serviços** no menu à esquerda, selecione **Todos os recursos**, e, em seguida, na lista de recursos, selecione **myVM1** que está localizado no grupo de recursos **TutorIntLBNAT-rg.**
...
9. Verifique se o endereço IP apresentado corresponde ao endereço de gateway NAT indicado no passo anterior:
    ...
    :::image type="content" source="./media/tutorial-nat-gateway-load-balancer-internal-portal/my-ip.png" alt-text="Screenshot do Internet Explorer mostrando IP de saída externa." border="true":::
...
4. **Insira tutorIntLBNAT-rg** e selecione **Delete**.
