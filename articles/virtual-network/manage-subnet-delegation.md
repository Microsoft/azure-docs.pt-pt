...
Crie uma rede virtual com o nome **myVnet**, com uma sub-rede de nome **mySubnet** em **myResourceGroup**, através de [az network vnet create](/cli/azure/network/vnet).
...
Utilize [a atualização da sub-rede de rede Az](/cli/azure/network/vnet/subnet#az_network_vnet_subnet_update) para atualizar a sub-rede chamada **mySubnet** com uma delegação para um serviço Azure.  Neste **exemplo, o Microsoft.DBforPostgreSQL/serversv2** é utilizado para a delegação de exemplo:
...
Para verificar se a delegação foi aplicada, utilize [a rede az vnet show](/cli/azure/network/vnet/subnet#az_network_vnet_subnet_show). Verifique se o serviço é delegado na sub-rede sob o nome do serviço de **propriedade:**
...
Utilize [a atualização da sub-rede de vnet](/cli/azure/network/vnet/subnet#az_network_vnet_subnet_update) da rede Az para remover a delegação da sub-rede denominada **mySubnet**:
...
```azurecli-interactive
  ...
  --remove delegations
```
Para verificar se a delegação foi removida, utilize [a rede az vnet show](/cli/azure/network/vnet/subnet#az_network_vnet_subnet_show). Verifique se o serviço é removido da sub-rede sob o serviço de **propriedadeName**:
...
### <a name="create-a-resource-group"></a>Criar um grupo de recursos
Criar um grupo de recursos com [o New-AzResourceGroup](/cli/azure/group). Um grupo de recursos do Azure é um contentor lógico no qual os recursos do Azure são implementados e geridos.
...
Utilize [o Add-AzDelegation](/powershell/module/az.network/add-azdelegation) para atualizar a sub-rede chamada **mySubnet** com uma delegação chamada **myDelegation** a um serviço Azure.  Neste **exemplo, o Microsoft.DBforPostgreSQL/serversv2** é utilizado para a delegação de exemplo:
...
```
Utilize [a Get-AzDelegation](/powershell/module/az.network/get-azdelegation) para verificar a delegação:
...
Utilize [remove-AzDelegation](/powershell/module/az.network/remove-azdelegation) para remover a delegação da sub-rede denominada **mySubnet**:
...
```azurepowershell-interactive
  ...
  Set-AzVirtualNetwork -VirtualNetwork $vnet
```
Utilize [a Get-AzDelegation](/powershell/module/az.network/get-azdelegation) para verificar se a delegação foi removida:
