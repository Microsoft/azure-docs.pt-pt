...
3. Em **DEFINIÇÕES**, **selecione Networking**, como mostrado na seguinte imagem:
   ...
   ![A screenshot mostra o portal Azure com definições de Networking para o meu V M V M Nic.](./media/diagnose-network-traffic-filter-problem/view-security-rules.png)
   ...
   A regra chamada **DenyAllInBound** é o que está a impedir a comunicação de entrada para o VM sobre a porta 80, a partir da internet, como descrito no [cenário](#scenario). A regra lista *0.0.0.0/0* para **FONTE**, que inclui a internet. Nenhuma outra regra com uma prioridade mais elevada (número inferior) permite a entrada da porta 80. Para permitir a entrada da porta 80 para o VM a partir da internet, consulte [Resolver um problema](#resolve-a-problem). Para saber mais sobre as regras de segurança e como a Azure as aplica, consulte [os grupos de segurança da Rede](./network-security-groups-overview.md).

   Na parte inferior da imagem, também vê **as regras do PORTO DE SAÍDA**. Sob isso estão as regras portuárias de saída para a interface de rede. Embora a imagem mostre apenas quatro regras de entrada para cada NSG, os seus NSGs podem ter muito mais do que quatro regras. Na imagem, vê **a VirtualNetwork** em **SOURCE** e **DESTINATION** e **AzureLoadBalancer** em **SOURCE**. **VirtualNetwork** e **AzureLoadBalancer** são [tags de serviço.](./network-security-groups-overview.md#service-tags) As etiquetas de serviço representam um grupo de prefixos de endereço IP para ajudar a minimizar a complexidade para a criação de regras de segurança.
...
4. Certifique-se de que o VM está no estado de funcionamento e, em seguida, selecione **regras de segurança eficazes,** como mostrado na imagem anterior, para ver as regras de segurança eficazes, mostradas na seguinte imagem:
   ...
   ![O Screenshot mostra o painel de regras de segurança eficaz com download selecionado e regra de entrada de entrada de Admissão AllowAzureLoadBalancerInbound selecionada.](./media/diagnose-network-traffic-filter-problem/view-effective-security-rules.png)
   ...
   ![A screenshot mostra prefixos de endereço para AllowAzureLoadBalancerInbound introduzido.](./media/diagnose-network-traffic-filter-problem/address-prefixes.png)
...
5. Os passos anteriores mostraram as regras de segurança de uma interface de rede chamada **myVMVMNic**, mas também viu uma interface de rede chamada **myVMVMNic2** em algumas das imagens anteriores. O VM neste exemplo tem duas interfaces de rede anexadas a ele. As regras de segurança eficazes podem ser diferentes para cada interface de rede.
   ...
   ![A screenshot mostra o portal Azure com definições de Networking para o meu V M V M Nic 2.](./media/diagnose-network-traffic-filter-problem/view-security-rules2.png)
...
- Saiba mais sobre [as regras de segurança predefinidas,](./network-security-groups-overview.md#default-security-rules) [etiquetas de serviço,](./network-security-groups-overview.md#service-tags)e [como a Azure processa as regras de segurança para o tráfego de entrada e saída](./network-security-groups-overview.md#network-security-groups) para um VM.