...
5. <a name="add-peering"></a>Introduza ou selecione os valores para as seguintes definições:
    ...
    Apenas um peering para esta rede virtual pode ter esta definição ativada.
        ...
        Não é possível utilizar gateways remotos se já tiver um gateway configurado na sua rede virtual. Para saber mais sobre como utilizar um gateway para o trânsito, veja [configurar um gateway de VPN que o trânsito no peering de rede virtual](../vpn-gateway/vpn-gateway-peering-gateway-transit.md?toc=%2fazure%2fvirtual-network%2ftoc.json)
...
- Ao criar um peering global, as redes virtuais em modo de peering podem existir em qualquer região de cloud pública do Azure ou regiões de cloud da China ou regiões de cloud do Governo. Não é possível configurar o peering entre nuvens. Por exemplo, uma VNet na cloud pública do Azure não pode ser agrupada para uma VNet na cloud do Azure China.
...
- Pode utilizar gateways remotos ou permitir que o trânsito de gateway no globalmente em modo de peering de redes virtuais e redes virtuais em modo de peering localmente.
...
| Microsoft.ClassicNetwork/virtualNetworks/peer/action                   | Necessário para criar um peering da rede virtual B (clássico) para a rede virtual A                                                                |
