...
Este artigo descreve como criar uma ligação de VPN de site a site entre duas redes virtuais em dois ambientes separados do Azure Stack Development Kit (ASDK). Quando configurar as ligações, saiba como funcionam os gateways VPN no Azure Stack.
...
* Dois servidores e outros pré-requisitos que cumprem os requisitos de hardware ASDK, conforme descrito em [início rápido: Avaliar o Kit de desenvolvimento do Azure Stack](azure-stack-deploy-overview.md).
...
Para concluir a configuração da ligação, tem de implementar dois ambientes de ASDK.

> [!NOTE]
> Para cada ASDK que implementar, siga os [instruções de implementação](azure-stack-run-powershell-script.md). Neste artigo, os ambientes de ASDK são chamados **POC1** e **POC2**.
...
A tabela seguinte resume a configuração de rede em ambos os ambientes de ASDK. Utilize o procedimento que aparece depois da tabela para adicionar o endereço externo BGPNAT específica para a sua rede.
...
> [!NOTE]
> Os endereços de IP de BGPNAT externos no ambiente de exemplo são 10.16.167.195 para a POC1 e 10.16.169.131 para a POC2. Utilize o procedimento seguinte para determinar os endereços IP de BGPNAT externos para os anfitriões ASDK e, em seguida, adicioná-los para a tabela de configuração de rede anterior.
...
Agora, pode criar os recursos de rede de POC1 que precisa para definir a seus gateways. As instruções seguintes descrevem como criar os recursos com o portal de utilizador do Azure Stack. Também pode utilizar o código do PowerShell para criar os recursos.
...
5. Selecione o **rede Virtual** item para escolher uma rede virtual. Selecione **VNET-01** da lista.
6. Selecione o **endereço IP público** item de menu. Quando o **escolher endereço IP público** é apresentada a janela, selecione **criar nova**.
...
### <a name="sign-in-as-a-tenant-again"></a>Inicie sessão como um inquilino novamente
...
4. O nome da sub-rede está predefinido como **GatewaySubnet**. As sub-redes de gateway são especiais e têm de ter este nome específico para funcionarem corretamente.
...
### <a name="create-local-network-gateway-resource"></a>Criar recurso do gateway de rede local

1. No portal de utilizador POC2, selecione **+ criar um recurso**.
2. Aceda a **Marketplace**e, em seguida, selecione **Networking**.
3. Na lista de recursos, selecione **gateway de rede Local**.
4. Na **Name**, introduza **POC1-GW**.
5. Na **endereço IP**, introduza o endereço externo BGPNAT para a POC1 listado anteriormente na tabela de configuração de rede.
6. Na **espaço de endereços**, da POC1, introduza o **10.0.10.0/23** espaço de endereços da **VNET-01**.
7. Certifique-se de que sua **subscrição**, **grupo de recursos**, e **localização** estão corretos e, em seguida, selecione **criar**.

## <a name="create-connection"></a>Criar ligação
...
9. Na **chave partilhada (PSK)**, introduza **12345**. Se escolher um valor diferente, lembre-se de que tem de corresponder o valor para a chave partilhada que criou na POC1. Selecione **OK**.
10. Reveja os **resumo** painel e, em seguida, selecione **OK**.

## <a name="create-a-virtual-machine"></a>Criar uma máquina virtual

Agora, crie uma máquina virtual na POC2 e colocá-la na sua sub-rede VM na sua rede virtual:
...
Uma vez que o ASDK é independente e isolada da rede no qual está implementado no anfitrião físico, o *externo* rede VIP que os gateways estão ligados a não é efetivamente externa. Em vez disso, a rede VIP está oculta atrás de um router que executa a tradução de endereços de rede.

O router é uma máquina virtual do Windows Server, chamada **AzS-bgpnat01**, que executa a função de encaminhamento e serviços de acesso remoto (RRAS) na infraestrutura ASDK. É necessário configurar a NAT na máquina virtual AzS-bgpnat01 para permitir a ligação de VPN de site a site ligar em ambas as extremidades.
...
> [!IMPORTANT]
> Tem de concluir este procedimento para ambos os ambientes de ASDK.
...
3. Copie e edite o seguinte script do PowerShell. Para configurar o NAT em cada Development Kit do Azure Stack, execute o script num ISE do PowerShell elevada do Windows. No script, adicione valores para o `External BGPNAT address` e `Internal IP address` marcadores de posição:
...
Agora que a ligação de site a site é estabelecida, deve validar que pode obter o tráfego que flui através do mesmo. Para validar, iniciar sessão a uma das máquinas virtuais que criou em qualquer ambiente ASDK. Em seguida, enviar um ping a máquina virtual que criou no outro ambiente.

Para garantir que envia o tráfego através da ligação de site a site, certifique-se de que enviar pings para o endereço IP direto (DIP) da máquina virtual na sub-rede remota, não o VIP. Para fazê-lo, encontre o endereço do DIP na outra extremidade da ligação. Guarde o endereço para utilização posterior.
...
6. Abra uma **Windows PowerShell** janela.
7. Enter **ipconfig /all**.
8. Um endereço IPv4 é apresentado o que se encontrem dentro **10.0.20.0/24**. No ambiente de exemplo, o endereço está **10.0.20.4**, mas seu endereço poderá ser diferente.
9. Para criar uma regra de firewall que permite que a máquina virtual responder a pings, execute o seguinte comando do PowerShell:
...
10. Da máquina virtual na POC2, enviar um ping a máquina virtual na POC1, através do túnel. Para fazer isso, enviar pings para o DIP que registou da VM01. No ambiente de exemplo, isto é **10.0.10.4**, mas não se esqueça de fazer ping no endereço que anotou no seu laboratório. Deverá ver um resultado semelhante ao seguinte exemplo:
...
3. Na **ligação** janela, as estatísticas **dados no** e **dados de saída** aparecem. Captura de ecrã seguinte, os números grandes são atribuídos para transferência de ficheiros adicionais. Deverá ver alguns valores diferentes de zero aí.
