...
O `createCallAgent` método toma como `CommunicationUserCredential` argumento, que aceita um símbolo de acesso [ao utilizador](../../access-tokens.md).
...
Para criar e iniciar uma chamada, precisa de utilizar uma das APIs na CallAgent e fornecer um utilizador que criou através da biblioteca de clientes da administração de Serviços de Comunicação.
...
Para fazer uma chamada a outro utilizador dos Serviços de Comunicação, invoque o `call` método e passe o `callAgent` CommunicationUser que [criou com a biblioteca da Administração de Serviços de Comunicação.](../../access-tokens.md)
...
Para colocar uma chamada de 1:n para um utilizador e um número PSTN, tem de especificar um Comunicador de Comunicação e um Número de Telefone para ambos os callees.
...
Para fazer uma chamada de vídeo, tem de enumerar as câmaras locais utilizando o dispositivo `getCameraList` API.
Uma vez selecionada a câmara desejada, utilize-a para construir um `LocalVideoStream` caso e passe-a para dentro `videoOptions` como um item dentro da matriz para o `localVideoStream` `call` método.
Assim que a sua chamada se ligar, começará automaticamente a enviar um fluxo de vídeo da câmara selecionada para o outro(s) participante(s)
```js
const deviceManager = await callClient.getDeviceManager();
...
* Para saber mais sobre outros participantes na chamada, inspecione a `remoteParticipant` coleção sobre o `call` caso.
```js
...
* Para saber se a chamada atual é uma chamada recebida, inspecione a `isIncoming` propriedade
```js
...
*  Para verificar se o microfone atual está silenciado, inspecione a `muted` propriedade:
```js
...
* Para ver se o fluxo de partilha de ecrã está a ser enviado a partir de um determinado ponto final, verifique a `isScreenSharingOn` propriedade:
```js
...
Para iniciar um vídeo, tem de enumerar câmaras utilizando o `getCameraList` método no `deviceManager` objeto. Em seguida, crie um novo exemplo de `LocalVideoStream` passar a câmara desejada para o método `startVideo` como argumento:
...
* Para verificar se este participante remoto está ou não silenciado, inspecione `isMuted` a propriedade:
```js
...
* Para verificar se este participante remoto está ou não a falar, inspecione `isSpeaking` a propriedade:
```js
...
* Para inspecionar todos os streams de vídeo que um determinado participante está enviando nesta chamada, verifique `videoStreams` a recolha:
```js
...
Tem de passar por um dos tipos de 'identificador' Isto resolverá assíncronos assim que o participante for removido da chamada.
...
Ao criar um `RendererView` pode especificar `scalingMode` e `mirrored` propriedades.
O modo de escala pode ser 'Alongamento', 'Crop', ou 'Fit' Se `Mirrored` for especificado, o fluxo renderizado será virado verticalmente.
...
Pode verificar o estado de permissão atual para um determinado tipo, `getPermissionState` chamando:
