---
title: Utilização das equipas de serviços de comunicação Azure Incorporadas para Android
description: Nesta visão geral, você vai aprender a usar a biblioteca Azure Communication Services Teams Embed para Android.
author: palatter
ms.author: palatter
ms.date: 24/02/2021
ms.topic: conceptual
...
Adicione a `MeetingEventListener` sua aula.
...
Coloque o `MeetingEventListener` in `this` .
...
Implementar os `onCallStateChanged` métodos e `onRemoteParticipantCountChanged` métodos.
...
Adicione a `MeetingIdentityProvider` sua aula.
...
Coloque o `MeetingIdentityProvider` in `this` .
...
Mapear cada um `userMri` para o avatar correspondente.
