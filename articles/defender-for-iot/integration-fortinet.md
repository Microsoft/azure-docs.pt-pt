...
## <a name="the-defender-for-iot-platform"></a>O Defender para a plataforma IoT
...
:::image type="content" source="media/integration-fortinet/firewall.png" alt-text="Vista da vista da janela fortiGate Firewall.":::
...
1. Guarde a chave API.

    :::image type="content" source="media/integration-fortinet/api-key.png" alt-text="A descrição do telemóvel gera automaticamente nova chave API":::

## <a name="block-suspected-traffic-using-the-fortigate-firewall"></a>Bloquear tráfego suspeito usando a firewall FortiGate

1. No painel esquerdo, selecione **Forwarding**.

    [:::image type="content" source="media/integration-fortinet/forwarding-view.png" alt-text="A opção de janela de encaminhamento num sensor.":::](media/integration-fortinet/forwarding-view.png#lightbox)

1. Selecione **Criar Regras de Encaminhamento** e definir parâmetros de regras.

    :::image type="content" source="media/integration-fortinet/new-rule.png" alt-text="Screenshot da janela de regra de encaminhamento criar":::
    ...
    | **Protocolos** | Por predefinição, todos os protocolos são selecionados.<br><br>Para selecionar um protocolo específico, selecione **Specific** e selecione o protocolo para o qual esta regra é aplicada. |
    | **Motores** | Por defeito, todos os motores de segurança estão envolvidos.<br><br>Para selecionar um motor de segurança específico para o qual esta regra é aplicada, selecione **Specific** e selecione o motor. |
    | **Notificações do Sistema** | Sensor avançado *on-line e offline.* Esta opção só está disponível se tiver iniciado sessão na consola de gestão no local. |

1. Para instruir o Defender para ioT a enviar regras de firewall para FortiGate, selecione **Ação** e, em seguida, selecione **Enviar para FortiGate**.
...
1. Para configurar a regra de encaminhamento do FortiGate:

    :::image type="content" source="media/integration-fortinet/configure.png" alt-text="Configure a janela 'Criar Reencaminhamento'":::

1. No painel **de ações,** definir os seguintes parâmetros:

    | Parâmetro | Descrição |
    |--|--|
    | Anfitrião | O tipo de endereço IP do servidor FortiGate. |
    | Porta | O tipo de porta do servidor FortiGate. |
    | Nome de utilizador | O tipo de nome de utilizador do servidor FortiGate. |
    | Chave de API | Introduza a chave API que criou no FortiGate. |
    | Interface de entrada| Defina como o bloqueio é executado:<br /><br />**Por Endereço IP**: Cria sempre políticas de bloqueio no Panorama com base no endereço IP.<br /><br />**Por FQDN ou Endereço IP**: Cria políticas de bloqueio no Panorama com base em FQDN, se existir, caso contrário, endereço IP.| 
    | Interface de saída |Desa estade o endereço de e-mail para o e-mail de notificação de política. <br /><br /> **Nota:** Certifique-se de que configura um Servidor de Correio em XSense. Se não for inserido nenhum endereço de e-mail, o XSense não envia um e-mail de notificação.| 
    |Configurar| Configurar as seguintes opções para permitir o bloqueio das fontes suspeitas pela firewall FortiGate: <br /><br />**Bloquear códigos de função ilegais**: Violações de protocolo - Valor de campo ilegal violando especificação do protocolo ICS (potencial exploração)<br /><br />**Bloquear atualizações não autorizadas de programação plc /firmware**: Alterações não autorizadas de PLC<br /><br />**Bloquear paragem de PLC não autorizada**: paragem plc (tempo de inatividade)<br /><br />**Bloquear alertas relacionados com malware**: Bloqueio das tentativas de malware industrial (TRITON, NotPetya, etc.). Pode selecionar a opção de **bloqueio automático.** Nesse caso, o bloqueio é executado automaticamente e imediatamente.<br /><br />*Bloquear digitalização não autorizada*: Digitalização não autorizada (reconhecimento potencial)
