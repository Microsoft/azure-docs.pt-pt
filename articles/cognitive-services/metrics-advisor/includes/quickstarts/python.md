...
| [Métrica](https://azuresdkdocs.blob.core.windows.net/$web/python/azure-ai-metricsadvisor/latest/azure.ai.metricsadvisor.models.html?highlight=metric#azure.ai.metricsadvisor.models.Metric) | A `Metric` é uma medida quantificável que é usada para monitorizar e avaliar o estado de um processo de negócio específico. Pode ser uma combinação de múltiplos valores de séries de tempo divididos em dimensões. Por exemplo, uma métrica de saúde web pode conter dimensões para a contagem de utilizadores e para o mercado en-us. |
...
Num novo método, crie declarações de importação como o exemplo abaixo. `metric_id`Substitua-o pela métrica que pretende configurar. Crie um cliente com as suas chaves e ponto final e use `client.create_metric_anomaly_detection_configuration` para criar uma nova configuração de deteção. As condições de limiar especificam os parâmetros para a deteção de anomalias.
...
Num novo método, crie declarações de importação como o exemplo abaixo. `anomaly_detection_configuration_id`Substitua-o pela configuração de deteção de anomalias e `hook_id` substitua-o pelo gancho que criou anteriormente. Crie um cliente com as suas chaves e ponto final e use `client.create_anomaly_alert_configuration()` para criar uma configuração de alerta. `metric_alert_configurations` é uma lista de `MetricAlertConfiguration` objetos que especificam as condições e o alcance de cada configuração.
...
Num novo método, crie uma declaração de importação como o exemplo abaixo. `alert_id`Substitua-o pelo ID para o seu alerta e `alert_config_id` substitua-o pelo ID de configuração de alerta. Crie um cliente com as suas chaves e ponto final e use `client.list_anomalies_for_alert()` para listar uma configuração de alerta. 
