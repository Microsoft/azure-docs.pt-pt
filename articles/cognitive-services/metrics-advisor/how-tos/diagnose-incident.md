...
A definição **de Dimensões** é uma lista de dimensões para um incidente, pode selecionar outros valores de dimensão disponíveis para cada um. Depois de alterados os valores de dimensão. A **definição de Timestamp** permite visualizar o incidente atual em diferentes momentos do tempo.
...
Selecione uma série a partir do seu feed de dados. Pode escolher a mesma granularidade ou outra. Selecione as dimensões do alvo e carregue a tendência da série e, em seguida, clique em **Ok** para compará-la com uma série anterior. A série será montada numa única visualização. Pode continuar a adicionar mais séries para comparação e obter mais informações. Clique no menu drop down no topo do separador **ferramentas Compare** para comparar os dados da série de tempo durante um período de tempo, deslocados.  

> [!Warning]
> Para permitir uma comparação deslocada, a granularidade dos seus dados deve apoiá-lo. Por exemplo, se os seus dados forem semanais e utilizar o **Dia ao longo do dia,** não obterá resultados. Neste exemplo, em vez disso, utilizaria o **Mês ao longo do mês.**

Depois de selecionar uma comparação deslocada, pode selecionar se pretende comparar os valores de dados, os valores delta ou a percentagem delta.
