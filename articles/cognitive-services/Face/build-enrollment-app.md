---
title: Construa uma app React para adicionar utilizadores a um serviço Face
titleSuffix: Azure Cognitive Services
description: Saiba como configurar o seu ambiente de desenvolvimento e implementar uma aplicação Face para obter o consentimento dos clientes.
...
ms.locfileid: "107505110"
---
# <a name="build-a-react-app-to-add-users-to-a-face-service"></a>Construa uma app React para adicionar utilizadores a um serviço Face

Este guia irá mostrar-lhe como começar com a aplicação de inscrição facial da amostra. A aplicação demonstra as melhores práticas para obter um consentimento significativo para adicionar os utilizadores a um serviço de reconhecimento facial e adquirir dados faciais de alta precisão. Um sistema integrado poderia utilizar uma aplicação como esta para fornecer controlo de acesso sem toque, verificação de identidade, rastreio de assiduidade, quiosque de personalização ou verificação de identidade, com base nos seus dados faciais.
...
A aplicação da amostra é escrita usando o JavaScript e a estrutura React Native. Atualmente pode ser implantado em dispositivos Android; mais opções de implantação estão vindo no futuro.
...
1. Clone o repositório de git para a [aplicação](https://github.com/azure-samples/cognitive-services-FaceAPIEnrollmentSample)da amostra .
...
## <a name="create-a-user-add-experience"></a>Criar uma experiência de adicionar ao utilizador  

Agora que criou a aplicação de amostras, pode adaptá-la às suas próprias necessidades.
...
O serviço fornece verificações de qualidade de imagem para ajudá-lo a escolher se a imagem é de qualidade suficiente para adicionar o cliente ou tentar o reconhecimento facial. Esta aplicação demonstra como aceder a quadros a partir da câmara do dispositivo, selecionar os quadros de maior qualidade e adicionar a face detetada no serviço API face. 
...
Note que a aplicação também oferece funcionalidade para eliminar as informações do utilizador e a opção de novo add.

> [!div class="mx-imgBorder"]
> ![página de gestão de perfis](./media/enrollment-app/10-manage-2.jpg)

Para alargar a funcionalidade da aplicação para cobrir toda a experiência, leia a [visão geral](enrollment-overview.md) para funcionalidades adicionais para implementar e as melhores práticas.
...
Neste guia, aprendeu a configurar o seu ambiente de desenvolvimento e a começar com a aplicação da amostra. Se é novo em Reagir Nativo, pode ler os seus médicos para aprender mais informações sobre [antecedentes.](https://reactnative.dev/docs/getting-started) Também pode ser útil familiarizar-se com [a Face API.](Overview.md) Leia as outras secções sobre a adição de utilizadores antes de começar a ser desenvolvimento.