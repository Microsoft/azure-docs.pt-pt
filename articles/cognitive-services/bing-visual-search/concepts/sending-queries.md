...
description: Saiba mais sobre os parâmetros da API REST usados no API da Pesquisa Visual do Bing.
...
O objeto `imageInfo` tem de incluir o campo `url` ou `imageInsightsToken`, não ambos. Defina o `url` campo como a URL de uma imagem acessível pela Internet. O tamanho máximo de imagem suportado é 1 MB.
