...
Os rótulos `positive` `negative` são, e `neutral` . A nível de documento, a `mixed` etiqueta de sentimento também pode ser devolvida. O sentimento do documento é determinado a seguir:
