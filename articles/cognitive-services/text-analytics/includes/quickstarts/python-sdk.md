...
Criar uma nova função chamada `analyze_batch_example()` que leva o cliente como um argumento, e depois chama a `begin_analyze_batch_actions()` função. O resultado será uma operação de longa duração que será sondada para obter resultados.
