...
No objeto cliente, crie uma função chamada `DetectLanguage()` que usa uma lista de documentos de entrada que serão criados posteriormente. Chame a função de `detect_language()` do cliente e obtenha o resultado. Em seguida, itere os resultados e imprima a ID de cada documento e a linguagem detectada.
