...
Criar uma nova função chamada `analyzeOperationExample()` , que chama a `beginAnalyzeTasks()` função. O resultado será uma operação de longa duração que será sondada para obter resultados.
...
Também pode utilizar a operação De analisar para detetar pii e extração de frases-chave. Veja a [amostra de Análise](https://github.com/Azure/azure-sdk-for-java/blob/master/sdk/textanalytics/azure-ai-textanalytics/src/samples/java/com/azure/ai/textanalytics/lro/AnalyzeTasksAsync.java) no GitHub.
