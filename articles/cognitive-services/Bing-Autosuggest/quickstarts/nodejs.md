...
2. Criar uma função chamada `get_suggestions()` . Utilize as variáveis dos últimos passos para formatar um URL de pesquisa para o pedido de API. O seu termo de pesquisa deve ser codificado por URL antes de ser enviado para a API.
...
1. Defina uma função nomeada `response_handler` que recebe uma chamada HTTP, como `response` parâmetro. Faça os seguintes passos dentro desta função:
    ...
    2. Guarde o corpo da resposta quando a `data` bandeira for chamada
    ...
    3. Quando uma `end` bandeira é sinalizada, use `JSON.parse()` e `JSON.stringify()` imprima a resposta.
...
2. Ligue `get_suggestions()` para enviar o pedido para a Bing Autosuggest API.
