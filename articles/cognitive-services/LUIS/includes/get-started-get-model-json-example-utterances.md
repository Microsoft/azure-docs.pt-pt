...
Se a matriz entityLabels não estiver vazia, `startCharIndex` e `endCharIndex` têm de marcar a entidade referida no campo `entityName`. O índice é baseado em zero. Se iniciar ou terminar a etiqueta num espaço do texto, a chamada da API para adicionar as palavras falha.
