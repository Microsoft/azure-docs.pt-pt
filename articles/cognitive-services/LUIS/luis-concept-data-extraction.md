...
Defina o parâmetro de `verbose=true`corda de consulta, . A resposta final é:
...
Defina o parâmetro de `show-all-intents=true`corda de consulta, . A resposta final é:
...
A maioria dos chatbots e aplicações precisam de mais do que o nome da intenção. Estes dados adicionais e opcionais provêm de entidades descobertas na expressão. Cada tipo de entidade devolve informações diferentes sobre a partida.
...
## <a name="simple-entity-data"></a>Dados simples da entidade

Uma [entidade simples](reference-entity-simple.md) é um valor aprendido por máquinas. Pode ser uma palavra ou frase.

## <a name="composite-entity-data"></a>Dados de entidades compósitas
...
Sem o parâmetro de `verbose=true`corda de consulta, :
...
Com o parâmetro de `verbose=true`corda de consulta, :
...
## <a name="pattern-roles-data"></a>Dados de papéis de padrão
Os papéis são diferenças contextuais de entidades.


#### <a name="v2-prediction-endpoint-response"></a>[Resposta final de previsão V2](#tab/V2)

O nome `Location`da entidade `Origin` é, com duas funções, e `Destination`.
...
Luis devolve todas as entidades descobertas na expressão. Como resultado, o seu chatbot pode ter de tomar uma decisão com base nos resultados. Uma expressão pode ter muitas entidades numa expressão:
...
Para a `when is the best time to go to red rock?`consulta , e a `red` app tem a palavra em mais de uma lista, a LUIS reconhece todas as entidades e devolve um conjunto de entidades como parte da resposta do ponto final da JSON:
...
Sem `verbose=true` a corda de consulta:
