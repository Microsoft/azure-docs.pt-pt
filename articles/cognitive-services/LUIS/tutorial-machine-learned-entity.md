...
ms.locfileid: "73828528"
---
# <a name="tutorial-extract-structured-data-with-machine-learned-entities-in-language-understanding-luis"></a>Tutorial: extrair dados estruturados com entidades aprendidas por computador no Reconhecimento vocal (LUIS)
...
Em termos práticos, você criará uma entidade aprendida por computador para representar um pedido para um aplicativo de pizza. A ordem deve ter todas as partes necessárias para fullfil o pedido. Para começar, a entidade incluirá todo o texto relacionado à ordem e, especificamente, o tamanho e a quantidade de pull. 

Um expressão para `deliver one large cheese pizza` deve extrair toda a expressão como a ordem, e também extrair `1` e `large`. 

Há uma decomposição adicional que você pode fazer, como ingredientes ou crust. Após este tutorial, você deve sentir confiante para adicionar esses subcomponentes à sua entidade de `Order` existente.
...
1. Na seção **gerenciar** , na guia **versões** , selecione a versão, selecione **clonar** para clonar a versão e nomeie-a `mach-learn`. Em seguida, selecione **concluído** para concluir o processo de clonagem. Como o nome da versão é utilizado como parte da rota de URL, o nome não pode conter carateres que não sejam válidos num URL.
    ...
    > [!TIP] 
    > A clonagem é uma prática recomendada antes de modificar seu aplicativo. Quando terminar com uma versão, exporte uma versão, como um arquivo. JSON ou. Lu, e verifique isso em seu controle do código-fonte.

1. Selecione **Compilar** e, em seguida, para ver os principais blocos de construção de um aplicativo Luis, **as intenções.**
...
1. Na lista declarações de exemplo, selecione o seguinte expressão. 
    ...
    > [!NOTE]
    > Uma entidade nem sempre será a expressão inteira. Nesse caso específico, `pickup` indica como o pedido deve ser recebido para que ele faça parte da entidade rotulada para o pedido. 

1. Na caixa **escolher um tipo de entidade** , selecione **Adicionar estrutura** e selecione **Avançar**. A estrutura é necessária para permitir subcomponentes, como tamanho e quantidade.
...
1. Na caixa **criar novo descritor de lista de frases** , digite o nome `SizeDescriptor` Insira os valores de: `small`, `medium`e `large`. Quando a caixa **sugestões** estiver preenchida, selecione `extra large`e `xl`. Selecione **concluído** para criar a nova lista de frases. 
    ...
    Esse descritor de lista de frases ajuda o subcomponente `Size` a localizar palavras relacionadas ao tamanho, fornecendo-a com o Word de exemplo. Essa lista não precisa incluir todas as palavras de tamanho, mas deve incluir palavras que são esperadas para indicar o tamanho. 
...
1. Rotule a entidade de `Order` no declarações restante junto com a entidade de tamanho. Os colchetes no texto indicam a entidade de `Order` rotulada e a entidade de `Size` no.
    ...
    > [!CAUTION]
    > Como tratar dados implícitos, como a letra `a` implicando em uma pizza única? Ou a falta de `pickup` e `delivery` para indicar onde a pizza é esperada? Ou a falta de um tamanho para indicar seu tamanho padrão ou pequeno ou grande? Considere o tratamento de dados implícitos tratados como parte de suas regras de negócios no aplicativo cliente. 
...
1. Após o treinamento, adicione um novo exemplo de expressão para entender como LUIS entende a entidade aprendida com o computador. 
...
1. Na caixa **adicionar entidades predefinidas** , procure e selecione **número** e, em seguida, selecione **concluído**. 
    ...
    ![Adicionar entidade predefinida](media/tutorial-machine-learned-entity/add-prebuilt-entity-as-constraint-to-quantity-subcomponent.png)
...
## <a name="label-example-utterance-with-subcomponent-for-quantity-to-teach-luis-about-the-entity"></a>Exemplo de rótulo expressão com subcomponente para a quantidade para ensinar LUIS sobre a entidade
