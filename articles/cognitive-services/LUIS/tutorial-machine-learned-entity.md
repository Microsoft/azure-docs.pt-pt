...
1. Na lista de palavras de exemplo, selecione as seguintes expressões.
    ...
    Comece a selecionar pouco antes do texto mais à esquerda `pickup` de (#1), em seguida, vá um pouco além do texto mais à direita , `anchovies` (#2 - isto termina o processo de rotulagem). Aparece um menu pop-up. Na caixa pop-up, insira o nome da entidade como `Order` (#3). Em seguida, selecione `Order Create new entity` a partir da lista (#4).
...
1. Para alterar a previsão numa entidade rotulada, selecione a marca de verificação na mesma linha.
    ...
    > [!div class="mx-imgBorder"]
    > ![Screenshot parcial de novo exemplo de expressão previsto com entidade](media/tutorial-machine-learned-entity/confirm-entity-prediction-for-new-example-utterance-added.png)
