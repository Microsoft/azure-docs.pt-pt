...
4. Adicione várias expressões que indiquem que um colaborador está a fazer algo bem ou uma área que precisa de ser melhorada:
    ...
    Selecione o **ver opções**, selecione **Mostrar valores de entidade** para ver os nomes.

    [![Aplicação de captura de ecrã do LUIS com expressões de exemplo em intenção, EmployeeFeedback](./media/luis-quickstart-intent-and-sentiment-analysis/hr-utterance-examples.png)](./media/luis-quickstart-intent-and-sentiment-analysis/hr-utterance-examples.png#lightbox)
...
1. Selecione **utilizar análise de sentimentos para determinar se a expressão de um utilizador é neutro, positivo ou negativo.** Para ativar esta definição. 
...
1. Vá para o final do URL o endereço e introduza a seguinte expressão:
    ...
    Tente outra expressão ao remover o valor para `q` na barra de endereço do browser: `William Jones did a terrible job presenting his ideas.` A classificação de sentimento indica um sentimento negativo, retornando uma pontuação baixa `0.18597582`.
