...
Para habilitar um limite de aviso, ele deve ser especificado em um objeto `SpeechConfig`. Esse objeto é verificado quando um novo reconhecedor é criado. Nos exemplos a seguir, vamos supor que você criou uma instância de `SpeechConfig` chamada `config`:
...
Para habilitar um limite de erro, ele deve ser especificado em um objeto `SpeechConfig`. Esse objeto é verificado quando um novo reconhecedor é criado. Nos exemplos a seguir, vamos supor que você criou uma instância de `SpeechConfig` chamada `config`:
...
> [!TIP]
> O valor padrão dessa propriedade é o valor máximo específico da plataforma para um tipo de dados `size_t`. Um reconhecimento típico consumirá entre 7 e 10 objetos internos.
