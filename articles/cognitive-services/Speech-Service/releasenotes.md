...
- **C++/C#/Java**: Faça o seu assistente de voz ou bot parar de ouvir imediatamente. `DialogServiceConnector`[(C++](/cpp/cognitive-services/speech/dialog-dialogserviceconnector) [, C#](/dotnet/api/microsoft.cognitiveservices.speech.dialog.dialogserviceconnector?view=azure-dotnet), [Java)](/java/api/com.microsoft.cognitiveservices.speech.dialog.dialogserviceconnector?view=azure-java-stable)agora tem um `StopListeningAsync()` método para `ListenOnceAsync()` acompanhar. Isto irá parar imediatamente a captura de áudio e esperar graciosamente por um resultado, tornando-o perfeito para ser usado com cenários de pressão de botões "stop now".
- **C++/C#/Java/JavaScript:** Faça o seu assistente de voz ou bot reagir melhor aos erros subjacentes do sistema. `DialogServiceConnector`[(C++](/cpp/cognitive-services/speech/dialog-dialogserviceconnector) [, C#](/dotnet/api/microsoft.cognitiveservices.speech.dialog.dialogserviceconnector?view=azure-dotnet), [Java,](/java/api/com.microsoft.cognitiveservices.speech.dialog.dialogserviceconnector?view=azure-java-stable) [JavaScript)](/javascript/api/microsoft-cognitiveservices-speech-sdk/dialogserviceconnector?view=azure-node-latest)tem agora um novo `TurnStatusReceived` manipulador de eventos. Estes eventos opcionais correspondem a todas as [`ITurnContext`](/dotnet/api/microsoft.bot.builder.iturncontext?view=botbuilder-dotnet-stable) resoluções sobre o Bot e reportarão falhas de execução quando ocorrerem, por exemplo, como resultado de uma exceção, tempo limite ou queda de rede entre o Direct Line Speech e o bot. `TurnStatusReceived` facilita a resposta às condições de insucesso. Por exemplo, se um bot demorar muito tempo numa consulta de base de dados de backend (por exemplo, procurar um produto), `TurnStatusReceived` permite que o cliente saiba repromptar com "desculpe, eu não consegui isso, poderia tentar novamente" ou algo semelhante.
...
- **Java**: [`DialogServiceConnector`](/java/api/com.microsoft.cognitiveservices.speech.dialog.dialogserviceconnector?view=azure-java-stable) agora tem um método que foi excluído `setSpeechActivityTemplate()` involuntariamente da língua anteriormente. Isto equivale à definição da `Conversation_Speech_Activity_Template` propriedade e solicitará que todas as futuras atividades do Quadro Bot originadas pelo serviço Direct Line Speech fundam o conteúdo fornecido nas suas cargas JSON.
- **Java**: Melhor depuração de baixo nível. A [`Connection`](/java/api/com.microsoft.cognitiveservices.speech.connection?view=azure-java-stable) classe tem agora um `MessageReceived` evento, semelhante a outros idiomas de programação (C++, C#). Este evento fornece acesso de baixo nível aos dados recebidos do serviço e pode ser útil para diagnósticos e depurações.
