...
## <a name="neural-and-custom-voices"></a>Vozes neurais e personalizadas

Use uma voz neural semelhante a um homem, ou crie a sua própria voz personalizada única ao seu produto ou marca. Para obter uma lista completa de línguas, locais e vozes apoiadas, consulte [o suporte linguístico.](language-support.md) Para saber mais sobre vozes neurais e personalizadas, consulte [a visão geral do texto para a fala](text-to-speech.md).
...
> [!NOTE]
> Este exemplo usa a `en-US-JennyNeural` voz. Para obter uma lista completa de vozes apoiadas, consulte [o suporte linguístico](language-support.md#text-to-speech).
...
Para mais informações, consulte. <a href="/dotnet/api/microsoft.cognitiveservices.speech.speechconfig.setproperty" target="_blank"> `SetProperty` </a>
...
Para mais informações, consulte. <a href="/cpp/cognitive-services/speech/speechconfig#setproperty" target="_blank"> `SetProperty` </a>
...
Para mais informações, consulte. <a href="/java/api/com.microsoft.cognitiveservices.speech.speechconfig.setproperty#com_microsoft_cognitiveservices_speech_SpeechConfig_setProperty_String_String_" target="_blank"> `setProperty` </a>
...
Para mais informações, consulte. <a href="/python/api/azure-cognitiveservices-speech/azure.cognitiveservices.speech.speechconfig#set-property-by-name-property-name--str--value--str-" target="_blank"> `set_property_by_name` </a>
...
Para mais informações, consulte. <a href="/javascript/api/microsoft-cognitiveservices-speech-sdk/speechconfig#setproperty-string--string-" target="_blank"> `setProperty` </a>
...
Para mais informações, consulte. <a href="/objectivec/cognitive-services/speech/spxspeechconfiguration#setpropertytobyname" target="_blank"> `setPropertyTo` </a>
...
Por predefinição, o serviço de texto-a-fala sintetiza o texto usando um estilo neutro de fala para vozes neurais. Você pode ajustar o estilo de fala para expressar diferentes emoções como alegria, empatia e calma, ou otimizar a voz para diferentes cenários como atendimento ao cliente, newscasting e assistente de voz, usando o `mstts:express-as` elemento. Este é um elemento opcional exclusivo do serviço Discurso.

Atualmente, os ajustes de estilo de fala são suportados para as seguintes vozes neurais:
* `en-US-AriaNeural`
* `en-US-JennyNeural`
* `en-US-GuyNeural`
...
A intensidade do estilo de fala pode ser alterada para melhor se adaptar ao seu caso de utilização. Pode especificar um estilo mais forte ou mais suave para `styledegree` tornar o discurso mais expressivo ou moderado. Atualmente, os ajustes de estilo de fala são suportados para vozes neurais chinesas (mandarim, simplificadas).

Além de ajustar os estilos de fala e o grau de estilo, também pode ajustar o `role` parâmetro para que a voz imite uma idade e género diferentes. Por exemplo, uma voz masculina pode levantar o tom e mudar a entoação para imitar uma voz feminina, mas o nome da voz não será alterado. Atualmente, os ajustes de função são suportados para estas vozes neurais chinesas (mandarim, simplificadas):
* `zh-CN-XiaomoNeural`
* `zh-CN-XiaoxuanNeural`

Acima as alterações são aplicadas ao nível da frase, e os estilos e role-plays variam por voz. Se um estilo ou role-play não for suportado, o serviço devolverá a fala da forma neutra padrão. Pode ver que estilos e funções são suportados por cada voz através da [lista de voz API](rest-text-to-speech.md#get-a-list-of-voices) ou através da plataforma [De criação](https://aka.ms/audiocontentcreation) de conteúdos áudio sem código.
...
```
> [!NOTE]
> No momento, `styledegree` apenas suporta vozes neurais chinesas (mandarim, simplificadas). `role` apenas apoia zh-CN-XiaomoNeural e zh-CN-XiaoxuanNeural.
...
Utilize esta tabela para verificar as funções apoiadas e as suas definições.

|Função                     | Description                |
|-------------------------|----------------------------|
|`role="Girl"`            | A voz imita uma rapariga. |
|`role="Boy"`             | A voz imita um rapaz. |
|`role="YoungAdultFemale"`| A voz imita uma jovem mulher adulta.|
|`role="YoungAdultMale"`  | A voz imita a um jovem adulto.|
...
|`role="SeniorMale"`      | A voz imita a um homem sénior.|
...
| Atributo | Descrição | Obrigatório / Opcional |
|-----------|-------------|---------------------|
| `type` | Especifica a localização do silêncio: <ul><li>`Leading` – no início do texto </li><li>`Tailing` – no final do texto </li><li>`Sentenceboundary` – entre frases adjacentes </li></ul> | Necessário |
...
O `lexicon` elemento contém pelo menos um `lexeme` elemento. Cada `lexeme` elemento contém pelo menos um elemento e um ou `grapheme` `grapheme` `alias` mais, e `phoneme` elementos. O `grapheme` elemento contém texto que descreve a <a href="https://www.w3.org/TR/pronunciation-lexicon/#term-Orthography" target="_blank">orthografia. </a> Os `alias` elementos são usados para indicar a pronúncia de um acrónimo ou um termo abreviado. O `phoneme` elemento fornece texto que descreve como o `lexeme` pronuncia-se.
...
Pode utilizar o `sapi` valor para o atributo com `alphabet` léxicos personalizados, conforme demonstrado abaixo:
...
## <a name="bookmark-element"></a>Elemento de marca de livros

O elemento marcador permite-lhe inserir marcadores personalizados em SSML para obter a compensação de cada marcador no fluxo de áudio.
Não vamos ler os elementos do marcador.
...
| Atributo | Descrição                                   | Obrigatório / Opcional                                        |
|-----------|-----------------------------------------------|------------------------------------------------------------|
|  `mark`   | Especifica o texto de referência do `bookmark` elemento. | Obrigatório. |

**Exemplo**

Como exemplo, talvez queira saber o tempo de compensação de cada palavra de flor como seguinte
...
### <a name="get-bookmark-using-speech-sdk"></a>Obtenha o marcador usando o Speech SDK

Pode subscrever o `BookmarkReached` evento na Speech SDK para obter as compensações do marcador.

> [!NOTE]
> `BookmarkReached` evento só está disponível desde a versão 1.16.0 do Speech SDK.

`BookmarkReached` os eventos são levantados à medida que os dados de áudio de saída ficam disponíveis, o que será mais rápido do que a reprodução de um dispositivo de saída.

* `AudioOffset` relata o tempo decorrido do áudio de saída entre o início da síntese e o elemento marcador. Isto é medido em unidades de cem nanossegundos (HNS) com 10.000 HNS equivalentes a 1 milissegundo.
* `Text` é o texto de referência do elemento marcador, que é a corda que definiu no `mark` atributo.

# <a name="c"></a>[C#](#tab/csharp)

Para mais informações, consulte. <a href="https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechsynthesizer.bookmarkreached" target="_blank"> `BookmarkReached` </a>
...
Para o exemplo SSML acima, o `BookmarkReached` evento será desencadeado duas vezes, e a saída da consola será
```text
...
Para mais informações, consulte. <a href="https://docs.microsoft.com/cpp/cognitive-services/speech/speechsynthesizer#bookmarkreached" target="_blank"> `BookmarkReached` </a>
...
Para mais informações, consulte. <a href="https://docs.microsoft.com/java/api/com.microsoft.cognitiveservices.speech.speechsynthesizer.bookmarkReached#com_microsoft_cognitiveservices_speech_SpeechSynthesizer_BookmarkReached" target="_blank"> `BookmarkReached` </a>
...
Para mais informações, consulte. <a href="https://docs.microsoft.com/python/api/azure-cognitiveservices-speech/azure.cognitiveservices.speech.speechsynthesizer#bookmark-reached" target="_blank"> `bookmark_reached` </a>
...
Para o exemplo SSML acima, o `bookmark_reached` evento será desencadeado duas vezes, e a saída da consola será
```text
Bookmark reached, audio offset: 825ms, bookmark text: flower_1.
Bookmark reached, audio offset: 1462.5ms, bookmark text: flower_2.
```
...
Para mais informações, consulte. <a href="https://docs.microsoft.com/javascript/api/microsoft-cognitiveservices-speech-sdk/speechsynthesizer#bookmarkReached" target="_blank"> `bookmarkReached` </a>
...
Para mais informações, consulte. <a href="https://docs.microsoft.com/objectivec/cognitive-services/speech/spxspeechsynthesizer#addbookmarkreachedeventhandler" target="_blank"> `addBookmarkReachedEventHandler` </a>
...
Para mais informações, consulte. <a href="https://docs.microsoft.com/swift/cognitive-services/speech/spxspeechsynthesizer#addbookmarkreachedeventhandler" target="_blank"> `addBookmarkReachedEventHandler` </a>
