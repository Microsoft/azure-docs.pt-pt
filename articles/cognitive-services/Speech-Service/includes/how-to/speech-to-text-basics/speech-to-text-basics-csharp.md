...
Depois de criar um [`SpeechConfig`, ](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechconfig?view=azure-dotnet)o próximo passo é inicializar um [`SpeechRecognizer`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechrecognizer?view=azure-dotnet). Quando rubricar um [`SpeechRecognizer`, ](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechrecognizer?view=azure-dotnet)terá de passar o seu `speechConfig`. Isto fornece as credenciais que o serviço de fala requer para validar o seu pedido.

Se está a reconhecer a fala usando o microfone padrão do seu dispositivo, eis como deve ser o [`SpeechRecognizer`:](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechrecognizer?view=azure-dotnet)

```csharp
using var recognizer = new SpeechRecognizer(speechConfig);
```

Se pretender especificar o dispositivo de entrada de áudio, terá de criar uma [`AudioConfig`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.audio.audioconfig?view=azure-dotnet) e fornecer o parâmetro `audioConfig` ao rubricar a sua [`SpeechRecognizer`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechrecognizer?view=azure-dotnet).

> [!TIP]
> [Aprenda a obter o ID do dispositivo para o seu dispositivo](../../../how-to-select-audio-input-devices.md)de entrada de áudio .

Primeiro, adicione a seguinte declaração `using`.
...
A [classe Recogniser](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechrecognizer?view=azure-dotne) para o C# SDK da Fala para expor alguns métodos que pode usar para reconhecimento da fala.
...
* Reconhecimento contínuo (assincron) - Inicia assincronicamente o funcionamento de reconhecimento contínuo. O utilizador regista-se em eventos e trata de vários estados de aplicação. Para parar o reconhecimento contínuo assíncrono, chame [`StopContinuousRecognitionAsync`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechrecognizer.stopcontinuousrecognitionasync?view=azure-dotnet).
...
Aqui está um exemplo de reconhecimento assíncrono usando [`RecognizeOnceAsync`: ](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechrecognizer.recognizeonceasync?view=azure-dotnet)

```csharp
var result = await recognizer.RecognizeOnceAsync();
```

Tens de escrever um código para lidar com o resultado. Esta amostra avalia o [`result.Reason`: ](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.recognitionresult.reason?view=azure-dotnet)
...
Comecemos por definir a entrada e rubricar um [`SpeechRecognizer`: ](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechrecognizer?view=azure-dotnet)

```csharp
using var audioConfig = AudioConfig.FromWavFileInput("YourAudioFile.wav");
using var recognizer = new SpeechRecognizer(speechConfig, audioConfig);
```
Em seguida, vamos criar uma variável para gerir o estado de reconhecimento da fala. Para começar, declaramos uma `TaskCompletionSource<int>` após as declarações anteriores.
...
Com tudo preparado, podemos [chamá`StopContinuousRecognitionAsync`. ](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechrecognizer.stopcontinuousrecognitionasync?view=azure-dotnet)
...
Uma tarefa comum para o reconhecimento da fala é especificar a linguagem de entrada (ou fonte). Vamos ver como mudarias a linguagem de entrada para italiano. No seu código, encontre o seu [`SpeechConfig`, ](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechconfig?view=azure-dotnet)em seguida, adicione esta linha diretamente abaixo dele.

```csharp
speechConfig.SpeechRecognitionLanguage = "it-IT";
```

A propriedade [`SpeechRecognitionLanguage`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.speechconfig.speechrecognitionlanguage?view=azure-dotnet) espera uma cadeia de formato local de linguagem. Pode fornecer qualquer valor na coluna **Locale** na lista de [locais/línguas](../../../language-support.md)suportados.
...
Para utilizar uma lista de frases, primeiro crie [um](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.phraselistgrammar?view=azure-dotnet)`PhraseListGrammar`objeto, em seguida, adicione palavras e frases específicas com [`AddPhrase`](https://docs.microsoft.com//dotnet/api/microsoft.cognitiveservices.speech.phraselistgrammar.addphrase?view=azure-dotnet).

Quaisquer alterações à [`PhraseListGrammar`](https://docs.microsoft.com/dotnet/api/microsoft.cognitiveservices.speech.phraselistgrammar?view=azure-dotnet) produzir efeitos sobre o próximo reconhecimento ou após uma reconexão ao serviço da Fala.
