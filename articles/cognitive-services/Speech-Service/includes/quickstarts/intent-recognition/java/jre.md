...
Antes de inicializar um objeto `IntentRecognizer`, você precisa criar uma configuração que usa sua chave e região LUIS Endpoing. Inserir este código no bloco try/catch no principal
...
A partir do objeto `IntentRecognizer`, você chamará o método `recognizeOnceAsync()`. Esse método permite que o serviço de fala saiba que você está enviando uma única frase para reconhecimento e que, assim que a frase for identificada para parar a fala Reconizing.
...
Neste ponto, seu código deve ter esta aparência: (adicionamos alguns comentários a esta versão) [!code-java[](~/samples-cognitive-services-speech-sdk/quickstart/java/jre/intent-recognition/src/speechsdk/quickstart/Main.java?range=6-76)]
