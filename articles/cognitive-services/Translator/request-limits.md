---
title: Limites de pedido - API de Texto tradutor
titleSuffix: Azure Cognitive Services
description: Este artigo lista os limites de pedido para a API de Texto tradutor. As acusações são incorridas com base na contagem de caracteres, não na frequência de pedidos com um limite de 5.000 caracteres por pedido. Os limites de caracteres são baseados em subscrição, com F0 limitado a 2 milhões de caracteres por hora.
...
Este artigo fornece limites de estrangulamento para a API de Texto tradutor. Os serviços incluem tradução, transliteração, deteção do comprimento da frase, deteção de linguagem e traduções alternativas.
...
A tabela seguinte enumera elementos de matriz e limites de caracteres para cada operação da API de Texto tradutor.
...
O seu limite de caracteres por hora baseia-se no seu nível de subscrição de Texto tradutor. 
...
| Idioma | Código | Limite de carácter |
|----------|------|-----------------|
| Chinês | zh | 132 |
| Alemão | de | 290 |
| Italiano | lo | 280 |
...
* [Referência da API de Texto do Microsoft Translator v3](https://docs.microsoft.com/azure/cognitive-services/translator/reference/v3-0-reference)
