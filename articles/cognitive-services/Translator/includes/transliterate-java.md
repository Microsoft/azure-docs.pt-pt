...
Adicione estas linhas à `Transliterate` aula. Em primeiro lugar, a chave de subscrição e o ponto final estão a ser lidos a partir de variáveis ambientais. Em seguida, você vai notar que, juntamente com os `api-version` dois parâmetros adicionais foram anexados ao `url` . Estes parâmetros são usados para definir a linguagem de entrada, e os scripts para transliteração. Nesta amostra, é definido para japonês `jpan` e latim ( ) e em latim ( `latn` ). 
...
Adicione esta linha à `Transliterate` classe para instantaneaizar o `OkHttpClient` seguinte:
...
Em seguida, vamos construir o pedido do POST. Sinta-se livre para alterar o texto para transliteração.
