...
Crie um novo projeto Go utilizando o seu IDE favorito ou editor ou nova pasta no seu ambiente de trabalho. Em seguida, copie este fragmento de código no seu projeto/pasta num ficheiro chamado `get-languages.go` .
...
Vamos criar a função principal para a nossa aplicação. Vai notar que é uma única linha de código. Isso é porque estamos a criar uma única função para obter e imprimir a lista de línguas suportadas para tradutor.
...
Copie este código na `getLanguages` função.
...
Adicione este código à `getLanguages` função para descodificar a resposta JSON e, em seguida, forte e imprima o resultado.
