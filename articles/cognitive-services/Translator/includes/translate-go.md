...
Vamos criar uma função para traduzir texto. Esta função terá um único argumento, a sua chave de subscrição tradutor.
...
Em seguida, vamos construir a URL. O URL é construído usando os `Parse()` `Query()` e métodos. Vai notar que os parâmetros são adicionados com o `Add()` método. Nesta amostra, estáa traduzir do inglês para o alemão e italiano: `de` `it` e.

Copie este código na `translate` função.
...
>[!NOTE]
> Para mais informações sobre pontos finais, rotas e parâmetros de pedido, consulte [Tradutor 3.0: Traduzir](https://docs.microsoft.com/azure/cognitive-services/translator/reference/v3-0-translate).

## <a name="create-a-struct-for-your-request-body"></a>Crie uma estrutura para o seu corpo de pedido

Em seguida, crie uma estrutura anónima para o corpo de pedido e codifique-a como JSON com `json.Marshal()` . Adicione este código à `translate` função.
...
Adicione este código à `translate` função para descodificar a resposta JSON e, em seguida, forte e imprima o resultado.
