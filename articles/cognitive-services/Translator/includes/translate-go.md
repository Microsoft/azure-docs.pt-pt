...
Crie um novo projeto Go usando seu IDE ou editor favorito. Em seguida, copie este fragmento de código para o seu projeto num ficheiro com o nome `translate-text.go`.
...
Vamos criar uma função para traduzir o texto. Essa função usará um único argumento, sua Tradução de Texto chave de assinatura.
...
Em seguida, vamos construir a URL. A URL é criada usando os `Parse()` métodos `Query()` e. Você observará que os parâmetros são adicionados com `Add()` o método. Neste exemplo, você está convertendo de Inglês para alemão e italiano: `de` e. `it`

Copie esse código para a `translate` função.
...
Em seguida, crie uma estrutura anônima para o corpo da solicitação e codifique-a `json.Marshal()`como JSON com. Adicione este código à `translate` função.
...
Adicione esse código à `translate` função para decodificar a resposta JSON e, em seguida, Formatar e imprimir o resultado.
