...
Em seguida, nesta pasta, `Translate.java`crie um ficheiro chamado .
...
Adicione estas linhas `Translate` à classe. Em primeiro lugar, a chave de subscrição e o ponto final estão a ser lidos a partir de variáveis ambientais. Em seguida, você vai notar `api-version`que, juntamente com os `url`dois parâmetros adicionais foram anexados ao . Estes parâmetros são utilizados para definir as saídas de tradução. Nesta amostra, está definido para`de`alemão (`it`) e italiano . 
...
Adicione esta linha `Translate` à classe `OkHttpClient`para instantaneamente:
