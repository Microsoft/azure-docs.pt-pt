...
Em seguida, nesta pasta, crie um ficheiro chamado `GetLanguages.java` .
...
Adicione estas linhas à `GetLanguages` aula. Você vai notar que a chave de subscrição e ponto final estão sendo lidos a partir de variáveis ambientais:
...
Adicione esta linha à `GetLanguages` classe para instantaneaizar o `OkHttpClient` seguinte:
...
Em seguida, vamos construir o `GET` pedido.
