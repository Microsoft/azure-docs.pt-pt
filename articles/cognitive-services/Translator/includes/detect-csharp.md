...
Dentro da `Program` classe, crie uma função chamada `DetectTextRequest()`. Essa classe encapsula o código usado para chamar o recurso de detecção e imprime o resultado no console do.
...
Em seguida, precisamos criar e serializar o objeto JSON que inclui o texto que passará pela detecção de idioma.
...
A última etapa é chamar `DetectTextRequest()` a `Main` função. Localize `static void Main(string[] args)` e substitua-o por este código:
