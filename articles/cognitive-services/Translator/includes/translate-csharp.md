...
O primeiro comando faz duas coisas. Cria uma nova aplicação de consola .NET, e cria um diretório chamado `translate-sample`. O segundo comando muda para o diretório para o seu projeto.
...
Este quickstart requer C# 7.1 ou mais tarde. Existem algumas formas de alterar a versão C# para o seu projeto. Neste guia, vamos mostrar-lhe como `translate-sample.csproj` ajustar o ficheiro. Para todas as opções disponíveis, tais como alterar o idioma no Estúdio Visual, consulte [Selecione a versão em língua C#](https://docs.microsoft.com/dotnet/csharp/language-reference/configure-language-version).

Abra o seu `translate-sample.csproj`projeto e abra. Certifique-se `LangVersion` de que está definido para 7.1 ou mais tarde. Se não houver um grupo imobiliário para a versão linguística, adicione estas linhas:
...
Em seguida, vamos criar um conjunto de aulas que são usadas ao desserializar a resposta JSON devolvida pela API de Texto tradutor.
...
Na `Program` aula, crie uma função assíncrona chamada `TranslateTextRequest()`. Esta função requer `subscriptionKey`quatro `host` `route`argumentos: `inputText`, , e .
...
O último passo `TranslateTextRequest()` é `Main` chamar a função. Nesta amostra, estamos traduzindo para alemão`de`,`it`italiano ,`ja`japonês`th`( e tailandês). Localize-o `static void Main(string[] args)` e substitua-o por este código:
...
Vais `Main`reparar nisso, estás a `subscriptionKey` `endpoint`declarar, `route`e. Além disso, está a pedir ao `Console.Readline()` utilizador para que `textToTranslate`entre e atribua o valor a .
