...
Dentro da `Program` classe, crie uma função assíncrona `TranslateTextRequest()`chamada. Essa função usa quatro argumentos: `subscriptionKey` `route`, `host`, e `inputText`.
...
Você observará que, `Main`em, está declarando `subscriptionKey`, `route` `host`e. Além disso, você está solicitando que o usuário insira `Console.Readline()` e atribua o `textToTranslate`valor a.
