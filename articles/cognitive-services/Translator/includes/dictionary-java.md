...
Em seguida, nesta pasta, `AltTranslation.java`crie um ficheiro chamado .
...
Adicione estas linhas `AltTranslation` à classe. Em primeiro lugar, a chave de subscrição e o ponto final estão a ser lidos a partir de variáveis ambientais. Em seguida, você vai notar `api-version`que, juntamente com os `url`dois parâmetros adicionais foram anexados ao . Estes parâmetros são utilizados para definir a entrada e saída de tradução. Nesta amostra, estes são`en`ingleses`es`e espanhóis .
...
Adicione esta linha `AltTranslation` à classe `OkHttpClient`para instantaneamente:
