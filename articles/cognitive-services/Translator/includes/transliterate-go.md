...
Vamos criar uma função para transliterar texto. Esta função terá um único argumento, a sua chave de subscrição de Texto tradutor.
...
Em seguida, vamos construir a URL. O URL é `Parse()` construído `Query()` usando os e métodos. Vai notar que os parâmetros `Add()` são adicionados com o método. Neste exemplo, estamos a transliterar de japonês para o alfabeto latino.

Copie este `transliterate` código na função.
...
Em seguida, crie uma estrutura anónima para o corpo `json.Marshal()`de pedido e codifique-a como JSON com . Adicione este código `transliterate` à função.
...
Adicione este código `transliterate` à função para descodificar a resposta JSON e, em seguida, forte e imprima o resultado.
