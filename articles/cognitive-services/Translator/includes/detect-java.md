...
Abra `Detect.java` e adicione estas declarações de importação:
...
Adicione estas linhas à `Detect` aula. Você vai notar que a chave de subscrição e ponto final estão sendo lidos a partir de variáveis ambientais:
...
Adicione esta linha à `Detect` classe para instantaneaizar o `OkHttpClient` seguinte:
...
Em seguida, vamos construir o pedido do POST. Sinta-se livre para alterar o texto para deteção de linguagem.
