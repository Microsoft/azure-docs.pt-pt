...
Dentro da `Program` classe, crie uma função assíncrona `TransliterateTextRequest()`chamada. Essa função usa quatro argumentos: `subscriptionKey` `route`, `host`, e `inputText`.
...
Você observará que, `Main`em, você está `subscriptionKey`declarando, `host`, `route`e o script a ser `textToTransliterate`transliterado.
