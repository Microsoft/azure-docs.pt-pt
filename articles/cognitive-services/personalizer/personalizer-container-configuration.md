---
title: Como funciona a Personalizer
titleSuffix: Azure Cognitive Services
description: Saiba como executar o contentor de Personalizer com o docker definições de configuração do comando de execução.
author: edjez
manager: nitinme
...
O `Billing` definição especifica o URI do ponto final da _Personalizer_ recurso no Azure utilizado para informações de faturação para o contentor do medidor. Tem de especificar um valor para esta definição de configuração e o valor tem de ser um URI de ponto de final válido para um _Personalizer_ recursos no Azure.

Esta definição pode ser encontrada no seguinte local:

* Portal do Azure: **Do personalizer** descrição geral, o nome `Endpoint`
...
Os contentores não usam a entrada ou saída monta para armazenar dados de serviço ou de treinamento. 

A sintaxe exata da localização de montagem do anfitrião varia consoante o sistema operativo anfitrião. Além disso, o [computador anfitrião](personalizer-container-howto.md#the-host-computer)da localização de montagem não estar acessível devido a um conflito entre as permissões usadas pela conta de serviço do docker e o anfitrião montar permissões de localização. 

|Opcional| Name | Tipo de dados | Descrição |
|-------|------|-----------|-------------|
|Não permitido| `Input` | String | Contentores de personalizer não utilize esta opção.|
...
Os exemplos seguintes utilizam as definições de configuração para ilustrar como escrever e usar `docker run` comandos.  Quando em execução, o contentor continua a ser executada até [parar](personalizer-container-howto.md#stop-the-container) -lo.
...
|-------------|-------|---|
|{BILLING_KEY} | A chave de ponto final do recurso Personalizer. |xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx|
|{BILLING_ENDPOINT_URI} | O valor de ponto final faturação incluindo região.|`https://westus2.api.cognitive.microsoft.com/`|

> [!IMPORTANT]
> O `Eula`, `Billing`, e `ApiKey` opções tem de ser especificadas para executar o contentor; caso contrário, não inicia o contentor.  Para obter mais informações, consulte [faturação](personalizer-container-howto.md#billing).
> O valor de ApiKey é o **chave** na página chaves do Azure Personalizer recursos. 

## <a name="personalizer-container-docker-examples"></a>Exemplos do personalizer contentor Docker

Os exemplos de Docker seguintes são para o contentor de Personalizer. 
...
[Compreender como utilizar o contentor do Docker](personalizer-container-howto.md)