...
1. Gere `npm install ms-rest-azure` no seu ambiente de desenvolvimento.
2. Gere `npm install @azure/cognitiveservices-entitysearch` no seu ambiente de desenvolvimento.
...
2. Crie uma instância de `CognitiveServicesCredentials` utilizando a sua chave de subscrição. Em seguida, crie uma instância do cliente de pesquisa com ele.
...
1. Envie um pedido de pesquisa de entidades com `entitiesOperations.search()`. Depois de receber uma resposta, imprima o `queryContext`, número de resultados devolvidos e a descrição do primeiro resultado.
