...
- **Função diretório** - É necessário especificar o nível de acesso que a conta de utilizador tem para recursos no seu inquilino. Estão disponíveis os seguintes níveis de permissão:
    ...
    - **Administrador limitado** - Selecione a função administrativa ou as funções para o utilizador. Para obter mais informações sobre as funções que podem ser selecionadas, consulte [atribuindo funções de administrador no Diretório Ativo Azure](../active-directory/users-groups-roles/directory-assign-admin-roles.md).
