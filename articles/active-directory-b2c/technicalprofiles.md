...
| Protocol | 0:1 | O protocolo usado para a comunicação com a outra entidade. |
...
| InputTokenFormat | 0:1 | O formato do token de entrada. Valores possíveis: `JSON`, `JWT`, `SAML11`ou. `SAML2` O `JWT` valor representa um token Web JSON de acordo com a especificação IETF. O `SAML11` valor representa um token de segurança SAML 1,1 de acordo com a especificação Oasis.  O `SAML2` valor representa um token de segurança SAML 2,0 de acordo com a especificação Oasis. |
...
| PersistedClaims | 0:1 | Uma lista das referências definidas anteriormente aos tipos de declaração que são persistidas pelo provedor de declarações relacionado ao perfil técnico. |
...
| Atributo | Requerido | Descrição |
| --------- | -------- | ----------- |
| Name | Sim | O nome de um protocolo válido com suporte pelo Azure AD B2C que é usado como parte do perfil técnico. Valores possíveis: `OAuth1` `OAuth2` ,,`Proprietary`, ,,`OpenIdConnect`, ou`None`. `SAML2` `session management` `self-asserted` |
| cliques | Não | Quando o nome do protocolo é definido `Proprietary`como, especifique o nome totalmente qualificado do assembly que é usado pelo Azure ad B2C para determinar o manipulador de protocolo. |
...
| PartnerClaimType | Não | O identificador do tipo de declaração do parceiro externo ao qual o tipo de declaração de política especificado é mapeado. Se o atributo PartnerClaimType não for especificado, o tipo de declaração de política especificado será mapeado para o tipo de declaração de parceiro do mesmo nome. Use essa propriedade quando o nome do tipo de declaração for diferente da outra entidade. Por exemplo, o primeiro nome de declaração é "exnamename", enquanto o parceiro usa uma declaração chamada "first_name". |
...
Usando **OnClaimsExistence**, **OnItemExistenceInStringCollectionClaim** ou **OnItemAbsenceInStringCollectionClaim**, o exige que você forneça os seguintes metadados: **ClaimTypeOnWhichToEnable** especifica o tipo da declaração a ser avaliada, **ClaimValueOnWhichToEnable** especifica o valor que deve ser comparado.
