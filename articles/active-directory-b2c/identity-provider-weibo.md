...
Para utilizar uma conta Weibo como fornecedor de identidade no Azure Ative Directory B2C (Azure AD B2C), precisa de criar uma aplicação no seu inquilino que o represente. Se ainda não tem uma conta Weibo, pode [https://weibo.com/signup/signup.php?lang=en-us](https://weibo.com/signup/signup.php?lang=en-us)inscrever-se em .
...
1. Introduza as informações **提交** necessárias e selecione (submeter).
...
1. Selecione(salvar). **保存以上信息**
1. **Selecione** (informação avançada).
1. Selecione **(** editar) ao lado do campo para OAuth2.000 (URL de redirecionamento). **授权设置**
1. Introduza `https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com/oauth2/authresp` o OAuth2.000 (URL de redirecionamento). **授权设置** Por exemplo, se o nome do seu inquilino `https://contoso.b2clogin.com/contoso.onmicrosoft.com/oauth2/authresp`for contoso, defino o URL para ser .
