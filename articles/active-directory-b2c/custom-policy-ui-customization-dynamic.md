...
Este artigo se concentra em como personalizar a interface do usuário do Azure AD B2C com *conteúdo dinâmico* usando políticas personalizadas. Para começar, consulte [personalização da interface do usuário em uma política personalizada](custom-policy-ui-customization.md).

>[!NOTE]
>O artigo Azure AD B2C, [Configurar a personalização da interface do usuário em uma política personalizada](custom-policy-ui-customization.md), discute os seguintes conceitos básicos:
...
No artigo [Configurar a personalização da interface do usuário em uma política personalizada](custom-policy-ui-customization.md) , você carrega arquivos HTML5 para o armazenamento de BLOBs do Azure. Esses arquivos HTML5 são estáticos e renderizam o mesmo conteúdo HTML para cada solicitação.
...
3. Nomeie o aplicativo (por exemplo, *contoso. AADB2C. UI*) e, em seguida, selecione **OK**.
    ...
    ![Criar novo projeto do Visual Studio](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project1.png)

4. Selecione o modelo de **aplicativo Web** .

5. Defina a autenticação como **sem autenticação**.

    ![Selecionar modelo de aplicativo Web](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project2.png)
...
10. Copie o conteúdo do arquivo e cole-o abaixo da definição de layout. Seu código deve ter a seguinte aparência:
    ...
    ![arquivo. cshtml unificado após a adição do HTML5](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-edit-view1.png)

### <a name="step-23-change-the-background-image"></a>Etapa 2,3: alterar a imagem de plano de fundo

Localize o elemento `<img>` que contenha o valor `ID` *background_background_image,* e, em seguida, substitua o valor `src` por **https://kbdevstorage1.blob.core.windows.net/asset-blobs/19889_en_1** ou qualquer outra imagem de fundo que queira utilizar.

![elemento img com valor src de background_background_image personalizado](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-static-background.png)
...
1. Abra **Controllers\HomeController.cs**e adicione o seguinte método:
    ...
    ![Alterar o controlador para renderizar a exibição](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-controller-view.png)
...
1. Em **Gerenciador de soluções**, clique com o botão direito do mouse no projeto **contoso. AADB2C. UI** e selecione **publicar**.
    ...
    ![Publicar no Microsoft Azure App Service](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish1.png)

2. Selecione o bloco **Microsoft Azure serviço de aplicativo** e, em seguida, selecione **publicar**.

    ![Criar novo serviço de aplicativo Microsoft Azure](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish2.png)
...
5. Copie o URL da página _unificada_ (por exemplo, _https://<app_name>.azurewebsites.net/home/unificados_).
...
1. Na [portal do Azure](https://portal.azure.com/), selecione **serviços de aplicativos**e, em seguida, selecione o nome do aplicativo de API.
    ...
    ![Selecione aplicativo de API no portal do Azure](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS1.png)
...
6. Alterar o valor da `LoadUri` de _~/tenant/default/unificado_ para _https://<app_name>.azurewebsites.net/home/unified_.
...
2. Abra **B2C_1A_signup_signin,** a política personalizada do partido que você carregou, e depois selecione **Run agora**.
    ...
    ![Sua política de inscrição ou entrada](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo1.png)
...
Modifique o método HomeController `unified` para aceitar o parâmetro campaignid. O método verifica então o valor do parâmetro e define a variável `ViewData["background"]` em conformidade.
...
2. Localize o elemento `<img>` com `background_background_image`de identificação e substitua o valor `src` por `@ViewData["background"]`.
...
3. Abra **B2C_1A_signup_signin,** a política personalizada de RP que carregou e, em seguida, selecione **Run agora**.
