...
Para utilizar uma conta Google como fornecedor de [identidade](authorization-code-flow.md) no Azure Ative Directory B2C (Azure AD B2C), precisa de criar uma aplicação na consola do Google Developers. Se ainda não tiver uma conta no Google, pode inscrever-se em [https://accounts.google.com/SignUp](https://accounts.google.com/SignUp).
...
1. Selecione **Credenciais** no menu esquerdo e, em seguida, selecione **Criar credenciais** > **Oauth cliente ID**.
1. No **tipo de aplicação,** selecione **aplicação Web**.
1. Introduza um **Nome** para `https://your-tenant-name.b2clogin.com` a sua aplicação, `https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com/oauth2/authresp` insira nas **origens JavaScript autorizadas,** e em **URIs de redirecionamento autorizado**. Substitua `your-tenant-name` pelo nome do seu inquilino. Você precisa usar todas as letras minúsculas ao introduzir o nome do seu inquilino, mesmo que o inquilino seja definido com letras maiúsculas em Azure AD B2C.
1. Clique em **Criar**.
