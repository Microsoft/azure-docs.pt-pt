...
| response_type | Sim | Deve `id_token` incluir o acesso ao OpenID Connect. Também pode incluir o `token`tipo de resposta . Se utilizar, `token`a sua aplicação pode receber imediatamente um sinal de acesso a partir do ponto final de autorização, sem fazer um segundo pedido para o ponto final de autorização.  Se utilizar `token` o tipo `scope` de resposta, o parâmetro deve conter um âmbito que indique para que recurso emitir o símbolo. |
...
| response_mode | Não | Especifica o método a utilizar para enviar o token resultante de volta para a sua aplicação.  Para fluxos `fragment`implícitos, use . |
| scope | Sim | Uma lista de âmbitos separados pelo espaço. Um único valor de âmbito indica à Azure AD ambas as permissões que estão a ser solicitadas. O `openid` âmbito indica uma permissão para iniciar sessão no utilizador e obter dados sobre o utilizador sob a forma de fichas de identificação. O `offline_access` âmbito é opcional para aplicações web. Indica que a sua aplicação precisa de um token de atualização para o acesso de longa duração aos recursos. |
...
| pronta | Não | O tipo de interação do utilizador que é necessário. Atualmente, o único `login`valor válido é . Este parâmetro obriga o utilizador a introduzir as suas credenciais nesse pedido. Um único sinal não faz efeito. |
...
Uma resposta bem `response_mode=fragment` `response_type=id_token+token` sucedida que usa e se parece com a seguinte, com quebras de linha para a legibilidade:
...
| state | Se `state` um parâmetro estiver incluído no pedido, o mesmo valor deve figurar na resposta. A aplicação deve `state` verificar se os valores no pedido e resposta são idênticos. |
...
Uma das propriedades deste documento `jwks_uri`de configuração é o . O valor para o mesmo fluxo de utilizador seria:

```HTTP
https://fabrikamb2c.b2clogin.com/fabrikamb2c.onmicrosoft.com/b2c_1_sign_in/discovery/v2.0/keys
```

Para determinar qual o fluxo do utilizador utilizado para assinar um símbolo de identificação (e de onde recolher os metadados), tem duas opções. Em primeiro lugar, o nome `acr` de `id_token`fluxo do utilizador está incluído na reclamação em . Para obter informações sobre como analisar as reclamações de um símbolo de identificação, consulte a [referência simbólica Azure AD B2C](tokens-overview.md). A outra opção é codificar o fluxo `state` do utilizador no valor do parâmetro quando emitir o pedido. Em seguida, `state` descodifique o parâmetro para determinar qual o fluxo do utilizador utilizado. Qualquer um dos métodos é válido.

Depois de ter adquirido o documento de metadados do ponto final dos metadados OpenID Connect, pode utilizar as teclas públicas RSA-256 (localizadas neste ponto final) para validar a assinatura do símbolo de identificação. Pode haver várias chaves listadas neste ponto final a `kid`qualquer momento, cada uma identificada por a . O cabeçalho `id_token` também `kid` contém uma reivindicação. Indica qual destas chaves foi usada para assinar o símbolo de identificação. Para obter mais informações, incluindo a aprendizagem sobre a validação de [fichas,](tokens-overview.md)consulte a [referência simbólica Azure AD B2C](tokens-overview.md).
...
* Valide `iat` `exp` o e reclama ções para garantir que o token de identificação não expirou.
...
| response_type |Necessário |Deve `id_token` incluir o acesso ao OpenID Connect.  Também pode incluir o `token`tipo de resposta . Se utilizar `token` aqui, a sua aplicação pode receber imediatamente um sinal de acesso a partir do ponto final de autorização, sem fazer um segundo pedido para o ponto final de autorização. Se utilizar `token` o tipo `scope` de resposta, o parâmetro deve conter um âmbito que indique para que recurso emitir o símbolo. |
...
| response_mode |Recomendado |Especifica o método utilizado para enviar o token resultante de volta para a sua aplicação. Para o fluxo `fragment`implícito, use . Dois outros modos podem `query` ser `form_post`especificados, e, mas não funcionam no fluxo implícito. |
...
| pronta |Necessário |Para refrescar e obter fichas num iframe oculto, use `prompt=none` para garantir que o iframe não fique preso na página de inscrição e volte imediatamente. |
| login_hint |Necessário |Para refrescar e obter fichas num iframe oculto, inclua o nome de utilizador do utilizador nesta dica para distinguir entre várias sessões que o utilizador possa ter num dado momento. Pode extrair o nome de utilizador de um `preferred_username` início de `profile` sessão anterior utilizando a `preferred_username` reclamação (o âmbito é necessário para receber a reclamação). |
| domain_hint |Necessário |Pode ser `consumers` ou `organizations`.  Para refrescar e obter fichas num iframe `domain_hint` escondido, inclua o valor no pedido.  Extrair `tid` a reclamação do símbolo de identificação de um início de `profile` sessão anterior para determinar `tid` qual o valor a utilizar (o âmbito é necessário para receber a reclamação). Se `tid` o valor `9188040d-6c67-4c5b-b112-36a304b66dad`da `domain_hint=consumers`reclamação for, utilize .  Caso contrário, `domain_hint=organizations`use . |

Ao definir `prompt=none` o parâmetro, este pedido ou sucede ou falha imediatamente, e regressa à sua aplicação.  Uma resposta bem sucedida é enviada para a sua aplicação no `response_mode` URI redirecionado indicado, utilizando o método especificado no parâmetro.
...
### <a name="error-response"></a>Resposta de erro
As respostas de erro também podem ser enviadas para o URI redirecionamento para que a aplicação possa manuseá-las adequadamente.  Pois, `prompt=none`um erro esperado parece com este exemplo:
...
Fichas de identificação e fichas de acesso expiram após um curto período de tempo. A sua aplicação deve estar preparada para refrescar estas fichas periodicamente.  Para refrescar qualquer tipo de ficha, execute o mesmo pedido de iframe `prompt=none` escondido que usamos num exemplo anterior, utilizando o parâmetro para controlar os passos da AD Azure.  Para receber `id_token` um novo valor, `response_type=id_token` `scope=openid`certifique-se `nonce` de usar e, e um parâmetro.
...
Pode simplesmente redirecionar o `end_session_endpoint` utilizador para o que está listado no mesmo documento de metadados OpenID Connect descrito em [Validate o token ID](#validate-the-id-token). Por exemplo:
