...
1. Para o **Redirect URI,** aceite o valor da **Web** e introduza `your-B2C-tenant-name` o seguinte URL em todas as letras minúsculas, substituindo pelo nome do seu inquilino Azure AD B2C.
...
1. No menu **de aplicações,** selecione **Certificados & segredos**e, em seguida, selecione **novo segredo de cliente**.
...
Para utilizar uma conta no Facebook como fornecedor de identidade no Azure AD B2C, é necessário criar uma aplicação no Facebook. Se ainda não tem uma conta no Facebook, [https://www.facebook.com/](https://www.facebook.com/)pode obtê-la em .
...
1. No **Site URL,** insira `https://your-tenant-name.b2clogin.com/` a substituição `your-tenant-name` com o nome do seu inquilino.
1. Introduza um URL para o `http://www.contoso.com/`URL da Política de **Privacidade,** por exemplo. O URL da política de privacidade é uma página que mantém para fornecer informações de privacidade para a sua aplicação.
...
Se o sinal em funcionamento for bem `https://jwt.ms` sucedido, é redirecionado para o qual exibe o Token Descodificado, semelhante a:
