...
Você pode obter um token de acesso para o API do Graph chamando o método ADAL `AuthenticationContext.AcquireToken()`. A ADAL devolve então um `access_token` que representa a identidade da aplicação.
...
Para definir um atributo personalizado em seu locatário B2C, consulte a [referência de atributo personalizado B2C](user-flow-custom-attributes.md).
...
Pode utilizar o nome completo, como `extension_55dc0861f9a44eb999e0a8a872204adb_Jersey_Number`, como propriedade nos objetos do utilizador. Atualize o arquivo JSON com a nova propriedade e um valor para a propriedade e, em seguida, execute:
