---
title: Personalizar a interface do usuário do Azure Active Directory B2C dinamicamente usando políticas personalizadas | Microsoft Docs
...
A `ContentDefinitions` seção contém uma série de `ContentDefinition` elementos XML. O atributo ID do `ContentDefinition` elemento Especifica o tipo de página relacionado à definição de conteúdo. Ou seja, o elemento define o contexto que um modelo HTML5/CSS personalizado irá aplicar. A tabela a seguir descreve o conjunto de IDs de definição de conteúdo que são reconhecidos pelo mecanismo IEF e os tipos de página relacionados a eles.
...
* Substitua os `LoadUri` elementos para apontar para o arquivo HTML5.
...
## <a name="step-2-create-mvc-view"></a>Passo 2: Criar exibição MVC
...
1. Clique com o botão direito do mouse na pasta views/Home e **adicione** > **New Item**.
...
Localize o `<img>` elemento que contém o `ID` valor *background_background_image*e, em seguida, `src` substitua o **https://kbdevstorage1.blob.core.windows.net/asset-blobs/19889_en_1** valor por ou qualquer outra imagem de plano de fundo que você deseja usar.

![elemento img com valor src personalizado background_background_image](media/active-directory-b2c-ui-customization-custom-dynamic/aadb2c-ief-ui-customization-add-static-background.png)
...
1. Abra **Controllers\HomeController.cs**e adicione o seguinte método:
    ...
    Depois de adicionar o  método unificado, seu código deverá ter a seguinte aparência:
...
2. Depure seu aplicativo Web e certifique-se de  que a página unificada esteja acessível `http://localhost:<Port number>/Home/unified`(por exemplo,).
...
1. Em **Gerenciador de soluções**, clique com o botão direito do mouse no projeto **contoso. AADB2C. UI** e selecione **publicar**.
    ...
    ![Publicar no serviço de aplicativo Microsoft Azure](media/active-directory-b2c-ui-customization-custom-dynamic/aadb2c-ief-ui-customization-publish1.png)
...
5. Copie a URL da página  unificada (por exemplo, _https://< app_name >. azurewebsites. net/Home/Unified_).

## <a name="step-3-configure-cors-in-azure-app-service"></a>Passo 3: Configurar CORS no serviço Azure App
...
Seu modelo HTML5 está pronto para uso. No entanto, ele não está disponível `ContentDefinition` no código. Antes de poder adicionar `ContentDefinition` a sua política personalizada, verifique se:
...
* Você está usando *URLs absolutas*, `https://yourdomain/content`como, para todos os links, conteúdo CSS e imagens.

## <a name="step-5-configure-your-content-definition"></a>Passo 5: Configurar sua definição de conteúdo
...
2. Pesquise o `<ContentDefinitions>` elemento e copie todo o conteúdo `<ContentDefinitions>` do nó.

3. Abra o arquivo de extensão (por exemplo, *TrustFrameworkExtensions. xml*) e, em seguida `<BuildingBlocks>` , procure o elemento. Se o elemento não existir, adicione-o.

4. Cole todo o conteúdo do `<ContentDefinitions>` nó que você copiou como um filho `<BuildingBlocks>` do elemento.

5. Procure o `<ContentDefinition>` nó que contém `Id="api.signuporsignin"` no XML que você copiou.

6. Altere o valor de `LoadUri` _~/Tenant/default/Unified_ para _https://< app_name >. azurewebsites. net/Home/Unified_.
...
## <a name="step-6-upload-the-policy-to-your-tenant"></a>Passo 6: Carregar a política para seu locatário
...
## <a name="step-7-test-the-custom-policy-by-using-run-now"></a>Passo 7: Testar a política personalizada usando executar agora
...
Altere o plano de fundo com base no parâmetro de cadeia de caracteres de consulta chamado campaignid. Seu aplicativo RP (aplicativos Web e móveis) envia o parâmetro para Azure AD B2C. Sua política lê o parâmetro e envia seu valor para o modelo HTML5.

### <a name="step-81-add-a-content-definition-parameter"></a>Etapa 8,1: Adicionar um parâmetro de definição de conteúdo

Adicione o `ContentDefinitionParameters` elemento fazendo o seguinte:
1. Abra o arquivo *SignUpOrSignin* da sua política (por exemplo, *SignUpOrSignin. xml*).

2. No nó, adicione o `UserJourneyBehaviors` nó: `<DefaultUserJourney>`
...
### <a name="step-82-change-your-code-to-accept-a-query-string-parameter-and-replace-the-background-image"></a>Etapa 8,2: Altere seu código para aceitar um parâmetro de cadeia de caracteres de consulta e substituir a imagem de plano de fundo
Modifique o método `unified` HomeController para aceitar o parâmetro campaignid. Em seguida, o método verifica o valor do parâmetro e `ViewData["background"]` define a variável de acordo.

1. Abra o arquivo *Controllers\HomeController.cs* e, em seguida, `unified` altere o método adicionando o seguinte trecho de código:
...
2. Localize o `<img>` elemento com ID `background_background_image`e substitua o `src` valor por `@ViewData["background"]`.
...
3. Abra **B2C_1A_signup_signin**, a política personalizada do RP que você carregou e selecione **executar agora**.
    Você deve ver a mesma imagem de plano de fundo exibida anteriormente.

4. Copie a URL da barra de endereços do navegador.

5. Adicione o  parâmetro de cadeia de caracteres de consulta campaignid ao URI. Por exemplo, adicione `&campaignId=hawaii`, conforme mostrado na imagem a seguir:
...
2. Volte para a "etapa 4" e faça o seguinte:
    ...
    a. Em sua política de extensão, localize `<ContentDefinition>` o nó que `Id="api.selfasserted"`contém `Id="api.localaccountsignup"`, e `Id="api.localaccountpasswordreset"`.

    b. Defina o `LoadUri` atributo para o URI do *selfasserted* .
