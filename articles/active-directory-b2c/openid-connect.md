...
- Garantindo que ocorreu uma certa força de autenticação, como a autenticação multi-factor Ad Azure.

Depois de validado o token de ID, pode iniciar uma sessão com o utilizador. Pode utilizar as reclamações no token de identificação para obter informações sobre o utilizador na sua aplicação. As utilizações para esta informação incluem exibição, registos e autorização.
...
Para definir o ID Token necessário em pedidos de logout, consulte o comportamento da [sessão de configuração no Azure Ative Directory B2C](session-behavior.md#secure-your-logout-redirect).

## <a name="next-steps"></a>Passos seguintes

- Saiba mais sobre [a sessão Azure AD B2C](session-behavior.md).