...
O contexto de cálculo permite-lhe controlar se a computação é feita localmente no nó de extremidade ou distribuída pelos nós do cluster do HDInsight.  Para obter um exemplo de configuração de um contexto de cálculo com o r Studio Server, consulte [executar um script R num cluster no HDInsight do Azure através do RStudio Server serviços ML](machine-learning-services-quickstart-job-rstudio.md).
