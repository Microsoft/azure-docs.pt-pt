...
- **Contextos predefinidos**. Com os kernels **PySpark**, **PySpark3**ou **Spark** , você não precisa definir os contextos Spark ou Hive explicitamente antes de começar a trabalhar com seus aplicativos. Eles estão disponíveis por padrão. Desses contextos são:
...
- **Mágicas de célula**. O kernel PySpark fornece algumas "mágicas" predefinidas, que são comandos especiais que você pode chamar com `%%` (por exemplo, `%%MAGIC` `<args>`). O comando mágico deve ser a primeira palavra em uma célula de código e permitir várias linhas de conteúdo. A palavra mágica deve ser a primeira palavra na célula. Adicionar qualquer coisa antes da mágica, até mesmo comentários, causa um erro.     Para obter mais informações sobre as mágicas, consulte [aqui](https://ipython.readthedocs.org/en/stable/interactive/magics.html).
   ...
   | --- | --- | --- |
   | Obter ajuda |`%%help` |Gera uma tabela de todas as mágicas disponíveis com o exemplo e a descrição |
   | info |`%%info` |Gera informações de sessão para o ponto de extremidade Livy atual |
   ...
   | sql |`%%sql -o <variable name>`<br> `SHOW TABLES` |Executa uma consulta do Hive contra o kontext sqlContext. Se o parâmetro `-o` for passado, o resultado da consulta será persistido no contexto de%% local do Python como um dataframe do [pandas](https://pandas.pydata.org/) . |
   ...
   | logs |`%%logs` |Gera os logs para a sessão Livy atual. |
   | delete |`%%delete -f -s <session number>` |Exclui uma sessão específica do ponto de extremidade Livy atual. Você não pode excluir a sessão que é iniciada para o próprio kernel. |
   ...
   > [!NOTE]  
   > Além das mágicas adicionadas pelo kernel PySpark, você também pode usar as [mágicas ipython internas](https://ipython.org/ipython-doc/3/interactive/magics.html#cell-magics), incluindo `%%sh`. Você pode usar a mágica `%%sh` para executar scripts e bloco de código no cluster cabeçalho.
...
A mágica `%%sql` dá suporte a parâmetros diferentes que você pode usar para controlar o tipo de saída que você recebe ao executar consultas. A tabela a seguir lista a saída.
...
| -m |`-m <METHOD>` |Onde o **método** é **Take** ou **Sample** (o padrão é **Take**). Se o método for **necessário**, o kernel selecionará os elementos da parte superior do conjunto de dados de resultado especificado por MAXROWS (descrito mais adiante nesta tabela). Se o método for de **exemplo**, o kernel exemplifica aleatoriamente os elementos do conjunto de dados de acordo com o parâmetro `-r`, descrito a seguir nesta tabela. |
...
Independentemente de o cluster usar o armazenamento do Azure ou Azure Data Lake Storage como a conta de armazenamento padrão, os blocos de anotações também são salvos no cluster cabeçalho em `/var/lib/jupyter`.
