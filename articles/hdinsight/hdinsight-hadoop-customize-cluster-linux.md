---
title: Personalizar clusters do Azure HDInsight ao utilizar as ações de script
...
* Devem ser armazenados num URI que é acessível a partir do cluster do HDInsight. Seguem-se as localizações de armazenamento possíveis:
    ...
    * Para clusters regulares:
      ...
      * Um blob numa conta de armazenamento do Azure que é a primário ou adicional conta de armazenamento para o cluster do HDInsight. HDInsight é concedido acesso a ambos os tipos de contas de armazenamento durante a criação do cluster.
        ...
        > [!IMPORTANT]  
        > Não é rotação da chave de armazenamento nesta conta de armazenamento do Azure, como fará com que as ações de script subsequentes com scripts armazenadas para efetuar a ativação.
...
Nesta secção, vai utilizar o [Add-AzHDInsightScriptAction](https://docs.microsoft.com/powershell/module/az.hdinsight/add-azhdinsightscriptaction) cmdlet para invocar scripts para personalizar um cluster. Antes de começar, certifique-se de instalar e configurar o Azure PowerShell. Para utilizar estes comandos do PowerShell, tem do [módulo AZ](https://docs.microsoft.com/powershell/azure/overview).
...
Para utilizar estes comandos do PowerShell, tem do [módulo AZ](https://docs.microsoft.com/powershell/azure/overview).
...
| cmdlet | Função |
| --- | --- |
| `Get-AzHDInsightPersistedScriptAction` |Obter informações sobre ações de script persistentes. |
| `Get-AzHDInsightScriptActionHistory` |Obter um histórico das ações de script aplicados ao cluster ou de detalhes de um script específico. |
| `Set-AzHDInsightPersistedScriptAction` |Promova uma ação de script ad hoc para uma ação de script persistentes. |
| `Remove-AzHDInsightPersistedScriptAction` |Despromova uma ação de script persistentes a uma ação ad hoc. |

> [!IMPORTANT]  
> `Remove-AzHDInsightPersistedScriptAction` não anular as ações executadas por um script. Este cmdlet Remove apenas o sinalizador persistente.
