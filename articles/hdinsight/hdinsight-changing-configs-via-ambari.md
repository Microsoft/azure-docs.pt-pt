...
Regra geral, ter o método de compressão splittable é importante, caso contrário muito poucos mappers serão criados. Se os dados de `bzip2` entrada forem texto, é a melhor opção. Para o formato ORC, o Snappy é a opção de compressão mais rápida.
...
A execução especulativa lança um certo número de tarefas duplicadas de forma a detetar e colocar na lista negra o rastreador de tarefas de execução lenta, ao mesmo tempo que melhora a execução global do trabalho, otimizando os resultados individuais das tarefas.
...
Quando ativados, pequenos ficheiros são combinados para menos tarefas de mapa. Isto melhora a eficiência dos postos de trabalho com muitos pequenos ficheiros. Para permitir, `pig.noSplitCombination` definido para verdade. O valor predefinido é false.

### <a name="tune-mappers"></a>Mappers de melodia

O número de mappers é controlado `pig.maxCombinedSplitSize`modificando a propriedade. Isto especifica o tamanho dos dados a serem processados por uma única tarefa de mapa. O valor predefinido é o tamanho do bloco padrão do sistema de ficheiros. Aumentar este valor resulta numa diminuição do número de tarefas mapeadas.
...
A utilização do tampão de `hbase.hregion.memstore.mslab.enabled`alocação local Memstore é determinada pela propriedade. Quando ativado (verdadeiro), isto evita a fragmentação da pilha durante o funcionamento de escrita pesada. O valor predefinido é true.
