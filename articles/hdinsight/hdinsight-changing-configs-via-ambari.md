...
O HDInsight fornece clusters [Apache Hadoop](https://hadoop.apache.org/) para aplicações de processamento de dados em larga escala. Gerir, monitorizar e otimizar estes complexos clusters multi-nó pode ser um desafio. [Apache Ambari](https://ambari.apache.org/) é uma interface web para gerir e monitorizar os clusters HDInsight Linux.  Para os clusters Windows, utilize a [Ambari REST API](hdinsight-hadoop-manage-ambari-rest-api.md).
...
Faça login em Ambari em `https://CLUSTERNAME.azurehdidnsight.net` com as suas credenciais de cluster. O ecrã inicial apresenta um painel de visão geral.
...
A UI web ambari pode ser usada para gerir anfitriões, serviços, alertas, configurações e vistas. A Ambari não pode ser usada para criar um cluster HDInsight, atualizar serviços, gerir pilhas e versões, desativar ou recomissãor anfitriões, ou adicionar serviços ao cluster.

## <a name="manage-your-clusters-configuration"></a>Gerencie a configuração do seu cluster

As definições de configuração ajudam a afinar um determinado serviço. Para modificar as definições de configuração de um serviço, selecione o serviço a partir da barra lateral dos **Serviços** (à esquerda) e, em seguida, navegue para o separador **Configs** na página de detalhes do serviço.
...
O tamanho da pilha NameNode Java depende de muitos fatores, como a carga no cluster, o número de ficheiros e o número de blocos. O tamanho padrão de 1 GB funciona bem com a maioria dos clusters, embora algumas cargas de trabalho possam requerer mais ou menos memória.
...
* `tez.grouping.min-size`: Limite inferior no tamanho de uma divisão agrunada, com um valor predefinido de 16 MB (16.777.216 bytes).
* `tez.grouping.max-size`: Limite superior no tamanho de uma divisão agrupada, com um valor predefinido de 1 GB (1.073.741.824 bytes).

Como regra de desempenho do polegar, diminua ambos estes parâmetros para melhorar a latência, aumente para mais rendimento.
...
1. Para modificar os parâmetros de limite, navegue para o separador **Configs** do serviço Tez. Expanda o painel **geral** e localize os parâmetros `tez.grouping.max-size` e `tez.grouping.min-size`.
...
Por exemplo, diga que tem um tamanho de dados de entrada de 50 GB. Esses dados em formato ORC com compressão Snappy são de 1 GB. A Hive estima o número de redutores necessários como: (número de bytes entradas para mappers / `hive.exec.reducers.bytes.per.reducer`).

Com as definições predefinidas, este exemplo são 4 redutores.

O parâmetro `hive.exec.reducers.bytes.per.reducer` especifica o número de bytes processados por redutor. O valor predefinido é de 64 MB. A sintonização deste valor aumenta o paralelismo e pode melhorar o desempenho. Ajustá-lo demasiado baixo também pode produzir demasiados redutores, afetando potencialmente o desempenho. Este parâmetro baseia-se nos seus requisitos específicos de dados, configurações de compressão e outros fatores ambientais.
...
1. Selecione **Editar** para modificar o valor para 128 MB (134.217.728 bytes) e, em seguida, prima **Enter** para poupar.
    ...
    Dado um tamanho de entrada de 1.024 MB, com 128 MB de dados por redutor, existem 8 redutores (1024/128).

1. Um valor incorreto para o parâmetro **Data per Reducer** pode resultar num grande número de redutores, afetando negativamente o desempenho da consulta. Para limitar o número máximo de redutores, `hive.exec.reducers.max` a um valor adequado. O valor padrão é 1009.
...
1. Para permitir a execução de consulta paralela, navegue até ao separador Hive **Config** e procure a propriedade `hive.exec.parallel`. O valor predefinido é false. Mude o valor para verdadeiro e, em seguida, pressione **Enter** para salvar o valor.

1. Para limitar o número de postos de trabalho a funcionar em paralelo, modificar a propriedade `hive.exec.parallel.thread.number`. O valor padrão é 8.
...
1. Para permitir uma execução de consulta vetorial, navegue até ao separador Hive **Configs** e procure o parâmetro `hive.vectorized.execution.enabled`. O valor predefinido é verdadeiro para a Colmeia 0.13.0 ou mais tarde.

1. Para permitir a execução vetorialpara o lado de redução da consulta, defina o parâmetro `hive.vectorized.execution.reduce.enabled` verdadeiro. O valor predefinido é false.
...
Para ativar o CBO, navegue para **a Hive** > **Configs** > **Definições** e encontre o **Optimizador baseado em custos de ativação**e, em seguida, mude o botão de alternância para **Ligar**.
...
* `hive.compute.query.using.stats`
    ...
    Quando é verdade, a Hive utiliza estatísticas armazenadas na sua metaloja para responder a consultas simples como `count(*)`.
...
* `hive.stats.fetch.partition.stats`
    ...
    Estatísticas básicas de partição, tais como o número de linhas, tamanho de dados e tamanho de ficheiro são armazenadas na metaloja. Quando definidos como verdadeiros, as estatísticas de partição são recolhidas a partir da metaloja. Quando falso, o tamanho do ficheiro é recolhido do sistema de ficheiros, e o número de linhas é recolhido do esquema da linha.
...
1. Para permitir a compressão intermédia, navegue até ao separador Hive **Configs** e, em seguida, coloque o parâmetro `hive.exec.compress.intermediate` verdadeiro. O valor predefinido é false.

    ![Executivo da colmeia compressa intermediário](./media/hdinsight-changing-configs-via-ambari/hive-exec-compress-intermediate.png)
...
1. Para definir o código de compressão intermédio, adicione a propriedade personalizada `mapred.map.output.compression.codec` ao ficheiro `hive-site.xml` ou `mapred-site.xml`.

1. Para adicionar uma definição personalizada:

    a. Navegue até **à Hive** > **Configs** > local **de colmeia personalizada** **avançado** > .

    b. **Selecione Adicionar Propriedade...** na parte inferior do painel personalizado do site da colmeia.

    c. Na janela Adicionar Propriedade, introduza `mapred.map.output.compression.codec` como chave e `org.apache.hadoop.io.compress.SnappyCodec` como valor.
    ...
    Isto irá comprimir o ficheiro intermédio utilizando a compressão Snappy. Uma vez adicionada a propriedade, aparece no painel personalizado do site da colmeia.

    > [!NOTE]  
    > Este procedimento modifica o ficheiro `$HADOOP_HOME/conf/hive-site.xml`.

### <a name="compress-final-output"></a>Comprimir a saída final

A saída final da Colmeia também pode ser comprimida.

1. Para comprimir a saída final da Colmeia, navegue para o separador Hive **Configs** e, em seguida, coloque o parâmetro `hive.exec.compress.output` verdadeiro. O valor predefinido é false.

1. Para escolher o código de compressão de saída, adicione a propriedade personalizada `mapred.output.compression.codec` ao painel personalizado do site da colmeia, conforme descrito no passo 3 da secção anterior.
...
* Para permitir a execução especulativa, navegue até ao separador Hive **Configs** e, em seguida, coloque o parâmetro `hive.mapred.reduce.tasks.speculative.execution` verdadeiro. O valor predefinido é false.

    ![Hive mapeada reduz em tarefas execução especulativa](./media/hdinsight-changing-configs-via-ambari/hive-mapred-reduce-tasks-speculative-execution.png)

### <a name="tune-dynamic-partitions"></a>Afinar divisórias dinâmicas

A Hive permite criar divisórias dinâmicas ao inserir registos numa tabela, sem pré-definir cada partição. Esta é uma característica poderosa, embora possa resultar na criação de um grande número de divisórias e um grande número de ficheiros para cada partição.

1. Para que a Hive faça divisórias dinâmicas, o valor do parâmetro `hive.exec.dynamic.partition` deve ser verdadeiro (o padrão).

1. Mude o modo dinâmico de partição para *rigoroso*. Em modo rigoroso, pelo menos uma partição tem de ser estática. Isto evita consultas sem o filtro de partição na cláusula WHERE, isto é, *impede rigorosamente* as consultas que digitalizam todas as divisórias. Navegue para o separador Hive **Configs** e, em seguida, coloque `hive.exec.dynamic.partition.mode` **para estrito**. O valor predefinido não é **restrito**.

1. Para limitar o número de divisórias dinâmicas a criar, modifique o parâmetro `hive.exec.max.dynamic.partitions`. O valor padrão é 5000.
...
O modo local permite que a Hive execute todas as tarefas de um trabalho numa única máquina, ou às vezes num único processo. Isto melhora o desempenho da consulta se os dados de entrada forem pequenos e a sobrecarga das tarefas de lançamento para consultas consumir uma percentagem significativa da execução total da consulta.

Para ativar o modo local, adicione o parâmetro `hive.exec.mode.local.auto` ao painel personalizado do sítio da colmeia, conforme explicado no passo 3 da secção de [compressão intermédia Enable.](#enable-intermediate-compression)
...
Para ativar este comportamento, adicione o parâmetro `hive.multigroupby.singlereducer` ao painel personalizado do sítio da colmeia, como explicado no passo 3 da secção de [compressão intermédia Enable.](#enable-intermediate-compression)
...
| Tipo de adesão | quando | como como | Configurações da colmeia | Comentários |
| -- | -- | -- | -- | -- |
...
| Reunião do mapa | <ul><li>Uma mesa pode caber na memória</li></ul> | <ul><li>Lê pequena tabela na mesa de hash de memória</li><li>Streams através de parte do grande arquivo</li><li>Junta-se a cada disco da mesa hash</li><li>As juntas são só pelo mapper</li></ul> | `hive.auto.confvert.join=true` | Muito rápido, mas limitado. |
| Ordenar balde de fusão | Se ambas as mesas forem: <ul><li>Ordenado o mesmo</li><li>Baldedo da mesma forma</li><li>Juntando-se à coluna ordenada/balde</li></ul> | Cada processo: <ul><li>Lê um balde de cada mesa</li><li>Processa a linha com o valor mais baixo</li></ul> | `hive.auto.convert.sortmerge.join=true` | Muito eficiente |
...
| `tez.am.resource.memory.mb` | 4 GB de limite superior para a maioria | Auto-Sintonizado |
...
As propriedades do [Apache Pig](https://pig.apache.org/) podem ser modificadas a partir da UI web ambari para afinar consultas de porco. Modificar as propriedades do porco de Ambari modifica diretamente as propriedades do Porco no ficheiro `/etc/pig/2.4.2.0-258.0/pig.properties`.
...
> [!NOTE]  
> Quaisquer configurações de nível de sessão sobrepõem os valores de propriedade no ficheiro `pig.properties`.
...
1. Para modificar o motor de execução, no painel **avançado de propriedades de porco,** encontre a propriedade `exectype`.
...
1. Para ativar o modo local, ajuste `pig.auto.local.enabled` **verdadeiro**. O valor predefinido é false.

1. Os empregos com uma dimensão de dados de entrada inferior ao `pig.auto.local.input.maxbytes` valor patrimonial são considerados pequenos empregos. O valor padrão é de 1 GB.

### <a name="copy-user-jar-cache"></a>Copiar cache de jarro de utilizador

O porco copia os ficheiros JAR exigidos pelos UDFs a uma cache distribuída para os disponibilizar para os nódosos de tarefa. Estes frascos não mudam com frequência. Se ativado, a definição de `pig.user.cache.enabled` permite que os frascos sejam colocados numa cache para os reutilizar em trabalhos geridos pelo mesmo utilizador. Isto resulta num pequeno aumento do desempenho do emprego.

1. Para permitir, `pig.user.cache.enabled` verdadeiro. O padrão é falso.

1. Para definir o caminho base dos frascos em cache, coloque `pig.user.cache.location` para o caminho base. A predefinição é `/tmp`.
...
* `pig.cachedbag.memusage`: A quantidade de memória atribuída a um saco. Um saco é uma coleção de tuples. Um tuple é um conjunto ordenado de campos, e um campo é um pedaço de dados. Se os dados de um saco estão para além da memória atribuída, é derramado para o disco. O valor padrão é de 0,2, o que representa 20% da memória disponível. Esta memória é partilhada em todos os sacos numa aplicação.
...
* `pig.tmpfilecompression`: Quando for verdade, permite a compressão temporária do ficheiro. O valor predefinido é false.

* `pig.tmpfilecompression.codec`: O códice de compressão a utilizar para comprimir os ficheiros temporários. Os códigos de compressão recomendados são [LZO](https://www.oberhumer.com/opensource/lzo/) e Snappy para uma menor utilização do CPU.

### <a name="enable-split-combining"></a>Ativar a combinação de divisão

Quando ativados, pequenos ficheiros são combinados para menos tarefas de mapa. Isto melhora a eficiência dos postos de trabalho com muitos pequenos ficheiros. Para permitir, `pig.noSplitCombination` verdadeiro. O valor predefinido é false.

### <a name="tune-mappers"></a>Mappers de melodia

O número de mappers é controlado modificando a propriedade `pig.maxCombinedSplitSize`. Isto especifica o tamanho dos dados a serem processados por uma única tarefa de mapa. O valor predefinido é o tamanho do bloco padrão do sistema de ficheiros. Aumentar este valor resulta numa diminuição do número de tarefas mapeadas.

### <a name="tune-reducers"></a>Redutores de melodia

O número de redutores é calculado com base no parâmetro `pig.exec.reducers.bytes.per.reducer`. O parâmetro especifica o número de bytes processados por redutor, por predefinição de 1 GB. Para limitar o número máximo de redutores, defino a propriedade `pig.exec.reducers.max`, por padrão 999.
...
O tamanho da pilha HBase especifica a quantidade máxima de pilha a ser usada em megabytes por *região* e servidores *principais.* O valor padrão é de 1.000 MB. Isto deve ser ajustado para a carga de trabalho do cluster.

1. Para modificar, navegue para o painel **HBase-env avançado** no separador HBase **Configs** e, em seguida, encontre a definição `HBASE_HEAPSIZE`.

1. Alterar o valor padrão para 5.000 MB.

    ![Apache Ambari HBase pilhagem de memória](./media/hdinsight-changing-configs-via-ambari/ambari-hbase-heapsize.png)
...
A cache do bloco é a cache de leitura. O seu tamanho é controlado pelo parâmetro `hfile.block.cache.size`. O valor padrão é de 0,4, que é 40 por cento da memória total do servidor da região. Quanto maior for o tamanho da cache do bloco, mais rápido será a leitura aleatória.
...
Todas as edições são armazenadas no amortecedor de memória, chamado *Memstore*. Isto aumenta a quantidade total de dados que podem ser escritos em disco numa única operação, e acelera o acesso subsequente às edições recentes. O tamanho da Memstore é definido pelos dois parâmetros seguintes:
...
A definição `hbase.client.scanner.caching` define o número de linhas lidas a partir do disco quando o método `next` é chamado num scanner.  O valor predefinido é 100. Quanto maior for o número, menos chamadas remotas feitas do cliente para o servidor da região, resultando em exames mais rápidos. No entanto, isso também aumentará a pressão de memória sobre o cliente.

![Número de linhas Apache HBase recolhidas](./media/hdinsight-changing-configs-via-ambari/hbase-num-rows-fetched.png)

> [!IMPORTANT]  
> Não detete o valor de tal forma que o tempo entre a invocação do próximo método num scanner seja maior do que o tempo de tempo do scanner. A duração do tempo limite do scanner é definida pela propriedade `hbase.regionserver.lease.period`.
...
A HBase armazena dados num formato interno de ficheiros, chamado *HFile*. A propriedade `hbase.hregion.max.filesize` define o tamanho de um único HFile para uma região.  Uma região é dividida em duas regiões se a soma de todos os HFiles numa região for maior do que este cenário.

![Apache HBase HRegion tamanho de ficheiro max](./media/hdinsight-changing-configs-via-ambari/hbase-hregion-max-filesize.png)
...
* A propriedade `hbase.hregion.memstore.flush.size` define o tamanho a que a Memstore é lavada ao disco. O tamanho padrão é de 128 MB.
...
* HBase bloqueia atualizações se a Memstore for (`hbase.hregion.memstore.flush.size` * `hbase.hregion.memstore.block.multiplier`) bytes.

    Com os valores predefinidos do tamanho do flush e do multiplicador de blocos, as atualizações são bloqueadas quando a Memstore tem 128 * 4 = 512 MB de tamanho. Para reduzir a contagem de bloqueio de atualização, aumente o valor de `hbase.hregion.memstore.block.multiplier`.
...
O tamanho da loja é definido pelos parâmetros `hbase.regionserver.global.memstore.UpperLimit` e `hbase.regionserver.global.memstore.LowerLimit`. A definição destes valores iguais entre si reduz as pausas durante as escritas (também causando uma descarga mais frequente) e resulta num aumento do desempenho da escrita.

### <a name="set-memstore-local-allocation-buffer"></a>Definir tampão de alocação local Memstore

A utilização do tampão de alocação local Memstore é determinada pela propriedade `hbase.hregion.memstore.mslab.enabled`. Quando ativado (verdadeiro), isto evita a fragmentação da pilha durante o funcionamento de escrita pesada. O valor predefinido é true.
