...
4. Nos ícones na parte superior da caixa de diálogo **drivers** , selecione o ícone de **+** para criar um driver.
...
6. À esquerda da janela SQuirreL SQL, selecione **aliases**. Em seguida, selecione o ícone de **+** para criar um alias de conexão.
...
7. Use os seguintes valores para a caixa de diálogo **Adicionar alias** :
    ...
    |Controlador|Use a lista suspensa para selecionar o driver do **Hive** .|
    |URL|JDBC: hive2://CLUSTERNAME.azurehdinsight.net: 443/padrão; transportmode = http; SSL = true; httpPath =/hive2. Substitua **CLUSTERNAME** pelo nome do cluster do HDInsight.|
    |Nome de Utilizador|O nome da conta de logon do cluster para seu cluster HDInsight. O padrão é **admin**.|
    |Palavra-passe|A senha da conta de logon do cluster.|
    ...
    > [!IMPORTANT]
    > Use o botão **testar** para verificar se a conexão funciona. Quando **a caixa de diálogo Conectar-se a: Hive no HDInsight** for exibida, selecione **conectar** para executar o teste. Se o teste for bem-sucedido, você verá uma caixa de diálogo **conexão bem-sucedida** . Se ocorrer um erro, consulte [solução de problemas](#troubleshooting).

    Para salvar o alias de conexão, use o botão **OK** na parte inferior da caixa de diálogo **Adicionar alias** .

8. Na lista suspensa **conectar ao** na parte superior do SQuirreL SQL, selecione **Hive no HDInsight**. Quando solicitado, selecione **conectar**.
...
**Causa**: esse erro é causado por uma versão mais antiga do arquivo Commons-codec. jar incluído com SQuirreL.

**Resolução**: para corrigir esse erro, use as seguintes etapas:

1. Saia do SQuirreL e vá para o diretório em que o SQuirreL está instalado em seu sistema, talvez `C:\Program Files\squirrel-sql-4.0.0\lib`. No diretório SquirreL, no diretório `lib`, substitua o Commons-codec. jar existente por um baixado do cluster HDInsight.

1. Reinicie o SQuirreL. O erro não deve ocorrer mais durante a conexão com o hive no HDInsight.
