...
1. Depois de desenvolver, compilar e testar seu programa MapReduce, use o comando `scp` para carregar o arquivo JAR para o cabeçalho.
    ...
    Substitua CLUSTERname pelo nome do cluster. Se você tiver usado uma senha para proteger a conta SSH, será solicitado a inserir a senha. Se você usou um certificado, talvez seja necessário usar o parâmetro `-i` para especificar o arquivo de chave privada.
...
1. Na sessão SSH, execute o programa MapReduce por meio do YARN.

    ```bash
    yarn jar mycustomprogram.jar mynamespace.myclass /example/data/sample.log /example/data/logoutput
    ```

    Esse comando envia o trabalho MapReduce para YARN. O arquivo de entrada é `/example/data/sample.log`e o diretório de saída é `/example/data/logoutput`. O arquivo de entrada e todos os arquivos de saída são armazenados no armazenamento padrão do cluster.
