...
1. Para verificar o status do trabalho, use o comando a seguir. Substitua `JOBID` pelo valor retornado na etapa anterior. Por exemplo, se o valor de retorno foi `{"id":"job_1415651640909_0026"}`, `JOBID` seria `job_1415651640909_0026`. Revisar local de `jq` conforme necessário.
...
* Envio em lote com o HDInsight baseado em Linux, ao usar a opção de `-batch` ao executar inserções, o Sqoop executará várias inserções em vez de enviar em lote as operações de inserção.
