...
description: Aprenda a usar SCP.NET para criar. Topologs de tempestade baseados em NET para uso com tempestade em execução em Azure HDInsight.
...
---
# <a name="scp-programming-guide-for-apache-storm-in-azure-hdinsight"></a>Guia de programação do SCP para a Tempestade Apache em Azure HDInsight
...
Quando o **NextTuple** for chamado, o seu código C# pode emitir um ou mais tuples. Se não há nada para emitir, este método deve voltar sem emitir nada.

Os métodos **NextTuple**, **Ack**e **Fail** são todos chamados num laço apertado num único fio de um processo C#. Quando não houver tuples para emitir, tenha o **NextTuple** a dormir por um curto período de tempo como 10 milissegundos. Este sono ajuda a evitar desperdiçar a disponibilidade do CPU.
...
Tal como os seus homólogos não transacionais, **NextTx,** **Ack**e **Fail** são todos chamados num loop apertado num único fio de um processo C#. Quando não houver tuples para emitir, tenha **nextTx** a dormir por um curto período de tempo como 10 milissegundos. Este sono ajuda a evitar desperdiçar a disponibilidade do CPU.

Quando a **NextTx** é chamada para iniciar uma nova transação, o parâmetro de saída *seqId* identifica a transação. A transação também é usada em **Ack** e **Fail**. O seu método **NextTx** pode emitir dados para o lado de Java. Os dados são armazenados no ZooKeeper para suportar a repetição. Como o ZooKeeper tem capacidade limitada, o seu código deve emitir apenas metadados e não dados em massa num bico de transações.
...
SCP.NET também fornece um conjunto simples de objetos-chave para os desenvolvedores programarem. Os objetos são **Context,** **StateStore**e **SCPRuntime.** São discutidos nesta secção.
...
O objeto **pluginType** indica o tipo plug-in do processo C#. Se o processo for executado em modo de teste local sem Java, o tipo plug-in é **SCP_NET_LOCAL**.
...
Quando o **modo simples** está definido como **verdadeiro,** o método **'Cometer'** elimina o ZNode correspondente em ZooKeeper. Caso contrário, o método elimina o ZNode atual e\_adiciona um novo nó no CAMINHO COMPROMETIDO.
...
O método **Inicializar** inicializa o ambiente de execução do SCP. Neste método, o processo C# conecta-se ao lado de Java para obter parâmetros de configuração e contexto de topologia.

O método **LaunchPlugin** inicia o ciclo de processamento de mensagens. Neste ciclo, o plug-in C# recebe mensagens do lado de Java. Estas mensagens incluem tuples e sinais de controlo. O plug-in processa então as mensagens, talvez ligando para o método de interface fornecido pelo seu código.
...
Neste modo, os plug-ins SCP no seu código C# funcionam dentro do Estúdio Visual durante a fase de desenvolvimento. Pode utilizar a interface **ILocalContext** neste modo. A interface fornece métodos para serializar os tuples emitidos para ficheiros locais e lê-los de volta para RAM.
...
| **scp-spout** |*nome executivo*<br />*args*<br />*campos*<br />*parâmetros* |Define um bico não transacional. A função executa a aplicação especificada pelo *nome executivo* e utiliza *args*.<br /><br />O parâmetro dos *campos* especifica os campos de saída para o bico.<br /><br />O parâmetro dos *parâmetros* é opcional. Utilize-o para especificar parâmetros como "nontransactional.ack.enabled". |
...
| Parâmetro | Descrição |
| --- | --- |
| "plugin.name" |O nome de ficheiro .exe do plug-in C# |
| "plugin.args" |Os argumentos plug-in |
| "output.schema" |O esquema de saída |
| "nontransactional.ack.enabled" |Se o reconhecimento é permitido para uma topologia não transacional |
...
O parâmetro *de recurso-dir* é opcional. Especifique-o quando pretender ligar uma aplicação C#. O diretório especificado contém a aplicação, dependências e configurações.
...
Os seus processos C# podem emitir tuples. Para isso, a plataforma serializa tuples em **objetos byte[]** e transfere os objetos para o lado java. A tempestade transfere estas tuples para os alvos.

Nos componentes a jusante, os processos C# recebem tuples de volta do lado de Java e convertem-nos para os tipos originais da plataforma. Todas estas operações estão escondidas pela plataforma.
...
Foram adicionados dois métodos no objeto SCP.NET **Contexto.** Emitem uma ou mais tuples para riachos específicos. O parâmetro *streamId* é uma corda. O seu valor deve ser o mesmo tanto no código C# como na especificação de definição de topologia.
...
* `[0,1]`especifica um conjunto de iDs de campo hash que começam com zero.

### <a name="hybrid-topology"></a>Topologia híbrida

O código da tempestade nativa está escrito em Java. SCP.NET melhorou a Tempestade para te deixar escrever código C# para lidar com a tua lógica de negócio. Mas SCP.NET também suporta topologias híbridas, que contêm não só c# bicos/parafusos, mas também bicos/parafusos Java.
...
Aqui `microsoft.scp.example.HybridTopology.Generator` está o nome da classe java.
...
Aqui `examples\HybridTopology\java\target\` está a pasta que contém o ficheiro Java spout/bolt JAR.

### <a name="serialization-and-deserialization-between-java-and-c"></a>Serialização e desserialização entre Java e C #

Um componente SCP inclui o lado Java e o lado C#. Para interagir com bicos/parafusos java nativos, a serialização e a desserialização devem ocorrer entre o lado java e o lado C#, como ilustrado no gráfico seguinte:
...
#### <a name="serialization-in-the-java-side-and-deserialization-in-the-c-side"></a>Serialização no lado de Java e desserialização no lado C#

Primeiro, fornecer a implementação padrão para a serialização no lado de Java e desserialização no lado C#.
...
#### <a name="serialization-in-the-c-side-and-deserialization-in-the-java-side"></a>Serialização no lado C# e desserialização no lado de Java
...
Também pode ligar a sua própria implementação de um serializador C# e um desserializer Java.

Este código é a interface para o serializador C#:
...
Este exemplo tem dois ficheiros de especificações: HelloWorld.spec e HelloWorld\_EnableAck.spec. O código C# pode descobrir se o reconhecimento `pluginConf` é ativado através da obtenção do objeto do lado java.
...
Quando uma instância **ISCPBatchBolt** é criada, `txAttempt` obtém o valor do objeto a partir de parâmetros de entrada.
...
Quando `FinishBatch` é `lastCommittedTxId` chamado, é atualizado se não for uma transação reproduzida.
...
### <a name="hybridtopology"></a>HíbridaTopologia

Esta topologia contém um bico java e um parafuso C#. Utiliza a implementação padrão de serialização e desserialização fornecida pela plataforma SCP. Consulte o ficheiro HybridTopology.spec\\nos exemplos da pasta HybridTopology para obter os detalhes do ficheiro de especificação. Consulte também SubmitTopology.bat para saber como especificar o caminho de classe Java.
