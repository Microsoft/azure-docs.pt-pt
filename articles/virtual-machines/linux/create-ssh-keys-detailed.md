...
Quando cria um VM Azure especificando a chave pública, o Azure copia a chave pública (no `.pub` formato) para a `~/.ssh/authorized_keys` pasta no VM. As teclas SSH `~/.ssh/authorized_keys` são usadas para desafiar o cliente a combinar a chave privada correspondente numa ligação SSH. Num VM Azure Linux que utiliza chaves SSH para autenticação, o Azure confunde o servidor SSHD para não permitir o início de sessão de senha, apenas as teclas SSH. Por isso, ao criar um VM Azure Linux com teclas SSH, pode ajudar a proteger a implementação do VM e a guardar o passo típico de configuração pós-implementação de palavras-passe incapacitantes no `sshd_config` ficheiro.
...
Para criar as teclas, é um comando `ssh-keygen` preferido, que está disponível com utilitários OpenSSH na Shell Azure Cloud, um anfitrião macOS ou Linux e Windows 10. `ssh-keygen`faz uma série de perguntas e, em seguida, escreve uma chave privada e uma chave pública correspondente. 
...
O comando seguinte gera ficheiros de `ssh-keygen` chave públicos e privados sSH RSA de 2048 por padrão no `~/.ssh` diretório. Se existir um par de chaves SSH na localização atual, esses ficheiros são substituídos.
...
`-f ~/.ssh/mykeys/myprivatekey`= o nome de ficheiro do ficheiro chave privado, se optar por não utilizar o nome predefinido. Um ficheiro de chave pública correspondente `.pub` anexado é gerado no mesmo diretório. O diretório tem de existir.
...
Se copiar e colar o conteúdo do ficheiro chave público no portal Azure ou num modelo de Gestor de Recursos, certifique-se de que não copia qualquer espaço branco adicional ou introduz quebras de linha adicionais. Por exemplo, se utilizar o macOS, pode canalizar o ficheiro de chaves públicas (por padrão, `~/.ssh/id_rsa.pub` ) para a **cópia** para copiar o conteúdo (existem outros programas Linux que fazem a mesma coisa, `xclip` como).
...
Verifique e utilize `ssh-agent` e `ssh-add` informe o sistema SSH sobre os ficheiros chave para que não seja necessário utilizar a palavra-passe interactivamente.
...
Pode criar e configurar um ficheiro config SSH para acelerar os `~/.ssh/config` logins e otimizar o comportamento do seu cliente SSH. 
...
Agora que tem um par de chaves SSH e um ficheiro de config SSH configurado, é possível iniciar sessão no seu VM Linux de forma rápida e segura. Quando executa o seguinte comando, o SSH localiza e carrega quaisquer definições do `Host myvm` bloco no ficheiro config SSH.
