# Descrição geral
## [Acerca das VMs do SQL Server do Linux](sql-server-linux-virtual-machines-overview.md)

# Inícios rápidos
## [Criar VM do SQL Server - Portal](provision-sql-server-linux-virtual-machine.md)

# Referência
## [CLI do Azure](/cli/azure/)
## [PowerShell](/powershell/azure/overview)
## [T-SQL](https://msdn.microsoft.com/library/azure/bb510741.aspx)
## [Controladores do SQL Server](https://msdn.microsoft.com/library/mt654049.aspx)
## [REST](/rest/api/)

# Recursos
## [FAQ](sql-server-linux-faq.md)
## [SQL Server na Documentação do Linux](https://docs.microsoft.com/sql/linux/sql-server-linux-overview)
## [Preços](../../windows/sql/virtual-machines-windows-sql-server-pricing-guidance.md?toc=%2fazure%2fvirtual-machines%2flinux%2fsql%2ftoc.json)
## [PaaS ou IaaS?](../../../sql-database/sql-database-paas-vs-sql-server-iaas.md?toc=%2fazure%2fvirtual-machines%2fwindows%2fsql%2ftoc.json)
## [SQL Server Data Tools (SSDT)](https://docs.microsoft.com/sql/ssdt/download-sql-server-data-tools-ssdt)
## [Ferramentas do SQL Server](https://docs.microsoft.com/sql/tools/overview-sql-tools)
## [Mapa do Azure](https://azure.microsoft.com/roadmap/?category=compute)
## [Stack Overflow](https://stackoverflow.com/search?q=%5Bazure-virtual-machine%5D+sql+server)