...
Por padrão, o nome de anfitrião é o mesmo que o nome VM quando se cria uma nova máquina virtual em Azure.  Para executar um script de cloud-init para alterar este nome de anfitrião padrão quando criar um VM `--custom-data` em Azure com [az vm criar](/cli/azure/vm), especifique o ficheiro cloud-init com o interruptor.  
...
Agora, crie um VM com [az vm criar](/cli/azure/vm) `--custom-data cloud_init_hostname.txt` e especificar o ficheiro cloud-init com o seguinte:
...
Uma vez criado, o Azure CLI mostra informações sobre o VM. Use `publicIpAddress` o SSH para o seu VM. Insira o seu próprio endereço da seguinte forma:

```bash
ssh <publicIpAddress>
```

Para ver o nome VM, utilize o comando da `hostname` seguinte forma:
