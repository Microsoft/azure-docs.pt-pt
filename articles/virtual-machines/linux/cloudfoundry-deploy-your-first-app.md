...
Para iniciar sessão, prepare-se `api` para o SYSTEMDOMAINURL que obteve a partir da implementação do mercado. Uma vez que a implementação predefinida utiliza `skip-ssl-validation` um certificado auto-assinado, deve incluir também o interruptor.
...
Agora, quando se implementa uma aplicação, é automaticamente criada no novo org e espaço. Para confirmar que não existem atualmente aplicações `cf apps` no novo org/space, escreva novamente.
...
Pode implementar a maioria das aplicações `push` para a Cloud Foundry utilizando o comando:
...
> [!NOTE] 
> Para saber mais sobre `cf push`o que acontece durante , veja [como as aplicações são encenadas][cf-push-docs] na documentação da Cloud Foundry.
...
Para ver os registos que já `recent` foram escritos, adicione o interruptor:
...
Por padrão, `cf push` apenas cria uma única instância da sua aplicação. Para garantir uma elevada disponibilidade e ativar a escala para uma maior saída, geralmente pretende executar mais do que uma instância das suas aplicações. Pode facilmente eliminar aplicações já `scale` implantadas utilizando o comando:

```bash
cf scale -i 2 hello-spring-cloud
```

Executar `cf app` o comando na aplicação mostra que a Cloud Foundry está a criar outra instância da aplicação. Uma vez iniciada a aplicação, a Cloud Foundry inicia automaticamente o tráfego de equilíbrio de carga.
