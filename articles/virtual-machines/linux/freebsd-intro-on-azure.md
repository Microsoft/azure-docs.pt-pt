...
Implantar uma máquina virtual FreeBSD é um processo simples usando uma imagem do Azure Marketplace do portal do Azure:
...
Durante a instalação, você será solicitado `Modify profile to update your $PATH and enable shell/tab completion now? (Y/n)`. Se você responder `y` e inserir `/etc/rc.conf` como `a path to an rc file to update`, você pode encontrar o problema `ERROR: [Errno 13] Permission denied`. Para resolver esse problema, você deve conceder o direito de gravação ao usuário atual no arquivo `etc/rc.conf`.
...
Atualmente, há suporte apenas para a chave RSA SSH. Uma chave SSH de várias linhas deve `---- BEGIN SSH2 PUBLIC KEY ----` começar com e `---- END SSH2 PUBLIC KEY ----`terminar com.
...
Opcionalmente, você pode obter um shell raiz usando `sudo -s`o.
