...
- SQL Server 2014: [Backup automatizado para máquinas virtuais SQL Server 2014 (Gerenciador de recursos)](https://docs.microsoft.com/azure/virtual-machines/windows/sql/virtual-machines-windows-sql-automated-backup)
...
* O SQL Server versões de não avaliação adquirem custos mais altos do que uma VM ' somente Windows ' implantada do Azure Marketplace. Consulte estes artigos para comparar os preços <https://azure.microsoft.com/pricing/details/virtual-machines/windows/> : <https://azure.microsoft.com/pricing/details/virtual-machines/sql-server-enterprise/>e. 
...
* Para o Windows Server 2008 R2, esse patch não existe e Always On precisaria ser usado da mesma maneira que o espelhamento de banco de dados, especificando um parceiro de failover na cadeia de conexão (feito por meio do padrão SAP. o parâmetro PFL Database/MSS/servidor-consulte a nota [SAP 965908]).
* Ao usar um ouvinte de grupo de disponibilidade, as VMs de banco de dados precisam estar conectadas a um Load Balancer dedicado. Para evitar que o Azure esteja atribuindo novos endereços IP em casos em que ambas as VMs sejam desligadas incidentalmente, uma delas deve atribuir endereços IP estáticos às interfaces de rede dessas VMs na configuração de Always On (definir um endereço IP estático é descrito em [ Este][virtual-networks-reserved-private-ip] artigo)
