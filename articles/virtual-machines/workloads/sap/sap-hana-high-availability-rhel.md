...
Começando pelo SAP HANA 2.0 SPS 01 SAP permite configurações ativas/ativas para replicação do sistema SAP HANA, onde os sistemas secundários de replicação do sistema SAP HANA podem ser utilizados ativamente para cargas de trabalho intensas de leitura. Para suportar esta configuração num cluster é necessário um segundo endereço IP virtual que permite aos clientes aceder à base de dados SAP HANA ativada por leitura secundária. Para garantir que o site de replicação secundária ainda pode ser acedido após a aquisição, o cluster precisa mover o endereço IP virtual em torno do secundário do recurso SAPHana.

Esta secção descreve os passos adicionais necessários para gerir a replicação do sistema ativado HANA Ative/Read num cluster de alta disponibilidade do Chapéu Vermelho com segundo IP virtual.    

Antes de prosseguir, certifique-se de que tem um cluster de alta disponibilidade de chapéu vermelho totalmente configurado que gere a base de dados SAP HANA, conforme descrito em segmentos acima da documentação.  
...
Para proceder a passos adicionais no fornecimento de segundo IP virtual, certifique-se de que configura o Balançador de Carga Azure, conforme descrito na secção [de Implementação Manual.](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/sap-hana-high-availability-rhel#manual-deployment)

1. Para o balanceador de carga **padrão,** siga abaixo os passos adicionais no mesmo equilibrador de carga que tinha criado na secção anterior.
   ...
   - Selecione **OK**.
   - Após a criação do novo pool IP frontal, note o endereço IP do pool.
...
2. Ao testar a falha do servidor, os segundos recursos IP virtuais **(rsc_secip_HN1_HDB03**) e o recurso de porta de carga Azure **(rsc_secnc_HN1_HDB03**) serão executados no servidor primário juntamente com os recursos IP virtuais primários.  Enquanto o servidor secundário está em baixo, as aplicações que estão ligadas à base de dados HANA ativada por leitura ligar-se-ão à base de dados HANA primária. O comportamento é esperado porque não pretende que as aplicações ligadas à base de dados HANA ativada por leitura sejam inacessíveis enquanto o servidor secundário estiver indisponível.
