---
title: Alta disponibilidade para SAP NetWeaver em VMs Azure
...
[sap-ha-multi-sid-guide]:high-availability-multi-sid.md (Configuração de alta disponibilidade DE SAP multi-SID)
...
Neste artigo, utilizamos modelos de Gestor de Recursos Azure para [netWeaver SAP de três níveis.](https://github.com/Azure/azure-quickstart-templates/) Para obter uma visão geral útil dos modelos, consulte [os modelos do GESTOR de Recursos SAP Azure](/archive/blogs/saponsqlserver/azure-quickstart-templates-for-sap).
...
No modelo de implantação clássica do Azure, existe uma dependência entre o equilibrador interno de carga Azure (serviço Azure Load Balancer) e o grupo de serviço de cloud. Todos os equilibradores internos precisam de um grupo de serviço de nuvem.

No Azure Resource Manager, não precisa de um grupo de recursos Azure para utilizar o Azure Load Balancer. O ambiente é mais simples e flexível.
...
3. Configure a Edição do Cluster SiOS DataKeeper para que espelhe o conteúdo do volume adicional ligado ao VHD da máquina virtual de origem ao volume adicional ligado à máquina virtual alvo. SiOS DataKeeper abstrata a fonte e os volumes locais alvo, e depois apresenta-os ao Windows Server Failover Clustering como um disco partilhado.
...
Para configurar o modelo multi-SID dos servidores de aplicações, no [modelo multi-SID dos servidores de aplicações,][sap-templates-3-tier-multisid-apps-marketplace-image]introduza valores para os seguintes parâmetros:
...
2. Selecione as suas definições com base no tipo de rede que tem. Para obter mais informações, veja os seguintes recursos:
   ...
   * Para implementações isoladas no Azure: Implemente uma máquina virtual adicional na mesma instância de Rede Virtual que serve como um servidor DNS. Adicione os endereços IP das máquinas virtuais Azure que criou para executar o serviço DNS.
...
Depois de instalar o Windows Failover Cluster com sucesso, é necessário fazer alterações em alguns limiares para adaptar a deteção de falhas às condições em Azure. Os parâmetros a alterar estão documentados neste blog: [https://techcommunity.microsoft.com/t5/Failover-Clustering/Tuning-Failover-Cluster-Network-Thresholds/ba-p/371834](https://techcommunity.microsoft.com/t5/Failover-Clustering/Tuning-Failover-Cluster-Network-Thresholds/ba-p/371834) . Assumindo que os seus dois VMs que constroem a Configuração do Cluster do Windows para ASCS/SCS estão na mesma SubNet, os seguintes parâmetros precisam de ser alterados para estes valores:  
...
Depois de instalar o SIOS DataKeeper em ambos os nós, tem de iniciar a configuração. O objetivo da configuração é ter uma replicação de dados sincronizada entre os VHDs adicionais ligados a cada uma das máquinas virtuais.
