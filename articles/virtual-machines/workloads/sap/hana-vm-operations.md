...
UltraSSD dá-lhe a possibilidade de definir um único disco que atenda a seu tamanho, IOPS e disco o intervalo de débito. Em vez de usar os gerentes de volume lógico como LVM ou MDADM com base no armazenamento Premium do Azure para construir os volumes que satisfazem os requisitos de débito do armazenamento e de IOPS. Pode executar uma mistura de configuração entre UltraSSD e o armazenamento Premium. Como resultado pode restringir a utilização de UltraSSD para os /hana/data críticos de desempenho e os volumes de /hana/log e abordar os outros volumes de armazenamento Premium do Azure
...
Os valores são apenas considerados um ponto de partida e tem de ser avaliada em comparação com as necessidades reais. A vantagem com SSD Ultra do Azure é que os valores de IOPS e débito podem ser adaptados sem a necessidade de encerrar a VM ou interrompendo a carga de trabalho aplicado ao sistema.   
