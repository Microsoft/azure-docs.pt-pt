...
Dois tipos de cópias de segurança devem ser executados para atingir esse recurso para restaurar:
...
- Se tem de restaurar uma base de dados para um determinado ponto no tempo, solicitar esse suporte do Microsoft Azure (por uma indisponibilidade de produção) ou o SAP HANA no Azure restauro para um determinado instantâneo de armazenamento. Um exemplo é uma restauração planeada de um sistema de proteção de segurança para o estado original.
...
Obtenha as mais recentes scripts de instantâneo e documentação de [GitHub](https://github.com/Azure/hana-large-instances-self-service-scripts/tree/master/snapshot_tools_v4.0). Ao baixar o pacote de script de instantâneo dos [GitHub](https://github.com/Azure/hana-large-instances-self-service-scripts/tree/master/snapshot_tools_v4.0), receberá três arquivos dos quais uma é uma documentação de PDF para a funcionalidade fornecida. Certifique-se de que prosseguir neste Roteiro as instruções no capítulo "Getting as ferramentas de instantâneo" quando transferir o conjunto de ferramentas.
...
SAP HANA no Azure (instâncias grandes) é fornecido com tamanhos de volume fixo para os volumes de dados e de registo do SAP HANA. Execução de instantâneos desses volumes come no espaço de volume. Tem de determinar, quando agendar instantâneos de armazenamento. Também terá de monitorizar o consumo de espaço dos volumes de armazenamento, bem como gerir o número de instantâneos que armazena. Pode desativar os instantâneos de armazenamento quando importar massas de dados ou efetuar outras alterações significativas na base de dados do HANA. 
...
Obtenha as mais recentes scripts de instantâneo e documentação de [GitHub](https://github.com/Azure/hana-large-instances-self-service-scripts/tree/master/snapshot_tools_v4.0). Para obter os passos detalhados listados acima, consulte [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf)

### <a name="consideration-for-mcod-scenarios"></a>Consideração para cenários MCOD
Se estiver a executar uma [cenário MCOD](https://launchpad.support.sap.com/#/notes/1681092) com várias instâncias do SAP HANA numa só unidade de instância grande do HANA, tem volumes de armazenamento separada aprovisionados para cada uma das instâncias do SAP HANA. Para obter detalhes sobre MDC e outras considerações, consulte [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf) capítulo **"Aspetos importantes a lembrar-se"**.
...
Este passo é descrito detalhadamente na verificação para versões mais recentes na [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf) capítulo **"Permitir a comunicação com o armazenamento de"**


### <a name="step-3-create-a-public-key"></a>Passo 3: Criar uma chave pública

Para ativar o acesso para as interfaces de instantâneo de armazenamento do seu inquilino de instância grande do HANA, tem de estabelecer um procedimento de início de sessão por meio de uma chave pública. No SAP HANA primeiro no servidor do Azure (instâncias grandes) no seu inquilino, crie uma chave pública para ser utilizado para aceder a infraestrutura de armazenamento. A chave pública garante que uma palavra-passe não é necessário para iniciar sessão para as interfaces de instantâneo de armazenamento. Criar uma chave pública também significa que não é necessário manter as credenciais de palavra-passe. Exatamente os passos como gerar público chave é descrita em [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf) capítulo **"Permitir a comunicação com o armazenamento de"**


### <a name="step-4-create-an-sap-hana-user-account"></a>Passo 4: Criar uma conta de utilizador do SAP HANA

Para iniciar a criação de instantâneos de SAP HANA, terá de criar uma conta de utilizador no SAP HANA que podem utilizar os scripts de instantâneo de armazenamento. Crie uma conta de utilizador do SAP HANA no SAP HANA Studio para esta finalidade. O utilizador tem de ser criado sob o SYSTEMDB e não sob a base de dados do SID para MDC. No ambiente de contentor único, o utilizador é criado na base de dados do inquilino. Esta conta tem de ter os seguintes privilégios: **Administrador de cópias de segurança** e **leitura do catálogo**. Para obter passos exatos configurar o utilizador e o utilizador, leia [GitHub](https://github.com/Azure/hana-large-instances-self-service-scripts/tree/master/snapshot_tools_v4.0) capítulo **"Permitir a comunicação com o SAP HANA"**
...
>[!IMPORTANT]
>Execute estes comandos de configuração com o mesmo contexto de utilizador, que os comandos de instantâneo são executados no. Caso contrário, os comandos de instantâneo não podem funcionar corretamente.
...
Baixe a versão mais recente dos scripts da [GitHub](https://github.com/Azure/hana-large-instances-self-service-scripts/tree/master/snapshot_tools_v4.0). A forma como os scripts vai ser instalado mudou majorly com a versão 4.0 dos scripts. Para obter detalhes exatos, leia [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf) capítulo **"Permitir a comunicação com o SAP HANA"**

Para obter a seqüência exata dos comandos, leia o capítulo **'Fácil instalação das ferramentas de instantâneo (predefinição)'** do documento [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf). Recomendamos a utilização da instalação predefinida. Se quiser atualizar da versão 3.x 4.0, consulte a secção **'Atualizar uma instalação existente'** dos [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf). Para desinstalar o conjunto de 4.0 ferramentas, siga as instruções em **'Desinstalação das ferramentas de instantâneo'** na [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf).

Não se esqueça de executar os passos descritos em **'Concluir a configuração das ferramentas de instantâneo'** dos [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf).

A finalidade das diferentes scripts e arquivos à medida que acabou de instalar está listada e detalhada na **"Quais são essas ferramentas de instantâneo?"** o documento [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf).

Antes de configurar as ferramentas de instantâneo, certifique-se de que também configurou as definições e localizações de cópia de segurança do HANA corretamente conforme descrito em **'SAP HANA configuração'** do documento [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf).

A configuração do conjunto de ferramentas do instantâneo é descrita detalhadamente nas **'Ficheiro de configuração - HANABackupCustomerDetails.txt'** do documento [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf).

#### <a name="testing-connectivity-with-sap-hana"></a>Testar a conectividade com o SAP HANA
...
Para obter detalhes, consulte **'Verificar a conectividade com o SAP HANA - testHANAConnection'** do documento [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf)

#### <a name="testing-storage-connectivity"></a>Testar a conectividade de armazenamento

A próxima etapa de teste é verificar a conectividade para o armazenamento com base nos dados que colocou na *HANABackupCustomerDetails.txt* configuração de ficheiros e, em seguida, executar um instantâneo de teste. Antes de executar o `azure_hana_backup` de comando, tem de executar este teste. A sequência de comandos para este teste está listada na **'Verificar a conectividade com o armazenamento - testStorageSnapshotConnection'** do documento [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf).
...
Se o instantâneo de teste tiver sido executado com êxito com o script, pode continuar com os instantâneos de armazenamento real de agendamento. Se não for bem-sucedida, investigue os problemas antes de prosseguir. O instantâneo de teste deve permanecer por perto, até que os primeiros instantâneos reais são feitos.
...
Para a sintaxe de comando exatas e a funcionalidade, leia **'Executar instantâneo cópia de segurança - azure_hana_backup'** do documento [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf).  

A execução do script `azure_hana_backup` cria o armazenamento de instantâneos nas seguintes três fases:
...
- Em casos extremos, acessar um armazenamento combinado de instantâneo sobre /hana/data e /hana/shared/SID que é 30 dias de antiguidade.
...
Após os primeiros instantâneos de armazenamento com êxito tiverem sido executados, tem de eliminar o instantâneo de teste que foram executado no passo 6. Leia **'Remover teste instantâneos - removeTestStorageSnapshot'** no documento [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf) para obter detalhes. 
...
Sintaxe da verificação de comandos e saídas **'Listar instantâneos - azure_hana_snapshot_details'** no documento [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf). 
...
Se já não pretende manter um conjunto de instantâneos com o prefixo de cópia de segurança **dailyhana** nos exemplos de sintaxe, pode executar o script com **0** como o número de retenção. Essa etiqueta de correspondência de todos os instantâneos, em seguida, são removidos. No entanto, a remoção de todos os instantâneos pode afetar as capacidades da funcionalidade de recuperação após desastre de instâncias grandes do HANA.
...
Para obter detalhes sobre o script, consulte **'Eliminar um instantâneo - azure_hana_snapshot_delete'** no documento [Microsoft instantâneo ferramentas para o SAP HANA no Azure](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/snapshot_tools_v4.0/Microsoft%20Snapshot%20Tools%20for%20SAP%20HANA%20on%20Azure%20v4.0.pdf).
...
Numa situação de diferente, um ponto anterior no tempo de recuperação poderá ser urgência baixa e planeadas dias com antecedência. Pode planear esta recuperação com o SAP HANA no Azure em vez de gerar um sinalizador de alta prioridade. Por exemplo, poderá ser planejando atualizar o software SAP através da aplicação de um novo pacote de aprimoramento. Em seguida, terá de reverter para um instantâneo que representa o estado antes da atualização do pacote de aprimoramento.
...
As possibilidades para obter um instantâneo restaurado com o novo conjunto de ferramentas estão documentadas na secção **'Como restaurar um instantâneo'** do documento [Manual recuperação guia para SAP HANA no Azure a partir de instantâneo de armazenamento](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/guides/Manual%20recovery%20of%20snapshot%20with%20HANA%20Studio.pdf).
...
1. Abra um pedido de suporte do Azure e inclui instruções sobre a restauração de um instantâneo específico.
   ...
   - Durante o restauro: SAP HANA no Azure podem pedir-lhe para assistir uma chamada de conferência para se certificar de coordenação, verificação e confirmação de que o instantâneo de armazenamento correta é restaurado. 

   - Após o restauro: SAP HANA no serviço do Azure notifica-o quando o instantâneo de armazenamento foi restaurado.
...
Outra possibilidade para obter, por exemplo, ficheiros de dados do SAP HANA recuperados de um instantâneo de armazenamento está documentada no passo 7 do documento [Manual recuperação guia para SAP HANA no Azure a partir de instantâneo de armazenamento](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/guides/Manual%20recovery%20of%20snapshot%20with%20HANA%20Studio.pdf).

O documento [Manual recuperação guia para SAP HANA no Azure a partir de instantâneo de armazenamento](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/guides/Manual%20recovery%20of%20snapshot%20with%20HANA%20Studio.pdf) ilustra a sequência de restauro da cópia de segurança do instantâneo. Use essa documentação para a execução de um restauro. 

>[!Note]
>Passo 7 não é necessário para executar se tem o instantâneo restaurado por operações de Microsoft.


### <a name="recover-to-another-point-in-time"></a>Recuperar para um outro ponto no tempo
O documento [Manual recuperação guia para SAP HANA no Azure a partir de instantâneo de armazenamento](https://github.com/Azure/hana-large-instances-self-service-scripts/blob/master/guides/Manual%20recovery%20of%20snapshot%20with%20HANA%20Studio.pdf) ilustra a sequência de restauro para um determinado ponto no tempo na secção **"Recuperar a base de dados para o seguinte ponto no tempo"**. Use essa documentação para a execução de um restauro para um determinado ponto no tempo. 


## <a name="next-steps"></a>Passos Seguintes
- Ver [princípios de recuperação após desastre e preparação](hana-concept-preparation.md).
