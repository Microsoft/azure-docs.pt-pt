...
ms.locfileid: "85218064"
---
# <a name="azure-storage-types-for-sap-workload"></a>Tipos de armazenamento Azure para carga de trabalho SAP
...
Existem vários métodos de redundância mais, que são todos descritos no artigo [Azure Storage replicação](https://docs.microsoft.com/azure/storage/common/storage-redundancy?toc=%2fazure%2fstorage%2fqueues%2ftoc.json) que se aplica a alguns dos diferentes tipos de armazenamento que a Azure tem para oferecer. 

A forma como estas opções de resiliência são aplicadas aos tipos de armazenamento Azure utilizados para SAP é descrita nas secções seguintes.
...
Nas próximas secções, os diferentes tipos de armazenamento Azure e a sua usabilidade para a carga de trabalho SAP são discutidos que se aplicam aos três cenários acima referidos. Uma categorização geral de como os diferentes tipos de armazenamento Azure devem ser utilizados está documentada no artigo [Que tipos de disco estão disponíveis no Azure?](https://docs.microsoft.com/azure/virtual-machines/linux/disks-types) As recomendações para a utilização dos diferentes tipos de armazenamento Azure para a carga de trabalho SAP não serão muito diferentes.
...
<sup>1</sup> Com a utilização do Acelerador de [Escrita Azure](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator) para famílias M/Mv2 VM para volumes de log/redo log <sup>2</sup> A utilização da ANF requer /hana/dados, bem como /hana/log para estar na ANF 
...
<sup>1</sup> Com a utilização do Acelerador de [Escrita Azure](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator) para famílias M/Mv2 VM para volumes de log/redo
...
- O IOPS per GiB não está a acompanhar
-  linear em todas as categorias de tamanho. Discos menores abaixo de 32 GiB têm taxas de IOPS mais altas por GiB. Para discos além de 32 GiB a 1024 GiB, a taxa de IOPS por GiB é entre 4-5 IOPS por GiB. Para discos maiores até 32.767 GiB, a taxa de IOPS por GiB está abaixo de 1
...
| Disco de dados | adequado | todos os sistemas - [especialmente para SAP HANA](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator) |
...
| IOPS máximo por disco | 20.000 [dependentes do tamanho do disco](https://azure.microsoft.com/pricing/details/managed-disks/) | Considere também [os limites de VM](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) |
| Produção SLA | SIM | - |
...
| HANA certificada | SIM | [especialmente para SAP HANA](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator) |
| Instantâneos de disco possíveis | SIM | - |
| Instantâneos VM de backup Azure possíveis | SIM | exceto para discos em cache [do acelerador de escrever](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator)  |
| Custos | MÉDIO | - |

O armazenamento premium Azure não preenche os KPI'kPI's de latência de armazenamento SAP HANA com os tipos comuns de caching oferecidos com armazenamento premium Azure. Para cumprir os KPI's de latência de armazenamento para as gravações SAP HANA, é necessário utilizar o caching do acelerador de escrita Azure, conforme descrito no artigo [Enable Write Accelerator](https://docs.microsoft.com/azure/virtual-machines/windows/how-to-enable-write-accelerator). O Azure Write Accelerator beneficia todos os outros sistemas DBMS para as suas gravações de transações e redações de registos de redo. Por isso, recomenda-se a sua utilização em todas as implementações do DBMS SAP. Para o SAP HANA, a utilização do Acelerador Azure Write em conjunto com o armazenamento premium Azure é obrigatória.
...
- IOPS provisível para o disco. Valores máximos diferentes aplicam-se à capacidade do disco. Leia o artigo [Disco ultra para](https://docs.microsoft.com/azure/virtual-machines/linux/disks-types#ultra-disk) mais detalhes
- Largura de banda de armazenamento a provisionada. A largura de banda máxima diferente aplica-se dependendo da capacidade do disco. Leia o artigo [Disco ultra para](https://docs.microsoft.com/azure/virtual-machines/linux/disks-types#ultra-disk) mais detalhes
...
- Implementações SAP HANA utilizando ações NFS v4.1 para /hana/data e /hana/log volumes e/ou NFS v4.1 ou volumes NFS v3 para volumes /hana/shared como documentado no artigo [CONFIGURAÇÕES de armazenamento de máquinas virtuais SAP HANA Azure](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage)
...
| Latência | muito baixo | - |
| IOPS SLA | SIM | - |
| IOPS linear à capacidade | estritamente linear  | Dependente do [Nível de Serviço](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-service-levels) |
| Produção SLA | SIM | - |
| Produção linear à capacidade | semi linear em parênteses | Dependente do [Nível de Serviço](https://docs.microsoft.com/azure/azure-netapp-files/azure-netapp-files-service-levels) |
| HANA certificada | SIM | - |
| Instantâneos de disco possíveis | SIM | - |
| Instantâneos VM de backup Azure possíveis | NO | - |
| Custos | Maior que o armazenamento Premium | - |
...
| --- | --- | --- | --- |
| HDD Standard | [Tamanhos para LMs Linux em Azure](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) | [Tamanhos para VMs windows em Azure](https://docs.microsoft.com/azure/virtual-machines/windows/sizes) | Provavelmente difícil de tocar nos limites de armazenamento de VMs médios ou grandes |
| SSD Standard | [Tamanhos para LMs Linux em Azure](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) | [Tamanhos para VMs windows em Azure](https://docs.microsoft.com/azure/virtual-machines/windows/sizes) | Provavelmente difícil de tocar nos limites de armazenamento de VMs médios ou grandes |
| Armazenamento Premium | [Tamanhos para LMs Linux em Azure](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) | [Tamanhos para VMs windows em Azure](https://docs.microsoft.com/azure/virtual-machines/windows/sizes) | Fácil de atingir limites de IOPS ou de produção de armazenamento VM com configuração de armazenamento |
| Armazenamento ultra disco | [Tamanhos para LMs Linux em Azure](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) | [Tamanhos para VMs windows em Azure](https://docs.microsoft.com/azure/virtual-machines/windows/sizes) | Fácil de atingir limites de IOPS ou de produção de armazenamento VM com configuração de armazenamento |
| Azure NetApp Files | [Tamanhos para LMs Linux em Azure](https://docs.microsoft.com/azure/virtual-machines/linux/sizes) | [Tamanhos para VMs windows em Azure](https://docs.microsoft.com/azure/virtual-machines/windows/sizes) | O tráfego de armazenamento está a utilizar largura de banda de produção de rede e não largura de banda de armazenamento! |
...
Para recomendações específicas sobre o tamanho das listras, leia a documentação para as diferentes configurações de [armazenamento de máquinas virtuais SAP HANA Azure](https://docs.microsoft.com/azure/virtual-machines/workloads/sap/hana-vm-operations-storage).
