...
## <a name="issues-with-creating-or-modifying-a-gallery"></a>Questões com a criação ou modificação de uma galeria ##
...
**Causa** : O nome próprio da galeria não satisfaz os requisitos de nomeação.  
**Solução alternativa** : Escolha um nome que satisfaça as seguintes condições: 1) Limite de 80 caracteres, 2) contém apenas letras, números, sublinhados em inglês e períodos, 3) começa e termina com letras ou números em inglês.
...
**Solução :** Escolha um nome para a galeria que satisfaça as seguintes condições: 1) Limite de 80 caracteres, 2) contém apenas letras, números, sublinhados e períodos, 3) começa e termina com letras ou números ingleses.
...
**Solução :** Verifique se o nome da região está escrito corretamente. Pode comandar este comando para ver a que regiões tem acesso. Se a região não constar da lista, apresente [um pedido de acesso](/troubleshoot/azure/general/region-access-request-process).

*Não pode eliminar recursos antes que os recursos aninhados sejam eliminados.*  
**Causa** : Tentou apagar uma galeria que contém pelo menos uma definição de imagem existente. Uma galeria deve estar vazia antes de poder ser apagada.  
**Solução alternativa** : Elimine todas as definições de imagem no interior da galeria e, em seguida, proceda à eliminação da galeria. Se a definição de imagem contiver versões de imagem, as versões de imagem devem ser eliminadas antes de eliminar as definições de imagem.
...
**Causa** : Já tem uma galeria existente no grupo de recursos com o mesmo nome e tentou criar outra galeria com o mesmo nome, mas numa região diferente.  
**Solução alternativa** : Utilize uma galeria diferente ou um grupo de recursos diferente.

## <a name="issues-with-creating-or-modifying-image-definitions"></a>Problemas com a criação ou modificação das definições de imagem ##

*Não é permitida a alteração da propriedade 'galleryImage.properties.<\> propriedade'.*  
**Causa:** Tentou alterar o tipo de SO, estado de SO, geração hiper V, oferta, editora, SKU. Não é permitido alterar qualquer uma destas propriedades.  
...
**Causa** : Já tem uma definição de imagem existente na mesma galeria e grupo de recursos com o mesmo nome e tentou criar outra definição de imagem com o mesmo nome e na mesma galeria, mas numa região diferente.  
...
**Causa** : A imagem <dadaDefinitionName \> termina com um período ou sublinhado.  
**Solução alternativa** : Escolha um nome para a definição de imagem que satisfaça as seguintes condições: 1) Limite de 80 caracteres, 2) contém apenas letras, números, hífenes, sublinhados e períodos, 3) começa e termina com letras ou números ingleses.
...
**Solução :** Escolha um valor que satisfaça as seguintes condições: 1) Limite de 128 caracteres para editor ou limite de 64 caracteres para oferta e SKU, 2) contém apenas letras, números, hífens, sublinhados, e períodos e 3) não termina com um período.
...
**Solução :** Verifique se o nome da galeria, da subscrição e do grupo de recursos estão corretos. Caso contrário, crie uma nova galeria chamada <\> galeriaName.

*A localização fornecida <região \> não está disponível para o tipo de recurso 'Microsoft.Compute/galleries'. A lista das regiões disponíveis para o tipo de recurso é ...*  
**Causa** : A região <\> está incorreta ou requer um pedido de acesso  
**Solução :** Verifique se o nome da região está escrito corretamente. Pode comandar este comando para ver a que regiões tem acesso. Se a região não constar da lista, apresente [um pedido de acesso](/troubleshoot/azure/general/region-access-request-process).
...
**Causa** : A propriedade de data de fim de vida não está devidamente formatada como uma data que é após a data de hoje.  
**Solução :** Forneça uma data no formato yyyy-MM-dd, yyyy-MM-dd'T'HH:mm:sszzz ou [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601)-válido.
...
**Causa** : Tentou criar uma nova definição de imagem com a mesma editora, oferta, trigémeo SKU como uma definição de imagem existente na mesma galeria.  
**Resumo :** Dentro de uma determinada galeria, todas as definições de imagem devem ter uma combinação única de editor, oferta, SKU. Escolha uma combinação única ou escolha uma nova galeria e crie novamente a definição de imagem.

*Não pode eliminar recursos antes que os recursos aninhados sejam eliminados.*  
**Causa** : Tentou eliminar uma definição de imagem que contém versões de imagem. Uma definição de imagem deve estar vazia antes de poder ser eliminada.  
...
**Causa** : Escolheu um ZRS Standard numa região que ainda não o apoia.  
**Solução alternativa** : Altere o tipo de conta de armazenamento para 'Premium \_ LRS' ou \_ 'Standard LRS'. Consulte a nossa documentação para obter a mais recente [lista de regiões](/azure/storage/common/storage-redundancy#zone-redundant-storage) com pré-visualização ZRS ativada.
...
**Solução :** Verifique se o nome da galeria, da subscrição e do grupo de recursos estão corretos. Caso contrário, crie uma nova galeria com o nome <galeriaName \> e/ou definição de imagem nomeada <imagemDefinitionName \> no grupo de recursos indicado.
...
**Causa** : A localização da imagem de origem (<origem \> Região) deve ser incluída na lista <publicação Das Regiões \>  
**Solução alternativa** : Incluir < fonteRegion \> na lista de <publicando Regiões. \>
...
**Solução alternativa** : Escolha um valor que esteja dentro do alcance de <minValue \> e <maxValue \> inclusive.
...
**Causa** : Não existe nenhuma fonte localizada no <recursosID \> ou na fonte <recursosID não está na mesma \> região que a imagem da galeria que está a ser criada.  
**Resumo** : Verifique se o <recursosID \> está correto e que a região de origem da versão de imagem de galeria é a mesma que a região do <recursosID\>
...
**Solução alternativa** : Remova ou altere os números de lúgubres duplicados.
...
**Causa** : Ao criar uma versão de imagem utilizando uma lista de discos e/ou instantâneos de disco, dois ou mais discos ou instantâneos de disco têm o mesmo ID de recurso.  
**Solução alternativa** : Remova ou altere quaisquer identificações de origem de disco duplicados.
...
**Causa** : O <recursosID \> está incorretamente formatado.  
**Resolução :** Verifique se o recursoID está correto.

*O id de origem: <recursosID \> deve ser uma imagem gerida, máquina virtual ou outra versão de imagem de galeria*  
**Causa** : O <recursosID \> está incorretamente formatado.  
**Resumo :** Se utilizar uma versão de imagem VM, gerida ou de imagem de galeria como imagem de origem, verifique se o ID de recurso do VM, imagem gerida ou versão de imagem de galeria está correto.

*O id de origem: <recursosID \> devem ser um disco ou instantâneo gerido.*  
**Causa** : O <recursosID \> está incorretamente formatado.  
**Resumo :** Se utilizar discos e/ou instantâneos de disco como fontes para a versão de imagem, verifique se os IDs de recursos dos discos e/ou instantâneos do disco estão corretos.
...
**Causa** : A geração Hypervisor da versão de imagem não corresponde à geração Hypervisor especificada na definição de imagem. O sistema operativo de definição de imagem é <V# \_ 1 \> e o sistema operativo da versão de imagem é <V# \_ 2 \> .  
**Solução alternativa** : Escolha uma fonte com a mesma geração Hypervisor que a definição de imagem ou crie/escolha uma nova definição de imagem que tenha a mesma geração Hypervisor que a versão de imagem.
...
**Causa** : A geração Hypervisor da versão de imagem não corresponde à geração Hypervisor especificada na definição de imagem. O sistema operativo de definição de imagem é <OsType \_ 1 \> e o sistema operativo da versão de imagem é <OsType \_ 2 \> .  
...
**Causa** : A fonte do '<\> recursosID' contém um disco de OS efémero. A Galeria de Imagens Partilhadas não suporta atualmente o disco EFÉMER OS.  
...
**Causa** : O disco '<diskID \> é um disco UltraSSD. A Galeria de Imagens Partilhadas não suporta atualmente discos Ultra SSD.  
**Solução alternativa** : Utilize uma fonte que contenha apenas discos premium SSD, Standard SSD e/ou Standard HDD.
...
**Solução alternativa** : Utilize uma fonte baseada num VM que contenha apenas discos premium SSD, Standard SSD e/ou Standard HDD.
...
**Solução alternativa** : Experimente a criação da versão de imagem mais tarde.
...
**Causa** : A Galeria de Imagens Partilhadas não suporta atualmente a criação de versões de imagem noutra subscrição a partir de outra imagem de origem se a imagem de origem estiver encriptada.  
...
**Causa** : O formato válido para uma versão de imagem é 3 inteiros separados por um período. O nome da versão de imagem não correspondeu ao formato válido.  
...
**Causa** : Adicione encriptação ao disco de dados de uma versão de imagem existente. Não é possível adicionar encriptação a uma versão de imagem existente.  
...
**Causa** : O recursoID da fonte pode estar incorreto.  
**Resumo :** Certifique-se de que o recursoID da fonte está correto.
...
**Solução alternativa** : Se utilizar encriptação no disco OS, utilize encriptação em todas as regiões.
...
**Solução alternativa** : Se utilizar encriptação num disco de dados, utilize encriptação em todas as regiões.

*Um lun inválido [número <\> ] foi especificado em encryption.dataDiskImages. O lun deve ser um dos seguintes valores ['0,9'].*  
**Causa** : O número lun especificado para a encriptação não corresponde a nenhum dos números de lúgubre para discos ligados ao VM.  
**Resumo :** Altere o número de lun na encriptação para o número lun de um disco de dados presente no VM.

*O número de <da \> região-alvo foi especificado na área-alvo de encriptação '<\> região'.*  
**Causa** : As definições de encriptação utilizadas na região <\> especificaram um número de lun pelo menos duas vezes.  
**Solução :** Alterar o número de lun na região <\> para garantir que todos os números de lun são únicos na região \> <.

*OSDiskImage e DataDiskImage não podem apontar para a mesma bolha <fonteID\>*  
**Causa** : A fonte do disco SO e pelo menos um disco de dados não são únicos.  
...
**Causa** : Uma chamada de atualização para a versão de imagem contém um novo disco de dados ou tem um novo número LUN para um disco.  
**Resumo :** Utilize os números LUN e os discos de dados da versão de imagem existente.
...
**Causa** : A Galeria de Imagens Partilhadas não suporta atualmente a utilização de uma encriptação de disco definida numa subscrição diferente.  
**Solução alternativa** : Crie a versão de imagem e a encriptação do disco definidas na mesma subscrição.

## <a name="issues-creating-or-updating-a-vm-or-scale-sets-from-image-version"></a>Problemas que criam ou atualizam um VM ou conjuntos de escala a partir da versão de imagem ##
...
**Causa** : A máquina virtual ou conjunto de escala foi criado usando uma imagem SIG em outro inquilino. Tentou fazer uma alteração na máquina virtual ou no conjunto de escala, mas não tem acesso à subscrição que detém a imagem.  
...
**Causa** : O VM está a ser criado numa região que não faz parte da lista de regiões publicadas para a imagem da galeria.  
...
**Causa** : Nome de utilizador, palavra-passe ou chaves de ssh de administração foram fornecidas para um VM que foi criado a partir de uma versão de imagem especializada.  
**Solução alternativa** : Não inclua o nome de utilizador, palavra-passe ou teclas ssh de administração se pretender criar um VM a partir dessa imagem. Caso contrário, utilize uma versão de imagem generalizada e forneça o nome de utilizador, palavra-passe ou teclas ssh.

*Falta o parâmetro 'osProfile' do parâmetro exigido (nulo).*  
**Causa** : O VM é criado a partir de uma imagem generalizada e falta-lhe o nome de utilizador, palavra-passe ou teclas ssh. Uma vez que as imagens generalizadas não retêm o nome de utilizador, palavra-passe ou teclas ssh, estes campos devem ser especificados durante a criação de um VM ou conjunto de escala.  
**Solução :** Especifique o nome de utilizador, palavra-passe ou teclas ssh ou utilize uma versão de imagem especializada.
...
**Causa** : A imagem de origem atual para o conjunto de escalas é uma imagem de origem generalizada, mas está a ser atualizada com uma imagem de origem especializada. A imagem de origem atual e a nova imagem de origem para um conjunto de escalas devem ser do mesmo estado.  
...
**Resumo :** Verifique se o **OSType** da versão de imagem que está a tentar criar o conjunto de escala de VM ou de máquina virtual tem o mesmo **OSType** da fonte que usou para criar a versão de imagem. 

## <a name="issues-creating-a-disk-from-an-image-version"></a>Problemas na criação de um disco a partir de uma versão de imagem ##

*O valor da imagem do parâmetroReferência é inválido.*  
**Causa** : Tentou exportar de uma versão SIG Image para um disco, mas utilizou uma posição LUN que não existe na imagem.    
