...
Uma vez entregue a sequência SysRq, a configuração do núcleo controlará a forma como o sistema reage. Para obter informações sobre a ativação e desativação do SysRq, consulte o [texto](https://aka.ms/kernelorgsysreqdoc) do *Guia de Administração SysRq* | [marcação](https://aka.ms/linuxsysrq).
...
A escolha do "Comando Enviar SysRq" abrirá um diálogo, que fornecerá opções comuns de SysRq ou aceitará uma sequência de comandos SysRq introduzidos no diálogo.  Isto permite que séries de SysRq realizem uma operação de alto nível, como um reboot seguro usando: `REISUB`.
...
|``h``  |   Mostrará ajuda (qualquer outra chave que não a listada aqui também mostrará ajuda, mas ``h`` é fácil de lembrar :-)
...
|``0``-``9`` | Define o nível de registo da consola, controlando quais as mensagens de kernel que serão impressas na sua consola. (``0``, por exemplo, o tornaria de modo a que apenas mensagens de emergência como PANICs ou OOPSes pudessem chegar à sua consola.)
