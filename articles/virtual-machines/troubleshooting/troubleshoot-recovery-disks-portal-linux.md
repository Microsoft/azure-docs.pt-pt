...
Examine o diagnóstico de inicialização e a captura de tela da VM para determinar por que sua VM não pode ser inicializada corretamente. Um exemplo comum seria uma entrada inválida em `/etc/fstab`ou um disco rígido virtual subjacente sendo excluído ou movido.
...
1. SSH para sua VM de solução de problemas usando as credenciais apropriadas. Se esse disco for o primeiro disco de dados anexado à sua VM de solução de problemas, provavelmente ele estará conectado a `/dev/sdc`. Use `dmseg` para listar os discos anexados:
...
Para obter mais informações sobre como usar o Gerenciador de recursos, consulte [Azure Resource Manager visão geral](../../azure-resource-manager/management/overview.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json).
