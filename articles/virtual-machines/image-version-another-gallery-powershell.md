...
description: Copie uma imagem de outra galeria usando o Azure PowerShell.
author: cynthn
...
ms.locfileid: "82796931"
---
# <a name="copy-an-image-from-another-gallery"></a>Copiar uma imagem de outra galeria

Se tiver várias galerias na sua organização, pode criar imagens a partir de imagens armazenadas noutras galerias. Por exemplo, você pode ter uma galeria de desenvolvimento e teste para criar e testar novas imagens. Quando estiverem prontos para serem utilizados em produção, pode copiá-los numa galeria de produção usando este exemplo. Também pode criar uma imagem a partir de uma imagem em outra galeria usando o [Azure CLI](image-version-another-gallery-cli.md).
...
Você precisará de informações da definição de imagem de origem para que você possa criar uma cópia dela na sua galeria de destino.

Enumera informações sobre as galerias existentes, definições de imagem e versões de imagem utilizando o cmdlet [Get-AzResource.](/powershell/module/az.resources/get-azresource)
...
Assim que tiver toda a informação de que necessita, pode obter o ID da versão de imagem de origem utilizando [a Get-AzGalleryImageVersion](/powershell/module/az.compute/get-azgalleryimageversion). Neste exemplo, estamos a `1.0.0` receber a `myImageDefinition` versão de `myGallery` imagem, da `myResourceGroup` definição, na galeria de origem, no grupo de recursos.
...
Precisa criar uma nova definição de imagem que corresponda à definição de imagem da sua fonte. Pode ver toda a informação necessária para recriar a definição de imagem utilizando [a Get-AzGalleryImageDefinition](/powershell/module/az.compute/get-azgalleryimagedefinition).
...
Crie uma nova definição de imagem, na sua galeria de destino, utilizando o cmdlet [New-AzGalleryImageDefinition](https://docs.microsoft.com/powershell/module/az.compute/new-azgalleryimageversion) e as informações a partir da saída acima.


Neste exemplo, a definição de imagem é denominada *myDestinationImgDef* na galeria denominada *myDestinationGallery*.
...
Crie uma versão de imagem utilizando a Versão De Imagem [New-AzGallery](https://docs.microsoft.com/powershell/module/az.compute/new-azgalleryimageversion). Terá de passar a identificação da imagem `--managed-image` de origem no parâmetro para criar a versão de imagem na sua galeria de destino. 
...
Neste exemplo, a galeria de destino *supor-se myDestinationGallery,* no grupo de recursos *myDestinationRG,* na localização *dos EUA Ocidentais.* A versão da nossa imagem é *1.0.0* e vamos criar uma réplica na região *centro-sul dos EUA,* e 2 réplicas na região *oeste dos EUA.* 
