...
>[!TIP]
> Pode ver o `az sql: 'vm' is not in the 'az sql' command group` erro se estiver a utilizar uma versão desatualizada do Azure CLI. Descarregue a [versão mais recente do Azure CLI](https://docs.microsoft.com/cli/azure/install-azure-cli-windows?view=azure-cli-latest) para ultrapassar este erro.
...
az sql vm add-to-group -n <VM2 Name> -g <Resource Group Name> --sqlvm-group <cluster name> `
  -b <bootstrap account password> -p <operator account password> -s <service account password>
```
Utilize este comando para adicionar quaisquer outros VMs do Servidor SQL ao cluster. Modifique `-n` apenas o parâmetro para o nome VM do Servidor SQL. 
...
O ID do recurso de `/subnets/<subnetname>` *sub-rede* é o valor do dispositivo anexado ao id de recurso do recurso de rede virtual. Para identificar o ID do recurso da sub-rede:
   ...
   1. Identifique o ID de recurso para `/subnets/<subnetname>` a rede virtual e adere ao fim da rede para criar o ID de recurso da sub-rede. Por exemplo:
