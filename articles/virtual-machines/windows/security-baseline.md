---
title: Linha de segurança Azure para Máquinas Virtuais do Windows
description: A linha de base de segurança windows Virtual Machines fornece orientações e recursos processuais para a implementação das recomendações de segurança especificadas no Azure Security Benchmark.
author: msmbaldwin
...
ms.locfileid: "107285097"
---
# <a name="azure-security-baseline-for-windows-virtual-machines"></a>Linha de segurança Azure para Máquinas Virtuais do Windows

Esta linha de base de segurança aplica orientações da [versão 1.0 do Benchmark de Segurança Azure](../../security/benchmarks/overview-v1.md) às Máquinas Virtuais do Windows. A Referência de Segurança do Azure disponibiliza recomendações para proteger as suas soluções cloud no Azure. O conteúdo é agrupado pelos controlos de **segurança definidos** pelo Azure Security Benchmark e pela orientação conexa aplicável às Máquinas Virtuais do Windows.

> [!NOTE]
> Foram excluídos **os controlos** não aplicáveis às Máquinas Virtuais do Windows, ou para os quais a responsabilidade é da Microsoft. Para ver como as máquinas virtuais do Windows mapeiam completamente o Benchmark de Segurança Azure, consulte o **[ficheiro de mapeamento de base de base de segurança do Windows Virtual Machines](https://github.com/MicrosoftDocs/SecurityBenchmarks/blob/master/Azure%20Offer%20Security%20Baselines/1.1/windows-virtual-machines-security-baseline-v1.1.xlsx)**.
...
**Orientação**: Pode implementar o Gateway de aplicações Azure para aplicações web com HTTPS/SSL ativado para certificados fidedignos. Com o Azure Application Gateway, direciona o tráfego web da sua aplicação para recursos específicos, atribuindo os ouvintes às portas, criando regras e adicionando recursos a um pool de backend como máquinas Virtual Windows.
...
Utilize a Política Azure para validar (e/ou remediar) configurações para recursos de rede relacionados com máquinas virtuais do Windows.
...
**Orientação**: O Centro de Segurança Azure fornece monitorização de registo de eventos de segurança para windows Virtual Machines. Dado o volume de dados que o registo do evento de segurança gera, este não é armazenado por padrão.

- [Se a sua organização pretender reter os dados de registo de eventos de segurança da máquina virtual, pode ser armazenado dentro de um nível de Recolha de Dados, altura em que pode ser consultado no Log Analytics. Existem diferentes níveis: Minimal, Common e All, que são detalhados no seguinte link](https://docs.microsoft.com/azure/security-center/security-center-enable-data-collection#data-collection-tier)
...
- [Como recolher registos e métricas da plataforma com o Azure Monitor](https://docs.microsoft.com/azure/azure-monitor/essentials/activity-log#view-the-activity-log)

- [Visão geral do agente de analítica de registo](../../azure-monitor/agents/log-analytics-agent.md)

- [Registar extensão de máquina virtual de analítica para Windows](../extensions/oms-windows.md)
...
**Orientação**: Ativar o agente Log Analytics, também designado por Agente de Monitorização da Microsoft (MMA), e configur-o para enviar registos para um espaço de trabalho do Log Analytics. O agente do Windows envia dados recolhidos de diferentes fontes para o seu espaço de trabalho Log Analytics no Azure Monitor, bem como quaisquer registos ou métricas únicos, tal como definidos numa solução de monitorização.
...
**Orientação**: Pode utilizar o Microsoft Antimalware para serviços em nuvem azure e máquinas virtuais e configurar as suas máquinas virtuais para registar eventos numa conta de armazenamento Azure. Configure um espaço de trabalho Log Analytics para ingerir os eventos a partir das Contas de Armazenamento e criar alertas sempre que apropriado. Siga as recomendações no Azure Security Center: "Compute &amp; Apps". 
...
**Orientação**: As máquinas virtuais do Windows e o Azure Ative Directory (Azure AD) não têm o conceito de senhas padrão. Cliente responsável por aplicações de terceiros e serviços de marketplace que podem usar senhas padrão.
...
### <a name="36-use-secure-azure-managed-workstations-for-administrative-tasks"></a>3.6: Utilizar estações de trabalho seguras e geridas pela Azure para tarefas administrativas 
...
**Orientação**: Os discos virtuais nas Máquinas Virtuais do Windows (VM) são encriptados em repouso utilizando a encriptação do lado do Servidor ou a encriptação do disco Azure (ADE). A Azure Disk Encryption aproveita a funcionalidade BitLocker do Windows para encriptar discos geridos com chaves geridas pelo cliente dentro do VM do hóspede. A encriptação do lado do servidor com as teclas geridas pelo cliente melhora no ADE, permitindo-lhe utilizar quaisquer tipos e imagens de OS para os seus VMs encriptando dados no serviço de Armazenamento.

- [Encriptação lateral do servidor de discos geridos pelo Azure](/azure/virtual-machines/windows/disk-encryption)

- [Encriptação do disco Azure para VMs windows](/azure/virtual-machines/windows/disk-encryption-overview)
...
- [Informação sobre a criação de modelos do Gestor de Recursos Azure](ps-template.md)
...
- [Como carregar um VM VHD personalizado para Azure](upload-generalized-managed.md)
...
- [Como integrar-se com identidades geridas pela Azure](../../azure-app-configuration/howto-integrate-azure-managed-service-identity.md)
...
**Orientação**: Utilize o Microsoft Antimalware para máquinas Azure Windows Virtual para monitorizar e defender continuamente os seus recursos.
...
**Orientação**: Quando faz cópia de segurança os discos geridos pelo Azure com a Cópia de Segurança Azure, os VMs são encriptados em repouso com a Encriptação do Serviço de Armazenamento (SSE). O Azure Backup também pode fazer backup de VMs Azure que são encriptados usando Azure Disk Encryption. A encriptação do disco Azure integra-se com chaves de encriptação BitLocker (BEKs), que são salvaguardadas num cofre-chave como segredos. A encriptação do disco Azure também se integra com chaves de encriptação chave Azure Key Vault (KEKs). Ativar Soft-Delete no Cofre de Chaves para proteger as chaves contra a eliminação acidental ou maliciosa.

- [Excluir suave para VMs](../../backup/soft-delete-virtual-machines.md)

- [Descrição geral da eliminação recuperável do Azure Key Vault](/azure/key-vault/general/overview-soft-delete)
