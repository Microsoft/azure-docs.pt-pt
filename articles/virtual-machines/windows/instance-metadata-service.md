...
```
> [!NOTE]
> A `-NoProxy` bandeira só está disponível no PowerShell 6 ou superior. Pode omitir a bandeira se não tiver uma configuração de procuração.

**Response**

> [!NOTE]
> A resposta é uma corda JSON. Canalizamos a nossa consulta REST através do `ConvertTo-Json` cmdlet para impressão bonita.
...
> [!NOTE]
> O certificado para nuvem pública e cada nuvem soberana será diferente.
...
## <a name="failover-clustering-in-windows-server"></a>Agrupamento de falha no servidor do Windows

Para certos cenários ao consultar o Serviço de Metadados de Exemplo com Clustering Failover, é necessário adicionar uma rota à tabela de encaminhamento.

1. Abrir o pedido de comando com privilégios de administrador.

1. Executar o seguinte comando e anotar o endereço da Interface para Destino de Rede ( `0.0.0.0` ) na Tabela de Rota IPv4.
...
> [!NOTE]
> A saída de exemplo a partir de um VM do Servidor do Windows com o Cluster Failover ativado contém apenas a Tabela de Rotas IPv4 para a simplicidade.
