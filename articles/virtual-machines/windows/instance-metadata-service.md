---
title: Serviço de metadados de caso Azure para Windows
description: Saiba mais sobre o Serviço de Metadados Azure Instance e como fornece informações sobre casos de máquinas virtuais atualmente em execução no Windows.
...
```
> [!NOTE]
> A `-NoProxy` bandeira só está disponível no PowerShell 6 ou mais tarde. Pode omitir a bandeira se não tiver uma configuração de procuração.

**Response**

> [!NOTE]
> A resposta é uma corda JSON. Encaneie a sua consulta REST através do `ConvertTo-Json` cmdlet para uma impressão bonita.
...
publicador | O editor da imagem VM. | 2017-04-02
...
Os metadados de armazenamento fazem parte do caso API, no `instance/compute/storageProfile` ponto final.
...
> [!NOTE]
> O certificado intermédio para Azure China 21Vianet é da DigiCert Global Root CA, em vez de Baltimore.
...
## <a name="failover-clustering-in-windows-server"></a>Agrupamento de falha no Windows Server
...
Se houver um elemento de dados não encontrado ou um pedido mal formado, o IMDS devolve erros HTTP padrão. Por exemplo:
