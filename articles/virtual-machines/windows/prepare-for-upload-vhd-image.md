...
1. Verifique se o VHD do Windows funciona corretamente no servidor local. Resolva quaisquer erros dentro da própria VM antes de tentar convertê-los ou carregá-los no Azure.
...
Você pode converter um disco virtual usando o comando [Convert-VHD](https://technet.microsoft.com/library/hh848454.aspx) no Windows PowerShell. Selecione **Executar como administrador** ao iniciar o PowerShell. 
...
### <a name="convert-from-vmware-vmdk-disk-format"></a>Converter do formato de disco do VMware VMDK
...
1. Remova qualquer rota persistente estática na tabela de roteamento:
   ...
   * Para exibir a tabela de rotas, `route print` execute no prompt de comando.
...
2. A porta RDP está configurada corretamente. A porta padrão é 3389:
   ...
   Set-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server\Winstations\RDP-Tcp' -name "PortNumber" -Value 3389 -Type DWord -force
    ```
    Quando você implanta uma VM, as regras padrão são criadas na porta 3389. Se você quiser alterar o número da porta, faça isso depois que a VM for implantada no Azure.
...
6. Reconectar
    ...
    ```
7. Limitar o número de conexões simultâneas:
...
9. Se a VM for parte de um domínio, verifique as políticas a seguir para certificar-se de que as configurações anteriores não sejam revertidas. 
    ...
    | Número limitado de configurações de conexão | Computador \ \ \ \ \ Configuration\policies\windows Administrativos\Componentes da área de trabalho do Remota \ sessão do Host\Connections | Limitar número de conexões                                                              |
...
5. Se a VM for parte de um domínio, verifique as seguintes políticas do Azure AD para certificar-se de que as configurações anteriores não são revertidas. 
    ...
    | Objetivo                                 | Política                                                                                                                                                  | Value                                   |
    |--------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------|
    | Habilitar os perfis do firewall do Windows | Computador \ \ \ Configuration\policies\windows Templates\Network\Network Connection\Windows Firewall\Domain Profile\Windows firewall   | Proteger todas as conexões de rede         |
    | Habilitar RDP                           | Computador \ \ \ Configuration\policies\windows Templates\Network\Network Connection\Windows Firewall\Domain Profile\Windows firewall   | Permitir exceções de Área de Trabalho Remota de entrada |
    |                                      | Computador \ \ \ Configuration\policies\windows Templates\Network\Network Connection\Windows Firewall\Standard Profile\Windows firewall | Permitir exceções de Área de Trabalho Remota de entrada |
    | Habilitar ICMP-v4                       | Computador \ \ \ Configuration\policies\windows Templates\Network\Network Connection\Windows Firewall\Domain Profile\Windows firewall   | Permitir exceções ICMP                   |
    |                                      | Computador \ \ \ Configuration\policies\windows Templates\Network\Network Connection\Windows Firewall\Standard Profile\Windows firewall | Permitir exceções ICMP                   |
...
2. Defina as configurações de Dados de Configuração da Inicialização (BCD). 
...
5. Verifique se nenhum outro aplicativo está usando a porta 3389. Essa porta é usada para o serviço RDP no Azure. Para ver quais portas são usadas na VM, execute `netstat -anob`:
...
6. Para carregar um VHD do Windows que seja um controlador de domínio:
   ...
   * Siga [estas etapas adicionais](https://support.microsoft.com/kb/2904015) para preparar o disco.
...
7. Verifique se você conhece a conta de administrador interna e a senha. Talvez você queira redefinir a senha do administrador local atual e verificar se pode usar essa conta para entrar no Windows por meio da conexão RDP. Essa permissão de acesso é controlada pelo objeto de Política de Grupo "permitir logon Serviços de Área de Trabalho Remota". Exibir este objeto no Editor de Política de Grupo Local aqui:
    ...
    Computador \ \ \ Configurações de atribuição de direitos \ Computer
...
8. Verifique as seguintes políticas do Azure AD para certificar-se de que você não está bloqueando o acesso RDP por meio de RDP ou da rede:
    ...
    - O computador \ \ \ \ \ \ \ \ negar de direitos acesso a este computador pela rede

    - O computador \ \ \ \ \ \ \ \ \ \ \ \ \ \% negar Serviços de Área de Trabalho Remota logon
...
- [Criar uma VM de um disco especializado](create-vm-specialized.md)
- [Criar uma VM com base em um disco VHD especializado](https://docs.microsoft.com/azure/virtual-machines/windows/create-vm-specialized-portal?branch=master)
...
As configurações a seguir não afetam o carregamento do VHD. No entanto, é altamente recomendável que você os tenha configurado.

* Instale o [agente de máquina virtual do Azure](https://go.microsoft.com/fwlink/?LinkID=394789&clcid=0x409). Em seguida, você pode habilitar extensões de VM. As extensões de VM implementam a maior parte da funcionalidade crítica que você pode querer usar com suas VMs. Você precisará das extensões, por exemplo, para redefinir senhas ou configurar o RDP. Para obter mais informações, consulte [visão geral do agente de máquina virtual do Azure](../extensions/agent-windows.md).
