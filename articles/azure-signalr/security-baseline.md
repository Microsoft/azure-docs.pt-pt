---
title: Linha de segurança Azure para o Serviço Azure SignalR
description: A linha de base de segurança do Serviço Azure SignalR fornece orientações processuais e recursos para a implementação das recomendações de segurança especificadas no Benchmark de Segurança Azure.
author: msmbaldwin
ms.service: signalr
ms.topic: conceptual
ms.date: 11/25/2020
...
ms.locfileid: "101094970"
---
# <a name="azure-security-baseline-for-azure-signalr-service"></a>Linha de segurança Azure para o Serviço Azure SignalR

Esta linha de base de segurança aplica orientações da [versão 2.0 do Azure Security Benchmark](../security/benchmarks/overview.md) para Azure SignalR. A Referência de Segurança do Azure disponibiliza recomendações para proteger as suas soluções cloud no Azure.
O conteúdo é agrupado pelos **controlos de segurança definidos** pelo Azure Security Benchmark e pela orientação conexa aplicável ao Azure SignalR. Foram excluídos **os controlos** não aplicáveis ao Azure SignalR.

 
Para ver como o Azure SignalR mapeia completamente para o Benchmark de Segurança Azure, consulte o [ficheiro de mapeamento de base de segurança Azure SignalR](https://github.com/MicrosoftDocs/SecurityBenchmarks/tree/master/Azure%20Offer%20Security%20Baselines).
...
**Orientação**: O Microsoft Azure SignalR Service não suporta a implantação diretamente numa rede virtual, pois não é possível alavancar certas funcionalidades de rede com os recursos da oferta, tais como grupos de segurança de rede, tabelas de rotas ou outros aparelhos dependentes da rede, como um Azure Firewall. 

No entanto, o Serviço Azure SignalR permite-lhe criar pontos finais privados para garantir o tráfego entre os recursos da sua rede virtual e do Serviço Azure SignalR.

Também pode utilizar tags de serviço e configurar as regras do grupo de segurança da rede para restringir o tráfego de entrada/saída ao Serviço Azure SignalR.

- [Utilize pontos finais privados para o serviço Azure SignalR](howto-private-endpoints.md)

- [Configure o controlo de acesso à rede](howto-network-access-control.md)

- [Utilize etiquetas de serviço para o Serviço Azure SignalR](howto-service-tags.md)
...
**Orientação**: Utilize pontos finais privados para proteger o tráfego entre a sua rede virtual e o Serviço Azure SignalR. Escolha a Azure ExpressRoute ou a rede virtual Azure (VPN) para criar ligações privadas entre centros de dados Azure e infraestruturas no local num ambiente de cosão. 
...
**Orientação**: Utilize o Azure Private Link para permitir o acesso privado ao Serviço Azure SignalR a partir das suas redes virtuais sem atravessar a internet.
...
**Orientação**: Utilize tags de serviço de rede virtual Azure para definir controlos de acesso à rede em grupos de segurança de rede ou firewall Azure configurados para os seus recursos de Serviço Azure SignalR. Ao criar regras de segurança, pode utilizar etiquetas de serviço em vez de endereços IP específicos. Ao especificar o nome da etiqueta de serviço (Por exemplo: AzureSignalR) no campo de origem ou destino adequado de uma regra, pode permitir ou negar o tráfego para o serviço correspondente. A Microsoft gere os prefixos de endereços englobados pela etiqueta de serviço e atualiza automaticamente a etiqueta de serviço à medida que os endereços mudam.
...
**Orientação**: O Serviço Azure SignalR utiliza o Azure Ative Directory (Azure AD) como o serviço de gestão de identidade e acesso padrão. Deve normalizar a Azure AD para governar a gestão de identidade e acesso da sua organização em:
...
O Serviço Azure SignalR suporta apenas a autenticação Azure AD para o plano de gestão, mas não para o plano de dados. Aqui está a lista de funções incorporadas no Serviço Azure SignalR:

- Contribuinte SignalR
- Leitor Signalr AccessKey
...
**Orientação**: O Serviço Azure SignalR utiliza identidades geridas pelo Azure para contas não humanas, como a que chama a montante num cenário sem servidor. Recomenda-se a utilização de funcionalidades de identidade geridas pela Azure para aceder a outros recursos Azure. O Serviço Azure SignalR pode autenticar de forma nativa os serviços ou recursos da Azure que suportam a autenticação do Azure Ative Directory (Azure AD) através da regra de concessão de acesso pré-definida sem utilizar códigos de credenciais rígidos em código fonte ou ficheiros de configuração.
...
**Orientação**: O Serviço Azure SignalR utiliza o Azure Ative Directory (Azure AD) para fornecer gestão de identidade e acesso aos recursos do SignalR. Essas identidades incluem identidades empresariais, como colaboradores, bem como identidades externas, como parceiros, fornecedores e distribuidores. Isto permite um único acesso para gerir e garantir o acesso aos dados e recursos da sua organização no local e na nuvem. Ligue todos os seus utilizadores, aplicações e dispositivos ao Azure AD para beneficiar de acesso seguro e ininterrupto e de mais visibilidade e controlo.
...
**Orientação**: O Serviço Azure SignalR utiliza o Azure Ative Directory (Azure AD) que suporta controlos de autenticação forte através da autenticação de vários fatores e métodos fortes sem palavras-passe.

- Autenticação multifactor - Ativar a autenticação multi-fator AD Azure e seguir as recomendações do Azure Security Center Identity and Access Management para as melhores práticas aplicáveis na sua configuração de autenticação multi-fator. A autenticação multifactor pode ser aplicada a todos os utilizadores selecionados ou ao nível por utilizador com base em condições de inscrição e fatores de risco.
...
**Orientação**: O Serviço Azure SignalR está integrado com o Azure Ative Directory no qual fornece as seguintes fontes de dados:
...
Estas fontes de dados podem ser integradas com sistemas Azure Monitor, Azure Sentinel ou sistemas de gestão de informação de terceiros (SIEM).
...
O Serviço Azure SignalR tem funções altamente privilegiadas. Limitar o número de contas ou funções altamente privilegiadas e proteger a Azure AD estas contas a um nível elevado, porque os utilizadores com este privilégio podem ler e modificar direta ou indiretamente todos os recursos do seu ambiente Azure.
...
- [Contribuinte SignalR](../role-based-access-control/built-in-roles.md#signalr-contributor)
...
**Orientação**: O Serviço Azure SignalR utiliza o controlo de acesso baseado em funções (Azure RBAC) para isolar o acesso a sistemas críticos de negócio, limitando quais as contas que têm acesso privilegiado às subscrições e grupos de gestão em que se encontram.
...
- [Leitor Signalr AccessKey](../role-based-access-control/built-in-roles.md#signalr-accesskey-reader)
...
**Orientação**: O Serviço SignalR está integrado com o controlo de acesso baseado em funções Azure (Azure RBAC) para gerir os seus recursos. O RBAC do Azure permite-lhe gerir o acesso aos recursos do Azure através da atribuição de funções. Atribua estas funções aos utilizadores, grupos de diretores de serviço e identidades geridas. Papéis incorporados pré-definidos existem para certos recursos, e estes papéis podem ser inventariados ou questionados através de ferramentas como Azure CLI, Azure PowerShell ou o portal Azure. 

Os privilégios que atribuir aos recursos através do RBAC do Azure devem estar sempre limitados àquilo que as funções requerem. Esta abordagem complementa a abordagem just-in-time (JIT) do Azure AD Privileged Identity Management (PIM) e deve ser revista periodicamente.

Funções incorporadas no Serviço SignalR:
...
- Iniciar saúde – O relatório de sinalização n fornece informações sobre o uso de aplicações geridas e atividades de inscrição do utilizador.

- Registos de auditoria - Capacidade de rastreio através de registos para todas as alterações efetuadas por várias funcionalidades no Azure AD. Os exemplos de registos de auditoria incluem as alterações feitas a quaisquer recursos no Azure AD, como adicionar ou remover utilizadores, aplicações, grupos, funções e políticas.

- Ins de sinal de risco - Um sinal de risco é um indicador para uma tentativa de inscrição que pode ter sido realizada por alguém que não é o legítimo proprietário de uma conta de utilizador.
- Utilizadores sinalizados para risco – Um utilizador de risco é um indicador de uma conta de utilizador que pode ter sido comprometida.
...
**Orientação**: O Serviço Azure SignalR não se destina a implantar em redes virtuais, pois não é capaz de permitir a registo de fluxo de fluxo de grupos de segurança de rede, de encaminhar o tráfego através de uma firewall ou de realizar capturas de pacotes.

No entanto, o Serviço Azure SignalR regista o tráfego de rede que processa para acesso ao cliente. Permitir registos de recursos dentro dos recursos implantados oferecendo recursos e configurar estes registos para serem enviados para uma conta de armazenamento para retenção e auditoria a longo prazo.

- [Registos de recursos para o Serviço Azure SignalR](signalr-howto-diagnostic-logs.md)
...
**Orientação**: Os registos de atividade, que estão automaticamente disponíveis, contêm todas as operações de escrita (PUT, POST, DELETE) para os seus recursos de Serviço Azure SignalR, exceto operações de leitura (GET). Os registos de atividade podem ser utilizados para encontrar um erro na resolução de problemas ou para monitorizar como um utilizador na sua organização modificou um recurso.

Ativar registos de recursos Azure para o Serviço Azure SignalR. Pode utilizar o Azure Security Center e a Azure Policy para permitir registos de recursos e recolher dados. Estes registos podem ser cruciais para mais tarde investigar incidentes de segurança e realizar exercícios forenses.
...
Além disso, ative e a bordo dados para Azure Sentinel ou um sistema de informação de segurança de terceiros e gestão de eventos (SIEM).
...
**Orientação**: O Serviço Azure SignalR suporta abaixo as políticas específicas do serviço que estão disponíveis no Azure Security Center para auditar e impor configurações dos seus recursos Azure. Isto pode ser configurado no Azure Security Center ou com iniciativas da Azure Policy.

Pode utilizar a Azure Blueprints para automatizar a implementação e configuração de ambientes de serviços e aplicações, incluindo modelos de Gestores de Recursos Azure, controlos de acesso baseados em funções Azure (Azure RBAC) e políticas, numa única definição de planta.

- [Trabalhar com políticas de segurança no Azure Security Center](../security-center/tutorial-security-policy.md)

- [Conformidade de auditoria dos recursos do Serviço Azure SignalR utilizando a Política Azure](signalr-howto-azure-policy.md)
...
**Orientação**: Utilize o Azure Security Center para monitorizar a sua linha de base de configuração e impor usando a Política Azure [negar] e [implementar se não existir] para impor uma configuração segura para o Serviço Azure SignalR. A política de serviço Azure SignalR inclui:
...
**Orientação**: Utilize o Centro de Segurança Azure e a Política Azure para estabelecer configurações seguras no Serviço Azure SignalR.
