---
title: Linha de base de segurança azure para base de dados azure para MySQL
description: Linha de base de segurança azure para base de dados azure para MySQL
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 03/23/2020
...
A Base de Base de Segurança Azure para base de dados Azure para MySQL contém recomendações que o ajudarão a melhorar a postura de segurança da sua implementação.
...
**Orientação**: Configure link privado para base de dados Azure para MySQL com pontos finais privados. O Private Link permite-lhe ligar-se a vários serviços PaaS em Azure através de um ponto final privado. O Azure Private Link essencialmente traz serviços Azure dentro da sua rede virtual privada (VNet). O tráfego entre a sua rede virtual e a instância MySQL viaja na rede de espinha dorsal da Microsoft.

Em alternativa, poderá utilizar pontos finais de serviço de rede virtual para proteger e limitar o acesso à rede à sua Base de Dados Azure para implementações MySQL. As regras de rede virtual são uma funcionalidade de segurança de firewall que controla se a base de dados Azure para o servidor MySQL aceita comunicações que são enviadas de determinadas subredes em redes virtuais.

Também pode proteger a sua Base de Dados Azure para o servidor MySQL com regras de firewall. A firewall do servidor impede todo o acesso ao seu servidor de base de dados até especificar quais os computadores que têm permissão. Para configurar a firewall, crie as regras de firewall que especificam intervalos de endereços IP aceitáveis. Pode criar regras de firewall ao nível do servidor.

Como configurar link privado para base de dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/howto-configure-privatelink-portal

Como criar e gerir pontos finais de serviço VNet e regras VNet na Base de Dados Azure para MySQL:https://docs.microsoft.com/azure/sql-database/sql-database-vnet-service-endpoint-rule-overview

Como configurar a Base de Dados Azure para as regras de firewall MySQL:https://docs.microsoft.com/azure/mysql/howto-manage-firewall-using-portal
...
**Orientação**: Utilize proteção avançada contra ameaças para base de dados Azure para MySQL. A Advanced Threat Protection deteta atividades anómalas que indicam tentativas incomuns e potencialmente nocivas de acesso ou exploração de bases de dados.

Ative a Norma de Proteção DDoS nas Redes Virtuais associadas à sua Base de Dados Azure para instâncias MySQL para proteger contra ataques DDoS. Utilize o Azure Security Center Integrated Threat Intelligence para negar comunicações com endereços IP da Internet maliciosos ou não utilizados conhecidos.

Como configurar a Proteção Avançada de Ameaças para a Base de Dados Azure para o MySQL:https://docs.microsoft.com/azure/mysql/howto-database-threat-protection-portal
...
**Orientação**: Quando a sua base de dados Azure para a instância MySQL estiver fixada a um ponto final privado, pode implantar máquinas virtuais na mesma rede virtual. Em seguida, pode configurar um grupo de segurança de rede (NSG) para reduzir o risco de exfiltração de dados. Ative os registos de fluxo do NSG e envie registos numa Conta de Armazenamento para auditoria de tráfego. Também pode enviar registos de fluxo NSG para um espaço de trabalho de Log Analytics e utilizar o Traffic Analytics para fornecer informações sobre o fluxo de tráfego na sua nuvem Azure. Algumas vantagens do Traffic Analytics são a capacidade de visualizar a atividade da rede e identificar pontos quentes, identificar ameaças de segurança, compreender padrões de fluxo de tráfego e identificar configurações erradas da rede.
...
**Orientação**: Para recursos que necessitem de acesso à sua Base de Dados Azure para instâncias MySQL, utilize etiquetas de serviço de rede virtuais para definir controlos de acesso à rede em Grupos de Segurança da Rede ou Firewall Azure. Ao criar regras de segurança, pode utilizar etiquetas de serviço em vez de endereços IP específicos. Especificando o nome da etiqueta de serviço (por exemplo, SQL. WestUs) no campo de origem ou destino apropriado de uma regra, pode permitir ou negar o tráfego para o serviço correspondente. A Microsoft gere os prefixos de endereço sacados pela etiqueta de serviço e atualiza automaticamente a etiqueta de serviço à medida que os endereços mudam.

Nota: A Base de Dados Azure para MySQL utiliza as etiquetas de serviço "Microsoft.Sql".
...
Compreender o uso da etiqueta de serviço para a Base de Dados Azure para mySQL:https://docs.microsoft.com/azure/mysql/concepts-data-access-and-security-vnet#terminology-and-description
...
**Orientação**: Defina e implemente configurações de segurança padrão para configurações de rede e recursos de rede associados à sua Base de Dados Azure para instâncias MySQL com a Política Azure. Utilize pseudónimos da Política Azure nos espaços de nome "Microsoft.DBforMySQL" e "Microsoft.Network" para criar políticas personalizadas para auditar ou impor a configuração de rede da sua Base de Dados Azure para instâncias MySQL. Também pode utilizar definições políticas incorporadas relacionadas com o networking ou a sua Base de Dados Azure para instâncias MySQL, tais como:
...
**Orientação**: Utilize etiquetas para recursos relacionados com segurança de rede e fluxo de tráfego para a sua Base de Dados Azure para instâncias MySQL para fornecer metadados e organização lógica.
...
**Orientação**: A Microsoft mantém a fonte de tempo utilizada para os recursos Azure, como a Base de Dados Azure para MySQL para os carimbos temporais nos registos.
...
**Orientação**: Ative as Definições de Diagnóstico e os Registos do Servidor e ingere os registos para agregar os dados de segurança gerados pela sua Base de Dados Azure para as instâncias MySQL. Dentro do Monitor Azure, utilize log analytics workspace(s) para consultar e realizar análises, e utilize contas de armazenamento Azure para armazenamento a longo prazo/arquivo. Em alternativa, pode ativar e embarcar dados para o Azure Sentinel ou um SIEM de terceiros.

Compreender registos de servidores para base de dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/concepts-monitoring#server-logs
...
**Orientação**: Ativar as definições de diagnóstico na sua Base de Dados Azure para casos MySQL para acesso a auditoria, consulta lenta e registos de métricas MySQL. Certifique-se de que ativa especificamente o registo de auditoria MySQL. Os registos de atividade, que estão automaticamente disponíveis, incluem fonte de evento, data, utilizador, carimbo de tempo, endereços de origem, endereços de destino e outros elementos úteis. Também pode ativar as Definições de Diagnóstico de Registo de Atividade do Azure e enviar os registos para o mesmo espaço de trabalho de Log Analytics ou Conta de Armazenamento.

Compreender registos de servidores para base de dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/concepts-monitoring#server-logs

Como configurar e aceder a registos de consultas lentas para a Base de Dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/howto-configure-server-logs-in-portal

Como configurar e aceder a registos de auditoria para a Base de Dados Azure para o MySQL:https://docs.microsoft.com/azure/mysql/howto-configure-audit-logs-portal
...
**Orientação**: Dentro do Monitor Azure, para o espaço de trabalho do Log Analytics utilizado para manter a sua Base de Dados Azure para registos MySQL, deteta o período de retenção de acordo com as regras de conformidade da sua organização. Utilize contas de armazenamento Azure para armazenamento a longo prazo/arquivo.
...
**Orientação**: Ativar proteção avançada de ameaças para base de dados Azure para MySQL. A Advanced Threat Protection deteta atividades anómalas que indicam tentativas incomuns e potencialmente nocivas de acesso ou exploração de bases de dados.

Além disso, pode ativar registos de servidores e definições de diagnóstico para o MySQL e enviar registos para um espaço de trabalho de Log Analytics. A bordo do seu espaço de trabalho Log Analytics para o Azure Sentinel, uma vez que fornece uma solução de resposta automatizada de orquestração de segurança (SOAR). Isto permite criar e utilizar os livros de jogadas (soluções automatizadas) para remediar problemas de segurança.

Como ativar a Proteção Avançada de Ameaças para base de dados Azure para MySQL (Pré-visualização):https://docs.microsoft.com/azure/mysql/howto-database-threat-protection-portal
...
**Orientação**: Não aplicável; A Base de Dados Azure para MySQL não processa nem produz registos relacionados com anti-malware.
...
**Orientação**: Não aplicável; A Base de Dados Azure para MySQL não processa nem produz registos relacionados com DNS.
...
**Orientação**: Manter um inventário das contas de utilizador que tenham acesso administrativo ao plano de gestão (por exemplo, portal Azure) da sua Base de Dados Azure para instâncias MySQL. Além disso, mantenha um inventário das contas administrativas que tenham acesso ao plano de dados (dentro da própria base de dados) da sua Base de Dados Azure para instâncias MySQL. (Ao criar o servidor MySQL, fornece credenciais para um utilizador administrador. Este administrador pode ser utilizado para criar utilizadores mySQL adicionais.)

A Base de Dados Azure para o MySQL não suporta o controlo de acesso baseado em funções incorporado, mas pode criar funções personalizadas com base em opções específicas de fornecedor de recursos.
...
Compreender a Base de Dados Azure para operações de fornecedor de recursos MySQL:https://docs.microsoft.com/azure/role-based-access-control/resource-provider-operations#microsoftdbformysql

Compreender a gestão de acesso saquede Azure para mySQL:https://docs.microsoft.com/azure/mysql/concepts-security#access-management
...
**Orientação**: A AD Azure não tem o conceito de senhas padrão.

Após a criação da Base de Dados Azure para o próprio recurso MySQL, o Azure obriga à criação de um utilizador administrativo com uma senha forte. No entanto, uma vez criada a instância MySQL, poderá utilizar a primeira conta de administração do servidor que criou para criar utilizadores adicionais e conceder acesso administrativo aos mesmos. Ao criar estas contas, certifique-se de configurar uma senha diferente e forte para cada conta.

Como criar contas adicionais para a Base de Dados Azure para o MySQL:https://docs.microsoft.com/azure/mysql/howto-create-users

Como atualizar a palavra-passe do administrador:https://docs.microsoft.com/azure/mysql/howto-create-manage-server-portal#update-admin-password
...
**Orientação**: Criar procedimentos operacionais padrão em torno da utilização de contas administrativas dedicadas que tenham acesso à sua Base de Dados Azure para instâncias MySQL. Utilize a identidade do Azure Security Center Identidade e gestão de acesso para monitorizar o número de contas administrativas.
...
Entenda como criar utilizadores de administração na Base de Dados Azure para mySQL:https://docs.microsoft.com/azure/mysql/howto-create-users
...
**Orientação**: A sessão na Base de Dados Azure para o MySQL é suportada tanto utilizando o nome de utilizador/palavra-passe configurado diretamente na base de dados, como utilizando uma identidade azure Ative Directory (AD) e utilizando um token Azure AD para ligar. Quando se utiliza um token Azure AD, são suportados diferentes métodos, tais como um utilizador da AD Azure, um grupo Azure AD ou uma aplicação Azure AD que liga à base de dados.

Separadamente, o acesso do avião de controlo para o MySQL está disponível através da REST API e suporta o SSO. Para autenticar, detete tede o cabeçalho de Autorização para os seus pedidos a um Token Web JSON que obtenha do Diretório Ativo Azure.

Utilize o Diretório Ativo Azure para autenticação com base de dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/howto-configure-sign-in-azure-ad-authentication

Compreender a Base de Dados Azure para mySQL REST API:https://docs.microsoft.com/rest/api/mysql/
...
**Orientação**: Utilize postos de trabalho privilegiados de acesso (PAWs) com Autenticação Multi-Factor (MFA) configurado para iniciar sessão e configurar recursos Azure.
...
**Orientação**: Permitir que a Proteção Avançada de Ameaças para a Base de Dados Azure para o MySQL gere alertas para atividades suspeitas.

Além disso, pode utilizar a Azure AD Privileged Identity Management (PIM) para a geração de registos e alertas quando ocorrer atividade suspeita ou insegura no ambiente.
...
Para iniciar a sua inscrição na Base de Dados Azure para o MySQL, recomenda-se a utilização de Um AD Azure e a utilização de um token Azure AD para ligar. Quando se utiliza um token Azure AD, são suportados diferentes métodos, tais como um utilizador da AD Azure, um grupo Azure AD ou uma aplicação Azure AD que liga à base de dados. 

As credenciais Azure AD também podem ser utilizadas para administração a nível de planos de gestão (por exemplo, o portal Azure) para controlar as contas de administração da MySQL.
...
**Orientação**: Reveja os registos do Diretório Ativo Azure para ajudar a descobrir contas velhas que podem incluir as que têm base de dados Azure para funções administrativas MySQL. Além disso, utilize as Análises de Acesso à Identidade do Azure para gerir eficientemente os membros do grupo, o acesso a aplicações empresariais que possam ser usadas para aceder à Base de Dados Azure para mySQL e atribuições de papéis. O acesso ao utilizador deve ser revisto regularmente, como a cada 90 dias, para garantir que apenas os Utilizadores certos tenham acesso continuado.

Compreender relatórios da AD Azurehttps://docs.microsoft.com/azure/active-directory/reports-monitoring/
...
**Orientação**: Ativar as definições de diagnóstico para a Base de Dados Azure para o MySQL e o Diretório Ativo Azure, enviando todos os registos para um espaço de trabalho de Log Analytics. Configure os alertas desejados (tais como tentativas de autenticação falhadas) dentro do Log Analytics.
...
**Orientação**: Não aplicável; O Bloqueio de Clientes ainda não é suportado para a Base de Dados Azure para o MySQL.
...
**Orientação**: Utilize etiquetas para ajudar a rastrear a Base de Dados Azure para instâncias MySQL ou recursos conexos que armazenem ou processem informações sensíveis.
...
**Orientação**: Implementar subscrições separadas e/ou grupos de gestão para desenvolvimento, teste e produção. Utilize uma combinação de regras de Link Privado, Pontos Finais de Serviço e/ou firewall para isolar e limitar o acesso à rede à sua Base de Dados Azure para instâncias MySQL.
...
Como configurar link privado para base de dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/concepts-data-access-security-private-link

Como criar e gerir pontos finais de serviço VNet e regras VNet na Base de Dados Azure para MySQL:https://docs.microsoft.com/azure/sql-database/sql-database-vnet-service-endpoint-rule-overview

Como configurar a Base de Dados Azure para as regras de firewall MySQL:https://docs.microsoft.com/azure/mysql/concepts-firewall-rules
...
**Orientação**: Ao utilizar VMs Azure para aceder à Base de Dados Azure para instâncias MySQL, utilize o Private Link, as configurações da rede MySQL, os grupos de segurança da rede e as etiquetas de serviço para mitigar a possibilidade de exfiltração de dados.

A Microsoft gere a infraestrutura subjacente à Base de Dados Azure para o MySQL e implementou controlos rigorosos para evitar a perda ou exposição de dados dos clientes.

Como mitigar a exfiltração de dados para a Base de Dados Azure para o MySQL:https://docs.microsoft.com/azure/mysql/concepts-data-access-security-private-link#data-exfiltration-prevention
...
**Orientação**: Base de dados Azure para suportes MySQL que ligam o seu servidor MySQL às aplicações do cliente utilizando a Camada de Tomadas Seguras (SSL). A imposição de ligações SSL entre o servidor de base de dados e as aplicações de cliente ajuda a proteger contra ataques "man-in-the-middle" ao encriptar o fluxo de dados entre o servidor e a sua aplicação. No portal Azure, certifique-se de que a "ligação SSL de aplicação" está ativada para todas as instâncias da sua Base de Dados Azure para as instâncias MySQL por padrão.

Atualmente, a versão TLS suportada para base de dados Azure para MySQL são TLS 1.0, TLS 1.1, TLS 1.2.

Como configurar a encriptação em trânsito para a Base de Dados Azure para o MySQL:https://docs.microsoft.com/azure/mysql/concepts-ssl-connection-security
...
**Orientação**: Utilize o controlo de acesso baseado em funções azure (RBAC) para controlar o acesso à Base de Dados Azure para o plano de controlo MySQL (por exemplo, portal Azure). Para acesso a plano de dados (dentro da própria base de dados), utilize consultas SQL para criar utilizadores e configurar permissões dos utilizadores. O RBAC não afeta as permissões dos utilizadores dentro da base de dados.

Como configurar o RBAC em Azure:https://docs.microsoft.com/azure/role-based-access-control/role-assignments-portal

Como configurar o acesso do utilizador com SQL para Base de Dados Azure para MySQL:https://docs.microsoft.com/azure/mysql/howto-create-users
...
Compreenda a encriptação em repouso para a Base de Dados Azure para o MySQL:https://docs.microsoft.com/azure/mysql/concepts-security

Como configurar as chaves geridas pelo cliente para a base de dados Azure para o MySQL:https://docs.microsoft.com/azure/mysql/concepts-data-encryption-mysql
...
**Orientação**: Utilize o Monitor Azure com o Registo de Atividades do Azure para criar alertas para quando ocorrerem alterações nas instâncias de produção da Base de Dados Azure para mySQL e outros recursos críticos ou relacionados.
...
**Orientação**: Atualmente não disponível; O Azure Security Center ainda não suporta a avaliação de vulnerabilidade para a Base de Dados Azure para o MySQL.
...
**Orientação**: Aplicar etiquetas na Base de Dados Azure para instâncias MySQL e outros recursos relacionados que dêem metadados para logicamente organizá-los em uma taxonomia.
...
**Orientação**: Utilize a marcação, os grupos de gestão e as assinaturas separadas, se for caso disso, para organizar e rastrear a Base de Dados Azure para instâncias MySQL e recursos conexos. Conciliar o inventário regularmente e garantir que os recursos não autorizados sejam eliminados da subscrição atempadamente.
...
**Orientação**: Defina e implemente configurações de segurança padrão para a sua Base de Dados Azure para instâncias MySQL com a Política Azure. Utilize pseudónimos da Política Azure no espaço de nome "Microsoft.DBforMySQL" para criar políticas personalizadas para auditar ou impor a configuração da rede da sua Base de Dados Azure para instâncias MySQL. Também pode utilizar definições políticas incorporadas relacionadas com a sua Base de Dados Azure para instâncias MySQL, tais como:
...
**Orientação**: Se utilizar definições de política personalizadas do Azure para a sua Base de Dados Azure para instâncias MySQL e recursos conexos, utilize o Azure Repos para armazenar e gerir de forma segura o seu código.
...
**Orientação**: Utilize pseudónimos da Política Azure no espaço de nome "Microsoft.DBforMySQL" para criar políticas personalizadas para alertar, auditar e impor configurações do sistema. Além disso, desenvolver um processo e um oleoduto para gerir as exceções políticas.
...
**Orientação**: Utilize pseudónimos da Política Azure no espaço de nome "Microsoft.DBforMySQL" para criar políticas personalizadas para alertar, auditar e impor configurações do sistema. Utilize a política Azure [auditoria], [negar], e [implementar se não existir] para impor automaticamente as configurações para a sua Base de Dados Azure para instâncias MySQL e recursos conexos.
...
**Orientação**: Para máquinas virtuais Azure ou aplicações web em execução no Serviço de Aplicações Azure que estão a ser utilizados para aceder à sua Base de Dados Azure para instâncias MySQL, utilize a Identidade de Serviço Gerida em conjunto com o Cofre de Chaves Azure para simplificar e proteger a Base de Dados Azure para a gestão secreta mySQL. Certifique-se de que o Soft Delete do cofre de chaves está ativado.
...
O anti-malware da Microsoft está ativado no hospedeiro subjacente que suporta os serviços Do Azure (por exemplo, Base de Dados Azure para SQL), no entanto não funciona com conteúdo de clientes.
...
**Orientação**: O anti-malware da Microsoft está ativado no hospedeiro subjacente que suporta os serviços Do Azure (por exemplo, Base de Dados Azure para MySQL), no entanto não funciona com conteúdo do cliente.

Pré-digitalização de qualquer conteúdo que seja enviado para recursos não computacionais do Azure, tais como Serviço de Aplicações, Armazenamento de Data Lake, Armazenamento Blob, Base de Dados Azure para MySQL, etc. A Microsoft não pode aceder aos seus dados nestes casos.
...
O anti-malware da Microsoft está ativado no hospedeiro subjacente que suporta os serviços Do Azure (por exemplo, Base de Dados Azure para MySQL), no entanto não funciona com conteúdo de clientes.
...
Compreenda cópias de segurança para a Base de Dados Azure para mySQL:https://docs.microsoft.com/azure/mysql/howto-restore-server-portal 
...
Compreenda a cópia de segurança e restaure na Base de Dados Azure para o MySQL:https://docs.microsoft.com/azure/mysql/concepts-backup
...
**Orientação**: A Base de Dados Azure para MySQL requer cópias de segurança completas, diferenciais e de registo de transações. Estas cópias de segurança permitem restaurar um servidor em qualquer ponto-a-tempo dentro do período de retenção de cópia de segurança configurado. O período de retenção de reserva padrão é de sete dias. Pode configurá-lo opcionalmente até 35 dias. Todas as cópias de segurança são encriptadas utilizando encriptação AES de 256 bits. Certifique-se de que o Soft Delete do cofre de chaves está ativado.
