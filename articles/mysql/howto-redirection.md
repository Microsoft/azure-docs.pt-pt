...
O comportamento de reorientação é determinado pelo valor de `mysqlnd_azure.enableRedirect` . O quadro abaixo descreve o comportamento da reorientação com base no valor deste parâmetro a partir da **versão 1.1.0+**.
...
|`on` ou `1`|- Se a ligação não utilizar sSL no lado do condutor, não será feita qualquer ligação. O seguinte erro será devolvido: *"mysqlnd_azure.enableRedirect está ligado, mas a opção SSL não está definida na cadeia de ligação. A reorientação só é possível com o SSL."*<br>- Se o SSL for utilizado no lado do condutor, mas a reorientação não for suportada no servidor, a primeira ligação é abortada e o seguinte erro é devolvido: *"A ligação abortada porque a redirecionamento não está ativada no servidor MySQL ou o pacote de rede não cumpre o protocolo de redirecionamento."*<br>- Se o servidor MySQL suportar a reorientação, mas a ligação redirecionada falhou por qualquer motivo, também aborte a primeira ligação proxy. Volte a devolva o erro da ligação redirecionada.|
|`preferred` ou `2`<br> (valor por defeito)|- mysqlnd_azure utilizarão a reorientação, se possível.<br>- Se a ligação não utilizar o SSL no lado do condutor, o servidor não suporta a reorientação ou a ligação redirecionada não se coata por qualquer razão não fatal enquanto a ligação proxy ainda for válida, ela recue para a primeira ligação proxy.|
...
- PERA PHP 
- php-mysql
- Base de dados Azure para servidor MySQL
...
3. Mude os diretórios para a pasta devolvida e certifique-se de que `mysqlnd_azure.so` está localizado nesta pasta. 
...
6. Crie um novo ficheiro .ini para `mysqlnd_azure` . Certifique-se de que a ordem do alfabeto do nome é a trás da mysqnld, uma vez que os módulos são carregados de acordo com a ordem de nome dos ficheiros ini. Por exemplo, se `mysqlnd` .ini for `10-mysqlnd.ini` nomeado, nomeie o mysqlnd ini como `20-mysqlnd-azure.ini` .
...
3. Extraia o ficheiro zip e encontre o DLL nomeado `php_mysqlnd_azure.dll` .
...
5. Copie o `php_mysqlnd_azure.dll` ficheiro no diretório devolvido no passo 4. 

6. Localize a pasta PHP contendo o `php.ini` ficheiro utilizando o seguinte comando:
...
Também pode confirmar que a reorientação está configurada com o código PHP da amostra abaixo. Crie um ficheiro PHP chamado `mysqlConnect.php` e colá o código abaixo. Atualize o nome do servidor, nome de utilizador e palavra-passe com o seu próprio. 
