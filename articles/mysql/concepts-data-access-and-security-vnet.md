...
*As regras de rede virtual* são um recurso de segurança de firewall que controla se o servidor de banco de dados do Azure para MySQL aceita comunicações enviadas de sub-redes específicas em redes virtuais. Este artigo explica por que o recurso de regra de rede virtual às vezes é a melhor opção para permitir a comunicação com segurança para o servidor do banco de dados do Azure para MySQL.
...
> [!NOTE]
> Esse recurso está disponível em todas as regiões do Azure em que o banco de dados do Azure para MySQL é implantado para servidores Uso Geral e com otimização de memória.
...
**Regra de rede virtual:** Uma regra de rede virtual para o servidor do banco de dados do Azure para MySQL é uma sub-rede listada na lista de controle de acesso (ACL) do seu banco de dados do Azure para o servidor MySQL. Para estar na ACL para o servidor do banco de dados do Azure para MySQL, a sub-rede deve conter o nome do tipo **Microsoft. SQL** .

Uma regra de rede virtual instrui o servidor do banco de dados do Azure para MySQL a aceitar comunicações de cada nó que está na sub-rede.
...
Se seu **Microsoft. SQL** Server era um nó em uma sub-rede em sua rede virtual, todos os nós na rede virtual podem se comunicar com o banco de dados do Azure para o servidor MySQL. Nesse caso, suas VMs podem se comunicar com o banco de dados do Azure para MySQL sem a necessidade de nenhuma regra de rede virtual ou regras de IP.

No entanto, a partir de agosto de 2018, o serviço de banco de dados do Azure para MySQL ainda não está entre os serviços que podem ser atribuídos diretamente a uma sub-rede.
...
Cada regra de rede virtual se aplica a todo o banco de dados do Azure para servidor MySQL, não apenas a um banco de dados específico no servidor. Em outras palavras, a regra de rede virtual se aplica no nível do servidor, não no nível do banco de dados.
...
- Cada servidor de banco de dados do Azure para MySQL pode ter até 128 entradas de ACL para qualquer rede virtual específica.
