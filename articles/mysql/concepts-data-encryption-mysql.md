---
title: Base de Dados Azure para encriptação de dados MySQL com uma chave gerida pelo cliente
...
> [!NOTE]
> Neste momento, você deve solicitar acesso para usar esse recurso. Para isso, contacte AskAzureDBforMySQL@service.microsoft.com.

A Base de Dados Azure para encriptação de dados MySQL com uma chave gerida pelo cliente permite-lhe trazer a sua própria chave (BYOK) para proteção de dados em repouso. Permite ainda que as organizações implementem a separação de deveres na gestão de chaves e dados. Com a encriptação gerida pelo cliente, é responsável e em pleno controlo do ciclo de vida de uma chave, permissões de utilização chave e auditoria de operações em chaves.

Para a Base de Dados Azure para MySQL, você define a encriptação de dados ao nível do servidor. Com esta forma de encriptação de dados, utiliza a chave na encriptação da chave de encriptação de dados (DEK). O DEK é uma chave assimétrica gerida pelo cliente, armazenada numa instância [azure key vault](../key-vault/key-Vault-secure-your-key-Vault.md) gerida pelo cliente e gerida pelo cliente. A DEK é descrita mais detalhadamente mais tarde neste artigo.
...
* Capacidade de separar a gestão chave da gestão de dados dentro de uma organização, para que um administrador do Key Vault possa revogar permissões de acesso chave para tornar uma base de dados encriptada inacessível.
* Maior confiança dos seus utilizadores finais, porque a Microsoft não consegue ver ou extrair chaves de encriptação no Key Vault.

## <a name="terminology-and-description"></a>Terminologia e descrição

Chave de encriptação de **dados (DEK)** : Uma chave Simétrica AES256 utilizada para encriptar uma partilha ou bloco de dados. A criptografia de cada bloco de dados com uma chave diferente torna os ataques de análise de criptografia mais difíceis. O acesso ao DEKs é necessário para o provedor de recursos ou instância do aplicativo que está Criptografando e descriptografando um bloco específico. Quando substituir um DEK por uma nova tecla, apenas os dados do seu bloco associado devem ser novamente encriptados com a nova tecla.
...
Para que um servidor MySQL utilize chaves geridas pelo cliente armazenadas no Key Vault para encriptação do DEK, um administrador key vault dá os seguintes direitos de acesso ao servidor:
...
* Key Vault e Azure Database para MySQL devem pertencer ao mesmo inquilino azure Ative Directory (Azure AD). O Cofre chave do inquilino cruzado e as interações do servidor não são suportadas. A movimentação de recursos posteriormente exige que você reconfigure a criptografia de dados.
* Deve ativar a função de eliminação suave no cofre da chave, para proteger contra a perda de dados se ocorrer uma eliminação acidental da chave (ou cofre de chaves). Os recursos eliminados suavemente são retidos durante 90 dias, a menos que o utilizador os recupere ou purgue entretanto. As ações de recuperação e limpeza têm suas próprias permissões associadas em uma política de acesso Key Vault. A função soft-delete está desligada por defeito, mas pode atilá-la através do PowerShell ou do Azure CLI (note que não pode permitir através do portal Azure).
...
* A data de ativação da chave (se definido) deve ser uma data e uma hora no passado. A data de validade (se definida) deve ser uma data e hora futuras.
...
Depois de a Base de Dados Azure para MySQL ser encriptada com a chave gerida por um cliente armazenada no Key Vault, qualquer cópia recém-criada do servidor também é encriptada. Pode fazer esta nova cópia através de uma operação local ou geo-restauração, ou através de réplicas de leitura. No entanto, a cópia pode ser alterada para refletir a chave gerida por um novo cliente para encriptação. Quando a chave gerida pelo cliente é alterada, as cópias de segurança antigas do servidor começam a utilizar a tecla mais recente.
...
Aprenda a configurar a encriptação de dados com uma chave gerida pelo cliente para a sua base de [dados Azure para o MySQL utilizando o portal Azure](howto-data-encryption-portal.md).
