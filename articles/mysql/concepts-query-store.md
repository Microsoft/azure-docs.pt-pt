---
title: Store de consulta na base de dados do Azure para MySQL
description: Este artigo descreve a funcionalidade de Store de consulta na base de dados do Azure para MySQL
author: ajlam
ms.author: andrela
ms.service: mysql
ms.topic: conceptual
...
1. Inicie sessão no portal do Azure e selecione a sua base de dados do Azure para o servidor MySQL.
...
Utilize o [portal do Azure](howto-server-parameters.md) ou [CLI do Azure](howto-configure-server-parameters-using-cli.md) para obter ou definir um valor diferente para um parâmetro.

## <a name="views-and-functions"></a>Funções e exibições

Ver e gerir Store de consulta com as seguintes vistas e funções. Qualquer pessoa na [função pública de privilégio selecione](howto-create-users.md#how-to-create-additional-admin-users-in-azure-database-for-mysql) pode utilizar estas vistas para ver os dados na consulta Store. Estas vistas só estão disponíveis na **mysql** base de dados.
...
- Se um servidor MySQL tem o parâmetro `default_transaction_read_only` , Store de consulta não é possível capturar os dados.
