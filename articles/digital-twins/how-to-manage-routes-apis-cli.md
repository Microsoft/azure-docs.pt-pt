...
Uma vez criados os recursos do ponto final, podes usá-los para um ponto final da Azure Digital Twins. Os exemplos a seguir mostram como criar pontos finais utilizando o [ponto final az dt criar](/cli/azure/dt/endpoint/create) comando para o [CLI das Gémeas Digitais Azure](how-to-use-cli.md). Substitua os espaços reservados nos comandos com os detalhes dos seus próprios recursos.
...
Para criar um ponto final que tenha a letra ativada, adicione o seguinte parâmetro de letra morta ao [ponto final az dt criar](/cli/azure/dt/endpoint/create) comando para o [CLI das Gémeas Digitais Azure](how-to-use-cli.md).
...
Aqui está um exemplo de uma mensagem de letra morta para uma [notificação de criação de gémeos:](concepts-event-notifications.md#digital-twin-lifecycle-notifications)
...
As rotas de eventos podem ser criadas com os comandos apis do plano de dados Azure Digital Twins [ **EventRoutes**](/rest/api/digital-twins/dataplane/eventroutes) ou [com os comandos CLI **da rota az dt**](/cli/azure/dt/route). O resto desta secção percorre o processo de criação.
...
As rotas também podem ser geridas usando os comandos [de rota az dt](/cli/azure/dt/route) para o CLI das Gémeas Digitais Azure. 
