...
[![digital twins edifício gráfico espacial](media/concepts/digital-twins-spatial-graph-building.png)](media/concepts/digital-twins-spatial-graph-building.png#lightbox)
...
- **Os dispositivos** são peças de equipamento virtuais ou físicas, por exemplo, `AwesomeCompany Device` e `Raspberry Pi 3`.
- **Os sensores** são objetos que detetam eventos, por exemplo, `AwesomeCompany Temperature Sensor` e `AwesomeCompany Presence Sensor`.
...
- **As bolhas** são anexadas a objetos (como espaços, dispositivos, sensores e utilizadores). São usados como ficheiros com tipo mímica e metadados, por exemplo, `maps`, `pictures`e `manuals`.
- **Os tipos alargados** são enumerações extensíveis que aumentam entidades com características específicas, por exemplo `SpaceType` e `SpaceSubtype`.
- **As ontologias** representam um conjunto de tipos alargados, por exemplo, `Default`, `Building`, `BACnet`e `EnergyGrid`.
- **As chaves e valores** de propriedade são características personalizadas de espaços, dispositivos, sensores e utilizadores. Podem ser utilizados juntamente com características incorporadas, por exemplo, `DeltaProcessingRefreshTime` como chave e `10` como valor.
- **As funções** são conjuntos de permissões atribuídas aos utilizadores e dispositivos no gráfico espacial, por exemplo, `Space Administrator`, `User Administrator`e `Device Administrator`.
...
- **Pontos finais** são os locais onde as mensagens de telemetria e os eventos de Gémeos Digitais podem ser encaminhados, por exemplo, `Event Hub`, `Service Bus`e `Event Grid`.
...
[![portal de Gémeos Digitais Management API](media/concepts/digital-twins-spatial-graph-management-api-url.png)](media/concepts/digital-twins-spatial-graph-management-api-url.png#lightbox)
