...
1. No VS Code, abra o arquivo `CMakeLists.txt` na pasta `azure-iot-sdk-c`. Abra o arquivo de `CMakeLists.txt` na pasta `azure-iot-sdk-c`, e não o que está na pasta `devkit_device`.

1. Adicione a linha abaixo na parte inferior do arquivo de `CMakeLists.txt` para incluir a pasta stub do código do dispositivo ao compilar:
