---
title: Exportar os dados do Azure IoT Central | Documentos da Microsoft
...
3. Escolha uma subscrição.
    ...
    > [!Note]
    > Agora, pode exportar dados para outras subscrições que estão **não são as mesmas** que aquela para a sua aplicação pay as you go IoT Central. Se conectar usando uma cadeia de ligação em vez disso.
...
2. No menu da esquerda, selecione **a exportação contínua de dados**.
    ...
    ![Criar novo Hub de eventos](media/howto-export-data/export_menu.png)
...
8. Para ativar a exportação contínua de dados, certifique-se **exportar dados** é **no**. Selecione **Guardar**.

    ![Configurar a exportação contínua de dados](media/howto-export-data/export_list.png)

9. Após alguns minutos, os seus dados é apresentada no seu destino escolhido.
