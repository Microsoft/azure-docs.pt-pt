...
Tem de publicar quaisquer alterações no modelo do dispositivo, quer precisem ou não de uma atualização de versão, antes que um operador possa usá-lo. O IoT Central impede-o de publicar alterações de rutura num modelo de dispositivo sem antes ver o modelo.
...
Esta secção resume as regras de versão que se aplicam aos modelos do dispositivo. Tanto os DCMs como as interfaces têm números de versão. O seguinte corte mostra o DCM para um dispositivo de sensor ambiental. O DCM tem duas interfaces: **DeviceInformation** e **EnvironmentalSensor**. Pode ver os números da versão no final dos `@id` campos. Para ver estas informações na UI Central IoT, selecione **Ver identidade** no editor de modelo do dispositivo.
...
* Depois de um DCM ser publicado, não é possível remover nenhuma interface, mesmo numa nova versão do modelo do dispositivo.
* Depois de um DCM ser publicado, pode adicionar uma interface se criar uma nova versão do modelo do dispositivo.
* Depois de um DCM ser publicado, pode substituir uma interface por uma versão mais recente se criar uma nova versão do modelo do dispositivo. Por exemplo, se o modelo do dispositivo sensor v1 utilizar a interface EnvironmentalSensor v1, pode criar um modelo de dispositivo sensor v2 que utiliza a interface EnvironmentalSensor v2.
...
1. Aceda à página **de modelos de dispositivo.**
...
1. Clique em **Guardar**. Estes valores substituem tudo o que foi inicialmente guardado no modelo do dispositivo e são utilizados em toda a aplicação.
...
Apenas versão do modelo do dispositivo quando estiver a tentar editar uma parte do modelo de capacidade do dispositivo que não pode editar na secção de personalizaçãos.

Para a versão de um modelo de dispositivo:

1. Aceda à página **de modelos de dispositivo.**
1. Selecione o modelo de dispositivo que está a tentar ver.
1. Clique no botão **Versão** no topo da página e dê ao modelo um novo nome. IoT Central sugere um novo nome, que pode editar.
1. Clique em **Criar**.
...
1. Clique no botão **Versão** no topo da página de interface.
1. Clique em **Criar**.
...
1. Aceda à página **do Explorador de Dispositivos.**
1. Selecione o dispositivo de que necessita para migrar para outra versão.
1. Escolha **Migrar**.
1. Selecione o modelo do dispositivo com o número de versão para o qual pretende migrar o dispositivo e escolher **Migrar**.
