---
title: O que são os modelos de aplicação no Azure IoT Central [ Central De Azure IoT ] Microsoft Docs
description: Os modelos de aplicação Azure IoT Central permitem-lhe saltar para o desenvolvimento da solução IoT.
author: philmea
ms.author: philmea
ms.date: 10/25/2019
ms.topic: conceptual
ms.service: iot-central
services: iot-central
ms.openlocfilehash: 244de9a3236977308a9f7bcaf5106e86a0984201
ms.sourcegitcommit: 75089113827229663afed75b8364ab5212d67323
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 04/22/2020
ms.locfileid: "82024681"
---
# <a name="what-are-application-templates"></a>O que são modelos de aplicação?

Os modelos de aplicação no Azure IoT Central são uma ferramenta para ajudar os construtores de soluções a iniciar o seu desenvolvimento de soluções IoT. Pode utilizar modelos de aplicativos para tudo, desde sentir o que é possível, até personalizar totalmente e a sua aplicação para revenda aos seus clientes.

Os modelos de aplicação consistem em:

- Tabliers do operador de amostras
- Modelos de dispositivo de amostra
- Dispositivos simulados que produzem dados em tempo real
- Regras e empregos pré-configurados
- Documentação rica, incluindo tutoriais e "how-tos"

Escolhe o modelo de aplicação quando cria a sua aplicação. Não pode alterar o modelo depois da aplicação ser criada.

## <a name="custom-templates"></a>Modelos personalizados

Se quiser criar a sua aplicação de raiz, escolha um dos dois modelos de aplicação personalizados:

- Aplicação personalizada
- Aplicação personalizada (legado)

Escolha o modelo de **aplicação Personalizado** a menos que tenha uma razão específica para usar o modelo legado.

## <a name="industry-focused-templates"></a>Modelos focados na indústria

A Azure IoT Central é uma plataforma de aplicação agnóstica da indústria. Os modelos de aplicação são exemplos focados na indústria disponíveis para estas indústrias hoje em dia, com mais para vir no futuro:

- [Retalho](../retail/overview-iot-central-retail.md)
  - Logística associada
  - Centro de distribuição digital
  - Análise na loja - monitorização de condições
  - Análise na loja - check-out
  - Gestão inteligente de inventário
- [Energia](../energy/overview-iot-central-energy.md)
  - Monitorização de contadores inteligentes
  - Monitorização de painéis solares
- [Governo](../government/overview-iot-central-government.md)
  - Gestão de resíduos conectados
  - Monitorização do consumo de água
  - Monitorização da qualidade da água
- [Cuidados de saúde.](../healthcare/overview-iot-central-healthcare.md)
  - Monitorização contínua do paciente

## <a name="next-steps"></a>Passos seguintes

Agora que sabe quais são os modelos de aplicação IoT Central, inicie-se com a criação de [uma Aplicação Central IoT](quick-deploy-iot-central.md).
