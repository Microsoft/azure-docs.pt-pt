...
| Bases de dados Azure SQL com integração de autenticação AD AZure habilitados | Yes | No | [Consulte as bases de dados do Azure SQL com a autenticação AZure AD](#list-azure-sql-databases-with-azure-ad-authentication) | Não é possível transferir uma base de dados Azure SQL com autenticação Azure AD ativada para um diretório diferente. Para mais informações, consulte [a autenticação do Azure Ative Directory](../azure-sql/database/authentication-aad-overview.md). | 
...
| Azure File Sync | Yes | Yes |  | O serviço de sincronização de armazenamento e/ou conta de armazenamento pode ser transferido para um diretório diferente. Para obter mais informações, consulte [perguntas frequentes (FAQ) sobre Ficheiros Azure](../storage/files/storage-files-faq.md#azure-file-sync) |
...
| Azure Kubernetes Service | Yes | No |  | Não é possível transferir o seu cluster AKS e os seus recursos associados para um diretório diferente. Para mais informações, consulte [Perguntas frequentes sobre o Serviço Azure Kubernetes (AKS)](../aks/faq.md) |
...
| Azure Active Directory Domain Services | Yes | No |  | Não é possível transferir um domínio gerido por Azure AD Domain Services para um diretório diferente. Para obter mais informações, consulte [perguntas frequentes (FAQs) sobre Azure Ative Directory (AD) Domain Services](../active-directory-domain-services/faqs.md) |
...
### <a name="install-the-azure-resource-graph-extension"></a>Instale a extensão do Gráfico de Recursos Azure
 ...
 A extensão Azure CLI para [Azure Resource Graph](../governance/resource-graph/index.yml), *gráfico de recursos, permite-lhe* utilizar o comando [azgraph](/cli/azure/graph) para consultar os recursos geridos pelo Azure Resource Manager. Usará este comando em passos posteriores.
...
- Utilize [a lista de ad-admin do servidor az sql](/cli/azure/sql/server/ad-admin#az_sql_server_ad_admin_list) e a extensão de gráfico [az](/cli/azure/graph) para ver se está a utilizar bases de dados Azure SQL com integração de autenticação AD AD ativada. Para mais informações, consulte [Configure e gerencie a autenticação do Azure Ative Directory com SQL](../azure-sql/database/authentication-aad-configure.md).
...
1. Utilize a extensão de [gráfico az](/cli/azure/graph) para listar outros recursos Azure com dependências conhecidas do diretório Azure.
