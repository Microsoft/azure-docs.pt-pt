...
|A depuração remota deve ser desativada para o aplicativo Web|Esta ação desabilita a depuração remota.|
...
|O CORS não deve permitir que todos os recursos acessem seu Aplicativo de funções|Essa ação impede que outros domínios acessem seu aplicativo de funções. Para permitir domínios específicos, insira-os no campo origens permitidas (separadas por vírgulas). <br>**Observação**: deixar o campo vazio bloqueará todas as chamadas entre origens. "título do campo de parâmetro:" origens permitidas "|
