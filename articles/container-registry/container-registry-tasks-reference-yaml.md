...
| `workingDirectory` | string | Sim | O diretório de trabalho do contentor durante o tempo de funcionamento. Se a propriedade for especificada numa tarefa, define a propriedade padrão `workingDirectory` de todos os degraus. Se especificado em um passo, substitui a propriedade fornecida pela tarefa. | Sim | `c:\workspace` em Janelas ou `/workspace` em Linux |
...
| `keyvault` | string | Sim | A URL secreta do cofre da chave azul. | Nenhum |
| `clientID` | string | Sim | Identificação do cliente da [identidade gerida atribuída pelo utilizador](container-registry-tasks-authentication-managed-identity.md) para os recursos da Azure. | Nenhum |
...
| `driver` | string | Sim | O motorista para gerir a rede. | Nenhum |
...
| `entryPoint` | string | Sim | Sobrepõe-se ao `[ENTRYPOINT]` contentor de um degrau. | Nenhum |
...
| `user` | string | Sim | O nome de utilizador ou UID de um recipiente | Nenhum |
