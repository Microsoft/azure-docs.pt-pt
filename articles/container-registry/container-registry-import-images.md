...
Se você ainda não tiver um registro de contêiner do Azure, crie um registro. Para obter as etapas [, consulte início rápido: Crie um registro de contêiner privado usando o](container-registry-get-started-azure-cli.md)CLI do Azure.
...
Por exemplo, importe a imagem mais recente do Windows Server Core `windows` do repositório no registro de contêiner da Microsoft.
...
O exemplo a seguir importa uma imagem por Resumo de manifesto (hash SHA-256, `sha256:...`representado como) em vez de por marca:
...
No exemplo a seguir, *mysourceregistry* está em uma assinatura diferente de *myregistry* no mesmo locatário de Active Directory. Forneça a ID de recurso do registro de origem com `--registry` o parâmetro. Observe que o `--source` parâmetro especifica apenas o repositório de origem e o nome da imagem, não o nome do servidor de logon do registro.
