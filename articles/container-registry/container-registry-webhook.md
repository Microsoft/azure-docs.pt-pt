---
title: WebHooks para responder às ações do registro
description: Saiba como usar WebHooks para disparar eventos quando ocorrem ações de Push ou de pull em seus repositórios de registro.
ms.topic: article
ms.date: 05/24/2019
...
Um registro de contêiner do Azure armazena e gerencia imagens de contêiner privadas do Docker, de forma semelhante ao modo como o Docker Hub armazena imagens públicas do Docker. Ele também pode hospedar repositórios para [gráficos Helm](container-registry-helm-repos.md) (visualização), um formato de empacotamento para implantar aplicativos no kubernetes. Você pode usar WebHooks para disparar eventos quando determinadas ações ocorrerem em um de seus repositórios de registro. WebHooks podem responder a eventos no nível do registro ou podem ter o escopo reduzido para uma marca de repositório específica. Com um registro [replicado geograficamente](container-registry-geo-replication.md) , você configura cada webhook para responder a eventos em uma réplica regional específica.
...
* CLI do Docker – para configurar seu computador local como um host do Docker e acessar os comandos da CLI do Docker, instale o [mecanismo do Docker](https://docs.docker.com/engine/installation/).
...
| Localização | Para um registro [replicado geograficamente](container-registry-geo-replication.md) , especifique a região do Azure da réplica de registro. 
