...
* Site de segmento Inquilinos por Cabeçalhos anfitriãos com ou sem comunicação SSL
...
* O Azure Service Bus fornece uma capacidade segura de mensagens e fluxo de dados para aplicações distribuídas e híbridas, tais como comunicação entre aplicações e aplicações e serviços no local, sem necessitar de firewall e segurança complexas infraestruturas. A utilização de Retransmissão de ônibus de serviço para recursos de aplicação para aceder aos serviços expostos como pontos finais pode pertencer ao inquilino (por exemplo, hospedado fora do sistema, como no local), ou podem ser serviços prestados especificamente para o inquilino (porque dados sensíveis e específicos do inquilino viajam através deles).
...
* As funções dos trabalhadores permitem-lhe fornecer e desprovisionar por recursos de inquilino (como quando um novo inquilino se inscreve ou cancela), recolher métricas para uso de medição, e gerir a escala seguindo um determinado horário ou em resposta à passagem de limiares de desempenho chave indicadores. Esta mesma função também pode ser usada para empurrar atualizações e atualizações para a solução.
