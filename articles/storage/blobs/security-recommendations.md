...
description: Conheça as recomendações de segurança para o armazenamento blob. Implementar esta orientação irá ajudá-lo a cumprir as suas obrigações de segurança, conforme descrito no nosso modelo de responsabilidade partilhada.
services: storage
author: tamram
ms.service: storage
ms.subservice: blobs
...
| Ligue o soft delete para os dados blob | A eliminação suave permite-lhe recuperar os dados blob depois de ter sido eliminado. Para obter mais informações sobre a eliminação suave, consulte [Soft delete para as bolhas de armazenamento azure](storage-blob-soft-delete.md). | - |
| Armazenar dados críticos de negócios em bolhas imutáveis | Configure as políticas legais de retenção e as políticas de retenção baseadas no tempo para armazenar dados blob num estado WORM (Write Once, Read Many). As bolhas armazenadas imutávelmente podem ser lidas, mas não podem ser modificadas ou eliminadas durante a duração do intervalo de retenção. Para mais informações, consulte os [dados críticos do negócio da loja com armazenamento imutável](storage-blob-immutable-storage.md). | - |
| Limite fichas de assinatura de acesso partilhado (SAS) apenas a ligações HTTPS | Exigir HTTPS quando um cliente usa um token SAS para aceder a dados blob ajuda a minimizar o risco de escutas. Para mais informações, consulte Grant acesso limitado aos recursos de [Armazenamento Azure utilizando assinaturas de acesso partilhado (SAS)](../common/storage-sas-overview.md). | - |

## <a name="identity-and-access-management"></a>Gestão de identidades e acessos

| Recomendação | Comentários | Centro de Segurança |
|-|----|--|
| Utilizar o Diretório Ativo Azure (Azure AD) para autorizar o acesso aos dados blob | A Azure AD oferece uma segurança e facilidade de utilização superiores sobre a Chave Partilhada para autorizar pedidos para armazenamento Blob. Para mais informações, consulte [Autorizar o acesso a blobs e filas Azure utilizando o Diretório Ativo Azure](../common/storage-auth-aad.md). | - |
...
| Utilize uma delegação de utilizadores SAS para conceder acesso limitado a dados blob aos clientes | Uma delegação de utilizadores SAS é assegurada com credenciais azure Ative Directory (Azure AD) e também pelas permissões especificadas para o SAS. Uma delegação de utilizadores SAS é análoga a um serviço SAS em termos do seu âmbito e função, mas oferece benefícios de segurança sobre o serviço SAS. Para mais informações, consulte Grant acesso limitado aos recursos de [Armazenamento Azure utilizando assinaturas de acesso partilhado (SAS)](../common/storage-sas-overview.md?toc=%2fazure%2fstorage%2fblobs%2ftoc.json). | - |
| Proteja as chaves de acesso à sua conta com o Cofre de Chaves Azure | A Microsoft recomenda a utilização de Anúncios Azure para autorizar pedidos de armazenamento Azure. No entanto, se tiver de utilizar a autorização da Chave Partilhada, proteja as chaves da sua conta com o Cofre de Chaves Azure. Pode saquear as chaves do cofre chave em tempo de funcionamento, em vez de as guardar com a sua aplicação. Para mais informações sobre o Cofre de Chaves Azure, consulte a [visão geral do Cofre de Chaves azure](../../key-vault/key-vault-overview.md). | - |
...
| Limitar o acesso anónimo ao público a contentores e bolhas | Anónimo, o público lê o acesso a um contentor e as suas bolhas concedem acesso apenas a esses recursos a qualquer cliente. Evite permitir o acesso à leitura pública a menos que o seu cenário o exija. | - |
