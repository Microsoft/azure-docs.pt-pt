...
:::row:::
    ...
    :::column:::
        ...
        Este artigo apresenta-o aos conceitos e funcionalidades do Azure File Sync. Uma vez que esteja familiarizado com o Azure File Sync, considere seguir o [guia de implementação do Azure File Sync](file-sync-deployment-guide.md) para experimentar este serviço.        
    :::column-end:::
:::row-end:::

Os ficheiros serão armazenados na nuvem nas [ações de ficheiros Azure](../files/storage-files-introduction.md). As ações de ficheiros Azure podem ser utilizadas de duas formas: montando diretamente estas ações de ficheiros Azure sem servidor (SMB) ou caching Azure file shares on-in usando Azure File Sync. Qual a opção de implementação que escolhe altera os aspetos que precisa de considerar como planeia para a sua implementação. 
...
* uma necessidade de ter mais servidores registados ou grupos de sincronização do que um único Serviço de Sincronização de Armazenamento pode suportar. Reveja os alvos da [escala de Sincronização de Ficheiros Azure](../files/storage-files-scale-targets.md?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json#azure-file-sync-scale-targets) para obter mais detalhes.
...
- Está a migrar de uma implementação DFS-R para uma implementação de Azure File Sync. Para obter mais informações, consulte [a implementação de uma replicação DFS (DFS-R) para Azure File Sync](file-sync-deployment-guide.md#migrate-a-dfs-replication-dfs-r-deployment-to-azure-file-sync).
...
Para obter mais informações, consulte [as métricas de desempenho do Azure File Sync](../files/storage-files-scale-targets.md?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json#azure-file-sync-performance-metrics) e os [alvos da escala de sincronização de ficheiros Azure](../files/storage-files-scale-targets.md?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json#azure-file-sync-scale-targets)
...
Para saber mais sobre o Azure File Sync e networking, consulte [as considerações de networking do Azure File Sync](file-sync-networking-overview.md).
...
Por padrão, as ações de ficheiros padrão podem abranger apenas até 5 TiB, mas pode aumentar o limite de ações para 100 TiB. Para aprender a aumentar o seu limite de ações, consulte [Ativar e criar grandes ações de ficheiros](../files/storage-files-how-to-create-large-file-share.md?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json).
...
Confira o [arquivo Azure File Sync e o artigo de visão geral da migração do ficheiro Azure,](../files/storage-files-migration-overview.md?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json) onde pode encontrar orientações detalhadas para o seu cenário.
...
> [!WARNING]
> Se precisar de utilizar robocopia /B com um agente Azure File Sync a funcionar no servidor de origem ou alvo, por favor atualize para a versão v12.0 ou superior do agente Azure File Sync. A utilização de Robocopia /B com versões de agente inferiores a v12.0 conduzirá à corrupção de ficheiros hierárquicos durante a cópia.

> [!Note]  
> A restauração de metal nu (BMR) pode causar resultados inesperados e não está atualmente suportada.

> [!Note]  
> Com a versão 9 do agente Azure File Sync, as imagens VSS (incluindo o separador Versões Anteriores) são agora suportadas em volumes que tenham o tiering de nuvem ativado. No entanto, deve ativar a compatibilidade da versão anterior através do PowerShell. [Saiba como.](file-sync-deployment-guide.md#self-service-restore-through-previous-versions-and-vss-volume-shadow-copy-service)
...
* [Considere as definições de firewall e proxy](file-sync-firewall-and-proxy.md)
* [Implementar os Ficheiros do Azure](../files/storage-how-to-create-file-share.md?toc=%2fazure%2fstorage%2ffilesync%2ftoc.json)
