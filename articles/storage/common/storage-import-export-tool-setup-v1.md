...
-   Antes de criar uma tarefa de importação, pode utilizar esta ferramenta para copiar dados para os discos rígidos que está a enviar para um centro de dados do Windows Azure.  
...
-   A sua subscrição deve incluir uma conta de armazenamento com espaço disponível suficiente para armazenar os ficheiros que está a importar.  
...
-   Você precisará de uma ou mais unidades que contenham dados para serem importados ou discos rígidos SATA vazios de 3,5 polegadas ligados à máquina de cópia.  
...
Se estiver a tentar reparar uma importação que falhou parcialmente, necessitará:  

- Os ficheiros de registo de cópia  

- A chave da conta de armazenamento  

  Se estiver a tentar reparar uma exportação que falhou parcialmente, necessitará:  
...
- Hddid.dll  
  ...
  Copie estes ficheiros para um diretório de trabalho, por exemplo, `c:\WAImportExport` . Em seguida, abra uma janela de linha de comando no modo administrador e coloque o diretório acima como diretório atual.  

  Para ajudar a saída do comando, executar a ferramenta sem parâmetros:  
...
* [Pré-visualização do uso do Drive para um trabalho de exportação](/previous-versions/azure/storage/common/storage-import-export-tool-previewing-drive-usage-export-v1)   
