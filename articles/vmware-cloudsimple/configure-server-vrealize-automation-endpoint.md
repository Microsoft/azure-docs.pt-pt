---
title: Azure VMware Solutions (AVS) - Configurar vCenter numa Nuvem Privada AVS para a automação vRealize
description: Descreve como configurar um servidor VMware vCenter na sua Nuvem Privada AVS como um ponto final para VMware vRealize Automation
author: sharaths-cs
ms.author: b-shsury
ms.date: 08/19/2019
ms.topic: article
...
---
# <a name="set-up-vcenter-on-your-avs-private-cloud-for-vmware-vrealize-automation"></a>Configurar vCenter na sua Nuvem Privada AVS para VMware vRealize Automation

Pode configurar um servidor VMware vCenter na sua Nuvem Privada AVS como ponto final para vMware vRealize Automation.

## <a name="before-you-begin"></a>Antes de começar

Complete estas tarefas antes de configurar o servidor vCenter:

* Configure uma [ligação VPN site-to-site](vpn-gateway.md#set-up-a-site-to-site-vpn-gateway) entre o seu ambiente no local e a sua Nuvem Privada AVS.
* [Configure o reencaminhamento de DNS no local de pedidos DNS](on-premises-dns-setup.md) para os servidores DNS para a sua Nuvem Privada AVS.
...
| Inventário de Máquinavirtual |  Criar a partir de existente<br>Criar Novo<br>Mover<br>Remover | 
...
| Configuração da máquina virtual |  Adicionar disco existente<br>Adicionar novo disco<br>Adicionar ou remover<br>Remover o disco<br>Segurança<br>Alterar a contagem de CPU<br>Alterar Recurso<br>Estender o disco virtual<br>Rastreio de alterações de disco<br>Memória<br>Modificar as definições do dispositivo<br>Mudar o nome<br>Definir Anotação (versão 5.0 e mais tarde)<br>Definições<br>Colocação de swapfile |
...
1. Inscreva-se no servidor vRealize Automation IaaS como administrador do IaaS que o Suporte AVS criou para si.
2. Implemente um agente vSphere para o ponto final da automatização vRealize.
    ...
    A convenção de nomeação do ficheiro instalador é setup_@5480.exe*vra-url* .
    3. Executar o instalador. No ecrã welcome, clique em **Next**.
    ...
    ![vRA instalar tipo](media/configure-vra-endpoint-install-type.png)
    7. Introduza a informação de entrada do servidor IaaS e clique em **Next**. Se estiver a utilizar o Diretório Ativo, introduza o nome de utilizador no formato **domain\user.** Caso contrário, utilize **user@domain** formato.
    ...
    10. Introduza um nome para o ponto final do servidor vCenter para a Nuvem Privada AVS. Grave o nome para utilização mais tarde no processo de configuração.
...
2. Selecione **pontos finais** de > de **infraestrutura** > **pontos finais**.
3. Selecione **New** > **virtual** > **vSphere**.
4. Introduza o nome final da vSphere que especificou no procedimento anterior.
5. Para **Endereço**, introduza o URL do Servidor VCenter Da Nuvem Privada AVS no formato https://*vcenter-fqdn*/sdk, onde *vcenter-fqdn* é o nome do servidor vCenter.
6. Introduza as credenciais para o utilizador administrativo vRealize Automation IaaS que o Suporte AVS criou para si.
...
8. Depois de um teste bem sucedido, clique em **OK** para criar o ponto final da vSphere.
    ![vRA endpoint acesso config](media/configure-vra-endpoint-vra-edit.png)
...
A saída é semelhante à seguinte. O valor do campo `managementEndpointName` é o nome final.
