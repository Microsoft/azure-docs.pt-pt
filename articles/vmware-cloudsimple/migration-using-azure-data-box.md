...
* Copiando os dados do Azure para a sua Nuvem Privada AVS.
...
* Crie uma [ligação de rede virtual Azure](cloudsimple-azure-network-connection.md) desde a sua Nuvem Privada AVS até à rede virtual onde a conta de armazenamento é criada seguindo os passos da [rede virtual Connect Azure para AVS utilizando o ExpressRoute](virtual-network-connection.md).
...
5. No passo 3 do assistente, especifique o nome para a loja de dados, o caminho e o servidor. Pode utilizar o endereço IP da sua Caixa de Dados para o servidor. O caminho da pasta estará no formato `/<StorageAccountName_BlockBlob>/<ContainerName>/`.
...
Os dados copiados para o seu dispositivo Data Box estarão disponíveis na sua conta de armazenamento Azure após o estado de encomenda da sua Caixa de Dados mostrar como concluído. Os dados podem agora ser copiados para a sua Solução VMware Azure. Os dados na conta de armazenamento devem ser copiados para a loja de dados vSAN da sua Nuvem Privada AVS utilizando o protocolo NFS. 

Primeiro, copie os dados de armazenamento blob para um disco gerido numa máquina virtual Linux em Azure utilizando **o AzCopy**. Disponibilize o disco gerido através do NFS, monte a quota NFS como uma loja de dados na sua Nuvem Privada AVS e, em seguida, copie os dados. Este método permite uma cópia mais rápida dos dados para a sua Nuvem Privada AVS.

### <a name="copy-data-to-your-avs-private-cloud-using-a-linux-virtual-machine-and-managed-disks-and-then-export-as-nfs-share"></a>Copie dados para a sua Nuvem Privada AVS usando uma máquina virtual Linux e discos geridos, e depois exporte como nFS share

1. Crie uma [máquina virtual Linux](../virtual-machines/linux/quick-create-portal.md) em Azure na mesma região onde a sua conta de armazenamento é criada e tem uma ligação de rede virtual Azure à sua Nuvem Privada AVS.
...
8. Atribuir permissões para endereços IP do cliente para aceder à parte nFS editando o ficheiro `/etc/exports`.

    ```bash
    sudo vi /etc/exports
    ```
    
    Introduza as seguintes linhas no ficheiro para cada IP anfitrião ESXi da sua Nuvem Privada AVS. Se estiver a criar partilhas para várias pastas, adicione todas as pastas.
...
9. Exportar as ações da NFS utilizando o comando `sudo exportfs -a`.

10. Reiniciar o servidor kernel NFS utilizando o comando `sudo systemctl restart nfs-kernel-server`.


### <a name="mount-the-linux-virtual-machine-nfs-share-as-a-datastore-on-an-avs-private-cloud-vcenter-cluster-and-then-copy-data"></a>Monte a máquina virtual Linux NFS compartilhe como uma loja de dados num cluster VCenter AVS Private Cloud e, em seguida, copie dados

A partilha nFS da sua máquina virtual Linux deve ser montada como uma loja de dados no seu cluster VCenter AVS Private Cloud. Depois de montado, os dados podem ser copiados da loja de dados NFS para a loja de dados AVS Private Cloud vSAN.

1. Inicie sessão no seu servidor VCenter VCenter Private Cloud.
...
5. No passo 3 do assistente, especifique o nome para a loja de dados, o caminho e o servidor. Pode utilizar o endereço IP da sua máquina virtual Linux para o servidor. O caminho da pasta estará no formato `/<folder>/<subfolder>/`.
...
1. A partir da sua AVS Private Cloud vCenter web UI, vá para **Storage**. Selecione uma máquina virtual DenFS de máquina sintética e, em seguida, selecione **Ficheiros**.
...
3. **Selecione Registar VM** para registar a máquina virtual no seu VCenter De Nuvem Privada AVS.
...
Depois de todos os dados ser copiados para a sua Nuvem Privada AVS, pode remover a loja de dados NFS da sua Nuvem Privada AVS:
...
3. Desmonte a loja de dados da máquina virtual Linux a partir do seu VCenter De Nuvem Privada AVS.
...
* Saiba mais sobre [diferentes](migrate-workloads.md)opções para a migração de cargas de trabalho para a sua Nuvem Privada AVS .
