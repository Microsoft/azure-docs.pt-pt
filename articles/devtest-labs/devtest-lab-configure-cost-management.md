...
4. Selecione **a tendência custo** na secção de rastreio de **custos** no menu esquerdo. A imagem que se segue mostra um exemplo de um gráfico de custos. 
   ...
   * As subscrições da CSP e da Dreamspark não são atualmente suportadas, uma vez que a Azure DevTest Labs utiliza as [APIs de faturação Azure](../cost-management-billing/manage/usage-rate-card-overview.md) para calcular o custo do laboratório, que não suporta subscrições de CSP ou Dreamspark.
