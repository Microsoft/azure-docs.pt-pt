...
Várias DMVs e eventos estendidos têm colunas e campos que especificam o número de leituras remotas de um servidor de páginas, que pode ser comparado com o total de leituras. O repositório de consultas também captura as leituras remotas como parte das estatísticas de tempo de execução de consulta.
...
Em um banco de dados que não é de hiperescala, IOPS de leitura e gravação combinados em relação a arquivos de data, em relação ao limite de IOPS de dados de [governança de recursos](/azure/sql-database/sql-database-resource-limits-database-server#resource-governance) , são relatados em modos de exibição [Sys. dm_db_resource_stats](/sql/relational-databases/system-dynamic-management-views/sys-dm-db-resource-stats-azure-sql-database) e [Sys. resource_stats](/sql/relational-databases/system-catalog-views/sys-resource-stats-azure-sql-database) , na coluna `avg_data_io_percent`. O mesmo valor é relatado no portal como _porcentagem de e/s de dados_. 

Em um banco de dados de hiperescala, essa coluna relata a utilização de IOPS de dados em relação ao limite do armazenamento local somente na réplica de computação, especificamente a e/s em relação a RBPEX e `tempdb`. Um valor de 100% nessa coluna indica que a governança de recursos está limitando o IOPS de armazenamento local. Se isso estiver correlacionado a um problema de desempenho, ajuste a carga de trabalho para gerar menos e/s ou aumente o objetivo do serviço de banco de dados para aumentar o [limite](sql-database-vcore-resource-limits-single-databases.md) _máximo de IOPS de data_ de governança de recursos. Para a governança de recursos de leituras e gravações do RBPEX, o sistema conta o IOs de 8 KB individual, em vez de um IOs maior que pode ser emitido pelo mecanismo de SQL Server. 
