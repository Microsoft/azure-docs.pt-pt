...
description: Gerencie os recursos computacionais em piscinas elásticas Azure SQL com muitas bases de dados.
...
> [!IMPORTANT]
> Em piscinas densas com muitas bases de dados ativas, pode não ser possível aumentar o número de bases de dados na piscina até aos máximos documentados para piscinas elásticas [DTU](sql-database-dtu-resource-limits-elastic-pools.md) e [vCore.](sql-database-vcore-resource-limits-elastic-pools.md)
> 
...
|`avg_storage_percent`|Utilização do espaço de armazenamento ao nível da piscina elástica. Disponível na `master` base de dados elastic_pool_resource_stats [sys.elastic_pool_resource_stats.](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-elastic-pool-resource-stats-azure-sql-database) Esta métrica também é emitida para o Monitor Azure, onde é [nomeado](https://docs.microsoft.com/azure/azure-monitor/platform/metrics-supported#microsoftsqlserverselasticpools) `storage_percent`, e pode ser visto no portal Azure.|Abaixo dos 80%. Pode aproximar-se a 100% para piscinas sem crescimento de dados.|
|`tempdb_log_used_percent`|Utilização de espaço `tempdb` de registo de transações na base de dados. Embora os objetos temporários criados numa base de dados `tempdb` não sejam visíveis noutras bases de dados na mesma piscina elástica, é um recurso partilhado para todas as bases de dados da mesma piscina. Uma longa transação `tempdb` de execução ou ocioso iniciada a partir de uma base de dados na piscina pode consumir uma grande parte do registo de transações, e causar falhas em consultas em outras bases de dados no mesmo pool. Disponível na vista [sys.dm_db_log_space_usage.](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-db-log-space-usage-transact-sql) Esta métrica também é emitida para o Monitor Azure, e pode ser vista no portal Azure. Consulte [exemplos](#examples) para uma consulta de amostra para devolver o valor atual desta métrica.|Abaixo dos 50%. Picos ocasionais até 80% são aceitáveis.|
|||
...
Estas opiniões podem ser utilizadas para monitorizar a utilização de recursos e a resolução de recursos em quase tempo real. A carga de trabalho dos utilizadores nas réplicas secundárias primárias e `SloSharedPool1` legíveis, incluindo georéplicas, está classificada no grupo de recursos e `UserPrimaryGroup.DBId[N]` carga de trabalho, onde `N` se destaca o valor de ID da base de dados.
...
Recomendamos limitar o número de bases de dados por servidor lógico a um número inferior ao máximo suportado. Em muitos cenários, usar até 1000-2000 bases de dados por servidor é o ideal. Para reduzir a probabilidade de eliminação acidental do servidor, recomendamos que coloque um [bloqueio de exclusão](https://docs.microsoft.com/azure/azure-resource-manager/resource-group-lock-resources) no servidor lógico ou no seu grupo de recursos.

No passado, certos cenários que envolviam bases de dados em movimento dentro, fora ou entre piscinas elásticas no mesmo servidor lógico eram mais rápidos do que quando moviam bases de dados entre servidores lógicos. Atualmente, todos os movimentos de base de dados executam à mesma velocidade, independentemente do servidor lógico de origem e destino.

## <a name="examples"></a>Exemplos

### <a name="monitoring-memory-utilization"></a>Monitorização da utilização da memória

Esta consulta calcula `oom_per_second` a métrica para cada conjunto de recursos, ao longo dos últimos 32 minutos. Esta consulta pode ser executada em qualquer base de dados numa piscina elástica.
...
ORDER BY pool_id;
```
### <a name="monitoring-tempdb-log-space-utilization"></a>Monitorização `tempdb` da utilização do espaço de registo

Esta consulta devolve o valor `tempdb_log_used_percent` atual da métrica. Esta consulta pode ser executada em qualquer base de dados numa piscina elástica.
...
* Para uma introdução a piscinas elásticas, consulte [piscinas elásticas que o ajudam a gerir e escalar várias bases de dados Azure SQL.](https://docs.microsoft.com/azure/sql-database/sql-database-elastic-pool)
