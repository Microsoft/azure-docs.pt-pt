...
- **Delegação subnet:** A subnet da instância gerida tem de ser delegada para `Microsoft.Sql/managedInstances` fornecedor de recursos.
- **Grupo de segurança de rede (NSG):** Um NSG precisa de ser associado à subnet da instância gerida. Pode utilizar um NSG para controlar o acesso ao ponto final dos dados da instância gerida filtrando o tráfego na porta 1433 e nas portas 11000-11999 quando a instância gerida está configurada para ligações redirecionais. O serviço irá adicionar automaticamente [as regras](#mandatory-inbound-security-rules-with-service-aided-subnet-configuration) necessárias para permitir o fluxo ininterrupto de tráfego de gestão.
- **Tabela de rota definida pelo utilizador (UDR):** Uma tabela UDR precisa de ser associada à subnet da instância gerida. Pode adicionar entradas à tabela de rotas para direcionar o tráfego que tem no local gamas IP privadas como destino através do gateway da rede virtual ou do aparelho de rede virtual (NVA). O serviço irá automaticamente adicionar [entradas necessárias](#user-defined-routes-with-service-aided-subnet-configuration) para permitir o fluxo ininterrupto de tráfego de gestão.
- **Pontos finais do serviço:** Os pontos finais do serviço poderiam ser usados para configurar regras de rede virtuais em contas de armazenamento que mantêm backups/registos de auditoria.
...
|            |9000, 9003                  |TCP     |65.55.188.0/24, 167.220.0.0/16, 131.107.0.0/16, 94.245.87.0/24|MI SUBNET  |Permitir |
...
> [!IMPORTANT]
> Certifique-se de que existe apenas uma regra de entrada para os portos 9000, 9003, 1438, 1440, 1452 e uma regra de saída para os portos 443, 12000. O fornecimento de instância gerida através de implementações do Gestor de Recursos Azure falhará se as regras de entrada e saída forem configuradas separadamente para cada porta. Se estas portas estiverem em regras separadas, a implantação falhará com o código de erro `VnetSubnetConflictWithIntendedPolicy`
