...
- A aplicação De dados SaaS Database Per Tenant da Wingtip Tickets É implementada. Para implantar em menos de cinco minutos, consulte [Deploy e explore a aplicação Wingtip SaaS](saas-dbpertenant-get-started-deploy.md)
...
- O lote de inquilinos adicionais foi provisionado, ver o tutorial de [**inquilinos da Provision.**](saas-dbpertenant-provision-and-catalog.md)
- Foi criada uma base de dados de conta de emprego e de conta de emprego. Consulte os passos adequados no tutorial de [**gestão da Schema.**](saas-tenancy-schema-management.md#create-a-job-agent-database-and-new-job-agent)
...
2. Desloque a variável $DemoScenario no script para corresponder à sua escolha de loja de análises:
    - Para utilizar a base de dados SQL sem loja de colunas, desloque-$DemoScenario **$DemoScenario** = **2**
    - Para utilizar a base de dados SQL com loja de colunas, **desempor $DemoScenario** = **3**  
...
3. Modifique @User a variável no topo `<User>` do script, substituindo pelo valor do utilizador utilizado quando implementou a aplicação Wingtip SaaS.
...
3. Modifique @User no topo do script `<User>` e substitua-o pelo nome de utilizador utilizado quando implementou a app Wingtip SaaS 
...
- [Análise de inquilinos cruzados usando dados extraídos - app multi-inquilino](saas-multitenantdb-tenant-analytics.md)