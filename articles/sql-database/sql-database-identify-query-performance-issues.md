...
Pode utilizar [insights inteligentes](sql-database-intelligent-insights-troubleshoot-performance.md#detectable-database-performance-patterns) de base de dados Azure SQL ou [DMVs](sql-database-monitoring-with-dmvs.md) do Servidor SQL para detetar este tipo de estrangulamentos de desempenho.
...
Um plano sub-ideal gerado pelo SQL Query Optimizer pode ser a causa de um desempenho lento de consulta. O SQL Query Optimizer pode produzir um plano sub-ideal devido a um índice em falta, estatísticas velhas, uma estimativa incorreta do número de linhas a serem processadas, ou uma estimativa imprecisa da memória necessária. Se souber que a consulta foi executada mais rapidamente no passado ou em outro caso (ou uma instância gerida ou uma instância de SQL Server), compare os planos de execução reais para ver se são diferentes.
...
- Identifique quaisquer índices em falta utilizando um destes métodos:
  ...
  - [Base de Dados Advisor](sql-database-advisor.md) para bases de dados individuais e reunidas.
  - DMVs. Este exemplo mostra-lhe o impacto de um índice em falta, como detetar um [índice em falta](sql-database-performance-guidance.md#identifying-and-adding-missing-indexes) usando DMVs, e o impacto da implementação da recomendação do índice em falta.
- Tente aplicar dicas de [consulta,](https://docs.microsoft.com/sql/t-sql/queries/hints-transact-sql-query) [estatísticas de atualização](https://docs.microsoft.com/sql/t-sql/statements/update-statistics-transact-sql)ou [reconstruir índices](https://docs.microsoft.com/sql/relational-databases/indexes/reorganize-and-rebuild-indexes) para obter o melhor plano. Ative a [correção automática](sql-database-automatic-tuning.md) do plano na Base de Dados Azure SQL para mitigar automaticamente estes problemas.

  Este [exemplo](sql-database-performance-guidance.md#query-tuning-and-hinting) mostra o impacto de um plano de consulta sub-ideal devido a uma consulta parametrizada, como detetar esta condição, e como usar uma dica de consulta para resolver.
...
- Detetar problemas de recursos utilizando o portal Azure: Para ver se os limites de recursos são o problema, consulte a monitorização dos recursos da Base de [Dados SQL](sql-database-monitor-tune-overview.md#sql-database-resource-monitoring). Para bases de dados individuais e piscinas elásticas, consulte [recomendações](sql-database-advisor.md) de desempenho do Database Advisor Advisor e [Questionry Performance Insights](sql-database-query-performance.md).
- Deteção de limites de recursos utilizando [Insights Inteligentes](sql-database-intelligent-insights-troubleshoot-performance.md#reaching-resource-limits)
- Deteção de problemas de recursos utilizando [DMVs:](sql-database-monitoring-with-dmvs.md)
  ...
  - [Muitas consultas individuais que consomem cumulativamente alta CPU](sql-database-monitoring-with-dmvs.md#many-individual-queries-that-cumulatively-consume-high-cpu)
  - 
...
- Para as [principais consultas que consomem CPU que você pode identificar:](sql-database-monitoring-with-dmvs.md#the-cpu-issue-occurred-in-the-past)
...
Utilize insights inteligentes para detetar [aumentos](sql-database-intelligent-insights-troubleshoot-performance.md#workload-increase) de carga de trabalho e [regressões de planos.](sql-database-intelligent-insights-troubleshoot-performance.md#plan-regression)
...
- Use Insights Inteligentes para identificar consultas com degradação do desempenho devido ao [aumento das esperas](sql-database-intelligent-insights-troubleshoot-performance.md#increased-wait-statistic)
...
[Visão geral da base de dados SQL](sql-database-monitor-tune-overview.md)