---
title: 'Portal do Azure: Criar uma instância gerida da base de dados SQL | Documentos da Microsoft'
description: Crie uma instância de base de dados SQL geridas, o ambiente de rede e a VM de cliente para o acesso.
...
Este início rápido explica-lhe como criar uma base de dados do SQL do Azure [instância gerida](sql-database-managed-instance.md) no portal do Azure.
...
4. Preencha os **instância gerida SQL** formulário com as informações pedidas, utilizando as informações na tabela seguinte.

   | Definição| Valor sugerido | Descrição |
   | ------ | --------------- | ----------- |
   | **Subscrição** | A sua subscrição. | Uma subscrição que te conceda permissão para criar novos recursos. |
   ...
   |**Tipo de ligação**|Escolha entre um Proxy e um tipo de ligação de redirecionamento.|Para obter mais informações sobre os tipos de ligação, veja [política de ligação de base de dados do Azure SQL](sql-database-connectivity-architecture.md#connection-policy).|
   ...
   ![formulário de instância gerida](./media/sql-database-managed-instance-get-started/managed-instance-create-form.png)

5. Para utilizar a instância gerida como um grupo de ativação pós-falha de instância secundário, selecione o Check-out e especifique a instância gerida DnsAzurePartner. Esta funcionalidade está em pré-visualização e não é mostrada na captura de ecrã seguinte.
6. Selecione **escalão de preço** aos recursos de computação e armazenamento de tamanho e para rever as opções de escalão de preço. O escalão de preço Fins Gerais com 32 GB de memória e 16 núcleos virtuais é o valor predefinido.
...
8. Quando tiver terminado, selecione **aplicar** para guardar a sua seleção. 
...
10. Selecione o **notificações** ícone para ver o estado da implementação.
    ...
    ![progresso da implementação de instância gerida](./media/sql-database-managed-instance-get-started/deployment-progress.png)
...
Após a implementação é concluída, reveja os recursos que foram criados e recuperar o nome de servidor completamente qualificado para utilização nos rápidos posteriores.
...
3. Na tabela de rotas, reveja as entradas para encaminhar o tráfego de e para dentro da rede virtual de instância gerida. Se criar ou configurar manualmente a sua tabela de rotas, tem de ser crie estas entradas na tabela de rotas.

   ![Entrada para uma sub-rede de instância gerida para local](./media/sql-database-managed-instance-get-started/udr.png)

4. Voltar para o grupo de recursos e selecione o grupo de segurança de rede para rever as regras de segurança.
...
7. Sobre o **descrição geral** separador, localize a **anfitrião** propriedade. Copie o endereço de anfitrião totalmente qualificado para a instância gerida para utilização no próximo início rápido.
...
- Para saber mais sobre como ligar a uma instância gerida:
  ...
  - Para um início rápido mostra como ligar a uma instância gerida a partir de uma máquina virtual do Azure, consulte [configurar uma ligação de máquina virtual do Azure](sql-database-managed-instance-configure-vm.md).
  - Para um início rápido mostra como ligar a uma instância gerida a partir de um computador de cliente no local, utilizando uma ligação ponto a site, consulte [configurar uma ligação ponto a site](sql-database-managed-instance-configure-p2s.md).
- Para restaurar uma base de dados existente do SQL Server no local para uma instância gerida: 
    - Utilize o [do Azure Database Migration Service (DMS) para a migração](../dms/tutorial-sql-server-to-managed-instance.md) para restaurar a partir de um ficheiro de cópia de segurança da base de dados. 
    - Utilize o [comando T-SQL RESTORE](sql-database-managed-instance-get-started-restore.md) para restaurar a partir de um ficheiro de cópia de segurança da base de dados.
- Para avançadas de monitorização de desempenho da base de dados de instância gerida com inteligência incorporada de resolução de problemas, veja [SQL Database do Azure Monitor utilizando a análise de SQL do Azure](../azure-monitor/insights/azure-sql.md).
