...
- Uma camada de computação apátrida que está a executar o processo `sqlservr.exe` e contém apenas dados transitórios e cached (por exemplo – cache de plano, piscina tampão, piscina de colunas). Este nó de Servidor SQL apátrida é operado pela Azure Service Fabric que inicia o processo, controla a saúde do nó e executa falhas noutro local, se necessário.
