...
* O banco de dados &lt;db_name @ no__t-1 no servidor &lt;Azure_instance @ no__t-3 não está disponível no momento. Repita a conexão mais tarde. Se o problema persistir, entre em contato com o atendimento ao cliente e forneça a ID de rastreamento de sessão de &lt;session_id @ no__t-1
* O banco de dados &lt;db_name @ no__t-1 no servidor &lt;Azure_instance @ no__t-3 não está disponível no momento. Repita a conexão mais tarde. Se o problema persistir, entre em contato com o atendimento ao cliente e forneça a ID de rastreamento de sessão de &lt;session_id @ no__t-1. (Microsoft SQL Server, Erro: 40613)
...
* Um aplicativo está consumindo muito espaço `TempDb`.
...
| Código de erro | Severity | Descrição |
| ---:| ---:|:--- |
| 10928 |20 |ID do recurso:% d. O limite de% s para o banco de dados é% d e foi atingido. Para obter mais informações, consulte [limites de recursos do banco de dados SQL para bancos de dados individuais e em pool](sql-database-resource-limits-database-server.md).<br/><br/>A ID do recurso indica o recurso que atingiu o limite. Para threads de trabalho, a ID de recurso = 1. Para sessões, a ID do recurso = 2.<br/><br/>Para obter mais informações sobre esse erro e como resolvê-lo, consulte: <br/>[limites de recursos do servidor de banco de dados](sql-database-resource-limits-database-server.md) &bull; &nbsp;<br/>&bull; &nbsp;[limites baseados em DTU para bancos de dados individuais](sql-database-service-tiers-dtu.md)<br/>&bull; &nbsp;[limites baseados em DTU para pools elásticos](sql-database-dtu-resource-limits-elastic-pools.md)<br/>&bull; &nbsp;[limites baseados em vCore para bancos de dados individuais](sql-database-vcore-resource-limits-single-databases.md)<br/>&bull; &nbsp;[limites baseados em vCore para pools elásticos](sql-database-vcore-resource-limits-elastic-pools.md)<br/>[limites de recursos de instância gerenciada](sql-database-managed-instance-resource-limits.md)&bull; &nbsp;. |
...
| [18470](https://docs.microsoft.com/sql/relational-databases/errors-events/database-engine-events-and-errors#errors-18000-to-18999) |14 |Falha de início de sessão para o utilizador '%.&#x2a;ls'. Falha A conta está desabilitada.%. &#x2a;ls |
...
| 40553 |16 |A sessão foi encerrada devido ao uso excessivo de memória. Tente modificar a consulta para processar menos linhas.<br/><br/> A redução do número de operações `ORDER BY` e `GROUP BY` em seu código Transact-SQL ajuda a reduzir os requisitos de memória de sua consulta. |
...
| 40615 |16 |não é possível abrir o servidor "{0}" pedido pelo início de sessão. O cliente com o endereço IP ' {1} ' não tem permissão para acessar o servidor.<br /><br />Para habilitar o acesso, use o portal do banco de dados SQL ou execute SP @ no__t-0set @ no__t-1Firewall @ no__t-2rule no banco de dados mestre para criar uma regra de firewall para esse endereço IP ou intervalo de endereços. Pode levar até cinco minutos para que essa alteração entre em vigor. |
