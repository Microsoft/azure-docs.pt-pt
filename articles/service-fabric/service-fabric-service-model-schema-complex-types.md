---
title: Tipos complexos de esquema XML do modelo de serviço do Azure Service Fabric
...
A quantidade total de memória de permuta que pode ser usada, em MB. Deve ser um inteiro positivo.
...
O limite flexível (em MB) para a governança de memória que é imposta somente quando a contenção de memória é detectada no nó. Deve ser um inteiro positivo.
...
Percentual utilizável de CPUs disponíveis (somente Windows). Deve ser um inteiro positivo. Se os limites de CPU forem especificados para o pacote de serviço, esse parâmetro será efetivamente ignorado.
...
Taxa máxima de e/s (leitura e gravação) em termos de IOps que podem ser usados. Deve ser um inteiro positivo.
...
A e/s máxima (bytes por segundo) que pode ser usada (leitura e gravação). Deve ser um inteiro positivo.
...
Cota de disco para contêineres.  Deve ser um inteiro positivo.
...
Limites de memória do kernel em bytes.  Deve ser um inteiro positivo.  Observe que esse é o Linux específico e o Docker no Windows irá gerar um erro se ele estiver definido.
...
Tamanho de/dev/shm em bytes. Se omitido, o sistema usará 64MB.  Deve ser um inteiro positivo.  Observe que isso é específico do Linux, no entanto, o Docker não fará erro se especificado.  Ele é simplesmente ignorado.
...
Especifica a porcentagem máxima tolerada de serviços não íntegros antes que o aplicativo seja considerado não íntegro. Porcentagem padrão: 0.
...
Especifica o percentual máximo tolerado de partições não íntegras antes que um serviço seja considerado não íntegro. Porcentagem padrão: 0.
...
Especifica o percentual máximo tolerado de réplicas não íntegras antes que uma partição seja considerada não íntegra. Porcentagem padrão: 0.
