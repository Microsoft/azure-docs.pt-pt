---
title: Dimensionamento do Cluster Service Fabric do Azure | Microsoft Docs
description: Saiba mais sobre como dimensionar os clusters do Azure Service Fabric para dentro ou para fora ou para baixo.
...
- Qualquer tipo de nó de [nível](service-fabric-cluster-capacity.md#the-durability-characteristics-of-the-cluster) de durabilidade Gold ou prata sempre deve ter cinco ou mais nós.
...
No entanto, o código de dimensionamento não precisa ser executado como um serviço no cluster para ser dimensionado. `IAzure` E`FabricClient` podem se conectar a seus recursos do Azure associados remotamente, de modo que o serviço de dimensionamento pode ser facilmente um aplicativo de console ou serviço do Windows em execução de fora do aplicativo Service Fabric.
