...
Além disso, o fornecedor do Sistema de Ficheiros ou o fornecedor de armazenamento Azure devem ser utilizados como um método de partilha de uma Loja de Imagens entre vários clusters - isto resultará na corrupção de dados de configuração de cluster, uma vez que cada cluster pode escrever dados contraditórios para a Loja de Imagens. Para partilhar pacotes de aplicações provisionados entre vários clusters, utilize ficheiros [sfpkg,][12] que podem ser enviados para qualquer loja externa com um URI de download.
