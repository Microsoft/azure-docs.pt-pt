...
| **Parâmetro** | **Valores permitidos** | **Política de atualização** | **Documentação de orientação ou descrição breve** |
| --- | --- | --- | --- |
|AdminOnlyHttpAudit |Bool, a predefinição é verdadeiro | Dinâmica | Exclua os pedidos HTTP que não afetam o estado do cluster de auditoria. Atualmente; não são excluídos apenas pedidos do tipo "GET"; mas isso está sujeito a alterações. |
...
|AuditHttpRequests |Bool, a predefinição é falso | Dinâmica | Ative ou desativar a auditoria de HTTP. O objetivo de auditoria é ver as atividades que foram realizadas face ao cluster; incluindo quem iniciou o pedido. Tenha em atenção que se trata de um registo de tentativa de melhor; e pode ocorrer perda de rastreio. Pedidos HTTP com a autenticação de "Usuário" não está registada. |
|CaptureHttpTelemetry|Bool, a predefinição é falso | Dinâmica | Ative ou desativar a telemetria HTTP. É a finalidade de telemetria para o Service Fabric poder capturar dados de telemetria para ajudar a planejar o futuro trabalho e identificar áreas problemáticas. Telemetria não grava todos os dados pessoais ou o corpo do pedido. Telemetria captura todas as solicitações HTTP, a menos que caso contrário, configurado. |
...
|FailuresOnlyHttpTelemetry | Bool, a predefinição é verdadeiro | Dinâmica | Se estiver ativada a captura de telemetria HTTP; Capture apenas os pedidos falhados. Isso serve para ajudar a reduzir o tempo limite no número de eventos gerados para a telemetria. |
|HttpTelemetryCapturePercentage | Int, a predefinição é 50 | Dinâmica | Se estiver ativada a captura de telemetria HTTP; Capture apenas uma percentagem de pedidos aleatória. Isso serve para ajudar a reduzir o tempo limite no número de eventos gerados para a telemetria. |
...
|HealthReportRetrySendInterval |Tempo em segundos, a predefinição é 30, mínimo é 1 |Dinâmica|Especifique o período de tempo em segundos. O intervalo no qual o componente de relatórios reenvia o estado de funcionamento acumulado relatórios para o Gestor de estado de funcionamento. |
...
|MaxUnusedAppTypeVersionsToKeep | Int, a predefinição é 3 |Dinâmica|Esta configuração define o número de versões de tipo de aplicações não utilizadas de ser ignorada para a limpeza. Este parâmetro é aplicável apenas se o parâmetro CleanupUnusedApplicationTypes está ativado. |
...
|Carregamento |cadeia de caracteres, predefinido for "Admin" | Dinâmica|A operação de carregamento do cliente do arquivo de configuração de segurança para a imagem. |
