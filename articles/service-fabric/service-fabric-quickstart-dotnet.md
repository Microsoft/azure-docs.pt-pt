---
title: Crie rapidamente uma aplicação .NET no Service Fabric in Azure
...
* Use ASP.NET núcleo como uma extremidade frontal web
...
Depois de instalar o tempo de execução, SDKs, ferramentas do Estúdio Visual, Docker, e ter o Docker a funcionar, criar um cluster de desenvolvimento local de cinco nós.

> [!Note]
> A razão para ter o Docker a funcionar quando cria o cluster é para que o cluster seja criado com características de contentores ativadas. Se o Docker não estiver a funcionar, terá de recriar o cluster para ativar as características do recipiente.
> Embora desnecessária para este arranque rápido em particular, a instrução para ter o Docker a funcionar quando cria o cluster está incluída como uma boa prática.
...
>
> [Configurar o Windows 10 ou o Windows Server para contentores](https://docs.microsoft.com/virtualization/windowscontainers/quick-start/set-up-environment?tabs=Windows-10-Client)
...
2. Execute o seguinte comando do PowerShell para criar um cluster de desenvolvimento:
   ...
   ```
3. Executar o seguinte comando para iniciar a ferramenta local de gestor de cluster:
...
Clique com o botão direito do rato no ícone do Visual Studio, no Menu Iniciar, e escolha **Executar como administrador**. Para anexar o debugger aos seus serviços, precisa de executar o Visual Studio como administrador.

Abra a solução **Voting.sln** do Visual Studio no repositório que clonou.

Por predefinição, o pedido de votação ouve na porta 8080.  A porta da aplicação está definida no ficheiro */VotingWeb/PackageRoot/ServiceManifest.xml*.  Pode atualizar o atributo **Port** do elemento **Endpoint** para alterá-la.  Para implementar e executar a aplicação localmente, a porta da aplicação tem de estar aberta e disponível no seu computador.  Se alterar a porta de aplicação, substitua o novo valor da porta de aplicação por "8080" ao longo deste artigo.
...
Quando a implementação estiver concluída, `http://localhost:8080` lance um navegador e abra para ver a extremidade frontal web da aplicação.
...
Quando vota na candidatura, ocorrem os seguintes eventos:
...
A aplicação deve estar em execução OK, mas pode utilizar o depurador para ver como as partes das chaves do trabalho de aplicação. Ao depurar a aplicação no Estúdio Visual, está a usar um cluster de desenvolvimento de Tecido de Serviço local. Pode ajustar a sua experiência de depuração ao seu cenário. Nesta aplicação, os dados são armazenados no serviço de back-end através de um dicionário fiável. O Visual Studio remove a aplicação por predefinição, quando para o depurador. Remover a aplicação faz com que os dados no serviço de back-end sejam também removidos. Para persistir os dados entre as sessões de depuração, pode alterar o **Modo de Depuração da Aplicação** como propriedade no projeto **Voting** no Visual Studio.
...
3. Regresse ao browser e clique numa opção de votação ou adicione uma opção nova. Atingiu o primeiro ponto de interrupção no controlador de API do front-end da Web.
   * Este passo é onde o JavaScript no navegador envia um pedido para o controlador Web API no serviço frontal.
...
4. Pressione **F5** para continuar
   ...
   - Está sem acordo no serviço de back-end.
...
7. No diálogo de aplicação de **tecido de serviço Publish,** verifique a atualização da caixa de **verificação da aplicação**.
8.  Alterar **o perfil de destino** para **PublishProfiles\Local.5Node.xml** e certifique-se de que o ponto final da **ligação** está definido para **cluster local**. 
...
11. Abra o seu navegador e navegue para o endereço do cluster na porta 19080. Por exemplo, `http://localhost:19080/`.
