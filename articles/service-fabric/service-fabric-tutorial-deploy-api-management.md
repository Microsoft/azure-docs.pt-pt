---
title: Integrar a Gestão da API com tecido de serviço em Azure
description: Saiba como começar rapidamente com a Azure API Management e encaminhar o tráfego para um serviço de back-end em Service Fabric.
ms.topic: conceptual
ms.date: 07/10/2019
...
Este artigo mostra-lhe como configurar [a Azure API Management](../api-management/api-management-key-concepts.md) com o Service Fabric para encaminhar o tráfego para um serviço de back-end em Service Fabric.  Quando tiver terminado, terá implementado a Gestão de API numa VNET e configurado uma operação de API para enviar tráfego para serviços sem estado de back-end. Para saber mais sobre os cenários da Gestão de API do Azure com o Service Fabric, veja o artigo [Overview](service-fabric-api-management-overview.md) (Descrição Geral).
...
Agora que tem um [cluster Windows](service-fabric-tutorial-create-vnet-and-windows-cluster.md) seguro no Azure, implante a API Management para a rede virtual (VNET) na sub-rede e NSG designada para a Gestão da API. Para este artigo, o modelo de Gestor de Recursos de Gestão da API está pré-configurado para utilizar os nomes do VNET, sub-rede e NSG que criou no tutorial do [cluster Windows](service-fabric-tutorial-create-vnet-and-windows-cluster.md) Este artigo implementa a seguinte topologia para Azure em que a API Management and Service Fabric está em sub-redes da mesma Rede Virtual:
...
Crie um serviço básico de ASP.NET apátrida core fiável usando o modelo de projeto de API web padrão. Como resultado, é criado um ponto final de HTTP para o seu serviço, que vai expor através da Gestão de API do Azure.
...
Inicie o Visual Studio como Administrador e crie um serviço ASP.NET Core:
 ...
 4. Selecione o modelo de projeto Core 2.1 da API Web ASP.NET.
 5. Uma vez criado o projeto, abra `PackageRoot\ServiceManifest.xml` e remova o atributo `Port` da configuração do recurso do ponto final:
    ...
    A remoção da porta permite que o Service Fabric especifique uma porta dinamicamente da gama portuária de aplicações, aberta através do Grupo de Segurança da Rede no modelo cluster Resource Manager, permitindo que o tráfego flua para ele a partir da API Management.
...
* **displayName** pode ser qualquer nome para a API. Para este artigo, utilize a "App de Tecido de Serviço".
...
Faça o súmis no Azure e selecione o ID de subscrição com o qual pretende remover o cluster.  Pode encontrar o ID da subscrição ao iniciar sessão no [portal do Azure](https://portal.azure.com). Elimine o grupo de recursos e todos os recursos de cluster utilizando o [cmdlet Remove-AzResourceGroup](/powershell/module/az.resources/remove-azresourcegroup).
