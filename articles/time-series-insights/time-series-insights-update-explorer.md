...
description: Saiba mais sobre funcionalidades e opções disponíveis no explorador de pré-visualização da Série de Tempo Azure.
author: deepakpalled
ms.author: dpalled
manager: cshankar
...
ms.locfileid: "86037312"
---
# <a name="azure-time-series-insights-preview-explorer"></a>Explorador de pré-visualização da série de tempo Azure

Este artigo descreve as várias funcionalidades e opções disponíveis na [aplicação web de](https://insights.timeseries.azure.com/preview/demo)pré-visualização da Série de Pré-visualização Azure.

## <a name="prerequisites"></a>Pré-requisitos

Para começar com o explorador de pré-visualização da Série De Tempo Azure, tem de:

* Tenha um ambiente de Time Series Insights aprovisionado. Saiba mais sobre o provisionamento de um caso lendo o tutorial de visualização da [Série de Tempo Azure.](./time-series-insights-update-create-environment.md)
* Fornecer acesso de [dados](./time-series-insights-data-access.md) ao ambiente Time Series Insights que criou para a conta. Você pode fornecer acesso aos outros, bem como a si mesmo.
* Adicione uma fonte de evento ao ambiente Time Series Insights para empurrar os dados para o ambiente:
...
## <a name="explore-the-time-series-insights-preview-explorer"></a>Explore o explorador de pré-visualização da Série De Tempo

O explorador de pré-visualização da Série De Tempo Azure é composto pelos seguintes sete elementos:

[![Visão geral do explorador de pré-visualização de séries de tempo](media/v2-update-explorer/preview-explorer-overview.png)](media/v2-update-explorer/preview-explorer-overview.png#lightbox)

1. [Painel ambiente](#1-environment-panel): Exibe todos os ambientes da série de tempo Azure.
...
O painel de ambiente exibe todos os ambientes de Insights da Série Tempo a que tem acesso. A lista inclui ambientes pay-as-you-go (Preview), bem como ambientes S1/S2 (Disponibilidade Geral). Basta selecionar o ambiente Time Series Insights que pretende utilizar para ser imediatamente levado para lá.
...
* **Modelo**: Use-o para empurrar novos tipos de visualização de séries de tempo, hierarquias e instâncias para o seu modelo de Insights de Séries Tempo.

### <a name="model-authoring"></a>Autoria do modelo

A Azure Time Series Insights Preview suporta operações completas de criação, leitura, atualização e eliminação (CRUD) no seu Modelo de Séries Tempores.
...
* **Tipo de modelo série de tempo**: Pode utilizar tipos de Insights de Séries de Tempo para definir variáveis ou fórmulas para fazer cálculos. Estão associados a uma instância de insights da Série temporal. Um tipo pode ter uma ou mais variáveis.
* **Hierarquia do Modelo série de tempo**: As hierarquias são organizações sistemáticas dos seus dados. As hierarquias retratam as relações entre diferentes entidades nos dados da Time Series Insights.
...
O poço exibe campos de exemplo e outros metadados associados a instâncias selecionadas de Insights de Séries De Tempo. Ao selecionar as caixas de verificação no lado direito, pode ocultar ou exibir instâncias específicas a partir do gráfico atual. 
...
Depois de efetuar a ação **Zoom,** o seu conjunto de dados selecionado será apresentado. Selecione o controlo do formato para pedalar através das representações de três eixos y dos seus dados de Insights de Séries Tempotamos.
...
Saiba mais sobre a opção **Connect to Power BI** em Time Series Insights [conector power bi nativo](concepts-power-bi.md).

## <a name="6-time-editor-panel"></a>6. Painel de editores de tempo

Ao trabalhar com o Time Series Insights, seleciona-se primeiro um intervalo de tempo. O tempo de tempo selecionado controlará o conjunto de dados que está disponível para manipulação com os widgets de atualização time Series Insights.
...
O painel de navegação de pré-visualização da Série De Tempo aparece no topo da sua aplicação Time Series Insights. Fornece as seguintes funcionalidades:
...
* Apresenta as informações atuais da conta de insusição da Série De Tempo.
* Utilize-o para alternar entre os temas disponíveis de Time Series Insights.
* Use-o para ver a [aplicação web de demonstração de pré-visualização.](https://insights.timeseries.azure.com/preview/demo)
...
A azure Time Series Insights Preview suporta dois temas:
...
Esta secção aplica-se apenas aos ambientes S1/S2 existentes que tentam utilizar o explorador na UI atualizada. É melhor utilizar o produto geralmente disponível e pré-visualizar em combinação. Adicionámos algumas funcionalidades da UI existente ao explorador atualizado, mas pode obter a experiência completa de UI para o ambiente S1/S2 no explorador de Insights da Série Temporal existente. 

Em vez da hierarquia, é apresentado o painel de termos de Insights da Série De Tempo. O painel de termos permite definir consultas no seu ambiente. Utilize-o também para filtrar dados com base num predicado.
...
O painel de editor de termos de visualização de termos de visualização da Série De Tempo toma os seguintes parâmetros:
...
- Leia o documento de pré-visualização da Série De Tempo sobre [modelação de dados](./concepts-model-overview.md).

- Aprenda [a diagnosticar e a resolver problemas na](./time-series-insights-update-how-to-troubleshoot.md) sua instância de Insights da Série De Tempo.
