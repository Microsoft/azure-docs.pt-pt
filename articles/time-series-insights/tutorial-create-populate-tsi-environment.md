...
Este tutorial irá guiá-lo pelo processo de criação de um ambiente do Time Series Insight, preenchido com dados a partir de dispositivos simulados. Neste tutorial, ficará a saber como:
...
1. Na página **Ambiente do Time Series Insights**, preencha os parâmetros necessários:
   ...
   Parâmetro|Descrição
   ---|---
   **Nome do ambiente** | Escolha um nome exclusivo para o ambiente do Time Series Insights. O nome do utilizado pelo Explorador do Time Series Insights e APIs de consulta.
   **Subscrição** | As subscrições são contentores de recursos do Azure. Escolha uma subscrição para criar o ambiente do Time Series Insights para.
   ...
   **Localização** | Escolha uma região do Datacenter para o seu ambiente do Time Series Insights. Para evitar os custos de largura de banda adicional e latência, é melhor manter o ambiente do Time Series Insights na mesma região que outros recursos de IoT.
...
Em seguida, crie a solução de simulação do dispositivo, que irá gerar dados de teste para preencher o seu ambiente do Time Series Insights:
...
1. Introduza os parâmetros necessários **solução de simulação do dispositivo criar** página.
   ...
   **Subscrição** | Especifique a mesma subscrição utilizada para a criação de seu ambiente do Time Series Insights, na secção anterior.
   **Região** | Especifica a mesma região utilizada para a criação do seu ambiente do Time Series Insights, na secção anterior.
...
Lembre-se que os dispositivos simulados precisam de estar ligados a um hub IoT para transmitirem dados dos dispositivos. Para os dados de fluxo no ambiente do Time Series Insights, terá de efetuar alterações de configuração para o IoT hub e o ambiente do Time Series Insights.
...
Agora ligar o novo hub IoT **grupo de consumidores** ponto final de eventos, para o ambiente do Time Series Insights, como um **origem do evento**.

1. Vá para o **descrição geral** página do grupo de recursos criado para o ambiente do Time Series Insights, em seguida, selecione o ambiente do Time Series Insights:
...
1. Na página de ambiente do Time Series Insights, selecione **origens de eventos**, em seguida, clique em **+ adicionar**.
...
1. Introduza os parâmetros necessários **nova origem de evento** página.
   ...
   **Nome da propriedade Carimbo de data/hora** | Especificar como `timestamp`.
   ...
   Quando terminar, clique em **Criar** para adicionar a origem de evento. Quando regressar ao grupo de recursos **descrição geral** página, juntamente com o seu recurso de ambiente do Time Series Insights, verá um novo recurso de "Origem de eventos do Time Series Insights".
...
1. No ambiente do Time Series Insights **descrição geral** página, clique nas **URL de Explorador do Time Series Insights** para abrir o Explorador do Time Series Insights.
...
1. Explorador do Time Series Insights carregará e efetuar a autenticação com a sua conta do portal do Azure. Após a exibição inicial, pode ver na área de gráfico que o ambiente do Time Series Insights, de fato, foi preenchido com dados de telemetria simulada. Para filtrar um intervalo de tempo mais curto, selecione a lista pendente no canto superior esquerdo. Introduza um intervalo de tempo suficientemente grande para abranger a duração da simulação de dispositivos. Em seguida, clique em pesquisa ampliar a classe.
...
1. Restrição o intervalo de tempo, permite que o gráfico para ampliar para a expansão distinta de transferência de dados, para o IoT hub e o ambiente do Time Series Insights. Repare também no texto **Transmissão concluída**, no canto superior direito, que mostra o número total de eventos encontrado. Também pode arrastar o **tamanho do intervalo** controlo de deslize para controlar a granularidade de desenho no gráfico.
...
Este tutorial cria vários serviços do Azure em execução, para suportar a solução de simulação de dispositivo e ambiente de Time Series Insights. Se quiser abandonar e/ou retardar a conclusão desta série de tutoriais, recomendamos eliminar todos os recursos para evitar incorrer em custos desnecessários.

No menu esquerdo do portal do Azure:

1. Clique nas **grupos de recursos** ícone, em seguida, selecione o grupo de recursos que criou para o ambiente de Time Series Insights. Na parte superior da página, clique em **Eliminar grupo de recursos**, escreva o nome do grupo de recurso e, em seguida, clique em **Eliminar**.
