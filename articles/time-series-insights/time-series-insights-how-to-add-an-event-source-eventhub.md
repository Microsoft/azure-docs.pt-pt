...
description: Saiba como adicionar uma fonte de eventos Azure Event Hubs ao ambiente time series Insights.
...
---
# <a name="add-an-event-hub-event-source-to-your-time-series-insights-environment"></a>Adicione uma fonte de eventos hub de evento ao seu ambiente de Insights de Séries Tempo
...
> [!NOTE]
> Os passos descritos neste artigo aplicam-se tanto aos ambientes de visualização de Insights GA da Série De Tempo e de Previsão de Insights de Séries Tempo.

## <a name="prerequisites"></a>Pré-requisitos

- Crie um ambiente de Insights de Séries Tempoturais, conforme descrito no [Criar um ambiente de Insights de Séries De Tempo Azure](./time-series-insights-update-create-environment.md).
...
- Crie um grupo de consumidores dedicado no centro de eventos que o ambiente Time Series Insights pode consumir. Cada fonte de eventos da Time Series Insights deve ter o seu próprio grupo de consumidores dedicado que não é partilhado com nenhum outro consumidor. Se vários leitores consumirem eventos do mesmo grupo de consumidores, todos os leitores são suscetíveis de apresentar falhas. Há um limite de 20 grupos de consumidores por centro de eventos. Para mais detalhes, leia o guia de programação do [Event Hubs.](../event-hubs/event-hubs-programming-guide.md)

### <a name="add-a-consumer-group-to-your-event-hub"></a>Adicione um grupo de consumidores ao seu centro de eventos

As aplicações utilizam grupos de consumidores para extrair dados dos Hubs de Eventos Azure. Para ler de forma fiável os dados do seu centro de eventos, forneça um grupo de consumidores dedicado que é usado apenas por este ambiente Time Series Insights.
...
1. Na página **de grupos de consumidores,** insira um novo valor único para **o Nome.**  Use este mesmo nome quando criar uma nova fonte de evento no ambiente Time Series Insights.
...
1. Localize o ambiente de Insights de Séries de Tempo existentes. No menu esquerdo, selecione **Todos os recursos**e, em seguida, selecione o ambiente De Insights de Séries Tempotuais.
...
1. Introduza um valor para **o nome de origem do Evento** que seja exclusivo deste ambiente time series Insights, como `Contoso-TSI-GA-Event-Hub-ES` .
...
1. Selecione os valores adequados para **a opção Importação**:
   ...
   * Se tiver um centro de eventos existente numa das suas subscrições, selecione **Use Event Hub a partir de subscrições disponíveis.** Esta opção é a abordagem mais fácil.
    ...
    *  A tabela a seguir descreve as propriedades necessárias para o Centro de Eventos de Utilização a partir da opção **de subscrições disponíveis:**
       ...
       | O nome do hub de eventos | O nome da instância do centro de eventos desejado. |
...
1. Adicione o nome dedicado do grupo de consumidores Time Series Insights que adicionou ao seu centro de eventos.

1. Selecione **Criar**.

   Após a criação da fonte do evento, o Time Series Insights começa automaticamente a transmitir dados para o seu ambiente.
