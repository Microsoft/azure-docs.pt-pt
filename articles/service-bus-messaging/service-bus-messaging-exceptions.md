...
| [TransacçãoExcepção](/dotnet/api/system.transactions.transactionexception?view=netcore-3.1&preserve-view=true) |A transação ambiente *(Transação.Corrente)* é inválida. Pode ter sido concluído ou abortado. A exceção interior pode fornecer informações adicionais. | |Tentar não ajuda. |
| [TransaçãoInDoubtExcepção](/dotnet/api/system.transactions.transactionindoubtexception?view=netcore-3.1&preserve-view=true) |Uma operação é tentada numa transação que está em dúvida, ou uma tentativa de efetivo da transação e a transação fica em dúvida. |O seu pedido deve tratar desta exceção (como um caso especial), uma vez que a transação pode já ter sido cometida. |- |
