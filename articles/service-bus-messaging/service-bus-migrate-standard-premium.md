...
1. Verifique o estado da migração utilizando o seguinte comando:
    ...
    * pendentesReplicaçõesOperagens = 0
...
1. Sincronizar entidades entre os espaços de nome padrão e premium.
   ...
   1. Espere até que a sincronização esteja completa. O estado está disponível na barra de estado.
        ![Configurar espaço de nome - entidades sincronizadas - progresso][]
        >[!IMPORTANT]
...
1. Rever alterações na página de resumo. Selecione **Complete Migration** para mudar de espaço de nomes e para completar a migração.
    ![Mudar espaço de nome - comutador menu][]  
    A página de confirmação aparece quando a migração está completa.
    ![Mudar espaço de nome - sucesso][]
...
### <a name="partitioned-entities"></a>Entidades particionadas
   ...
   As entidades divididas foram apoiadas no nível Standard para proporcionar uma melhor disponibilidade numa configuração multi-arrendatária. Com a disponibilização de recursos dedicados disponíveis por espaço de nome no escalão Premium, isso já não é necessário.
...
A migração pode ser abortada usando o `Abort` comando ou utilizando o portal Azure.
...
![Abortar fluxo][]
![- abortar fluxo abortar sincronização Abortar - abortar completo][]
