...
- Python 3.4x ou superior, com o pacote [de ônibus de serviço Python Azure][Python Azure Service Bus package] instalado. Para mais informações, consulte o Guia de [Instalação Python](/azure/python/python-sdk-azure-install). 
...
Adicione o seguinte código para criar um objeto **ServiceBusClient.** Substitua `<connectionstring>` pelo valor de cadeia de cadeia de ligação primária do bus de serviço. Pode encontrar este valor ao abrigo de políticas de **acesso partilhado** no seu espaço de nome sacar o seu ônibus de serviço no [portal Azure][Azure portal].

```python
sb_client = ServiceBusClient.from_connection_string('<connectionstring>')
```

O seguinte código utiliza o método `create_queue` do **ServiceBusClient** para criar uma fila chamada `taskqueue` com definições predefinidas:

```python
sb_client.create_queue("taskqueue")
```

Pode utilizar opções para anular as definições de fila predefinidas, tais como tempo de mensagem para viver (TTL) ou tamanho máximo do tópico. O seguinte código cria uma fila chamada `taskqueue` com um tamanho máximo de fila de 5 GB e valor TTL de 1 minuto:
...
Para enviar uma mensagem para uma fila de ônibus de serviço, uma aplicação chama o método `send` no objeto **ServiceBusClient.** O seguinte exemplo de código cria um cliente de fila e envia uma mensagem de teste para a `taskqueue` fila. Substitua `<connectionstring>` pelo valor de cadeia de cadeia de ligação primária do bus de serviço. 
...
O cliente da fila recebe mensagens de uma fila utilizando o método `get_receiver` no objeto **ServiceBusClient.** O seguinte exemplo de código cria um cliente de fila e recebe uma mensagem da fila `taskqueue`. Substitua `<connectionstring>` pelo valor de cadeia de cadeia de ligação primária do bus de serviço. 
