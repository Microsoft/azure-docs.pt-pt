...
| Propriedade | Descrição |
|----------|-------------|
|**Connection name** (Nome da ligação) | Name of the API connection. Necessário. |
|**Ethereum RPC endpoint** | HTTP address of the Azure Blockchain Service transaction node. Necessário. For more information, see [Get the RPC endpoint](#get-the-rpc-endpoint). |
