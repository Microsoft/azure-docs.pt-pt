...
1. Selecione **Adicionar**.
    ...
    --------|---------|------------
    Nome | mywatcher | Enter a unique name for a connected Blockchain Data Manager.
    ...
    Nome da ligação | cosmosdb | Enter a unique name of the outbound connection where blockchain transaction data is sent.
...
1. Create a new container for the storage account. Select **Containers > Container**.
    ...
    | Definição | Descrição |
    |---------|-------------|
    | Nome  | Name the container. For example, *smartcontract* |
...
1. Selecione **Adicionar**.
    ...
    Definição | Descrição
    --------|------------
    Nome | Enter a unique name for the blockchain application to track.
...
1. In the **Add container** pane, enter the settings for the new container.
    ...
    | Definição | Descrição
    |---------|-------------|
    | ID da base de dados | Enter **blockchain-data** as the name for the new database. |
    | Débito | Leave the throughput at **400** request units per second (RU/s). Se pretender reduzir a latência, pode aumentar o débito mais tarde.|
    | Container ID | Enter **Messages** as the name for your new container. |
    | Chave de partição | Use **/MessageType** as the partition key. |
...
Todas as aplicações lógicas têm de iniciar com um acionador, que é desencadeado quando um evento específico acontece ou quando uma condição específica é cumprida. Sempre que o acionador é acionado, o motor do Logic Apps cria uma instância da aplicação lógica que inicia e executa o fluxo de trabalho. Use an Azure Event Grid trigger to sends blockchain transaction data from Event Grid to Cosmos DB.
...
1. Create an API connection to your Event Grid Topic.
    ...
    | Definição | Descrição
    |---------|-------------|
    | Subscrição | Choose the subscription that contains the Event Grid Topic. |
    | Tipo de Recurso | Choose **Microsoft.EventGrid.Topics**. |
    | Nome do Recurso | Choose the name of the Event Grid Topic where Blockchain Data Manager is sending transaction data messages. |
...
1. Create an API connection to your Cosmos DB database.
    ...
    | Definição | Descrição
    |---------|-------------|
    | Nome da Ligação | Choose the subscription that contains the Event Grid Topic. |
...
Parabéns! You have successfully created a transaction message explorer using Blockchain Data Manager and Azure Cosmos DB.
...
1. Selecione **Eliminar grupo de recursos**. Verify deletion by entering the resource group name and select **Delete**.
