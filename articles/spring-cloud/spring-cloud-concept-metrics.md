...
Em Azure Spring Cloud, existem dois pontos de vista para métricas.
...
Os detalhes de todas as métricas podem ser encontrados na [secção](#user-metrics-options) abaixo.
...
O intervalo de tempo para mostrar também pode ser modificado. O intervalo de tempo pode ser escolhido entre os últimos 30 minutos e os 30 dias, ou um intervalo de tempo personalizado.
...
>[!TIP]
> Pode construir os seus próprios gráficos na página de métricas e fixá-los no seu **Dashboard.** Comece por nomear a sua ficha.  Em seguida, selecione **Pin para o painel no canto superior direito**. Já pode verificar a sua aplicação no seu Portal **Dashboard**.
...
>|----|----|----|------------|
>| TomcatErrorCount<br><br>Tomcat Global Error (precotado) (removido a 1 de julho de 2020) | tomcat.global.erro | Contagem | Número de erros ocorre de pedidos processados |
...
>| Name | Nome métrico do actuador de primavera | Unidade | Detalhes |
>|----|----|----|------------|
>| SystemCpuUsagePercentage<br><br>Percentagem de Utilização do CPU do Sistema (precotado) (removido em 1 de julho de 2020) | system.cpu.usage | Percentagem | Uso recente do CPU para todo o sistema. Este valor é um duplo no intervalo [0.0,1.0]. Um valor de 0,0 significa que todos os CPUs estiveram inativos durante o período de tempo observado recentemente, enquanto um valor de 1.0 significa que todas as CPUs estavam a funcionar ativamente a 100% do tempo durante o período recente observado.|
...
>| OldGenMemoryPoolBytes<br><br>Tamanho dos dados de geração antiga (precotado) (removido a 1 de julho de 2020) | jvm.gc.live.data.size | Bytes | Tamanho da memória de geração velha depois de um GC completo. |
>| jvm.gc.live.data.size | jvm.gc.live.data.size | Bytes | Tamanho da memória de geração velha depois de um GC completo. |
>| OldGenPromotedBytes<br><br>Promover ao tamanho dos dados de geração antiga (precotado) (removido a 1 de julho de 2020) | jvm.gc.memory.promovido | Bytes | Contagem de aumentos positivos no tamanho do pool de memória de geração antiga antes de GC para depois de GC. |
...
>| YoungGenPromotedBytes<br><br>Promover ao tamanho dos dados de geração jovem (preprecado) (removido a 1 de julho de 2020) | jvm.gc.memory.alocado | Bytes | Incrementado para um aumento no tamanho do pool de memória de geração jovem após um GC para antes do próximo. |
...
>| GCPauseTotalCount<br><br>GC Pause Count (precotado) (removido em 1 de julho de 2020) | jvm.gc.pausa (contagem total) | Contagem | Contagem total de GC após o início deste JMV, incluindo Young and Old GC. |
...
>| GCPauseTotalTime<br><br>GC Pausa Tempo Total (precedido) (removido em 1 de julho de 2020) | jvm.gc.pausa (tempo total) | Milissegundos | Total de tempo de GC consumido após o início deste JMV, incluindo Young and Old GC. |
...
>| tomcat.threads.current | tomcat.threads.current | Contagem | Contagem de fios de corrente tomcat |
>| tomcat.global.request.avg.time | tomcat.global.request.avg.time | Milissegundos | Tomcat Pede tempo médio |
...
>| Name | Nome métrico do actuador de primavera | Unidade | Detalhes |
>|----|----|----|------------|
>| TomcatSentBytes<br><br>Tomcat Total Enviado Bytes (precotado) (removido em 1 de julho de 2020) | tomcat.global.enviado | Bytes | Quantidade de dados que o servidor web tomcat enviado |
>| tomcat.global.enviado | tomcat.global.enviado | Bytes | Quantidade de dados que o servidor web tomcat enviado |
>| TomcatReceivedBytes<br><br>Tomcat Total Recebido Bytes (precotado) (removido em 1 de julho de 2020) | tomcat.global.recebeu | Bytes | Quantidade de dados que o servidor web Tomcat recebeu |
>| tomcat.global.recebeu | tomcat.global.recebeu | Bytes | Quantidade de dados que o servidor web Tomcat recebeu |
>| TomcatRequestTotalTime<br><br>Tomcat Request Total Time (precotado) (removido em 1 de julho de 2020) | tomcat.global.request (tempo total) | Milissegundos | Tempo total do servidor web Tomcat para processar os pedidos |
>| TomcatRequestTotalCount<br><br>Tomcat Request Total Count (precotado) (removido em 1 de julho de 2020) | tomcat.global.request (contagem total) | Contagem | Contagem total de pedidos processados do servidor web Tomcat |
...
>| TomcatRequestMaxTime<br><br>Tomcat Request Max Time (preceded) (removido em 1 de julho de 2020) | tomcat.global.request.max | Milissegundos | Tempo máximo do servidor web Tomcat para processar um pedido |
...
>| Name | Nome métrico do actuador de primavera | Unidade | Detalhes |
>|----|----|----|------------|
>| TomcatSessionActiveMaxCount<br><br>Tomcat Session Max Ative Count (precotado) (removido em 1 de julho de 2020) | tomcat.sessions.ative.max | Contagem | Número máximo de sessões que estiveram ativas ao mesmo tempo |
...
>| TomcatSessionAliveMaxTime<br><br>Tomcat Session Max Alive Time (precessão) (removido a 1 de julho de 2020) | tomcat.sessions.alive.max | Milissegundos | Tempo mais longo (em segundos) que uma sessão expirada tinha sido vivo |
...
>| TomcatSessionCreatedCount<br><br>Tomcat Session Created Count (precotado) (removido em 1 de julho de 2020) | tomcat.sessions.created | Contagem | Número de sessões que foram criadas |
>| tomcat.sessions.created | tomcat.sessions.created | Contagem | Número de sessões que foram criadas |
>| TomcatSessionExpiredCount<br><br>Tomcat Session Expired Count (precotado) (removido em 1 de julho de 2020) | tomcat.sessions.expirou | Contagem | Número de sessões que expiraram |
>| tomcat.sessions.expirou | tomcat.sessions.expirou | Contagem | Número de sessões que expiraram |
>| TomcatSessionRejectedCount<br><br>Tomcat Session Rejected Count (precotado) (removido em 1 de julho de 2020) | tomcat.sessions.rejeitado | Contagem | Número de sessões que não foram criadas porque o número máximo de sessões ativas atingiu. |
