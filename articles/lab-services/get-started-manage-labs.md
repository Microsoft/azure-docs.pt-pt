...
ms.locfileid: "93380290"
---
# <a name="get-started-with-lab-services"></a>Começar com os Serviços de Laboratório 

Como estudante, pode utilizar o Azure Lab Services para aceder ao software padrão da indústria necessário para os seus programas de estudo em máquinas virtuais (VM). 

Os professores precisam de saber como ensinar os alunos a utilizar os Serviços do Laboratório Azure na sua instrução através de um para um aluno emitido hardware.

Este artigo fornece informações para o pessoal docente sobre como aceder, gerir e ensinar os alunos a utilizar os Serviços do Laboratório Azure.

## <a name="overview"></a>Descrição geral

O que é um VM e como funcionam?
...
Para obter mais informações, consulte [Criar e gerir os laboratórios de sala de aula.](how-to-manage-classroom-labs.md)
...
1. Abaixo dos custos & azulejo **de faturação, existem** também azulejos para Modelos, Piscinas de Máquinas Virtuais, Utilizadores e Horários, que permitem modificar aspetos e ver mais detalhes no Laboratório de Sala de Aula.
    ...
    1. Modelo - descreve a data em que o modelo foi criado, e publicado pela última vez. 
    1. Piscina de Máquinas Virtuais - número de VMs atribuídos e não atribuídos.
    1. Utilizadores - número de utilizadores registados e utilizadores que foram adicionados ao laboratório, mas não registados.
    1. Horários - apresenta os próximos eventos agendados para o laboratório e um link para ver mais eventos.
...
Para ajudar a reduzir os custos e poupar as quotas dos alunos, as paragens automáticas serão ativadas para os nossos laboratórios. As paragens automáticas desligam os VMs após um período de inatividade (sem entradas de rato ou teclado). As paragens automáticas funcionam em duas fases, primeiro um aluno será desligado do VM após um período de inatividade. Neste momento, o VM ainda está **em execução** e os alunos são capazes de se conectar. Após outro período de inatividade desligado, o VM desliga-se sozinho.

As paragens automáticas são uma importante ferramenta de poupança de custos, no entanto representam um desafio para os estudantes no que diz respeito a salvar o seu trabalho e a renderizar grandes ficheiros de projetos. Se os alunos estiverem frequentemente desligados ou os VMs estiverem a desligar-se demasiado rapidamente. Por favor contacte o seu administrador CTE. 
...
A coisa mais importante a lembrar quando gerimos os nossos VMs é que sempre que uma máquina está **a funcionar,** estamos a incorrer em custos, mesmo que ninguém esteja ligado ao VM.
...
### <a name="connect-to-virtual-machines"></a>Ligar às máquinas virtuais
...
1. A sua lista de utilizadores será agora atualizada com e-mails, status, convite e horário de quota.
    ...
    Depois de os alunos estarem inscritos num laboratório, os seus nomes serão atualizados com os primeiros e últimos nomes do diretório do MPS.
...
Uma vez adicionados aos utilizadores ao laboratório, terão de se registar para aceder aos VMs. Isto pode ser feito convidando utilizadores do portal Azure Web Services, que enviará um e-mail contendo o link de registo para o laboratório. Ou copiando e colando o link de inscrição num e-mail, ou outra forma de comunicação com os alunos.
...
Depois de ter convidado os utilizadores através do portal Azure ou partilhado o link, poderá monitorizar quais os utilizadores que se registaram com sucesso na página de **Utilizadores** na coluna **Status.** 
