---
title: Configure replicação transacional entre a Instância Gerida Azure SQL e o Servidor SQL
...
ms.locfileid: "84118695"
---
# <a name="tutorial-configure-transactional-replication-between-azure-sql-managed-instance-and-sql-server"></a>Tutorial: Configure replicação transacional entre a instância gerida pelo Azure SQL e o Servidor SQL
...
![Replicação entre um pub SQL MI, SQL MI Dist, e um sub SQL Server](./media/replication-two-instances-and-sql-server-configure-tutorial/sqlmi-to-sql-replication.png)
...
Para saber mais, consulte a visão geral da [Instância Gerida Azure SQL](sql-managed-instance-paas-overview.md)e os artigos de [Replicação Transacional SQL.](replication-transactional-overview.md)

Para configurar a replicação entre um editor de instância gerido e um assinante de instância gerido, consulte a [replicação transacional Configure entre duas instâncias geridas](replication-between-two-instances-configure-tutorial.md).
...
- O nome da instância gerida pelo distribuidor deve ser: `sql-mi-distributor` (juntamente com alguns caracteres para aleatoriedade) e deve estar na mesma rede virtual que _a instância gerida pelo editor._
   ...
   ![Utilize a vnet do editor para o distribuidor](./media/replication-two-instances-and-sql-server-configure-tutorial/use-same-vnet-for-distributor.png)

Para mais informações sobre a criação de um caso gerido, consulte [Criar uma instância gerida no portal](instance-create-quickstart.md)
...
Para obter mais informações sobre a implementação de um VM de servidor SQL para o Azure, consulte [Quickstart: Create SQL Server VM](../virtual-machines/windows/sql-vm-create-portal-quickstart.md).
...
1. Forneça um nome arbitrário para a sua zona DNS, tais como `repldns.com` .
...
1. Forneça o nome do seu VM do Servidor SQL, bem como o endereço IP interno privado.
   ...
   ![Configurar um disco](./media/replication-two-instances-and-sql-server-configure-tutorial/configure-a-record.png)
...
Uma vez estabelecida a conectividade e tiver uma base de dados de amostras, pode configurar a distribuição na sua `sql-mi-distributor` instância gerida. Para o fazer, siga estes passos:
...
1. Abra uma nova janela **de consulta** e execute o seguinte código Transact-SQL para configurar a distribuição na instância gerida pelo distribuidor:
   ...
   > [!NOTE]
   > Certifique-se de que utiliza apenas as pestanas traseiras `\` para o @working_directory parâmetro. A utilização de um corte dianteiro `/` () pode causar um erro ao ligar-se à parte do ficheiro.
...
1. Na página Base de Dados de **Publicação,** selecione a base de `ReplTutorial` dados que criou anteriormente. Selecione **Seguinte**.
...
1. Na página de Segurança do **Agente,** selecione Definições de **Segurança..**. Forneça credenciais de login do SQL Server para utilizar para o agente Snapshot e para se ligar ao Editor. Selecione **OK** para fechar a página de Segurança do **Agente Instantâneo.** Selecione **Seguinte**.
   ...
   ![Configurar a segurança do Agente Snapshot](./media/replication-two-instances-and-sql-server-configure-tutorial/snapshot-agent-security.png)
...
A utilização de um corte para a frente em vez de barrar no caminho do ficheiro para a parte do ficheiro pode causar este erro.
Isto está bem: `\\replstorage.file.core.windows.net\replshare` isto pode causar um erro de OS 55:`'\\replstorage.file.core.windows.net/replshare'`
...
Consulte o seguinte artigo de funcionalidades de segurança de funcionalidades de segurança de recursos de instância [geridas](sql-managed-instance-paas-overview.md#security-features) para uma lista completa de formas de proteger a sua base de dados. São discutidos os seguintes recursos de segurança:
