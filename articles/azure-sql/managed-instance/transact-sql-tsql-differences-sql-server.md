---
title: Diferenças T-SQL entre o Servidor SQL & Instância Gerida Azure SQL
description: Este artigo discute as diferenças transact-SQL (T-SQL) entre uma Instância Gerida Azure SQL e o Servidor SQL.
...
ms.locfileid: "84116759"
---
# <a name="t-sql-differences-between-sql-server--azure-sql-managed-instance"></a>Diferenças T-SQL entre o Servidor SQL & Instância Gerida Azure SQL
[!INCLUDE[appliesto-sqlmi](../includes/appliesto-sqlmi.md)]

Este artigo resume e explica as diferenças de sintaxe e comportamento entre o Azure SQL Managed Instance e o SQL Server. 


A SQL Managed Instance proporciona uma alta compatibilidade com o motor de base de dados do Servidor SQL no local, e a maioria das funcionalidades são suportadas numa Instância Gerida sQL.

![Migração](./media/transact-sql-tsql-differences-sql-server/migration.png)

Existem algumas limitações paaS que são introduzidas em SQL Managed Instance e algumas alterações de comportamento em comparação com o Servidor SQL. As diferenças dividem-se nas seguintes categorias:<a name="Differences"></a>
...
As questões conhecidas temporáriamente que são descobertas na Instância Gerida sQL e que serão resolvidas no futuro são descritas na página de notas de [lançamento](../database/doc-changes-updates-release-notes.md).

## <a name="availability"></a>Disponibilidade

### <a name="always-on-availability-groups"></a><a name="always-on-availability-groups"></a>Sempre em Grupos de Disponibilidade

[A alta disponibilidade](../database/high-availability-sla.md) é incorporada na Instância Gerida sQL e não pode ser controlada pelos utilizadores. As seguintes declarações não são apoiadas:
...
As Instâncias Geridas sQL têm cópias de segurança automáticas, para que os utilizadores possam criar cópias de `COPY_ONLY` dados completas. As cópias de segurança diferenciais, de registo e de instantâneo sem suporte não são suportadas.

- Com uma Instância Gerida SQL, pode fazer o back up a uma base de dados de instância apenas para uma conta de armazenamento Azure Blob:
...
- Com uma Instância Gerida SQL, pode fazer o backup de uma base de dados de instância com até 32 riscas, o que é suficiente para bases de dados até 4 TB se for utilizada compressão de backup.
- Não é possível executar numa base de dados encriptada com encriptação transparente de `BACKUP DATABASE ... WITH COPY_ONLY` dados (TDE) gerida pelo serviço. O TDE gerido pelo serviço obriga as cópias de segurança a serem encriptadas com uma chave TDE interna. A chave não pode ser exportada, por isso não podes restaurar a reserva. Utilize cópias de segurança automáticas e restauro pontual, ou utilize [tDE (BYOK) gerido pelo cliente.](../database/transparent-data-encryption-tde-overview.md#customer-managed-transparent-data-encryption---bring-your-own-key) Também pode desativar a encriptação na base de dados.
- O tamanho máximo da risca de reserva utilizando o `BACKUP` comando numa Instância Gerida SQL é de 195 GB, que é o tamanho máximo da bolha. Aumente o número de listras no comando de reserva para reduzir o tamanho das listras individuais e permaneça dentro deste limite.
    ...
    > - Restaurar na Instância Gerida SQL.
    >
    > O `Restore` comando numa Instância Gerida SQL suporta tamanhos de bolha maiores nos ficheiros de backup porque um tipo de blob diferente é usado para armazenamento dos ficheiros de backup carregados.
...
- Com a SQL Managed Instance, a auditoria funciona ao nível do servidor. Os `.xel` ficheiros de registo são armazenados no armazenamento da Blob Azure.
- Com a Base de Dados Azure SQL, a auditoria funciona ao nível da base de dados. Os `.xel` ficheiros de registo são armazenados no armazenamento da Blob Azure.
...
A auditoria do XEvent em SQL Managed Instance suporta os alvos de armazenamento da Blob Azure. Os registos de ficheiros e janelas não são suportados.
...
- A sintaxe `TO FILE` não é suportada porque uma Instância Gerida sQL não consegue aceder a partilhas de ficheiros do Windows.
...
Uma Instância Gerida por SQL não pode aceder a partilhas de ficheiros e pastas windows, pelo que se aplicam os seguintes constrangimentos:
...
Uma Instância Gerida Por SQL não consegue aceder a ficheiros, por isso os fornecedores criptográficos não podem ser criados:
...
- Os diretores de servidores azure Ative Directory (Azure AD) (logins) criados com a sintaxe [CREATE LOGIN](/sql/t-sql/statements/create-login-transact-sql?view=azuresqldb-mi-current) ou o [CREATE USER FROM LOGIN [Azure AD Login]](/sql/t-sql/statements/create-user-transact-sql?view=azuresqldb-mi-current) são suportados. Estes logins são criados ao nível do servidor.
    ...
    A SQL Managed Instance suporta os principais da base de dados Azure AD com a sintaxe `CREATE USER [AADUser/AAD group] FROM EXTERNAL PROVIDER` . Esta funcionalidade também é conhecida como Azure AD continha utilizadores de base de dados.
...
- O utilizador da AD Azure que criou a instância tem [privilégios administrativos ilimitados.](../database/logins-create-manage.md)
- Os utilizadores de bases de dados ad azure não administradores podem ser criados através da `CREATE USER ... FROM EXTERNAL PROVIDER` sintaxe. Ver [CREATE USER ... Do FORNECEDOR EXTERNO.](../database/authentication-aad-configure.md#create-contained-users-mapped-to-azure-ad-identities)
- Os diretores de servidores Azure AD (logins) suportam as funcionalidades SQL dentro de uma instância gerida pela SQL. As funcionalidades que requerem interação transversal, independentemente de estarem dentro do mesmo inquilino da AD Azure ou de diferentes inquilinos, não são suportadas para utilizadores da AD Azure. Exemplos de tais características são:
...
- A exportação/importação de bases de dados utilizando ficheiros bacpac são suportadas para utilizadores de AD Azure em Instância Gerida SQL utilizando [sSMS V18.4 ou posteriores](/sql/ssms/download-sql-server-management-studio-ssms), ou [SQLPackage.exe](/sql/tools/sqlpackage-download).
  - As seguintes configurações são suportadas utilizando ficheiro bacpac de base de dados: 
    ...
    - Exportar uma base de dados da SQL Managed Instance e importar para a Base de Dados SQL dentro do mesmo domínio Azure AD. 
    - Exportar uma base de dados da Base de Dados SQL e importar para A Instância Gerida SQL dentro do mesmo domínio Azure AD.
    - Exportar uma base de dados da SQL Managed Instance e importar para o SQL Server (versão 2012 ou posterior).
...
- Apenas o login principal ao nível do servidor, que é criado pelo processo de provisionamento de instância gerida pelo SQL, membros das funções do servidor, tais como `securityadmin` `sysadmin` ou, ou outros logins com permissão ALTER ANY LOGIN ao nível do servidor, pode criar diretores de servidores Azure AD (logins) na base de dados principal da Instância Gerida SQL.
...
- O login Azure AD deve ser membro de um Anúncio Azure dentro do mesmo diretório que é usado para o Azure SQL Managed Instance.
...
- É permitido sobrepor-se aos diretores de servidores da AD Azure (logins) com uma conta de administração Azure AD. Os diretores de servidores Da Azure AD (logins) têm precedência sobre o administrador da AD Azure quando resolve o diretor e aplica permissões à Instância Gerida sQL.
- Durante a autenticação, aplica-se a seguinte sequência para resolver o princípio autenticador:
    ...
    3. Se a conta Azure AD for uma administração Azure AD configurada por portal especial para a Instância Gerida SQL, que não existe nas vistas do sistema SQL Managed Instance, aplique permissões fixas especiais do administrador Azure AD para AQL Managed Instance (modo legado).
...
- Um caminho de ficheiro não pode ser especificado na `ALTER DATABASE ADD FILE (FILENAME='path')` declaração t-SQL. Retire `FILENAME` do script porque uma Instância Gerida SQL coloca automaticamente os ficheiros. 
...
- Ativar e desativar o Agente de Servidor SQL não é atualmente suportado em Instância Gerida SQL. O SQL Agent está sempre em execução.
- As definições do Agente do Servidor SQL são lidas apenas. O procedimento `sp_set_agent_properties` não é suportado em Instância Gerida SQL. 
- Tarefas
  ...
  - As Instâncias Geridas sQL não podem aceder a recursos externos, por exemplo, partilhas de rede através de robocopia. 
...
Uma Instância Gerida por SQL não pode aceder a partilhas de ficheiros e pastas Windows, pelo que os ficheiros devem ser importados do armazenamento da Blob De Azure:
...
- `OPENROWSET`pode ser usado para ler dados de outras bases de dados únicas Azure SQL, Instâncias Geridas SQL ou instâncias do Servidor SQL. Outras fontes, como as bases de dados oracle ou os ficheiros Excel, não são suportadas.
...
As declarações dBCC não documentadas que são ativadas no Servidor SQL não são suportadas em Instâncias Geridas SQL.
...
- [DBCC CHECKDB](/sql/t-sql/database-console-commands/dbcc-checkdb-transact-sql) com opções REPAIR_ALLOW_DATA_LOSS, REPAIR_FAST e REPAIR_REBUILD não podem ser utilizados porque a base de dados não pode ser definida no `SINGLE_USER` modo - ver alter [diferenças de BASE DE DADOS](#alter-database-statement). A potencial corrupção na base de dados é tratada pela equipa de apoio do Azure. Contacte o suporte do Azure se houver alguma indicação de corrupção na base de dados.

### <a name="distributed-transactions"></a>Transações distribuídas

AMSDTC e [as transações elásticas](../database/elastic-transactions-overview.md) não são suportadas atualmente em Instâncias Geridas sQL.
...
Servidores ligados em Instâncias Geridas SQL suportam um número limitado de alvos:

- Os alvos suportados são instâncias SQL Managed Instance, SQL Database e SQL Server. 
...
- [A Replicação transacional](replication-transactional-overview.md) está disponível para pré-visualização pública em Instância Gerida SQL com alguns constrangimentos:
    - Todos os tipos de participantes de replicação (Editor, Distribuidor, Pull Subscriber e Push Subscriber) podem ser colocados em Instâncias Geridas sQL, mas o editor e o distribuidor devem estar na nuvem ou ambos no local.
    - As Instâncias Geridas sQL podem comunicar com as versões recentes do Servidor SQL. Consulte a matriz de [versões suportadas](replication-transactional-overview.md#supportability-matrix) para obter mais informações.
...
- [Replicação entre um editor SQL MI e um assinante SQL MI](replication-between-two-instances-configure-tutorial.md)
- [Replicação entre um editor SQL MI, distribuidor SQL MI e subscritor sQL Server](replication-two-instances-and-sql-server-configure-tutorial.md)
...
- Os backups das bases de dados corrompidas podem ser restaurados dependendo do tipo de corrupção, mas os backups automatizados não serão tomados até que a corrupção seja corrigida. Certifique-se de que executa `DBCC CHECKDB` a fonte SQL Managed Instance e utilize backup `WITH CHECKSUM` para evitar este problema.
- A restauração do ficheiro de uma base de `.BAK` dados que contenha qualquer limitação descrita neste documento (por exemplo, `FILESTREAM` ou `FILETABLE` objetos) não pode ser restaurada em Instância Gerida sQL.
...
- As cópias de segurança que contenham bases de dados superiores a 4 tb ou objetos OLTP em memória com o tamanho total maior do que o tamanho descrito nos [limites](resource-limits.md) de recursos não podem ser restauradas em instância Business Critical.
...
- `SERVERPROPERTY('EngineEdition')`devolve o valor 8. Esta propriedade identifica exclusivamente um Caso Gerido SQL. Ver [SERVERPROPERTY](/sql/t-sql/functions/serverproperty-transact-sql).
- `SERVERPROPERTY('InstanceName')`devoluções NULO porque o conceito de instância tal como existe para o SQL Server não se aplica a uma Instância Gerida SQL. Ver [SERVERPROPERTY ('Nome de instância')](/sql/t-sql/functions/serverproperty-transact-sql).
...
- `@@SERVICENAME`devoluções NULO porque o conceito de serviço tal como existe para o SQL Server não se aplica a uma Instância Gerida SQL. Ver [ @SERVICENAME @](/sql/t-sql/functions/servicename-transact-sql).
...
-  Não é possível colocar outros recursos (por exemplo, máquinas virtuais) na sub-rede onde implementou a sua Instância Gerida SQL. Implementar estes recursos usando uma subrede diferente.
- A subnet deve dispor de um número suficiente de [endereços IP](connectivity-architecture-overview.md#network-requirements)disponíveis . O mínimo é de 16, enquanto a recomendação é ter pelo menos 32 endereços IP na sub-rede.
- [Os pontos finais do serviço não podem ser associados à sub-rede da Instância Gerida SQL](connectivity-architecture-overview.md#network-requirements). Certifique-se de que a opção de pontos finais do serviço está desativada quando criar a rede virtual.
...
- Há algumas regras de [segurança que devem ser aplicadas na sub-rede](connectivity-architecture-overview.md#network-requirements).

### <a name="vnet"></a>VNET
- A VNet pode ser implantada utilizando o Modelo de Recurso - O Modelo Clássico para VNet não é suportado.
- Após a criação de uma Instância Gerida SQL, não é suportada a mudança da Instância Ou VNet gerida sQL para outro grupo de recursos ou subscrição.
- Alguns serviços como Ambientes de Serviço de Aplicações, aplicações lógicas e Instâncias Geridas SQL (usados para geo-replicação, replicação transacional ou através de servidores ligados) não podem aceder a Instâncias Geridas SQL em diferentes regiões se os seus VNets estiverem ligados através de [peering global.](../../virtual-network/virtual-networks-faq.md#what-are-the-constraints-related-to-global-vnet-peering-and-load-balancers) Pode ligar-se a estes recursos através do ExpressRoute ou VNet-to-VNet através de Gateways VNet.

### <a name="tempdb"></a>TEMPDB

O tamanho máximo do ficheiro não pode ser superior a `tempdb` 24 GB por núcleo num nível de Propósito Geral. O tamanho máximo num nível Business Critical é limitado pelo tamanho de `tempdb` armazenamento SQL Managed Instance. `Tempdb`o tamanho do ficheiro de registo está limitado a 120 GB no nível de Propósito Geral. Algumas consultas podem devolver um erro se precisarem de mais de 24 GB por núcleo `tempdb` ou se produzirem mais de 120 GB de dados de registo.
...
Uma instância gerida por SQL coloca informações verbosas em registos de erros. Há muitos eventos do sistema interno que estão registados no registo de erros. Utilize um procedimento personalizado para ler registos de erros que filtram algumas entradas irrelevantes. Para mais informações, consulte [a SQL Managed Instance – sp_readmierrorlog](https://blogs.msdn.microsoft.com/sqlcat/2018/05/04/azure-sql-db-managed-instance-sp_readmierrorlog/) ou a [extensão de instância gerida sQL (pré-visualização)](/sql/azure-data-studio/azure-sql-managed-instance-extension#logs) para o Azure Data Studio.

## <a name="next-steps"></a>Próximos passos

- Para mais informações sobre instâncias geridas pela SQL, consulte o que é a Instância Gerida pela [SQL?](sql-managed-instance-paas-overview.md)
- Para obter uma lista de funcionalidades e comparação, consulte a comparação de características do [Azure SQL Managed Instance](../database/features-comparison.md).
- Para atualizações de lançamento e estado de problemas conhecidos, consulte notas de [lançamento sQL Managed Instance](../database/doc-changes-updates-release-notes.md)
- Para um arranque rápido que lhe mostre como criar uma nova Instância Gerida SQL, consulte [Create a SQL Managed Instance](instance-create-quickstart.md).