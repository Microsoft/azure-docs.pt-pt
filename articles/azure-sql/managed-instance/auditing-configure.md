...
2. Crie um **recipiente** de armazenamento Azure onde os registos de auditoria são armazenados.
   ...
   1. Fornecer um nome **de** recipiente, definir **o nível de acesso público** ao **Privado,** e, em seguida, clicar **EM OK**.
    ...
    > [!IMPORTANT]
    > Os clientes que desejem configurar uma loja de registo imutável para os seus eventos de auditoria ao nível do servidor ou da base de dados devem seguir as [instruções fornecidas pelo Azure Storage](../../storage/blobs/storage-blob-immutability-policies-manage.md#enabling-allow-protected-append-blobs-writes). (Certifique-se de que selecionou **Permita apêndices adicionais** quando configurar o armazenamento de bolhas imutáveis.)
...
3. Depois de criar o contentor para os registos de auditoria, existem duas formas de o configurar como o alvo dos registos de auditoria: utilizar o [T-SQL](#blobtsql) ou [utilizar o SQL Server Management Studio (SSMS) UI](#blobssms):
   ...
   - <a id="blobssms"></a>**Configure o armazenamento de bolhas para registos de auditoria utilizando o SQL Server Management Studio 18:**
     ...
     1. <a id="createspec"></a>Depois de configurar o recipiente blob como alvo para os registos de auditoria, crie e ative uma especificação de auditoria do servidor ou especificação de auditoria de base de dados como faria para o SQL Server:

   - [Criar guia T-SQL de especificação de auditoria do servidor](/sql/t-sql/statements/create-server-audit-specification-transact-sql)
   - [Criar guia T-SQL de especificação de auditoria de base de dados](/sql/t-sql/statements/create-database-audit-specification-transact-sql)
...
- Utilize a função do sistema `sys.fn_get_audit_file` (T-SQL) para devolver os dados de registo de auditoria em formato tabular. Para obter mais informações sobre a utilização desta função, consulte a [documentação sys.fn_get_audit_file](/sql/relational-databases/system-functions/sys-fn-get-audit-file-transact-sql).

- Pode explorar registos de auditoria utilizando uma ferramenta como [o Azure Storage Explorer.](https://azure.microsoft.com/features/storage-explorer/) No Azure Storage, os registos de auditoria são guardados como uma coleção de ficheiros blob dentro de um recipiente que foi definido para armazenar os registos de auditoria. Para mais detalhes sobre a hierarquia da pasta de armazenamento, convenções de nomeação e formato de registo, consulte a Referência do [Formato do Registo de Auditoria blob](../database/audit-log-format.md).
