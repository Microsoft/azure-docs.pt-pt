...
1. No [portal Azure, crie](https://portal.azure.com/)uma base de dados Premium ou Business Critical num servidor. Delineie a **Fonte** para a base de dados da amostra AdventureWorksLT. Para obter instruções detalhadas, consulte Criar a sua primeira base de [dados Azure SQL](database/single-database-create-quickstart.md).
...
4. Colhe o script T-SQL em SSMS e, em seguida, execute o script. As declarações da `MEMORY_OPTIMIZED = ON` cláusula CREATE TABLE são cruciais. Por exemplo:
...
Você pode usar uma janela *RML Cmd Prompt* para executar a nossa linha de comando ostress.exe. Os parâmetros da linha de comando diretamente `ostress` para:
...
Os nossos testes In-Memory mostraram que o desempenho melhorou **nove vezes** para esta carga de trabalho simplista, com `ostress` uma execução num VM Azure na mesma região do Azure que a base de dados.
...
- [Saiba como o Quorum duplica a carga de trabalho da base de dados chave enquanto baixa o DTU em 70% com o OLTP in-memory na Base de Dados Azure SQL](https://customers.microsoft.com/story/quorum-doubles-key-databases-workload-while-lowering-dtu-with-sql-database)
