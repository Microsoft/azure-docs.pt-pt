...
Ao construir e implantar serviços em nuvem com base de dados Azure SQL, você usa [geo-replicação ativa](active-geo-replication-overview.md) ou [grupos de auto-falha](auto-failover-group-overview.md) para fornecer resiliência a interrupções regionais e falhas catastróficas. A mesma funcionalidade permite criar aplicações distribuídas globalmente otimizadas para o acesso local aos dados. Este artigo discute padrões comuns de aplicação, incluindo os benefícios e as compensações de cada opção.
...
Se ocorrer uma paragem na região B, o processo de replicação entre a base de dados primária e secundária é suspenso, mas a ligação entre os dois permanece intacta (1). O Gestor de Tráfego deteta que a conectividade com a Região B está quebrada e marca a aplicação web de ponto final 2 como Degradada (2). O desempenho da aplicação não é impactado neste caso, mas a base de dados fica exposta e, portanto, em maior risco de perda de dados caso a região A falhe sucessivamente.

> [!NOTE]
> Para a recuperação de desastres, recomendamos a configuração com implantação de aplicações limitada a duas regiões. Isto porque a maioria das geografias azure têm apenas duas regiões. Esta configuração não protege a sua aplicação de uma falha catastrófica simultânea de ambas as regiões. Num caso improvável de tal falha, pode recuperar as suas bases de dados numa terceira região utilizando a [operação de georestauro](disaster-recovery-guidance.md#recover-using-geo-restore).
>
...
![Cenário 3. Configuração com primária nos EUA Orientais.](./media/designing-cloud-solutions-for-disaster-recovery/scenario3-a.png)
...
![Cenário 3. Transição primária para o Norte da Europa.](./media/designing-cloud-solutions-for-disaster-recovery/scenario3-b.png)
...
* Para aprender sobre geo-replicação ativa, consulte [a geo-replicação ativa.](active-geo-replication-overview.md)
* Para conhecer os grupos de auto-failover, consulte [os grupos de falha automática](auto-failover-group-overview.md).
* Para obter informações sobre geo-replicação ativa com piscinas elásticas, consulte estratégias de recuperação de desastres de [piscina elástica.](disaster-recovery-strategies-for-applications-with-elastic-pool.md)
