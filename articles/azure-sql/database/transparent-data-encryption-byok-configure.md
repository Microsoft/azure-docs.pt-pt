...
description: Saiba como configurar uma Base de Dados Azure SQL e a Azure Synapse Analytics para começar a usar encriptação transparente de dados (TDE) para encriptação em repouso usando PowerShell ou CLI.
...
Este artigo passa por como usar uma chave do Cofre chave Azure para encriptação transparente de dados (TDE) na Base de Dados Azure SQL ou Azure Synapse Analytics (anteriormente SQL DW). Para saber mais sobre o TDE com integração do Cofre de Chaves Azure - Traga o suporte da sua própria chave (BYOK), visite [o TDE com chaves geridas pelo cliente no Cofre de Chaves Azure](transparent-data-encryption-byok-overview.md).
...
Se tiver um [servidor](logical-servers.md)existente, utilize o seguinte para adicionar uma identidade Azure AD ao seu servidor:
