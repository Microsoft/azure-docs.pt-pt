---
title: Importar ou exportar uma Base de Dados Azure SQL sem permitir que os serviços do Azure acedam ao servidor.
...
Este artigo mostra-lhe como importar ou exportar uma Base de Dados Azure SQL quando a *Allow Azure Services* está definida para *off* no servidor. O fluxo de trabalho utiliza uma máquina virtual Azure para executar o SqlPackage para realizar a operação de importação ou exportação.
...
3. Selecione **Definir a firewall** do servidor na barra de ferramentas. A página de **definições de Firewall** para o servidor abre.
   ...
   ![regra de firewall IP de nível servidor](./media/database-import-export-azure-services-off/server-firewall-rule.png)
...
5. Selecione **Guardar**. Uma regra de firewall IP de nível de servidor é criada para a porta de abertura de endereço IP da sua máquina virtual 1433 no servidor.
...
> [!IMPORTANT]
> Para ligar à base de dados TAzure SQL por trás de uma firewall corporativa, a firewall deve ter a porta 1433 aberta.
...
- Para uma discussão de todo o processo de migração da base de dados SQL Server, incluindo recomendações de desempenho, consulte a migração da base de [dados Do Servidor SQL para a Base de Dados Azure SQL](migrate-to-database-from-sql-server.md).
