...
ms.locfileid: "84045397"
---
# <a name="tutorial-configure-active-geo-replication-and-failover-in-the-azure-portal-azure-sql-database"></a>Tutorial: Configure geo-replicação ativa e failover no portal Azure (Base de Dados Azure SQL)
...
Para obter as melhores práticas utilizando grupos de falha automática, consulte as melhores práticas para a Base de [Dados Azure SQL](auto-failover-group-overview.md#best-practices-for-sql-database) e [as melhores práticas para a Instância Gerida Azure SQL](auto-failover-group-overview.md#best-practices-for-sql-managed-instance). 
...
* Uma base de dados em Azure SQL Database: A base de dados primária que pretende replicar para uma região geográfica diferente.
...
2. Na página base de dados SQL, **selecione geo-replicação**e, em seguida, selecione a região para criar a base de dados secundária. Pode selecionar qualquer outra região que não a região que acolhe a base de dados primária, mas recomendamos a [região emparelhada.](../../best-practices-availability-paired-regions.md)
...
7. Quando o processo de sementeira estiver concluído, a base de dados secundária mostra o seu estado.
    ...
    ![Sementeing completo](./media/active-geo-replication-configure-portal/seeding-complete.png)
...
3. Na lista **SECONDARIES,** selecione a base de dados que pretende tornar-se a nova primária e clique em **Failover Forçado**.
...
* Para aprender sobre grupos de auto-failover, consulte [grupos de falha automática](auto-failover-group-overview.md)
