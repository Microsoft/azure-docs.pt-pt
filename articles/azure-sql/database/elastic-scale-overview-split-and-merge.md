...
1. [Tutorial de ferramenta seleto de ferramenta de fusão de base de dados](elastic-scale-configure-deploy-split-and-merge.md)
2. [Configuração de segurança de fusão split](elastic-scale-split-merge-security-configuration.md)
...
- **Flexibilidade**
  ...
  As aplicações precisam de se esticar de forma flexível para além dos limites de uma única base de dados na Base de Dados Azure SQL. Utilize a ferramenta para mover dados conforme necessário para novas bases de dados, mantendo a integridade.
...
- **Sharding-awareness**
  ...
  - **Outras tabelas**
    ...
    A informação sobre as tabelas de referência vs. sharded é fornecida pelas `SchemaInfo` APIs no mapa do fragmento. O exemplo que se segue ilustra a utilização destas APIs num determinado objeto de gestor de mapas de fragmentos:
...
- **Fragmento de alvo (dividido)**
  ...
  Uma vez que tenha fornecido as informações sobre a origem da sua operação dividida, tem de definir onde pretende que os dados sejam copiados, fornecendo o nome do servidor e da base de dados para o alvo.
...
1. Siga os passos em [Implementar um serviço de fusão split.](elastic-scale-configure-deploy-split-and-merge.md)
