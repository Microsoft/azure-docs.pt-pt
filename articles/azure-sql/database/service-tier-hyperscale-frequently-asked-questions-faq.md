...
- Este FAQ não pretende ser um guia ou responder a perguntas sobre como usar uma base de dados de Hiperescala. Para uma introdução à Hyperscale, recomendamos que consulte a documentação de hiperescala de base de [dados Azure SQL.](service-tier-hyperscale.md)

## <a name="general-questions"></a>Perguntas gerais

### <a name="what-is-a-hyperscale-database"></a>O que é uma base de dados de hiperescala

Uma base de dados de hiperescala é uma base de dados na Base de Dados SQL no nível de serviço de hiperescala que é apoiado pela tecnologia de armazenamento de escala hiperescala. Uma base de dados de hiperescala suporta até 100 TB de dados e fornece alta capacidade de entrada e desempenho, bem como uma escala rápida para se adaptar aos requisitos de carga de trabalho. A escala é transparente para a aplicação – conectividade, processamento de consultas, etc. funciona como qualquer outra base de dados na Base de Dados Azure SQL.
...
O nível de serviço Hyperscale está atualmente disponível nas regiões listadas no âmbito da visão geral de hiperescala de base de [dados Azure SQL](service-tier-hyperscale.md#regions).
...
Sim. Para obter mais informações e limites sobre o número de bases de dados de Hiperescala por servidor, consulte os limites de recursos de base de [dados SQL para bases de dados individuais e agrofadas num servidor](resource-limits-logical-server.md).
...
- **Escalando dentro e fora**
  ...
  O fornecimento de cada uma destas réplicas de cálculo adicionais pode ser feito em tempo constante e é uma operação online. Pode ligar-se a estas réplicas de cálculo adicionais apenas de leitura, definindo o argumento na sua corda de `ApplicationIntent` ligação a `ReadOnly` . Quaisquer ligações com a intenção de `ReadOnly` aplicação são automaticamente encaminhadas para uma das réplicas de cálculo adicionais apenas de leitura.

## <a name="deep-dive-questions"></a>Questões de mergulho profundo

### <a name="can-i-mix-hyperscale-and-single-databases-in-a-single-server"></a>Posso misturar hiperescala e bases de dados individuais num único servidor
...
Não. Para obter mais RAM, você precisa atualizar para um tamanho de computação mais alto. Para mais informações, consulte o armazenamento em hiperescala e os [tamanhos da computação.](resource-limits-vcore-single-databases.md#hyperscale---provisioned-compute---gen5)
...
## <a name="data-size-and-storage-questions"></a>Tamanho dos dados e questões de armazenamento
...
### <a name="does-my-tempdb-scale-as-my-database-grows"></a>Será que a minha escala à medida que a `tempdb` minha base de dados cresce

A sua `tempdb` base de dados está localizada no armazenamento local de SSD e é dimensionada proporcionalmente ao tamanho da computação que você disponibiliza. O seu `tempdb` está otimizado para proporcionar o máximo de benefícios de desempenho. `tempdb`o tamanho não é configurável e é gerido para si.
...
Sim. Pode mover as bases de dados existentes na Base de Dados Azure SQL para Hyperscale. Esta é uma migração de sentido único. Não se pode mover bases de dados de Hiperscale para outro nível de serviço. Para provas de conceito (POCs), recomendamos que faça uma cópia da sua base de dados e emigra a cópia para hyperscale.
...
A Hiperscale é capaz de consumir 100 MB/s de dados novos/alterados, mas o tempo necessário para mover dados para bases de dados na Base de Dados Azure SQL também é afetado pela produção disponível da rede, pela velocidade de leitura de fontes e pelo objetivo do nível de serviço de base de dados alvo.
...
SQL Server 2005. Para mais informações, consulte [Migrate para uma única base de dados ou uma base de dados agrupina](migrate-to-database-from-sql-server.md#migrate-to-a-single-database-or-a-pooled-database). Para questões de compatibilidade, consulte problemas de compatibilidade com a migração da base de [dados Resolvendo.](migrate-to-database-from-sql-server.md#resolving-database-migration-compatibility-issues)
...
### <a name="are-the-database-backups-managed-for-me-by-azure-sql-database"></a>São as cópias de dados geridas para mim pela Azure SQL Database
...
### <a name="does-hyperscale-support-point-in-time-restore"></a>O suporte de hiperescala restaura o ponto no tempo
...
Sim. Algumas funcionalidades de Base de Dados Azure SQL não são suportadas em Hiperescala, incluindo, mas não se limitando à retenção de backup a longo prazo. Depois de migraras bases de dados para Hyperscale, essas funcionalidades deixam de funcionar.
...
Não. Os processos de hiperescala funcionam em nódos de tecido de [serviço](https://azure.microsoft.com/services/service-fabric/) (VMs), não em recipientes.
...
### <a name="does-the-size-of-my-tempdb-database-also-grow-as-the-compute-is-scaled-up"></a>O tamanho da minha base de dados também cresce à medida que `tempdb` a computação é dimensionada

Sim. A base de dados aumentará automaticamente à medida que `tempdb` o cálculo cresce.  
...
## <a name="read-scale-out-questions"></a>Ler perguntas de escala
...
Pode ligar-se a estas réplicas de cálculo adicionais apenas de leitura, definindo o argumento na sua corda de `ApplicationIntent` ligação a `ReadOnly` . Quaisquer ligações marcadas são `ReadOnly` automaticamente encaminhadas para uma das réplicas de cálculo adicionais.  
...
O resultado é se estiver ligado a uma réplica secundária apenas de `READ_ONLY` leitura, e se estiver ligado à réplica `READ_WRITE` primária. Note que o contexto da base de dados deve ser definido para o nome da base de dados Hyperscale, não para a base de `master` dados.
...
### <a name="do-i-get-different-tempdb-sizing-for-my-primary-compute-and-my-additional-secondary-compute-replicas"></a>Tenho `tempdb` tamanhos diferentes para a minha computação primária e as minhas réplicas adicionais de cálculo secundário

Não. A sua base de dados está configurada com base no fornecimento de tamanho de cálculo, as suas réplicas de cálculo secundário têm o mesmo tamanho que `tempdb` a computação primária.
