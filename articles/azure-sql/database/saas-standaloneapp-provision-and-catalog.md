...
* As três aplicações de inquilinos de amostra estão implantadas. Para implementar estas aplicações em menos de cinco minutos, consulte Implementar e explorar o padrão de aplicação autónoma de [bilhetes SaaS](../../sql-database/saas-standaloneapp-get-started-deploy.md).
...
1. No PowerShell ISE, abra *...\Learning Modules\UserConfig.psm* e atualize o **\<user\>** valor para o valor utilizado ao implementar as três aplicações de amostra.  **Guarde o ficheiro.**
...
1. Adicione um ponto de rutura colocando o cursor em qualquer lugar da linha que diz, `& $PSScriptRoot\New-Catalog.ps1` e, em seguida, pressione **F9**.
...
1. Abra o [portal Azure](https://portal.azure.com/) e navegue pelos grupos de recursos.  Abra o grupo **de \<user\> ** recursos de catálogo de ponta-alas e note o servidor de catálogo e base de dados.
...
1. Explore o esquema da base de dados do catálogo de *inquilinos.*
   * Os objetos no esquema são todos fornecidos pela Biblioteca de Clientes da Base de `__ShardManagement` Dados Elástica.
...
1. Adicione um ponto de rutura no script colocando o cursor em qualquer lugar na linha 49 que diz, `& $PSScriptRoot\New-TenantApp.ps1` e depois prima **F9**.
1. Executar o guião premindo **F5**.
1.  Depois de a execução do guião parar no ponto de rutura, prima **F11** para entrar no script New-Catalog.ps1.
1.  Trace a execução do script usando as opções do menu Debug, F10 e F11, para passar ou entrar em funções chamadas.
...
Você pode explorar como o catálogo é usado para apoiar vários cenários de cross-tenant usando a versão base de dados/inquilino da [aplicação Wingtip Tickets SaaS](../../sql-database/saas-dbpertenant-wingtip-app-overview.md).
