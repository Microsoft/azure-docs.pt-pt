...
* Anotações que mostram recomendações de desempenho de [consultores](database-advisor-implement-performance-recommendations.md) de base de dados

![Query Performance Insight](./media/query-performance-insight-use/opening-title.png)

## <a name="prerequisites"></a>Pré-requisitos

Consulta Performance Insight requer que a [Consulta Store](https://msdn.microsoft.com/library/dn817826.aspx) esteja ativa na sua base de dados. Está automaticamente ativado para todas as bases de dados em Bases de Dados Azure SQL por padrão. Se a Loja de Consultas não estiver a funcionar, o portal Azure irá pedir-lhe para o ativar.
...
2. A partir do menu do lado esquerdo, abra a Visão de **Desempenho Inteligente**de Performance de Desempenho  >  **Query Performance Insight**.
...
5. Abra recomendações **de desempenho inteligente**e verifique se  >  **Performance recommendations** existem recomendações de desempenho disponíveis. Para obter mais informações sobre recomendações de desempenho incorporadas, consulte O Consultor de Bases de [Dados Azure SQL](database-advisor-implement-performance-recommendations.md).
6. Utilize sliders ou ícones de zoom para alterar o intervalo observado.

   ![Painel de instrumentos de desempenho](./media/query-performance-insight-use/performance.png)

> [!NOTE]
> Para que a Azure SQL Database forneça a informação no Query Performance Insight, a Consulta Store precisa de capturar algumas horas de dados. Se a base de dados não tiver atividade ou se a Consulta Store não estiver ativa durante um determinado período, os gráficos ficarão vazios quando a Consulta Performance Insight apresentar essa gama de tempo. Pode ativar a Consulta Store a qualquer momento se não estiver a funcionar. Para mais informações, consulte [as melhores práticas com a Consulta Store](https://docs.microsoft.com/sql/relational-databases/performance/best-practice-with-the-query-store).
>

Para recomendações de desempenho na base de dados, selecione [Recomendações](database-advisor-implement-performance-recommendations.md) na lâmina de navegação De query Performance Insight.
...
1. Selecione ou limpe consultas individuais para incluí-las ou excluí-las do gráfico utilizando caixas de verificação.
   ...
   >
   > 1. No portal Azure, selecione Monitorização de Base de **Dados Azure SQL**  >  **Monitoring**.
...
3. Utilize sliders e botões de zoom para alterar o intervalo de observação e investigar picos de consumo:
   ...
   ![Deslizadores e botões de zoom para alterar o intervalo](./media/query-performance-insight-use/zoom.png)
...
5. Selecione o botão **Go >** para ver a vista personalizada.
   ...
   >
   > 1. Selecione Monitorização da Base de **Dados Azure SQL**  >  **Monitoring**.
   > 2. Selecione **Métricas**.
   > 3. Selecione **+Adicionar gráfico**.
   > 4. Selecione a percentagem de DTU na tabela.
   > 5. Além disso, selecione **Last 24 horas** no menu superior esquerdo e mude-o para um minuto.
...
Por exemplo, muitos sites baseados em dados acedem fortemente à base de dados para cada pedido do utilizador. Embora o agrupamento de ligações ajude, o aumento do tráfego de rede e a carga de processamento no servidor podem abrandar o desempenho. Em geral, mantenha as viagens de ida e volta ao mínimo.
...
4. Selecione o botão **Go >** para ver a vista personalizada.
   ...
   ![Contagem de execução de consulta](./media/query-performance-insight-use/top-execution.png)
...
Estes ícones são anotações. Mostram recomendações de desempenho do Consultor de Base de [Dados Azure SQL.](database-advisor-implement-performance-recommendations.md) Ao pairar sobre uma anotação, você pode obter informações resumidas sobre recomendações de desempenho.
...
Se quiser compreender mais ou aplicar a recomendação do consultor, selecione o ícone para abrir detalhes da ação recomendada. Se esta for uma recomendação ativa, pode aplicá-la imediatamente ao portal.
   ...
   ![Detalhes de anotação de consulta](./media/query-performance-insight-use/annotation-details.png)
