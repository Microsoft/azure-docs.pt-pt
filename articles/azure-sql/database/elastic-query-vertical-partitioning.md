...
> [!NOTE]
> Certifique-se de que o `<username>` sufixo "nome de servidor" não inclui qualquer sufixo de ** \@ "nome de servidor".**

## <a name="create-external-data-sources"></a>Criar fontes de dados externas

Sintaxe:

<External_Data_Source> ::= CRIAR DADOS EXTERNOS <DATA_SOURCE_NAME> COM (TIPO = RDBMS, LOCALIZAÇÃO = '<fully_qualified_server_name>', DATABASE_NAME = '<remote_database_name>',  
...
CRIAR MESA EXTERNA [ database_name . [schema_name] . | schema_name. ] table_name  
...
* **Referência à base**de dados remota : A tabela externa DDL refere-se a uma fonte de dados externa. A fonte de dados externa especifica o nome do servidor e o nome da base de dados da base de dados remota onde os dados reais da tabela são armazenados.
...
O sp \_ executar remote utiliza a fonte de dados externa fornecida nos parâmetros de \_ invocação para executar a declaração T-SQL dada na base de dados remota. Utiliza a credencial da fonte de dados externa para se ligar à base de dados remota.  
