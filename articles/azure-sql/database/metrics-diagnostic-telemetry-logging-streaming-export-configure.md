...
description: Saiba como configurar a exportação de métricas e registos de recursos em streaming, incluindo análises de diagnóstico inteligentes da Azure SQL Database e Azure SQL Managed Instance para o destino da sua escolha de armazenar informações sobre a utilização de recursos e estatísticas de execução de consultas.
...
ms.locfileid: "84117757"
---
# <a name="configure-streaming-export-of-azure-sql-database-and-sql-managed-instance-diagnostic-telemetry"></a>Configure a exportação de streaming da Base de Dados Azure SQL e da telemetria de diagnóstico sQL Managed Instance
...
O mais importante entre a telemetria de diagnóstico que pode exportar é o log Intelligent Insights (SQLInsights). [A Intelligent Insights](intelligent-insights-overview.md) usa inteligência incorporada para monitorizar continuamente o uso da base de dados através da inteligência artificial e detetar eventos disruptivos que causam um fraco desempenho. Uma vez detetado, é realizada uma análise detalhada que gera um registo de Insights Inteligentes com uma avaliação inteligente do problema. Esta avaliação consiste numa análise de causas fundamentais do problema de desempenho da base de dados e, sempre que possível, recomendações para melhorias de desempenho. É necessário configurar a exportação de streaming deste registo para visualizar o seu conteúdo.

Além de transmitir a exportação do registo Smart Insights, também pode exportar uma variedade de métricas de desempenho e registos de bases de dados adicionais. A tabela seguinte descreve as métricas de desempenho e os registos de recursos que pode configurar para o streaming de exportação para um dos vários destinos. Esta telemetria de diagnóstico pode ser configurada para bases de dados individuais, piscinas elásticas e bases de dados reunidas, e instâncias geridas e bases de dados de instâncias.

| Telemetria de diagnóstico para bases de dados | Suporte de base de dados Azure SQL | Suporte de Instância Gerida do SQL do Azure |
| :------------------- | ----- | ----- |
...
| [SQLInsights](#intelligent-insights-dataset): Contém Insights Inteligentes no desempenho de uma base de dados. Para saber mais, consulte [Insights Inteligentes.](intelligent-insights-overview.md) | Sim | Sim |
...
| :------------------- | ------------------- |
| **Conjunto elástico** | [As métricas básicas](metrics-diagnostic-telemetry-logging-streaming-export-configure.md#basic-metrics) contêm percentagem eDTU/CPU, limite eDTU/CPU, percentagem de leitura de dados físicos, percentagem de escrita de registo, percentagem de sessões, percentagem de trabalhadores, armazenamento, percentagem de armazenamento, limite de armazenamento e percentagem de armazenamento XTP. |
...
3. Selecione **ligar os diagnósticos** se não existirem definições anteriores ou selecione **a definição editar** para editar uma definição anterior.
   ...
   ![Ativar diagnósticos para piscinas elásticas](./media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-container-elasticpool-enable.png)
...
7. Selecione a caixa de verificação para telemetria de diagnóstico de piscina elástica: Métricas **básicas.**
   ![Configure diagnósticos para piscinas elásticas](./media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-container-elasticpool-selection.png)
...
### <a name="single-and-pooled-databases-in-azure-sql-database"></a>Bases de dados únicas e agréns na Base de Dados Azure SQL

Pode configurar um recurso de base de dados único ou agrupado para recolher a seguinte telemetria de diagnóstico:

| Recurso | Telemetria de monitorização |
| :------------------- | ------------------- |
| **Base de dados única ou agréu** | [As métricas básicas](metrics-diagnostic-telemetry-logging-streaming-export-configure.md#basic-metrics) contêm percentagem de DTU, DTU usado, limite dTU, percentagem de CPU, percentagem de leitura de dados físicos, percentagem de log write, sucesso/falhado/bloqueado por ligações de firewall, percentagem de sessões, percentagem de trabalhadores, armazenamento, percentagem de armazenamento, percentagem de armazenamento XTP e impasses. |
...
4. **Selecione Adicionar definição de diagnóstico** para configurar o streaming paralelo de dados de diagnóstico para vários recursos.
   ...
   ![Ativar diagnósticos para bases de dados individuais e reunidas](./media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-database-sql-enable.png)
...
8. Para uma experiência avançada de monitorização baseada em um minuto, selecione a caixa de verificação para métricas **Básicas.**
   ...
   ![Configure diagnósticos para base de dados Azure SQL](./media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-database-sql-selection.png)
9. Selecione **Guardar**.
...
### <a name="instances-in-azure-sql-managed-instance"></a>Casos em Caso Gerido Azure SQL
...
3. Selecione **ligar os diagnósticos** se não existirem definições anteriores ou selecione **a definição editar** para editar uma definição anterior.
   ...
   ![Ativar diagnósticos para instância gerida](./media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-container-mi-enable.png)
...
### <a name="databases-in-azure-sql-managed-instance"></a>Bases de dados em Instância Gerida Azure SQL
...
3. Selecione **ligar os diagnósticos** se não existirem definições anteriores ou selecione **a definição editar** para editar uma definição anterior.
   ...
   ![Ativar diagnósticos, por exemplo, bases de dados](./media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-database-mi-enable.png)
...
6. Selecione as caixas de verificação para telemetria de diagnóstico de base de dados: **SQLInsights,** **QueryStoreRuntimeStatistics**, **QueryStoreWaitStatistics**, e **Errors**.
   ![Configurar diagnósticos, por exemplo, bases de dados](./media/metrics-diagnostic-telemetry-logging-streaming-export-configure/diagnostics-settings-database-mi-selection.png)
7. Selecione **Guardar**.
...
> [!IMPORTANT]
> O módulo PowerShell Azure Resource Manager ainda é suportado pelo Azure, mas todo o desenvolvimento futuro é para o módulo Az.Sql. Para estes cmdlets, consulte [AzureRM.Sql](https://docs.microsoft.com/powershell/module/AzureRM.Sql/). Os argumentos para os comandos no módulo Az e nos módulos AzureRm são substancialmente idênticos.
...
Forneça o ID de recursos do espaço de trabalho \<$WSID\> como parâmetro ao executar o script para enviar dados de diagnóstico de `Enable-AzureRMDiagnostics.ps1` vários recursos para o espaço de trabalho.

- Para obter a identificação do espaço de trabalho \<$WSID\> do destino para os seus dados de diagnóstico, utilize o seguinte script:
  ...
  \<subID\>Substitua-a pelo ID de subscrição, com o nome do grupo de recursos e com o nome do espaço de \<RG_NAME\> \<WS_NAME\> trabalho.
...
As métricas de base de dados Azure SQL e Azure SQL Managed Instance e os registos de recursos que são transmitidos para um espaço de trabalho de Log Analytics podem ser consumidos pela Azure SQL Analytics. O Azure SQL Analytics é uma solução em nuvem que monitoriza o desempenho de bases de dados individuais, piscinas elásticas e bases de dados reunidas, e casos geridos e bases de dados de instâncias em escala e em várias subscrições. Pode ajudá-lo a recolher e visualizar métricas de desempenho, e tem inteligência incorporada para resolução de problemas de desempenho.

![Visão geral da Análise Azure SQL](./media/metrics-diagnostic-telemetry-logging-streaming-export-configure/azure-sql-sol-overview.png)

### <a name="installation-overview"></a>Descrição geral da instalação

Pode monitorizar uma recolha de bases de dados e coleções de bases de dados com a Azure SQL Analytics executando os seguintes passos:
...
1. Procure o Azure SQL Analytics no Azure Marketplace e selecione-o.
   ...
   ![Pesquisa por Azure SQL Analytics no portal](./media/metrics-diagnostic-telemetry-logging-streaming-export-configure/sql-analytics-in-marketplace.png)
...
Pode utilizar o SQL Analytics como painel hierárquico para visualizar os recursos da sua base de dados.

- Para aprender a utilizar o Azure SQL Analytics, consulte monitor [utilizando o SQL Analytics](../../azure-monitor/insights/azure-sql.md).
...
|---|---|---|
|sqlserver_process_core_percent<sup>1</sup>|Por cento do núcleo do processo SQL|Percentagem de utilização de CPU para o processo SQL, medida pelo sistema operativo.|
|<sup>sqlserver_process_memory_percent 1</sup> |Por cento da memória do processo SQL|Percentagem de utilização da memória para o processo SQL, medida pelo sistema operativo.|
...
Os detalhes da telemetria disponíveis para todos os registos estão documentados nas tabelas seguintes. Para mais informações, consulte [a telemetria de diagnóstico suportada.](#diagnostic-telemetry-for-export)
...
|SKU_s|Produto SKU de instância gerida sql |
...
Saiba mais sobre o formato de [log Insights Inteligentes.](intelligent-insights-use-diagnostics-log.md)
...
- [Criação de alertas para a Base de Dados Azure SQL e para a Instância Gerida Azure SQL](../../azure-monitor/insights/azure-sql.md#analyze-data-and-create-alerts)
