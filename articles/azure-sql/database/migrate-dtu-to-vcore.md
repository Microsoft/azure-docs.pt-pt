...
description: Migrar uma base de dados em Azure SQL Database do modelo DTU para o modelo vCore. Migrar para vCore é semelhante à atualização ou degradação entre os níveis padrão e premium.
...
ms.locfileid: "84051326"
---
# <a name="migrate-azure-sql-database-from-the-dtu-based-model-to-the-vcore-based-model"></a>Migrar base de dados Migrate Azure SQL do modelo baseado em DTU para o modelo baseado em vCore
...
A migração de uma base de dados na Base de Dados Azure SQL do modelo de compra baseado em DTU para o modelo de compra baseado em vCore é semelhante à atualização ou degradação entre os níveis de serviço padrão e premium no modelo de compra baseado em DTU.
...
Se estiver a criar um geo-secundário na piscina elástica para uma única base de dados primária, certifique-se de que a configuração da piscina corresponde ao tamanho da base de `maxVCore` dados primária. Se você está criando um geo-secundário para uma primária em outra piscina elástica, recomendamos que as piscinas tenham as mesmas `maxVCore` configurações.
...
- Para obter os tamanhos específicos da computação e as escolhas de tamanho de armazenamento disponíveis para bases de dados únicas, consulte [os limites de recursos baseados na Base de Dados SQL vCore para bases de dados únicas](resource-limits-vcore-single-databases.md).
- Para os tamanhos específicos da computação e escolhas de tamanho de armazenamento disponíveis para piscinas elásticas, consulte [os limites de recursos baseados em Bases de Dados SQL vCore para piscinas elásticas](resource-limits-vcore-elastic-pools.md).
