...
ms.locfileid: "84042982"
---
# <a name="deploy-and-explore-a-multitenant-saas-app-that-uses-the-database-per-tenant-pattern-with-azure-sql-database"></a>Implemente e explore uma aplicação SaaS multiarrendatária que utiliza o padrão de base de dados por inquilino com base de dados Azure SQL
...
1. Utilize o URL para abrir o Site de Eventos no seu navegador web: http://events.wingtip-dpt.&lt .utilizador &gt; .trafficmanager.net. Substitua &lt; o utilizador pelo valor de utilizador da sua &gt; implementação.
...
3. *LoadGenerator.ps1* inicia um trabalho de fundo PowerShell para cada base de dados de inquilinos:
    ...
    - Cada trabalho causa uma carga baseada em CPU numa base de dados de inquilinos executando *sp_CpuLoadGenerator*. A intensidade e duração da carga variam consoante `$DemoScenario` .
...
4. Se, o gerador de carga iniciar os trabalhos de `$OneTime = $false` fundo e, em seguida, continuar a funcionar. A cada 10 segundos, monitoriza os novos inquilinos que são provisionados. Se `$OneTime = $true` definir, o LoadGenerator inicia os trabalhos de fundo e, em seguida, deixa de funcionar em primeiro plano. Para este tutorial, `$OneTime = $false` saia.
...
- Para aprender sobre trabalhos elásticos, consulte [Gerir bases de dados em nuvem em escala.](../../sql-database/elastic-jobs-overview.md)
