...
description: Saiba como escrever uma função R para computação estatística avançada na Base de Dados Azure SQL utilizando serviços de machine learning (pré-visualização).
...
Este artigo descreve como incorporar funções matemáticas e de utilidade R num procedimento armazenado sql. Funções estatísticas avançadas que são complicadas de implementar em T-SQL podem ser feitas em R com apenas uma única linha de código.
...
Para simplificar, vamos utilizar o pacote R `stats` que é instalado e carregado por padrão com Azure SQL Database usando Serviços de Aprendizagem automática (pré-visualização). O pacote contém centenas de funções para tarefas estatísticas comuns, entre elas a `rnorm` função. Esta função gera um número especificado de números aleatórios utilizando a distribuição normal, dado um desvio padrão e meios.
...
Para chamar esta linha de R de T-SQL, corra `sp_execute_external_script` e adicione a função R no parâmetro do script R, assim:
...
É fácil quando combinado com SQL. Você define um procedimento armazenado que obtém os argumentos do utilizador, em seguida, passar esses argumentos para o script R como variáveis.
...
- A linha começa com `@params` todas as variáveis utilizadas pelo código R e os correspondentes tipos de dados SQL.

- As linhas que seguem imediatamente mapeiam os nomes de parâmetro SQL para os nomes variáveis R correspondentes.
...
## <a name="use-r-utility-functions-for-troubleshooting"></a>Use funções de utilidade R para resolução de problemas
...
Como a `utils` embalagem está instalada mas não é carregada por defeito, deve utilizar a `library()` função para carregá-la primeiro.
...
> [!TIP]
> Muitos utilizadores gostam de usar as funções de cronometragem do sistema em R, como `system.time` e , para capturar o tempo utilizado pelos `proc.time` processos R e analisar problemas de desempenho.
