...
Se estiver a usar o RODBC na Base de Dados Azure SQL, então não pode escrever para uma mesa temporária, seja ela criada dentro ou fora da `sp_execute_external_script` sessão. A suver é utilizar [RxOdbcData](https://docs.microsoft.com/machine-learning-server/r-reference/revoscaler/rxodbcdata) e [rxDataStep](https://docs.microsoft.com/machine-learning-server/r-reference/revoscaler/rxdatastep) (com overwrite=FALSE e append="rows") para escrever para uma tabela temporária global criada antes da `sp_execute_external_script` consulta.
...
- Consulte a visão geral, Serviços de Machine Learning de Base de [Dados Azure SQL com R (pré-visualização)](machine-learning-services-overview.md).
