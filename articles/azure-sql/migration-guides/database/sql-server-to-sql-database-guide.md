---
title: 'Sql Server para Azure SQL Database: Guia de migração'
...
Neste guia, aprende [a migrar a](https://azure.microsoft.com/migration/migration-journey) sua instância SQL Server para Azure SQL Database. 
...
Para obter mais informações sobre migração, consulte a visão geral da [migração.](sql-server-to-sql-database-overview.md) Para outros guias de migração, consulte [a Migração da Base de Dados.](https://docs.microsoft.com/data-migration) 
...
Depois de verificar que o seu ambiente de origem é suportado, comece com a fase de pré-migração. Descubra todas as fontes de dados existentes, avalie a viabilidade da migração e identifique quaisquer problemas de bloqueio que possam impedir a sua [migração em nuvem Azure.](https://azure.microsoft.com/migration)
...
> [!IMPORTANT]
> Para obter mais informações sobre os passos específicos associados à realização de um corte como parte das migrações utilizando DMS, consulte [o corte de migração de desempenho](../../../dms/tutorial-sql-server-to-azure-sql.md).

## <a name="migration-recommendations"></a>Recomendações de migração

Para acelerar a migração para a Base de Dados Azure SQL, deve considerar as seguintes recomendações:
...
- Para saber mais sobre [Azure Migrate](https://azure.microsoft.com/services/azure-migrate) ver
