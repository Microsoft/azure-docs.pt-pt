...
Tutorial [ **concluído: Configure grupos de disponibilidade para SQL Server em máquinas virtuais RHEL em Azure**](rhel-high-availability-stonith-tutorial.md)

## <a name="create-the-load-balancer-in-the-azure-portal"></a>Crie o equilibrador de carga no portal Azure

As seguintes instruções levam-no através dos passos 1 a 4 do [Create e configurar o equilibrador](../windows/availability-group-load-balancer-portal-configure.md#create-and-configure-the-load-balancer-in-the-azure-portal) de carga na secção do portal Azure do artigo [do portal Load - Azure.](../windows/availability-group-load-balancer-portal-configure.md)
...
6. Selecione cada máquina virtual no ambiente e associe o endereço IP apropriado a cada seleção.
    ...
    :::image type="content" source="media/rhel-high-availability-listener-tutorial/add-backend-pool.png" alt-text="Adicione piscina de backend":::
...
1. Crie um grupo que contenha o e o `virtualip` `azure_load_balancer` recurso:
...
1. Inicie sessão em cada nó VM. Utilize o seguinte comando para abrir o ficheiro dos anfitriões e configurar a resolução de nome supérbio para `ag1-listener` a máquina em cada máquina.

    ```
    sudo vi /etc/hosts
    ```

    No **vi** editor, `i` insira para inserir texto, e numa linha em branco, adicione o IP do `ag1-listener` . Em seguida, adicione `ag1-listener` depois de um espaço ao lado do IP.
...
1. Utilize o SQLCMD para iniciar sessão no nó principal do Servidor SQL utilizando o nome de ouvinte do grupo de disponibilidade:
    ...
    - Utilize um login que tenha sido previamente criado e `<YourPassword>` substitua-o pela palavra-passe correta. O exemplo abaixo utiliza o `sa` login que foi criado com o Servidor SQL.
...
1. Executar o seguinte comando para falhar manualmente sobre a réplica primária para `<VM2>` ou outra réplica. Substitua-o pelo valor do seu nome de `<VM2>` servidor.
...
> [Configure um balancer de carga para um grupo de disponibilidade em VMs de servidor Estivado Azure](../windows/availability-group-load-balancer-portal-configure.md)
