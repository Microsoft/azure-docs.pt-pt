...
Este artigo fornece respostas a algumas das questões mais comuns sobre a execução do [SQL Server em Máquinas Virtuais do Windows (VMs) em Azure](https://azure.microsoft.com/services/virtual-machines/sql-server/).
...
1. **Como generalizo o SQL Server no Azure VM e uso-o para implantar novos VMs?**
   ...
   Pode implementar um VM do Servidor do Windows (sem o SQL Server instalado nele) e utilizar o processo [sysprep SQL](/sql/database-engine/install-windows/install-sql-server-using-sysprep?view=sql-server-ver15) para generalizar o SQL Server em Azure VM (Windows) com os meios de instalação do SQL Server. Os clientes que tenham [Software Assurance](https://www.microsoft.com/licensing/licensing-programs/software-assurance-default?rtc=1&activetab=software-assurance-default-pivot%3aprimaryr3) podem obter os seus meios de instalação a partir do [Centro de Licenciamento de Volume.](https://www.microsoft.com/Licensing/servicecenter/default.aspx) Os clientes que não têm Software Assurance podem utilizar os meios de configuração a partir de uma imagem VM do Azure Marketplace SQL Server que tem a edição desejada.
