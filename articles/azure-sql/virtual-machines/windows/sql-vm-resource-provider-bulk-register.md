...
ms.locfileid: "84051137"
---
# <a name="bulk-register-sql-virtual-machines-in-azure-with-the-sql-vm-resource-provider"></a>Registar em massa as máquinas virtuais do SQL no Azure com o fornecedor de recursos da VM do SQL
...
- Uma [subscrição Azure](https://azure.microsoft.com/free/) que foi [registada no fornecedor de recursos](sql-vm-resource-provider-register.md#register-subscription-with-rp) e contém máquinas virtuais SQL Server não registadas. 
...
Abra um terminal administrativo da PowerShell e navegue até onde guardou o `RegisterSqlVMs.psm1` ficheiro. Em seguida, executar o seguinte cmdlet PowerShell para importar o script como um módulo: 
...
O relatório é gerado como um ficheiro chamado onde o carimbo de `.txt` tempo é o momento em que o `RegisterSqlVMScriptReport<Timestamp>.txt` cmdlet foi iniciado. O relatório enumera os seguintes detalhes:
...
| Número de VMs registados com sucesso | A contagem de máquinas virtuais que foram registadas com sucesso após a execução do cmdlet. Lista as máquinas virtuais registadas no formato `SubscriptionID, Resource Group, Virtual Machine` . | 
...
Copie o script completo e guarde-o como `RegisterSqLVMs.psm1` .
