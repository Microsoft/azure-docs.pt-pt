---
title: Como utilizar o armazenamento Azure para backup do SQL Server e restaurar Microsoft Docs
...
* **Arquivo de reserva**: O serviço de armazenamento Azure Blob oferece uma melhor alternativa à opção de fita frequentemente utilizada para arquivar cópias de segurança. O armazenamento de fitas pode exigir transporte físico para uma instalação fora do local e medidas para proteger os meios de comunicação. Armazenar as suas cópias de segurança no Azure Blob Storage proporciona uma opção instantânea, altamente disponível e uma opção de arquivamento durável.
...
Para mais detalhes, consulte [a Cópia de Segurança do Servidor SQL e o Restauro com o Serviço de Armazenamento Azure Blob](https://go.microsoft.com/fwlink/?LinkId=271617).

As duas secções seguintes introduzem o serviço de armazenamento Azure Blob, incluindo os componentes necessários do SQL Server. É importante compreender os componentes e a sua interação para utilizar com sucesso a cópia de segurança e restaurar a partir do serviço de armazenamento Azure Blob.

## <a name="azure-blob-storage-service-components"></a>Componentes do serviço de armazenamento Azure Blob
Os seguintes componentes Azure são utilizados quando se baseiam no serviço de armazenamento Azure Blob.

| Componente | Descrição |
| --- | --- |
| **Conta de Armazenamento** |A conta de armazenamento é o ponto de partida para todos os serviços de armazenamento. Para aceder a um serviço de Armazenamento Azure Blob, primeiro crie uma conta de Armazenamento Azure. Para obter mais informações sobre o serviço de armazenamento Azure Blob, consulte [Como utilizar o Serviço de Armazenamento Azure Blob](https://azure.microsoft.com/develop/net/how-to-guides/blob-storage/) |
| **Contentor** |Um recipiente fornece um agrupamento de um conjunto de bolhas, e pode armazenar um número ilimitado de Blobs. Para escrever uma cópia de segurança do SQL Server num serviço Azure Blob, deve ter pelo menos o recipiente raiz criado. |
...
Os seguintes componentes do SQL Server são utilizados quando se baseiam no serviço de armazenamento Azure Blob.
...
| **Credencial** |As informações necessárias para ligar e autenticar ao serviço de armazenamento Azure Blob são armazenadas como credenciais.  Para que o SQL Server escreva cópias de segurança para uma Mancha Azure ou seja restaurada a partir dele, deve ser criada uma credencial SQL Server. Para obter mais informações, consulte [a Credencial do Servidor SQL](https://msdn.microsoft.com/library/ms189522.aspx). |

> [!NOTE]
> O Sql Server 2016 foi atualizado para suportar bolhas de blocos. Consulte [Tutorial: Utilizar o serviço de armazenamento Microsoft Azure Blob com bases de dados SQL Server 2016](https://msdn.microsoft.com/library/dn466438.aspx) para obter mais detalhes.
> 
> 
...
2. Em seguida, passe por um dos seguintes tutoriais que o acompanham através da criação de uma conta de armazenamento e realização de um restauro.
   ...
   * **SQL Server 2014**: [Tutorial: SQL Server 2014 Backup e Restaurar ao Microsoft Azure Blob Storage Service](https://msdn.microsoft.com/library/jj720558\(v=sql.120\).aspx).
   * **SQL Server 2016**: [Tutorial: Utilização do serviço de armazenamento Microsoft Azure Blob com bases de dados SQL Server 2016](https://msdn.microsoft.com/library/dn466438.aspx)
3. Reveja documentação adicional a partir de [Backup e Restauro do Servidor SQL com o Microsoft Azure Blob Storage Service](https://msdn.microsoft.com/library/jj919148.aspx).
