...
description: Este artigo cobre padrões de aplicação para SQL Server em VMs Azure. Fornece aos arquitetos e desenvolvedores de soluções uma base para uma boa arquitetura e design de aplicações.
...
Determinar quais os padrões ou padrões de aplicação a utilizar para as suas aplicações baseadas no SQL-Server em ambiente Azure é uma decisão de design importante e requer uma compreensão sólida de como o SQL Server e cada componente de infraestrutura do Azure funcionam em conjunto. Com o SQL Server em Serviços de Infraestrutura Azure, pode migrar, manter e monitorizar facilmente as aplicações existentes do SQL Server incorporadas no Windows Server para máquinas virtuais em Azure.
...
Antes de começar a ler este artigo, deve ter conhecimento sobre os conceitos fundamentais do SQL Server e do Azure. Para obter informações, consulte [os livros de servidor SQL online,](https://msdn.microsoft.com/library/bb545450.aspx) [SQL Server em Azure Virtual Machines](sql-server-on-azure-vm-iaas-what-is-overview.md) e [Azure.com](https://azure.microsoft.com/).
...
### <a name="choose-sql-server-in-an-azure-virtual-machine-when"></a>Escolha o SQL Server numa Máquina Virtual Azure, quando:
...
* Pretende aproveitar as capacidades do ambiente Azure, mas a Base de Dados Azure SQL não suporta todas as funcionalidades que a sua aplicação necessita. Isto pode incluir as seguintes áreas:
  ...
  * **Conformidade HIPAA:** Os clientes de saúde e fornecedores independentes de software (ISVs) podem escolher [o SQL Server em Azure Virtual Machines](sql-server-on-azure-vm-iaas-what-is-overview.md) em vez de [Azure SQL Database](../../database/sql-database-paas-overview.md) porque o SQL Server numa Máquina Virtual Azure é coberto pelo ACORDO DE ASSOCIADO DE NEGÓCIOS DA HIPAA (BAA). Para obter informações sobre a conformidade, consulte [o Microsoft Azure Trust Center: Compliance](https://azure.microsoft.com/support/trust-center/compliance/).
...
## <a name="pattern-with-azure-vms-azure-sql-database-and-azure-app-service-web-apps"></a>Padrão com VMs Azure, Base de Dados Azure SQL e Serviço de Aplicações Azure (Web Apps)
...
4. Crie postos de trabalho programados e alertas que retravam dados no local num disco de máquina virtual em Azure. Para obter mais informações, consulte [a Cópia de Segurança do Servidor SQL e a Restauração com o Serviço de Armazenamento Azure Blob](https://msdn.microsoft.com/library/jj919148.aspx) e Cópia de Segurança e Restauro para O Servidor [SQL em Máquinas Virtuais Azure](../../../azure-sql/virtual-machines/windows/backup-restore.md).
...
* Crie um servidor web tradicional (IIS - Serviços de Informação na Internet) em Azure e aceda bases de dados em SQL Server em Azure Virtual Machines.
* Implementar e implementar um serviço de nuvem para Azure. Em seguida, certifique-se de que este serviço de nuvem pode aceder a bases de dados em SQL Server em máquinas Virtuais Azure. Um serviço de nuvem pode incluir vários papéis web e trabalhador.

A tabela seguinte fornece uma comparação do desenvolvimento web tradicional com Azure Cloud Services e Azure Web Apps no que diz respeito ao SQL Server em Azure Virtual Machines. A tabela inclui aplicações web Azure, uma vez que é possível utilizar o SQL Server em Azure VM como fonte de dados para aplicações web Azure através do seu endereço IP virtual público ou nome DNS.

|  | Desenvolvimento web tradicional em Azure Virtual Machines | Serviços cloud em Azure | Web Hosting com Azure Web Apps |
| --- | --- | --- | --- |
