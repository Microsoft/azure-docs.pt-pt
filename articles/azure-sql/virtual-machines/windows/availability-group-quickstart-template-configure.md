...
description: Utilize modelos de quickstart Azure para criar o Cluster Failover do Windows, junte-se a VMs do Servidor SQL para o cluster, crie o ouvinte e configure o equilíbrio interno de carga em Azure.
...
### <a name="availability-group-listener-for-availability-group-ag-name-already-exists"></a>O ouvinte do grupo de disponibilidade para o grupo de disponibilidade \<AG-Name> ' ' já existe
...
Este erro pode ter duas causas. Ou a conta de domínio especificada não existe, ou falta os dados do Nome Principal do [Utilizador (UPN).](/windows/desktop/ad/naming-properties#userprincipalname) O modelo de configuração de **101 sql-vm-ag-ag-espera** uma conta de domínio na forma UPN (isto é, ), mas algumas contas de *user@domain.com* domínio podem estar faltando. Isto normalmente acontece quando um utilizador local foi migrado para ser a primeira conta de administrador de domínio quando o servidor foi promovido a um controlador de domínio, ou quando um utilizador foi criado através do PowerShell. 
...
4. Selecione o separador **Conta.** Se a caixa de nomes de início de **sessão do Utilizador** estiver em branco, esta é a causa do seu erro. 
    ...
    ![Conta de utilizador em branco indica falta da UPN](./media/availability-group-quickstart-template-configure/account-missing-upn.png)
