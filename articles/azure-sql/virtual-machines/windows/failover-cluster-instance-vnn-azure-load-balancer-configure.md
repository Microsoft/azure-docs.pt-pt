---
title: Configure Azure Load Balancer uma instância de cluster de failover VNN
description: Aprenda a configurar um Azure Load Balancer para encaminhar o tráfego para o nome de rede virtual (VNN) para a sua instância de cluster failover (FCI) com o SQL Server em VMs Azure para alta disponibilidade e recuperação de desastres (HADR).
...
Este artigo ensina-o a configurar um equilibrador de carga utilizando o serviço Azure Load Balancer. O equilibrador de carga irá encaminhar o tráfego para a sua [instância de cluster failover (FCI)](failover-cluster-instance-overview.md) com o SQL Server em VMs Azure para uma elevada disponibilidade e recuperação de desastres (HADR). 

Para uma opção de conectividade alternativa para o SQL Server 2019 CU2 e mais tarde, considere um [nome de rede distribuído](failover-cluster-instance-distributed-network-name-dnn-configure.md) em vez de configuração simplificada e melhor falha. 
