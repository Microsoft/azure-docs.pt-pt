---
title: Configure a DNN para a instância de cluster de failover
...
Nas Máquinas Virtuais Azure, o nome de rede distribuído (DNN) encaminha o tráfego para o recurso agrupado apropriado. Fornece uma forma mais fácil de ligar ao exemplo de cluster failover do SQL Server (FCI) do que o nome de rede virtual (VNN), sem a necessidade de um Balançador de Carga Azure. 

Este artigo ensina-o a configurar um recurso DNN para encaminhar o tráfego para o seu caso de cluster de failover com o SQL Server em VMs Azure para uma elevada disponibilidade e recuperação de desastres (HADR). 

A funcionalidade DNN está atualmente disponível apenas no SQL Server 2019 CU2 e mais tarde no Windows Server 2016 e mais tarde. 

Para uma opção de conectividade alternativa, considere um [nome de rede virtual e um Balanceador de Carga Azure.](failover-cluster-instance-vnn-azure-load-balancer-configure.md) 

## <a name="overview"></a>Descrição geral

O nome de rede distribuído (DNN) substitui o nome de rede virtual (VNN) como o ponto de ligação quando utilizado com uma [instância de cluster de failover Always On em VMs do SQL Server](failover-cluster-instance-overview.md). Isto anula a necessidade de um tráfego de encaminhamento do Balançador de Carga Azure para o VNN, simplificando a implementação, manutenção e melhorando o failover. 

Com uma implementação FCI, o VNN ainda existe, mas o cliente conecta-se ao nome DNN DNS em vez do nome VNN. 

## <a name="prerequisites"></a>Pré-requisitos 

Antes de completar os passos deste artigo, já deve ter:

- SQL Server 2019 na CU2 ou mais tarde, no Windows Server 2016 e mais tarde
...
- Atualmente, um DNN com FCI é suportado apenas para SQL Server 2019 CU2 e mais tarde no Windows Server 2016 e mais tarde. 
