---
title: 'Tutorial: Implemente websites Nuxt.js renderizados por servidor estoque em Aplicações Web Azure Static'
description: Gere e implemente sites dinâmicos Nuxt.js com Aplicações Web Estáticas Azure.
...
ms.locfileid: "83599824"
---
# <a name="deploy-server-rendered-nuxtjs-websites-on-azure-static-web-apps-preview"></a>Implemente websites Nuxt.js renderizados por servidor estoque na pré-visualização de aplicações web da Azure Static

Neste tutorial, você aprende a implementar um site estática [nuxt.js](https://nuxtjs.org) gerado para [Aplicações Web Estáticas Azure](overview.md). Para começar, aprende-se a configurar, configurar e implementar uma aplicação Nuxt.js. Durante este processo, você também aprende a lidar com desafios comuns muitas vezes enfrentados ao gerar páginas estáticas com Nuxt.js
...
## <a name="set-up-a-nuxtjs-app"></a>Criar uma aplicação Nuxt.js

Você pode configurar um novo projeto Nuxt.js usando `create-nuxt-app` . Em vez de um novo projeto, neste tutorial começa-se por clonar um repositório existente. Este repositório é criado para demonstrar como implementar uma aplicação dinâmica Nuxt.js como um site estático.

1. Crie um novo repositório sob a sua conta GitHub a partir de um repositório de modelos.
1. Navegar para<http://github.com/staticwebdev/nuxtjs-starter/generate>
...
1. Navegue para a nova aplicação Nuxt.js:
...
1. Inicie a aplicação Nuxt.js em desenvolvimento:
...
## <a name="generate-a-static-website-from-nuxtjs-build"></a>Gerar um site estático a partir da construção de Nuxt.js

Quando se constrói um site Nuxt.js utilizando `npm run build` , a aplicação é construída como uma aplicação web tradicional, não como um site estático. Para gerar um local estático, utilize a seguinte configuração de aplicação.

1. Atualize o script de construção do _pacote.json_para gerar apenas um site estático usando o `nuxt generate` comando:
...
1. Gerar um local estático:
    ...
    A Nuxt.js gerará o site estático e copiará-o numa pasta _dist_ na raiz do seu diretório de trabalho.
...
1. O palco mudou todos os ficheiros:
...
### <a name="create-an-azure-static-web-apps-preview-resource"></a>Criar um recurso de pré-visualização de aplicações estátáticas azure
...
A nova conta De Aplicações Web Estáticas precisa de ter acesso ao repositório com a sua aplicação Nuxt.js para que possa implementar automaticamente os compromissos.
...
1. Escolha o **mestre** como o ramo da queda do *Ramo.*
   ...
   :::image type="content" source="media/deploy-nuxtjs/connect-github.png" alt-text="Ligar GitHub":::
...
1. Digite **dist** na caixa de texto de localização de artefactos da *App.*
...
A razão para isso é que Nuxt.js gerou o site estático, só o fez para a página inicial. Nuxt.js pode gerar `.html` ficheiros estáticos equivalentes para cada ficheiro de `.vue` páginas, mas há uma exceção. 

Se a página for uma página dinâmica, por `_id.vue` exemplo, não terá informação suficiente para gerar um HTML estático a partir de tal página dinâmica. Terá de providenciar explicitamente os caminhos possíveis para as rotas dinâmicas.

## <a name="generate-static-pages-from-dynamic-routes"></a>Gerar páginas estáticas de rotas dinâmicas

1. Atualize o ficheiro _nuxt.config.js_ de modo a que a Nuxt.js utilize uma lista de todos os dados disponíveis para gerar páginas estáticas para cada quadro/biblioteca:
   ...
   > [!NOTE]
   > `routes`é uma função de asincronização, para que possa fazer um pedido a uma API nesta função e utilizar a lista devolvida para gerar os caminhos.
