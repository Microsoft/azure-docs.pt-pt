---
title: Deteção de fraude em tempo real usando Azure Stream Analytics
...
8. Nomeie o novo centro de eventos `asa-eh-frauddetection-demo`. Pode utilizar um nome diferente. Se o fizeres, tome nota disso, porque precisas do nome mais tarde. Não precisas de definir outras opções para o centro do evento neste momento.
...
3.  Adicione uma política denominada `asa-policy-manage-demo` e para **Reclamação,** selecione **Gerir**.
...
3. No elemento `<appSettings>`:
   ...
   * Detete o valor da chave `Microsoft.ServiceBus.ConnectionString` para a cadeia de ligação. 
   ...
   A secção `<appSettings>` será o seguinte exemplo. (Para maior clareza, as linhas estão embrulhadas e alguns caracteres foram removidos do símbolo de autorização.)

   ![Ficheiro config TelcoGenerator mostra nome do centro de eventos e linha de ligação](./media/stream-analytics-real-time-fraud-detection/stream-analytics-telcogenerator-config-file-app-settings.png)
...
1. Altere a consulta no editor de código para o seguinte: 
    ...
    Esta consulta é como qualquer sql de adesão, exceto para a função `DATEDIFF` na união. Esta versão do `DATEDIFF` é específica do Streaming Analytics, e deve figurar na cláusula `ON...BETWEEN`. Os parâmetros são uma unidade de tempo (segundos neste exemplo) e os pseudónimos das duas fontes para a adesão. Isto é diferente da função padrão do SQL `DATEDIFF`.
...
4. No painel **de saídas,** clique em **Adicionar** e selecione **o armazenamento Blob**. Em seguida, preencha a nova página de saída com as seguintes informações:
   ...
   |Contentor  | asa-fraudulentcalls-demo | Escolha Criar novo e insira um nome de contentor. |
