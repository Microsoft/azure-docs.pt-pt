...
4. No painel Criar espaço de **nome,** introduza um nome de espaço de nome como `<yourname>-eh-ns-demo` . Pode utilizar qualquer nome para o espaço de nome, mas o nome deve ser válido para um URL e deve ser único em todo o Azure. 
...
8. Nomeie o novo centro de `asa-eh-frauddetection-demo` eventos. Pode utilizar um nome diferente. Se o fizeres, tome nota disso, porque precisas do nome mais tarde. Não precisas de definir outras opções para o centro do evento neste momento.
...
3. Adicione uma política nomeada `asa-policy-manage-demo` e para **Claim,** selecione **Gerir**.
...
1. No editor onde copiou a corda de ligação, tome nota do `EntityPath` valor e, em seguida, retire o par (não se esqueça de `EntityPath` remover o ponto e vírgula que o precede). 
...
3. No `<appSettings>` elemento:
   ...
   * Detete o valor da `EventHubName` chave para o nome do hub do evento (isto é, para o valor do caminho da entidade).
   * Desloque o valor da chave para a cadeia de `Microsoft.ServiceBus.ConnectionString` ligação. 
...
2. No portal, volte ao painel de trabalho streaming Analytics. (Se fechar o painel, procure `asa_frauddetection_job_demo` no painel de todos os **recursos.)**
...
4. No painel **De consulta,** clique nos pontos ao lado da `CallStream` entrada e, em seguida, selecione dados da amostra a **partir da entrada**.
...
Como alternativa, pode obter um ficheiro .json que tenha dados de amostra [saindo do GitHub,](https://github.com/Azure/azure-stream-analytics/blob/master/Sample%20Data/telco.json)e, em seguida, carregar esse ficheiro .json para usar como dados de amostra para a `CallStream` entrada. 
...
1. Altere a consulta no editor de código para o seguinte:
    ...
    Esta consulta utiliza a `Timestamp By` palavra-chave na cláusula para especificar qual o campo de `FROM` carimbo sinuoso no fluxo de entrada a utilizar para definir a janela Tumbling. Neste caso, a janela divide os dados em segmentos pelo `CallRecTime` campo em cada registo. (Se não for especificado nenhum campo, a operação de janela utiliza o tempo em que cada evento chega ao centro do evento. Ver "Hora de Chegada Vs Tempo de Aplicação" na Referência da Linguagem da [Consulta de Análise de Fluxo](https://docs.microsoft.com/stream-analytics-query/stream-analytics-query-language-reference). 

    A projeção inclui , que devolve um carimbo temporal `System.Timestamp` para o fim de cada janela. 

    Para especificar que pretende utilizar uma janela Tumbling, utiliza a função [TUMBLINGWINDOW](https://docs.microsoft.com/stream-analytics-query/tumbling-window-azure-stream-analytics) na `GROUP BY` cláusula. Na função, especifice uma unidade de tempo (em qualquer lugar de um microsegundo a um dia) e um tamanho de janela (quantas unidades). Neste exemplo, a janela Tumbling consiste em intervalos de 5 segundos, para que você obtenha uma contagem por país/região por cada 5 segundos de chamadas.
...
Para verificar estes casos, pode utilizar uma auto-adesão dos dados de streaming para se juntar ao fluxo para si mesmo com base no `CallRecTime` valor. Pode então procurar registos de chamadas onde o `CallingIMSI` valor (o número de origem) é o mesmo, mas o `SwitchNum` valor (país/região de origem) não é o mesmo.

Quando utilizar uma adesão aos dados de streaming, a união deve fornecer alguns limites para a distância das linhas correspondentes poderem ser separadas a tempo. (Como notado anteriormente, os dados de streaming são efetivamente infinitos.) Os limites de tempo para a relação são especificados dentro `ON` da cláusula da adesão, utilizando a `DATEDIFF` função. Neste caso, a adesão baseia-se num intervalo de 5 segundos de dados de chamadas.
...
1. Altere a consulta no editor de código para o seguinte: 
    ...
    Esta consulta é como qualquer sql se juntar, exceto para a `DATEDIFF` função na união. Esta versão `DATEDIFF` é específica do Streaming Analytics, e deve figurar na `ON...BETWEEN` cláusula. Os parâmetros são uma unidade de tempo (segundos neste exemplo) e os pseudónimos das duas fontes para a adesão. Isto é diferente da função SQL `DATEDIFF` padrão.
...
1. A partir do canto superior esquerdo do portal Azure, selecione **Criar uma**conta de  >  **Storage**  >  **armazenamento**de recursos . Preencha a página de trabalho da conta de armazenamento com **nome** definido para "asaehstorage", **Localização** definida para "East US", **grupo de recursos** definido para "asa-eh-ns-rg" (hospedar a conta de armazenamento no mesmo grupo de recursos que o trabalho de Streaming para maior desempenho). As restantes definições podem ser mantidas nos respetivos valores predefinidos.  
...
2. No portal Azure, volte ao painel de trabalho streaming Analytics. (Se fechar o painel, procure `asa_frauddetection_job_demo` no painel de todos os **recursos.)**
