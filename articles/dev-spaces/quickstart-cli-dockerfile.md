---
title: Implementar uma aplicação com um Dockerfile no Kubernetes através dos espaços de desenvolvimento do Azure
...
ms.date: 07/08/2019
ms.topic: quickstart
description: Implementar um microsserviço com um Dockerfile no AKS com os espaços de desenvolvimento do Azure
...
ms.locfileid: "67706840"
---
# <a name="quickstart-develop-an-application-with-a-dockerfile-on-kubernetes-using-azure-dev-spaces"></a>Início rápido: Desenvolver uma aplicação com um Dockerfile no Kubernetes através dos espaços de desenvolvimento do Azure
...
Neste artigo, vai utilizar o [aplicação de exemplo do Azure Dev espaços](https://github.com/Azure/dev-spaces) para demonstrar a utilização de espaços de desenvolvimento do Azure. O aplicativo de exemplo é escrito em Go, mas pode usar praticamente qualquer linguagem, desde que fornecer um Dockerfile válido para colocar a aplicação num contentor e executá-lo no AKS.

Clonar a aplicação a partir do GitHub e navegue para o *dev-espaços/exemplos/golang/obter-iniciada/webfrontend* diretório:
...
A aplicação de exemplo inclui um Dockerfile válido. Para gerar os ativos de gráfico Helm para executar a aplicação no Kubernetes, utilize o `azds prep` comando:

```cmd
azds prep --public
```

Tem de executar o `prep` comando a partir do *dev-espaços/exemplos/golang/obter-iniciada/webfrontend* diretório ao gerar corretamente os ativos de gráfico do Helm.
...
Pode ver o serviço em execução ao abrir o URL público e navegar para o `/api` caminho. O URL público é apresentado no resultado do `azds up` comando. Neste exemplo, é o URL público `http://default.webfrontend.1234567890abcdef1234.eus.azds.io/` e navegaria para `http://default.webfrontend.1234567890abcdef1234.eus.azds.io/api`.
...
1. Atualização [29 de linha `src/app/main.go` ](https://github.com/Azure/dev-spaces/blob/master/samples/golang/getting-started/webfrontend/src/app/main.go#L29) para:
