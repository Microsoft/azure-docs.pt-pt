...
![A imagem mostra a seleção do comando "Azure Dev Spaces: Prepare ficheiros de configuração para espaços Azure Dev" a partir da janela da Paleta de Comando.](media/common/command-palette.png)
...
![A imagem é do canto superior esquerdo da janela Visual Studio Code. O ícone de depurado é destacado, o painel esquerdo é intitulado "DEBUG", e uma lista à direita do título mostra "Launch Program (A Z D S)](media/get-started-node/debug-configuration-nodejs2.png)
...
![O painel de ações de Debug é um pequeno painel no centro superior da página (logo abaixo do título da página). O botão Reiniciar exibe uma seta circular e é realçado. A imagem do botão hover é "Reiniciar (controlo + turno + F 5)".](media/common/debug-action-refresh.png)
