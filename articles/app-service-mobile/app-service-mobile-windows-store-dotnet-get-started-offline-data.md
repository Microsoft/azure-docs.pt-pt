...
4. Abra o arquivo MainPage.xaml.cs e remova a marca de comentário da definição de `#define OFFLINE_SYNC_ENABLED`.
...
Quando você executa o aplicativo pela primeira vez, o manipulador de eventos `OnNavigatedTo` chama `InitLocalStoreAsync`. Esse método, por sua vez, chama `SyncAsync` para sincronizar o armazenamento local com o banco de dados back-end. O aplicativo tenta sincronizar na inicialização.
...
2. Pressione a tecla **F5** para recompilar e executar o aplicativo. O aplicativo sincroniza suas alterações locais com o back-end do aplicativo móvel do Azure usando operações de push e pull quando o manipulador de eventos de `OnNavigatedTo` é executado.
