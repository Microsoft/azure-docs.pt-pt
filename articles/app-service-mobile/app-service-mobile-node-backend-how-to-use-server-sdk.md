...
### <a name="howto-cmdline-basicapp"></a>Crie um node.js básico de volta usando a linha de comando

Todas as aplicações móveis Node.js back end começa como uma aplicação ExpressJS. ExpressJS é o quadro de serviço web mais popular disponível para Node.js. Pode criar uma [Express] expressa básica da seguinte forma:
...
1. Executar `npm init` para inicializar a estrutura do pacote:
   ...
   O comando `npm init` faz um conjunto de perguntas para inicializar o projeto. Veja a saída do exemplo:

   ![A saída de npm init][0]

1. Instale as `express` e `azure-mobile-apps` bibliotecas do repositório da NPM:
...
Esta aplicação cria uma Web API otimizada para dispositivos móveis com um único ponto final (`/tables/TodoItem`) que fornece acesso não autenticado a uma loja de dados SQL subjacente utilizando um esquema dinâmico. É adequado para seguir os quickstarts da biblioteca do cliente:

* [Quickstart do cliente Android]
* [Apache Cordova Cliente quickstart]
* [iOS Cliente quickstart]
* [Cliente da Loja windows]
* [Xamarin.iOS cliente quickstart]
* [Xamarin.Android Cliente quickstart]
* [Xamarin.Forms Cliente quickstart]

Pode encontrar o código para esta aplicação básica na [amostra de basicapp no GitHub].

### <a name="howto-vs2015-basicapp"></a>Crie um Node.js back end usando Visual Studio 2015

O Visual Studio 2015 requer uma extensão para desenvolver aplicações nonóios dentro do IDE. Para iniciar, instale as [Node.js Ferramentas 1.1 para Estúdio Visual]. Quando terminar a instalação, crie uma aplicação Express 4.x:

1. Abra a caixa de diálogo **New Project** (de **File** > **New** > **Project**).
1. Expandir **os modelos** > **JavaScript** > **Node.js**.
...
1. Selecione **Fechar**.
1. Abra o ficheiro app.js para adicionar suporte para o Mobile Apps SDK. Na linha 6, na parte inferior da biblioteca, `require` declarações, adicione o seguinte código:
    ...
    Na linha 27, aproximadamente, após as outras declarações `app.use`, adicione o seguinte código:
...
1. Ou executa a aplicação localmente (a API é servida em `http://localhost:3000`) ou publica para o Azure.

### <a name="create-node-backend-portal"></a>Crie um Lado atrás do Nó usando o portal Azure

Pode criar uma aplicação móvel no [Portal do Azure] Pode completar os seguintes passos ou criar um cliente e servidor em conjunto seguindo o tutorial de [aplicação create a mobile.](app-service-mobile-ios-get-started.md) O tutorial contém uma versão simplificada destas instruções e é o melhor para projetos de prova de conceito.
...
4. Execute o comando `git clone` utilizando o URL do clone Git. Introduza a sua palavra-passe quando necessário, como no seguinte exemplo:

        $ git clone https://username@todolist.scm.azurewebsites.net:443/todolist.git

5. Navegue no diretório local (`/todolist` no exemplo anterior) e note que os ficheiros do projeto foram descarregados. Localize o ficheiro todoitem.json no diretório `/tables`. Este ficheiro define permissões na mesa. Também encontre o ficheiro todoitem.js no mesmo diretório. Define os scripts de operação CRUD para a mesa.
6. Depois de efazer alterações nos ficheiros do projeto, executar os seguintes comandos para adicionar, comprometer e, em seguida, fazer upload das alterações para o site:
   ...
   Quando adiciona novos ficheiros ao projeto, primeiro tem de executar o comando `git add .`.

O site é republicado cada vez que um novo conjunto de compromissos é empurrado para o site.

### <a name="howto-publish-to-azure"></a>Publique o seu Lado de Volta do Nó ao Azure
...
## <a name="TableOperations"></a>Operações de mesa
...
### <a name="howto-dynamicschema"></a>Defina tabelas usando um esquema dinâmico
...
Como uma boa prática, deve definir cada tabela num ficheiro JavaScript no `tables` diretório e, em seguida, utilizar o método `tables.import()` para importar as tabelas. Alargando a amostra de aplicação básica, ajustaria o ficheiro app.js:
...
As tabelas usam um esquema dinâmico por defeito. Para desligar o esquema dinâmico a nível global, defina a definição de aplicações `MS_DynamicSchema` falsas no portal Azure.

Pode encontrar um exemplo completo na [amostra toda no GitHub].

### <a name="howto-staticschema"></a>Definir tabelas utilizando um esquema estático

Pode definir explicitamente as colunas para expor através da API web. O Node.js SDK de aplicações azure-mobile adiciona automaticamente quaisquer colunas extra necessárias para sincronização de dados offline à lista que fornece. Por exemplo, as aplicações de cliente quickstart requerem uma tabela com duas colunas: `text` (uma corda) e `complete` (uma Boolean).  
A tabela pode ser definida na definição de tabela JavaScript (localizada no diretório `tables`) da seguinte forma:
...
Se definir as tabelas estáticamente, também deve ligar para o método `tables.initialize()` para criar o esquema de base de dados no arranque. O método `tables.initialize()` devolve uma [promessa] para que o serviço web não sirva pedidos antes da base de dados ser inicializada.

### <a name="howto-sqlexpress-setup"></a>Use o SQL Server Express como uma loja de dados de desenvolvimento na sua máquina local
...
1. Descarregue e instale [Microsoft SQL Server 2014 Express]. Certifique-se de que instala o Expresso SQL Server 2014 com edição de Ferramentas. A menos que necessite explicitamente de suporte de 64 bits, a versão de 32 bits consome menos memória ao correr.
...
1. Executar o Estúdio de Gestão SQL Server 2014 e ligar-se à sua instância local do SQL Server Express:
   ...
   1. Expandir ** > ** **Logins** no Object Explorer.
...
A aplicação Node.js lê a variável ambiente `SQLCONNSTR_MS_TableConnectionString` para a cadeia de ligação para esta base de dados. Pode definir esta variável no seu ambiente. Por exemplo, pode usar o PowerShell para definir esta variável ambiental:
...
### <a name="howto-config-localdev"></a>Configure o seu projeto para o desenvolvimento local

As Aplicações Móveis lêum ficheiro JavaScript chamado *azureMobile.js* do sistema de ficheiros local. Não utilize este ficheiro para configurar as Aplicações Móveis SDK em produção. Em vez disso, utilize **as definições** da App no [Portal do Azure].
...
Recomendamos que adicione o ficheiro **.gitignore** **(ou** outro controlo de código de origem) para evitar que as palavras-passe sejam armazenadas na nuvem. Configure sempre as **definições** de produção nas definições da App dentro do [Portal do Azure].

### <a name="howto-appsettings"></a>Configure as definições de aplicativos para a sua aplicação móvel

A maioria das definições no ficheiro azureMobile.js têm uma configuração de aplicação equivalente no [Portal do Azure]. Utilize a seguinte lista para configurar a sua aplicação nas definições da **App:**

| Definição de aplicativos | definição azureMobile.js | Descrição | Valores válidos |
|:--- |:--- |:--- |:--- |
| **MS_MobileAppName** |nome |Nome da app |string |
| **MS_MobileLoggingLevel** |logging.level |Nível mínimo de registo de mensagens para registar |erro, aviso, informação, verbosa, depuração, tolo |
| **MS_DebugMode** |depurado |Ativa ou desativa o modo de depuração |VERDADEIRO, FALSO |
...
| **MS_DisableVersionHeader** |versão (definida para indefinida) |Desativa o cabeçalho X-ZUMO-Server-Version |VERDADEIRO, FALSO |
| **MS_SkipVersionCheck** |skipversioncheck |Desativa a verificação da versão API do cliente |VERDADEIRO, FALSO |
...
### <a name="howto-use-sqlazure"></a>Utilize a Base de Dados SQL como a sua loja de dados de produção
...
1. Nas novas aplicações móveis, selecione **Definições** > **Aplicação Móvel** > **Dados** >  **+Add**.
1. No painel de ligação de **dados Adicionar,** selecione Base de **Dados SQL - Configure as definições necessárias** > **Criar uma nova base de dados**. Introduza o nome da nova base de dados na caixa **nome.**
...
### <a name="howto-tables-getidentity"></a>Use reclamações de autenticação com as suas tabelas
Pode configurar várias reclamações que são solicitadas quando a autenticação for configurada. Estas alegações não estão normalmente disponíveis através do `context.user` objeto. No entanto, pode recuperá-los utilizando o método `context.user.getIdentity()`. O método `getIdentity()` devolve uma promessa que resolve um objeto. O objeto é fundamentado pelo método de autenticação (`facebook`, `google`, `twitter`, `microsoftaccount`ou `aad`).
...
Para ver quais as reclamações disponíveis, utilize um navegador web para ver o ponto final `/.auth/me` do seu site.

### <a name="howto-tables-disabled"></a>Desativar o acesso a operações específicas de tabela
...
### <a name="howto-tables-query"></a>Ajuste a consulta que é usada com operações de mesa
...
As operações que normalmente executam uma consulta têm uma propriedade de consulta que você pode ajustar usando uma cláusula de `where`. A propriedade de consulta é um objeto [ConsultaJS] que é usado para converter uma consulta OData para algo que o final de dados pode processar. Para casos simples de igualdade (como o anterior), pode usar um mapa. Também pode adicionar cláusulas SQL específicas:

```javascript
context.query.where('myfield eq ?', 'value');
```

### <a name="howto-tables-softdelete"></a>Configure um suave apague numa mesa

Um apague suavemente não apaga os registos. Em vez disso, marca-os como eliminados na base de dados, definindo a coluna eliminada como verdadeira. O Mobile Apps SDK remove automaticamente os registos eliminados dos resultados, a menos que o Mobile Client SDK utilize `IncludeDeleted()`. Para configurar uma tabela para uma eliminação suave, coloque a propriedade `softDelete` no ficheiro de definição de tabela:
...
### <a name="howto-tables-seeding"></a>Semente a sua base de dados com dados
...
Recomendamos que ligue explicitamente para o método `tables.initialize()` para criar a tabela quando o serviço começar a funcionar.

### <a name="Swagger"></a>Ativar o suporte swagger
...
Provavelmente só quer permitir o apoio da Swagger em edições de desenvolvimento. Pode fazê-lo utilizando a definição de `NODE_ENV` aplicação:

```javascript
var mobile = azureMobileApps({ swagger: process.env.NODE_ENV !== 'production' });
```

O ponto final `swagger` está localizado em http://*seu site*.azurewebsites.net/swagger. Pode aceder à UI swagger através do `/swagger/ui` ponto final. Se optar por exigir a autenticação em toda a sua aplicação, a Swagger produz um erro. Para obter os melhores resultados, opte por permitir pedidos não autenticados nas definições de Autenticação/Autorização do Serviço de Aplicação Azure e, em seguida, controlar a autenticação utilizando a propriedade `table.access`.
...
## <a name="a-namepushpush-notifications"></a>notificações <a name="push"/>Push
...
O seguinte código mostra como utilizar o `push` objeto para enviar uma notificação push de transmissão para dispositivos iOS registados:
...
### <a name="push-user"></a>Envie notificações push a um utilizador autenticado utilizando tags
...
### <a name="howto-customapi-basic"></a>Defina uma API personalizada

Além da API de acesso a dados através do ponto final `/tables`, as Aplicações Móveis podem fornecer cobertura API personalizada. As APIs personalizadas são definidas de forma semelhante às definições de tabela e podem aceder a todas as mesmas instalações, incluindo a autenticação.

Se pretender utilizar a Autenticação do Serviço app com uma API personalizada, tem de configurar primeiro a Autenticação do Serviço de Aplicações no [Portal do Azure] Para mais informações, consulte o guia de configuração do fornecedor de identidade que pretende utilizar:
...
1. Crie um ficheiro JavaScript de definição API no diretório `api`.
1. Utilize o método de importação para importar o diretório `api`.
...
Vejamos um exemplo de API que devolve a data do servidor utilizando o método `Date.now()`. Aqui está o ficheiro api/date.js:
...
Cada parâmetro é um dos verbos RESTful padrão: GET, POST, PATCH ou DELETE. O método é uma função [Utensílios de meio expressJS] padrão que envia a saída necessária.

### <a name="howto-customapi-auth"></a>Exigir autenticação para acesso a uma API personalizada

O Mobile Apps SDK implementa a autenticação da mesma forma tanto para o ponto final `tables` como para as APIs personalizadas. Para adicionar autenticação à API desenvolvida na secção anterior, adicione uma propriedade `access`:
...
O mesmo símbolo utilizado para o ponto final `tables` deve ser utilizado para APIs personalizados que exijam autenticação.

### <a name="howto-customapi-auth"></a>Manuseie grandes uploads de ficheiros
...
### <a name="howto-customapi-sql"></a>Executar declarações SQL personalizadas
...
### <a name="howto-diagnostic-logs"></a>Depuração, diagnóstico e resolução de problemas aplicativos móveis
...
As aplicações Node.js têm acesso a uma vasta gama de ferramentas de registo de diagnóstico. Internamente, o Mobile Apps Node.js SDK utiliza [winston] para diagnóstico de registo. O registo é ativado automaticamente quando ativa o modo de depuração ou define a definição de aplicação `MS_DebugMode` verdadeira no [Portal do Azure]. Os registos gerados aparecem nos registos de diagnóstico no [Portal do Azure].
...
[Configure Autenticação de Diretório Ativo Do Azure]: ../app-service/configure-authentication-provider-aad.md
