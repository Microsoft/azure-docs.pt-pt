...
2. Abra o arquivo QSTodoService.cs e remova a marca de comentário da definição de `#define OFFLINE_SYNC_ENABLED`.
...
Ligue a rede novamente.  Quando você executa o aplicativo pela primeira vez, o método `RefreshDataAsync` é chamado. Isso, por sua vez, chama `SyncAsync` para sincronizar o armazenamento local com o banco de dados back-end.

1. Abra QSToDoService.cs no projeto compartilhado e reverta a alteração da propriedade **applicationURL** .
2. Recompile e execute o aplicativo. O aplicativo sincroniza as alterações locais com o back-end do aplicativo móvel do Azure usando as operações de push e pull quando o método de `OnRefreshItemsSelected` é executado.
