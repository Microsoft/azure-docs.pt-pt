---
title: Funcionalidades do motor de regras de CDN do Azure da Verizon Premium | Documentos da Microsoft
description: Funcionalidades do motor de regras de documentação de referência para a CDN do Azure da Verizon Premium.
services: cdn
author: mdgattuso
...
Este artigo apresenta uma lista de descrições detalhadas dos recursos disponíveis para a rede de entrega de conteúdos (CDN) do Azure [motor de regras](cdn-verizon-premium-rules-engine.md).
...
[Token Auth](#token-auth) | Determina se a autenticação baseada em tokens é aplicada a um pedido.
...
## <a name="azure-cdn-from-verizon-premium-rules-engine-features-reference"></a>CDN do Azure da referência de funcionalidades do motor de regras de Premium da Verizon
...
**Comportamento predefinido:** Ativado.
...
**Objetivo:** Limita a largura de banda para a resposta fornecida pelos POPs.
...
**Comportamento predefinido:** Apenas respostas GET são colocadas em cache.
...
**Comportamento predefinido:** Substitua.
...
**Comportamento predefinido:** Chave de cache de um pedido é determinado pelo URI do pedido.
...
**Objetivo:** Permite que uma nota a ser adicionados dentro de uma regra.
...
**Objetivo:** Determina o que acontece quando um resultados do pedido numa falha de acerto na cache parcial num POP.
...
**Comportamento predefinido:** Transferência progressiva de HTTP suporta multimédia MP4 e F4V por predefinição.
...
**Objetivo:** Determina se os pedidos de não-cache de um cliente HTTP serão reencaminhados para o servidor de origem.
...
**Objetivo:** Controles quanto posterior à hora de expiração normal um ativo em cache pode ser servido a partir de um preenchimento quando o POP não consegue revalide o elemento em cache com o servidor de origem.
...
**Comportamento predefinido:** Desativar. Revalidação só pode ocorrer depois que TTL o conteúdo em cache expirou.
...
Esta funcionalidade não é necessária para a colocação em cache correta e de entrega de conteúdos, mas pode ser útil como uma solução alternativa. Por exemplo, geradores de conteúdo dinâmicos nos servidores de origem inadvertidamente podem resultar em respostas de 0 bytes enviadas para os POPs. Esses tipos de respostas são normalmente colocadas em cache pelos POPs. Se souber que uma resposta de 0 bytes nunca é uma resposta válida para esse conteúdo, esta funcionalidade pode impedir que esses tipos de ativos que está a ser atendido aos seus clientes.
...
**Objetivo:** Determina se a expirou os conteúdos em cache serão entregues quando ocorre um erro durante a revalidação de cache ou ao obter os conteúdos solicitados a partir do servidor de origem do cliente.
...
302|Foi encontrado|Este código de estado redireciona os utilizadores não autorizados para o URL especificado no cabeçalho Location. Este código de estado é o método padrão do setor da execução de um redirecionamento.
...
**Objetivo:** Determina se as comparações de URL feitas pela autenticação baseada em tokens são maiúsculas e minúsculas.
