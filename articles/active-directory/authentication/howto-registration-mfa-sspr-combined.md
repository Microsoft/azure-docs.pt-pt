...
Antes do registo combinado, os utilizadores registaram métodos de autenticação para a autenticação de multi-factores Azure e reset de palavra-passe self-service (SSPR) separadamente. As pessoas estavam confusas de que métodos semelhantes eram usados para autenticação multi-factor Azure e SSPR, mas tiveram que se registar para ambas as funcionalidades. Agora, com registo combinado, os utilizadores podem registar-se uma vez e obter os benefícios tanto da Autenticação Multi-Factor Azure como do SSPR.
...
1. Sob **controlos de**  >  **acesso,** escolha **o acesso ao bloco**e, em seguida, **selecione**.
...
Aprenda a [forçar os utilizadores a reregistar métodos de autenticação](howto-mfa-userdevicesettings.md#manage-user-authentication-options).
