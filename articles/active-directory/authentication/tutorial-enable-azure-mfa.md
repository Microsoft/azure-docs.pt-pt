...
description: Neste tutorial, aprende-se a ativar a Autenticação Multi-Factor Azure para um grupo de utilizadores e testa o fator secundário durante um evento de entrada.
...
As políticas de autenticação multi-factor e acesso condicional do Azure conferem a flexibilidade para permitir mFA para os utilizadores durante eventos específicos de entrada.

> [!IMPORTANT]
> Este tutorial mostra a um administrador como ativar a autenticação multi-factor Azure.
>
...
> [!div class="checklist"]
> * Criar uma política de acesso condicional para permitir a autenticação de multi-factor Azure para um grupo de utilizadores
...
* Um grupo do qual o utilizador não administrador é membro, como o *MFA-Test-Group*. Você ativa a autenticação multi-factor Azure para este grupo neste tutorial.
...
A forma recomendada de ativar e utilizar a autenticação multi-factor Azure é com políticas de Acesso Condicional. O Acesso Condicional permite criar e definir políticas que reagem a assinar em eventos e solicitar ações adicionais antes de um utilizador ter acesso a uma aplicação ou serviço.
...
As políticas de Acesso Condicional só podem ser *definidas* para Reportar se quiser ver como a configuração impactaria os utilizadores, ou *Off* se não quiser a política de utilização agora. Como um grupo de teste de utilizadores foi direcionado para este tutorial, permite ativar a política e, em seguida, testar a Autenticação Multi-Factor Azure.
...
Agora inscreva-se no portal Azure. Como o portal Azure foi configurado na política de Acesso Condicional para exigir uma verificação adicional, obtém-se uma solicitação de autenticação multi-factor Azure.
...
1. Inscreva-se com o utilizador de teste não administrador, como *o testuser*. É-lhe exigido que se registe e utilize a Autenticação Multi-Factor Azure. Siga as instruções para concluir o processo e verifique se insinou-se com sucesso no portal Azure.
...
> [!div class="checklist"]
> * Criar uma política de acesso condicional para permitir a autenticação de multi-factor Azure para um grupo de utilizadores AD Azure
> * Configure as condições de política que solicitam para o MFA
> * Teste o processo de MFA como utilizador
