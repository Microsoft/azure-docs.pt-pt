...
ms.locfileid: "80332241"
---
# <a name="tutorial-integrate-a-single-forest-with-a-single-azure-ad-tenant"></a>Tutorial: Integrar uma única floresta com um único inquilino azure aD

Este tutorial acompanha-o através da criação de um ambiente de identidade híbrida utilizando o Azure Ative Directory (Azure AD) Connect cloud provisioning.

![Criar](media/tutorial-single-forest/diagram1.png)

Você pode usar o ambiente que você cria neste tutorial para testar ou para se familiarizar com o fornecimento de nuvens.
...
1. Identifique um servidor de anfitriões associado ao domínio que execute o Windows Server 2012 R2 ou superior com um tempo mínimo de 4 GB de RAM e .NET 4.7.1+ tempo de execução 
...
2. Se houver uma firewall entre os seus servidores e a AD Azure, configure os seguintes itens:
   ...
   - Se a sua firewall ou proxy lhe permitir especificar sufixos seguros, adicione as ligações t a ** \*.msappproxy.net** e ** \*.servicebus.windows.net**. Caso contrário, permita o acesso às gamas IP do Centro de [Dados Azure,](https://www.microsoft.com/download/details.aspx?id=41653)que são atualizadas semanalmente.
...
3. À esquerda, selecione **Azure Ative Directory**, clique em **Azure AD Connect**, e no centro selecione **Gerir o fornecimento (pré-visualização)**.
...
6. No ecrã de salpicos, **Aceite** os termos de licenciamento e clique em **Instalar**.

   ![Ecrã de Boas-Vindas](media/how-to-install/install1.png)
...
8. No ecrã **Connect Ative Diretório,** clique em **Adicionar diretório** e, em seguida, iniciar sessão com a sua conta de administrador de domínio de Diretório Ativo.  NOTA: A conta de administrador de domínio não deve ter requisitos de alteração de palavra-passe. Caso a palavra-passe expire ou mude, terá de reconfigurar o agente com as novas credenciais. Esta operação irá adicionar o seu diretório no local.  Clique em **Seguinte**.

   ![Ecrã de Boas-Vindas](media/how-to-install/install3.png)

9. No ecrã completo da **Configuração,** clique em **Confirmar**.  Esta operação registar-se-á e reiniciará o agente.

   ![Ecrã de Boas-Vindas](media/how-to-install/install4.png)
