...
1. Inscreva-se no servidor de união de domínio.  Se estiver a usar o tutorial  [de ambiente Basic A D e Azure,](tutorial-basic-ad-azure.md) seria DC1.
...
5.  Clique em **Nova Configuração** 
 ![ Screenshot do Azure A D Connect cloud sync screen com o link "Nova configuração" realçado.](media/tutorial-single-forest/configure-1.png)
...
![Screenshot do ecrã de sincronização de nuvem Azure A D Connect mostrando estado saudável.](media/how-to-configure/manage-4.png)
...
Irá agora verificar se os utilizadores que tinha no seu diretório no local foram sincronizados e agora existem no seu inquilino AZURE AD.  Esteja ciente de que isto pode levar algumas horas para ser concluído.  Para verificar se os utilizadores estão sincronizados, faça o seguinte.
...
4. Verifique se vê os novos utilizadores no seu inquilino</br>

## <a name="test-signing-in-with-one-of-your-users"></a>Teste de sessão com um dos seus utilizadores

1. Navegue para [https://myapps.microsoft.com](https://myapps.microsoft.com)
2. Inscreva-se com uma conta de utilizador que foi criada no seu inquilino.  Terá de iniciar sção utilizando o seguinte formato: user@domain.onmicrosoft.com (). Utilize a mesma palavra-passe que o utilizador utiliza para iniciar sins insição no local.</br>
   ![Verificação](media/tutorial-single-forest/verify-1.png)</br>

Agora, configura um ambiente de identidade híbrida com sucesso utilizando a sincronização de nuvem AD Connect Azure.
