...
![Relação entre os utilizadores finais, o Azure AD e aplicações publicadas](./media/application-proxy-configure-native-client-application/richclientflow.png)
...
1. Na barra lateral, selecione **do Azure Active Directory**. O **do Azure Active Directory** é apresentada a página de descrição geral.
1. Na barra lateral descrição geral do Azure AD, selecione **registos das aplicações**. É apresentada a lista de todos os registos de aplicações.
1. Selecione **novo registo**. O **registar uma aplicação** é apresentada a página.

   ![Criar um novo registo de aplicação no portal do Azure](./media/application-proxy-configure-native-client-application/create.png)

1. Na **nome** cabeçalho, especifique um nome a apresentar de destinada ao utilizador para a sua aplicação.
1. Sob o **tipos de conta suportados** de título, selecione um nível de acesso utilizando estas diretrizes:
...
1. Na **URI de redirecionamento** cabeçalho, selecione **cliente público (móvel e ambiente de trabalho)** , e, em seguida, escreva o URI de redirecionamento para a sua aplicação.
1. Selecione e leia os **políticas de plataforma da Microsoft**e, em seguida, selecione **registar**. Uma página de descrição geral para o novo registo de aplicação é criada e apresentada.
...
1. Selecione **adicionar uma permissão**. O **permissões de pedido de API** é apresentada a página.
1. Sob o **selecionar uma API** definição, selecione **APIs minha organização utiliza**. É apresentada uma lista que contém as aplicações no seu diretório que expõem APIs.
1. Escreva na caixa de pesquisa ou desloque-se para encontrar o proxy de aplicações que publicou no [passo 1: Publicar a aplicação de proxy](#step-1-publish-your-proxy-application)e, em seguida, selecione a aplicação do proxy.
1. Na **o tipo de permissões de seu aplicativo requer?** de título, selecione o tipo de permissão. Se precisar de seu aplicativo nativo aceder à API de aplicação de proxy como o utilizador com sessão iniciada, escolha **permissões delegadas**. Se seu aplicativo nativo é executado como um serviço em segundo plano ou daemon sem um utilizador com sessão iniciada, escolha **permissões de aplicação**.
1. Na **selecionar permissões** cabeçalho, selecione a permissão pretendida e selecione **adicionar permissões**. O **permissões API** página para seu aplicativo nativo agora mostra o proxy de aplicação e permissão da API que foi adicionado.
