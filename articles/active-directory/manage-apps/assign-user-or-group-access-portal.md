...
Para atribuir um utilizador ou grupo a uma aplicação empresarial, tem de ter as permissões adequadas para gerir a aplicação da empresa e tem de ser administrador global do diretório. For Microsoft Applications (por exemplo, aplicações do Office 365), utilize o PowerShell para atribuir utilizadores a uma aplicação empresarial.
...
1. Execute o comando `$sp.AppRoles` para apresentar as funções disponíveis para a aplicação de análise de área de trabalho. Neste exemplo, queremos atribuir a Eduarda Almeida a função de analista (acesso limitado).
   ...
   ![Mostra as funções disponíveis para um utilizador com função de análise de área de trabalho](./media/assign-user-or-group-access-portal/workplace-analytics-role.png)
