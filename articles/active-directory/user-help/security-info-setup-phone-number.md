...
5. Responda à chamada telefônica de verificação, enviada para o número de telefone que você inseriu e siga as instruções.
    ...
    Suas informações de segurança são atualizadas e você pode usar chamadas telefônicas para verificar sua identidade ao usar a verificação em duas etapas ou a redefinição de senha. Se pretender efazer chamadas telefónicas do seu método predefinido, consulte a secção de [métodos](#change-your-default-security-info-method) de segurança padrão alteração do artigo.
...
>[!Important]
>Se você excluir chamadas telefônicas por engano, não há como desfazê-las. Terá de adicionar o método novamente, seguindo os passos na secção de [chamadas telefónicas](#set-up-phone-calls-from-the-security-info-page) deste artigo.
