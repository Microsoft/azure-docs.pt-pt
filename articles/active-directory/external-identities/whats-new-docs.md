---
title: Novidades no Azure Ative Directory identidades externas
description: Documentação nova e atualizada para as identidades externas do Azure Ative Directory.
ms.date: 03/08/2021
ms.service: active-directory
ms.subservice: B2B
ms.topic: reference
ms.workload: identity
ms.author: mimart
author: msmimart
manager: CelesteDG
ms.openlocfilehash: dda3f7c8f4a9a21e16ebab7d243a004c01a40eed
ms.sourcegitcommit: 6386854467e74d0745c281cc53621af3bb201920
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 03/08/2021
ms.locfileid: "102453671"
---
# <a name="azure-active-directory-external-identities-whats-new"></a>Identidades externas do Azure Ative Directory: Quais as novidades

Bem-vindos às novidades da documentação de identidades externas do Azure Ative Directory. Este artigo lista novos docs que foram adicionados e aqueles que tiveram atualizações significativas nos últimos três meses. Para saber quais as novidades com o serviço de identidades externas, veja [as novidades no Azure Ative Directory](../fundamentals/whats-new.md).

## <a name="february-2021"></a>Fevereiro de 2021

### <a name="new-articles"></a>Artigos novos

- [Redefinir o estado de resgate de um utilizador convidado](reset-redemption-status.md)

### <a name="updated-articles"></a>Artigos atualizados

- [Azure Ative Directy B2B boas práticas](b2b-fundamentals.md)
- [Ativar a colaboração externa B2B e gerir quem pode convidar convidados](delegate-invitations.md)
- [Azure Ative Directory B2B colaboração FAQs](faq.md)
- [E-mail autenticação de senha única](one-time-passcode.md)
- [Resgate de convite para colaboração B2B do Azure Active Directory](redemption-experience.md)
- [Resolução de problemas Azure Ative Directy B2B colaboração](troubleshoot.md)
- [Propriedades de um utilizador de colaboração Azure Ative Directory B2B](user-properties.md)
- [O que é o acesso de utilizadores convidados no Azure Active Directory B2B?](what-is-b2b.md)
- [Identidades externas do Azure Ative Directory: Quais as novidades](whats-new-docs.md)
- [Permitir ou bloquear convites para utilizadores B2B de organizações específicas](allow-deny-list.md)
- [Azure Ative Directy B2B colaboração API e personalização](customize-invitation-api.md)
- [Convidar utilizadores internos para a colaboração B2B](invite-internal-users.md)
- [Partilha externa microsoft 365 e colaboração B2B do Azure Ative (Azure AD)](o365-external-user.md)
- [Federação direta com FS AD e fornecedores de terceiros para utilizadores convidados (pré-visualização)](direct-federation.md)

## <a name="january-2021"></a>Janeiro de 2021

### <a name="updated-articles"></a>Artigos atualizados
- [Permitir ou bloquear convites para utilizadores B2B de organizações específicas](allow-deny-list.md)
- [Como os utilizadores da sua organização podem convidar utilizadores convidados para uma aplicação](add-users-information-worker.md)


## <a name="december-2020"></a>Dezembro de 2020

### <a name="updated-articles"></a>Artigos atualizados
- [Azure Ative Directory B2B colaboração FAQs](faq.md)
- [Adicione o Google como um fornecedor de identidade para utilizadores convidados B2B](google-federation.md)
- [Fornecedores de Identidades Externas](identity-providers.md)
- [Resgate de convite para colaboração B2B do Azure Active Directory](redemption-experience.md)
- [Adicione um conector API a um fluxo de utilizador](self-service-sign-up-add-api-connector.md)
- [Adicione um fluxo de trabalho de aprovação personalizado à inscrição de self-service](self-service-sign-up-add-approvals.md)
- [Resolução de problemas Azure Ative Directy B2B colaboração](troubleshoot.md)
- [O que é o acesso de utilizadores convidados no Azure Active Directory B2B?](what-is-b2b.md)
- [Azure Ative Directy B2B boas práticas](b2b-fundamentals.md)
- [Ativar a colaboração externa B2B e gerir quem pode convidar convidados](delegate-invitations.md)
- [E-mail autenticação de senha única](one-time-passcode.md)

## <a name="november-2020"></a>Novembro de 2020

### <a name="updated-articles"></a>Artigos atualizados
- [Partilha externa microsoft 365 e colaboração B2B do Azure Ative (Azure AD)](o365-external-user.md)
- [Conceder aos recursos na cloud acesso às contas dos parceiros geridas localmente com a colaboração B2B do AAD](hybrid-on-premises-to-cloud.md)
- [Propriedades de um utilizador de colaboração Azure Ative Directory B2B](user-properties.md)


## <a name="october-2020"></a>Outubro de 2020

### <a name="updated-articles"></a>Artigos atualizados
- [Adicione o Google como um fornecedor de identidade para utilizadores convidados B2B](google-federation.md)
- [Como os utilizadores da sua organização podem convidar utilizadores convidados para uma aplicação](add-users-information-worker.md)
- [Permitir ou bloquear convites para utilizadores B2B de organizações específicas](allow-deny-list.md)
- [Azure Ative Directory B2B colaboração FAQs](faq.md)
- [Documentação sobre Identidades Externas](index.yml)
- [Resgate de convite para colaboração B2B do Azure Active Directory](redemption-experience.md)
- [Adicione um fluxo de trabalho de aprovação personalizado à inscrição de self-service](self-service-sign-up-add-approvals.md)
- [O que são identidades externas no Diretório Ativo Azure?](compare-with-b2c.md)
- [Adicione um conector API a um fluxo de utilizador](self-service-sign-up-add-api-connector.md)


## <a name="september-2020"></a>Setembro de 2020

### <a name="updated-articles"></a>Artigos atualizados
- [Os elementos do e-mail de convite de colaboração B2B - Azure Ative Directory](invitation-email-elements.md)
- [Resolução de problemas Azure Ative Directy B2B colaboração](troubleshoot.md)
- [Modelo de faturação para identidades externas Azure AD](external-identities-pricing.md)
- [Adicione o Google como um fornecedor de identidade para utilizadores convidados B2B](google-federation.md)


## <a name="august-2020"></a>Agosto de 2020

### <a name="new-articles"></a>Artigos novos
- [Modelo de faturação para identidades externas Azure AD](external-identities-pricing.md)


### <a name="updated-articles"></a>Artigos atualizados
- [Ativar a colaboração externa B2B e gerir quem pode convidar convidados](delegate-invitations.md)
- [Adicione um conector API a um fluxo de utilizador](self-service-sign-up-add-api-connector.md)
- [Adicione um fluxo de trabalho de aprovação personalizado à inscrição de self-service](self-service-sign-up-add-approvals.md)


