---
title: Restrições - Azure Active Directory e limites do serviço | Documentos da Microsoft
description: Restrições de utilização e outros limites de serviço para o serviço do Azure Active Directory
services: active-directory
documentationcenter: ''
author: curtand
manager: mtillman
editor: ''
ms.service: active-directory
ms.subservice: users-groups-roles
ms.topic: article
ms.workload: identity
ms.date: 02/21/2019
ms.author: curtand
ms.custom: aaddev;it-pro
ms.reviewer: jeffsta
ms.collection: M365-identity-device-management
ms.openlocfilehash: 1191fb5c9aefb55896d5c44714f1061ff2674a8d
ms.sourcegitcommit: d4dfbc34a1f03488e1b7bc5e711a11b72c717ada
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 06/13/2019
ms.locfileid: "64415208"
---
# <a name="azure-ad-service-limits-and-restrictions"></a>Restrições e limites de serviço do Azure AD
Este artigo contém as restrições de utilização e outros limites de serviço para o serviço do Azure Active Directory (Azure AD). Se estiver procurando o conjunto completo de limites de serviço do Microsoft Azure, veja [subscrição do Azure e limites do serviço, Quotas e restrições](../../azure-subscription-service-limits.md).

[!INCLUDE [AAD-service-limits](../../../includes/active-directory-service-limits-include.md)]

## <a name="next-steps"></a>Passos Seguintes
* [Inscrever-se no Azure como uma organização](../fundamentals/sign-up-organization.md)
* [Como as subscrições do Azure são associadas ao Azure AD](../fundamentals/active-directory-how-subscriptions-associated-directory.md)
