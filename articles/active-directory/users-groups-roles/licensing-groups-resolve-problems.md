...
Quando você atribui licenças diretamente a usuários individuais, sem usar o licenciamento baseado em grupo, a operação de atribuição pode falhar. Por exemplo, quando você executa o cmdlet `Set-MsolUserLicense` do PowerShell em um sistema de usuário, o cmdlet pode falhar por vários motivos relacionados à lógica de negócios. Por exemplo, pode haver um número insuficiente de licenças ou um conflito entre dois planos de serviço que não podem ser atribuídos ao mesmo tempo. O problema é imediatamente relatado de volta para você.
...
1. Para localizar os usuários em um estado de erro em um grupo específico, abra o painel do grupo. Em **licenças**, uma notificação será exibida se houver usuários em um estado de erro.
...
3. Para localizar todos os grupos que contêm pelo menos um erro, na folha **Azure Active Directory** , selecione **licenças**e, em seguida, selecione **visão geral**. Uma caixa de informações é exibida quando os grupos exigem sua atenção.

   ![Visão geral e informações sobre grupos em estado de erro](./media/licensing-groups-resolve-problems/group-errors-widget.png)

4. Marque a caixa para ver uma lista de todos os grupos com erros. Você pode selecionar cada grupo para obter mais detalhes.
...
**Persisti** Não há licenças suficientes disponíveis para um dos produtos especificados no grupo. Você precisa comprar mais licenças para o produto ou liberar licenças não utilizadas de outros usuários ou grupos.
...
**Persisti** Ao atualizar a atribuição de licença em um usuário ou grupo, você pode ver que o atributo email e ProxyAddresses do Azure AD de alguns usuários são alterados.
...
## <a name="how-do-you-manage-licenses-for-products-with-prerequisites"></a>Como gerenciar licenças para produtos com pré-requisitos?
...
Se tentarmos atribuir esse produto por conta própria a um grupo, o portal retornará um erro. A seleção da notificação de erro mostra os seguintes detalhes:

![Grupo, pré-requisito ausente](./media/licensing-groups-resolve-problems/group-prerequisite-required.png)

Se selecionarmos os detalhes, ele mostrará a seguinte mensagem de erro:
...
Também é possível criar um grupo autônomo que contenha apenas os produtos mínimos necessários para que o complemento funcione. Ele pode ser usado para licenciar apenas os usuários selecionados para o produto de complemento. Neste exemplo, atribuímos os seguintes produtos ao mesmo grupo:
...
## <a name="how-do-you-force-license-processing-in-a-group-to-resolve-errors"></a>Como você força o processamento de licenças em um grupo a resolver erros?
...
## <a name="how-do-you-force-license-processing-on-a-user-to-resolve-errors"></a>Como você força o processamento de licenças em um usuário a resolver erros?
