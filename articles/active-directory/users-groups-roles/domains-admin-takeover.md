...
Alguns produtos que incluem o SharePoint e o OneDrive, como o Office 365, não suportam a aquisição externa. Se esse é o seu cenário, ou se você é um administrador e quer assumir uma organização aD Azure não gerida ou "sombra" criada por utilizadores que usaram a inscrição de self-service, você pode fazê-lo com uma aquisição interna de administração.

1. Crie um contexto de utilizador na organização não gerida através da inscrição no Power BI. Por conveniência, por exemplo, estes passos assumem esse caminho.
...
4. Inscreva-se no centro de administração da [Microsoft 365](https://portal.office.com/admintakeover) com a conta de utilizador do Power BI. Recebe uma mensagem que o instrui a **tornar-se o Administrador** do nome de domínio que já foi verificado na organização não gerida. selecione **Sim, quero ser o administrador.**
...
Quando os registos DNS TXT forem verificados no seu registo de nome de domínio, pode gerir a organização Azure AD.

Quando completar os passos anteriores, é agora o administrador global da organização do Quarto Café no Office 365. Para integrar o nome de domínio com os seus outros serviços Azure, pode removê-lo do Office 365 e adicioná-lo a uma organização gerida diferente em Azure.
...
3. Certifique-se de que a nova conta de utilizador tem privilégios administrativos globais para a organização Azure AD.
...
6. Inscreva-se no centro de [administração da Azure AD](https://portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/Overview) com uma conta que é a administração global da organização Azure AD.
...
> [!NOTE]
> Qualquer utilizador do serviço Power BI ou Azure Rights Management que tenha licenças atribuídas na organização office 365 deve guardar os seus dashboards se o nome de domínio for removido. Devem iniciar sessão com um nome de utilizador como *utilizador\@fourthcoffeexyz.onmicrosoft.com* em vez de utilizador *\@quartocafé.xyz*.

## <a name="external-admin-takeover"></a>Aquisição externa de administradores

Se já gere uma organização com serviços Azure ou Office 365, não pode adicionar um nome de domínio personalizado se já for verificado noutra organização da Azure AD. No entanto, a partir da sua organização gerida em Azure AD você pode assumir uma organização não gerida como uma aquisição de administração externa. O procedimento geral segue o artigo [Adicione um domínio personalizado ao Azure AD](../fundamentals/add-custom-domain.md).
...
Você pode opcionalmente usar a [opção **ForceTakeover** ](#azure-ad-powershell-cmdlets-for-the-forcetakeover-option) para remover o nome de domínio da organização não gerida e verificar na organização desejada. 

#### <a name="more-information-about-rms-for-individuals"></a>Mais informações sobre RMS para indivíduos

Para [o RMS para indivíduos](/azure/information-protection/rms-for-individuals), quando a organização não gerida está na mesma região que a organização que possui, a chave da organização de [proteção de informação Azure](/azure/information-protection/plan-implement-tenant-key) criada automaticamente e [os modelos](/azure/information-protection/configure-usage-rights#rights-included-in-the-default-templates) de proteção padrão são adicionalmente movidos com o nome de domínio.

A chave e os modelos não são movidos quando a organização não gerida está numa região diferente. Por exemplo, se a organização não gerida estiver na Europa e a organização que possui estiver na América do Norte.
...
`get-msoldomain` | Mostra os seus nomes de domínio associados à organização atual.
