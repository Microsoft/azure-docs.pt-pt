...
- [Crie uma definição de *função* personalizada](roles-create-custom.md) e [adicione-lhe permissões a partir de uma lista pré-configurada](roles-custom-available-permissions.md). Estas são as mesmas permissões usadas nas funções incorporadas.
- [Crie uma *tarefa de função* ](roles-assign-powershell.md) para atribuir o papel personalizado.
