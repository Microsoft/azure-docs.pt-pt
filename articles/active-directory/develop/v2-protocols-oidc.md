...
Normalmente, usaria este documento de metadados para configurar uma biblioteca de OpenID Connect ou SDK; a biblioteca usaria os metadados para realizar seu trabalho. No entanto, se não estiver a utilizar uma biblioteca de ligação OpenID pré-criadas, pode seguir os passos no restante deste artigo efetuar início de sessão numa aplicação web com o ponto final v2.0.
...
| domain_hint |Opcional | O realm do utilizador num diretório federado.  Isso ignora o processo de descoberta baseada em e-mail que o utilizador passa por na versão 2.0 início de sessão página, uma experiência de usuário ligeiramente mais simplificado. Para inquilinos que estão federados por meio de um diretório no local, como o ADFS, isso normalmente resulta num totalmente integrado início de sessão devido a sessão de início de sessão existente. |
