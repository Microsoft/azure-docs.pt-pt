...
Consulte o [aplicativo Web que entra em usuários](scenario-web-app-sign-user-overview.md) para obter detalhes
...
No aplicativo Web, para **chamar a API da Web** em nome do usuário, você usará o MSAL `ConfidentialClientApplication`. Você usará o fluxo de código de autorização, armazenando o token adquirido no cache de token. Em seguida, o controlador adquirirá tokens silenciosamente do cache, quando necessário. O MSAL atualiza o token, se necessário.

Para obter detalhes, consulte [APIs da Web de chamadas de aplicativo Web](scenario-web-app-call-api-overview.md)
...
Finalmente, e embora não seja recomendado, você pode usar o [nome de usuário/senha](https://aka.ms/msal-net-up) em aplicativos cliente públicos. Esse fluxo ainda é necessário em alguns cenários (como DevOps), mas lembre-se de que usá-lo irá impor restrições em seu aplicativo. Por exemplo, os aplicativos que usam esse fluxo não poderão entrar no usuário que precisa executar a autenticação multifator (acesso condicional). Ele não permitirá que seu aplicativo se beneficie de um logon único. Ele também está relacionado aos princípios da autenticação moderna e é fornecido apenas por motivos herdados.
...
Consulte [aplicativo de área de trabalho que chama APIs da Web](scenario-desktop-overview.md) para obter detalhes
...
Como para aplicativos da área de trabalho, para adquirir um token para chamar uma API da Web, um aplicativo móvel usará os métodos de aquisição de tokens interativos do MSAL PublicClientApplication's. No iOS e no Android, o MSAL, por padrão, usa o navegador da Web do sistema. Mas você pode direcioná-lo para usar a exibição da Web inserida. Existem especificidades, dependendo da plataforma móvel: (UWP, iOS, Android).
...
Consulte [aplicativo móvel que chama APIs da Web](scenario-mobile-overview.md) para obter detalhes
...
Consulte [API Web protegida](scenario-protected-web-api-overview.md) para obter detalhes
...
Consulte [API Web que chama APIs da Web](scenario-web-api-call-api-overview.md) para obter detalhes
...
| [Aplicativo Web que assina usuários](scenario-web-app-sign-user-overview.md) <br/>[![Aplicativo Web que faz logon de usuários](media/scenarios/scenario-webapp-signs-in-users.svg)](scenario-web-app-sign-user-overview.md) | ![ASP.NET](media/sample-v2-code/logo_NET.png)</br> ASP.NET ![Núcleo de ASP.NET](media/sample-v2-code/logo_NETcore.png)Núcleo de ASP.NET | ![Núcleo de ASP.NET](media/sample-v2-code/logo_NETcore.png)Núcleo de ASP.NET | ![Núcleo de ASP.NET](media/sample-v2-code/logo_NETcore.png)Núcleo de ASP.NET
| [Aplicativo Web que chama APIs da Web](scenario-web-app-call-api-overview.md) <br/> [![Aplicativo Web que chama APIs da Web](media/scenarios/web-app.svg)](scenario-web-app-call-api-overview.md) | ![ASP.NET](media/sample-v2-code/logo_NET.png) </br> ASP.NET + MSAL.NET </br> ![Núcleo de ASP.NET](media/sample-v2-code/logo_NETcore.png)ASP.NET Core + MSAL.NET ![MSAL Java](media/sample-v2-code/logo_java.png) msal4j ![MSAL Python](media/sample-v2-code/logo_python.png) Flask + MSAL Python| ![Núcleo de ASP.NET](media/sample-v2-code/logo_NETcore.png)ASP.NET Core + MSAL.NET ![MSAL Java](media/sample-v2-code/logo_java.png) msal4j ![MSAL Python](media/sample-v2-code/logo_python.png) Flask + MSAL Python| ![Núcleo de ASP.NET](media/sample-v2-code/logo_NETcore.png)ASP.NET Core + MSAL.NET ![MSAL Java](media/sample-v2-code/logo_java.png) msal4j ![MSAL Python](media/sample-v2-code/logo_python.png) Flask + MSAL Python
