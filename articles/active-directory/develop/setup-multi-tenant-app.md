---
title: Configure uma nova aplicação multi-inquilino
description: Saiba como configurar uma aplicação como multi-inquilino, e como as aplicações multi-inquilinos funcionam
services: active-directory
author: rwike77
manager: CelesteDG
ms.service: active-directory
ms.subservice: develop
ms.custom: aaddev
ms.workload: identity
ms.topic: conceptual
ms.date: 07/15/2019
ms.author: ryanwi
ROBOTS: NOINDEX
ms.openlocfilehash: 9c6272901600886958d4339d067ca75bb9a73793
ms.sourcegitcommit: 910a1a38711966cb171050db245fc3b22abc8c5f
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 03/19/2021
ms.locfileid: "101649126"
---
# <a name="how-to-configure-a-new-multi-tenant-application"></a>Como configurar uma nova aplicação multi-inquilino

Aqui está uma lista de tópicos recomendados para saber mais sobre aplicações multi-inquilinos:

- Tenha uma compreensão geral do [que significa ser uma aplicação multi-inquilino](./developer-glossary.md#multi-tenant-application)
- Obtenha uma compreensão geral de [como configurar uma aplicação para ser multi-inquilino](./howto-convert-app-to-be-multi-tenant.md)
- Obtenha uma visão geral passo a passo de como o quadro de [consentimento Azure AD é usado para implementar o consentimento](./quickstart-register-app.md), que é necessário para aplicações multi-inquilinos
- Para obter mais profundidade, saiba [como uma aplicação multi-arrendatário é configurada e codificada de ponta a ponta,](./howto-convert-app-to-be-multi-tenant.md)incluindo como registar- se, usar o ponto final "comum", implementar o consentimento "utilizador" e "administrador", como implementar cenários multi-camadas mais avançados

## <a name="next-steps"></a>Passos seguintes
[AzureAD Microsoft Q&A](/answers/topics/azure-active-directory.html)