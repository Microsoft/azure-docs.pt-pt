...
> [!NOTE]
> Este artigo assume que já está familiarizado com a construção de um único pedido de inquilino para a Azure AD. Se não estiver, comece com um dos arranques rápidos na [página inicial][AAD-Dev-Guide]do guia do desenvolvedor .

Existem quatro passos simples para converter a sua aplicação numa aplicação multi-inquilina Azure AD:
...
> [!NOTE]
> Os registos de clientes nativos, bem como [as aplicações da plataforma de identidade da Microsoft,](./active-directory-appmodel-v2-overview.md) são multi-inquilinos por padrão. Não precisa de tomar nenhuma medida para tornar estes registos de inscrição multi-inquilino.
...
Quando um único pedido de inquilino valida um token, verifica a assinatura do token com as chaves de assinatura do documento de metadados. Este teste permite-lhe certificar-se de que o valor do emitente no token corresponde ao que foi encontrado no documento de metadados.
...
Para que um utilizador inscreva-se numa aplicação em Azure AD, a aplicação deve ser representada no arrendatário do utilizador. Isto permite que a organização faça coisas como aplicar políticas únicas quando os utilizadores do seu inquilino se inscrevem na aplicação. Para um único pedido de inquilino, este registo é simples; é o que acontece quando regista a inscrição no [portal Azure.][AZURE-portal]
