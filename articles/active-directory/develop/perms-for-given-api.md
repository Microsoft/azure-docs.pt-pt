---
title: Selecionar permissões para uma determinada API
description: Saiba como as permissões solicitam que os pedidos de clientes e recursos para aplicações que está a desenvolver
services: active-directory
author: rwike77
manager: CelesteDG
ms.service: active-directory
ms.subservice: develop
ms.custom: aaddev
ms.workload: identity
ms.topic: conceptual
ms.date: 07/15/2019
ms.author: ryanwi
ROBOTS: NOINDEX
ms.openlocfilehash: fe6e7277761c670d5af8743e6330ffe2340d235c
ms.sourcegitcommit: f28ebb95ae9aaaff3f87d8388a09b41e0b3445b5
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 03/29/2021
ms.locfileid: "101647460"
---
# <a name="how-to-select-permissions-for-a-given-api"></a>Como selecionar permissões para uma determinada API

## <a name="recommended-documents"></a>Documentos recomendados

- Saiba mais sobre como as aplicações do cliente usam [pedidos de permissão de pedidos delegados e de permissão](./developer-glossary.md#permissions) de aplicação para aceder a recursos.
- Consulte instruções passo a passo sobre como [configurar os pedidos de permissão de um pedido de cliente](./quickstart-configure-app-access-web-apis.md)
- Para obter mais profundidade, saiba como as aplicações de recursos expõem [os âmbitos](./developer-glossary.md#scopes) e funções de [aplicação](./developer-glossary.md#roles) às aplicações dos clientes, que se manifestam como permissões delegadas e de candidatura, respectivamente, no portal Azure.

## <a name="next-steps"></a>Passos seguintes

[AzureAD Microsoft Q&A](/answers/topics/azure-active-directory.html)