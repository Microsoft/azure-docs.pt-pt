...
Dependendo da forma como o seu cliente é construído, pode utilizar um ou mais dos fluxos de autenticação suportados pela plataforma de identidade da Microsoft. Estes fluxos podem produzir vários tipos de fichas, bem como códigos de autorização, e requerem fichas diferentes para fazê-los funcionar.
...
A MSAL pode interativamente solicitar ao utilizador que as suas credenciais assinem e obtenham um símbolo utilizando essas credenciais.

![Diagrama de fluxo interativo](media/msal-authentication-flows/interactive.png)

Para obter mais informações sobre a aquisição interativa de fichas em plataformas específicas, consulte:

- [MSAL.js pedidos interativos](msal-js-prompt-behavior.md)
...
A MSAL apoia o fluxo [de subvenção implícito OAuth 2.](v2-oauth2-implicit-grant-flow.md) Este fluxo permite que a aplicação obtenha fichas da plataforma de identidade da Microsoft sem realizar uma troca de credenciais de servidor de back-end. Este fluxo permite que a aplicação assine no utilizador, mantenha uma sessão e obtenha fichas para outras APIs da web, tudo dentro do código JavaScript do cliente.
...
A MSAL apoia o fluxo [de concessão de código de autorização OAuth 2.](v2-oauth2-auth-code-flow.md) Este fluxo pode ser utilizado em apps que são instaladas num dispositivo para ter acesso a recursos protegidos como APIs web. Isto permite-lhe adicionar acesso de início de s-in e API às suas aplicações móveis e desktop.
...
A MSAL suporta o [fluxo de autenticação OAuth 2 em nome.](v2-oauth2-on-behalf-of-flow.md) Este fluxo é utilizado quando uma aplicação invoca um serviço ou API web que, por sua vez, precisa de chamar outro serviço ou API web. A ideia é propagar a identidade e permissões de utilizador delegadas através da cadeia de pedidos. Para que o serviço de nível médio faça pedidos autenticados para o serviço a jusante, este necessita de garantir um sinal de acesso a partir da plataforma de identidade da Microsoft *em nome do* utilizador.
...
A MSAL suporta o fluxo de código do [dispositivo OAuth 2](v2-oauth2-device-code.md) que permite que os utilizadores entrem em dispositivos com restrições de entrada, como smart TVs, dispositivos IoT e impressoras. A autenticação interativa com Azure AD requer um navegador web. Quando o dispositivo ou sistema operativo não fornece um navegador web, o fluxo de código do dispositivo permite que o utilizador utilize outro dispositivo como um computador ou telemóvel para assinar interativamente.
...
A MSAL suporta a [concessão de credenciais de senha do proprietário de recursos OAuth 2](v2-oauth-ropc.md) (ROPC), que permite que uma aplicação assine no utilizador manuseando diretamente a sua palavra-passe. Na sua aplicação para desktop, pode utilizar o username/password flow para adquirir um símbolo silenciosamente. Não é necessária UI ao utilizar a aplicação.
