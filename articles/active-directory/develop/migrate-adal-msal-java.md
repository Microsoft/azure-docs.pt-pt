---
title: Guia de migração do ADAL para o MSAL para Java – plataforma de identidade da Microsoft | Azure
...
O MSAL para Java (MSAL4J) é a biblioteca de autenticação que recomendamos que você use com a plataforma de identidade da Microsoft. Nenhum recurso novo será implementado em ADAL4J. Todos os esforços que avançam estão concentrados em melhorar o MSAL.
...
O ADAL4J adquire tokens para recursos, enquanto o MSAL4J adquire tokens para escopos. Várias classes MSAL4J exigem um parâmetro de escopos. Esse parâmetro é uma lista de cadeias de caracteres que declaram as permissões e os recursos desejados que são solicitados. Consulte [escopos de Microsoft Graph](https://docs.microsoft.com/graph/permissions-reference) para ver os escopos de exemplo.

## <a name="core-classes"></a>Classes principais

No ADAL4J, a classe `AuthenticationContext` representa sua conexão com o serviço de token de segurança (STS) ou servidor de autorização por meio de uma autoridade. No entanto, o MSAL4J é projetado em relação aos aplicativos cliente. Ele fornece duas classes separadas: `PublicClientApplication` e `ConfidentialClientApplication` para representar aplicativos cliente.  A última, `ConfidentialClientApplication`, representa um aplicativo projetado para manter um segredo com segurança, como um identificador de aplicativo para um aplicativo daemon.

A tabela a seguir mostra como o ADAL4J Functions é mapeado para as novas funções MSAL4J:
...
MSAL4J define o conceito de conta por meio da interface `IAccount`. Essa é uma alteração significativa do ADAL4J, mas é uma boa porque captura o fato de que o mesmo usuário pode ter várias contas, e talvez até mesmo em diretórios diferentes do Azure AD. O MSAL4J fornece mais informações em cenários de convidado, pois as informações da conta doméstica são fornecidas.

## <a name="cache-persistence"></a>Persistência de cache

ADAL4J não tinha suporte para cache de token.
O MSAL4J adiciona um [cache de token](msal-acquire-cache-tokens.md) para simplificar o gerenciamento de tempos de vida de token atualizando automaticamente os tokens expirados quando possível e impedindo solicitações desnecessárias para que o usuário forneça credenciais quando possível.
