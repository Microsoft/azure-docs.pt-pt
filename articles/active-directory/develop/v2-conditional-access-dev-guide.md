...
Quando a sua aplicação tenta aceder a um serviço com uma política de Acesso Condicional, pode encontrar um desafio de Acesso Condicional. Este desafio está codificado no `claims` parâmetro que vem numa resposta da Azure AD. Aqui está um exemplo deste parâmetro de desafio:
...
Na Web API 1, apanhamos o erro `error=interaction_required` e enviamos de volta o desafio para a aplicação de ambiente de `claims` trabalho. Nessa altura, a aplicação de desktop pode fazer uma nova chamada e anexar o desafio como um parâmetro de `acquireToken()` corda extra de `claims` consulta. Este novo pedido requer que o utilizador faça a autenticação de vários fatores e, em seguida, envie este novo token de volta para a Web API 1 e complete o fluxo em nome do fluxo.
...
Em MSAL.js, existem algumas funções que obtêm fichas: `loginPopup()` `acquireTokenSilent(...)` , , e `acquireTokenPopup(…)` `acquireTokenRedirect(…)` .
...
Quando uma aplicação precisa de um sinal de acesso para chamar uma API web, tenta `acquireTokenSilent(…)` um . Se a sessão do token expirar ou tivermos de cumprir uma política de Acesso Condicional, então a função *token adquire* falha e a aplicação utiliza `acquireTokenPopup()` ou `acquireTokenRedirect()` .
...
Vamos dar um exemplo com o nosso cenário de Acesso Condicional. O utilizador final acabou de aterrar no site e não tem uma sessão. Fazemos uma `loginPopup()` chamada, recebemos um símbolo de identificação sem autenticação de vários fatores. Em seguida, o utilizador acerta num botão que requer que a app solicite dados de uma API web. A aplicação tenta fazer uma chamada mas falha uma vez que o utilizador ainda não realizou a `acquireTokenSilent()` autenticação de vários fatores e precisa de cumprir a política de Acesso Condicional.
