...
Se tem uma aplicação Xamarin Android que utiliza atualmente a Biblioteca de Autenticação do Diretório Ativo Azure para .NET (ADAL.NET) e um corretor de [autenticação,](msal-android-single-sign-on.md)é hora de migrar para a Biblioteca de Autenticação do [Microsoft para .NET ](msal-overview.md) (MSAL.NET).
