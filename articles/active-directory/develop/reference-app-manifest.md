...
1. Selecione o **do Azure Active Directory** serviço e, em seguida, selecione **registos das aplicações**.
...
| `publicClient` | Boolean | Especifica se esta aplicação é um cliente público (por exemplo, um aplicativo instalado em execução num dispositivo móvel). <br><br> _Nota: Trata-se disponível apenas na experiência de registos (legada) de aplicação. Substituído por `allowPublicClient` no [registos das aplicações](https://go.microsoft.com/fwlink/?linkid=2083908) experiência._ | |
| `publisherDomain` | String | O domínio verificado do publicador para a aplicação. Só de leitura. | https://www.contoso.com |
| `replyUrls` | Matriz de cadeia de caracteres | Esta propriedade de valor múltiplos contém a lista de valores de redirect_uri registados que do Azure AD aceitará como destinos ao retornar tokens. <br><br> _Nota: Trata-se disponível apenas na experiência de registos (legada) de aplicação. Substituído por `replyUrlsWithType` no [registos das aplicações](https://go.microsoft.com/fwlink/?linkid=2083908) experiência._ | |
...
1. Se o seu fluxo de trabalho tem de guardar os manifestos no seu repositório de origem para utilizar mais tarde, sugerimos que rebasing os manifestos guardados no seu repositório com aquele que pode ver na **registos das aplicações** experiência.
