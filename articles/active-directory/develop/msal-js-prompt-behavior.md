...
A biblioteca MSAL.js (a partir de v0.2.4) não envia um parâmetro rápido durante os pedidos interativos (`loginRedirect`, `loginPopup`, `acquireTokenRedirect` e `acquireTokenPopup`), pelo que não impõe qualquer comportamento rápido. Para pedidos de token silenciosos utilizando o método `acquireTokenSilent`, o MSAL.js passa um parâmetro rápido definido para `none`.
...
**nenhuma:** Este valor irá garantir que o utilizador não vê qualquer solicitação interativa. Recomenda-se que não passe este valor para métodos interativos em MSAL.js, uma vez que pode ter comportamentos inesperados. Em vez disso, utilize o método `acquireTokenSilent` para conseguir chamadas silenciosas.

## <a name="next-steps"></a>Passos seguintes

Leia mais sobre o parâmetro `prompt` no protocolo de [subvenção implícita OAuth 2.0](v2-oauth2-implicit-grant-flow.md) que a biblioteca MSAL.js utiliza.
