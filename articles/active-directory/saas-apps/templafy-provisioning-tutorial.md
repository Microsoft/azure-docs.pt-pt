...
* Recomenda-se que um único utilizador da AD Azure seja atribuído à Templafy para testar a configuração automática de fornecimento do utilizador. Usuários e/ou grupos adicionais podem ser atribuídos posteriormente.
...
5. No âmbito da secção **de Credenciais de Administrador,** a entrada `https://scim.templafy.com/scim` no **URL do Arrendatário**. Insere o valor da **chave SCIM API** recuperado anteriormente em **Secret Token**. Clique na **ligação de teste** para garantir que o Azure AD pode ligar-se à Templafy. Se a ligação falhar, certifique-se de que a sua conta Templafy tem permissões de administrador e tente novamente.
