...
* Recomenda-se que um único utilizador da AD Azure seja atribuído ao Soloinsight-CloudGate SSO para testar a configuração automática de fornecimento de utilizadores. Usuários e/ou grupos adicionais podem ser atribuídos posteriormente.
...
1. Inscreva-se na consola [De administrador Soloinsight-CloudGate SSO](https://soloinsight.sigateway.com/login). Navegar para As Definições de **Sistema de Administração e Sistema**.
...
5. No âmbito da secção **de Credenciais de Administrador,** a entrada `https://sigateway.com/scim/v2/sync/serviceproviderconfig` no **URL do Arrendatário**. Insera o valor token de **autenticação SCIM** recuperado anteriormente em **Ficha Secreta**. Clique em **Ligação de Teste** para garantir que o Azure AD pode ligar-se ao SSO Soloinsight-CloudGate. Se a ligação falhar, certifique-se de que a sua conta SSO Soloinsight-CloudGate tem permissões de administrador e tente novamente.
