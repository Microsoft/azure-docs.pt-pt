...
* Recomenda-se que um único utilizador da AD Azure seja atribuído ao Zscaler Private Access (ZPA) para testar a configuração automática de fornecimento de utilizadores. Usuários e/ou grupos adicionais podem ser atribuídos posteriormente.
...
1. Inscreva-se na consola [de acesso privado zscaler (ZPA)](https://admin.private.zscaler.com/). Navegue para **A Configuração De Administração > IdP**.
...
8.  Para localizar o URL do **Inquilino,** navegue para **a Configuração Da Administração > IdP**. Clique no nome da configuração idp recém-adicionada listada na página.
