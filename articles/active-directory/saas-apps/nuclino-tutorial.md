...
Neste tutorial, você aprenderá a integrar o Nuclino com o Azure Active Directory (Azure AD).
A integração do Nuclino ao Azure AD oferece os seguintes benefícios:

* No Azure AD, é possível controlar quem tem acesso ao Nuclino.
* Você pode permitir que seus usuários sejam conectados automaticamente ao Nuclino (logon único) com suas contas do Azure AD.
...
Para configurar a integração do Azure AD ao Nuclino, você precisará dos seguintes itens:
...
Nesta seção, você configurará e testará o logon único do Azure AD com o Nuclino, com base em um usuário de teste chamado **Brenda Simon**.
Para que o logon único funcione, é necessário estabelecer uma relação de vínculo entre um usuário do Azure AD e o usuário relacionado no Nuclino.
...
2. **[Configurar o logon único do Nuclino](#configure-nuclino-single-sign-on)** – para configurar as configurações de logon único no lado do aplicativo.
3. **[Criar um utilizador de teste do Azure AD](#create-an-azure-ad-test-user)**  - para testar o Azure AD início de sessão único com Eduarda Almeida.
4. **[Atribua o utilizador de teste do Azure AD](#assign-the-azure-ad-test-user)**  - para ativar a Eduarda Almeida utilizar o Azure AD início de sessão único.
5. **[Criar usuário de teste do Nuclino](#create-nuclino-test-user)** – para ter um equivalente de Brenda Simon no Nuclino que esteja vinculado à representação do usuário no Azure AD.
...
1. Na [portal do Azure](https://portal.azure.com/), na página de integração de aplicativos do **Nuclino** , selecione **logon único**.
...
4. Na seção **configuração básica do SAML** , se você quiser configurar o aplicativo no modo iniciado pelo **IDP** , execute as seguintes etapas:
    ...
    ![Informações de logon único de domínio e URLs do Nuclino](common/idp-intiated.png)
...
5. Clique em **definir URLs adicionais** e execute a seguinte etapa se desejar configurar o aplicativo no modo iniciado pelo **SP** :
    ...
    ![Informações de logon único de domínio e URLs do Nuclino](common/metadata-upload-additional-signon.png)

    Na caixa de texto **URL de logon** , digite uma URL usando o seguinte padrão:`https://app.nuclino.com/<UNIQUE-ID>/login`

    > [!NOTE]
    > Esse valor não é real. Atualize esse valor com a URL de logon real. Contate a [equipe de suporte ao cliente do Nuclino](mailto:contact@nuclino.com) para obter esse valor. Você também pode consultar os padrões mostrados na seção **configuração básica do SAML** no portal do Azure.

6. O aplicativo Nuclino espera as asserções SAML em um formato específico, o que exige que você adicione mapeamentos de atributo personalizados à sua configuração de atributos de token SAML. A captura de tela a seguir mostra a lista de atributos padrão. Clique em **Editar** ícone para abrir a caixa de diálogo **atributos de usuário** .

    ![image](common/edit-attribute.png)

7. Além de acima, o aplicativo Nuclino espera que mais alguns atributos sejam passados de volta na resposta SAML. Na seção **declarações do usuário** , na caixa de diálogo **atributos de usuário** , execute as seguintes etapas para adicionar o atributo de token SAML, conforme mostrado na tabela abaixo:
...
9. Na seção **Configurar Nuclino** , copie as URLs apropriadas de acordo com seu requisito.
...
1. Em uma janela diferente do navegador da Web, entre no site da empresa do Nuclino como um administrador.

2. Clique no **ícone**.

    ![Configuração do Nuclino](./media/nuclino-tutorial/configure1.png)

3. Clique no **SSO do Azure ad** e selecione **configurações da equipe** na lista suspensa.
...
Nesta seção, você permitirá que Brenda Simon use o logon único do Azure concedendo acesso ao Nuclino.

1. Na portal do Azure, selecione **aplicativos empresariais**, selecione **todos os aplicativos**e, em seguida, selecione **Nuclino**.
...
Nesta seção, um usuário chamado Brenda Simon é criado em Nuclino. O Nuclino dá suporte ao provisionamento de usuário just-in-time, que é habilitado por padrão. Não há nenhum item de ação para você nesta seção. Se um usuário ainda não existir no Nuclino, um novo será criado após a autenticação.
