...
* Pode permitir que os utilizadores ser automaticamente sessão iniciada para Zscaler (Single Sign-On) com as suas contas do Azure AD.
...
* Zscaler logon único habilitado subscrição

## <a name="scenario-description"></a>Descrição do cenário

Neste tutorial, configure e teste do Azure AD início de sessão único num ambiente de teste.

* Suporta Zscaler **SP** iniciada SSO

* Suporta Zscaler **Just In Time** aprovisionamento de utilizadores
...
Nesta secção, configure e teste do Azure AD início de sessão único com o Zscaler com base num utilizador de teste **Eduarda Almeida**.
Para o início de sessão único funcionar, uma relação de ligação entre um utilizador do Azure AD e o utilizador relacionado no Zscaler deve ser estabelecido.
...
2. **[Configurar Zscaler Single Sign-On](#configure-zscaler-single-sign-on)**  - para configurar as definições de início de sessão único no lado do aplicativo.
3. **[Criar um utilizador de teste do Azure AD](#create-an-azure-ad-test-user)**  - para testar o Azure AD início de sessão único com Eduarda Almeida.
4. **[Atribua o utilizador de teste do Azure AD](#assign-the-azure-ad-test-user)**  - para ativar a Eduarda Almeida utilizar o Azure AD início de sessão único.
5. **[Criar utilizador de teste do Zscaler](#create-zscaler-test-user)**  - para ter um equivalente da Eduarda Almeida na Zscaler que está ligado à representação de utilizador do Azure AD.
...
1. Na [portal do Azure](https://portal.azure.com/), na **Zscaler** página de integração de aplicações, selecione **início de sessão único**.
...
4. Sobre o **configuração básica de SAML** secção, execute os seguintes passos:
    ...
    ![Zscaler domínio e URLs únicas início de sessão em informações](common/sp-signonurl.png)

    Na **URL de início de sessão** caixa de texto, escreva um URL com o seguinte padrão:  `https://<companyname>.zscaler.net`

    > [!NOTE]
    > O valor não é real. Atualize o valor com o URL de início de sessão real. Contacte [equipa de suporte de cliente do Zscaler](https://www.zscaler.com/company/contact) para obter o valor. Também pode consultar os padrões mostrados a **configuração básica de SAML** secção no portal do Azure.

5. Seu aplicativo Zscaler espera que as asserções SAML num formato específico, o que requer a adição de mapeamentos de atributos personalizado à sua configuração de atributos de token SAML. Captura de ecrã seguinte mostra a lista de atributos predefinidos. Clique em **edite** ícone para abrir **atributos do utilizador** caixa de diálogo.

    ![image](common/edit-attribute.png)

6. Além dos acima, o aplicativo Zscaler espera mais alguns atributos a serem passados na resposta SAML. No **afirmações de utilizador** secção sobre o **atributos de utilizador** caixa de diálogo, execute os seguintes passos para adicionar o atributo de token de SAML conforme mostrado na tabela a seguir:
...
8. Sobre o **configurar Zscaler** secção, copie os URLs apropriados de acordo com seus requisitos.
...
### <a name="configure-zscaler-single-sign-on"></a>Configurar Zscaler Single Sign-On

1. Para automatizar a configuração no Zscaler, tem de instalar **segura de aplicações meu início de sessão da extensão de browser** ao clicar em **instalar a extensão**.

    ![Extensão My apps](common/install-myappssecure-extension.png)

2. Depois de adicionar a extensão para o navegador, clique em **Zscaler configuração** irá direcioná-lo para o aplicativo Zscaler. A partir daí, forneça as credenciais de administrador a iniciar sessão em Zscaler. A extensão do browser irá configurar o aplicativo para e automatizar passos 3 a 6 automaticamente.

    ![Configurar o sso](common/setup-sso.png)

3. Se desejar configurar manualmente o Zscaler, abra uma nova janela de browser e inicie sessão no site da sua empresa Zscaler como administrador e execute os seguintes passos:
...
1. No portal do Azure, selecione **aplicações empresariais**, selecione **todos os aplicativos**, em seguida, selecione **Zscaler**.
...
### <a name="create-zscaler-test-user"></a>Criar utilizador de teste do Zscaler

Nesta secção, um usuário chamado Eduarda Almeida é criado na Zscaler. Zscaler suporta o aprovisionamento de utilizadores de just-in-time, que está ativado por predefinição. Não existe nenhum item de ação para nesta secção. Se um utilizador já não existir no Zscaler, é criado um novo após a autenticação.
...
Quando clica no mosaico do Zscaler no painel de acesso, deve ser automaticamente sessão iniciada no Zscaler para o qual configura o SSO. Para obter mais informações sobre o painel de acesso, consulte [introdução ao painel de acesso](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction).
