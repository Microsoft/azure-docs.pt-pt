...
* Recomenda-se que um único utilizador da AD Azure seja designado para a Promapp para testar a configuração automática de fornecimento de utilizadores. Usuários e/ou grupos adicionais podem ser atribuídos posteriormente.
...
5. No âmbito da secção **de Credenciais de Administrador,** a entrada `https://api.promapp.com/api/scim` no **URL do Arrendatário**. Insera o valor token de **autenticação SCIM** recuperado anteriormente em **Ficha Secreta**. Clique em **Ligação** de Teste para garantir que o Azure AD pode ligar-se ao Promapp. Se a ligação falhar, certifique-se de que a sua conta Promapp tem permissões de administrador e tente novamente.
