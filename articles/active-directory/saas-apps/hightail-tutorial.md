---
title: 'Tutorial: integração de SSO (logon único) do Azure Active Directory com o hightail | Microsoft Docs'
...
---
# <a name="tutorial-azure-active-directory-single-sign-on-sso-integration-with-hightail"></a>Tutorial: integração de SSO (logon único) do Azure Active Directory com o hightail

Neste tutorial, você aprenderá a integrar o hightail com o Azure Active Directory (Azure AD). Ao integrar o hightail ao Azure AD, você pode:

* Controle no Azure AD quem tem acesso ao Hightail.
* Habilite seus usuários a serem conectados automaticamente ao hightail com suas contas do Azure AD.
...
1. Na seção **Adicionar da Galeria** , digite **hightail** na caixa de pesquisa.
...
Configure e teste o SSO do Azure AD com o hightail usando um usuário de teste chamado **B. Simon**. Para que o SSO funcione, você precisa estabelecer uma relação de vínculo entre um usuário do Azure AD e o usuário relacionado no Hightail.
...
1. **[Configurar o SSO do hightail](#configure-hightail-sso)** – para configurar as configurações de logon único no lado do aplicativo.
    * **[Criar usuário de teste do hightail](#create-hightail-test-user)** – para ter um equivalente de B. Simon em hightail que esteja vinculado à representação do usuário no Azure AD.
...
1. Na [portal do Azure](https://portal.azure.com/), na página de integração de aplicativos do **hightail** , localize a seção **gerenciar** e selecione **logon único**.
...
1. O aplicativo hightail espera as asserções SAML em um formato específico, o que exige que você adicione mapeamentos de atributo personalizados à sua configuração de atributos de token SAML. A captura de tela a seguir mostra a lista de atributos padrão.

    ![imagem](common/default-attributes.png)

1. Além de acima, o aplicativo hightail espera que mais alguns atributos sejam passados de volta na resposta SAML, que são mostrados abaixo. Esses atributos também são preenchidos previamente, mas você pode examiná-los de acordo com seus requisitos.
...
1. Na seção **Configurar hightail** , copie as URLs apropriadas com base em seu requisito.
...
Nesta seção, você habilitará B. Simon para usar o logon único do Azure concedendo-lhe acesso ao Hightail.
...
1. Para automatizar a configuração no hightail, você precisa instalar a **extensão do navegador de entrada seguro de meus aplicativos** clicando em **instalar a extensão**.

    ![Extensão de meus aplicativos](common/install-myappssecure-extension.png)

1. Depois de adicionar a extensão ao navegador, clique em **Configurar o hightail** irá direcioná-lo para o aplicativo Hightail. A partir daí, forneça as credenciais de administrador para entrar no Hightail. A extensão do navegador irá configurar automaticamente o aplicativo para você e automatizar as etapas de 3-6.

    ![Configuração da instalação](common/setup-sso.png)

1. Se você quiser configurar o hightail manualmente, em outra janela do navegador, abra o portal de administração do **hightail** .

1. Clique no **ícone do usuário** no canto superior direito da página. 

    ![Configurar logon único](./media/hightail-tutorial/configure1.png)

1. Clique em Exibir guia do **console do administrador** .

    ![Configurar logon único](./media/hightail-tutorial/configure2.png)

1. No menu na parte superior, clique na guia **SAML** e execute as seguintes etapas:

    ![Configurar logon único](./media/hightail-tutorial/configure3.png)
...
- [Experimente o hightail com o Azure AD](https://aad.portal.azure.com/)