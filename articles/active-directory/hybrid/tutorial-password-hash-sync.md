---
title: 'Tutorial: Integrar uma única floresta de AD para Azure usando PHS'
description: Demonstra como configurar um ambiente de identidade híbrida usando a sincronização de haxixe de palavra-passe.
...
O seguinte tutorial irá acompanhá-lo através da criação de um ambiente de identidade híbrida usando a sincronização de haxixe de palavra-passe.  Este ambiente pode então ser usado para testar ou para se familiarizar mais com o funcionamento de uma identidade híbrida.
...
A primeira coisa que temos de fazer, para pôr o nosso ambiente de identidade híbrida a funcionar é criar uma máquina virtual que será usada como o nosso servidor Ative Directory no local.  Faça o seguinte:
...
![Screenshot que mostra o ecrã de definições express e o botão 'Utilizar as definições expresso'.](media/tutorial-password-hash-sync/express1.png)</br>
5. No ecrã AD Do Ligar ao Azure, insira o nome de utilizador e passe a palavra-passe do administrador global do Azure AD. Clique em **Seguinte**.  
...
9. Depois de concluída a instalação, inscreva-se e inscreva-se novamente antes de utilizar o Gestor de Serviço de Sincronização ou o Editor de Regras de Sincronização.
...
2. Iniciar sôms com uma conta de utilizador que foi criada no nosso novo inquilino.  Terá de iniciar s-in utilizando o seguinte formato: user@domain.onmicrosoft.com (). Utilize a mesma palavra-passe que o utilizador utiliza para iniciar sômposições no local.</br>
