...
| Perfil | Baixa | Melhorada | High | Especializada | Seguro | Isolado |
| :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
1. A partir do portal Azure, navegue até ao **Diretório Ativo do Azure** > **utilizadores** > **Novo utilizador.**
...
1. introduza:
   ...
   * **Papel de diretório** - **administrador limitado** e selecione o papel de **Administrador Intune.**
...
1. Vá ao **Diretório Ativo do > ** Dispositivos > **Dispositivos** .
...
1. Navegue pelo **Azure Ative Directory** > **Mobility (MDM e MAM)**  > **Microsoft Intune**.
...
1. Vá ao **Microsoft Intune** > **Device > inscrição** **do Windows** > página de estado de **inscrição** > **Definições**de > **padrão** .
...
1. Selecione **a inscrição do dispositivo** > os perfis de implementação > de **inscrição** **do Windows** > **criar perfil**.
1. introduza:

   * Nome - **Perfil de implantação seguro da estação**de trabalho .
   * Descrição - **Implantação de postos de trabalho seguros**.
   * Set **Converter todos os dispositivos direcionados para Autopilot** para **Sim**. Esta definição garante que todos os dispositivos da lista são registados no serviço de implementação do Autopilot. O processo de registo demora até 48 horas, pelo que deverá aguardar.
...
1. Selecione **Seguinte**.
1. Escolha **atribuições** > **Atribuir a** > **Grupos Selecionados**. Em **grupos Selecionados para incluir,** escolha **Postos de Trabalho Seguros**.
1. Selecione **Seguinte**.
...
1. Vá ao **Microsoft Intune** > **As atualizações** de software > **Windows 10 Update Rings**.
1. introduza:
...
1. Navegue para **o Microsoft Intune** > **Conformidade do Dispositivo** > Windows Defender **ATP**.
...
1. **Detete connect Windows versão 10.0.15063 e acima para Windows Defender ATP** to **On**.
1. Selecione **Guardar**.
...
\* Compliance Especializada é um script que aplica a configuração especializada fornecida no NCSC Windows10 SecurityBaseline.
...
* Aqui é onde você pode encontrar os perfis de configuração do dispositivo Intune criados pelos scripts: **Portal Azure** > **Microsoft Intune** > **Dispositivo** > **Perfis**.
* Aqui é onde você pode encontrar as políticas de conformidade do dispositivo Intune criadas pelos scripts: **Portal Azure** > **Microsoft Intune** > As **Políticas**de > de conformidade do **dispositivo** .
...
Execute o roteiro de exportação de dados Intune `DeviceConfiguration_Export.ps1` do [repositório De Configuração do Dispositivo GiuHub](https://github.com/microsoftgraph/powershell-intune-samples/tree/master/DeviceConfiguration) para exportar todos os perfis intune atuais.
...
1. Extraios os ficheiros e tome nota da localização do ficheiro `GoogleChromeStandaloneEnterprise64.msi`.
1. No **portal Azure** navegue para as **aplicações** do Cliente Intune > **Microsoft Intune** > **Apps** > **Add**.
1. No **tipo de App,** escolha **Linha de Negócios.**
1. No ficheiro de **pacote**app, selecione o ficheiro `GoogleChromeStandaloneEnterprise64.msi` a partir da localização extraída e selecione **OK**.
...
Poderá ter de instalar aplicações do Windows de 32 bits ou outras aplicações cuja implementação requer preparações especiais. Nestes casos, a ferramenta de preparação de [conteúdo win32 da Microsoft](https://github.com/Microsoft/Microsoft-Win32-Content-Prep-Tool) pode fornecer um ficheiro de formato `.intunewin` pronto a utilizar para instalação.
...
1. Selecione **User and Groups** > Selecionar utilizadores e **grupos** 
1. **Selecione Incluir** funções de **Diretório** > > Escolha as funções > Administrador Global, Administrador de Funções Privilegiadas, Administrador de Autenticação Privilegiada, Administrador de Segurança, Administrador de Conformidade, Administrador de Acesso Condicional, Administrador de Aplicação, Administrador de Aplicações em Nuvem, Administrador de Serviço Intune
1. Selecione **Excluir** > Escolha **utilizadores e grupos** > **Selecione selecione utilizadores excluídos** > Selecione o seu grupo **BreakGlass de emergência.**
1. Selecione **aplicações ou ações cloud** > Selecione **todas as aplicações em nuvem**
1. Selecione **Condições** > Selecione Plataformas de **Dispositivos** > Escolha configurar **Sim** > selecione as plataformas de **dispositivos selecionados** Escolha **o Windows**
1. Selecione **controlos de acesso** > Selecione **Grant Access** **Yes** > Escolha o dispositivo de necessidade de ser marcado **como conforme**. 
1. Selecione ** > de política de ativação**
...
1. Navegue no **portal Azure** > **Microsoft Intune** > **Device configuração** > **scripts PowerShell** > **Add**.
...
1. **Selecione Configurar**.
   ...
   1. Selecione **OK**.
1. Selecione **Criar**.
1. Selecione **Atribuições** > **Selecione grupos**.
...
1. Para inscrever o seu dispositivo, necessita das seguintes informações:
   ...
   * Pode executar [o Get-WindowsAutoPilotInfo](https://aka.ms/Autopilotshell) para obter um ficheiro de haxixe CSV com todas as informações necessárias para a inscrição do dispositivo.
     ...
     Execute `Get-WindowsAutoPilotInfo – outputfile device1.csv` para obter a informação como um ficheiro CSV que pode importar para Intune.

     > [!NOTE]
     > O guião requer direitos elevados. Funciona como assinado à distância. O comando `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned` permite que o script corra corretamente.
...
1. No **portal Azure**, vá à **microsoft Intune** > **Device de inscrição** > dispositivos de ** > de inscrição do Windows** - **Gerencie os dispositivos do Autopilot do Windows**.
1. Selecione **Importar** e escolha o seu ficheiro CSV.
1. Adicione o dispositivo ao grupo de segurança **Secure Workstations.**
1. No dispositivo Windows 10 pretende configurar, vá às **Definições do Windows** > **Atualização e** **Recuperação**de > de Segurança .
...
1. No **portal Azure,** vá ao espaço de **trabalho Azure Sentinel** > Selecione espaço de trabalho de monitorização de **estações** de trabalho seguras
1. Selecione **Conectores de Dados**
1. Escolha **o Diretório Ativo Azure** > Open Connector Page > Depois de rever o Pré-requisito. Proceda à Configuração e selecione **Connect** para ambos os registos de acesso a AD Azure, bem como registos de auditoria azure AD.
1. Escolha a **Atividade Azure** > Página de Conector Aberto > Depois de rever o Pré-requisito. Proceda ao Configure Azure Activity Logs > Selecione a sua subscrição > Selecione **Connect**
...
Configure o seu [painel ATP Defender](https://securitycenter.windows.com/machines). Utilizando orientação na visão geral do painel de [instrumentos de gestão](/windows/security/threat-protection/microsoft-defender-atp/tvm-dashboard-insights)de ameaças e vulnerabilidades.
...
1. Selecione **definições avançadas** > **Data** > **Registos de Eventos do Windows**
1. Em **Recolher eventos a partir dos seguintes registos de eventos** 
1. introduza:
   * 'Microsoft-Windows-AppLocker/EXE e DLL' > **Informação** não selecionada
   * 'Microsoft-Windows-AppLocker/MSI e Script' > **Informação** não selecionada
   * 'Microsoft-Windows-AppLocker/App-Deployment embalado' > **Informação** não selecionada
