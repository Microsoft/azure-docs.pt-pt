...
1. Recebe um [tíquete de concessão de tíquete (TGT)](https://docs.microsoft.com/windows/desktop/secauthn/ticket-granting-tickets) Kerberos que é usado para acessar recursos ingressados no AD. Se a tentativa de obter o TGT para o domínio de ligação AAD falhar (o tempo de saída relacionado do DCLocator pode causar um atraso), as entradas do Credential Manager são tentadas, ou o utilizador pode receber uma autenticação popup solicitando credenciais para o recurso alvo.
