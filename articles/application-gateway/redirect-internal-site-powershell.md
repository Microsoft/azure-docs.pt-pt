...
description: Saiba como criar um gateway de aplicações que redirecione o tráfego interno da web para o pool de backend apropriado dos servidores usando a Azure Powershell.
...
Em seguida, pode associar a configuração de redireccionamento a uma nova regra chamada *contosoOrgRule* utilizando [Add-AzApplicationGatewayRequestRoutingRule](/powershell/module/az.network/add-azapplicationgatewayrequestroutingrule).
...
Neste exemplo, cria-se um conjunto de escala de máquina virtual que suporta o pool de backend que criou. O conjunto de escala que cria chama-se *myvmss* e contém duas instâncias de máquina virtual nas quais instala o IIS. Pode atribuir o conjunto de dimensionamento ao conjunto de back-end quando configurar as definições de IP.
