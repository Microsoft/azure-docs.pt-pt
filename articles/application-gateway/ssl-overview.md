...
- Se **escolher o nome de anfitrião do endereço de backend** em vez do campo Host na definição http backend, então o cabeçalho SNI é sempre definido para o pool backend FQDN e o CN no servidor de backend TLS/SSL deve corresponder ao seu FQDN. Os membros do backend pool com IPs não são apoiados neste cenário.
