...
Esta imagem ilustra uma aplicação que tem três ouvintes. Os dois primeiros são ouvintes multi-sites para `http://acme.com/*` e `http://fabrikam.com/*`, respectivamente. Ambos ouvem no porto 80. O terceiro é um ouvinte básico que tem a terminação da camada de camada de tomadas seguras (SSL) de ponta a ponta.
...
Application Gateway consome 1 endereço IP privado por instância, além de outro endereço IP privado se um IP frontal privado estiver configurado.

O Azure também reserva 5 endereços IP em cada subnet para uso interno: os primeiros 4 e os últimos endereços IP. Por exemplo, considere 15 casos de gateway de aplicação sem IP frontal privado. Precisa de pelo menos 20 endereços IP para esta sub-rede: 5 para uso interno e 15 para as instâncias de gateway da aplicação. Então, precisa de uma sub-rede de /27 ou maior.

Considere uma sub-rede que tenha 27 instâncias de gateway de aplicação e um endereço IP para um IP frontal privado. Neste caso, necessita de 33 endereços IP: 27 para as instâncias de gateway da aplicação, 1 para a frente privada e 5 para uso interno. Então, precisa de uma sub-rede de /26 ou maior.
...
#### <a name="user-defined-routes-supported-on-the-application-gateway-subnet"></a>Rotas definidas pelo utilizador suportadas na subnet gateway da aplicação
...
Para o V2 SKU, os UDRs não são suportados na subnet Application Gateway. Para mais informações, consulte O Portal de [Aplicações Azure v2 SKU](application-gateway-autoscaling-zone-redundant.md#differences-with-v1-sku).

> [!NOTE]
> Os UDRs não são suportados para o V2 SKU a partir de agora.
...
Pode centralizar a gestão de certificados SSL e reduzir a sobrecarga de encriptação-desencriptação para uma quinta de servidores de back-end. O manuseamento sSL centralizado também permite especificar uma política Central SSL que é adequada aos seus requisitos de segurança. Pode escolher a política *SSL padrão,* *predefinida*ou *personalizada.*

Configura a política SSL para controlar as versões protocolais SSL. Pode configurar uma porta de entrada de aplicação para utilizar uma versão de protocolo mínimo para apertos de mão TLS1.0, TLS1.1 e TLS1.2. Por predefinição, sSL 2.0 e 3.0 estão desativados e não são configuráveis. Para mais informações, consulte a visão geral da política do [Application Gateway SSL](https://docs.microsoft.com/azure/application-gateway/application-gateway-ssl-policy-overview).
