...
Siga os passos abaixo para criar um [objeto principal](../active-directory/develop/app-objects-and-service-principals.md#service-principal-object)de serviço Azure Ative (AAD). Por favor, grave os `appId` `password` , e `objectId` valores - estes serão usados nos seguintes passos.

1. Criar diretor de serviço de AD[(Ler mais sobre Azure RBAC):](../role-based-access-control/overview.md)
    ```azurecli
...
1. Crie o ficheiro de parâmetros que será usado na implementação do modelo do Gestor de Recursos Azure mais tarde.
    ...
    EOF
    ```
    Para implementar um cluster **ativado por Kubernetes RBAC,** desloque o `aksEnableRBAC` campo para `true`
...
- [Gateway de aplicação](./overview.md) v2
...
- [Identidade Gerida](../active-directory/managed-identities-azure-resources/overview.md), que será usada pela [Identidade do Pod AAD](https://github.com/Azure/aad-pod-identity/blob/master/README.md)
...
Obtenha credenciais para o seu AKS recém-implantado[(ler mais):](../aks/kubernetes-walkthrough.md#connect-to-the-cluster)
```azurecli
...
### <a name="install-aad-pod-identity"></a>Instalar identidade do pod do AAD
  A Azure Ative Directory Pod Identity fornece acesso simbólico ao [Azure Resource Manager (ARM)](../azure-resource-manager/management/overview.md).
