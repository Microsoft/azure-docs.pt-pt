...
Saiba como o Application Gateway e o Security Center ajudam a proteger os recursos da sua aplicação web. A firewall da aplicação web de gateway de aplicação de aplicação (WAF) integra-se com [o Security Center](../security-center/security-center-intro.md) para fornecer uma visão perfeita para prevenir, detetar e responder a ameaças a aplicações web desprotegidas no seu ambiente.
