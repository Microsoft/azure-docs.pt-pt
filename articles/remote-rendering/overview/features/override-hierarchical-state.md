...
O `HierarchicalStateOverrideComponent` pode ser ligado em vários níveis de uma hierarquia de objetos. Uma vez que só pode haver um componente `HierarchicalStateOverrideComponent` de cada tipo numa entidade, cada um gere os estados para oculto, see-through, selecionado, cor e colisão.
...
Pode alterar estados diretamente `SetState` ou através da função:
...
Um exemplo `HierarchicalStateOverrideComponent` de si mesmo não acrescenta muito tempo de execução. No entanto, é sempre uma boa prática manter o número de componentes ativos baixos. Por exemplo, ao implementar um sistema de seleção que realça o objeto escolhido, recomenda-se eliminar o componente quando o destaque for removido. Manter os componentes em torno de características neutras pode rapidamente somar.
