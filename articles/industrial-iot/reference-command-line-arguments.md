---
title: Argumentos de linha de comando da Microsoft OPC Publisher
description: Este artigo fornece uma visão geral dos argumentos da linha de comando da editora OPC
author: jehona-m
ms.author: jemorina
...
ms.locfileid: "104787592"
---
# <a name="command-line-arguments"></a>Argumentos de linha de comando

No seguinte, existem vários argumentos de linha de comando descritos que podem ser usados para definir configurações globais para o OPC Publisher.

## <a name="opc-publisher-command-line-arguments-for-version-25-and-below"></a>Argumentos da linha de comando da OPC Publisher para a versão 2.5 e abaixo

* Utilização: opcpublisher.exe \<applicationname> \<iothubconnectionstring> [ \<options> ]
    
* nome de aplicação: o nome de aplicação da OPC UA a utilizar, exigido O nome da aplicação também é utilizado para registar o editor com este nome no registo do dispositivo IoT Hub.
    
* iothubconnectionstring: o cordão de conexão do proprietário IoT Hub, opcional. Normalmente, especifica o cordão de ligação do proprietário do IoTHub apenas no primeiro início da aplicação. A cadeia de ligação é encriptada e armazenada na loja de certificados das plataformas.
Em chamadas posteriores, é lido de lá e reutilizado. Se especificar o eixo de ligação em cada partida, o dispositivo, que é criado para a aplicação no registo do dispositivo IoT Hub, é removido e recriado de cada vez.
    
Existem algumas variáveis ambientais, que podem ser usadas para controlar a aplicação:
```
...
> [!NOTE] 
> Os argumentos da linha de comando sobressaem as definições variáveis do ambiente.
...
## <a name="opc-publisher-command-line-arguments-for-version-26-and-above"></a>Argumentos da linha de comando da OPC Publisher para a versão 2.6 e superior
```
      --pf, --publishfile=VALUE
                                       the filename to configure the nodes  to publish.
...
Outros recursos podem ser encontrados nos repositórios do GitHub:
