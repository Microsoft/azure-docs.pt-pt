...
Para evitar tentativas de entrada maliciosas repetidas, a Azure Ative Directory Domain Services (Azure AD DS) bloqueia contas após um limiar definido. Este bloqueio de conta também pode acontecer por acidente sem um incidente de ataque de sinal. Por exemplo, se um utilizador introduzir repetidamente a palavra-passe errada ou se um serviço tentar utilizar uma senha antiga, a conta fica bloqueada.
...
Uma conta de utilizador em Azure AD DS é bloqueada quando um limiar definido para tentativas de inscrição mal sucedidas foi cumprido. Este comportamento de bloqueio de conta foi concebido para protegê-lo de tentativas de inscrição repetidas de força bruta que podem indicar um ataque digital automatizado.
...
* **O utilizador trancou-se lá fora.**
    * Após uma recente alteração da palavra-passe, o utilizador continuou a utilizar uma palavra-passe anterior? A política de bloqueio de conta padrão de 5 tentativas falhadas em 2 minutos pode ser causada pelo utilizador, inadvertidamente, redando uma senha antiga.
...
* **A palavra-passe foi alterada num ambiente diferente e a nova palavra-passe ainda não foi sincronizada.**
    * Se uma palavra-passe de conta for alterada fora do Azure AD DS, como num ambiente de DS AD pré-pré-m, pode levar alguns minutos para que a alteração da palavra-passe se sincronize através do Azure AD e para o Azure AD DS.
    * Um utilizador que tente iniciar seduca num recurso através do Azure AD DS antes que o processo de sincronização da palavra-passe tenha concluído faz com que a sua conta seja bloqueada.
...
Veja todos os eventos de bloqueio de conta entre 26 de junho de 2019 às 9h00. e 1 de julho de 2019 meia-noite, classificado ascendente pela data e hora:
