...
> [!NOTE]
> Ao configurar o Azure AD para autenticação do AKS, são configuradas duas aplicações do Azure AD. Esta operação deve ser concluída por um administrador de inquilino do Azure.
...
Crie a aplicação de servidor componente com o [az ad app crie][az-ad-app-create] command, then update the group membership claims using the [az ad app update][az-ad-app-update] comando. O exemplo seguinte utiliza a *aksname* variável definida a [antes de começar](#before-you-begin) secção e cria uma variável
...
Agora, criar um principal de serviço para a aplicação de servidor com o [az ad sp criar][az-ad-sp-create] command. This service principal is used to authenticate itself within the Azure platform. Then, get the service principal secret using the [az ad sp credential reset][az-ad-sp-credential-reset] comando e a atribuir à variável com o nome *serverApplicationSecret* para uso em um dos seguintes passos:
...
A segunda aplicação do Azure AD é utilizada quando um utilizador iniciar sessão para o cluster do AKS com a CLI do Kubernetes (`kubectl`). Esta aplicação de cliente recebe o pedido de autenticação do usuário e verifica as suas credenciais e permissões. Criar a aplicação do Azure AD para o componente de cliente que utilize o [az ad app criar][az-ad-app-create] comando:
...
Criar um principal de serviço para a aplicação de cliente que utilize o [az ad sp criar][az-ad-sp-create] comando:
...
Adicione as permissões para a aplicação de cliente e componentes da aplicação de servidor para utilizar a comunicação de oAuth2 fluxo usando a [adicionar permissão da aplicação ad az][az-ad-app-permission-add] command. Then, grant permissions for the client application to communication with the server application using the [az ad app permission grant][az-ad-app-permission-grant] comando:
...
Obter o ID de inquilino da sua subscrição do Azure com o [show de conta de az][az-account-show] command. Then, create the AKS cluster using the [az aks create][az-aks-create] comando. O comando para criar o cluster do AKS fornece o servidor e cliente IDs da aplicação, o segredo de principal do serviço de aplicações de servidor e o seu ID de inquilino:
...
Crie a ClusterRoleBinding com o [kubectl aplicar][kubectl-apply] de comando e especifique o nome de ficheiro do seu manifesto YAML de:
