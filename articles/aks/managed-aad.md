...
> [!Note]
> Os aglomerados AKS existentes com integração AAD (Azure Ative Directory) não são afetados pela nova experiência aAD gerida pela AKS.

A integração da Azure AD com a AAD gerida pela AKS foi concebida para simplificar a experiência de integração da Azure AD, onde os utilizadores eram anteriormente obrigados a criar uma aplicação para clientes, uma aplicação de servidor, e exigiu que o inquilino da AD Azure concedesse permissões ao Diretório. Na nova versão, o fornecedor de recursos AKS gere as aplicações de clientes e servidores para si.

## <a name="limitations"></a>Limitações

* Não pode atualmente atualizar um cluster integrado AKS AAD existente para a nova experiência aAD gerida pela AKS.
...
- O Azure CLI, versão 2.5.1 ou posterior
...
Uma criação bem sucedida de um cluster AAD gerido pela AKS tem a seguinte secção no corpo de resposta
```
"Azure ADProfile": {
...
* Conheça o Controlo de [Acesso baseado em papel azure AD][azure-ad-rbac].
* Utilize [kubelogin](https://github.com/Azure/kubelogin) para aceder a funcionalidades de autenticação Azure que não estejam disponíveis no kubectl.
