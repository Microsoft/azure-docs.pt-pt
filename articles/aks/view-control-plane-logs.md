...
description: Saiba como ativar e ver os registos do plano de controlo Kubernetes no Serviço Azure Kubernetes (AKS)
...
Com o Azure Kubernetes Service (AKS), os componentes do avião de controlo, como o *kube-apiserver* e o *controlador de kube-controller-manager* são fornecidos como um serviço gerido. Cria e gere os nós que executam o tempo de execução do *kubelet* e do contentor e implementa as suas aplicações através do servidor API gerido de Kubernetes. Para ajudar a resolver problemas com a sua aplicação e serviços, poderá ter de visualizar os registos gerados por estes componentes do avião de controlo. Este artigo mostra-lhe como utilizar registos do Azure Monitor para ativar e consultar os registos dos componentes do avião de controlo Kubernetes.
...
Os registos do Monitor Azure estão ativados e geridos no portal Azure. Para ativar a recolha de registos para os componentes do avião de controlo Kubernetes no seu cluster AKS, abra o portal Azure num navegador web e complete os seguintes passos:
...
2. No lado esquerdo, escolha **as definições de Diagnóstico**.

3. Selecione o seu cluster AKS, como *o myAKSCluster,* e depois opte por adicionar a **definição de diagnóstico**.
  :::image type="content" source="media\view-control-plane-logs\select-add-diagnostic-setting.PNG" alt-text="A screenshot do portal Azure numa janela do navegador mostrando definições de Diagnóstico, indicando &quot;Adicionar definição de diagnóstico&quot; deve ser selecionada":::

4. Introduza um nome, como *myAKSClusterLogs,* selecione a opção de **Enviar para log analytics espaço de trabalho**.

5. Selecione um espaço de trabalho existente ou crie um novo. Se criar um espaço de trabalho, forneça um nome de espaço de trabalho, um grupo de recursos e uma localização.

6. Na lista de registos disponíveis, selecione os registos que pretende ativar. Para este exemplo, ative os registos *de administração de kube-audit* e *kube-audit-admin.* Os registos comuns incluem o *kube-apiserver,* *o controlador-controlador de kube* e *o kube-scheduler*. Pode retornar e alterar os registos recolhidos assim que os espaços de trabalho do Log Analytics estiverem ativados.

7. Quando estiver pronto, **selecione Guardar** para ativar a recolha dos registos selecionados.
  :::image type="content" source="media\view-control-plane-logs\settings-selected.PNG" alt-text="Screenshot do ecrã 'Add diagnostic setting' do portal Azure. Um destino de 'Enviar para Log Analytics workspace' e logs 'kube-audit' e 'kube-audit-admin' são selecionados":::
