...
- [Mais detalhes sobre os pontos finais privados da Grade de Eventos](network-security.md#private-endpoints)

- [Mais detalhes sobre a Firewall IP grid de eventos](network-security.md#ip-firewall)
...
- [Como usar a etiqueta de serviço para a grelha de eventos Azure](network-security.md#service-tags)
...
Você também pode usar definições de política incorporadas relacionadas com Azure Event Grid, tais como:- Os domínios da grelha de eventos Azure devem usar links privados - Os tópicos da Grelha de Eventos Azure devem usar links privadosAzure
- [políticas incorporadas para recursos da Grade de Eventos](../governance/policy/samples/built-in-policies.md#event-grid)
...
**Orientação**: A gestão de acesso aos recursos da Grade de Eventos é controlada através do Azure Ative Directory (AD). A Azure AD não tem o conceito de senhas padrão.
...
**Orientação**: Não aplicável; O serviço de Grelha de Eventos não utiliza autenticação multi-factor
...
**Orientação**: Não aplicável. A Event Grid não utiliza o Azure AD para autenticar clientes editoriais de eventos; suporta a autenticação através de teclas SAS.
...
Utilize o Azure Ative Directory (AD) Gestão de Identidade Privilegiada (PIM) para a geração de registos e alertas quando ocorrem atividades suspeitas ou inseguras no ambiente.
...
**Orientação**: A Azure Event Grid suporta a utilização do Azure Ative Directory (AD) para autorizar pedidos aos recursos da Grelha de Eventos. Com o Azure AD, pode utilizar o controlo de acesso baseado em funções (RBAC) para conceder permissões a um principal de segurança, que pode ser um utilizador ou um diretor de serviço de aplicação.
