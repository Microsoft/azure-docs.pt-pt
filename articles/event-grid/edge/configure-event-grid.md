...
| Nome da Propriedade | Descrição |
| ---------------- | ------------ |
| `metrics__reporterType` | Tipo repórter para ponto de métricas. O padrão é `none` e desativa as métricas. Definição para `prometheus` permitir métricas no formato de exposição Prometheus.