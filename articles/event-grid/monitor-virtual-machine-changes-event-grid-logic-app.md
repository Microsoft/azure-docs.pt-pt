...
1. Agora subscreva a sua aplicação lógica para eventos da editora. Forneça os detalhes sobre a subscrição do seu evento, conforme descrito na tabela seguinte, por exemplo:
   ...
   | **Nome de recursos** |  Sim | <*event-publisher-Azure-resource-name*> | Selecione o nome de recurso Azure para o editor do evento. Esta lista varia em com base no tipo de recurso que selecionou. Para este tutorial, selecione o nome para o grupo de recursos Azure que inclui a sua máquina virtual. |
...
1. Guarde a aplicação lógica. Na barra de ferramentas do designer, **selecione Save**. Para entrar em colapso e ocultar os detalhes de uma ação na sua aplicação lógica, selecione a barra de título da ação.
...
1. Crie uma condição que verifique o evento `body` por um objeto onde a propriedade seja igual à `data` `operationName` `Microsoft.Compute/virtualMachines/write` operação. Saiba mais sobre o [esquema de eventos do Event Grid](../event-grid/event-schema.md).
   ...
   1. Na primeira linha, em **E**, clique dentro da caixa esquerda. Na lista de conteúdos dinâmicos que aparece, selecione **Expression**.
   ...
   1. No editor de expressão, introduza esta expressão, que devolve o nome de operação do gatilho, e selecione **OK**:
      ...
      ![Introduza a expressão para extrair o nome da operação](./media/monitor-virtual-machine-changes-event-grid-logic-app/condition-add-data-operation-name.png)
   ...
   ![Condição preenchida que compara a operação](./media/monitor-virtual-machine-changes-event-grid-logic-app/complete-condition.png)
...
1. Na caixa se for **verdadeira** da condição, selecione **Adicione uma ação**.
...
1. Guarde a aplicação lógica. Para entrar em colapso e ocultar os detalhes de cada ação na sua aplicação lógica, selecione a barra de título da ação.
