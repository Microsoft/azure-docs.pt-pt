...
| escreverMethod | O método usado para escrever dados na Base de Dados Azure para PostgreSQL.<br>Os valores permitidos são: **CopyCommand** (pré-visualização, que é mais performante), **BulkInsert** (predefinição). | No |
