...
|:--- |:--- |:--- |
| tipo | A `type` propriedade deve ser definida para `AzureBlobStorage` (sugerida) ou `AzureStorage` (ver as seguintes notas). | Yes |
| conexãoStragem | Especifique as informações necessárias para ligar ao Armazenamento para a `connectionString` propriedade. <br/> Também pode colocar a chave de conta no Cofre da Chave Azure e retirar a `accountKey` configuração da cadeia de ligação. Para mais informações, consulte as seguintes amostras e as credenciais da Loja no artigo [do Azure Key Vault.](store-credentials-in-key-vault.md) | Yes |
...
>[!NOTE]
>Se estiver a usar o `AzureStorage` tipo de serviço ligado, ainda é suportado como está. Mas sugerimos que use o novo `AzureBlobStorage` tipo de serviço ligado para a frente.
...
| Propriedade | Descrição | Obrigatório |
|:--- |:--- |:--- |
| tipo | A `type` propriedade deve ser definida para `AzureBlobStorage` (sugerida) ou `AzureStorage` (ver a seguinte nota). | Yes |
| SasUri | Especifique a assinatura de acesso partilhado URI aos recursos de Armazenamento, tais como bolha ou recipiente. <br/>Marque este campo `SecureString` para armazená-lo de forma segura na Fábrica de Dados. Também pode colocar o token SAS no Cofre da Chave Azure para utilizar a rotação automática e remover a porção de token. Para mais informações, consulte as seguintes amostras e [guarde as credenciais no Cofre da Chave Azure.](store-credentials-in-key-vault.md) | Yes |
| connectVia | O [tempo de integração](concepts-integration-runtime.md) a ser utilizado para ligar à loja de dados. Pode utilizar o tempo de funcionamento da integração Azure ou o tempo de integração auto-hospedado (se a sua loja de dados estiver numa rede privada). Se esta propriedade não for especificada, o serviço utiliza o tempo de execução de integração Azure padrão. | No |
...
>- Se aceder ao armazenamento do blob através de um ponto final privado utilizando o Data Flow, note quando a autenticação principal do serviço é utilizada O Fluxo de Dados liga-se ao ponto final da ADLS Gen2 em vez do ponto final blob. Certifique-se de que cria o ponto final privado correspondente em ADF para permitir o acesso.
...
> - Se aceder ao armazenamento do blob através de um ponto final privado utilizando o Data Flow, note quando a autenticação de identidade gerida é utilizada O Fluxo de Dados liga-se ao ponto final da ADLS Gen2 em vez do ponto final blob . Certifique-se de que cria o ponto final privado correspondente em ADF para permitir o acesso.
...
| Propriedade   | Descrição                                                  | Obrigatório |
| ---------- | ------------------------------------------------------------ | -------- |
| tipo       | A  propriedade tipo da localização no conjunto de dados deve ser definida para **AzureBlobStorageLocation**. | Yes      |
...
> [!NOTE]
> O `$logs` recipiente, que é criado automaticamente quando o Storage Analytics está ativado para uma conta de armazenamento, não é mostrado quando uma operação de listagem de contentores é realizada através da UI da Fábrica de Dados. O percurso do ficheiro deve ser fornecido diretamente para que a Data Factory consuma ficheiros do `$logs` recipiente.
...
| Propriedade                 | Descrição                                                  | Obrigatório |
| ------------------------ | ------------------------------------------------------------ | -------- |
| tipo                     | A `type` propriedade em baixo deve ser definida para `storeSettings` `AzureBlobStorageWriteSettings` . | Yes      |
...
Neste caso, todos os ficheiros que foram `/data/sales` obtidos são transferidos para `/backup/priorSales` .
...
**Opção nome de ficheiro:** Determina como os ficheiros de destino são nomeados na pasta de destino. As opções de nome do ficheiro são:
   ...
   - **Padrão**: Introduza um padrão que enumera os seus ficheiros de saída por partição. Por exemplo, `loans[n].csv` vai criar , e assim por `loans1.csv` `loans2.csv` diante.
...
| Propriedade | Descrição | Obrigatório |
|:--- |:--- |:--- |
| tipo | A `type` propriedade do conjunto de dados deve ser definida para `AzureBlob` . | Yes |
...
| fileName | Nome ou filtro wildcard para as bolhas sob o `folderPath` valor especificado. Se não especificar um valor para esta propriedade, o conjunto de dados aponta para todas as bolhas na pasta. <br/><br/>Para o filtro, os wildcards permitidos são: `*` (corresponde a zero ou mais caracteres) e `?` (corresponde a zero ou caracteres individuais).<br/>- Exemplo 1: `"fileName": "*.csv"`<br/>- Exemplo 2: `"fileName": "???20180427.txt"`<br/>Use `^` para escapar se o seu nome de ficheiro tiver um wildcard ou este personagem de fuga dentro.<br/><br/>Quando `fileName` não é especificado para um conjunto de dados de saída e não é especificado na pia de `preserveHierarchy` atividade, a atividade Copy gera automaticamente o nome blob com o seguinte padrão: "*Dados.". atividade executar ID GUID]. [GUID se FlattenHierarchy]. [formato se configurado]. [compressão se configurado]*". Por exemplo: "Data.0a405f8a-93ff-4c6f-b3be-f69616f1df7a.txt.gz". <br/><br/>Se copiar de uma fonte tabular utilizando um nome de mesa em vez de uma consulta, o padrão de nome é `[table name].[format].[compression if configured]` . Por exemplo: "MyTable.csv". | No |
| modificadoDatetimeStart | Os ficheiros são filtrados com base no atributo: última modificada. Os ficheiros serão selecionados se o seu último tempo modificado estiver dentro do intervalo de tempo entre `modifiedDatetimeStart` e `modifiedDatetimeEnd` . . O tempo é aplicado ao fuso horário UTC no formato de "2018-12-01T05:00:00Z". <br/><br/> Tenha em atenção que ativar esta definição afetará o desempenho geral do movimento de dados quando pretender filtrar grandes quantidades de ficheiros. <br/><br/> As propriedades podem ser `NULL` , o que significa que nenhum filtro de atributo de ficheiro será aplicado no conjunto de dados.  Quando `modifiedDatetimeStart` tiver um valor de data, mas `modifiedDatetimeEnd` `NULL` for, serão selecionados os ficheiros cujo último atributo modificado é superior ou igual ao valor da data.  Quando `modifiedDatetimeEnd` tiver um valor de data mas `modifiedDatetimeStart` `NULL` for, serão selecionados os ficheiros cujo último atributo modificado é inferior ao valor da data.| No |
...
| Propriedade | Descrição | Obrigatório |
|:--- |:--- |:--- |
| tipo | A `type` propriedade da fonte de atividade copy deve ser definida para `BlobSource` . | Yes |
| recursivo | Indica se os dados são lidos novamente a partir das sub-dobradeiras ou apenas a partir da pasta especificada. Note que quando `recursive` está definido e a pia é uma loja baseada em `true` ficheiros, uma pasta ou sub-dobragem vazia não é copiada ou criada na pia.<br/>Os valores permitidos são `true` (padrão) e `false` . | No |
| maxConcurrentConnections |O limite superior das ligações simultâneas estabelecidas na loja de dados durante a atividade. Especifique um valor apenas quando pretende limitar ligações simultâneas.| No |
...
| Propriedade | Descrição | Obrigatório |
|:--- |:--- |:--- |
| tipo | A `type` propriedade do lavatório de atividade da Cópia deve ser definida para `BlobSink` . | Yes |
| copyOportundo | Define o comportamento da cópia quando a fonte é ficheiros de uma loja de dados baseada em ficheiros.<br/><br/>Os valores permitidos são:<br/><b>- Preservar AHierarquia (predefinição)</b>: Preserva a hierarquia do ficheiro na pasta alvo. O percurso relativo do ficheiro de origem para a pasta de origem é idêntico ao caminho relativo do ficheiro alvo para a pasta alvo.<br/><b>- FlattenHierarchy</b>: Todos os ficheiros da pasta de origem estão no primeiro nível da pasta alvo. Os ficheiros-alvo têm nomes autogerados. <br/><b>- MergeFiles</b>: Funde todos os ficheiros da pasta de origem para um ficheiro. Se o nome do ficheiro ou do blob for especificado, o nome do ficheiro fundido é o nome especificado. Caso contrário, é um nome de ficheiro autogerado. | No |
| maxConcurrentConnections |O limite superior das ligações simultâneas estabelecidas na loja de dados durante a atividade. Especifique um valor apenas quando pretende limitar ligações simultâneas.| No |
