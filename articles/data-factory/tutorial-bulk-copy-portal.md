...
Neste cenário, tem uma série de tabelas na Base de Dados Azure SQL que pretende copiar para a Azure Synapse Analytics. Segue-se a sequência lógica de passos no fluxo de trabalho que ocorre nos pipelines:
...
* **Base de Dados Azure SQL**. Esta base de dados contém os dados de origem. Criar uma base de dados na Base de Dados SQL com dados de amostras de Adventure Works LT seguindo criar uma base de dados em artigo [de Base de Dados Azure SQL.](../azure-sql/database/single-database-create-quickstart.md) Este tutorial copia todas as tabelas desta base de dados de amostras para um Azure Synapse Analytics.
* **Azure Synapse Analytics**. Este armazém de dados contém os dados copiados da Base de Dados SQL. Se não tiver um espaço de trabalho Azure Synapse Analytics, consulte o [Get start with Azure Synapse Analytics](..\synapse-analytics\get-started.md) para obter passos para criar um.

## <a name="azure-services-to-access-sql-server"></a>Serviços do Azure para aceder ao SQL Server

Tanto para a Base de Dados SQL como para a Azure Synapse Analytics, permita que os serviços da Azure acedam ao servidor SQL. Certifique-se de que **os serviços e recursos do Azure para aceder a esta** definição de servidor estão **ligados** para o seu servidor. Esta definição permite que o serviço Data Factory leia dados a partir da sua Base de Dados Azure SQL e escreva dados para o seu Azure Synapse Analytics. 
...
Neste tutorial, ligue a sua Base de Dados Azure SQL, Azure Synapse Analytics e Azure Blob Storage stores à sua fábrica de dados. A Base de Dados SQL do Azure é o arquivo de dados de origem. O Azure Synapse Analytics é a loja de dados da pia/destino. O Azure Blob Storage deve encenar os dados antes de os dados serem carregados no Azure Synapse Analytics utilizando o PolyBase. 
...
1. Na janela **New Linked Service,** selecione **Azure Synapse Analytics**, e clique em **Continuar**. 
1. Na janela **New Linked Service (Azure Synapse Analytics),** faça os seguintes passos: 
...
O conjunto de dados de saída **AzureSqlDWDataset** refere-se a **AzureSqlDWLinkedService**. O serviço ligado especifica a cadeia de ligação para ligar ao Azure Synapse Analytics. O conjunto de dados especifica a base de dados e a tabela para a qual os dados são copiados. 
...
1. Na janela **New Dataset,** selecione **Azure Synapse Analytics** e, em seguida, clique em **Continuar**.
...
O oleoduto  **IterateAndCopySQLTables** tem uma lista de tabelas como parâmetro. Para cada tabela da lista, copia dados da tabela na Base de Dados Azure SQL para Azure Synapse Analytics usando cópia encenada e PolyBase.
...
1. Mude para o separador **Sink** e siga os passos abaixo: 
    ...
    1. Para a opção Tabela, a definição predefinitiva é "Nenhuma". Se não tiver tabelas pré-criadas no sink Azure Synapse Analytics, ative a opção **de tabela Auto,** a atividade de cópia criará automaticamente tabelas para si com base nos dados de origem. Para mais detalhes, consulte Auto [crie tabelas de pias](copy-activity-overview.md#auto-create-sink-tables). 
...
1. Confirme que os dados foram copiados para o target Azure Synapse Analytics que usou neste tutorial. 
