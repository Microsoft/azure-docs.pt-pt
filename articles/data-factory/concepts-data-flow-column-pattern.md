...
Para adicionar um padrão de coluna numa coluna derivada ou no separador Agregados de uma transformação agregada, clique no ícone plus à direita de uma coluna existente. **Selecione Adicionar o padrão da coluna**. 

![padrões de coluna](media/data-flow/columnpattern.png "Padrões de Coluna")

Utilize o construtor de [expressão](concepts-data-flow-expression-builder.md) para introduzir a condição de correspondência. Crie uma expressão booleana que corresponda às colunas com base no `name` `type` , e da `stream` `position` coluna. O padrão afetará qualquer coluna, derivada ou definida, onde a condição retorna verdadeiramente.
...
O padrão de coluna acima corresponde a cada coluna do tipo duplo e cria uma coluna agregada por correspondência. O nome da nova coluna é o nome da coluna combinada com "_total". O valor da nova coluna é a soma arredondada e agregada do duplo valor existente.
