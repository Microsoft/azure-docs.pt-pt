...
Por padrão, os autores da experiência de interface de utilizador da Azure Data Factory (UX) diretamente contra o serviço de fábrica de dados. Esta experiência tem as seguintes limitações:

- O serviço Data Factory não inclui um repositório para armazenar as entidades JSON para as suas alterações. A única forma de poupar alterações é através do botão **Publicar Todas** as alterações e todas as alterações são publicadas diretamente no serviço de fábrica de dados.
- O serviço Data Factory não está otimizado para o controlo de colaboração e versão.

Para proporcionar uma melhor experiência de autoria, a Azure Data Factory permite-lhe configurar um repositório Git com o Azure Repos ou o GitHub. Git é um sistema de controlo de versão que permite uma mudança mais fácil de rastrear e colaboração. Este tutorial irá delinear como configurar e trabalhar num repositório git, juntamente com destacar as melhores práticas e um guia de resolução de problemas.

> [!NOTE]
> A integração de git da fábrica de dados Azure não está disponível na Nuvem do Governo Azure.

## <a name="advantages-of-git-integration"></a>Vantagens da integração no Git

Abaixo está uma lista de algumas das vantagens que a integração git proporciona à experiência de autor:
...
-   **Defesas parciais:** Ao ser autor do serviço de fábrica de dados, não é possível guardar alterações como rascunho e todas as publicações devem passar na validação da fábrica de dados. Se os seus oleodutos não estão terminados ou simplesmente não quer perder alterações em caso de falha informática, a integração git permite alterações incrementais dos recursos da fábrica de dados, independentemente do estado em que se encontram. Configurar um repositório git permite-lhe guardar alterações, permitindo-lhe publicar apenas quando tiver testado as suas alterações à sua satisfação.
-   **Colaboração e controlo:** Se tiver vários membros da equipa a contribuir para a mesma fábrica, pode querer deixar que os seus companheiros de equipa colaborem uns com os outros através de um processo de revisão de códigos. Também pode configurar a sua fábrica de modo a que nem todos os contribuintes têm permissões iguais. Alguns membros da equipa só podem ser autorizados a fazer alterações via Git e apenas algumas pessoas na equipa estão autorizadas a publicar as alterações na fábrica.
-   **Melhor CI/CD:**  Se estiver a implantar em vários ambientes com um [processo de entrega contínua,](continuous-integration-deployment.md)a integração git facilita certas ações. Algumas destas ações incluem:
    -   Configure o seu gasoduto de libertação para disparar automaticamente assim que houver alterações na sua fábrica 'dev'.
    -   Personalize as propriedades da sua fábrica que estão disponíveis como parâmetros no modelo de Gestor de Recursos. Pode ser útil manter apenas o conjunto de propriedades necessáriocomo parâmetros, e ter todo o resto codificado.
-   **Melhor Desempenho:** Uma fábrica média com integração de gits carrega 10 vezes mais rápido do que uma autoria contra o serviço de fábrica de dados. Esta melhoria de desempenho deve-se ao descarregamento de recursos via Git.
...
O repo Azure Repos Git pode estar em um inquilino azure ative diretório diferente. Para especificar um inquilino do Azure AD diferente, tem de ter permissões de administrador para a subscrição do Azure que está a utilizar.
...
Recomenda-se utilizar o Azure Key Vault para armazenar quaisquer cordas de ligação ou senhas ou autenticação de identidade gerida para serviços ligados à fábrica de dados. Por razões de segurança, a fábrica de dados não guarda segredos em Git. Quaisquer alterações aos Serviços Linked que contenham segredos como palavras-passe são publicadas imediatamente no serviço Azure Data Factory.

A utilização de key vault ou de autenticação MSI também facilita a integração e implementação contínua, uma vez que não terá de fornecer estes segredos durante a implementação do modelo do Gestor de Recursos.
...
- Um utilizador tem vários ramos. Numa sucursal de recurso, eliminaram um serviço ligado que não está associado ao AKV (os serviços ligados não AKV são publicados imediatamente independentemente de estarem em Git ou não) e nunca fundiram a sucursal de recurso no ramo de colaboração.
...
- Um utilizador envia manualmente um serviço não-AKV ou um JSON de Funcionamento de Integração. Referem esse recurso a partir de outro recurso, como um conjunto de dados, um serviço ligado ou um pipeline. Um serviço não AKV ligado criado através do UX é publicado imediatamente porque as credenciais precisam de ser encriptadas. Se fizer o upload de um conjunto de dados referindo esse serviço ligado e tentar publicar, o UX permitirá porque existe no ambiente git. Será rejeitado no momento da publicação, uma vez que não existe no serviço de fábrica de dados.

## <a name="switch-to-a-different-git-repository"></a>Mude para um repositório git diferente

Para mudar para um repositório Git diferente, clique no ícone **definições git Repo** no canto superior direito da página de visão geral da Fábrica de Dados. Se não consegue ver o ícone, limpe a cache do navegador local. Selecione o ícone para remover a associação com o repo atual.
...
> [!IMPORTANT]
> Remover a configuração git de uma fábrica de dados não apaga nada do repositório. A fábrica conterá todos os recursos publicados. Pode continuar a editar a fábrica diretamente contra o serviço.
