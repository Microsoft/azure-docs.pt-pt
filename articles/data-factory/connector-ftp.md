...
|:--- |:--- |:--- |
| tipo | A propriedade tipo deve ser definida para: **FtpServer**. | Yes |
| anfitrião | Especifique o nome ou endereço IP do servidor FTP. | Yes |
| porta | Especifique a porta na qual o servidor FTP está a ouvir.<br/>Os valores permitidos são: inteiro, o valor por defeito é **de 21**. | No |
| ativarSl | Especificar se deve utilizar o FTP num canal SSL/TLS.<br/>Os valores permitidos são: **verdadeiros** (padrão), **falsos.** | No |
| enableServerCertificateValidation | Especificar se deve ativar a validação do certificado TLS/SSL do servidor quando estiver a utilizar o ftp sobre o canal SSL/TLS.<br/>Os valores permitidos são: **verdadeiros** (padrão), **falsos.** | No |
| authenticationType | Especifique o tipo de autenticação.<br/>Os valores permitidos são: **Básico,** **Anónimo** | Yes |
| userName | Especifique o utilizador que tem acesso ao servidor FTP. | No |
| palavra-passe | Especifique a palavra-passe para o utilizador (userName). Marque este campo como um SecureString para armazená-lo de forma segura na Data Factory, ou [fazer referência a um segredo armazenado no Cofre da Chave Azure](store-credentials-in-key-vault.md). | No |
...
| Propriedade   | Descrição                                                  | Necessário |
| ---------- | ------------------------------------------------------------ | -------- |
| tipo       | A propriedade tipo `location` em conjunto de dados deve ser definida como **FtpServerLocation**. | Yes      |
...
| Propriedade                 | Descrição                                                  | Necessário                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| tipo                     | A propriedade tipo em baixo `storeSettings` deve ser definida para **FtpReadSettings**. | Yes                                           |
...
| fileName | **Nome ou filtro wildcard** para os ficheiros sob a especificada "pastaPath". Se não especificar um valor para esta propriedade, o conjunto de dados aponta para todos os ficheiros da pasta. <br/><br/>Para o filtro, os wildcards permitidos são: `*` (corresponde a zero ou mais caracteres) e `?` (corresponde a zero ou caracteres individuais).<br/>- Exemplo 1:`"fileName": "*.csv"`<br/>- Exemplo 2:`"fileName": "???20180427.txt"`<br/>Use `^` para escapar se o seu nome de ficheiro real tiver wildcard ou este char de fuga dentro. |No |
