...
Almofadas esquerdas a corda pelo acolchoado fornecido até que tenha um certo comprimento. Se a corda for igual ou superior ao comprimento, então é aparada ao * ``lpad('dumbo', 10, '-') -> '-----dumbo'`` 
* ``lpad('dumbo', 4, '-') -> 'dumb'`` 
* comprimento ''lpad('dumbo', 8, '<>') -> '<><dumbo'``
___
### <code>ltrim</code>
...
As almofadas direitas da corda pelo acolchoado fornecido até que tenha um certo comprimento. Se a corda for igual ou maior do que o * ``rpad('dumbo', 10, '-') -> 'dumbo-----'`` 
* ``rpad('dumbo', 4, '-') -> 'dumb'`` 
* ``rpad('dumbo', 8, '<>') -> 'dumbo<><'`` 
___
### <code>rtrim</code>comprimento, então é aparada ao comprimento do rtrim</code>
...
## do ano de uma data As seguintes funções estão disponíveis apenas em transformações agregadas, pivô, despivote e janelas___
### <code>avg</code>
...
Com base num critério, obtém a variação imparcial de uma coluna * ``varianceSampleIf(region == 'West', sales)`` 
## Funções janela As seguintes funções só estão disponíveis em transformações de janelas___
### <code>cumeDist</code>
