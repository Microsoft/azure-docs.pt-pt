...
Adiciona um par de cordas ou números. Adiciona uma data a alguns dias. Adiciona uma duração a um carimbo de tempo. Anexa um conjunto de tipos semelhantes ao outro. O mesmo que o operador +* ``add(10, 20) -> 30``  
* ``10 + 20 -> 30``  
...
Adicione meses a uma data ou marca ção. Você pode opcionalmente passar um fuso horário* ``addMonths(toDate('2016-08-31'), 1) -> toDate('2016-09-30')``  
...
Calcula um valor tangencial inverso* ``atan(0) -> 0.0``  
___
### <code>atan2</code>
...
Devolve o menor inteiro não menor que o número* ``ceil(-0.1) -> 0``  
___
### <code>coalesce</code>
...
Recolhe todos os valores da expressão no grupo agregado numa matriz. As estruturas podem ser recolhidas e transformadas em estruturas alternativas durante este processo. O número de itens será igual ao número de linhas nesse grupo e pode conter valores nulos. O número de itens recolhidos deve ser pequeno* ``collect(salesPerson)``
...
Compara dois valores do mesmo tipo. Retorna o inteiro negativo se o valor1 < valor2, 0 se valor1 == valor2, valor positivo se valor1 > valor2* ``(compare(12, 24) < 1) -> true``  
...
Calcula uma cosina hiperbólica de um valor* ``cosh(0) -> 1.0``  
___
### <code>crc32</code>
...
Divide par de números. O mesmo que o /operador* ``divide(20, 10) -> 2``  
* ``20 / 10 -> 2``  
___
### <code>endsWith</code>
...
Sempre devolve um valor falso. Utilize a sintaxe da função (falsa)se existir uma coluna denominada "falsa"* ``(10 + 20 > 30) -> false``  
* ``(10 + 20 > 30) -> false()``  
___
### <code>filter</code>
...
Devolve o maior inteiro não maior que o número* ``floor(-0.1) -> -1``  
___
...
Comparar maior operador. O mesmo que > operador* ``greater(12, 24) -> false``  
* ``('dumbo' > 'dum') -> true``  
* ``(toTimestamp('2019-02-05 08:21:34.890', 'yyyy-MM-dd HH:mm:ss.SSS') > toTimestamp('2019-02-03 05:19:28.871', 'yyyy-MM-dd HH:mm:ss.SSS')) -> true``  
___
### <code>greaterOrEqual</code>
<code><b>greaterOrEqual(<i>&lt;value1&gt;</i> : any, <i>&lt;value2&gt;</i> : any) => boolean</b></code><br/><br/>
Comparação maior ou igual operador. O mesmo que >= operador* ``greaterOrEqual(12, 12) -> true``  
* ``('dumbo' >= 'dum') -> true``  
___
### <code>greatest</code>
...
Com base numa condição aplica-se um valor ou outro. Se outros não especificados forem considerados NULOS. Ambos os valores devem ser compatíveis (numérico, corda...)* ``iif(10 + 20 == 30, 'dumbo', 'gumbo') -> 'dumbo'``  
...
Converte a primeira letra de cada palavra em maiúsculas. As palavras são identificadas como separadas pelo espaço branco* ``initCap('cool iceCREAM') -> 'Cool Icecream'``  
___
### <code>instr</code>
...
Verifique se a linha está marcada para apagar. Para transformações que toquem mais do que um fluxo de entrada pode passar o índice (1 baseado) do fluxo. O índice de fluxo deve ser 1 ou 2 e o valor padrão é 1* ``isDelete()``  
* ``isDelete(1)``  
___
### <code>isError</code>
<code><b>isError([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Verifica se a linha está marcada como um erro. Para transformações que toquem mais do que um fluxo de entrada pode passar o índice (1 baseado) do fluxo. O índice de fluxo deve ser 1 ou 2 e o valor padrão é 1* ``isError()``  
* ``isError(1)``  
___
### <code>isIgnore</code>
<code><b>isIgnore([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Verifica se a fila está marcada para ser ignorada. Para transformações que toquem mais do que um fluxo de entrada pode passar o índice (1 baseado) do fluxo. O índice de fluxo deve ser 1 ou 2 e o valor padrão é 1* ``isIgnore()``  
* ``isIgnore(1)``  
___
### <code>isInsert</code>
<code><b>isInsert([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Verifique se a linha está marcada para ser inserida. Para transformações que toquem mais do que um fluxo de entrada pode passar o índice (1 baseado) do fluxo. O índice de fluxo deve ser 1 ou 2 e o valor padrão é 1* ``isInsert()``  
* ``isInsert(1)``  
___
### <code>isMatch</code>
<code><b>isMatch([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Verifica se a fila está à altura da procura. Para transformações que toquem mais do que um fluxo de entrada pode passar o índice (1 baseado) do fluxo. O índice de fluxo deve ser 1 ou 2 e o valor padrão é 1* ``isMatch()``  
* ``isMatch(1)``  
___
### <code>isNull</code>
...
Verifique se a linha está marcada para atualização. Para transformações que toquem mais do que um fluxo de entrada pode passar o índice (1 baseado) do fluxo. O índice de fluxo deve ser 1 ou 2 e o valor padrão é 1* ``isUpdate()``  
* ``isUpdate(1)``  
___
### <code>isUpsert</code>
<code><b>isUpsert([<i>&lt;value1&gt;</i> : integer]) => boolean</b></code><br/><br/>
Verifique se a linha está marcada para ser inserida. Para transformações que toquem mais do que um fluxo de entrada pode passar o índice (1 baseado) do fluxo. O índice de fluxo deve ser 1 ou 2 e o valor padrão é 1* ``isUpsert()``  
* ``isUpsert(1)``  
___
...
Comparação menor do que ou igual operador. O mesmo que <= operador* ``least(10, 30, 15, 20) -> 10``  
...
Comparação menor do que ou igual operador. O mesmo que <= operador* ``lesserOrEqual(12, 12) -> true``  
* ``('dumbo' <= 'dum') -> false``  
___
...
Calcula o valor do registo. Uma base opcional pode ser fornecida e outro número Euler se usado* ``log(100, 10) -> 2``  
___
### <code>log10</code>
...
Diminui uma corda* ``lower('GunChus') -> 'gunchus'``  
___
### <code>lpad</code>
<code><b>lpad(<i>&lt;string to pad&gt;</i> : string, <i>&lt;final padded length&gt;</i> : integral, <i>&lt;padding&gt;</i> : string) => string</b></code><br/><br/>
Almofadas esquerdas a corda pelo acolchoado fornecido até que tenha um certo comprimento. Se a corda for igual ou maior do que o comprimento, então é aparada ao comprimento* ``lpad('dumbo', 10, '-') -> '-----dumbo'``  
* ``lpad('dumbo', 4, '-') -> 'dumb'``  
*''lpad('dumbo', 8, '<>') -> '<><dumbo'``  
___
### <code>ltrim</code>
...
Modulo de par de números. O mesmo que o operador %* ``mod(20, 8) -> 4``  
* ``20 % 8 -> 4``  
___
### <code>month</code>
...
Nega um número. Transforma números positivos em negativo e vice-versa* ``negate(13) -> -13``  
___
...
Devolve um valor NULO. Utilize a sintaxe da função (nula)se existir uma coluna denominada "nula". Qualquer operação que utilize resultará num NULO* ``isNull('dumbo' + null) -> true``  
* ``isNull(10 * null) -> true``  
* ``isNull('') -> false``  
* ``isNull(10 + 20) -> false``  
* ``isNull(10/0) -> true``  
...
Eleva um número para o poder de outro* ``power(10, 2) -> 100``  
___
### <code>reduce</code>
<code><b>reduce(<i>&lt;value1&gt;</i> : array, <i>&lt;value2&gt;</i> : any, <i>&lt;value3&gt;</i> : binaryfunction, <i>&lt;value4&gt;</i> : unaryfunction) => any</b></code><br/><br/>
Acumula elementos numa matriz. Reduzir espera uma referência a um acumulador e um elemento na primeira função de expressão como #acc e #item e espera que o valor resultante como #result seja usado na função de segunda expressão* ``toString(reduce(['1', '2', '3', '4'], '0', #acc + #item, #result)) -> '01234'``  
___
...
Substitua todas as ocorrências de uma subcadeia por outra subcadeia na corda dada. Se o último parâmetro for omitido, é padrão para a cadeia vazia* ``replace('doggie dog', 'dog', 'cat') -> 'catgie cat'``  
...
As almofadas direitas da corda pelo acolchoado fornecido até que tenha um certo comprimento. Se a corda for igual ou maior do que o comprimento, então é aparada ao comprimento* ``rpad('dumbo', 10, '-') -> 'dumbo-----'``  
* ``rpad('dumbo', 4, '-') -> 'dumb'``  
* ``rpad('dumbo', 8, '<>') -> 'dumbo<><'``  
___
### <code>rtrim</code>rtrim</code>
<code><b>rtrim(<i>&lt;string to trim&gt;</i> : string, [<i>&lt;trim characters&gt;</i> : string]) => string</b></code><br/><br/>
A direita apara uma série de personagens principais. Se o segundo parâmetro não for especificado, apara o espaço branco. Então apara qualquer personagem especificado no segundo parâmetro* ``rtrim('  dumbo  ') -> '  dumbo'``  
* ``rtrim('!--!du!mbo!', '-!') -> '!--!du!mbo'``  
___
### <code>second</code>
...
Extrai um subconjunto de uma matriz de uma posição. A posição é 1 baseada. Se o comprimento for omitido, é indefinido para o fim da corda* ``slice([10, 20, 30, 40], 1, 2) -> [10, 20]``  
...
Obtém o código soundex para a corda* ``soundex('genius') -> 'G520'``  
___
### <code>split</code>
...
Verifica se a corda começa com a corda fornecida* ``startsWith('dumbo', 'du') -> true``  
___
### <code>subDays</code>
<code><b>subDays(<i>&lt;date/timestamp&gt;</i> : datetime, <i>&lt;days to subtract&gt;</i> : integral) => datetime</b></code><br/><br/>
Subtrai a meses de uma data ou carimbo temporal. O mesmo que o operador para data* ``subDays(toDate('2016-08-08'), 1) -> toDate('2016-08-07')``  
___
### <code>subMonths</code>
...
Extrai uma subcadeia de um certo comprimento de uma posição. A posição é 1 baseada. Se o comprimento for omitido, é indefinido para o fim da corda* ``substring('Cat in the hat', 5, 2) -> 'in'``  
...
Converte qualquer numérico/data/carimbo/corda para representação binária* ``toBinary(3) -> [0x11]``  
___
### <code>toBoolean</code>
...
Converte a cadeia da data de entrada até à data utilizando um formato de data de entrada opcional. Consulte o SimpleDateFormat da Java para obter formatos disponíveis. Se o formato da data de entrada for omitido, o formato predefinido é yyyy-[M]M-[d]d. Os formatos aceites são "[yyyy, yyyy-[M]M, yyyy-[M]M-[d]d, yyyy-[M]M-[dT* ]* ``toDate('2012-8-18') -> toDate('2012-08-18')``  
...
Converte qualquer numérico ou corda para um valor inteiro. Um formato decimal java opcional pode ser usado para a conversão. Truncates qualquer longo, flutuante, duplo* ``toInteger(123) -> 123``  
* ``toInteger('123') -> 123``  
* ``toInteger('$123', '$###') -> 123``  
___
### <code>toLong</code>
<code><b>toLong(<i>&lt;value&gt;</i> : any, [<i>&lt;format&gt;</i> : string], [<i>&lt;locale&gt;</i> : string]) => long</b></code><br/><br/>
Converte qualquer numérico ou corda para um valor longo. Um formato decimal java opcional pode ser usado para a conversão. Truncates qualquer flutuador, duplo* ``toLong(123) -> 123``  
* ``toLong('123') -> 123``  
* ``toLong('$123', '$###') -> 123``  
___
### <code>toShort</code>
<code><b>toShort(<i>&lt;value&gt;</i> : any, [<i>&lt;format&gt;</i> : string], [<i>&lt;locale&gt;</i> : string]) => short</b></code><br/><br/>
Converte qualquer numérico ou corda a um valor curto. Um formato decimal java opcional pode ser usado para a conversão. Truncates qualquer inteiro, longo, flutuante, duplo* ``toShort(123) -> 123``  
* ``toShort('123') -> 123``  
* ``toShort('$123', '$###') -> 123``  
___
### <code>toString</code>
<code><b>toString(<i>&lt;value&gt;</i> : any, [<i>&lt;number format/date format&gt;</i> : string]) => string</b></code><br/><br/>
Converte um tipo de dados primitivo numa corda. Para números e data pode ser especificado um formato. Se não especificado, a predefinição do sistema é escolhida. O formato decimal java é usado para números. Consulte o Java SimpleDateFormat para todos os formatos de data possíveis; o formato padrão é yyyy-MM-dd* ``toString(10) -> '10'``  
...
Substitua um conjunto de caracteres por outro conjunto de caracteres na cadeia. Os caracteres têm 1 a 1 substituição* ``translate('(bojjus)', '()', '[]') -> '[bojjus]'``  
* ``translate('(gunchus)', '()', '[') -> '[gunchus'``  
___
### <code>trim</code>
<code><b>trim(<i>&lt;string to trim&gt;</i> : string, [<i>&lt;trim characters&gt;</i> : string]) => string</b></code><br/><br/>
Apara uma série de personagens principais e de rastos. Se o segundo parâmetro não for especificado, apara o espaço branco. Então apara qualquer personagem especificado no segundo parâmetro* ``trim('  dumbo  ') -> 'dumbo'``  
* ``trim('!--!du!mbo!', '-!') -> 'du!mbo'``  
___
### <code>true</code>
<code><b>true() => boolean</b></code><br/><br/>
Sempre devolve um verdadeiro valor. Utilize a sintaxe da função (verdadeira)se existir uma coluna denominada "verdadeira"* ``(10 + 20 == 30) -> true``  
* ``(10 + 20 == 30) -> true()``  
___
### <code>typeMatch</code>
<code><b>typeMatch(<i>&lt;type&gt;</i> : string, <i>&lt;base type&gt;</i> : string) => boolean</b></code><br/><br/>
Corresponde ao tipo da coluna. Só pode ser usado em expressões de padrão.número corresponde curto, inteiro, longo, duplo, flutuante ou decimal, fósforos integrais curtos, inteiros, jogos longos, fracionados duplos, flutuadores, decimais e data-data data ou tipo de marca ção* ``typeMatch(type, 'number')``  
...
##Funções agregadas As seguintes funções só estão disponíveis em transformações agregadas, pivô, pivô e janelas___
### <code>avg</code>
...
Com base em critérios obtém a média dos valores de uma coluna* ``avgIf(region == 'West', sales)``  
___
### <code>count</code>
...
Obtém a covariance populacional entre duas colunas* ``covariancePopulation(sales, profit)``  
___
### <code>covariancePopulationIf</code>
<code><b>covariancePopulationIf(<i>&lt;value1&gt;</i> : boolean, <i>&lt;value2&gt;</i> : number, <i>&lt;value3&gt;</i> : number) => double</b></code><br/><br/>
Com base num critério, obtém a covariance populacional de duas colunas* ``covariancePopulationIf(region == 'West', sales)``  
___
### <code>covarianceSample</code>
<code><b>covarianceSample(<i>&lt;value1&gt;</i> : number, <i>&lt;value2&gt;</i> : number) => double</b></code><br/><br/>
Obtém a amostra de coovariano de duas colunas* ``covarianceSample(sales, profit)``  
___
### <code>covarianceSampleIf</code>
<code><b>covarianceSampleIf(<i>&lt;value1&gt;</i> : boolean, <i>&lt;value2&gt;</i> : number, <i>&lt;value3&gt;</i> : number) => double</b></code><br/><br/>
Com base num critério, obtém a amostra de covariano de duas colunas* ``covarianceSampleIf(region == 'West', sales, profit)``  
___
### <code>first</code>
<code><b>first(<i>&lt;value1&gt;</i> : any, [<i>&lt;value2&gt;</i> : boolean]) => any</b></code><br/><br/>
Obtém o primeiro valor de um grupo de colunas. Se o segundo parâmetro ignorar Nulls é omitido, é assumido falso* ``first(sales)``  
* ``first(sales, false)``  
___
### <code>kurtosis</code>
...
Obtém o último valor de um grupo de colunas. Se o segundo parâmetro ignorar Nulls é omitido, é assumido falso* ``last(sales)``  
* ``last(sales, false)``  
___
### <code>max</code>
...
Com base em critérios, obtém o valor máximo de uma coluna* ``maxIf(region == 'West', sales)``  
___
### <code>mean</code>
<code><b>mean(<i>&lt;value1&gt;</i> : number) => number</b></code><br/><br/>
Obtém a média dos valores de uma coluna. O mesmo que o AVG* ``mean(sales)``  
___
### <code>meanIf</code>
<code><b>meanIf(<i>&lt;value1&gt;</i> : boolean, <i>&lt;value2&gt;</i> : number) => number</b></code><br/><br/>
Com base num critério, obtém a média dos valores de uma coluna. O mesmo que avgIf* ``meanIf(region == 'West', sales)``  
___
### <code>min</code>
...
Com base em critérios, obtém a variação populacional de uma coluna* ``variancePopulationIf(region == 'West', sales)``  
___
...
Com base num critério, obtém a variância imparcial de uma coluna* ``varianceSampleIf(region == 'West', sales)``  
...
Obtém o valor do primeiro parâmetro avaliado n linhas antes da linha atual. O segundo parâmetro é o número de linhas para olhar para trás e o valor padrão é 1. Se não houver tantas linhas, um valor de nulo é devolvido a menos que um valor predefinido seja especificado* ``lag(amount, 2)``  
* ``lag(amount, 2000, 100)``  
___
### <code>lead</code>
<code><b>lead(<i>&lt;value&gt;</i> : any, [<i>&lt;number of rows to look after&gt;</i> : number], [<i>&lt;default value&gt;</i> : any]) => any</b></code><br/><br/>
Obtém o valor do primeiro parâmetro avaliado n linhas após a linha atual. O segundo parâmetro é o número de linhas para olhar para a frente e o valor padrão é 1. Se não houver tantas linhas, um valor de nulo é devolvido a menos que um valor predefinido seja especificado* ``lead(amount, 2)``  
...
Atribui uma linha sequencial numerando para linhas em uma janela começando com 1* ``rowNumber()``  
