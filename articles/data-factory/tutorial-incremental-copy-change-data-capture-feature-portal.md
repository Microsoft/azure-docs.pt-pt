...
> [!NOTE]
> Para quem utiliza o SQL MI consulte [aqui](https://docs.microsoft.com/azure/data-factory/connector-azure-sql-database-managed-instance#prerequisites) informações sobre o acesso através de ponto final público vs privado. Se utilizar o ponto final privado, seria necessário executar este gasoduto utilizando um tempo de integração auto-hospedado. O mesmo se aplicaria aos que executam o SQL Server on-prem, em cenários VM ou VNet.
...
> [!NOTE]
> Note que o gatilho só será executado uma vez que tenha sido publicado. Adicionalmente, o comportamento esperado da janela de tumbling é executar todos os intervalos históricos desde a data de início até agora. Mais informações sobre os gatilhos das janelas de tumbling podem ser encontradas [aqui.](https://docs.microsoft.com/azure/data-factory/how-to-create-tumbling-window-trigger) 
