...
description: Saiba como copiar dados do Phoenix para armazenamentos de dados de coletor com suporte usando uma atividade de cópia em um pipeline de Azure Data Factory.
...
ms.locfileid: "73680437"
---
# <a name="copy-data-from-phoenix-using-azure-data-factory"></a>Copiar dados do Phoenix usando o Azure Data Factory 

Este artigo descreve como usar a atividade de cópia no Azure Data Factory para copiar dados do Phoenix. Ele se baseia no artigo [visão geral da atividade de cópia](copy-activity-overview.md) que apresenta uma visão geral da atividade de cópia.
...
As seções a seguir fornecem detalhes sobre as propriedades que são usadas para definir Data Factory entidades específicas ao conector Phoenix.
...
| hospedeira | O endereço IP ou nome do host do servidor de Phoenix. (ou seja, 192.168.222.160)  | Sim |
| porta | A porta TCP que o servidor Phoenix usa para escutar conexões de cliente. O valor padrão é 8765. Se você se conectar ao Azure HDInsights, especifique a porta como 443. | Não |
| httpPath | A URL parcial correspondente ao servidor de Phoenix. (ou seja,/gateway/sandbox/Phoenix/Version). Especifique `/hbasephoenix0` se estiver usando o cluster HDInsights.  | Não |
| authenticationType | O mecanismo de autenticação usado para se conectar ao servidor de Phoenix. <br/>Os valores permitidos são: **Anonymous**, **UsernameAndPassword**, **WindowsAzureHDInsightService** | Sim |
| o nome de utilizador | O nome de usuário usado para se conectar ao servidor de Phoenix.  | Não |
...
>[!NOTE]
>Se o cluster não oferecer suporte à sessão adesiva, por exemplo, HDInsight, adicione explicitamente o índice de nó no final da configuração do caminho http, por exemplo, especifique `/hbasephoenix0` em vez de `/hbasephoenix`.
...
Para obter uma lista completa das seções e propriedades disponíveis para definir conjuntos de os, consulte o artigo [conjuntos de valores](concepts-datasets-linked-services.md) . Esta seção fornece uma lista das propriedades com suporte pelo conjunto de acordos de Phoenix.

Para copiar dados do Phoenix, defina a propriedade Type do conjunto de dado como **phoenixobject**. As propriedades a seguir têm suporte:
...
Para obter uma lista completa de seções e propriedades disponíveis para definir atividades, consulte o artigo [pipelines](concepts-pipelines-activities.md) . Esta seção fornece uma lista das propriedades com suporte pela fonte Phoenix.

### <a name="phoenix-as-source"></a>Phoenix como fonte

Para copiar dados do Phoenix, defina o tipo de origem na atividade de cópia como **Phoenix**. As propriedades a seguir têm suporte na seção **origem** da atividade de cópia:

| Propriedade | Descrição | Necessário |
|:--- |:--- |:--- |
| tipo | A propriedade Type da fonte da atividade de cópia deve ser definida como: **Phoenix** | Sim |
