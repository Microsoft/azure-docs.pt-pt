...
Uma fábrica de dados pode ter um ou mais pipelines. Os pipelines são agrupamentos lógicos de atividades que, em conjunto, realizam uma tarefa. Por exemplo, um pipeline poderia conter um conjunto de atividades que ingerire limpar dados de registo, e, em seguida, iniciar um fluxo de dados de mapeamento para analisar os dados de registo. O pipeline permite-lhe gerir as atividades como um conjunto em vez de cada uma individualmente. Implanta-se e programa o oleoduto em vez das atividades de forma independente.

As atividades num pipeline definem as ações a efetuar nos seus dados. Por exemplo, pode utilizar uma atividade de cópia para copiar dados de um SQL Server no local para um Armazenamento de Blobs do Azure. Em seguida, utilize uma atividade de fluxo de dados ou uma atividade de Notebook databricks para processar e transformar dados do armazenamento blob para um pool Azure Synapse Analytics para além dos quais são construídas soluções de reporte de inteligência empresarial.

A Data Factory tem três agrupamentos de atividades: [atividades](copy-activity-overview.md)de movimento de dados, atividades de [transformação de dados](transform-data.md)e atividades de [controlo.](control-flow-web-activity.md) Uma atividade pode ter zero ou mais [conjuntos de dados](concepts-datasets-linked-services.md) de entrada e produzir um ou mais [conjuntos de dados](concepts-datasets-linked-services.md) de saída. O diagrama seguinte mostra a relação entre pipelines, atividades e conjuntos de dados no Data Factory:
...
concurrency | O número máximo de corridas simultâneas que o gasoduto pode ter. Por defeito, não há máximo. Se o limite de moeda for atingido, as corridas adicionais de gasoduto serão em fila até que as anteriores estejam concluídas | Number | Não 
