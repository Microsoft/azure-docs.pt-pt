...
| ***Localizar os ficheiros para copiar** _ |  |  |
...
| recursivo | Indica se os dados são lidos novamente a partir das sub-dobradeiras ou apenas a partir da pasta especificada. Quando a recursiva é definida como verdadeira e a pia é uma loja baseada em ficheiros, uma pasta ou sub-dobrador vazio não é copiado ou criado na pia. <br>Os valores permitidos são _true* (padrão) e *falsos*.<br>Esta propriedade não se aplica quando se `fileListPath` configura. |Não |
