...
| isEspEnabled      | Especifique '*verdadeiro*' se o cluster HDInsight for [o Pacote de Segurança Empresarial](https://docs.microsoft.com/azure/hdinsight/domain-joined/apache-domain-joined-architecture) ativado. O padrão é*falso.* | No       |
...
## <a name="azure-machine-learning-studio-linked-service"></a>Serviço ligado ao Azure Machine Learning Studio
Você cria um serviço ligado ao Azure Machine Learning Studio para registar um ponto final de pontuação de lote machine learning para uma fábrica de dados.
...
| updateResourceEndpoint | O URL de recurso de atualização para um ponto final do Serviço Web de Aprendizagem de Máquinas Azure utilizado para atualizar o serviço web preditivo com um ficheiro de modelo treinado | No                                       |
...
| inquilino                 | Especifique a informação do inquilino (nome de domínio ou ID do inquilino) sob a qual a sua aplicação reside. Pode recuperá-lo pairando sobre o rato no canto superior direito do portal Azure. | Necessário se a actualizaçãoResourceEndpoint for especificado | No |
| connectVia             | O Tempo de Integração a ser utilizado para despachar as atividades para este serviço ligado. Pode utilizar o Tempo de Execução da Integração Azure ou o Tempo de Execução de Integração Auto-hospedado. Se não for especificado, utiliza o tempo de execução de integração Azure predefinido. | No |
