...
A secção da **estrutura** é opcional. Define o esquema do conjunto de dados contendo uma recolha de nomes e tipos de dados de colunas. Utiliza a secção da estrutura para fornecer informações de tipo que são usadas para converter tipos e colunas de mapas da fonte para o destino. No exemplo seguinte, o conjunto de `slicetimestamp`dados `projectname`tem `pageviews`três colunas: , e . São do tipo String, String e Decimal, respectivamente.
...
| formato |Cadeia de formato a utilizar quando o `Datetime` `Datetimeoffset`tipo é do tipo .NET: ou . |Não |
...
| offset |Timepan pelo qual o início e o fim de todas as fatias de conjunto de dados são deslocados. <br/><br/>Note que se o **âncoraDateTime** e **offset** forem especificados, o resultado é a mudança combinada. |Não |ND |

### <a name="offset-example"></a>exemplo de compensação
Por padrão,`"frequency": "Day", "interval": 1`as fatias diárias () começam às 12:00 (meia-noite) Hora Universal Coordenada (UTC). Se pretender que a hora de início seja de 6 AM UTC, em vez disso, detete a contrapartida como mostrado no seguinte corte:
...
O conjunto de dados seguinte é mensal, e é produzido no dia 3`3.08:00:00`de cada mês às 8:00 am ( ):
...
## <a name="dataset-policy"></a><a name="Policy"></a>Política de conjunto de dados
...
| --- | --- | --- | --- | --- |
| mínimoTamanhoMB |Valida que os dados no **armazenamento do Blob Azure** cumprem os requisitos mínimos de tamanho (em megabytes). |Armazenamento de Blobs do Azure |Não |ND |
...
| --- | --- | --- | --- |
| dadosAtraso |O tempo para atrasar a verificação da disponibilidade dos dados externos para a fatia dada. Por exemplo, pode atrasar uma verificação horária utilizando esta definição.<br/><br/>A definição aplica-se apenas ao presente. Por exemplo, se for 1:00 PM agora e este valor for de 10 minutos, a validação começa às 13:10.<br/><br/>Note que esta definição não afeta fatias no passado. Fatias com dados do **tempo** + de corte**Atraso** < **Agora** são processados sem demora.<br/><br/>Os tempos superiores às 23:59 `day.hours:minutes:seconds` horas devem ser especificados utilizando o formato. Por exemplo, para especificar 24 horas, não use 24:00:00.00. Em vez disso, use 1:00:00:00. Se utilizar 24:00:00, é tratado como 24 dias (24.00:00:00). Durante 1 dia e 4 horas, especifique 1:04:00:00. |Não |0 |
