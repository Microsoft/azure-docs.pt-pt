...
Também pode usar este script abaixo para gerar um haxixe de linha usando todas as colunas que estão presentes no seu fluxo, sem precisar de nomear cada coluna:
...
Este código funcionará como a ```string_agg()``` função T-SQL e agregará valores de cadeia numa matriz. Em seguida, pode lançar essa matriz numa corda para usar com destinos SQL.
