...
Para problemas de conector, tais como um erro de encontro utilizando a atividade de cópia, consulte [os Conectores](connector-troubleshoot-guide.md)da Fábrica de Dados Troubleshoot Azure .
...
- **Causa:** O aglomerado foi encerrado. Para aglomerados interativos, esta questão pode ser uma condição de raça.

- **Recomendação**: Para evitar este erro, utilize grupos de trabalho.

### <a name="error-code-3204"></a>Código de erro: 3204

- **Mensagem:**`Job execution failed.`

- **Causa**: As mensagens de erro indicam vários problemas, tais como um estado de cluster inesperado ou uma atividade específica. Muitas vezes, não aparece nenhuma mensagem de erro.
...
- **Recomendação**: Reduzir o número de postos de trabalho submetidos à Data Lake Analytics. Ou altera os gatilhos da Data Factory e as configurações de condivisões nas atividades, ou aumenta os limites no Data Lake Analytics.
...
- **Recomendação**: Verifique se o principal de serviço ou certificado que o utilizador fornece para os trabalhos de Data Lake Analytics tem acesso tanto à conta Data Lake Analytics, como à instância padrão de Armazenamento de Data Lake da pasta raiz.
...
- **Recomendação**: O trabalho foi submetido ao Data Lake Analytics, e o guião lá, ambos falharam. Investigue em Data Lake Analytics. No portal, vá à conta data Lake Analytics e procure o trabalho utilizando o ID de execução da atividade da Fábrica de Dados (não utilize o ID de execução do pipeline). O trabalho lá fornece mais informações sobre o erro, e vai ajudá-lo a resolver problemas.

   Se a resolução não for clara, contacte a equipa de suporte do Data Lake Analytics e forneça o trabalho de Localizador de Recursos Universais (URL), que inclui o nome da sua conta e o ID do trabalho.
...
- **Recomendação**: Os Métodos Httpmethods suportados são: PUT, POST, GET, DELETE, OPTIONS, HEAD e TRACE.

### <a name="error-code-3603"></a>Código de erro: 3603

- **Mensagem:**`Response Content is not a valid JObject.`

- **Causa**: A função Azure que foi chamada não devolveu uma Carga Útil JSON na resposta. A atividade da Azure Data Factory (ADF) Azure suporta apenas o conteúdo de resposta da JSON.

- **Recomendação**: Atualizar a função Azure para devolver uma carga útil válida da JSON, tal como uma função C# pode regressar`(ActionResult)new OkObjectResult("{\"Id\":\"123\"}");`
...
- **Recomendação**: Fixe os detalhes da função Azure e tente novamente.
...
- **Recomendação**: Verifique se a definição de função de entrada Azure A definição JSON tem dados de serviço ligados.
...
- **Recomendação**: Verifique se a definição json da atividade do Input AzureMLExecutePipeline tem corretamente ligado os dados do serviço.
...
- **Recomendação**: Verifique se a credencial é válida e de novo.
...
- **Recomendação**: Verifique se a credencial no Serviço Linked é válida e tem permissão para aceder ao Azure Machine Learning.
...
- **Causa:** Existe um erro de servidor no Azure Machine Learning.

- **Recomendação**: Voltar a tentar mais tarde. Contacte a equipa de Machine Learning Azure para obter ajuda se o problema continuar.
...
- **Recomendação**: Verifique se o Azure Machine Learning está a procurar mais registos de erros e, em seguida, fixe o gasoduto ML.
...
- **Recomendação**: Vá ao portal Azure e encontre o seu armazenamento, em seguida, copie e cole a cadeia de ligação no seu serviço ligado e tente novamente.

### <a name="error-code-2108"></a>Código de erro: 2108

- **Mensagem:**`Error calling the endpoint '%url;'. Response status code: '%code;'`

- **Causa**: O pedido falhou devido a um problema subjacente, como conectividade de rede, falha dNS, validação do certificado do servidor ou timeout.
...
- **Recomendação**: Verifique se o tipo de serviço ligado é um dos tipos suportados para a atividade. Por exemplo, o tipo de serviço ligado para as atividades de HDI pode ser HDInsight ou HDInsightOnDemand.
...
- **Causa:** O tipo de propriedade fornecida não está correto.

- **Recomendação**: Fixe o tipo de propriedade e tente novamente.
...
- **Causa**: O tipo de nuvem não é suportado ou não pode ser determinado para armazenamento a partir do EndpointSuffix.

- **Recomendação**: Use o armazenamento noutra nuvem e tente novamente.
...
- **Recomendação**: Certifique-se de que o ficheiro executável existe. Se o programa começou, verifique se *stdout.txt* e *stderr.txt* foram enviados para a conta de armazenamento. É uma boa prática incluir registos no seu código para depuração.
...
- **Recomendação**: As atividades personalizadas achatam a estrutura da pasta sob pastaCaminho. Se precisar de preservar a estrutura da pasta, feche os ficheiros e extrai-os no Lote Azure utilizando um comando desfecho.
...
- **Recomendação**: Contacte o apoio da ADF para obter mais assistência.
...
- **Causa**: Houve um erro ao criar o cluster DeI a pedido.
...
- **Recomendação**: Esta questão pode ser transitória. Tente novamente o seu trabalho, e se o problema persistir, contacte o apoio da ADF para obter mais assistência.
...
- **Recomendação**: Tente outro armazenamento. Se esta opção não for uma solução aceitável, contacte a equipa de apoio da ADF para obter mais assistência.
...
- **Recomendação**: Considere a prestação de um diretor de serviço, que tenha permissões para criar um cluster HDInsight na subscrição fornecida e tentar novamente. Verifique se as [Identidades de Gestão estão corretamente configuradas](https://docs.microsoft.com/azure/hdinsight/hdinsight-managed-identities).

   Se esta opção não for uma solução aceitável, contacte a equipa de apoio da ADF para obter mais assistência.
...
- **Recomendação**: Verifique se o cluster não foi eliminado e se o URI fornecido está correto. Ao abrir o URI num browser, deverá ver o Ambari UI. Se o cluster estiver numa rede virtual, o URI deve ser o URI privado. Para abri-la, utilize uma Máquina Virtual (VM) que faça parte da mesma rede virtual.
   ...
   Para mais informações, consulte [a ligação direta aos serviços Apache Hadoop](https://docs.microsoft.com/azure/hdinsight/hdinsight-plan-virtual-network-deployment#directly-connect-to-apache-hadoop-services).
...
- **Recomendação**: O problema pode ser a conectividade Geral HDInsight ou a conectividade da rede. Primeiro confirme que o HDInsight Ambari UI está disponível em qualquer navegador. Então verifique se as suas credenciais ainda são válidas.
   ...
   Se estiver a utilizar um tempo de execução integrado auto-hospedado (IR), execute este passo a partir do VM ou da máquina onde o IR auto-hospedado está instalado. Em seguida, tente submeter o trabalho de volta à Data Factory. Se ainda falhar, contacte a equipa da Data Factory para obter apoio.

   Para mais informações, leia [A UI da Ambari](https://docs.microsoft.com/azure/hdinsight/hdinsight-hadoop-manage-ambari#ambari-web-ui)Web .
...
- **Recomendação**: Corrija as credenciais e reimplante o serviço ligado. Primeiro verifique se as credenciais funcionam no HDInsight abrindo o cluster URI em qualquer navegador e tentando iniciar sessão. Se as credenciais não funcionarem, pode redefini-las a partir do portal Azure.
   ...
   Para o cluster ESP, redefinir a palavra-passe através do reset da [palavra-passe self service](https://docs.microsoft.com/azure/active-directory/user-help/active-directory-passwords-update-your-own-password).
...
- **Recomendação**: Ocorre frequentemente um erro de 502 quando o seu processo ambari Server foi desligado. Pode reiniciar os Serviços Ambari reiniciando o nó da cabeça.
    ...
    1. Reinicie o nó de cabeça ativo.
       ...
       Para mais informações, veja a documentação de resolução de problemas do Azure HDInsight. Por exemplo:

       * [Ambari UI 502 error](https://hdinsight.github.io/ambari/ambari-ui-502-error.html) (Erro de 502 de IU do Ambari)
       * [RpcTimeoutExcep para servidor de segunda mão apache Spark](https://docs.microsoft.com/azure/hdinsight/spark/apache-spark-troubleshoot-rpctimeoutexception)
       * [Resolução de problemas de erros de gateway no Gateway](https://docs.microsoft.com/azure/application-gateway/application-gateway-troubleshooting-502)de Aplicação .
...
- **Recomendação**: Limite o número de postos de trabalho simultâneos submetidos ao HDInsight. Consulte a moeda da atividade da Data Factory se os postos de trabalho estiverem a ser submetidos pela mesma atividade. Mude os gatilhos para que as condutas de gasodutos simultâneos se espalhem ao longo do tempo.
   ...
   Consulte a [documentação do HDInsight](https://docs.microsoft.com/azure/hdinsight/hdinsight-hadoop-templeton-webhcat-debug-errors) para ajustar `templeton.parallellism.job.submit` como o erro sugere.
...
- **Causa**: O trabalho foi submetido ao cluster de IDE e falhou lá.

- **Recomendação:** 

 1. Verifique ambari UI:
    1. Certifique-se de que todos os serviços ainda estão em funcionamento.
    1. Da Ambari UI, verifique a secção de alerta no seu painel de instrumentos.
    ...
    1. Reveja a sua memória de ARN. Se a sua memória de ARN for elevada, o processamento dos seus trabalhos pode ser adiado. Se não tiver recursos suficientes para acomodar a sua aplicação/trabalho Spark, dimensione o cluster para garantir que o cluster tem memória e núcleos suficientes. 
...
- **Causa**: Os serviços ligados ao armazenamento utilizados no serviço ligado ao HDInsight (HDI) ou na atividade HDI, estão configurados com uma autenticação MSI que não é suportada.

- **Recomendação**: Forneça cordas de ligação completas para as contas de armazenamento utilizadas no serviço ligado ao HDI ou na atividade HDI.
...
- **Causa**: As informações de ligação para o cluster HDI estão incorretas, o utilizador desde que não tenha permissões para realizar a ação necessária, ou o serviço HDInsight tem problemas em responder aos pedidos da ADF.

- **Recomendação**: Verifique se a informação do utilizador está correta e que o Ambari UI para o cluster HDI pode ser aberto num browser a partir do VM onde o IR está instalado (para um IR auto-hospedado), ou pode ser aberto a partir de qualquer máquina (para O IR Azure).
...
- **Causa**: A JSON fornecida para a ação do guião é inválida.

- **Recomendação**: A mensagem de erro deve ajudar a identificar o problema. Fixe a configuração json e tente novamente.

   Consulte o serviço ligado à procura do [Azure HDInsight](https://docs.microsoft.com/azure/data-factory/compute-linked-services#azure-hdinsight-on-demand-linked-service) para obter mais informações.
...
- **Recomendação**: Siga a mensagem de erro para corrigir o problema. Se não houver informações suficientes para o resolver, contacte a equipa de HDI e forneça-lhes o ID do lote e o ID do trabalho, que pode ser encontrado na execução de atividade Output na página de Monitorização ADF. Para resolver ainda mais os problemas, recolha o registo completo do trabalho do lote.

   Para obter mais informações sobre como recolher o registo completo, consulte [O registo completo de um trabalho de lote](https://docs.microsoft.com/rest/api/hdinsightspark/hdinsight-spark-batch-job#get-the-full-log-of-a-batch-job).
...
- **Recomendação**: Siga os links na página de monitorização da execução da atividade na página de monitorização ADF para resolver o problema do cluster HDInsight Spark. Contacte a equipa de suporte HDInsight para obter mais assistência.
...
- **Recomendação**: Lotes de resolução de problemas no cluster HDInsight Spark. Contacte o suporte HDInsight para obter mais assistência. 
   ...
   Para obter mais informações sobre como recolher o registo completo, consulte [O registo completo de um trabalho](https://docs.microsoft.com/rest/api/hdinsightspark/hdinsight-spark-batch-job#get-the-full-log-of-a-batch-job)de lote , e partilhe o registo completo com suporte HDInsight para obter mais assistência.
...
- **Recomendação**: Forneça um caminho para um ficheiro que exista.
...
- **Recomendação**: Selecione um IR Azure e tente novamente.
...
- **Causa**: Ou as credenciais fornecidas estão erradas para o cluster, ou houve uma configuração de rede ou problema de ligação, ou o IR está com problemas de ligação ao cluster.
...
- **Recomendação:** 
    ...
    1. Se o IR auto-hospedado não está a ser usado, então o cluster HDI deve ser acessível publicamente. Abra o Ambari UI num navegador e verifique se abre. Se houver algum problema com o cluster ou os serviços nele, contacte a equipa de suporte da HDInsight para obter assistência.

       O URL do cluster HDI utilizado no serviço ligado ao ADF deve estar acessível para ADF IR (auto-hospedado ou Azure) para que a ligação de teste passe e para corridas para funcionar. Este estado pode ser verificado abrindo o URL a partir de um navegador, quer a partir de VM, quer de qualquer máquina pública.
...
- **Recomendação**: Forneça as credenciais corretas para ligar ao HDI e tente novamente.
...
- **Recomendação**: Verifique se o script existe e que o serviço associado ligado tem as credenciais adequadas para uma ligação.
...
- **Causa:** A Df tentou estabelecer uma ligação de conectividade de base de dados aberta (ODBC) ao cluster HDI, e falhou com um erro.

- **Recomendação:** 

   1. Confirme se configura corretamente a sua ligação de conectividade de base de dados ODBC/Java (JDBC).
      1. Para a JDBC, se estiver a utilizar a mesma rede virtual, pode obter esta ligação a partir de:<br>
        `Hive -> Summary -> HIVESERVER2 JDBC URL`
      1. Para garantir que tem a configuração correta do JDBC, consulte [a Hive Anígua De apurada através do condutor JDBC em HDInsight](https://docs.microsoft.com/azure/hdinsight/hadoop/apache-hadoop-connect-hive-jdbc-driver).
      1. Para abrir a base de dados (ODB), consulte [Tutorial: Consulta Apache Hive com ODBC e PowerShell](https://docs.microsoft.com/azure/hdinsight/interactive-query/apache-hive-query-odbc-driver-powershell) para garantir que tem a configuração correta. 
   1. Verifique se o Hiveserver2, a Hive Metastore e o Hiveserver2 Interactive estão ativos e a funcionar. 
   1. Verifique a interface de utilizador ambari (UI):
      1. Certifique-se de que todos os serviços ainda estão em funcionamento.
      1. A partir da Ambari UI, verifique a secção de alerta no seu painel de instrumentos.
         1. Para obter mais informações sobre alertas e resoluções para alertas, consulte [Gerir e Monitorizar um Cluster ](https://docs.cloudera.com/HDPDocuments/Ambari-2.7.5.0/managing-and-monitoring-ambari/content/amb_predefined_alerts.html).
...
- **Recomendação**: Apenas as cordas de ligação completas são suportadas no principal serviço ligado ao armazenamento para atividades de IDE. Verifique se não está a utilizar autorizações ou pedidos de MSI.
...
- **Recomendação**: Este erro ocorre quando a ADF se prepara para atividades de IDE e tenta copiar ficheiros para o armazenamento principal antes de submeter o trabalho ao HDI. Verifique se existem ficheiros na localização fornecida e se a ligação de armazenamento está correta. Dado que as atividades da ADF HDI não suportam a autenticação MSI em contas de armazenamento relacionadas com as atividades de IDE, verifique se esses serviços ligados têm chaves completas ou estão a utilizar o Cofre chave Azure.
...
- **Recomendação**: Verifique se o ficheiro existe realmente e se o serviço ligado com informações de ligação que apontam para este ficheiro tem as credenciais corretas.
...
- **Recomendação**: Verifique se as propriedades do serviço ligado ao armazenamento de ficheiros estão corretamente configuradas.
...
- **Recomendação**: Verifique se as propriedades do serviço ligado ao armazenamento do script estão corretamente configuradas.
...
- **Recomendação**: Verifique se o serviço ligado selecionado tem um dos tipos suportados para a atividade. As atividades da HDI suportam serviços ligados ao AzureBlobStorage e ao AzureBlobFSStorage.

   Para mais informações, leia Compare opções de [armazenamento para uso com clusters Azure HDInsight](https://docs.microsoft.com/azure/hdinsight/hdinsight-hadoop-compare-storage-options)
...
- **Recomendação**: Abra o portal Azure e tente encontrar o recurso HDI com o nome fornecido e, em seguida, verifique o estado de provisionamento. Contacte a equipa de suporte HDInsight para obter mais assistência.
...
- **Recomendação**: Forneça uma conta de armazenamento Azure Blob como um armazenamento adicional para o serviço ligado à procura hDInsight.
...
- **Causa**: Este problema deve-se à conectividade da Rede, a uma falha dNS, a uma validação de certificado de servidor ou a um tempo limite.

- **Recomendação**: Validar que o ponto final que está a tentar atingir está a responder aos pedidos. Pode utilizar ferramentas como **O Violinista/Carteiro**.

### <a name="error-code-2108"></a>Código de erro: 2108

- **Mensagem:**`Error calling the endpoint '%url;'. Response status code: '%code;'`

- **Causa**: O pedido falhou devido a um problema subjacente, como conectividade de rede, falha dNS, validação de certificado de servidor ou um tempo limite.
