...
* Quando você clica no coletor no modo de exibição de nó, verá a linhagem de coluna. Há três métodos diferentes que as colunas são acumuladas em todo o fluxo de dados para chegar ao coletor. São:
  ...
  * Computada Você usa a coluna para processamento condicional ou dentro de uma expressão em seu fluxo de dados, mas não a Lande no coletor
  * Gerados A coluna é uma nova coluna que você gerou em seu fluxo, ou seja, ela não estava presente na origem
  * Adequados A coluna originada da origem e sua está mapeando para um campo de coletor
