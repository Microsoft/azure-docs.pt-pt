...
Os IPs públicos em VNETs baseados em ARM são atualmente o único tipo de recurso protegido. Os serviços PaaS (multitenant) não são suportados na apresentação. Ver [Azure DDoS Protection Standard arquiteturas de referência.](ddos-protection-reference-architectures.md)

## <a name="are-classicrdfe-protected-resources-supported"></a>Os recursos clássicos/RDFE estão protegidos?
Apenas os recursos protegidos baseados na ARM são suportados na pré-visualização. As VMs nas implementações Classic/RDFE não são suportadas. O apoio não está atualmente previsto para os recursos Clássicos/RDFE. Ver [Azure DDoS Protection Standard arquiteturas de referência.](ddos-protection-reference-architectures.md)
...
Ver [Azure DDoS Protection Standard arquiteturas de referência.](ddos-protection-reference-architectures.md)
...
Não, a personalização da política não está disponível neste momento.

## <a name="can-i-allowlistbloclist-specific-ip-addresses"></a>Posso permitir endereços IP específicos de lista/bloclist?
