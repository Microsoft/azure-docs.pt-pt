...
Você pode restaurar um servidor para outra região do Azure em que o serviço estará disponível se você tiver configurado o servidor para backups com redundância geográfica. Se um incidente de grande escala em uma região resultar na indisponibilidade do seu aplicativo de banco de dados, você poderá restaurar um servidor de backups com redundância geográfica para um servidor em qualquer outra região. Há um atraso entre o momento em que um backup é feito e quando ele é replicado em uma região diferente. Esse atraso pode ser de até uma hora, portanto, se ocorrer um desastre, pode haver uma perda de dados de até uma hora.
