...
> [!NOTE]
> Neste momento, você deve solicitar acesso para usar esse recurso. Para isso, contacte AskAzureDBforPostgreSQL@service.microsoft.com.

A Base de Dados Azure para encriptação de dados de servidor único PostgreSQL com uma chave gerida pelo cliente permite-lhe trazer a sua própria chave (BYOK) para a proteção de dados em repouso. Permite ainda que as organizações implementem a separação de deveres na gestão de chaves e dados. Com a encriptação gerida pelo cliente, é responsável e, em pleno controlo, pelo ciclo de vida de uma chave, permissões de utilização chave e auditoria de operações em chaves.

Para a Base de Dados Azure para o servidor Single PostgreSQL, define a encriptação de dados ao nível do servidor. Com esta forma de encriptação de dados, utiliza a chave na encriptação da chave de encriptação de dados (DEK). O DEK é uma chave assimétrica gerida pelo cliente, armazenada numa instância [azure key vault](../key-vault/key-Vault-secure-your-key-Vault.md) gerida pelo cliente e gerida pelo cliente. A DEK é descrita mais detalhadamente mais tarde neste artigo.
...
* Key Vault e Azure Database para o servidor Single PostgreSQL devem pertencer ao mesmo inquilino azure Ative Directory (Azure AD). O Cofre chave do inquilino cruzado e as interações do servidor não são suportadas. A movimentação de recursos posteriormente exige que você reconfigure a criptografia de dados.
