...
|pg_replica_log_delay_in_bytes|Atraso máximo entre réplicas|Bytes|A latência em bytes entre o mestre e a réplica mais retardada. Essa métrica está disponível somente no servidor mestre.|
|pg_replica_log_delay_in_seconds|Atraso de réplica|Segundos|O tempo desde a última transação reproduzida. Essa métrica está disponível somente para servidores de réplica.|

## <a name="server-logs"></a>Registos do servidor
Você pode habilitar o registro em log em seu servidor. Esses logs também estão disponíveis por meio dos logs de diagnóstico do Azure em [logs de Azure monitor](../azure-monitor/log-query/log-query-overview.md), hubs de eventos e conta de armazenamento. Para saber mais sobre registro em log, visite a página [logs do servidor](concepts-server-logs.md) .
...
- Leia nosso blog sobre [as práticas recomendadas para monitorar o servidor](https://azure.microsoft.com/blog/best-practices-for-alerting-on-metrics-with-azure-database-for-postgresql-monitoring/).
