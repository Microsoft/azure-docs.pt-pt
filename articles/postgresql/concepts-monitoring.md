...
A Base de Dados Azure para PostgreSQL fornece várias métricas que dão uma visão sobre o comportamento dos recursos que suportam o servidor PostgreSQL. Cada métrica é emitida com uma frequência de um minuto, e tem até 30 dias de história. Pode configurar alertas sobre as métricas. Para orientação passo a passo, consulte [como configurar alertas](howto-alert-on-metric.md). Outras tarefas incluem a criação de ações automatizadas, a realização de análises avançadas e o histórico de arquivamento. Para mais informações, consulte a visão geral das [Métricas Azure](../monitoring-and-diagnostics/monitoring-overview-metrics.md).
...
|pg_replica_log_delay_in_bytes|Max Lag Através de Réplicas|Bytes|O lag in bytes entre o mestre e a réplica mais atrasada. Esta métrica está disponível apenas no servidor principal.|
...
[Consulta Performance Insight](concepts-query-performance-insight.md) trabalha em conjunto com a Query Store para fornecer visualizações acessíveis a partir do portal Azure. Estes gráficos permitem identificar consultas chave que impactam o desempenho. A Consulta Performance Insight está acessível a partir da secção de resolução de problemas de **Suporte + Resolução** de Problemas da sua Base de Dados Azure para a página do portal do servidor PostgreSQL.
