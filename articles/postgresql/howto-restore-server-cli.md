...
Ao criar um servidor por meio do comando `az postgres server create`, o parâmetro `--geo-redundant-backup` decide sua opção de redundância de backup. Se `Enabled`, os backups com redundância geográfica serão feitos. Ou se `Disabled` backups com redundância local forem feitos. 

O período de retenção de backup é definido pelo parâmetro `--backup-retention-days`. 

Para obter mais informações sobre como definir esses valores durante a criação, consulte o [início rápido da CLI do servidor do banco de dados do Azure para PostgreSQL](quickstart-create-server-database-azure-cli.md).
...
Para restaurar o servidor, use o comando CLI do Azure [AZ postgres Server Restore](/cli/azure/postgres/server) .
...
O comando `az postgres server restore` requer os seguintes parâmetros:
...
Para criar um servidor usando um backup com redundância geográfica, use o comando CLI do Azure `az postgres server georestore`.
...
```
Este comando cria um novo servidor chamado *mydemoserver-georestore* no leste dos EUA que pertencerá ao *MyResource*. É um servidor Uso Geral, Gen 4 com 8 vCores. O servidor é criado a partir do backup com redundância geográfica de *mydemoserver*, que também está no grupo de recursos *MyResource* Group
...
O comando `az postgres server georestore` requer os seguintes parâmetros:
...
|nome de SKU| GP_Gen4_8 | Esse parâmetro define o tipo de preço, a geração de computação e o número de vCores do novo servidor. GP_Gen4_8 mapeia para um servidor Uso Geral, Gen 4 com 8 vCores.|
