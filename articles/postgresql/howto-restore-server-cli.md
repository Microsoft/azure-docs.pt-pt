...
description: Aprenda a definir configurações de backup e restaurar um servidor na Base de Dados Azure para PostgreSQL - Servidor Único utilizando o Azure CLI.
author: rachel-msft
ms.author: raagyema
...
---
# <a name="how-to-back-up-and-restore-a-server-in-azure-database-for-postgresql---single-server-using-the-azure-cli"></a>Como fazer cópias de segurança e restaurar um servidor na Base de Dados Azure para PostgreSQL - Servidor Único utilizando o Azure CLI
...
Ao criar um `az postgres server create` servidor através `--geo-redundant-backup` do comando, o parâmetro decide a sua Opção de Redundância de Backup. Se, `Enabled`são tomadas cópias de segurança georedundantes. Ou `Disabled` se forem tomadas cópias de segurança redundantes localmente. 

O período de retenção de `--backup-retention-days`reserva é definido pelo parâmetro . 

Para obter mais informações sobre a definição destes valores durante a criação, consulte a Base de [Dados Azure para o servidor PostgreSQL CLI Quickstart](quickstart-create-server-database-azure-cli.md).
...
Para criar um servidor utilizando uma cópia de `az postgres server georestore` segurança georedundante, utilize o comando Azure CLI.
...
```
Este comando cria um novo servidor chamado *mydemoserver-georestored* no Leste dos EUA que pertencerá ao *myresourcegroup*. É um General Purpose, servidor Gen 4 com 8 vCores. O servidor é criado a partir da cópia de segurança georedundantdo do *mydemoserver,* que também está no grupo de recursos *myresourcegroup*
...
|sku-name| GP_Gen4_8 | Este parâmetro define o nível de preços, a geração de cálculo e o número de vCores do novo servidor. GP_Gen4_8 mapas para um Propósito Geral, servidor Gen 4 com 8 vCores.|
