...
Crie um servidor flexível com o [comando de criação de servidor flexível az postgreSQL.](/cli/azure/postgresql/flexible-server?view=azure-cli-latest&preserve-view=true) O seguinte comando cria um servidor utilizando predefinições e valores de serviço a partir do contexto local do seu Azure CLI:
