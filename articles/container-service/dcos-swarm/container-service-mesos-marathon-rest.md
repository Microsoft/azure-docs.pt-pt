---
title: (PRECADO) Gerir cluster Azure DC/OS com Marathon REST API
description: Coloque os recipientes num cluster de serviço de contentores Azure DC/OS utilizando a API Marathon REST.
author: iainfoulds
...
---
# <a name="deprecated-dcos-container-management-through-the-marathon-rest-api"></a>(PRECADO) Gestão de contentores DC/OS através da API marathon REST

[!INCLUDE [ACS deprecation](../../../includes/container-service-deprecation.md)]

O DC/OS fornece um ambiente para implementação e dimensionamento de cargas de trabalho em cluster, abstraindo o hardware subjacente. Na parte superior do DC/OS, existe uma estrutura que gere o agendamento e a execução de cargas de trabalho de computação. Embora existam quadros disponíveis para muitas cargas de trabalho populares, este documento faz com que você tenha começado a criar e escalar as implementações de contentores usando a API Marathon REST. 
...
## <a name="access-the-dcos-apis"></a>Aceda às APIs de DC/OS
Depois de estar ligado ao cluster de Serviço de Contentores Azure, pode aceder ao DC/OS e a relações REST APIs através de http: \/ /localhost:local-port. Os exemplos neste documento partem do princípio de que está a utilizar um túnel na porta 80. Por exemplo, os pontos finais da Maratona podem ser alcançados em URIs começando com http: \/ /localhost/marathon/v2/. 
...
Você implanta recipientes formatados por Docker através da API Marathon REST usando um ficheiro JSON que descreve a implantação pretendida. A amostra seguinte coloca um contentor Nginx num agente privado no aglomerado. 
...
Encontre o valor da `host` saída (um endereço IP semelhante `10.32.0.x` a), e o valor de `ports` .


Agora faça uma ligação terminal SSH (não uma ligação com túneis) à gestão FQDN do cluster. Uma vez ligado, faça o seguinte pedido, substituindo os valores corretos `host` `ports` de:
...
> [!NOTE]
> O URI é http: \/ /localhost/marathon/v2/apps/ seguido do ID da aplicação à escala. Se estiver a utilizar a amostra Nginx que é fornecida aqui, o URI será http: \/ /localhost/marathon/v2/apps/nginx.
...
> [!NOTE]
> O URI é http: \/ /localhost/marathon/v2/apps/ seguido do ID da aplicação à escala. Se estiver a utilizar a amostra Nginx fornecida aqui, o URI seria http: \/ /localhost/marathon/v2/apps/nginx.
...
* [Leia mais sobre os pontos finais mesos HTTP](https://mesos.apache.org/documentation/latest/endpoints/)
