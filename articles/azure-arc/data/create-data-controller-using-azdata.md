...
ms.locfileid: "90939944"
---
# <a name="create-azure-arc-data-controller-using-the-azure-data-cli-azdata"></a>Criar controlador de dados Azure Arc utilizando o CLI de dados Azure (azdata)
...
Para criar o Controlador de Dados Azure Arc utilizando o CLI de dados Azure, terá de ter o CLI de dados Azure instalado.

   [Instalar o CLI de Dados Azure](install-client-tools.md)
...
Siga a secção apropriada abaixo, dependendo da plataforma-alvo para configurar a sua criação.
...
Para criar um controlador de dados no Azure Red Hat OpenShift, terá de executar os seguintes comandos contra o seu cluster para relaxar as restrições de segurança. Trata-se de um requisito temporário que será removido no futuro.
> [!NOTE]
>   Use o mesmo espaço de nome aqui e no `azdata arc dc create` comando abaixo. O exemplo `arc` é.

Em primeiro lugar, descarregue a restrição de contexto de segurança personalizada (SCC) do [GitHub](https://raw.githubusercontent.com/microsoft/azure_arc/master/data_services/deployment/yaml/arc-data-scc.yaml) e aplique-a no seu cluster.
...
Para criar um controlador de dados na Plataforma de Contentores OpenShift do Chapéu Vermelho, terá de executar os seguintes comandos contra o seu cluster para relaxar as restrições de segurança. Trata-se de um requisito temporário que será removido no futuro.
> [!NOTE]
>   Use o mesmo espaço de nome aqui e no `azdata arc dc create` comando abaixo. O exemplo `arc` é.
...
Em primeiro lugar, comece por criar um novo ficheiro de perfil de implementação personalizado baseado no perfil de implementação azure-arc-openshift, executando o seguinte comando. Este comando criará um diretório `custom` no seu diretório de trabalho atual e um ficheiro de perfil de implantação personalizado `control.json` nesse diretório.
...
Por predefinição, o perfil de implementação azure-arc-openshift utiliza `NodePort` como tipo de serviço. Se estiver a utilizar um cluster OpenShift que esteja integrado com um equilibrador de carga, pode alterar a configuração para utilizar o tipo de serviço LoadBalancer utilizando o seguinte comando:
