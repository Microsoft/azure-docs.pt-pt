...
ms.locfileid: "107832865"
---
# <a name="enable-azure-vm-extensions-by-using-arm-template"></a>Ativar extensões de VM Azure utilizando o modelo ARM

Este artigo mostra-lhe como usar um modelo de Gestor de Recursos Azure (modelo ARM) para implementar extensões Azure VM, suportadas por servidores ativados pelo Azure Arc.
...
Para utilizar a extensão do agente Azure Monitor Dependency, é fornecida a seguinte amostra para funcionar no Windows e Linux. Se não estiver familiarizado com o agente de dependência, consulte [a visão geral dos agentes do Monitor Azure](../../azure-monitor/agents/agents-overview.md#dependency-agent).
...
## <a name="deploy-azure-key-vault-vm-extension-preview"></a>Implementar extensão VM do cofre da chave Azure (pré-visualização)

O JSON seguinte mostra o esquema para a extensão VM do Cofre de Chaves (pré-visualização). A extensão não requer configurações protegidas - todas as suas configurações são consideradas informações públicas. A extensão requer uma lista de certificados monitorizados, frequência de sondagens e a loja de certificados de destino. Especificamente:
...
## <a name="deploy-the-azure-defender-integrated-scanner"></a>Implementar o scanner integrado Azure Defender

Para utilizar a extensão do scanner integrado Azure Defender, é fornecida a seguinte amostra para funcionar no Windows e Linux. Se não estiver familiarizado com o scanner integrado, consulte [a visão geral da solução de avaliação de vulnerabilidade do Azure Defender](../../security-center/deploy-vulnerability-assessment-vm.md) para máquinas híbridas.
...
* Pode implementar, gerir e remover extensões VM utilizando o [Azure PowerShell,](manage-vm-extensions-powershell.md)do [portal Azure](manage-vm-extensions-portal.md), ou do [Azure CLI](manage-vm-extensions-cli.md).
