...
ms.locfileid: "81113469"
---
# <a name="create-a-suggester-to-enable-autocomplete-and-suggestions-in-azure-cognitive-search"></a>Crie um sugestionista para permitir a conclusão automática e sugestões na Pesquisa Cognitiva Azure

Na Pesquisa Cognitiva Azure, a funcionalidade "search-as-you-type" ou typeahead baseia-se numa construção **sugestionante** que se adiciona a um índice de [pesquisa](search-what-is-an-index.md). Um sugestionador suporta duas variantes de pesquisa como o tipo de pesquisa: *autocomplete*, que completa o termo ou frase que está a escrever, e *sugestões* que devolvem uma pequena lista de documentos correspondentes.  

A seguinte imagem, a partir da primeira aplicação Create na amostra [C#,](tutorial-csharp-type-ahead-and-suggestions.md) ilustra ambas as experiências. A Autocomplete antecipa o que o utilizador pode escrever, terminando "tw" com "in" como o termo de pesquisa prospetivo. Sugestões são resultados de pesquisa reais, cada um representando um documento correspondente. Para sugestões, pode emergir qualquer parte de um documento que melhor descreva o resultado. Neste exemplo, as sugestões são representadas pelo campo de nome do hotel.
...
Ao criar prefixos, um sugestor tem a sua própria cadeia de análise, semelhante à utilizada para a pesquisa completa de texto. No entanto, ao contrário da análise em plena pesquisa de texto, um sugestor só pode usar analisadores predefinidos (lucene padrão, [analisadores](index-add-language-analyzers.md)de linguagem , ou outros analisadores na lista de [analisadores predefinidos](index-add-custom-analyzers.md#predefined-analyzers-reference). [Os analisadores](index-add-custom-analyzers.md) e configurações personalizadas são especificamente proibidos de evitar configurações aleatórias que produzem maus resultados.
