...
Para consultas completas de pesquisa de texto, os resultados são automaticamente classificados por uma pontuação de pesquisa, calculada com base na frequência de termos e proximidade de um documento, com pontuações mais altas a ir em documentos com mais ou mais fósforos num termo de pesquisa. 
...
Dada a flex na encomenda de resultados, é melhor explorar outras opções se a consistência for um requisito de aplicação. A abordagem mais fácil é classificar por um valor de campo, como classificação ou data. Para cenários em que se queira classificar por um campo específico, como uma classificação ou data, pode definir explicitamente uma [ `$orderby` expressão](query-odata-filter-orderby-syntax.md), que pode ser aplicada a qualquer campo que seja indexado como **Sortível**.

Outra opção é utilizar um [perfil de pontuação personalizado.](index-add-scoring-profiles.md) Os perfis de pontuação dão-lhe mais controlo sobre o ranking de itens nos resultados de pesquisa, com a capacidade de aumentar os jogos encontrados em campos específicos. A lógica de pontuação adicional pode ajudar a ultrapassar pequenas diferenças entre réplicas porque as pontuações de pesquisa de cada documento estão mais distantes. Recomendamos o [algoritmo de classificação](index-ranking-similarity.md) para esta abordagem.
