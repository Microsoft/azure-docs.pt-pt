...
Azure Search, vai utilizar o `$count`, `$top`, e `$skip` parâmetros para esses valores de retorno. A exemplo a seguir mostra um pedido de exemplo para total chega num índice chamado "-catálogo online", é devolvido como `@odata.count`:
...
No Azure Search, usaria `$select` e uma [pedido de API de pesquisa](https://docs.microsoft.com/rest/api/searchservice/search-documents) para implementar esta experiência.
...
Se seu design de solução incluído páginas de pesquisa dedicadas para tipos específicos de conteúdo (por exemplo, um aplicativo de varejo online tem departamentos listados na parte superior da página), pode inserir um [expressão de filtro](search-filters.md) juntamente com um **onClick** evento para abrir uma página num estado previamente filtrado.
