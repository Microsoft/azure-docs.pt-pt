...
ms.locfileid: "90530629"
---
# <a name="what-is-azure-cognitive-search"></a>O que é a Pesquisa Cognitiva do Azure?

Azure Cognitive Search ([anteriormente conhecido como "Azure Search"](whats-new.md#new-service-name)é um serviço de pesquisa em nuvem que fornece apis e ferramentas para desenvolvedores APIs e ferramentas para adicionar uma rica experiência de pesquisa sobre conteúdo privado, heterogéneo em aplicações web, móveis e empresariais.

A Cognitive Search fornece um motor de indexação e consulta e armazenamento persistente de índices de pesquisa. Enquadra-se entre as lojas de dados externas que contribuem com conteúdo, e uma app do cliente que envia pedidos de consulta e trata respostas. As duas cargas de trabalho primárias de um serviço de pesquisa incluem *indexação* e *consulta.*
...
A indexação é quando o seu código ou ferramenta define um esquema de índice e carrega dados no seu serviço de pesquisa. Opcionalmente, pode adicionar habilidades cognitivas durante a indexação para aplicar processos de IA. Ao fazê-lo, podem criar novas informações e estruturas úteis para cenários de pesquisa e mineração de conhecimento.

Uma vez que um índice existe, a sua aplicação de cliente envia pedidos para um serviço de pesquisa e lida com respostas. A experiência de pesquisa é definida no seu cliente usando APIs da Azure Cognitive Search, com execução de consulta sobre um índice de pesquisa que você cria, possui e armazena no seu serviço.

A funcionalidade é exposta através de uma simples [REST API](/rest/api/searchservice/) ou [.NET SDK](search-howto-dotnet-sdk.md) que dissimula a complexidade inerente da obtenção de informações. Também pode utilizar o portal Azure para a administração de serviços e gestão de conteúdos, com ferramentas para prototipagem e consulta dos seus índices. Por o serviço executar na cloud, as infraestruturas e disponibilidade são geridas pela Microsoft.
...
+ O conteúdo bruto é um grande ficheiro de texto ou imagem indiferenciado ou ficheiros de aplicações armazenados no armazenamento de Azure Blob ou na Cosmos DB. Pode aplicar [competências cognitivas](cognitive-search-concept-intro.md) durante a indexação para extrair texto, criar estrutura ou criar novas informações, como texto traduzido ou entidades.
...
Depois de povoar um índice, pode [emitir consultas de pesquisa](search-query-overview.md) no seu ponto final de serviço utilizando pedidos HTTP simples com [API REST](/rest/api/searchservice/Search-Documents) ou o [.NET SDK](/dotnet/api/microsoft.azure.search.idocumentsoperations).
