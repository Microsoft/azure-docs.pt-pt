...
Aumentar as réplicas e as partições são eventos faturáveis que aumentam seu custo, mas, a menos que você esteja indexando continuamente sob a carga máxima, você pode adicionar escala para a duração do processo de indexação e ajustar os níveis de recursos para baixo após a indexação Finaliza.

## <a name="use-indexers"></a>Usar indexadores

[](search-indexer-overview.md) Os indexadores são usados para rastrear fontes de dados externas para conteúdo pesquisável. Embora não seja especificamente destinado à indexação em larga escala, vários recursos do indexador são particularmente úteis para acomodar conjuntos de dados maiores:
...
+ O particionamento de dados em fontes de dados individuais menores permite o processamento paralelo. Você pode dividir um conjunto de dados grande em conjuntos de dados menores e, em seguida, criar várias definições de fonte de dados que podem ser indexadas em paralelo.

> [!NOTE]
> Os indexadores são específicos da fonte de dados, portanto usar uma abordagem de indexador só é viável para fontes de dados selecionadas no Azure: [Banco de dados SQL](search-howto-connecting-azure-sql-database-to-azure-search-using-indexers.md), [armazenamento](search-howto-indexing-azure-blob-storage.md)de BLOBs, [armazenamento de tabelas](search-howto-indexing-azure-tables.md) [Cosmos DB](search-howto-index-cosmosdb.md).
...
+ Mapeie cada conjunto de dados para sua própria [fonte de dados](https://docs.microsoft.com/rest/api/searchservice/create-data-source), emparelhado com seu [](https://docs.microsoft.com/rest/api/searchservice/create-indexer)próprio indexador.
+ Para pesquisa cognitiva, faça referência ao [](https://docs.microsoft.com/rest/api/searchservice/create-skillset) mesmo conpróprio conpróprio de cada definição de indexador.
