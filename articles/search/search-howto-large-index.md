...
+ O particionamento de dados em fontes de dados individuais menores permite o processamento paralelo. Você pode dividir os dados de origem em componentes menores, como em vários contêineres no armazenamento de BLOBs do Azure e, em seguida, criar vários [objetos de fonte de dados](https://docs.microsoft.com/rest/api/searchservice/create-data-source) correspondentes no Azure pesquisa cognitiva que podem ser indexados em paralelo.
...
3. Distribua dados em vários contêineres em um nível que os indexadores do Azure Pesquisa Cognitiva podem alcançar. Isso pode ser várias tabelas no banco de dados SQL do Azure, vários contêineres no armazenamento de BLOBs do Azure ou várias coleções. Defina um objeto de fonte de dados para cada tabela ou contêiner.
...
4. Criar e agendar vários indexadores para execução em paralelo:
   ...
   + Dentro de cada definição de indexador, agende o mesmo padrão de execução de tempo de execução. Por exemplo, `"schedule" : { "interval" : "PT8H", "startTime" : "2018-05-15T00:00:00Z" }` cria uma agenda em 2018-05-15 em todos os indexadores, executando em intervalos de oito horas.

No horário agendado, todos os indexadores iniciam a execução, carregam dados, aplicam aprimoramentos (se você configurou um pipeline de pesquisa cognitiva) e gravando no índice. O Azure Pesquisa Cognitiva não bloqueia o índice para atualizações. As gravações simultâneas são gerenciadas, com repetição se uma gravação específica não tiver sucesso na primeira tentativa.
