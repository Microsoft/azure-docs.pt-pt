...
description: Modificar criadores de texto e filtros de caracteres usados em Azure Search consultas de pesquisa de texto completo.
ms.date: 08/08/2019
services: search
ms.service: search
ms.topic: conceptual
...
## <a name="create-custom-analyzers"></a>Criar analisadores personalizados
 ...
 O `tokenizer_name` é o nome de um `token_filter_name_1` criador e `token_filter_name_2` `char_filter_name_1` são `char_filter_name_2` os nomes dos filtros de token, e são os nomes dos filtros de caracteres (consulte o [criadores](#Tokenizers), [filtros de token](#TokenFilters) e filtros de caractere tabelas para valores válidos).
...
Depois que um analisador, um criador, um filtro de token ou um filtro de caractere é definido, ele não pode ser modificado. Novos poderão ser adicionados a um índice existente somente se o `allowIndexDowntime` sinalizador for definido como true na solicitação de atualização de índice:
