...
> * Defina um pipeline que utilize OCR, deteção de idiomas e entidade e reconhecimento de frases-chave.
...
Este tutorial usa C# e aAzure.Search.Docbiblioteca de clientes para criar uma fonte de [ **dados,**](/dotnet/api/overview/azure/search.documents-readme) índice, indexante e skillset.

O indexante conecta-se a um recipiente blob especificado no objeto de origem de dados, e envia todo o conteúdo indexado a um índice de pesquisa existente.

O skillset está ligado ao indexante. Utiliza habilidades incorporadas da Microsoft para encontrar e extrair informação. Os passos no oleoduto incluem reconhecimento de caracteres óticos (OCR) em imagens, deteção de linguagem em texto, extração de frases-chave e reconhecimento de entidades (organizações). As novas informações criadas pelo oleoduto são armazenadas em novos campos num índice. Uma vez preenchido o índice, pode utilizar os campos em consultas, facetas e filtros.
...
1. Nas   >  **Teclas de** Definições, copie uma chave de administração para obter plenos direitos sobre o serviço. Existem duas chaves de administração intercambiáveis, previstas para a continuidade do negócio, caso precise de rolar uma. Pode utilizar a tecla primária ou secundária nos pedidos de adição, modificação e eliminação de objetos.
