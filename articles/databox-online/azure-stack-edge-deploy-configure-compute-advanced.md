---
title: Tutorial para filtrar, analisar dados para implementação avançada com computação em Azure Stack Edge [ Microsoft Docs
description: Aprenda a configurar o papel de computação no Azure Stack Edge e use-o para transformar dados para um fluxo avançado de implementação antes de enviar para o Azure.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "82570770"
---
# <a name="tutorial-transform-data-with-azure-stack-edge-for-advanced-deployment-flow"></a>Tutorial: Transforme dados com Azure Stack Edge para fluxo avançado de implementação

Este tutorial descreve como configurar uma função de cálculo para um fluxo avançado de implementação no seu dispositivo Azure Stack Edge. Depois de configurar a função de cálculo, o Azure Stack Edge pode transformar dados antes de enviá-los para o Azure.
...
| Tipo             | Utilize o serviço Azure Stack Edge para implementar módulos      | Utilize o serviço IoT Hub para implementar módulos |
...
1. No portal Azure do seu recurso Azure Stack Edge, vá ao **Overview**. No painel direito, no azulejo **Compute,** selecione **Iniciar**.
    ...
    ![Começar com a computação](./media/azure-stack-edge-deploy-configure-compute-advanced/configure-compute-1.png)

2. No azulejo do **cálculo Configure Edge,** selecione **Configure compute**.

    ![Começar com a computação](./media/azure-stack-edge-deploy-configure-compute-advanced/configure-compute-2.png)
...
3. Na lâmina de **cálculo Configure Edge,** insere o seguinte:
    ...
    ![Começar com a computação](./media/azure-stack-edge-deploy-configure-compute-advanced/configure-compute-3.png)
...
4. Selecione **Criar**. A criação de recursos do IoT Hub leva alguns minutos. Após a criação do recurso IoT Hub, as atualizações de azulejos de **computação Configure Edge** para mostrar a configuração da computação. Para confirmar que a função de computação Edge foi configurada, selecione **Ver config** no azulejo **computacional Configure.**
    ...
    ![Começar com a computação](./media/azure-stack-edge-deploy-configure-compute-advanced/configure-compute-4.png)
...
Não existem módulos personalizados neste dispositivo Edge. Pode adicionar um módulo personalizado ou pré-construído. Para aprender a criar um módulo personalizado, vá desenvolver [um módulo C# para o seu dispositivo Azure Stack Edge](azure-stack-edge-create-iot-edge-module.md).
...
1. Vá ao **cálculo edge > Começar.** No azulejo **adicionar módulos,** selecione o tipo de cenário como **avançado**. Selecione **Ir para IoT Hub**.
    ...
    ![Selecione implementação avançada](./media/azure-stack-edge-deploy-configure-compute-advanced/add-module-1.png)
...
2. No seu recurso IoT Hub, vá ao **dispositivo IoT Edge** e, em seguida, selecione o seu dispositivo IoT Edge.
    ...
    ![Vá ao dispositivo IoT Edge no IoT Hub](./media/azure-stack-edge-deploy-configure-compute-advanced/add-module-2.png)

3. Em **detalhes do Dispositivo,** selecione **Módulos de Conjunto**.

    ![A ligação de Módulos definidos](./media/azure-stack-edge-deploy-configure-compute-advanced/add-module-3.png)
...
4. Em **Módulos Add,** faça o seguinte:
    ...
    2. Especifique as definições do módulo personalizado do IoT Edge. Insera os seguintes valores.
        ...
        |Campo  |Valor  |
        |---------|---------|
        |Nome     | Um nome único para o módulo. Este módulo é um recipiente de estivador que pode implantar para o dispositivo IoT Edge associado ao seu Edge Azure Stack Edge.        |
        ...
        ![O recipiente criar opções](./media/azure-stack-edge-deploy-configure-compute-advanced/add-module-5.png) 
...
5.    Em **Rotas de Especificação,** desloque as rotas entre módulos.  
    ...
    ![As Rotas de Especificação](./media/azure-stack-edge-deploy-configure-compute-advanced/add-module-7.png)

    Pode substituir a *rota* pela seguinte corda de rota que copiou anteriormente. Neste exemplo, introduza o nome da parte local que irá empurrar os dados para a partilha da nuvem. Substitua `modulename` o com o nome do módulo. Selecione **Seguinte**.
        
    ```
    "route": "FROM /* WHERE topic = 'mydbesmbedgelocalshare1' INTO BrokeredEndpoint(\"/modules/filemove/inputs/input1\")"
    ```

    ![A secção Rotas Especificar](./media/azure-stack-edge-deploy-configure-compute-advanced/add-module-8.png)
