...
1. No portal Azure, vá ao seu recurso Azure Stack Edge e, em seguida, vá para **o gateway de armazenamento cloud > Shares**. **Selecione + Adicione a partilha** na barra de comando.
    ...
    ![Selecione adicionar a partilha](media/azure-stack-edge-gpu-manage-shares/add-share-1.png)
...
6. Este passo depende de estar a criar uma partilha SMB ou NFS.
    - **Se criar uma partilha SMB** - no campo **Utilizador local com todos os privilégios**, escolha **Criar novo** ou **Utilizar existente**. Se criar um novo utilizador local, forneça o **nome de utilizador**, a **palavra-passe** e, em seguida, confirme a palavra-passe. Esta ação atribui as permissões ao utilizador local. Depois de ter atribuído as permissões aqui, pode utilizar o Explorador de Ficheiros para modificar estas permissões.
        ...
        ![Adicionar uma partilha SMB](media/azure-stack-edge-gpu-manage-shares/add-smb-share.png)
    ...
    - **Se criar uma partilha NFS** - tem de fornecer os **Endereços IP dos clientes permitidos** que podem aceder à partilha.
        ...
        ![Adicionar uma partilha NFS](media/azure-stack-edge-gpu-manage-shares/add-nfs-share.png)
...
1. No portal Azure, vá ao seu recurso Azure Stack Edge e, em seguida, vá para **o gateway de armazenamento cloud > Shares**. **Selecione + Adicione a partilha** na barra de comando.
    ...
    ![Selecione adicionar a partilha 2](media/azure-stack-edge-gpu-manage-shares/add-local-share-1.png)
...
7. Selecione **Criar**. 
    ...
    ![Criar ações locais](media/azure-stack-edge-gpu-manage-shares/add-local-share-2.png)
    ...
    ![Ver atualizações Partilhar lâmina](media/azure-stack-edge-gpu-manage-shares/add-local-share-3.png)
    
    Selecione a partilha para ver o ponto de montagem local para os módulos de computação Edge para esta partilha.

    ![Ver detalhes de partilha local](media/azure-stack-edge-gpu-manage-shares/add-local-share-4.png)
...
1. No portal Azure, vá ao seu recurso Azure Stack Edge e, em seguida, vá para **o gateway de armazenamento cloud > Shares**. Na lista das ações, selecione a parte que pretende montar. A coluna **usada para cálculo** mostrará o estado de **desativado** para a parte selecionada.
    ...
    ![Selecionar partilha](media/azure-stack-edge-gpu-manage-shares/mount-share-1.png)

2. Selecione **Mount**.

    ![Selecione montagem](media/azure-stack-edge-gpu-manage-shares/mount-share-2.png)

3. Quando solicitado para confirmação, selecione **Sim**. Isto vai montar a parte.

    ![Confirmar montagem](media/azure-stack-edge-gpu-manage-shares/mount-share-3.png)
...
4. Depois de a parte ser montada, vá para a lista de ações. Verá que a coluna **Usada para cálculo** mostra o estado da partilha como **Ativado**.
    ...
    ![Partilhar montado](media/azure-stack-edge-gpu-manage-shares/mount-share-4.png)
...
5. Selecione a partilha novamente para ver o ponto de montagem local para a partilha. O módulo de computação Edge utiliza este ponto de montagem local para a partilha.
    ...
    ![Ponto de montagem local para a parte](media/azure-stack-edge-gpu-manage-shares/mount-share-5.png)
...
1. No portal Azure, vá ao seu recurso Azure Stack Edge e, em seguida, vá para **o gateway de armazenamento cloud > Shares**. Na lista das ações, selecione a parte que pretende desmontar. Certifique-se de que a parte que desmonta não é utilizada por nenhum módulo. Se a partilha for utilizada por um módulo, verá problemas com o módulo correspondente.
    ...
    ![Selecione share 2](media/azure-stack-edge-gpu-manage-shares/unmount-share-1.png)

2.  Selecione **Desmonte**.

    ![Selecione desmonte](media/azure-stack-edge-gpu-manage-shares/unmount-share-2.png)

3. Quando solicitado para confirmação, selecione **Sim**. Isto vai desmontar a parte.

    ![Confirmar desmontagem](media/azure-stack-edge-gpu-manage-shares/unmount-share-3.png)
...
4. Depois de a parte não ser desmontada, vá para a lista de ações. Verá que a coluna **usada para computação** mostra o estado da partilha como **Desativada**.
    ...
    ![Partilha desmontada](media/azure-stack-edge-gpu-manage-shares/unmount-share-4.png)
...
1. Na lista de partilhas, selecione e clique na partilha que pretende eliminar.
    ...
    ![Selecione partilha 3](media/azure-stack-edge-gpu-manage-shares/delete-share-1.png)

2. Clique em **Eliminar**.

    ![Clicar em eliminar](media/azure-stack-edge-gpu-manage-shares/delete-share-2.png)

3. Quando lhe for pedida a confirmação, clique em **Sim**.

    ![Confirmar eliminação](media/azure-stack-edge-gpu-manage-shares/delete-share-3.png)
...
1.  No portal do Azure, aceda a **Partilhas**. Selecione e clique na partilha que pretende atualizar.
    ...
    ![Selecione partilha 4](media/azure-stack-edge-gpu-manage-shares/refresh-share-1.png)

2.  Clique em **Atualizar**. 

    ![Clicar em atualizar](media/azure-stack-edge-gpu-manage-shares/refresh-share-2.png)
...
3.  Quando lhe for pedida a confirmação, clique em **Sim**. É iniciada uma tarefa para atualizar os conteúdos da partilha no local.
    ...
    ![Confirmar atualização](media/azure-stack-edge-gpu-manage-shares/refresh-share-3.png)
...
![Carimbo de data/hora atualizado](media/azure-stack-edge-gpu-manage-shares/refresh-share-4.png)
...
2. Vá a **Recipientes** e selecione **+ Recipiente** para criar um recipiente. Nomeie este recipiente como *novo detido.* Deite o **nível de acesso público** ao Contentor.
    ...
    ![Sincronização automatizada para ficheiros presos 1](media/azure-stack-edge-gpu-manage-shares/image-1.png)
...
3. Selecione o nome do recipiente e deite os seguintes metadados:  
    ...
    ![Sincronização automatizada para ficheiros presos 2](media/azure-stack-edge-gpu-manage-shares/image-2.png)
...
4. Crie uma nova partilha no seu dispositivo. Mapeá-lo para o recipiente fixado, escolhendo a opção de recipiente existente. Marque a parte como lido apenas. Crie um novo utilizador e especifique o nome de utilizador e uma senha correspondente para esta partilha.  
    ...
    ![Sincronização automatizada para ficheiros presos 3](media/azure-stack-edge-gpu-manage-shares/image-3.png)
...
6. Selecione **Os dados de Atualização** no portal Azure para que o dispositivo descarregue a política de fixação para esse recipiente de armazenamento Azure em particular.  
    ...
    ![Sincronização automatizada para ficheiros presos 4](media/azure-stack-edge-gpu-manage-shares/image-4.png)
...
1. Aceda a **Descrição Geral** no seu recurso. Na lista de partilhas, escolha e clique numa partilha associada à conta de armazenamento que precisa de sincronizar.
    ...
    ![Selecione partilhar com conta de armazenamento relevante](media/azure-stack-edge-gpu-manage-shares/sync-storage-key-1.png)

2. Clique em **Sincronizar chave de armazenamento**. Clique em **Sim** quando lhe for pedida a confirmação.

     ![Selecione a chave de armazenamento sync](media/azure-stack-edge-gpu-manage-shares/sync-storage-key-2.png)
