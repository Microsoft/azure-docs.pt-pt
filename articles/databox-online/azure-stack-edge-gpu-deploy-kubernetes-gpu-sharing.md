---
title: Implementar a carga de trabalho de Kubernetes utilizando a partilha de GPU no dispositivo GPU Azure Stack Edge Pro
description: Descreve como pode implementar uma carga de trabalho partilhada pela GPU através de Kubernetes no seu dispositivo GPU Azure Stack Edge Pro.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "103565395"
---
# <a name="deploy-a-kubernetes-workload-using-gpu-sharing-on-your-azure-stack-edge-pro"></a>Implemente uma carga de trabalho kubernetes usando a partilha de GPU no seu Azure Stack Edge Pro

Este artigo descreve como as cargas de trabalho contentorizadas podem partilhar as GPUs no seu dispositivo GPU Azure Stack Edge Pro. Neste artigo, você terá dois empregos, um sem a partilha de contexto da GPU e outro com a partilha de contexto ativada através do Serviço Multi-Processo (MPS) no dispositivo. Para mais informações, consulte o [Serviço Multi-Processo.](https://docs.nvidia.com/deploy/pdf/CUDA_Multi_Process_Service_Overview.pdf)
...
1. Criou um espaço de nome e um utilizador. Também concedeu ao utilizador o acesso a este espaço de nome. Tem o ficheiro kubeconfig deste espaço de nome instalado no sistema de clientes que utilizará para aceder ao seu dispositivo. Para obter instruções detalhadas, consulte [Connect to e gerencie um cluster Kubernetes através de kubectl no seu dispositivo GPU Azure Stack Edge Pro](azure-stack-edge-gpu-create-kubernetes-cluster.md#configure-cluster-access-via-kubernetes-rbac). 

1. Guarde a seguinte implantação `yaml` no seu sistema local. Usará este ficheiro para executar a implantação de Kubernetes. Esta implantação baseia-se em [recipientes Simples CUDA](https://docs.nvidia.com/cuda/wsl-user-guide/index.html#running-simple-containers) que estão disponíveis publicamente a partir da Nvidia. 
...
## <a name="job-without-context-sharing"></a>Trabalho sem partilha de contexto

Você executará o primeiro trabalho para implementar uma aplicação no seu dispositivo no espaço de `mynamesp1` nomes . Esta implementação de aplicações também mostrará que a partilha de contexto da GPU não é ativada por padrão. 

1. Lista todas as cápsulas em execução no espaço de nomes. Execute o seguinte comando: 
    ...
    No resources found.
    ```
1. Inicie um trabalho de implantação no seu dispositivo utilizando o dispositivo de implementação.yaml fornecido anteriormente. Execute o seguinte comando: 

    ```powershell
    kubectl apply -f <Path to the deployment .yaml> -n <Name of the namespace> 
    ```  

    Este trabalho cria dois contentores e executa uma simulação n-body em ambos os contentores. O número de iterações de simulação é especificado no `.yaml` . Para obter mais informações, consulte [a simulação do corpo N.](https://physics.princeton.edu//~fpretori/Nbody/intro.htm)
...
1. Para listar as cápsulas iniciadas na implantação, executar o seguinte comando:
    ...
    Há duas cápsulas `cuda-sample1-cf979886d-xcwsq` e a funcionar no teu `cuda-sample2-68b4899948-vcv68` dispositivo.

1. Pegue os detalhes das cápsulas. Execute o seguinte comando:
    ...
    PS C:\WINDOWS\system32>
    ```
    A saída indica que ambas as cápsulas foram criadas com sucesso pelo trabalho. 
...
1. Enquanto ambos os contentores estão executando a simulação n-body, veja a utilização da GPU a partir da saída Nvidia smi. Vá à interface PowerShell do dispositivo e corra `Get-HcsGpuNvidiaSmi` .
    ...
    [10.100.10.10]: PS>    
    ```
    Como pode ver, existem dois recipientes (Tipo = C) em execução com simulação n-body em GPU 0. 

1. Monitorize a simulação n-corpo. Executar os `get pod` comandos. Aqui está uma saída de exemplo quando a simulação está em execução. 
    ...
    Quando a simulação estiver completa, a saída indicará isso. Aqui está uma saída de exemplo:
...
1. Após a simulação estar concluída, pode visualizar os registos e o tempo total para a conclusão da simulação. Execute o seguinte comando:
    ...
    PS C:\WINDOWS\system32>    
    ```
1. Não deve haver processos em curso na GPU neste momento. Pode verificar isto visualizando a utilização da GPU utilizando a saída Nvidia smi.
...
## <a name="job-with-context-sharing"></a>Trabalho com partilha de contexto

Você executará o segundo trabalho para implantar a simulação n-body em dois contentores CUDA quando a partilha de contexto da GPU estiver ativada embora o MPS. Primeiro, ativará o MPS no dispositivo.
...
1. Para ativar o MPS no seu dispositivo, executar o `Start-HcsGpuMPS` comando.
    ...
    [10.100.10.10]: PS>    
    ```
1. Executar o trabalho usando a mesma colocação `yaml` que usou anteriormente. Pode ser necessário eliminar a implantação existente. Ver [Eliminar a implementação](#delete-deployment).
...
1. Enquanto a simulação está em andamento, você pode ver a saída Nvidia smi. A saída mostra processos correspondentes aos recipientes cuda (tipo M + C) com simulação n-body e o serviço MPS (tipo C) como funcionamento. Todos estes processos partilham GPU 0.
...
1. Após a simulação estar concluída, pode visualizar os registos e o tempo total para a conclusão da simulação. Execute o seguinte comando:
    ...
    ```powershell
        ...
        PS C:\WINDOWS\system32>
    ```
1. Depois da simulação estar completa, pode ver novamente a saída Smi da Nvidia. Apenas o processo nvidia-cuda-mps-servidor para o serviço MPS mostra como funcionamento. 
...
Pode ser necessário eliminar as implementações quando estiver a funcionar com MPS ativado e com o MPS desativado no seu dispositivo.

Para eliminar a implantação do seu dispositivo, executar o seguinte comando: 
...
- [Implemente uma carga de trabalho IoT Edge com a partilha de GPU no seu Azure Stack Edge Pro](azure-stack-edge-gpu-deploy-iot-edge-gpu-sharing.md).
