...
> [!WARNING]
> Este quickstart usa os **antigos hubs de eventos e** pacotes **azure-eventhubs-eph.** Para um arranque rápido que usa o mais recente pacote de **eventos de mensagens azure,** consulte [Enviar e receber eventos usando centros de eventos de mensagens azure](get-started-java-send-v2.md). Para mover a sua aplicação de usar o pacote antigo para um novo, consulte o [Guia para migrar de hubs de eventos azure para centros de eventos de mensagens azure](https://github.com/Azure/azure-sdk-for-java/blob/master/sdk/eventhubs/azure-messaging-eventhubs/migration-guide.md). 
...
### <a name="construct-connection-string"></a>Construir cadeia de ligação

Utilize a classe ConnectionStringBuilder para construir um valor de cadeia de ligação para passar para a instância cliente do Event Hubs. Substitua os espaços reservados com os valores obtidos quando criou o espaço de nome e o centro de eventos:
...
Crie um evento singular transformando uma cadeia na sua codificação de byte UTF-8. Em seguida, crie uma nova instância de cliente do Event Hubs a partir da cadeia de ligação e envie a mensagem:   
...
Quando a chave de partição não estiver definida, então as mensagens serão rodadas para todas as divisórias disponíveis
...
**EventProcessorHost** é uma classe Java que simplifica receber eventos de Event Hubs gerindo pontos de verificação persistentes e recebeções paralelas desses Centros de Eventos. Utilizando o EventProcessorHost, pode dividir eventos em vários recetores, mesmo quando hospedado em diferentes nós. Este exemplo mostra como utilizar o EventProcessorHost para um recetor único.
...
3. Selecione a conta de armazenamento recém-criada e, em seguida, selecione **Access Keys:**
    ...
    Copie o valor chave1 para uma localização temporária. Vai utilizá-lo mais tarde no tutorial.
...
2. Utilize o seguinte código para criar uma nova classe designada `EventProcessorSample`. Substitua os espaços reservados pelos valores utilizados quando criou o centro de eventos e a conta de armazenamento:
   ...
   ```
3. Crie mais `EventProcessor`uma classe chamada, utilizando o seguinte código:
...
Quando a chave de partição não está definida, então as mensagens são rodadas para todas as divisórias disponíveis
...
### <a name="implementing-a-custom-checkpointmanager-for-eventprocessorhost-eph"></a>Implementação de um CheckpointManager personalizado para O Anfitrião do Processador de Eventos (EPH)
...
O gestor de checkpoint predefinido utiliza o armazenamento de blob, mas se substituir o gestor de checkpoint utilizado pela EPH com a sua própria implementação, pode utilizar qualquer loja que pretenda apoiar a implementação do seu gestor de checkpoint.
...
Dentro da sua implementação, pode substituir o mecanismo de controlo predefinido e implementar os nossos próprios pontos de verificação com base na sua própria loja de dados (como SQL Server, CosmosDB e Azure Cache for Redis). Recomendamos que a loja usada para apoiar a implementação do seu gestor de checkpoint seja acessível a todos os casos eph que estão a processar eventos para o grupo de consumidores.

Pode utilizar qualquer datastore disponível no seu ambiente.

A classe com.microsoft.azure.eventprocessorhost.EventProcessorHost fornece-lhe dois construtores que lhe permitem anular o gestor de checkpoint para o seu Anfitrião de Processador de Eventos.
