...
O Service Fabric Mesh suporta qualquer linguagem de programação ou estrutura que possa ser executada num contentor. O Visual Studio 2019 e o Visual Studio Code, suporte de ferramentas fornece uma edição e depuração poderosas experiência para aplicativos .NET e .NET Core. 
...
- Criar e implementar novas aplicações de microsserviços à escala no Azure.  Integrar noutros serviços do Azure ou aplicações existentes em execução em contentores. Cada microsserviço é parte de uma aplicação segura e de rede isolada. Os microsserviços tem políticas de governação de recursos definidas para a CPU núcleos, memória, espaço em disco e muito mais.
...
Gerir facilmente os aplicativos em execução, a monitorização de aplicações e a depuração em ambientes de produção. Esta gestão inclui as atualizações de aplicações e controle de versão. Estas aplicações podem ser constituídas por um único ou múltiplos microsserviços isolados na sua própria rede. As aplicações são executadas com eficiência com rápidos tempos de implementação, posicionamento e ativação pós-falha.
...
- Tirar partido de todas as funcionalidades de recursos do Azure Resource Manager. Exemplos desses recursos incluem o registo de auditoria e [controlo de acesso baseado em funções](/azure/role-based-access-control/overview)). Todos os recursos que implementa para o serviço de malha do Service Fabric no Azure são recursos do Azure Resource Manager. Esses recursos incluem aplicações, serviços, segredos e assim por diante.
...
O Service Fabric Mesh cria uma coleção de clusters que abrange [Zonas de Disponibilidade do Azure](/azure/availability-zones/az-overview) e/ou fronteiras regionais geo-políticas. Malha de recursos de infraestrutura do serviço descreve a aplicativos com um conjunto de objetivos, como o dimensionamento, os requisitos de hardware, requisitos de durabilidade e políticas de segurança.  Quando a aplicação é implementada, o Service Fabric Mesh encontra o local ideal para executá-la.
