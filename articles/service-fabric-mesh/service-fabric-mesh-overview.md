...
O Service Fabric Mesh suporta qualquer linguagem de programação ou estrutura que possa ser executada num contentor. O visual Studio 2019 e o suporte de ferramentas Visual Studio Code proporcionam uma poderosa experiência de edição e depuração para aplicações .NET e .NET Core. 
...
Gerir facilmente aplicações de funcionamento, monitorizar aplicações e depuração em ambientes de produção. Esta gestão inclui atualizações de aplicações e versão. Estas aplicações podem ser constituídas por um único ou múltiplos microsserviços isolados na sua própria rede. As aplicações são executadas com eficiência com rápidos tempos de implementação, posicionamento e ativação pós-falha.
...
- Aproveite todas as funcionalidades dos recursos do Gestor de Recursos Azure. Exemplos destas funcionalidades incluem pista de auditoria e [controlo de acesso baseado em papéis).](/azure/role-based-access-control/overview) Todos os recursos que você implementa para o serviço de malha de tecido de serviço em Azure são recursos do Gestor de Recursos Azure. Estes recursos incluem aplicações, serviços, segredos, e assim por diante.
...
O Service Fabric Mesh cria uma coleção de clusters que abrange [Zonas de Disponibilidade do Azure](/azure/availability-zones/az-overview) e/ou fronteiras regionais geo-políticas. Service Fabric Mesh descreve aplicações com um conjunto de intenções tais como escala, requisitos de hardware, requisitos de durabilidade e políticas de segurança.  Quando a aplicação é implementada, o Service Fabric Mesh encontra o local ideal para executá-la.
