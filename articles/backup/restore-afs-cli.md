...
Para instalar e utilizar a CLI localmente, tem de executar a CLI do Azure versão 2.0.18 ou posterior. Para encontrar a versão da CLI, execute `az --version`. Se precisar de instalar ou atualizar, veja [Instalar a CLI do Azure](/cli/azure/install-azure-cli?view=azure-cli-latest).
...
Utilize o cmdlet da [lista de recuperação de backup az](/cli/azure/backup/recoverypoint?view=azure-cli-latest#az-backup-recoverypoint-list) para listar todos os pontos de recuperação para a partilha de ficheiros apoiado.
...
* **--nome do contentor**: o nome da conta de armazenamento que acolhe a parte original do ficheiro. Para recuperar o nome ou o nome amigável do seu recipiente, utilize o comando [da lista de contentores de reserva az.](/cli/azure/backup/container?view=azure-cli-latest#az-backup-container-list)
* **--nome do item**: O nome da partilha de ficheiros original com ressarçamento que pretende utilizar para a operação de restauro. Para recuperar o nome ou o nome amigável do seu item de reserva, utilize o comando [da lista de produtos de reserva az.](/cli/azure/backup/item?view=azure-cli-latest#az-backup-item-list)
...
O atributo **Nome** na saída corresponde ao nome do trabalho que é criado pelo serviço de backup para a sua operação de restauro. Para acompanhar o estado do trabalho, use o [az backup job show](/cli/azure/backup/job?view=azure-cli-latest#az-backup-job-show) cmdlet.
...
* **SourceFilePath**: O caminho absoluto do ficheiro, a restaurar dentro da partilha de ficheiros, como uma cadeia. Este caminho é o mesmo caminho usado no download de [ficheiros de armazenamento az](/cli/azure/storage/file?view=azure-cli-latest#az-storage-file-download) ou [az armazenamento de arquivos](/cli/azure/storage/file?view=azure-cli-latest#az-storage-file-show) CLI comandos.
...
Utilize o cmdlet restaurar o [restauro-azurefiles az](/cli/azure/backup/restore?view=azure-cli-latest#az-backup-restore-restore-azurefiles) com modo de restauro definido para *a instalação original* para restaurar ficheiros ou pastas específicos para a sua localização original.
...
Para restaurar ficheiros ou pastas específicos para uma localização alternativa, utilize o cmdlet [restaurar azurefiles de restauro az](/cli/azure/backup/restore?view=azure-cli-latest#az-backup-restore-restore-azurefiles) com modo de restauro definido para *alternar* e especificar os seguintes parâmetros relacionados com o alvo:
