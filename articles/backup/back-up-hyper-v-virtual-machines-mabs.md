...
5. O servidor MABS utiliza VSS nos volumes que hospedam dados de recuperação para que estejam disponíveis várias cópias-sombra. Cada uma destas cópias sombra fornece uma recuperação separada. Os pontos de recuperação vss são armazenados no servidor MABS. A cópia temporária que é feita no servidor em funcionamento do Hyper-V, é armazenada apenas durante a sincronização MABS.
