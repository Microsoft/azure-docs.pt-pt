...
- O Azure Backup utiliza [instantâneos incrementais](../virtual-machines/disks-incremental-snapshots.md#restrictions) do disco gerido. Os instantâneos incrementais são uma cópia de segurança pontual e pontual dos discos geridos que são faturados para as alterações delta no disco desde o último instantâneo. São sempre armazenados no armazenamento mais rentável, armazenamento HDD padrão, independentemente do tipo de armazenamento dos discos-mãe. O primeiro instantâneo do disco ocupará o tamanho usado do disco, e os instantâneos incrementais consecutivos armazenam alterações delta no disco desde o último instantâneo.
