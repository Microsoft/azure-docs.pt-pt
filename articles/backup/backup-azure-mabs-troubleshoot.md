...
| Operação | Detalhes do erro | Solução |
| --- | --- | --- |
| Restaurar | **Código de erro**: Erro de credenciais de CBPServerRegisteredVaultDontMatchWithCurrent/cofre: 100110 <br/> <br/>**Mensagem de erro**: As credenciais do cofre fornecidas são diferentes do cofre no qual o servidor está registrado | **Motivo**: Esse problema ocorre quando você está tentando restaurar arquivos para o servidor alternativo a partir do servidor original usando a opção de recuperação externa do DPM e se o servidor que está sendo recuperado e o servidor original do qual o backup dos dados é feito não estão associados ao mesmo Cofre do serviço de recuperação.<br/> <br/>**Solução alternativa** Para resolver esse problema, certifique-se de que o servidor original e o alternativo estejam registrados no mesmo cofre.|
