---
title: Fazer cópias de segurança de VMs do Azure num cofre dos serviços de recuperação utilizando cópia de segurança do Azure
description: Descreve como fazer cópias de segurança de VMs do Azure num cofre dos serviços de recuperação utilizando cópia de segurança do Azure
services: backup
...
Este artigo descreve como fazer cópias de segurança de VMs do Azure num cofre dos serviços de recuperação utilizando a [Azure Backup](backup-overview.md) serviço. 

Neste artigo, vai aprender a:

> [!div class="checklist"]
> * Certifique-se de que o suporte e pré-requisitos para uma cópia de segurança.
...
Em algumas circunstâncias, poderá ter de configurar o agente de VM do Azure em VMs do Azure ou de permitir explicitamente o acesso de saída numa VM.
...
Em geral, não precisa de permitir explicitamente o acesso de rede de saída para uma VM do Azure, para que consiga comunicar com o Azure Backup.
Se não conseguir ligar as suas VMs, e se vir o erro **ExtensionSnapshotFailedNoNetwork**, deve permitir explicitamente o acesso. A extensão de cópia de segurança, em seguida, pode comunicar com endereços IP públicos do Azure para o tráfego de cópia de segurança.


#### <a name="explicitly-allow-outbound-access"></a>Permitir explicitamente o acesso de saída

Se uma VM não é possível ligar ao serviço de cópia de segurança, permita explicitamente o acesso de saída através de um dos métodos resumidos na tabela.

**Opção** | **Ação** | **Detalhes** 
--- | --- | --- 
**Configurar regras NSG** | Permitir a [intervalos IP do datacenter do Azure](https://www.microsoft.com/download/details.aspx?id=41653). | Em vez de permitir que e gerir a cada intervalo de endereços, pode adicionar uma regra de grupo (NSG) de segurança de rede que permite o acesso ao serviço de cópia de segurança do Azure ao utilizar um [etiqueta de serviço](backup-azure-arm-vms-prepare.md#set-up-an-nsg-rule-to-allow-outbound-access-to-azure). [Saiba mais](../virtual-network/security-overview.md#service-tags).<br/><br/> Não existem sem custos adicionais.<br/><br/> As regras são simples de gerenciar com etiquetas de serviço.
**Implementar um proxy** | Implemente um servidor de proxy HTTP para o encaminhamento de tráfego. | Este método fornece acesso a todo do Azure e não apenas armazenamento.<br/><br/> É permitido um controle granular sobre os URLs de armazenamento.<br/><br/> Há um acesso de ponto único de internet para as VMs.<br/><br/> Existem custos adicionais para um proxy.
**Configurar a Firewall do Azure** | Permitir o tráfego através da Firewall do Azure na VM, através de uma etiqueta do FQDN para o serviço de cópia de segurança do Azure. |  Esse método é simples de usar, se tiver que configurar na sub-rede de uma rede virtual de Firewall do Azure.<br/><br/> Não é possível criar suas próprias etiquetas FQDN ou modificar os FQDNs numa marca.<br/><br/> Se utilizar Managed Disks do Azure, poderá ter de abertura de portas adicionais (porta 8443) nas firewalls.
...
### <a name="allow-firewall-access-by-using-an-fqdn-tag"></a>Permitir o acesso de firewall através de uma etiqueta do FQDN
...
Por predefinição, o seu Cofre tem [armazenamento georredundante (GRS)](https://docs.microsoft.com/azure/storage/common/storage-redundancy-grs). Recomendamos GRS para a cópia de segurança primária. Pode usar [armazenamento localmente redundante (LRS)](https://docs.microsoft.com/azure/storage/common/storage-redundancy-lrs?toc=%2fazure%2fstorage%2fblobs%2ftoc.json) para uma opção mais barata.

Modificar o tipo de replicação de armazenamento da seguinte forma:

1. No portal, selecione o novo cofre. Sob **configurações**, selecione **propriedades**.
2. Na **propriedades**, em **configuração de cópia de segurança**, selecione **atualização**.
...
1. No cofre > **descrição geral**, selecione **+ cópia de segurança**.
...
2. Na **objetivo de cópia de segurança** > **em que a sua carga de trabalho é executado?**, selecione **Azure**. Na **o que fazer quiser a cópia de segurança?**, selecione **Máquina Virtual** >  **OK**.
...
3. Na **política de cópia de segurança**, selecione a política que pretende associar ao cofre. Em seguida, selecione **OK**.
...
4. Na **selecionar máquinas virtuais** painel, selecione as VMs que irão utilizar a política de cópia de segurança especificada > **OK**.
   ...
   A VM selecionada é validada. Pode selecionar apenas as VMs na mesma região que o cofre. As VMs só podem ser copiadas num único cofre.
...
5. Na **cópia de segurança**, selecione **ativar cópia de segurança**.
   ...
   Este passo implementa a política para o Cofre e para as VMs. Também instala a extensão de cópia de segurança no agente VM em execução na VM do Azure.
...
Uma VM em execução fornece a maior possibilidade de obter um ponto de recuperação consistente com aplicações. No entanto, a VM é uma cópia de segurança, mesmo que ele é desativado e não é possível instalar a extensão. Ele é conhecido como uma VM offline. Neste caso, o ponto de recuperação será consistentes de falhas.
...
2. Na **itens de cópia de segurança**, selecione **Máquina Virtual do Azure**.
3. Na **itens de cópia de segurança** , selecione as reticências (**...** ).
4. Selecione **agora a cópia de segurança**.
...
6. Monitorize as notificações de portais. Pode monitorizar o progresso da tarefa no dashboard do cofre > **tarefas de cópia de segurança** > **em curso**. Dependendo do tamanho da sua VM, a criação de cópia de segurança inicial poderá demorar algum tempo.
