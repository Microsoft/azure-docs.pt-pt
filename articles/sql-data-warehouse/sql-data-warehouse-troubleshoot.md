...
| Gerar scripts falha no SSMS                               | Gerar um guião para o SQL Analytics falha se a opção "Gerar script para objetos dependentes" for definida para "True". Como suposição, os utilizadores devem ir manualmente às **Ferramentas -> Opções ->SQL Server Object Explorer -> Gerar script para opções dependentes e definir para falsos** |
