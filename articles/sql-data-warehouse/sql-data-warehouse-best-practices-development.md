...
Consulte também [noções básicas sobre transações][Understanding transactions], [Optimizing transactions][Optimizing transactions], particionamento de [tabelas]particionamento de[tabelas,] [TRUNCATE TABLE][TRUNCATE TABLE], [ALTER TABLE][ALTER TABLE], [criar tabela como SELECT (CTAS)][criar tabela como SELECT (CTAS)]
...
Consulte também [índices][Table indexes], [Columnstore indexes guide][Columnstore indexes guide]de tabela, recriando [índices columnstore][recriando índices columnstore]

## <a name="next-steps"></a>Passos Seguintes
Se não encontrar o que procura neste artigo, experimente utilizar a "Pesquisa de documentos" no lado esquerdo desta página para pesquisar todos os documentos do Azure SQL Data Warehouse.  is a place for you to ask questions to other users and to the SQL Data Warehouse Product Group.  We actively monitor this forum to ensure that your questions are answered either by another user or one of us.  If you prefer to ask your questions on Stack Overflow, we also have an [Azure SQL Data Warehouse Stack Overflow Forum][Azure SQL Data Warehouse Stack Overflow Forum]O [Fórum de SQL data warehouse do Azure][Azure SQL Data Warehouse MSDN Forum] .
