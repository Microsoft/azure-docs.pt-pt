...
Azure SQL Data Warehouse CPU, memória e e/s são agrupados em unidades de escala de computação chamado unidades do Data Warehouse (DWUs). A DWU representa uma medida de abstrata, normalizada de recursos de computação e desempenho. Uma alteração para o seu nível de serviço altera o número de DWUs que estão disponíveis para o sistema, que por sua vez ajusta o desempenho e o custo, de seu sistema.

Para um desempenho mais elevado, pode aumentar o número de unidades de armazém de dados. Para um desempenho menor, reduza unidades do data warehouse. Os custos de armazenamento e computação são faturados separadamente, para que as unidades de armazém de dados de alteração não afetam os custos de armazenamento.

Desempenho de unidades do data warehouse baseia-se estas métricas de carga de trabalho do armazém de dados:

- Velocidade uma consulta de armazém de dados padrão podem analisar um grande número de linhas e, em seguida, efetuar uma agregação complexa. Esta operação é intensivas de e/s e CPU.
- A rapidez no armazém de dados pode ingerir dados de Blobs de armazenamento do Azure ou do Azure Data Lake. Esta operação é a rede e intensivas da CPU.
- Velocidade da [ `CREATE TABLE AS SELECT` ](/sql/t-sql/statements/create-table-as-select-azure-sql-data-warehouse) comando T-SQL, pode copiar uma tabela. Esta operação envolve a leitura de dados do armazenamento, a distribuição dos mesmos por nós da aplicação e a escrita novamente no armazenamento. Esta operação é a CPU, IO e rede intensiva.
...
À medida que aumenta a unidades de armazém de dados, linearmente estão a aumentar a recursos de computação. Geração 2 fornece o melhor desempenho das consultas e a escala mais elevada. Sistemas de geração 2 também tirar o máximo partido da cache.
...
3. Identifique quaisquer requisitos adicionais para periódicos períodos de atividade de pico. Picos de cargas de trabalho que mostram significativas e os vales na atividade poderão ter de ser aumentados com frequência.
...
1. Submeter a consulta seguinte para verificar o estado da operação
...
Essa DMV devolve informações sobre várias operações de gestão no seu armazém de dados SQL como a operação e o estado da operação, que é IN_PROGRESS ou COMPLETED.
...
- Para uma operação de aumento vertical, o sistema Desanexa todos os nós de computação, Aprovisiona os nós de computação adicional e, em seguida, volta a ligar para a camada de armazenamento.
- Para uma operação de redução vertical, o sistema Desanexa todos os nós de computação e, em seguida, volta a ligar apenas os nós necessários para a camada de armazenamento.
