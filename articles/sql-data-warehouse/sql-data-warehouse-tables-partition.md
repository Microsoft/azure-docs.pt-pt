...
O método mais eficiente para dividir uma divisória que já contém dados é usar uma declaração `CTAS`. Se a mesa dividida for uma loja de colunas agrupada, a divisória da mesa deve estar vazia antes de poder ser dividida.
...
A seguinte consulta encontra a contagem de filas utilizando a visão de catálogo `sys.partitions`:
...
Tudo o que resta é alinhar os dados com os novos limites de divisória suster usando `CTAS`e, em seguida, mudar os dados de volta para a tabela principal.
...
Carregar dados em divisórias com troca de divisórias é uma forma conveniente de encenar novos dados numa tabela que não é visível para os utilizadores o interruptor nos novos dados.  Pode ser um desafio em sistemas ocupados para lidar com a contenção de bloqueio associada à troca de divisórias.  Para limpar os dados existentes numa divisória, um `ALTER TABLE` usado para desativar os dados.  Em seguida, outra `ALTER TABLE` foi obrigada a mudar os novos dados.  No SQL Analytics, a opção `TRUNCATE_TARGET` é suportada no comando `ALTER TABLE`.  Com `TRUNCATE_TARGET` o comando `ALTER TABLE` sobrepor os dados existentes na partilha com novos dados.  Abaixo está um exemplo que usa `CTAS` para criar uma nova tabela com os dados existentes, insere novos dados e, em seguida, muda todos os dados de volta para a tabela alvo, sobrepondo os dados existentes.
