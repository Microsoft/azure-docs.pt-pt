...
| ----------------------- | ------------------------------------------------------------ | ---------------- |
| Percentagem de CPU          | Utilização da CPU em todos os nós para o data warehouse      | Méd., mín., máx.    |
| Percentagem de ES de Dados      | Utilização de e/s em todos os nós para o data warehouse       | Méd., mín., máx.    |
| Porcentagem de memória       | Utilização de memória (SQL Server) em todos os nós para o data warehouse | Méd., mín., máx.   |
| Consultas ativas          | Número de consultas ativas em execução no sistema             | Soma              |
| Consultas em fila          | Número de consultas em fila aguardando para iniciar a execução          | Soma              |
| Conexões com êxito  | Número de conexões bem-sucedidas com os dados                 | Soma, contagem       |
| Ligações com Falhas      | Número de conexões com falha para o data warehouse           | Soma, contagem       |
| Bloqueado pelo firewall     | Número de logons no data warehouse que foi bloqueado     | Soma, contagem       |
| Limite de DWU               | Objetivo de nível de serviço do data warehouse                | Méd., mín., máx.    |
| Porcentagem de DWU          | Máximo entre percentual de CPU e porcentagem de e/s de dados        | Méd., mín., máx.    |
| DWU usado                | Limite de DWU * DWU percentual                                   | Méd., mín., máx.    |
| Porcentagem de acesso ao cache    | (acertos de cache/erro de cache) * 100 em que acertos de cache é a soma de todos os segmentos columnstore acertos no cache SSD local e o erro de cache é a perda de segmentos columnstore no cache SSD local somado em todos os nós | Méd., mín., máx.    |
| Percentual de cache usado   | (cache usado/capacidade de cache) * 100 em que o cache usado é a soma de todos os bytes no cache SSD local em todos os nós e a capacidade de cache é a soma da capacidade de armazenamento do cache SSD local em todos os nós | Méd., mín., máx.    |
| Porcentagem de tempdb local | Utilização de tempdb local em todos os nós de computação-os valores são emitidos a cada cinco minutos | Méd., mín., máx.    |
