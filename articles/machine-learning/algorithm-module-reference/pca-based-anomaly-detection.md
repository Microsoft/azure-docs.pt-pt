...
Para obter mais informações sobre como funciona o PCA, e sobre a implementação para deteção de anomalias, consulte estes trabalhos:
...
2. No painel direito do módulo de **deteção de anomalias baseado em PCA,** clique na opção **modo treino** e indique se pretende treinar o modelo utilizando um conjunto específico de parâmetros ou utilizar uma varredura de parâmetros para encontrar os melhores parâmetros.
...
4. Especifique a quantidade de amostragem excessiva para realizar durante o treino de PCA aleatório. Em problemas de deteção de anomalias, os dados desequilibrados dificultam a aplicação de técnicas padrão de APC. Especificando alguma quantidade de sobreamostras, pode aumentar o número de casos-alvo.
    ...
    > [!NOTE]
    > Não é possível visualizar o conjunto de dados sobre-amostrado. Para obter mais informações sobre como a sobreamostragem é utilizada com PCA, consulte [Notas Técnicas](#technical-notes).
...
A avaliação dos resultados de um modelo de deteção de anomalias requer alguns passos adicionais:
