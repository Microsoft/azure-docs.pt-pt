...
* **Armazenar os seus ficheiros num [conjunto de dados](/python/api/azureml-core/azureml.data)de aprendizagem automática Azure .** Isto previne problemas de latência experimentais, e tem as vantagens de aceder a dados a partir de um alvo de computação remota, o que significa que a autenticação e montagem são geridas pela Azure Machine Learning. Saiba mais sobre como especificar um conjunto de dados como fonte de dados de entrada no seu script de treino com [Conjuntos de dados](how-to-train-with-datasets.md).
...
Ao escrever alterações, recomendamos escrever ficheiros para armazenamento através de um conjunto de dados de aprendizagem automática Azure com um [objeto OutputFileDatasetConfig](/python/api/azureml-core/azureml.data.output_dataset_config.outputfiledatasetconfig). Ver [como criar um OutputFileDatasetConfig](how-to-train-with-datasets.md#where-to-write-training-output).

Caso contrário, escreva ficheiros para a `./outputs` pasta e/ou. `./logs`
...
* Saiba mais sobre [o acesso aos dados a partir do armazenamento.](how-to-access-data.md)
