...
Há várias outras bibliotecas de aprendizado de máquina em DSVMs, como o popular pacote de `scikit-learn` que faz parte da distribuição Anaconda Python para DSVMs. Para verificar a lista de pacotes disponíveis no Python, R e Julia, execute os gestores de pacote respectivos.
...
| A utilização ou executá-lo      | Como uma ferramenta de interface do usuário. No Windows, inicie um prompt de comando, execute R e, dentro do R, execute `rattle()`. No Linux, conecte-se com o X2Go, inicie um terminal, execute o R e, em R, execute `rattle()`. |
