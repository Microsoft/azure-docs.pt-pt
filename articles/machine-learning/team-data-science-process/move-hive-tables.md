...
Você pode executar comando como `hive -e "<your hive query>;` submeter consultas simples da Hive diretamente na Linha de Comando Hadoop. Aqui está um exemplo, onde a caixa vermelha descreve o comando que submete a consulta da Colmeia, e a caixa verde descreve a saída da consulta da Colmeia.
...
No exemplo seguinte, a saída da consulta da Colmeia é escrita para um diretório blob `queryoutputdir` dentro do recipiente padrão do cluster Hadoop. Aqui, você só precisa fornecer o nome do diretório, sem o nome blob. Um erro é lançado se fornecer nomes de diretório e blob, tais como `wasb:///queryoutputdir/queryoutput.txt` .
...
Ao consultar tabelas divididas, recomenda-se adicionar a condição de partição no **início** da `where` cláusula, o que melhora a eficiência de pesquisa.
