...
Para utilizar a geração de esquemas, inclua a `inference-schema` versão 1.1.0 ou superior do pacote de código aberto no ficheiro de dependências. Para obter mais informações sobre este pacote, [https://github.com/Azure/InferenceSchema](https://github.com/Azure/InferenceSchema) consulte. Para gerar uma swagger em conformidade para o consumo automatizado de serviços web, a função de script de pontuação() deve ter a forma API de:
...
* Devolva um dicionário do tipo "StandardPythonParameterType" chamado **Resultados** e aninhados.
