...
Essa classe de erros significa um argumento em algum lugar fornecido era inválido. Isto pode ser uma credencial ou a localização de armazenamento do Azure para algo passado para o serviço web. Por favor, olhe para o campo de erro "código" na secção "detalhes" para diagnosticar qual argumento específico era inválido.
...
| BadVersionCall | O parâmetro de versão inválida foi passado durante a chamada da API: {0}. Verifique a página de ajuda da API para transmitir a versão correta e tente novamente. |
| BatchJobInputsNotSpecified | Os seguintes contributos necessários não foram especificados com o pedido: {0}. Certifique-se de todos os dados de entrada é especificado e tente novamente. |
| BatchJobInputsTooManySpecified | O pedido especificado entradas mais do que definido no serviço. Lista de contributos aceites: {0}. Certifique-se de todos os dados de entrada está corretamente especificada e tente novamente. |
| BlobNameTooLong | O caminho de armazenamento de bolhas azure previsto para a saída de diagnóstico é demasiado longo: {0}. Encurte o caminho e tente novamente. |
...
| ContainerValidationFailed | A validação do recipiente blob falhou com este erro: {0}. |
...
| ExpiryTimeInThePast | O prazo de validade previsto é no passado: {0}. Forneça uma hora de expiração futura em UTC e tente novamente. Para nunca expirar, definir a hora de expiração como NULL. |
...
| InvalidBlob | Especificação de bolha inválida para blob: {0}. Verifique se essa cadeia de ligação / caminho relativo ou especificação do token SAS está correta e tente novamente. |
| InvalidBlobConnectionString | A cadeia de ligação especificada para uma das bolhas de entrada/saída inválidas: {0}. Corrija isso e tente novamente. |
...
| InvalidInputNames | Nomes de entrada de serviço inválidos especificados no pedido: {0}. . Mapear os dados de entrada para as entradas de serviço correto e tente novamente. |
...
| MissingOutputOverrideSpecification | Falta-lhe a especificação de bolha para a sobreposição da saída {0}. Especifique uma localização de blob válido com o pedido ou remova a especificação de saída, se for o pretendido sem substituição de localização. |
...
| MissingRequiredOutputOverrides | Ao chamar um ponto de extremidade de serviço encriptada é obrigatório para passar na saída substituições para saídas do serviço. Falta sobreposições neste momento para estas saídas: {0} |
...
| ModelPackageIdInvalid | Pacote de modelo inválido Id. Verifique se o pacote de modelo Id está correto e tente novamente. |
...
| WebServiceConcurrentRequestRequirementInvalid | Não é possível alterar os requisitos simultâneos de pedidos para {0} serviço web. |
| WebServiceIdInvalid | Id de serviço web inválido fornecido. Id de serviço Web deve ser um guid válido. |
| WebServiceTooManyConcurrentRequestRequirement | Não é possível definir o requisito de pedido simultâneo para mais de {0}. |
| WebServiceTypeInvalid | Tipo de serviço web inválido fornecido. Verifique se que o tipo de serviço web válido está correto e tente novamente. Tipos de serviço web válidos: {0}. |
...
| Código de erro | Mensagem de utilizador |
| ---------- |--------------|
| CannotStartJob | O trabalho não pode ser iniciado porque está em {0} estado. |
...
| Código de erro | Mensagem de utilizador |
| ---------- |--------------|
| OutOfMemoryLimit | O modelo de consumir mais memória do que foi posse para ele. A memória máxima permitida para o modelo é {0} MB. Verifique o seu modelo para problemas. |
...
| InvalidMemoryConfiguration | Configuração de Memória Inválida, ConfigValue: {0} |
...
| PortDataTypeNotSupported | O id do porto={0} tem um tipo de dados não suportado: {1}. |
| ResourceDownload |  |
| ResourceLoadFailed |  |
| ServiceUrisNotFound |  |
| SwaggerGeneration | A geração Swagger falhou, Detalhes: {0} |
| UnexpectedScoreStatus |  |
| UnknownBackendErrorResponse |  |
| UnknownError |  |
| UnknownJobStatusCode | Código de estado de trabalho desconhecido {0}. |
| UnknownModuleError |  |
...
| RequestThrottled | O pedido foi acelerado para {0} ponto final. A moeda máxima para o ponto final é {1}. |
