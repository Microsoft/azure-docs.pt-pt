---
title: Como detectar os descompassos de dados (pré-visualização) nas implantações do AKS
...
ms.locfileid: "67332795"
---
# <a name="how-to-detect-data-drift-preview-on-models-deployed-to-azure-kubernetes-service"></a>Como detectar os descompassos de dados (pré-visualização) em modelos implementados no serviço Kubernetes do Azure
Neste artigo, irá aprender a monitorizar para [descompassos dados](concept-data-drift.md) entre os dados de conjunto de dados e inferência de tipos de treinamento de um modelo implementado. 

Desvios de dados é uma das razões principais em que a precisão do modelo degrada ao longo do tempo. Isso acontece quando os dados fornecidos para um modelo na produção são diferentes dos dados utilizados para preparar o modelo. O serviço Azure Machine Learning pode monitorizar os descompassos de dados com o detetor de desvios de dados. Se for detectado descompassos, o serviço pode enviar um alerta para.  
...
- Configurar o [recoletores de dados de modelo](how-to-enable-data-collection.md) recolher dados da implementação do AKS do modelo e confirmarem os dados é recolhidos no `modeldata` contentor de Blobs.
...
Para obter mais informações, consulte a [DataDrift](https://docs.microsoft.com/python/api/azureml-contrib-datadrift/?view=azure-ml-py) referência.

## <a name="submit-a-datadriftdetector-run"></a>Submeter uma execução de DataDriftDetector

Com o DataDriftDetector configurado, pode submeter um [descompassos de dados, executar](https://docs.microsoft.com/python/api/azureml-contrib-datadrift/azureml.contrib.datadrift.datadriftdetector%28class%29?view=azure-ml-py#run-target-date--services--compute-target-name-none--create-compute-target-false--feature-list-none--drift-threshold-none-) numa determinada data para o modelo. 
...
## <a name="get-data-drift-analysis-results"></a>Obter dados descompassos os resultados da análise
...
![Dados Descompassos Show](media/how-to-monitor-data-drift/drift_show.png)
...
## <a name="schedule-data-drift-detection"></a>Deteção de desvios de dados de agenda 

Ativar uma agenda de desvios de dados executa uma DataDriftDetector executar a frequência especificada. Se o coeficiente de desvios é superior ao limiar especificado, é enviado um e-mail. 
...
Definindo o coeficiente de desvios limiar de alerta e fornecendo um endereço de e-mail, um [do Azure Monitor](https://docs.microsoft.com/azure/azure-monitor/overview) alerta de e-mail é enviado, se o coeficiente de desvios é superior ao limiar. Todas as métricas de desvios de dados são armazenadas no recurso de informações da aplicação associado com a área de trabalho do serviço do Azure Machine Learning para configurar alertas personalizados ou de ações. Pode seguir a ligação no e-mail do alerta para a consulta de informações da aplicação.
