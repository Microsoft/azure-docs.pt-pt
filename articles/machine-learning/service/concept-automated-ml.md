...
Também pode inspecionar as informações de execução com sessão iniciada, que [contém métricas](how-to-understand-accuracy-metrics.md) reunidas durante a execução. A execução de treinamento produz um objeto de Python serializados (`.pkl` ficheiro) que contém o modelo e, em seguida, o pré-processamento de dados.
...
Criação de previsões é parte integrante de qualquer empresa, quer seja a pedido de receita, inventário, vendas ou cliente. Pode utilizar o ML automatizado para combinar técnicas e abordagens e obter recomendada de alta qualidade-série de tempo de previsão. 

Uma experimentação de séries temporais automatizada é tratada como um problema de multivariada. Nos últimos valores de séries temporais são "articulados" para se tornar dimensões adicionais para o regressor foi, juntamente com outros indicadores. Esta abordagem, ao contrário dos métodos de série de tempo clássicas, tem uma vantagem de incorporação naturalmente de várias variáveis contextuais e a respetiva relação entre si durante o treinamento. ML automatizada aprende um modelo único, mas, muitas vezes, internamente, ser também autor para todos os itens no horizontes de conjunto de dados e de predição. Mais dados, portanto, estão disponíveis para estimar os parâmetros de modelo e generalização a série que não foram visto torna-se possível. 

Saiba mais e ver um exemplo desse [automatizada de machine learning para previsão de séries de tempo](how-to-auto-train-forecast.md).
...
|[HDInsight](../../hdinsight/spark/apache-spark-run-machine-learning-automl.md)|Aumentar horizontalmente as suas tarefas de formação de ML automáticas, no Spark em clusters do HDInsight em paralelo.|
...
+ Experimentar o [amostras de bloco de notas do Jupyter para aprendizagem automatizada](https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/automated-machine-learning/)
