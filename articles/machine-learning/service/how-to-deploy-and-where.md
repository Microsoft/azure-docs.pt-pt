...
Saiba como implementar o modelo de machine learning como um serviço web na cloud do Azure ou para dispositivos do IoT Edge. As informações neste documento mostra como implementar os seguintes destinos de computação:
...
1. Registre-se modelo (s).
1. Implemente o modelo (s).
1. Teste implementado modelo (s).
...
- Um modelo. Se não tiver um modelo preparado, pode usar o modelo e os ficheiros de dependência fornecida no [deste tutorial](http://aka.ms/azml-deploy-cloud).

- O [extensão de CLI do Azure para o serviço Machine Learning](reference-azure-machine-learning-cli.md), ou o [SDK de Python do Azure Machine Learning](https://aka.ms/aml-sdk).

## <a id="registermodel"></a> Registe-se de um modelo de machine learning

O registro de modelo é uma forma de armazenar e organizar seus modelos ensinados na cloud do Azure. Modelos são registrados na sua área de trabalho do serviço do Azure Machine Learning. O modelo pode ser treinado com o Azure Machine Learning ou importados a partir de um modelo de formação em outro lugar. Os exemplos seguintes demonstram como registar um modelo do ficheiro:

### <a name="register-a-model-from-an-experiment-run"></a>Registe-se de um modelo a partir de uma execução de experimentação

**Exemplo de Scikit-Saiba com a CLI**
...
## <a name="how-to-deploy"></a>Como implementar

Para implementar como um serviço web, tem de criar uma configuração de inferência de tipos (`InferenceConfig`) e uma configuração de implementação. A configuração de inferência de tipos, especifique os scripts e as dependências necessárias para atender o seu modelo. A configuração de implementação é especificar detalhes de como a servir o modelo no destino de computação.
...
**Exemplo de Scikit-Saiba com geração de Swagger:**
```python
import json
import numpy as np
...
* Um diretório que contém os elementos precisos para realizar a inferência
...
* O ficheiro de conda que descreve os pacotes Python necessários para executar inferência
...
## <a name="where-to-deploy"></a>Onde pretende implementar
...
**Para implementar localmente, terá de ter o Docker instalado no seu computador local.**
...
> Se não tiver criado ou anexado um AKS cluster go <a href="#create-attach-aks">aqui</a>.

#### Implementar no AKS <a id="deploy-aks"></a>

Pode implementar no AKS com a CLI do Azure ML:
```azurecli-interactive
az ml model deploy -ct myaks -m mymodel:1 -n aksservice -ic inferenceconfig.json -dc deploymentconfig.json
```

Também pode utilizar o SDK de Python:
```python
...
Para obter mais informações sobre como configurar a implementação do AKS, incluindo o dimensionamento automático, consulte a [AksWebservice.deploy_configuration](https://docs.microsoft.com/python/api/azureml-core/azureml.core.webservice.akswebservice) referência.

**Estimativa de tempo:** Cerca de cinco minutos.

#### Criar ou ligar um cluster do AKS <a id="create-attach-aks"></a>
Criar ou anexar um cluster do AKS é um **um processo de tempo** sua área de trabalho. Depois de um cluster tiver sido associado a sua área de trabalho, pode usá-lo para várias implementações. 

Se eliminar o cluster ou o grupo de recursos que o contém, tem de criar um novo cluster da próxima vez que precisa implantar.
...
## Definições de configuração avançada <a id="advanced-config"></a>
...
O suporte para implementar no Edge está em pré-visualização. Para obter mais informações, consulte a [implementar o Azure Machine Learning como um módulo do IoT Edge](https://docs.microsoft.com/en-us/azure/iot-edge/tutorial-deploy-machine-learning) artigo.
