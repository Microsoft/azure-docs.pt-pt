...
### <a name="azure-machine-learning-sdk-for-python-v1220"></a>Azure Machine Learning SDK para Python v1.22.0
+ **Correções e melhorias de bugs**
  + **azureml-automl-core**
    + Bug fixo onde uma dependência extra de pip foi adicionada ao ficheiro conda yml para modelos de visão.
  + **azureml-automl-runtime**
    + Corrigiu um bug onde os modelos de previsão clássica (por exemplo, AutoArima) podiam receber dados de formação em que não existiam linhas com valores-alvo imputados. Isto violou o contrato de dados destes modelos. * Corrigimos vários bugs com comportamento de lag-by-ocorrência no operador de atraso da série de tempo. Anteriormente, a operação de lag-by-ocorrência não marcava corretamente todas as linhas imputadas e, por isso, nem sempre geraria os valores corretos do lag de ocorrência. Também corrigiu alguns problemas de compatibilidade entre o operador do lag e o operador da janela rolante com comportamento de lag-by-ocorrência. Isto previamente resultou na queda do operador da janela rolante de algumas linhas dos dados de formação que deveria utilizar de outra forma.
  + **azureml-core**
    + Adicionar suporte para autenticação token por público.
    + Adicione `process_count` à [Configuração PyTorch](/python/api/azureml-core/azureml.core.runconfig.pytorchconfiguration?preserve-view=true&view=azure-ml-py) para suportar trabalhos pyTorch multi-processos multi-processo.
  + **azureml-pipeline-steps**
    ...
    + [ParallelRunConfig](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.parallelrunconfig?preserve-view=true&view=azure-ml-py): adicione argumentos allowed_failed_count e allowed_failed_percent para verificar o limiar de erro no nível do mini lote. O limiar de erro tem 3 sabores agora:
       + error_threshold - o número de mini-itens de lote autorizados; 
       + allowed_failed_count - o número de mini lotes autorizados; 
       + allowed_failed_percent, a percentagem de mini lotes falhados permitidos. 
       
       Um trabalho vai parar se exceder algum deles. error_threshold é necessário para mantê-lo para trás compatibilidade. Desa cosar o valor para -1 para ignorá-lo.
    + Manuseamento fixo do espaço branco em nome AutoMLStep.
    + ScriptRunConfig é agora suportado por HyperDriveStep
  + **azureml-train-core**
    + As corridas de HyperDrive invocadas de um ScriptRun serão agora consideradas uma corrida para crianças.
    + Adicione `process_count` à [Configuração PyTorch](/python/api/azureml-core/azureml.core.runconfig.pytorchconfiguration?preserve-view=true&view=azure-ml-py) para suportar trabalhos pyTorch multi-processos multi-processo.
  + **azureml-widgets**
    + Adicione widget ParallelRunStepDetails para visualizar o estado de um ParallelRunStep.
    + Permite que os utilizadores de hiperdrive vejam um eixo adicional no gráfico de coordenadas paralelas que mostra o valor métrico correspondente a cada conjunto de hiperparímetros para cada criança executada.
...
### <a name="azure-machine-learning-sdk-for-python-v1210"></a>Azure Machine Learning SDK para Python v1.21.0
+ **Correções e melhorias de bugs**
  + **azure-cli-ml**
    + Texto de ajuda CLI fixo ao utilizar AmlCompute com identidade assatribuída ao utilizador
  + **azureml-contrib-automl-dnn-vision**
    + Os botões de implementação e descarregamento tornar-se-ão visíveis para as visão autoML, e os modelos podem ser implantados ou descarregados semelhantes a outras correções AutoML. Existem dois novos ficheiros (scoring_file_v_1_0_0.py e conda_env_v_1_0_0.yml) que contêm um script para executar inferencing e um ficheiro yml para recriar o ambiente conda. O ficheiro 'model.pth' também foi renomeado para utilizar a extensão '.pt'.
  + **azureml-core**
    + Apoio msi para azure-cli-ml
    + Suporte de identidade gerido atribuído ao utilizador.
    + Com esta alteração, os clientes devem ser capazes de fornecer uma identidade atribuída ao utilizador que possa ser usada para obter a chave do cofre chave do cliente para encriptação em repouso.
    +  corrigir row_count=0 para o perfil de ficheiros muito grandes - corrigir erro na conversão dupla para valores delimitados com estofamento de espaço branco
    + Remover bandeira experimental para o conjunto de dados de saída GA
    + Atualizar documentação sobre como obter versão específica de um Modelo
    + Permitir atualizar o espaço de trabalho para o acesso ao modo misto em caso de ligação privada
    + Correção para remover registo adicional na datastore para função de currículo
    + Apoio CLI/SDK adicionado para atualizar identidade atribuída ao utilizador primário do espaço de trabalho
  + **azureml-interpret**
    + atualizado azureml-interpret para interpret-community 0.16.0
    + otimizações de memória para cliente explicação em azureml-interpret
  + **azureml-train-automl-runtime**
    + Streaming ativado para execuções ADB
  + **azureml-train-core**
    + Correção para remover registo adicional na datastore para função de currículo
  + **azureml-widgets**
    + Os clientes não devem ver alterações na visualização de dados de execução existentes utilizando o widget, e agora terão suporte se utilizarem opcionalmente hiperparímetros condicional.
    + O widget de execução do utilizador agora inclui uma explicação detalhada para o porquê de uma corrida estar no estado de fila.


 ## <a name="2021-01-11"></a>2021-01-11

### <a name="azure-machine-learning-sdk-for-python-v1200"></a>Azure Machine Learning SDK para Python v1.20.0
+ **Correções e melhorias de bugs**
  + **azure-cli-ml**
    + framework_version adicionado na OptimizationConfig. Será utilizado quando o modelo estiver registado na estrutura MULTI.
  + **azureml-contrib-optimização**
    + framework_version adicionado na OptimizationConfig. Será utilizado quando o modelo estiver registado na estrutura MULTI.
  + **azureml-pipeline-steps**
    + Introduzindo o CommandStep que assumiria o comando para processar. O comando pode incluir executáveis, comandos de conchas, scripts, etc.
  + **azureml-core**
    + Agora a criação de espaço de trabalho suporta a identidade atribuída pelo utilizador. Adicionar o suporte uai da SDK/CLI
...
### <a name="azure-machine-learning-studio-notebooks-experience-december-update"></a>Experiência de cadernos do estúdio de aprendizagem de máquinas Azure (atualização de dezembro)
+ **Novas funcionalidades**
  + Pesquisa de nome de ficheiros do utilizador. Os utilizadores podem agora pesquisar todos os ficheiros guardados num espaço de trabalho.
  + Markdown Side by Side support per Notebook Cell. Numa célula portátil, os utilizadores podem agora ter a opção de visualizar a sintaxe de marcação e marcação lado a lado.
  + Barra de Estado celular. A barra de estado indica em que estado está uma célula de código, se uma célula foi bem sucedida, e quanto tempo demorou a ser executada. 
...
### <a name="azure-machine-learning-sdk-for-python-v1190"></a>Azure Machine Learning SDK para Python v1.19.0
+ **Correções e melhorias de bugs**
  + **azureml-automl-core**
    + Adicionou suporte experimental para dados de teste ao AutoMLStep.
    + Adicionou a implementação inicial do núcleo da função de ingestão de conjuntos de teste.
    + Mudou referências para sklearn.externals.joblib para depender diretamente do joblib.
    + introduzir um novo tipo de tarefa AutoML de "segmentação de exemplos de imagem".
  + **azureml-automl-runtime**
    + Adicionou a implementação inicial do núcleo da função de ingestão de conjuntos de teste.
    + Quando todas as cordas de uma coluna de texto têm um comprimento de exatamente 1 caracteres, o afeto de palavra-grama TfIdf não funcionará porque o seu tokenizer ignora as cordas com menos de 2 caracteres. A alteração de código atual permitirá ao AutoML lidar com este caso de utilização.
    + introduzir um novo tipo de tarefa AutoML de "segmentação de exemplos de imagem".
  + **azureml-contrib-automl-dnn-nlp**
    + Pr inicial para novo pacote dnn-nlp
  + **azureml-contrib-automl-dnn-vision**
    + introduzir um novo tipo de tarefa AutoML de "segmentação de exemplos de imagem".
  + **azureml-contrib-automl-pipeline-steps**
    + Este novo pacote é responsável pela criação de passos necessários para muitos modelos treinam/inferências. - Move também o código de comboio/inferência para azureml.train.automl.runtime package para que quaisquer correções futuras estejam automaticamente disponíveis através de lançamentos ambientais curados.
  ...
  + **azureml-mlflow**
    + AzureML-MLflow adiciona guardas seguros em scripts remotos para evitar o encerramento antecipado das execuções submetidas.
  + **azureml-pipeline-core**
    + Fixo um bug na definição de um gasoduto padrão para o ponto final do gasoduto criado via UI
  + **azureml-pipeline-steps**
    + Adicionou suporte experimental para dados de teste ao AutoMLStep.
  + **azureml-tensorboard**
    + Fixação dos avisos xref para documentação no pacote azureml-core
  ...
  + **azureml-train-automl-runtime**
    + Adicionou a implementação inicial do núcleo da função de ingestão de conjuntos de teste.
    + Corrija o cálculo das explicações brutas para o melhor modelo AutoML se os modelos AutoML forem treinados utilizando validation_size definição.
    + Mudou referências para sklearn.externals.joblib para depender diretamente do joblib.
  + **azureml-train-core**
    + HyperDriveRun.get_children_sorted_by_primary_metric() deve completar mais rápido agora
    + Melhor manuseamento de erros no HyperDrive SDK.
    +  Precipreciou todas as classes de estimadores a favor da utilização do ScriptRunConfig para configurar as experiências. As classes preprecadas incluem:
        + MMLBase
        + Estimador
        + PyTorch 
        + TensorFlow 
        + Chainer 
        + SKLearn
    + Precitou o uso de Nccl e Gloo como tipos de entrada válidos para classes de estimadores a favor da utilização de Configuração PyTorch com ScriptRunConfig.
    + Precitou o uso de Mpi como um tipo de entrada válido para as classes do Estimador a favor da utilização de MpiConfiguration com ScriptRunConfig.
    + Adicionar propriedade de comando à configuração de execução. A funcionalidade permite que os utilizadores executem um comando ou executáveis reais no cálculo através do AzureML SDK.

    +  Precipreciou todas as classes de estimadores a favor da utilização do ScriptRunConfig para configurar as experiências. As classes precamadas incluem: + MMLBaseEstimator + Estimador + PyTorch + TensorFlow + Chainer + SKLearn
    + Precitou o uso de Nccl e Gloo como um tipo válido de entrada para classes de estimadores em favor da utilização de Configuração de PyTorch com ScriptRunConfig. 
    + Precitou o uso de Mpi como um tipo válido de entrada para as classes do Estimador a favor da utilização de MpiConfiguration com ScriptRunConfig.
...
+ **Correções e melhorias de bugs**
  ...
  + Desempenho melhorado 
  + Melhoria da velocidade e da fiabilidade do núcleo
  + Grande carregamento de ficheiros. Já pode carregar ficheiro >95mb
...
+ **Correções e melhorias de bugs**
  + Melhoria da fiabilidade da velocidade e do núcleo
  + Atualizações do UI do Jupyter Widget
...
+ **Correções e melhorias de bugs**
  + **azure-cli-ml**
    + O AKSWebservice e o AKSEndpoints suportam agora os limites de CPU e de recursos de memória ao nível do pod. Estes limites opcionais podem ser usados através da definição `--cpu-cores-limit` e `--memory-gb-limit` bandeiras em chamadas CLI aplicáveis
  + **azureml-core**
  ...
  + **azureml-sdk**
    + `azureml-sdk` apoiar oficialmente Python 3.8.
  + **azureml-train-core**
...
+ **Correções e melhorias de bugs**
  ...
  + **azureml-mlflow**
    ...
    + Apoio Azure RBAC para operações de AzureML-MLflow 
...
+ **Correções e melhorias de bugs**
  ...
  + **azureml-core**
    ...
    + Adicione image_name e image_label parâmetros ao Modelo.pacote() para permitir renomear a imagem do pacote construído.
    + Novo método `set_pip_requirements()` para definir toda a secção de pip de uma [`CondaDependencies`](/python/api/azureml-core/azureml.core.conda_dependencies.condadependencies?preserve-view=true&view=azure-ml-py) vez.
  ...
  + **azureml-train-automl-runtime**
    ...
    + Falhas de execução de crianças fixas quando os dados contêm nan e a exibição é desligada.
  + **azureml-train-core**
    + Suporte adicional para especificar opções de pip (por exemplo --extra-index-url) no ficheiro de requisitos do pip passado para um [`Estimator`](/python/api/azureml-train-core/azureml.train.estimator.estimator?preserve-view=true&view=azure-ml-py) `pip_requirements_file` parâmetro através.
...
Agora pode criar, editar e partilhar cadernos e ficheiros de machine learning diretamente dentro da experiência web do estúdio de Azure Machine Learning. Você pode usar todas as aulas e métodos disponíveis em [Azure Machine Learning Python SDK](/python/api/overview/azure/ml/intro?preserve-view=true&view=azure-ml-py) de dentro destes cadernos Começar [aqui](./how-to-run-jupyter-notebooks.md)
...
+ **Correções e melhorias de bugs**
  ...
  + **azureml-core**
    ...
    + Acrescentou uma opção para [Environment.build_local()](/python/api/azureml-core/azureml.core.environment.environment?preserve-view=true&view=azure-ml-py) para empurrar uma imagem para o registo do espaço de trabalho AzureML
...
+ **Correções e melhorias de bugs**
  ...
  + [**azureml-core**](/python/api/azureml-core)
    ...
    + Altere [`Dataset.get_by_id`](/python/api/azureml-core/azureml.core.dataset%28class%29#get-by-id-workspace--id-) para devolver o nome e versão do registo se o conjunto de dados estiver registado.
    ...
    + Os conjuntos de dados recuperados durante uma execução serão rastreados e podem ser vistos na página de detalhes de execução ou ligando [`run.get_details()`](/python/api/azureml-core/azureml.core.run%28class%29#get-details--) após a execução estar concluída.
  ...
  + [**azureml-datadrift**](/python/api/azureml-datadrift)
    ...
    + Novos métodos `create_from_model()` e `create_from_dataset()` para a [`DataDriftDetector`](/python/api/azureml-datadrift/azureml.datadrift.datadriftdetector%28class%29) classe. O `create()` método será depretado.
  ...
  + **azureml-train-core**
    + Apoio PyTorch 1.3 no [`PyTorch`](/python/api/azureml-train-core/azureml.train.dnn.pytorch) estimador
...
+ **Correções e melhorias de bugs**
  ...
  + **[azureml-core](/python/api/azureml-core)**
    + Adicionou API a conjuntos de dados não registados. conjunto de dados. [unregister_all_versions()](/python/api/azureml-core/azureml.data.abstract_datastore.abstractdatastore#unregister--).
    ...
    + Consertado um bug em [run.get_metrics](/python/api/azureml-core/azureml.core.run.run#get-metrics-name-none--recursive-false--run-type-none--populate-false-) onde os pedidos falhariam se uma corrida tivesse muitos filhos
  ...
  + **[azureml-train-core](/python/api/azureml-train-core)**
    + Suporte tensorFlow 2.0 adicionado no [Estimador TensorFlow.](/python/api/azureml-train-core/azureml.train.dnn.tensorflow)
  + **[azureml-train-automl](/python/api/azureml-train-automl-runtime/)**
...
### <a name="azure-machine-learning-sdk-for-python-v1065"></a>Azure Machine Learning SDK para Python v1.0.65
  ...
  + **Novas funcionalidades**
    ...
    + Ambientes com curadoria adicionada. Estes ambientes foram pré-configurados com bibliotecas para tarefas comuns de aprendizagem automática, e foram pré-construídos e em cache como imagens docker para uma execução mais rápida. Aparecem por padrão na lista de ambiente do [Workspace,](/python/api/azureml-core/azureml.core.workspace%28class%29)com prefixo "AzureML".
...
+ **Correções e melhorias de bugs**
  ...
  + **azureml-train-automl**
    ...
    + Mensagem de depreciação adicionada para explain_model e retrieve_model_explanations())
  + **[azureml-pipeline-core](/python/api/azureml-pipeline-core)**
    + Adicionei um [caderno](https://aka.ms/pl-modulestep) para descrever [o Módulo](/python/api/azureml-pipeline-core/azureml.pipeline.core.module%28class%29), [ModVersion, e [ModuleStep](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.modulestep).
  + **[azureml-pipeline-steps](/python/api/azureml-pipeline-steps)**
    + Adicionado [RScriptStep](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.rscriptstep) para suportar script R executado através do pipeline AML.
...
+ Azure ML Hardware Modelos Acelerados em [FPGAs](how-to-deploy-fpga-web-service.md) estão geralmente disponíveis.
  ...
  + Implementar o recipiente num dispositivo de servidor [Azure Data Box Edge](../databox-online/azure-stack-edge-overview.md)
...
+ **Azure ML CLI está geralmente disponível (GA)**<br/> Os modelos podem agora ser implantados com um único comando CLI. Temos feedback comum dos clientes que ninguém implementa um modelo ML de um caderno Jupyter. A [**documentação de referência do CLI**](./reference-azure-machine-learning-cli.md) foi atualizada.
...
Foi [`PipelineEndpoint`](/python/api/azureml-pipeline-core/azureml.pipeline.core.pipeline_endpoint.pipelineendpoint?preserve-view=true&view=azure-ml-py) introduzido para adicionar uma nova versão de um oleoduto publicado, mantendo o mesmo ponto final.
...
+ **Novas funcionalidades**
  ...
  Para obter uma lista das versões suportadas pela mais recente versão SDK, consulte a documentação do [Estimador DNN](/python/api/azureml-train-core/azureml.train.dnn?preserve-view=true&view=azure-ml-py).
...
+ **Novas funcionalidades**
  + A Azure Machine Learning agora fornece suporte de primeira classe para o popular DNN framework Chainer. A utilização de [`Chainer`](/python/api/azureml-train-core/azureml.train.dnn.chainer?preserve-view=true&view=azure-ml-py) utilizadores de classe pode facilmente treinar e implementar modelos Chainer.
...
+ **Correções e melhorias de bugs**
  + Adicionámos suporte em oleodutos Azure Machine Learning para definir a propriedade source_directory_data_store para uma loja de dados desejada (como um armazenamento de [bolhas) em RunConfigurations](/python/api/azureml-core/azureml.core.runconfig.runconfiguration?preserve-view=true&view=azure-ml-py) que são fornecidas ao [PythonScriptStep](/python/api/azureml-pipeline-steps/azureml.pipeline.steps.python_script_step.pythonscriptstep?preserve-view=true&view=azure-ml-py). Por predefinição, utilize a loja de ficheiros Azure como a loja de dados de suporte, que pode colidir com problemas de estrangulamento quando um grande número de passos são executados simultaneamente.
