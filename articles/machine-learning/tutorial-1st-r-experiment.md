...
1. Uma lista de pastas exibe a exibição de cada usuário que acessa o espaço de trabalho.  Selecione a pasta para clonar a pasta **Train-and-Deploy-to-ACI** .

## <a name="a-nameopenopen-rstudio"></a><a name="open">abrir RStudio

Use RStudio em uma instância de computação ou VM de notebook para executar este tutorial.  
...
1. No RStudio, sua pasta **Train-and--Deploy-to-ACI** tem alguns níveis de **usuários** na seção **arquivos** no canto inferior direito.  Selecione a pasta **Train-and-Deploy-to-ACI** para localizar os arquivos necessários neste tutorial.

> [!Important]
> O restante deste artigo contém o mesmo conteúdo que você vê no **Train-and-Deploy-to-ACI. Arquivo RMD** . Se você tiver experiência com o RMarkdown, sinta-se à vontade para usar o código desse arquivo.  Ou você pode copiar/colar os trechos de código a partir daí, ou deste artigo em um script R ou na linha de comando.  
...
* Conectar-se a um espaço de trabalho, para que sua instância de computação possa se comunicar com recursos remotos
