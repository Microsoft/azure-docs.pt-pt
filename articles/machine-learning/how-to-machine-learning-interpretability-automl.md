...
print(engineered_explanations.get_feature_importance_dict())
```
Para modelos treinados com ML automatizado, você pode obter o melhor modelo usando o `get_output()` método e explicações computacionais localmente.  Pode visualizar os resultados da explicação com `ExplanationDashboard` o `interpret-community` pacote.
