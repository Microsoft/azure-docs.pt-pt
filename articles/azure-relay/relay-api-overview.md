...
A biblioteca-quadro .NET apenas suporta o modelo de programação wCF e baseia-se num protocolo binário proprietário baseado no `net.tcp` transporte wCF. Este protocolo e biblioteca mantém-se para retrocompatibilidade com aplicações existentes.
...
O `hyco-https` módulo altere e sobrepõe parcialmente os módulos de node.js `http` `https` e, proporcionando uma implementação de ouvintes HTTPS compatível com muitos módulos e aplicações nó.js existentes que dependem destes módulos centrais.

Os `hyco-ws` `hyco-websocket` módulos e módulos alterem os `ws` populares e os `websocket` módulos para o Node.js, proporcionando implementações alternativas de ouvintes que permitem que módulos e aplicações dependam de qualquer módulo para trabalhar por trás do Relay de Ligações Híbridas.
