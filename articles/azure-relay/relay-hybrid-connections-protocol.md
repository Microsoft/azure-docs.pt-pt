...
Os ouvintes que se ligam às Conexões Híbridas com suporte HTTP DEVEM lidar com o `request` gesto. Um ouvinte que não lida com o tratamento `request` e, portanto, causa erros de tempo repetidos enquanto está ligado pode ser listado na lista negra pelo serviço no futuro.
...
1. Se a corda de consulta contiver uma `sb-hc-token` expressão, a expressão será sempre retirada da corda de consulta. Será avaliado se a autorização de retransmissão estiver ligada.
...
3. Só se a autorização de retransmissão for ligada, e se os cabeçalhos de pedido contiverem um `Authorization` cabeçalho, e nenhuma das expressões anteriores estiver presente, o cabeçalho será avaliado e despido. Caso contrário, o `Authorization` é sempre transmitido como está.
...
O nome de domínio totalmente qualificado do espaço de `namespace-address` nome Sulista Azure que acolhe a Conexão Híbrida, tipicamente do `{myname}.servicebus.windows.net` formulário.
...
| `{path}`         | Sim      | O caminho espaço de nome codificado por URL da Ligação Híbrida pré-configurada para registar este ouvinte ligado. Esta expressão está anexada à parte do `$hc/` caminho fixo.
...
O mesmo se aplica ao `Sec-WebSocket-Extensions` cabeçalho. Se a estrutura suportar uma extensão, deve definir o cabeçalho para a resposta do lado do servidor do aperto de mão necessário `Sec-WebSocket-Extensions` para a extensão.
...
`{path}`é o caminho espaço de espaço de nome codificado por URL da Ligação Híbrida pré-configurada para registar este ouvinte. Esta expressão está anexada à parte do `$hc/` caminho fixo.
...
Isto permite ao cliente remetente passar argumentos de envio para o ouvinte que aceita quando não é possível incluir cabeçalhos HTTP. A expectativa é que o quadro do ouvinte analise a parte do caminho fixo e o nome registado do caminho e torne o restante, possivelmente sem argumentos de corda de consulta pré-fixados, `sb-` disponíveis para o pedido de decisão para decidir se aceitam a ligação.
...
##### <a name="rejecting-the-socket"></a>Rejeitando a tomada
 ...
 Rejeitar a tomada após inspecionar a mensagem requer um aperto de mão semelhante para que o código de estado e a descrição do `accept` estado que comunicam a razão da rejeição possam voltar para o remetente.
 ...
 Para rejeitar a tomada, o cliente retira o endereço URI da mensagem e anexa dois parâmetros de corda de `accept` consulta, da seguinte forma:
...
Se não houver corpo, as molduras do corpo são omitidas. O indicador para se um corpo está presente é a `body` propriedade booleana na mensagem de pedido.
...
O conteúdo da JSON `request` é o seguinte:

* **endereço** - corda URI. Este é o endereço de encontro a utilizar para este pedido. Se o pedido de entrada for superior a 64 kB, o restante desta mensagem fica vazio e o cliente DEVE iniciar um aperto de mão equivalente à `accept` operação descrita abaixo. Em seguida, o serviço colocará o completo `request` na tomada Web estabelecida. Se se pode esperar que a resposta ultrapasse os 64 kB, o ouvinte DEVE também iniciar um aperto de mão de encontro e, em seguida, transferir a resposta sobre a tomada Web estabelecida.
...
* **solicitarTarget** – corda. Este imóvel detém o "Alvo de [Pedido" (RFC7230, Secção 5.3)](https://tools.ietf.org/html/rfc7230#section-5.3) do pedido. Isto inclui a porção de corda de consulta, que é despojada de todos os `sb-hc-` parâmetros pré-fixados.
...
A resposta é um objeto da JSON chamado "resposta". As regras para o manuseamento do conteúdo do corpo são exatamente como com a `request` mensagem e baseadas na `body` propriedade.

* **requestId** – corda. Necessário. O `id` valor patrimonial da mensagem a ser `request` respondida.
...
Para respostas superiores a 64 kB, a resposta DEVE ser entregue sobre uma tomada de ponto de encontro. Além disso, se o pedido exceder 64 kB, e o `request` único contiver o campo de endereços, deve ser estabelecida uma tomada de encontro para obter o `request` . Uma vez estabelecida uma tomada de encontro, as respostas ao respetivo cliente e os pedidos subsequentes desse respetivo cliente DEVEM ser entregues sobre a tomada de encontro enquanto persiste.

O URL no deve ser utilizado como é para estabelecer a tomada de `address` `request` encontro, mas contém os seguintes parâmetros:
...
Quando o sinal de escuta estiver prestes a expirar, pode substituí-lo enviando uma mensagem de texto para o serviço através do canal de controlo estabelecido. A mensagem contém um objeto JSON chamado `renewToken` , que define a seguinte propriedade neste momento:
...
O _nome space-address_ é o nome de domínio totalmente qualificado do espaço de nome Sulista Azure que acolhe a Conexão Híbrida, tipicamente do formulário `{myname}.servicebus.windows.net` .

O pedido pode conter cabeçalhos http extra arbitrários extra, incluindo os definidos pela aplicação. Todos os cabeçalhos fornecidos fluem para o ouvinte e podem ser encontrados no `connectHeader` objeto da mensagem de controlo de **aceitação.**

As opções de parâmetros de corda de consulta são as seguintes:

| Param          | Necessário? | Descrição
| -------------- | --------- | -------------------------- |
| `sb-hc-action` | Sim       | Para o papel do remetente, o parâmetro deve ser `sb-hc-action=connect` .
| `{path}`       | Sim       | (ver o parágrafo seguinte)
...
| `sb-hc-id`     | Não        | Um ID opcional que permite o rastreio de diagnóstico de ponta a ponta e é disponibilizado ao ouvinte durante o aperto de mão aceite.
 ...
 É o caminho espaço de espaço de `{path}` nome codificado por URL da Ligação Híbrida pré-configurada para registar este ouvinte. A `path` expressão pode ser estendida com um sufixo e uma expressão de corda de consulta para comunicar mais. Se a Ligação Híbrida estiver registada no caminho, a expressão pode ser seguida pelos parâmetros de corda de `hyco` `path` consulta `hyco/suffix?param=value&...` definidos aqui. Uma expressão completa pode então ser a seguinte:

```
wss://{namespace-address}/$hc/hyco/suffix?param=value&sb-hc-action=...[&sb-hc-id=...&]sbc-hc-token=...
```

A expressão é transmitida ao ouvinte no endereço URI contido na mensagem de `path` controlo "aceitar".
...
O símbolo também pode ser transportado no `ServiceBusAuthorization` cabeçalho ou `Authorization` http. O símbolo pode ser omitido se a Ligação Híbrida estiver configurada para permitir pedidos anónimos.
...
O serviço adiciona o nome de anfitrião do espaço de retransmissão a `Via` .
