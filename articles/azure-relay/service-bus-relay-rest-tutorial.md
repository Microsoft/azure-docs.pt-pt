...
description: 'Tutorial: Construa uma aplicação de hospedagem Azure Relay que exponha uma interface baseada em REST.'
...
1. Selecione **Browse,** em seguida, procure e escolha **WindowsAzure.ServiceBus**. Selecione **Instalar**e aceitar os termos de utilização.
    ...
    Este passo adiciona referências ao Service Bus e *System.ServiceModel.dll*. Este pacote adiciona automaticamente referências às bibliotecas de autocarros de serviço e ao WCF `System.ServiceModel` .

1. Adicione explicitamente uma referência `System.ServiceModel.Web.dll` ao projeto. No **Solution Explorer,** clique à direita **Referências** sob a pasta do projeto e selecione **Adicionar Referência**.
...
1. Mude o nome do espaço de `ImageListener` nome para `Microsoft.ServiceBus.Samples` .
...
1. Dentro da `IImageContract` interface, declare um método para a operação única que o `IImageContract` contrato expõe na interface e aplique o `OperationContract` atributo ao método que pretende expor como parte do contrato de autocarro de serviço público.
...
1. No `OperationContract` atributo, adicione o `WebGet` valor.
   ...
   A adição do valor permite que o serviço de `WebGet` retransmissão encaminhe os pedidos HTTP GET `GetImage` para, e traduzir os valores de devolução de `GetImage` uma `HTTP GETRESPONSE` resposta. Mais tarde no tutorial, você usará um navegador web para aceder a este método, e para exibir a imagem no navegador.
...
1. Imediatamente depois da definição`IImageContract`, declare um canal que herde das interfaces `IImageContract` e `IClientChannel`.
   ...
   Um canal é o objeto de WCF através do qual o serviço e o cliente passam informações entre si. Mais tarde, cria o canal na sua aplicação de anfitrião. O Azure Relay utiliza este canal para passar os pedidos http GET do navegador para a sua `GetImage` implementação. O retransmissor também utiliza o canal para pegar no valor de `GetImage` devolução e traduzi-lo num `HTTP GETRESPONSE` para o navegador cliente.
...
Para criar um serviço de Retransmissão WCF estilo REST, primeiro crie o contrato utilizando uma interface. O passo seguinte consiste em implementar a interface. Este procedimento envolve a criação de uma classe chamada `ImageService` que implementa a interface definida pelo `IImageContract` utilizador. Depois de implementar o contrato, configura a interface utilizando um ficheiro *App.config.* O ficheiro de configuração contém informações necessárias para a aplicação. Estas informações incluem o nome do serviço, o nome do contrato e o tipo de protocolo que é utilizado para comunicar com o serviço de retransmissão. O código utilizado para estas tarefas aparece no exemplo que se segue ao procedimento.
...
1. Adicione as `using` seguintes declarações associadas:
...
1. Imediatamente após o código anterior, adicione o seguinte `GetImage` método na classe para devolver uma mensagem HTTP que `ImageService` contenha a imagem.
...
1. O elemento XML `<system.serviceModel>` é um elemento de WCF que define um ou vários serviços. Aqui, é usado para definir o nome de serviço e o ponto final. Na parte inferior do `<system.serviceModel>` elemento, mas ainda `<system.serviceModel>` dentro, adicione um `<bindings>` elemento que tenha o seguinte conteúdo:
...
1. Depois do elemento `<bindings>`, adicione um elemento `<services>`. Assim como nos enlaces, pode definir vários serviços num único ficheiro de configuração. No entanto, neste tutorial, definirá apenas um.
    ...
    Este conteúdo configura um serviço que utiliza o padrão previamente definido `webHttpRelayBinding` . Também utiliza o padrão `sbTokenProvider` , que é definido no passo seguinte.

1. Depois do `<services>` elemento, crie um `<behaviors>` elemento com o seguinte conteúdo, substituindo a tecla Assinatura de Acesso Partilhado `SAS_KEY` (SAS). Para obter uma chave SAS do [portal Azure,][Azure portal]consulte [Obter credenciais](service-bus-relay-tutorial.md#get-management-credentials)de gestão .
...
1. Ainda em *App.config,* no elemento, substitua todo o valor da cadeia de ligação pela cadeia de `<appSettings>` ligação que obteve anteriormente a partir do portal.
...
O seguinte código mostra a implementação do contrato e do serviço para um serviço baseado em REST que está em funcionamento no Service Bus utilizando a `WebHttpRelayBinding` encadernação.
...
1. Na declaração de `Main()` funções, crie uma variável para armazenar o espaço de nome do seu projeto. Certifique-se de substituir pelo nome do espaço de `yourNamespace` nome retransmissor que criou anteriormente.
...
O anfitrião do serviço é o objeto de WCF que cria uma instância na aplicação anfitriã. Este exemplo passa-lhe o tipo de hospedeiro que pretende criar, que é um `ImageService` , e também o endereço em que pretende expor a aplicação anfitriã.
