...
ms.locfileid: "83698764"
---
# <a name="azure-synapse-analytics-shared-metadata"></a>Metadados partilhados da Azure Synapse Analytics

O Azure Synapse Analytics permite que os diferentes motores computacionais do espaço de trabalho partilhem bases de dados e tabelas entre as suas piscinas Spark (pré-visualização) e o motor on-demand SQL (pré-visualização).
...
3. A Spark criou bases de dados e as suas mesas apoiadas pelo Parquet tornam-se visíveis no motor a pedido do Espaço de Trabalho SQL. [As bases](database.md) de dados são criadas automaticamente nos metadados a pedido do SQL, e tanto as [tabelas externas como geridas criadas](table.md) por um trabalho spark são tornadas acessíveis como tabelas externas nos metadados on-demand sQL no `dbo` esquema da base de dados correspondente. 
...
Se um objeto de metadados for eliminado ou alterado com a Spark, as alterações são captadas e propagadas ao motor on-demand SQL. A sincronização é assíncrona e as mudanças refletem-se no motor SQL após um curto atraso.
