...
Assumindo que os itens no contentor Cosmos DB `date_rep` `cases` têm, e `geo_id` propriedades, os resultados desta consulta são apresentados no quadro seguinte:
...
> [!IMPORTANT]
> `OPENROWSET` função sem `WITH` cláusula expõe ambos os valores com os tipos esperados e os valores com tipos introduzidos incorretamente. Este trabalho foi concebido para a exploração de dados e não para reporte. Não analise os valores JSON devolvidos desta função para construir relatórios e usar [cláusula COM](#querying-items-with-full-fidelity-schema) explícita para criar os seus relatórios.
...
### <a name="querying-items-with-full-fidelity-schema"></a>Consulta de itens com esquema de fidelidade total

Ao consultar o esquema de fidelidade completo, precisa especificar explicitamente o tipo SQL e o tipo de propriedade do Cosmos DB esperado na `WITH` cláusula. Não utilize `OPENROWSET` sem cláusula nos `WITH` relatórios porque o formato do conjunto de resultados pode ser alterado na pré-visualização com base no feedback.

No exemplo seguinte, assumimos que `string` é o tipo correto para a propriedade e o tipo correto para a `geo_id` `int32` `cases` propriedade:
...
Se tiver valores com outros tipos `cases.int64` `cases.float64` (,) que não possam ser limpos no recipiente Cosmos DB, terá de os referenciar explicitamente em `WITH` cláusula e combinar os resultados. A seguinte consulta agrega ambos `int32` , e armazenado na `int64` `float64` `cases` coluna:
