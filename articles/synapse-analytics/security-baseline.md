---
title: Linha de segurança para Azure Synapse Analytics
description: Linha de base de segurança Azure para Synapse Analytics
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 06/22/2020
...
**Orientação**: Ao ligar-se à sua piscina Azure Synapse SQL e tiver ativado registos de fluxo do grupo de segurança da rede (NSG), envie registos para uma Conta de Armazenamento Azure para auditoria de tráfego. Também pode enviar registos de fluxo para um espaço de trabalho do Log Analytics ou transmiti-los para Os Centros de Eventos. Se necessário para investigar a atividade anómala, ative a captura de pacotes do Observador de Rede.
...
**Orientação**: Utilize o Registo de Atividades Azure para monitorizar as configurações de recursos de rede e detetar alterações nos recursos de rede relacionados com a sua piscina Azure Synapse SQL. Crie alertas dentro do Azure Monitor que irão desencadear quando ocorrerem alterações nos recursos críticos da rede.
...
**Orientação**: Uma política de auditoria pode ser definida para uma base de dados específica ou como uma política de servidor padrão em Azure (que acolhe base de dados SQL ou Azure Synapse). Uma política de servidor aplica-se a todas as bases de dados existentes e recém-criadas no servidor.
...
**Orientação**: Ative a auditoria ao nível do servidor Azure SQL para a sua piscina Synapse SQL e escolha um local de armazenamento para os registos de auditoria (Azure Storage, Log Analytics ou Event Hubs).
...
**Orientação**: Ao armazenar registos relacionados com a sua piscina Synapse SQL numa Conta de Armazenamento, Log Analytics ou centros de eventos, desempate o período de retenção de registos de acordo com os regulamentos de conformidade da sua organização.
...
* [Como configurar alertas personalizados para a Base de Dados Azure SQL](https://docs.microsoft.com/azure/sql-database/sql-database-insights-alerts-portal?view=azps-1.4.0)
...
**Orientação**: Não aplicável; para recursos relacionados com o seu pool Synapse SQL, a solução anti-malware é gerida pela Microsoft na plataforma subjacente.
...
**Orientação**: Não aplicável; nenhum registo DNS é produzido por recursos relacionados com a sua piscina Synapse SQL.
...
**Orientação**: O controlo de acesso baseado em funções (Azure RBAC) tem funções incorporadas que devem ser explicitamente atribuídas e são consultadas. Utilize o módulo Azure AD PowerShell para realizar consultas ad-hoc para descobrir contas que são membros de grupos administrativos.

Também pode configurar funções ao nível da base de dados, bem como criar funções personalizadas para os utilizadores que acedam aos seus recursos de Base de Dados Azure SQL. As funções ao nível do servidor não estão atualmente disponíveis para O SQL Azure Synapse.
...
* [Compreender funções ao nível da base de dados](https://docs.microsoft.com/sql/relational-databases/security/authentication-access/database-level-roles?view=sql-server-ver15)
...
**Orientação**: O Azure Ative Directory não tem o conceito de palavras-passe padrão. Ao a provisionar uma base de dados/servidor Azure SQL ou piscina Sinaapse SQL, recomenda-se que opte por integrar a autenticação com o Azure Ative Directory.
...
* [Compreender a auditoria do servidor SQL](https://docs.microsoft.com/sql/relational-databases/security/auditing/sql-server-audit-database-engine?view=sql-server-ver15)
...
**Orientação**: Crie um administrador Azure Ative Directory (AD) para o servidor Azure SQL Database na sua piscina Synapse SQL.
...
**Orientação**: Em cenários de suporte em que a Microsoft necessita de aceder a dados relacionados com a Base de Dados Azure SQL na sua piscina Sinapse SQL, o Azure Customer Lockbox fornece uma interface para que você reveja e aprove ou rejeite pedidos de acesso a dados.
...
**Orientação**: Para qualquer Base de Dados Azure SQL na sua piscina Synapse SQL armazenando ou processando informações sensíveis, marque a base de dados e recursos relacionados como sensíveis através de tags. Configure o Private Link em conjunto com as etiquetas de serviço do Grupo de Segurança da Rede (NSG) nas suas instâncias de Base de Dados Azure SQL para impedir a exfiltração de informações sensíveis.
...
**Orientação**: Qualquer recurso relacionado com o seu pool Synapse SQL que seja necessário para operações comerciais, mas que possa incorrer em maior risco para a organização, deve ser isolado dentro da sua própria máquina virtual e/ou rede virtual e suficientemente seguro com um Azure Firewall ou Grupo de Segurança de Rede.
...
**Orientação**: Aproveite o Centro de Segurança Azure para realizar verificações de base para quaisquer recursos relacionados com a sua piscina Synapse SQL.
...
* [Backup e restauro na piscina Azure Synapse SQL](https://docs.microsoft.coms/azure/synapse-analytics/sql-data-warehouse/backup-and-restore)

* [Como apoiar as teclas do Cofre da Chave Azure](https://docs.microsoft.com/powershell/module/azurerm.keyvault/backup-azurekeyvaultkey?view=azurermps-6.13.0)
...
**Orientação:** Teste periodicamente os seus pontos de restauro para garantir que as suas imagens são válidas. Para restaurar uma piscina SQL existente a partir de um ponto de restauro, você pode usar o portal Azure ou PowerShell. Teste de restauração de chaves geridas pelo cliente.

* [Como restaurar as teclas do Cofre da Chave Azure](https://docs.microsoft.com/powershell/module/azurerm.keyvault/restore-azurekeyvaultkey?view=azurermps-6.13.0)

* [Backup e restauro na piscina Azure Synapse SQL](https://docs.microsoft.coms/azure/synapse-analytics/sql-data-warehouse/backup-and-restore)

* [Como restaurar uma piscina SQL existente](https://docs.microsoft.com/azure/synapse-analytics/sql-data-warehouse/sql-data-warehouse-restore-active-paused-dw)
...
**Orientação**: Siga as [Regras de Engajamento da Microsoft](https://www.microsoft.com/msrc/pentest-rules-of-engagement?rtc=1) para garantir que os seus Testes de Penetração não violam as políticas da Microsoft.

Pode encontrar mais informações sobre a estratégia da Microsoft e a execução de testes de penetração em Red Teaming e sites ao vivo contra infraestruturas, serviços e aplicações geridas pela Microsoft [aqui.](https://gallery.technet.microsoft.com/Cloud-Red-Teaming-b837392e)
