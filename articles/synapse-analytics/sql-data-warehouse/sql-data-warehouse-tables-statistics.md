...
description: Recomendações e exemplos para a criação e atualização de estatísticas de otimização de consultas em tabelas no Azure SQL Data Warehouse.
...
ms.locfileid: "80351188"
---
# <a name="table-statistics-in-azure-sql-data-warehouse"></a>Estatísticas de tabelas no Armazém de Dados Azure SQL

Recomendações e exemplos para a criação e atualização de estatísticas de otimização de consultas em tabelas no Azure SQL Data Warehouse.

## <a name="why-use-statistics"></a>Por que usar estatísticas

Quanto mais Azure SQL Data Warehouse souber sobre os seus dados, mais rápido poderá executar consultas contra ele. Depois de carregar dados no SQL Data Warehouse, recolher estatísticas sobre os seus dados é uma das coisas mais importantes que pode fazer para otimizar as suas consultas. O otimizador de consultas do SQL Data Warehouse é um otimizador baseado no custo. Compara o custo de vários planos de consulta, e depois escolhe o plano com o menor custo. Na maioria dos casos, escolhe o plano que executará o mais rápido. Por exemplo, se o optimizador estimar que a data em que a sua consulta está a ser filtrada, irá devolver uma linha, escolherá um plano. Se estimar que a data selecionada devolverá 1 milhão de linhas, devolverá um plano diferente.

## <a name="automatic-creation-of-statistic"></a>Criação automática de estatística

Quando a base de dados AUTO_CREATE_STATISTICS opção está posta, o SQL Data Warehouse analisa as consultas de entrada dos utilizadores para estatísticas em falta. Se faltarem estatísticas, o optimizador de consultas cria estatísticas sobre colunas individuais no predicado ou junta-se às condições para melhorar as estimativas de cardinalidade para o plano de consulta. A criação automática de estatísticas é atualmente ligada por defeito.
...
Se o seu armazém de dados não tiver AUTO_CREATE_STATISTICS configurado, recomendamos que ative esta propriedade executando o seguinte comando:
...
A criação automática de estatísticas é feita de forma sincronizada para que possa incorrer num desempenho de consulta ligeiramente degradado se as suas colunas faltarem estatísticas. O tempo para criar estatísticas para uma única coluna depende do tamanho da tabela. Para evitar uma degradação mensurável do desempenho, especialmente no benchmarking de desempenho, deve garantir que as estatísticas foram criadas primeiro executando a carga de trabalho de referência antes de perfilar o sistema.

> [!NOTE]
> A criação de estatísticas será registada em [sys.dm_pdw_exec_requests](/sql/relational-databases/system-dynamic-management-views/sys-dm-pdw-exec-requests-transact-sql?view=azure-sqldw-latest) num contexto de utilizador diferente.

Quando as estatísticas automáticas forem criadas, assumirão o formulário: _WA_Sys<_ id de coluna de 8 dígitos em Hex>_<id de tabela de 8 dígitos em Hex>. Pode ver estatísticas que já foram criadas executando o comando [de SHOW_STATISTICS DBCC:](/sql/t-sql/database-console-commands/dbcc-show-statistics-transact-sql?view=azure-sqldw-latest)

```sql
DBCC SHOW_STATISTICS (<table_name>, <target>)
```

O table_name é o nome da tabela que contém as estatísticas a apresentar. Esta não pode ser uma mesa externa. O alvo é o nome do índice alvo, estatísticas ou coluna para o qual apresentar informações estatísticas.

## <a name="updating-statistics"></a>Atualização de estatísticas

Uma das melhores práticas é atualizar as estatísticas sobre as colunas de datas todos os dias à medida que são adicionadas novas datas. Cada vez que novas filas são carregadas no armazém de dados, novas datas de carga ou datas de transação são adicionadas. Estes alteram a distribuição de dados e fazem as estatísticas desatualizadas. Inversamente, as estatísticas sobre uma coluna país/região numa tabela de clientes podem nunca precisar de ser atualizadas, porque a distribuição de valores geralmente não muda. Assumindo que a distribuição é constante entre os clientes, adicionar novas linhas à variação da tabela não vai alterar a distribuição de dados. No entanto, se o seu armazém de dados contiver apenas um país/região e trouxer dados de um novo país/região, resultando em dados de vários países/regiões armazenados, então precisa de atualizar estatísticas sobre a coluna país/região.
...
**As colunas** de data num armazém de dados, por exemplo, geralmente precisam de atualizações estatísticas frequentes. Cada vez que novas filas são carregadas no armazém de dados, novas datas de carga ou datas de transação são adicionadas. Estes alteram a distribuição de dados e fazem as estatísticas desatualizadas. Inversamente, as estatísticas sobre uma coluna de género numa tabela de clientes podem nunca precisar de ser atualizadas. Assumindo que a distribuição é constante entre os clientes, adicionar novas linhas à variação da tabela não vai alterar a distribuição de dados. No entanto, se o seu armazém de dados contém apenas um sexo e uma nova exigência resulta em múltiplos sexos, então precisa atualizar estatísticas sobre a coluna de género.

Para mais informações, consulte a orientação geral para [as estatísticas.](/sql/relational-databases/statistics/statistics)

## <a name="implementing-statistics-management"></a>Implementação da gestão estatística

Muitas vezes é uma boa ideia alargar o seu processo de carregamento de dados para garantir que as estatísticas são atualizadas no final da carga. A carga de dados é quando as tabelas mudam mais frequentemente o seu tamanho e/ou a sua distribuição de valores. Portanto, este é um lugar lógico para implementar alguns processos de gestão.
...
Para criar estatísticas sobre uma coluna, basta fornecer um nome para o objeto estatístico e o nome da coluna.

Esta sintaxe utiliza todas as opções padrão. Por padrão, o SQL Data Warehouse amostra **20%** da tabela quando cria estatísticas.
...
Para criar um objeto estatístico multi-colunas, basta usar os exemplos anteriores, mas especificar mais colunas.
...
O SQL Data Warehouse não dispõe de um procedimento armazenado pelo sistema equivalente a sp_create_stats no Servidor SQL. Este procedimento armazenado cria um único objeto de estatísticas de colunas em todas as colunas da base de dados que ainda não tem estatísticas.
...
Para criar estatísticas amostradas em todas as colunas da tabela, insira 3, e a amostra por cento. Estes procedimentos usam uma taxa de amostra de 20%.

```sql
EXEC [dbo].[prc_sqldw_create_stats] 3, 20;
```

Para criar estatísticas amostradas em todas as colunas
...
Ao atualizar objetos estatísticos específicos, pode minimizar o tempo e os recursos necessários para gerir as estatísticas. Isto requer algum pensamento para escolher os melhores objetos estatísticos para atualizar.
...
> [!NOTE]
> Ao atualizar todas as estatísticas numa tabela, o SQL Data Warehouse faz uma digitalização para provar a tabela para cada objeto estatístico. Se a tabela for grande e tiver muitas colunas e muitas estatísticas, poderá ser mais eficiente atualizar as estatísticas individuais com base nas necessidades.
...
Para obter a sintaxe completa, consulte [Estatísticas atualizadas](/sql/t-sql/statements/update-statistics-transact-sql).
...
|:--- |:--- |
| [sys.colunas](/sql/relational-databases/system-catalog-views/sys-columns-transact-sql) |Uma linha para cada coluna. |
...
| [sys.schemas](/sql/relational-databases/system-catalog-views/sys-objects-transact-sql) |Uma fila para cada esquema na base de dados. |
| [sys.estatísticas](/sql/relational-databases/system-catalog-views/sys-stats-transact-sql) |Uma linha para cada objeto estatístico. |
| [sys.stats_columns](/sql/relational-databases/system-catalog-views/sys-stats-columns-transact-sql) |Uma linha para cada coluna no objeto estatístico. Liga-se de volta às colunas sys.columns. |
...
|:--- |:--- |
| [STATS_DATE](/sql/t-sql/functions/stats-date-transact-sql) |Data o objeto estatístico foi atualizado pela última vez. |
| [DBCC SHOW_STATISTICS](/sql/t-sql/database-console-commands/dbcc-show-statistics-transact-sql) |Nível resumo e informação detalhada sobre a distribuição de valores, tal como compreendido pelo objeto estatístico. |
...
Os metadados sobre as estatísticas. O histograma exibe a distribuição de valores na primeira coluna-chave do objeto estatístico. O vetor de densidade mede a correlação entre colunas. O SQL Data Warehouse calcula estimativas de cardinalidade com qualquer um dos dados do objeto estatístico.
...
DBCC SHOW_STATISTICS() é mais rigorosamente implementado no Armazém de Dados SQL em comparação com o Servidor SQL:
