...
[Documentação de](/python/api/overview/azure/keyvault-keys-readme?view=azure-python)  |  referência da API [Código fonte da biblioteca](https://github.com/Azure/azure-sdk-for-python/tree/master/sdk/keyvault)  |  [Pacote (Índice de Pacote Python)](https://pypi.org/project/azure-keyvault/)
...
A forma mais simples de autenticar uma aplicação baseada na nuvem é com uma identidade gerida; consulte [Autenticar para Azure Key Vault](../general/authentication.md) para mais detalhes. 
...
A biblioteca de clientes Azure Key Vault para Python permite-lhe gerir chaves e ativos relacionados, tais como certificados e segredos. As amostras de código abaixo mostrar-lhe-ão como criar um cliente, criar uma chave, recuperar uma chave e apagar uma chave.
...
Agora que a sua aplicação é autenticada, pode colocar uma chave no seu keyvault 

```python
rsa_key = client.create_rsa_key(myKey,size=2048)
```

Pode verificar se a chave foi definida com o comando [de teclado az keyvault:](/cli/azure/keyvault/key?view=azure-cli-latest#az-keyvault-key-show)
...
A sua chave está agora guardada como `retrieved_key` .

### <a name="delete-a-key"></a>Eliminar uma chave

Finalmente, vamos apagar a chave do seu cofre

```python
client.delete_key(keyName)
```

Pode verificar se a chave desapareceu com o comando [do teclado az keyvault:](/cli/azure/keyvault/key?view=azure-cli-latest#az-keyvault-key-show)
