...
Antes de começar a codificação que tem de criar alguns recursos, colocar um segredo no seu Cofre de chaves e atribuir permissões.
...
Este exemplo cria um grupo de recursos na localização E.U.A. oeste:
...
O grupo de recursos criado recentemente será usado neste tutorial.

### <a name="create-a-key-vault-and-populate-it-with-a-secret"></a>Criar um cofre de chaves e preenchê-lo com um segredo

Criar um cofre de chaves no seu grupo de recursos, fornecendo a [az keyvault criar](/cli/azure/keyvault?view=azure-cli-latest#az-keyvault-create) comando com as seguintes informações:
...
Agora, adicione um segredo ao Cofre de chaves com o [conjunto de segredos de keyvault de az](/cli/azure/keyvault/secret?view=azure-cli-latest#az-keyvault-secret-set) comando
...
Crie uma máquina virtual utilizando um dos seguintes métodos:
...
Criar uma identidade atribuída de sistema para a máquina virtual com o [atribuir identidade da vm de az](/cli/azure/vm/identity?view=azure-cli-latest#az-vm-identity-assign) comando:
...
Atribuir as permissões de identidade criado anteriormente para a sua chave de cofre com o [az keyvault conjunto-policy](/cli/azure/keyvault?view=azure-cli-latest#az-keyvault-set-policy) comando:
...
Para iniciar sessão para a máquina virtual, siga as instruções em [Connect e inicie sessão na máquina virtual do Azure a executar o Windows](https://docs.microsoft.com/azure/virtual-machines/windows/connect-logon).

## <a name="set-up-the-console-app"></a>Configurar a aplicação de consola

Criar uma aplicação de consola e instalar os pacotes necessários com o `dotnet` comando.
...
### <a name="install-the-packages"></a>Instalar os pacotes
 ...
 Na janela da consola, instale os pacotes de .NET obrigatório neste início rápido:
