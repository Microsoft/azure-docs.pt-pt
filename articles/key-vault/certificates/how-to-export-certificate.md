...
ms.locfileid: "107767912"
---
# <a name="export-certificates-from-azure-key-vault"></a>Certificados de exportação do Azure Key Vault

Saiba como exportar certificados da Azure Key Vault. Pode exportar certificados utilizando o Azure CLI, a Azure PowerShell ou o portal Azure. 
...
Veja [exemplos e definições de parâmetros](/cli/azure/keyvault/certificate#az_keyvault_certificate_download) para obter mais informações.
...
Para obter mais informações, consulte [as definições de parâmetros.](/cli/azure/keyvault/secret#az_keyvault_secret_download)
...
Este comando exporta toda a cadeia de certificados com chave privada (ou seja, o mesmo que foi importado). O certificado está protegido por senha.
Para obter mais informações sobre o comando e parâmetros **Get-AzKeyVaultCertificate,** consulte [Get-AzKeyVaultCertificate - Exemplo 2](/powershell/module/az.keyvault/Get-AzKeyVaultCertificate).
...
Os certificados do Azure App Service são uma forma conveniente de comprar certificados SSL. Pode atribuí-las a Azure Apps a partir do portal. Depois de os importar, os certificados do Serviço de Aplicações estão localizados sob **segredos.**
