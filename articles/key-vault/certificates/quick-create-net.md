...
Começa com a biblioteca de clientes do Certificado Azure Key Vault para .NET. [Azure Key Vault](../general/overview.md) é um serviço de nuvem que fornece uma loja segura para certificados. Pode armazenar com segurança chaves, senhas, certificados e outros certificados. Os cofres de chaves do Azure podem ser criados e geridos através do portal do Azure. Neste arranque rápido, aprende-se a criar, recuperar e apagar certificados de um cofre de chaves Azure utilizando a biblioteca de clientes .NET
...
Neste exemplo, para simplificar, pode utilizar o certificado auto-assinado com a política de emissão por defeito. Para esta tarefa, utilize o método [StartCreateCertificateAsync.](/dotnet/api/azure.security.keyvault.certificates.certificateclient.startcreatecertificateasync) O primeiro parâmetro do método aceita um nome de certificado e a política de[certificados](https://docs.microsoft.com/dotnet/api/azure.security.keyvault.certificates.certificatepolicy).
