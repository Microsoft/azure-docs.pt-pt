...
1. No [portal Azure,](https://portal.azure.com)selecione **Criar uma** > aplicação de**função****computacional** > de recurso .
...
2. Na lâmina **da App função,** introduza os seguintes valores e, em seguida, selecione **OK** para criar a aplicação:
    ...
    |**Conta de Armazenamento**|{autogerado}| A conta de armazenamento de que as Funções Azure precisam para o armazenamento geral.|

3. Na lâmina de aplicações de **funções PacketCaptureExample,** selecione**funções** >**+** **personalizadas** > .
...
1. Selecione **definições** > de aplicativo seletiva**sapre para editor de serviço**de aplicações .
...
#### <a name="azurecredpassword"></a>Palavra-passe do AzureCred

O valor da variável ambiente AzureCredPassword é o valor que obtém ao executar a seguinte amostra PowerShell. Este exemplo é o mesmo que é mostrado na secção de **credenciais encriptadas** anteriores. O valor que é necessário é `$Encryptedpassword` a produção da variável.  Esta é a palavra-passe principal do serviço que encriptou utilizando o script PowerShell.
...
1. Vá ao aplicativo de funções. Em seguida, selecione definições > de **aplicação de função****Configurar definições**de aplicações .
