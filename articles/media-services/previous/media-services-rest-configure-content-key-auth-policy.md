---
title: Configure uma política de autorização de chaves de conteúdo com o REST - Azure / Microsoft Docs
description: Saiba como configurar uma política de autorização para uma chave de conteúdo utilizando a API REST Dos Serviços de Media.
...
ms.locfileid: "76773702"
---
# <a name="dynamic-encryption-configure-a-content-key-authorization-policy"></a>Encriptação dinâmica: Configure uma política de autorização de chave de conteúdo  
[!INCLUDE [media-services-selector-content-key-auth-policy](../../../includes/media-services-selector-content-key-auth-policy.md)]

## <a name="overview"></a>Visão geral
 Pode utilizar o Azure Media Services para fornecer o seu conteúdo encriptado (dinamicamente) com o Advanced Encryption Standard (AES) utilizando chaves de encriptação de 128 bits e a gestão de direitos digitais PlayReady ou Widevine (DRM). A Media Services também fornece um serviço para a entrega de chaves e licenças PlayReady/Widevine a clientes autorizados.
...
- [Integre uma aplicação baseada em Azure Media Services OWIN MVC com o Azure Ative Directory e restringa a entrega da chave de conteúdo com base em alegações da JWT](http://www.gtrifonov.com/2015/01/24/mvc-owin-azure-media-services-ad-integration/)

### <a name="some-considerations-apply"></a>Algumas considerações aplicam-se
* Para utilizar embalagens dinâmicas e encriptação dinâmica, certifique-se de que o ponto final de streaming a partir do qual pretende transmitir o seu conteúdo encontra-se no estado "Running".
...
* O ponto final de streaming dos Media Services define o valor do cabeçalho CORS Access-Control-Allow-Origin em resposta pré-voo como o wildcard "\*." Este valor funciona bem com a maioria dos jogadores, incluindo Azure Media Player, Roku e JWPlayer, entre outros. No entanto, alguns jogadores que usam dash.js não funcionam porque, com o modo de credenciais definido para "incluir", XMLHttpRequest no seu dash.js não permite que o wildcard "\*" como o valor do Access-Control-Allow-Origin. Como suposições para esta limitação em dash.js, se hospedar o seu cliente a partir de um único domínio, a Media Services pode especificar esse domínio no cabeçalho de resposta pré-voo. Para assistência, abra um bilhete de apoio através do portal Azure.

## <a name="aes-128-dynamic-encryption"></a>Encriptação dinâmica AES-128
> [!NOTE]
> Quando trabalha com a Media Services REST API, aplicam-se as seguintes considerações.
> 
> Quando acede a entidades em Serviços de Media, deve definir campos e valores específicos nos seus pedidos HTTP. Para mais informações, consulte [Configuração para media services REST API desenvolvimento](media-services-rest-how-to-use.md).
> 
> 
> 
...
#### <a id="ContentKeyAuthorizationPolicies"></a>Criar políticas de autorização de conteúdo
Pedido:
...
#### <a id="AddAuthorizationPolicyToKey"></a>Adicione uma política de autorização à chave de conteúdo
Pedido:
...
O exemplo seguinte cria uma política de autorização com uma restrição simbólica. Neste exemplo, o cliente deve apresentar um símbolo que contenha a chave de assinatura (VerificationKey), um emitente simbólico e reclamações necessárias.

### <a name="create-contentkeyauthorizationpolicies"></a>Criar políticas de autorização de conteúdo
Crie uma política de restrição simbólica, como mostra a secção " Criar Políticas de Autorização de[Conteúdochave](#ContentKeyAuthorizationPolicies)de Conteúdo ."

### <a name="create-contentkeyauthorizationpolicyoptions"></a>Criar opções de política de autorização de conteúdo
Pedido:
...
Link ContentKeyAuthorizationPolicies com opções, como mostrado na secção " Criar Políticas de[Autorização de ConteúdoChave](#ContentKeyAuthorizationPolicies)."

#### <a name="add-an-authorization-policy-to-the-content-key"></a>Adicione uma política de autorização à chave de conteúdo
Adicionar Política de Autorização ao ContentKey, como mostrado na secção " Adicionar uma política de[autorização à chave de conteúdo](#AddAuthorizationPolicyToKey)."
...
#### <a id="ContentKeyAuthorizationPolicies2"></a>Criar políticas de autorização de conteúdo
Pedido:
...
### <a name="token-restriction"></a>Restrição simbólica
Para configurar a opção de restrição simbólica, é necessário utilizar um XML para descrever os requisitos de autorização do token. A configuração de restrição simbólica XML deve estar em conformidade com o esquema XML mostrado na secção " Esquema de[restrição token](#schema)."

#### <a name="create-contentkeyauthorizationpolicies"></a>Criar políticas de autorização de conteúdo
Criar Políticas de Autorização de Autorização de Conteúdo, como mostrado na secção " Criar Políticas de[Autorização de Chave de Conteúdo](#ContentKeyAuthorizationPolicies2)."
...
> [!NOTE]
> A restrição IP às políticas de autorização chave de conteúdo ainda não está disponível no serviço.
...
Agora que configuraa a política de autorização de uma chave de conteúdo, consulte a política de entrega de [ativos configurar](media-services-rest-configure-asset-delivery-policy.md).
