...
* Familiarize-se com a proteção de conteúdos online para fairPlay. Para mais informações, consulte os seguintes artigos e amostras:
...
## <a name="configuration-in-media-services"></a>Configuração em Serviços de Media
Para a configuração do modo offline FPS através do [Media Services .NET SDK,](https://www.nuget.org/packages/windowsazure.mediaservices)utilize a versão SDK .NET SDK 4.0.0.4 ou posterior, que fornece a API necessária para configurar o modo offline FPS.
...
A primeira alteração é definir um "modo offline de ativação" Boolean, chamado objDRMSettings.EnableOfflineMode, que é verdade quando permite o cenário de DRM offline. Dependendo deste indicador, faça a seguinte alteração para a configuração FairPlay:
...
A segunda alteração é adicionar a terceira chave no Dicionário<AssetDeliveryPolicyConfigurationKey,> de cadeias.
Adicione o AssetDeliveryPolicyConfigurationKey como mostrado aqui:
...
Após este passo, o <Dictionary_AssetDeliveryPolicyConfigurationKey> cadeia na política de entrega de ativos do FPS contém as seguintes três entradas:

* AssetDeliveryPolicyConfigurationKey.FairPlayBaseAcquisitionUrl ou AssetDeliveryPolicyConfigurationKey.FairPlayAcquisitionUrl, dependendo de fatores como o servidor fpS KSM/key utilizado e se reutiliza a mesma política de entrega de ativos em vários ativos
* AssetDeliveryPolicyConfigurationKey.CommonEncryptionIVForCbcs
* Chave de configuração de política de entrega de ativos.permitir licença persistente
...
* Código da amostra: amostra HLSCatalog para o modo offline FPS na versão SDK do Servidor de Streaming \FairPlay 3.1\Development\Cliente\HLSCatalog_With_FPS\HLSCatalog\. Na aplicação de amostraHLSCatalog, os seguintes ficheiros de código são utilizados para implementar funcionalidades de modo offline:
...
![Streams de aplicações offline FairPlay iOS](media/media-services-protect-hls-with-offline-FairPlay/media-services-offline-FairPlay-ios-app-streams.png)
...
* Configuração de proteção de conteúdos dos Media Services através dos Media Services .NET API configura a encriptação dinâmica FairPlay e a entrega da licença FairPlay em Media Services.
* Um leitor de iOS baseado na amostra do FPS Server SDK configura um leitor de iOS que pode reproduzir conteúdo FPS em modo de streaming online ou no modo offline.
* Os vídeos FPS da amostra são usados para testar o modo offline e o streaming online.
* Uma FAQ responde a perguntas sobre o modo offline fpS.
