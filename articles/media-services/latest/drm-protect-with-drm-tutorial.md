...
Pode utilizar os Serviços de Multimédia do Azure para entregar transmissões em fluxo encriptadas com licenças do Microsoft PlayReady, Google Widevine ou Apple FairPlay. Para obter uma explicação aprofundada, consulte [a proteção de conteúdo com encriptação dinâmica](drm-content-protection-concept.md).
...
![AMS com vídeo protegido pela DRM no Azure Media Player](./media/drm-protect-with-drm-tutorial/ams_player.png)
...
* Reveja o artigo [Descrição Geral da Proteção de Conteúdo](drm-content-protection-concept.md).
* Reveja o [sistema de proteção de conteúdos multi-DRM design com controlo de acesso.](architecture-design-multi-drm-system.md)
...
Conforme mencionado acima, o objeto **Transformação** é a receita e uma [Tarefa](transform-jobs-concept.md) é o pedido real para os Serviços de Multimédia aplicarem essa **Transformação** a um determinado conteúdo de áudio ou vídeo de entrada. O **Trabalho** especifica informações como a localização do vídeo de entrada e a localização para a saída.
...
* Configuração
    ...
    As licenças do [PlayReady](drm-playready-license-template-concept.md) e [Widevine](drm-widevine-license-template-concept.md) estão configuradas para que possam ser fornecidas pelo serviço de entrega de licenças dos Serviços de Multimédia. Mesmo que esta aplicação de amostra não configuure a licença [FairPlay,](drm-fairplay-license-overview.md) contém um método que pode usar para configurar o FairPlay. Pode adicionar a configuração FairPlay como outra opção.
...
1. Criar um [localizador de streaming.](stream-streaming-locators-concept.md)
