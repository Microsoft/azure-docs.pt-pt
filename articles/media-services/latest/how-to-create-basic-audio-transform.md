---
title: Criar uma transformação áudio básica
description: Crie uma transformação áudio básica utilizando a API dos Media Services.
services: media-services
documentationcenter: ''
author: IngridAtMicrosoft
manager: femila
editor: ''
ms.assetid: ''
ms.service: media-services
ms.devlang: multiple
ms.topic: how-to
ms.tgt_pltfrm: multiple
ms.workload: na
ms.date: 11/18/2020
ms.author: inhenkel
ms.openlocfilehash: 80edac3106a79a706ee1ef18c48d41d1ad6c3eaf
ms.sourcegitcommit: f28ebb95ae9aaaff3f87d8388a09b41e0b3445b5
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 03/29/2021
ms.locfileid: "101724596"
---
# <a name="create-a-basic-audio-transform"></a>Criar uma transformação áudio básica

[!INCLUDE [media services api v3 logo](./includes/v3-hr.md)]

Este artigo mostra como criar uma transformação áudio básica.  O modo básico foi adicionado ao `#Microsoft.Media.AudioAnalyzerPreset` lançamento para 2020-05-01.

## <a name="prerequisites"></a>Pré-requisitos

Siga os passos na [Criar uma conta de Serviços](./create-account-howto.md) de Comunicação para criar a conta e grupo de recursos necessários para criar um ativo.

## <a name="methods"></a>Métodos

### <a name="using-the-rest-api"></a>Utilizar a API REST

[!INCLUDE [media-services-cli-instructions.md](./includes/task-create-basic-audio-rest.md)]

## <a name="next-steps"></a>Passos seguintes

[!INCLUDE [transforms next steps](./includes/transforms-next-steps.md)]
