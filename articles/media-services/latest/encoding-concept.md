...
Este tópico fornece orientação sobre como codificar seu conteúdo com os serviços de mídia v3.
...
Ao codificar com os serviços de mídia, você usa predefinições para informar ao codificador como os arquivos de mídia de entrada devem ser processados. Por exemplo, você pode especificar a resolução de vídeo e/ou o número de canais de áudio que deseja no conteúdo codificado.

Você pode começar rapidamente com uma das predefinições internas recomendadas com base nas práticas recomendadas do setor ou pode optar por criar uma predefinição personalizada para direcionar seus requisitos específicos de cenário ou dispositivo. Para obter mais informações, consulte [codificar com uma transformação personalizada](customize-encoder-presets-how-to.md).
...
Atualmente, há suporte para as seguintes predefinições:
...
- **EncoderNamedPreset. H264MultipleBitrate1080p**: produz um conjunto de oito arquivos MP4 alinhados a GOP, variando de 6000 kbps a 400 Kbps e áudio AAC estéreo. A resolução começa em 1080p e fica inativa para 360p.
- **EncoderNamedPreset. H264MultipleBitrate720p**: produz um conjunto de seis arquivos MP4 alinhados a GOP, variando de 3400 kbps a 400 Kbps e áudio AAC estéreo. A resolução começa em 720p e desce para 360p.
- **EncoderNamedPreset. H264MultipleBitrateSD**: produz um conjunto de cinco arquivos MP4 alinhados a GOP, variando de 1600 kbps a 400 Kbps e áudio AAC estéreo. A resolução começa em 480p e desce para 360p.
...
[StandardEncoderPreset](https://docs.microsoft.com/rest/api/media/transforms/createorupdate#standardencoderpreset) descreve as configurações a serem usadas ao codificar o vídeo de entrada com o codificador padrão. Use essa predefinição ao personalizar predefinições de transformação.
...
Os serviços de mídia dão suporte total à personalização de todos os valores em predefinições para atender às suas necessidades e requisitos de codificação específicos. Para obter exemplos que mostram como personalizar as predefinições do codificador, consulte a lista abaixo:
...
## <a name="scaling-encoding-in-v3"></a>Dimensionando a codificação em v3
