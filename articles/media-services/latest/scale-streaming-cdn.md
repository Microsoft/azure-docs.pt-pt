---
title: Streaming de escala com cdn
titleSuffix: Azure Media Services
description: Conheça um serviço de streaming que entrega conteúdo diretamente a uma aplicação de jogador de clientes ou a uma Rede de Entrega de Conteúdos (CDN).
...
Ao decidir se permite ou não a CDN no [ponto final](streaming-endpoint-concept.md)de streaming dos Serviços de Media, considere o número de espectadores antecipados. O CDN só ajuda se estiver a antecipar muitos espectadores para o seu conteúdo. Se a moeda máxima do espectador for inferior a 500, é recomendado desativar o CDN uma vez que a CDN melhor se baseia em moeda. 
...
* O [ponto final](streaming-endpoint-concept.md) de streaming `hostname` e o URL de streaming permanecem os mesmos, quer ative ou não o CDN.
...
Pode determinar se a alteração do DNS foi feita num ponto final de streaming (o tráfego está a ser direcionado para o Azure CDN) utilizando https://www.digwebinterface.com. Se vir azureedge.net nomes de domínio nos resultados, o tráfego está agora a ser apontado para o CDN.

## <a name="origin-assist-cdn-prefetch"></a>Assistência de Origem CDN-Prefetch

O cache cDN é um processo reativo. Se a CDN puder prever o próximo objeto que será solicitado, a CDN pode solicitar e cache o próximo objeto. Com este processo, você pode obter um cache-hit para todos (ou a maioria) dos objetos, assim melhorando o desempenho.
...
Para atingir este objetivo, um ponto final de streaming (origem) e CDN precisam de trabalhar de mãos dadas: 
...
- A Prefetch melhora a qualidade da reprodução de vídeo: pré-posicionamento dos segmentos de vídeo antecipados na borda durante a reprodução, reduzindo a latência ao espectador e melhorando os tempos de descarregamento do segmento de vídeo. Isto resulta num tempo de arranque de vídeo mais rápido e em ocorrências de retamponamento mais baixas.
- Este conceito é aplicável ao cenário geral de origem CDN, não se limitando aos meios de comunicação.
...
> [!NOTE]
> Esta funcionalidade ainda não se aplica ao Akamai CDN integrado com o ponto final de streaming dos Media Services. No entanto, está disponível para clientes da Media Services que tenham um contrato pré-existente da Akamai e que exijam integração personalizada entre a Akamai CDN e a origem dos Media Services.

### <a name="how-it-works"></a>Como funciona

O suporte da CDN para os cabeçalhos *CDN-Prefetch* (para streaming ao vivo e vídeo a pedido) está disponível para clientes que tenham contrato direto com a Akamai CDN. A funcionalidade envolve as seguintes trocas de cabeçalhos HTTP entre a Akamai CDN e a origem dos Serviços de Media:
...
A funcionalidade *Origin-Assist CDN-Prefetch* suporta os seguintes protocolos de streaming para streaming ao vivo e a pedido:
...
* Suponha que tenha um vídeo a pedido. Se a CDN-prefetch estiver ativada, esta funcionalidade implica que uma vez que um cliente solicite o primeiro segmento de vídeo, a prefetch iniciará um loop para pré-rebuscar todos os segmentos de vídeo subsequentes no mesmo bitrate? 
    ...
    Não, a CDN-prefetch só é realizada após um pedido/resposta iniciado pelo cliente. A CDN-prefetch nunca é acionada por uma pré-fetch, para evitar um loop de pré-fetch. 
* A funcionalidade CDN-Prefetch de Assistência de Origem está sempre ligado? Como pode ser ligado/desligado? 

    Por predefinição, esta funcionalidade está desativada. Os clientes precisam ligá-lo via Akamai API.
* Para streaming ao vivo, o que aconteceria ao Origin-Assist se o próximo segmento ou fragmento ainda não estiver disponível? 
    ...
    Neste caso, a origem dos Serviços de Media não fornecerá cabeçalho CDN-Origin-Assist-Prefetch-Path e CDN-prefetch não ocorrerá.
* Como funciona o *CDN-Prefetch de Assistência de Origem* com filtros de manifesto dinâmicos? 

    Esta função funciona independentemente do filtro manifesto. Quando o próximo fragmento sair de uma janela de filtro, o seu URL ainda será localizado olhando para o manifesto bruto do cliente, e depois devolvido como cabeçalho de resposta prefetch CDN. Isto significa que o CDN obterá o URL de um fragmento que é filtrado do manifesto DASH/HLS/Smooth. No entanto, o jogador nunca fará um pedido get à CDN para obter esse fragmento, porque esse fragmento não está incluído no manifesto DASH/HLS/Smooth detido pelo leitor (o jogador não sabe a existência desse fragmento).
* A lista de reprodução DASH MPD/HLS/Manifesto liso pode ser prebuscada?

    Não, DASH MPD, lista de reprodução master HLS, lista de reprodução de variantes HLS ou URL de manifesto liso não é adicionado ao cabeçalho pré-fetch.
* Os URLs de prefetch são relativos ou absolutos? 

    Embora a Akamai CDN permita ambos, a origem dos Serviços de Media apenas fornece URLs relativos para o caminho de pré-busca porque não há nenhum benefício aparente na utilização de URLs absolutos.
