...
Os valores possíveis são: `people` `animatedCharacters` , `keywords` e `labels` `sentiments` `emotions` `topics` `keyframes` `transcript` `ocr` `speakers` `scenes` `namedEntities` .
