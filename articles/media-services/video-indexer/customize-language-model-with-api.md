...
O [comboio um modelo de idioma](https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Train-Language-Model) API treina um modelo de linguagem personalizado na conta especificada com o conteúdo nos ficheiros que foram carregados e habilitados no modelo de idioma.
...
O devolvido `id` é um ID único usado para distinguir entre modelos linguísticos, enquanto `languageModelId` é usado tanto para carregar um vídeo para [indexar](https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Upload-Video) e [reindexar um APIs de vídeo](https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Re-Index-Video) (também conhecido como em Video `linguisticModelId` Indexer upload/REINDex APIs).

## <a name="delete-a-language-model"></a>Excluir um modelo de linguagem

A [API de um modelo de idioma](https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Delete-Language-Model) elimina um modelo de linguagem personalizado da conta especificada. Qualquer vídeo que estivesse a usar o modelo linguagem apagado manterá o mesmo índice até que reindex o vídeo. Se voltar a usar o vídeo, pode atribuir um novo modelo de Linguagem ao vídeo. Caso contrário, o Video Indexer utilizará o seu modelo padrão para reindexar o vídeo.
...
A [atualização de um modelo de Idioma](https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Update-Language-Model) API atualiza um modelo personalizado de pessoa linguística na conta especificada.
...
A [atualização de um ficheiro](https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Update-Language-Model-file) permite-lhe atualizar o nome e o estado de um ficheiro num modelo de Linguagem personalizado na conta `enable` especificada.
...
A API [obtém](https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Get-Language-Model) informações sobre o modelo linguístico especificado na conta especificada, como o idioma e os ficheiros que estão no modelo idioma.
...
A [API obtém todos os](https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Get-Language-Models) modelos de Linguagem personalizados na conta especificada numa lista.
...
A API [de eliminação](https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Delete-Language-Model-File) elimina o ficheiro especificado do modelo de Linguagem especificado na conta especificada.
...
A [obter metadados de um ficheiro](https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Get-Language-Model-File-Data) API devolve o conteúdo e os metadados no ficheiro especificado a partir do modelo idioma escolhido na sua conta.
