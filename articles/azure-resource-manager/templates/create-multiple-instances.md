...
Ambos os usos são descritos mais detalhadamente neste artigo. Para obter um tutorial, consulte [tutorial: criar várias instâncias de recursos usando modelos do Resource Manager](template-tutorial-create-multiple-instances.md).
...
Para criar mais de um valor para uma propriedade em um recurso, adicione um `copy` matriz no elemento Properties. Essa matriz contém objetos e cada objeto tem as seguintes propriedades:
...
O Resource Manager expande a matriz de `copy` durante a implantação. O nome da matriz se torna o nome da propriedade. Os valores de entrada se tornam as propriedades do objeto. O modelo implantado se torna:
...
Para criar várias instâncias de uma variável, use a propriedade `copy` na seção variáveis. Você cria uma matriz de elementos construídos com base no valor na propriedade `input`. Você pode usar a propriedade `copy` dentro de uma variável ou no nível superior da seção de variáveis. Ao usar `copyIndex` dentro de uma iteração variável, você deve fornecer o nome da iteração.

> [!NOTE]
> A iteração variável também suporta um argumento compensado. A contrapartida deve ser seguida do nome da iteração, como o copyIndex ('diskNames', 1).
>
...
Para estabelecer uma relação pai/filho com uma instância do data factory, forneça um nome para o conjunto de dados que inclui o nome do recurso pai. Utilize o formato: `{parent-resource-name}/{child-resource-name}`.
...
* Para percorrer um tutorial, consulte [tutorial: criar várias instâncias de recursos usando modelos do Resource Manager](template-tutorial-create-multiple-instances.md).

* Se você quiser saber mais sobre as seções de um modelo, consulte [criando modelos de Azure Resource Manager](template-syntax.md).
* Para saber como implantar seu modelo, consulte [implantar um aplicativo com Azure Resource Manager modelo](deploy-powershell.md).
