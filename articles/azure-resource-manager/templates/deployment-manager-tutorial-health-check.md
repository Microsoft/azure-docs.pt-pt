...
Para completar este tutorial é necessário:
...
Se ainda não descarregou as amostras utilizadas no tutorial pré-requisito, pode descarregar [os modelos e os artefactos](https://github.com/Azure/azure-docs-json-samples/raw/master/tutorial-adm/ADMTutorial.zip) e desapertá-lo localmente. Em seguida, executar o script PowerShell a partir da secção tutorial pré-requisito [Prepare os artefactos](./deployment-manager-tutorial.md#prepare-the-artifacts). O script cria um grupo de recursos, cria um recipiente de armazenamento, cria um recipiente blob, carrega os ficheiros descarregados e, em seguida, cria um token SAS.

* Faça uma cópia do URL com ficha SAS. Este URL é necessário para povoar um campo nos dois ficheiros de parâmetros: ficheiro de parâmetros de topologia e ficheiro de parâmetros de lançamento.
* Abra _CreateADMServiceTopology.Parameters.js_ e atualize os valores de `projectName` e `artifactSourceSASLocation` .
* Abra _CreateADMRollout.Parameters.js_ e atualize os valores de `projectName` e `artifactSourceSASLocation` .
...
Para implantar a função Azure, selecione **Experimente-a** para abrir a Concha da Nuvem Azure e, em seguida, cole o seguinte script na janela do invólucro. Para colar o código, clique com o botão direito na janela da concha e, em seguida, **selecione Pasta**.
...
1. Para testar o simulador de monitorização de saúde, abra os URLs que criou no passo anterior. Os resultados para o estado insalubre serão semelhantes a:
...
1. Abra _CreateADMRollout.jssobre_ o que criou no Use [Azure Deployment Manager com modelos de Gestor de Recursos](./deployment-manager-tutorial.md). Este ficheiro JSON faz parte do download.  Veja [Pré-requisitos](#prerequisites).
...
1. Atualizar a `dependsOn` definição de lançamento para incluir o novo passo de verificação de saúde definido:
...
1. Atualização `stepGroups` para incluir o passo de verificação de saúde. `healthCheckStep`Chama-se `postDeploymentSteps` `stepGroup2` de. `stepGroup3`e `stepGroup4` só são implantados se o estado saudável for *saudável* ou *avisado.*
    ...
    Se comparar a `stepGroup3` secção antes e depois de revista, esta secção depende agora de `stepGroup2` . Isto é necessário quando `stepGroup3` os grupos de passos subsequentes dependem dos resultados da monitorização da saúde.
...
Executar o seguinte script PowerShell para implantar a topologia. Você precisa do mesmo _CreateADMServiceTopology.jse_ _CreateADMServiceTopology.Parameters.jsem_ que usou no [Use Azure Deployment Manager com modelos de Gestor de Recursos](./deployment-manager-tutorial.md).
...
Utilize o URL de estado pouco saudável que criou na [Criar um simulador de serviço de verificação de saúde](#create-a-health-check-service-simulator). Você precisa doCreateADMServiceTopology.jsrevisto _e_ o mesmo _CreateADMServiceTopology.Parameters.jsem_ que usou no Use [Azure Deployment Manager com modelos de Gestor de Recursos](./deployment-manager-tutorial.md).
...
Para verificar o progresso do lançamento, utilize o seguinte script PowerShell:
...
Depois de concluído o lançamento, verá um grupo de recursos adicional criado para os EUA Ocidentais.

## <a name="deploy-the-rollout-with-the-healthy-status"></a>Implementar o lançamento com o estado saudável

Repita esta secção para recolocar o lançamento com o URL de estado saudável. Depois do lançamento estar concluído, verá mais um grupo de recursos criado para os EUA Orientais.

## <a name="verify-the-deployment"></a>Verificar a implementação

1. Abra o [portal do Azure](https://portal.azure.com).
1. Navegue para as novas aplicações web sob os novos grupos de recursos criados pela implementação do lançamento.
