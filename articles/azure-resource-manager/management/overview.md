...
Para saber mais sobre modelos de Azure Resource Manager, confira [implantação de modelo visão geral](../templates/overview.md).
...
* **provedor de recursos** -um serviço que fornece recursos do Azure. Por exemplo, um provedor de recursos comum é Microsoft. Compute, que fornece o recurso de máquina virtual. Microsoft. Storage é outro provedor de recursos comum. Consulte [provedores de recursos e tipos](resource-providers-and-types.md).
* **Modelo do Resource Manager** – um arquivo JavaScript Object Notation (JSON) que define um ou mais recursos a serem implantados em um grupo de recursos ou assinatura. O modelo pode ser utilizado para implementar os recursos de forma consistente e repetida. Consulte [implantação de modelo visão geral](../templates/overview.md).
* **sintaxe declarativa** - sintaxe que permite afirmar "Aqui está o que pretendo criar" sem ter de escrever a sequência de comandos de programação para criá-la. O modelo do Resource Manager é um exemplo de sintaxe declarativa. No ficheiro, é possível definir as propriedades da infraestrutura de implementação para o Azure.  Consulte [implantação de modelo visão geral](../templates/overview.md).
...
* Para saber mais sobre como mover recursos, confira [mover recursos para um novo grupo de recursos ou assinatura](move-resource-group-and-subscription.md).

* Para saber mais sobre como marcar recursos, confira [usar marcas para organizar os recursos do Azure](tag-resources.md).

* Para saber mais sobre como bloquear recursos, consulte [Bloquear recursos para evitar alterações inesperadas](lock-resources.md).

* Para obter informações sobre como criar modelos para implantações, consulte [implantação de modelo visão geral](../templates/overview.md).
