---
title: Criar uma avaliação Azure SQL
description: Saiba como avaliar os casos sql para migração para Azure SQL Gerenciado Instância e Azure SQL Database
author: rashi-ms
ms.author: rajosh
ms.topic: tutorial
ms.date: 02/07/2021
...
- Certifique-se de que [criou](./create-manage-projects.md) um projeto Azure Migrate e que a ferramenta Azure Migrate: Discovery and assessment tool adicionada.
- Para criar uma avaliação, é necessário configurar um aparelho Azure Migrate para [VMware](how-to-set-up-appliance-vmware.md). O aparelho descobre servidores no local e envia metadados e dados de desempenho para a Azure Migrate. [Saiba mais](migrate-appliance.md)

## <a name="azure-sql-assessment-overview"></a>Visão geral da avaliação do Azure SQL
Pode criar uma avaliação Azure SQL com critérios de dimensionamento como **baseados no desempenho**. 

**Critérios de dimensionamento** | **Detalhes** | **Dados**
--- | --- | ---
**Com base no desempenho** | Avaliações com base em dados de desempenho recolhidos | A **configuração Azure SQL** recomendada baseia-se em dados de desempenho de instâncias e bases de dados SQL, que incluem utilização de CPU, contagem de núcleos, organizações de ficheiros de base de dados e tamanhos, IOs de ficheiros, consulta de lote por segundo e tamanho de memória e utilização por cada uma das bases de dados.

[Saiba mais](concepts-azure-sql-assessment-calculation.md) sobre as avaliações do Azure SQL.
...
1. Reveja o tipo de implementação recomendado para a instância SQL que é determinada de acordo com a matriz abaixo:
    ...
    - **Tipo de implantação de destino** (selecionado em propriedades de avaliação): **Recomendado**
        ...
        **Prontidão Azure SQL DB** | **Prontidão Azure SQL MI** | **Tipo de implantação recomendado** | **Configuração Azure SQL e estimativas de custos calculadas?**
        ...
        Não está pronto. | Não está pronto. | Potencialmente pronto para Azure VM [Saiba mais](concepts-azure-sql-assessment-calculation.md#potentially-ready-for-azure-vm) | Não
