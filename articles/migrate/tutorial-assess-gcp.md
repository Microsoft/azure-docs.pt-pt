---
title: Avaliar os casos de VM GCP para a migração para Azure com Azure Migrate
description: Descreve como avaliar os casos de VM GCP para migração para Azure usando Azure Migrate.
author: rashi-ms
ms.author: rajosh
ms.manager: abhemraj
ms.topic: tutorial
...
Como parte da sua viagem de migração para Azure, você avalia as suas cargas de trabalho no local para determinar a prontidão na nuvem, identificar riscos e estimar custos e complexidade.

Este artigo mostra-lhe como avaliar os exemplos de VM da Plataforma cloud do Google (GCP) para migração para Azure, utilizando a ferramenta Azure Migrate: Discovery and assessment.
...
- Antes de seguir os passos neste tutorial, complete o primeiro tutorial desta série para [descobrir o seu inventário no local.](tutorial-discover-gcp.md) 


## <a name="decide-which-assessment-to-run"></a>Decidir qual a avaliação a executar

Decida se pretende executar uma avaliação utilizando critérios de dimensionamento baseados em dados de configuração do servidor/metadados recolhidos como-está no local ou com base em dados de desempenho.
...
**Com base no desempenho** | Avaliar com base em dados de desempenho recolhidos. | O tamanho Azure VM recomendado baseia-se em dados de CPU e utilização da memória.<br/><br/> O tipo de disco recomendado baseia-se no IOPS e na produção dos discos no local.
