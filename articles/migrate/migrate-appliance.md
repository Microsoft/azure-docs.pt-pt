...
Detalhes de pasta por servidor | ((Pasta)recipiente). ChildEntity.Type
Detalhes do Datacenter por servidor | ((Datacenter)recipiente). VmFolder
...
Detalhes do anfitrião por servidor | ((HostSystem)container). VM

### <a name="performance-data"></a>Dados de desempenho

Aqui estão os dados de desempenho que um aparelho recolhe para um servidor em execução em VMware e envia para a Azure.

**Dados** | **Contador** | **Impacto da avaliação**
--- | --- | ---
Utilização da CPU | cpu.usage.average | Tamanho/custo do servidor recomendado
Utilização da memória | mem.usage.média | Tamanho/custo do servidor recomendado
Produção de leitura de disco (MB por segundo) | virtualDisk.read.average | Cálculo para o tamanho do disco, custo de armazenamento, tamanho do servidor
Disco escreve produção (MB por segundo) | virtualDisk.write.average | Cálculo para o tamanho do disco, custo de armazenamento, tamanho do servidor
Operações de leitura de disco por segundo | virtualDisk.numberReadAveraged.average | Cálculo para o tamanho do disco, custo de armazenamento, tamanho do servidor
Disco escreve operações por segundo | virtualDisk.numberWriteAveraged.average  | Cálculo para o tamanho do disco, custo de armazenamento, tamanho do servidor
Produção de leitura de NIC (MB por segundo) | net.received.average | Cálculo para o tamanho do servidor
NIC escreve produção (MB por segundo) | net.transmitted.average  |Cálculo para o tamanho do servidor

### <a name="installed-software-inventory"></a>Inventário de software instalado

O aparelho recolhe dados sobre o inventário de software instalado nos servidores.

#### <a name="windows-server-software-inventory-data"></a>Dados de inventário de software do servidor do Windows

Aqui estão os dados de inventário de software que o aparelho recolhe de cada servidor Windows descoberto no seu ambiente VMware.
...
#### <a name="linux-server-software-inventory-data"></a>Dados de inventário de software de servidor linux

Aqui estão os dados de inventário de software que o aparelho recolhe de cada servidor Linux descoberto no seu ambiente VMware. Com base no sistema operativo do servidor, um ou mais dos comandos são executados.
...
Estado de potência do servidor | Msvm_ComputerSystem | EnabledState
**Por detalhes de disco** |
...
--- | --- | ---
Processador virtual hiper-V do hiperconselhante | % Tempo de execução do hóspede | Tamanho/custo do servidor recomendado
Servidor de memória dinâmica Hiper-V | Pressão corrente (%)<br/> Memória Física Visível do Hóspede (MB) | Tamanho/custo do servidor recomendado
Dispositivo de armazenamento virtual hiper-V | Ler Bytes/Segundo | Cálculo para o tamanho do disco, custo de armazenamento, tamanho do servidor
Dispositivo de armazenamento virtual hiper-V | Escrever Bytes/Segundo | Cálculo para o tamanho do disco, custo de armazenamento, tamanho do servidor
Adaptador de rede virtual Hiper-V | Bytes Recebidos/Segundo | Cálculo para o tamanho do servidor
Adaptador de rede virtual Hiper-V | Bytes Enviados/Segundo | Cálculo para o tamanho do servidor

- A utilização do CPU é a soma de toda a utilização, para todos os processadores virtuais ligados a um servidor.
...
| Contagem de discos | fdisk -l \| egrep 'Disco. \* bytes' \| awk '{print $2}' \| corte -f1 -d ':' |
...
- No aparelho, nos   >  **Programas e Funcionalidades** do Painel de Controlo .
