...
**Deteção** | O dispositivo de migrações para Azure pode descobrir até 10.000 VMs VMware em um vCenter Server.
...
**Suporte** | **Detalhes**
--- | ---
**Deteção** | A descoberta é sem agente, usando credenciais de convidado do computador e acessando remotamente computadores usando chamadas de WMI e SSH.
...
**Permissões (descoberta de aplicativo)** | conta vCenter Server com acesso apenas para leitura, e privilégios habilitados para **máquinas virtuais > Operações de Hóspedes**.
...
Esta opção encontra-se atualmente em pré-visualização. [Saiba mais](how-to-create-group-machine-dependencies-agentless.md). Os requisitos são resumidos na tabela a seguir.
...
**Conta do Linux** | A visualização precisa de uma conta de usuário com privilégio de raiz.<br/><br/> Alternadamente, a conta de utilizador necessita destas permissões em ficheiros /bin/netstat e /bin/ls: CAP_DAC_READ_SEARCH e CAP_SYS_PTRACE.
