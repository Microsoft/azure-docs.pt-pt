...
1. Formatar a sua mensagem de entrada com um envelope XML. Por exemplo, consulte o seguinte exemplo de carga útil XML:
...
##### <a name="the-requestcontext-on-the-ireplychannel-was-closed-without-a-reply-being-sent"></a>O PedidoContexto no IReplyChannel foi encerrado sem que fosse enviada uma resposta

Esta mensagem de erro significa que falhas inesperadas acontecem quando o manipulador de catch-all para o canal termina o canal devido a um erro e reconstrói o canal para processar outras mensagens.

Para reconhecer que a sua aplicação lógica recebeu o IDoc, [adicione uma ação de Resposta](../connectors/connectors-native-reqres.md#add-a-response-action) que devolva um código de `200 OK` estado. Deixe o corpo vazio e não mude ou adicione aos cabeçalhos. O IDoc é transportado através do tRFC, o que não permite uma carga útil de resposta.

Para rejeitar o IDoc, responda com qualquer código de estado HTTP que não `200 OK` seja . Em seguida, o adaptador SAP devolve uma exceção à SAP em seu nome. Só deve rejeitar o IDoc para sinalizar erros de transporte de volta ao SAP, como um IDoc mal rectado que a sua aplicação não pode processar. Não deve rejeitar um IDoc por erros de nível de aplicação, tais como problemas com os dados contidos no IDoc. Se atrasar a aceitação do transporte para validação do nível de aplicação, poderá sofrer um desempenho negativo devido ao bloqueio da sua ligação ao transporte de outros IDocs.

Se estiver a receber esta mensagem de erro e experimentar falhas sistémicas a ligar para as Aplicações Lógicas, verifique se configura as definições de rede para o seu serviço de gateway de dados no local para o seu ambiente específico. Por exemplo, se o seu ambiente de rede exigir a utilização de um representante para ligar para os pontos finais do Azure, tem de configurar o seu serviço de gateway de dados no local para utilizar o seu representante. Para obter mais informações, consulte [a Configuração Proxy](/dotnet/framework/network-programming/proxy-configuration).

Se estiver a receber esta mensagem de erro e experimentar falhas intermitentes a ligar para as Aplicações Lógicas, poderá ter de aumentar a contagem de repetições e/ou o intervalo de repetição. 

1. Consulte as definições SAP no ficheiro de configuração do serviço de gateway de dados no local, `Microsoft.PowerBI.EnterpriseGateway.exe.config` . A definição de contagem de repetição `WebhookRetryMaximumCount="2"` parece. A definição de intervalo de repetição parece `WebhookRetryDefaultDelay="00:00:00.10"` e o formato timepan é `HH:mm:ss.ff` . 
    
1. Guarde as suas alterações e reinicie o seu portal de dados no local.

##### <a name="the-segment-or-group-definition-e2edk36001-was-not-found-in-the-idoc-meta"></a>O segmento ou definição de grupo E2EDK36001 não foi encontrado no meta IDoc

Esta mensagem de erro significa que falhas esperadas acontecem com outros erros. Por exemplo, a falha na geração de uma carga útil IDoc XML porque os seus segmentos não são lançados pela SAP. Como resultado, falta o tipo de metadados do tipo de segmento necessário para a conversão.
...
1. Antes de começar, precisa de uma aplicação lógica com um gatilho SAP. Se ainda não o tem na sua aplicação lógica, siga os passos anteriores neste tópico para [configurar uma aplicação lógica com um gatilho SAP](#receive-message-from-sap).
...
1. [Adicione uma ação de resposta à sua aplicação lógica](../connectors/connectors-native-reqres.md#add-a-response-action) para responder imediatamente com o estado do seu pedido SAP. É uma boa prática adicionar esta ação imediatamente após o seu gatilho, para libertar o canal de comunicação com o seu servidor SAP. Escolha um dos seguintes códigos de estado `statusCode` () para utilizar na sua ação de resposta:
    ...
    * **204 No Content**, o que significa que o servidor cumpriu com sucesso o pedido e não existe nenhum conteúdo adicional para enviar no corpo de carga útil de resposta. 

    * **200 OK.** Este código de estado contém sempre uma carga útil, mesmo que o servidor gere um corpo de carga útil de comprimento zero. 
