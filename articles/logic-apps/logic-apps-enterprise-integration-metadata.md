...
1. Forneça estas informações para o artefacto que pretende encontrar:
   ...
   |----------|---------|-------|-------------| 
   | **Tipo de artefacto** | Yes | **Schema,** **Mapa,** **Parceiro,** **Acordo,** ou um tipo personalizado | O tipo para o artefacto que quer | 
   | **Nome do artefacto** | Yes | <*nome de artefacto*> | O nome do artefacto que quer | 
   ||| 
...
1. Adicione a ação que deseja para manusear esses metadados, por exemplo:
   ...
   1. Forneça informações sobre os metadados de artefactos que pretende gerir. 
      ...
      | Propriedade | Necessário | Valor | Descrição | 
      |----------|----------|-------|-------------| 
      | **Método** | Yes | <*operação a executar*> | A operação HTTP para executar o artefacto. Por exemplo, esta ação HTTP utiliza o método **GET.** | 
      | **URI** | Yes | <*metadados-localização*> | Para aceder ao `routingUrl` valor dos metadados a partir do artefacto que recuperou, pode utilizar uma expressão, por exemplo: <p>`@{outputs('Integration_Account_Artifact_Lookup')['properties']['metadata']['routingUrl']}` | 
      | **Cabeçalhos** | No | <*cabeçalho-valores*> | Quaisquer saídas de cabeçalho do gatilho que pretende passar para a ação HTTP. Por exemplo, para passar no valor da propriedade do `headers` gatilho: pode utilizar uma expressão, por exemplo: <p>`@triggeroutputs()['headers']` | 
      | **Corpo** | No | <*conteúdo corporal*> | Qualquer outro conteúdo que pretenda passar pela propriedade da ação `body` HTTP. Este exemplo transmite os valores do artefacto `properties` para a ação HTTP: <p>1. Clique dentro da propriedade **Body** para que apareça a lista de conteúdos dinâmicos. Se não aparecerem propriedades, escolha **Ver mais.** <br>2. Da lista de conteúdos dinâmicos, no âmbito **do Integration Account Artifact Lookup**, selecione **Properties**. | 
      |||| 
