...
**Nota no prefixo:** O prefixo URI é calculado por segmento e não por carácter. Por `/a/b` exemplo, é um prefixo `/a/b/c` para, mas não para `/a/bc` .
...
Quando criar um símbolo de uma política de acesso partilhado, coloque `skn` o campo no nome da política. Esta política deve conceder a permissão **DeviceConnect.**
...
Um gateway protocolário poderia usar o mesmo símbolo para todos os dispositivos simplesmente definindo o recurso URI para `myhub.azure-devices.net/devices` .
...
Também pode utilizar o comando de extensão CLI [az iot hub de identidade](/cli/azure/ext/azure-iot/iot/hub/device-identity?view=azure-cli-latest) do dispositivo para configurar certificados X.509 para dispositivos.
...
3. O serviço simbólico devolve um símbolo. O símbolo é criado utilizando `/devices/{deviceId}` ou `/devices/{deviceId}/module/{moduleId}` `resourceURI` as, com `deviceId` a autenticação do dispositivo ou como o `moduleId` módulo a ser autenticado. O serviço simbólico usa a política de acesso partilhado para construir o símbolo.
