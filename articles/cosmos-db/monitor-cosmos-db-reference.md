---
title: Monitorização da referência de dados do Azure Cosmos DB Microsoft Docs
...
Para mais informações, consulte uma lista de [todas as métricas da plataforma suportadas no Azure Monitor.](../azure-monitor/platform/metrics-supported.md)
...
| **operationName** | **Operação Nome** | O nome da operação. O nome da operação pode ser  `Create` , `Update` `Read` ou `ReadFeed` `Delete` `Replace` `Execute` `SqlQuery` `Query` `JSQuery` `Head` `HeadFeed` `Upsert` .   |
