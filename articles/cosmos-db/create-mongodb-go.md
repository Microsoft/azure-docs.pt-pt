...
A aplicação da amostra `todo` é uma ferramenta de gestão baseada em linha de comando escrita em Go. A API da Azure Cosmos DB para mongoDB é [compatível com o protocolo de arame MongoDB,](https://docs.microsoft.com/azure/cosmos-db/mongodb-introduction#wire-protocol-compatibility)permitindo que qualquer condutor de cliente da MongoDB se conectem ao mesmo. Esta aplicação utiliza o [controlador Go para mongoDB](https://github.com/mongodb/mongo-go-driver) de uma forma transparente para a aplicação que os dados são armazenados numa base de dados Azure Cosmos DB.

## <a name="prerequisites"></a>Pré-requisitos
- Uma conta Azure com uma subscrição ativa. [Crie um de graça.](https://azure.microsoft.com/free) Ou [experimente o Azure Cosmos DB gratuitamente](https://azure.microsoft.com/try/cosmosdb/) sem uma subscrição Azure. Também pode utilizar o [Emulador DB Azure Cosmos](https://aka.ms/cosmosdb-emulator) com a corda `.mongodb://localhost:C2y6yDjf5/R+ob0N8A7Cgv30VRDJIWEHLM+4QDU5DE2nQ9nDuVTqobD4b8mGGyPMbIZnqyMsEcaGQy67XIw/Jw==@localhost:10255/admin?ssl=true`de ligação .
...
1. Abra um pedido de comando, `git-samples`crie uma nova pasta chamada e feche o pedido de comando.
...
[`clientOptions`](https://pkg.go.dev/go.mongodb.org/mongo-driver@v1.3.2/mongo/options?tab=doc#ClientOptions)encapsula a cadeia de ligação para O Azure Cosmos DB, que é passado usando uma variável ambiental (detalhes na próxima secção). A ligação é [`mongo.NewClient`](https://pkg.go.dev/go.mongodb.org/mongo-driver@v1.3.2/mongo?tab=doc#NewClient) inicializada `clientOptions` utilizando a qual a instância é passada. função é invocada para confirmar conectividade bem sucedida (é uma estratégia de falha rápida) [ `Ping` ](https://pkg.go.dev/go.mongodb.org/mongo-driver@v1.3.2/mongo?tab=doc#Client.Ping)
...
### <a name="create-a-todo-item"></a>Criar `todo` um item

Para criar `todo`um, temos uma [`mongo.Collection`](https://pkg.go.dev/go.mongodb.org/mongo-driver@v1.3.2/mongo?tab=doc#Collection) pega [`InsertOne`](https://pkg.go.dev/go.mongodb.org/mongo-driver@v1.3.2/mongo?tab=doc#Collection.InsertOne) para um e invocamos a função. 
...
Passamos por `Todo` uma estrutura que contém a descrição e o `pending`estado (que está inicialmente definido para)
...
### <a name="update-a-todo-item"></a>Atualizar `todo` um item

A `todo` pode ser atualizado `_id`com base no seu . Um [`bson.D`](https://pkg.go.dev/go.mongodb.org/mongo-driver@v1.3.2/bson?tab=doc#D) filtro é criado `_id` com base no e outro é criado para`completed` `pending`a informação atualizada, que é um novo estado ( ou ) neste caso. Finalmente, [`UpdateOne`](https://pkg.go.dev/go.mongodb.org/mongo-driver@v1.3.2/mongo?tab=doc#Collection.UpdateOne) a função é invocada com o filtro e o documento atualizado
...
A `todo` é eliminado `_id` com base no seu e é encapsulado sob a forma de um [`bson.D`](https://pkg.go.dev/go.mongodb.org/mongo-driver@v1.3.2/bson?tab=doc#D) caso. [`DeleteOne`](https://pkg.go.dev/go.mongodb.org/mongo-driver@v1.3.2/mongo?tab=doc#Collection.DeleteOne)é invocado para apagar o documento.
...
Mude para o diretório onde clonou a `go build`aplicação e a construa (utilizando).
...
Para `MONGODB_CONNECTION_STRING` a variável ambiental, substitua os espaços reservados para `<COSMOSDB_ACCOUNT_NAME>` e`<COSMOSDB_PASSWORD>`
...
Pode escolher os seus `MONGODB_DATABASE` `MONGODB_COLLECTION` valores preferidos e ou deixá-los como estão.
...
Se for bem sucedido, deverá ver uma `_id` saída com o MongoDB do documento recém-criado:
...
Para atualizar o `todo` estado de um (por `completed` exemplo, `todo` alterá-lo para estado), utilize o ID

```bash
./todo --update 5e9fd6b1bcd2fa6bd267d4c4,completed
```

Listar apenas `todo`os s concluídos
...
Eliminar `todo` um uso do seu ID

```bash
./todo --delete 5e9fd6b1bcd2fa6bd267d4c4,completed
```

Listar `todo`os s para confirmar
...
Neste arranque rápido, aprendeu a criar uma conta API Azure Cosmos DB MongoDB usando a Azure Cloud `todo`Shell, e criar e executar uma app de linha de comando Go para gerir s. Agora, pode importar dados adicionais para a sua conta do Azure Cosmos DB.
