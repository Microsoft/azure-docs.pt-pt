---
title: Perguntas frequentes sobre o modo de escala automática em Azure Cosmos DB
...
Com a produção aprovisionada em modo de escala automática, o Azure Cosmos DB irá gerir e escalar automaticamente os RU/s do seu recipiente ou base de dados com base na utilização. Este artigo responde a perguntas comumente feitas sobre escala automática.

## <a name="frequently-asked-questions"></a>Perguntas mais frequentes

### <a name="is-autoscale-mode-supported-for-all-apis"></a>O modo de escala automática é suportado para todas as APIs?
Sim, o modo de escala automática é suportado para todas as APIs: Core (SQL), Gremlin, Table, Cassandra e API para MongoDB.

### <a name="is-autoscale-mode-supported-for-multi-master-accounts"></a>O modo de escala automática é suportado para contas multi-master?
Sim, o modo de escala automática é suportado para contas multi-master. Os max RU/s estão disponíveis em cada região que é adicionado à conta Cosmos. 
...
O modo de escala automática pode ser ativado em novos recipientes e bases de dados criadas através do portal Azure. 

### <a name="is-there-cli-or-sdk-support-to-create-containers-or-databases-with-autoscale-mode"></a>Existe suporte CLI ou SDK para criar recipientes ou bases de dados com modo de escala automática?
Atualmente, só é possível criar recursos com modo de escala automática a partir do portal Azure. O suporte para CLI e SDK ainda não está disponível.
...
Atualmente, pode ativar automaticamente a escala em novos recipientes e bases de dados ao criá-los. O suporte para permitir o modo de escala automática em recipientes e bases de dados existentes ainda não está disponível. Pode migrar os contentores existentes para um novo contentor utilizando a [Azure Data Factory](../data-factory/connector-azure-cosmos-db.md) ou [alterar o feed](change-feed.md). 

### <a name="can-i-turn-off-autoscale-mode-on-a-container-or-database"></a>Posso desligar o modo de escala automática num recipiente ou numa base de dados?
Sim, pode desativar a escala automática mudando para a opção 'Manual' para a entrada prevista. Na versão atual, depois de mudar do modo de escala automática para o modo manual, não é possível voltar a ativar automaticamente o mesmo recurso. 

### <a name="is-autoscale-mode-supported-for-shared-throughput-databases"></a>O modo de escala automática é suportado para bases de dados de produção partilhadas?
Sim, o modo de escala automática é suportado para bases de dados de entrada partilhadas. Para ativar esta funcionalidade, selecione o modo de escala automática e a opção de **entrada provisioniana** ao criar a base de dados. 
...
Para bases de dados de entradas partilhadas com modo de escala automática ativado, o número de coleções permitidas é: MIN(25, Max RU/s de base de dados / 1000). Por exemplo, se a entrada máxima da base de dados for de 20.000 RU/s, a base de dados pode ter MIN(25, 20.000 RU/s / 1000) = 20 coleções. 
...
No modo de escala automática, pode escalar instantaneamente para cima ou escalar o RU/s dentro da gama mínima e máxima de RU/s, com base no tráfego de entrada. A faturação é feita numa granularidade de 1 hora, onde é cobrado pelo RU/s mais alto numa hora particular.

### <a name="can-i-specify-a-custom-max-throughput-rus-value-for-autoscale-mode"></a>Posso especificar um valor de entrada máxima personalizada (RU/s) para o modo de escala automática?
...
* Conheça os [benefícios da produção aprovisionada no modo de escala automática](provision-throughput-autoscale.md#autoscale-benefits).
