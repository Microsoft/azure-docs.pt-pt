...
```
O seguinte comando irá **desativar a** SSR para todas as coleções na sua conta de base de dados. Pode levar até 15 min para que esta mudança entre em vigor.
```bash
az cosmosdb update --name accountname --resource-group resourcegroupname --capabilities EnableMongo DisableRateLimitingResponses
```
