...
| **409** | `"Conflicting request to resource has been attempted. Retry to avoid conflicts."`Isso geralmente acontece quando o vértice ou uma borda com um identificador já existe no grafo.| 
...
| **500** | Mensagem de erro que `"NotFoundException: Entity with the specified id does not exist in the system."` contém indica que um banco de dados e/ou coleção foi recriado com o mesmo nome. Esse erro desaparecerá em 5 minutos, conforme a alteração se propaga e invalida caches em diferentes componentes de Cosmos DB. Para evitar esse problema, use o banco de dados e os nomes de coleção exclusivos a cada vez.| 
