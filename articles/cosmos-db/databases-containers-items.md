...
Você pode criar um ou vários bancos de dados do Azure Cosmos em sua conta. Um banco de dados é análogo a um namespace. Um banco de dados é a unidade de gerenciamento para um conjunto de contêineres de Cosmos do Azure. A tabela seguinte mostra como uma base de dados do Cosmos do Azure está mapeada para várias entidades de API específicas:

| Entidade Cosmos do Azure | SQL API | API de Cassandra | API do Azure Cosmos DB para MongoDB | API do Gremlin | API de Tabela |
| --- | --- | --- | --- | --- | --- |
...
| Propriedade definida pelo sistema | Gerado pelo sistema ou configurável pelo usuário | Objetivo | SQL API | API de Cassandra | API do Azure Cosmos DB para MongoDB | API do Gremlin | API de Tabela |
| --- | --- | --- | --- | --- | --- | --- | --- |
|\_sessão | Gerado pelo sistema | Identificador exclusivo do contentor | Sim | Não | Não | Não | Não |
|\_ETag | Gerado pelo sistema | Etiqueta de entidade utilizada para controlo de simultaneidade otimista | Sim | Não | Não | Não | Não |
|\_CALs | Gerado pelo sistema | Última timestamp atualizado do contentor | Sim | Não | Não | Não | Não |
|\_auto-restauração | Gerado pelo sistema | Endereçável URI do contentor | Sim | Não | Não | Não | Não |
|id | Configurável pelo usuário | Nome exclusivo definido pelo utilizador do contentor | Sim | Sim | Sim | Sim | Sim |
...
|changeFeedPolicy | Configurável pelo usuário | Utilizado para ler as alterações efetuadas aos itens num contentor. Para obter detalhes, consulte o [feed de alterações](change-feed.md). | Sim | Não | Não | Não | Sim |
...
|ID | Qualquer um dos | Nome exclusivo definido pelo usuário em uma partição lógica. Se o usuário não especificar a ID, o sistema gerará automaticamente uma. | Sim | Sim | Sim | Sim | Sim |
|Arbitrárias propriedades definidas pelo utilizador | Definidas pelo utilizador | Propriedades definidas pelo usuário representadas na representação nativa da API (incluindo JSON, BSON e CQL) | Sim | Sim | Sim | Sim | Sim |
