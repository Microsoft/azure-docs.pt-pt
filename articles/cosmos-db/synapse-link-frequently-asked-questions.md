...
No lançamento de pré-visualização pública, o Synapse Link é suportado apenas para a API AZure Cosmos DB SQL (Core). O suporte para a API da Cosmos DB para a Mongo DB & Cassandra API está atualmente sob pré-visualização.  Para solicitar o acesso à pré-visualização fechada, contacte a equipa DB da [Azure Cosmos](mailto:cosmosdbsynapselink@microsoft.com).
