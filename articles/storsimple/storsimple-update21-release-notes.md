...
| 2 |volumes afixados localmente |Em casos raros, o sistema falharia ao criar um volume fixado localmente. Este bug foi corrigido nesta versão. |Yes |Não |
| 3 |Tiering |Houve acidentes esporádicos quando os metadados dos StorSimple Cloud Appliances (8010 e 8020) se dilaceraram para a nuvem. Esta questão é corrigida nesta versão. |Não |Yes |
...
| 5 |Autenticação openstack |Ao utilizar o Openstack como fornecedor de serviços de nuvem, o utilizador desabrochou com um bug pouco frequente relacionado com a autenticação onde o parser JSON resultou numa falha. Este bug está fixo nesta versão. |Yes |Não |
| 6 |Cópia do lado do anfitrião |Em versões anteriores do software, um bug pouco frequente relacionado com o tempo ODX foi visto ao copiar os dados de um volume para outro volume. Isto resultaria num failover do controlador e o sistema poderia potencialmente entrar no modo Recovery. Este bug está fixo nesta versão. |Yes |Não |
| 7 |Instrumentação de gestão do Windows (WMI) |Nas versões anteriores do software, houve vários casos de falha de procuração web com a exceção " \<ManagementException> Falha de carga do fornecedor". Este bug foi atribuído a uma fuga de memória do WMI e está agora corrigido. |Yes |Não |
