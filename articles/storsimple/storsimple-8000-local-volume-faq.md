...
**Q.** Atualizei recentemente o meu dispositivo 8100 para o Update 3.0 e quando tento criar um volume localmente fixado, o tamanho máximo disponível é de apenas 6 TB e não 8,5 TB. Por que não posso criar um volume de 8,5 TB?

**A** Se o seu dispositivo estiver a ser atualizado 3.0, pode fornecer volumes fixados localmente até 8,5 TB OU volumes nihosos até 200 TB no dispositivo 8100. Se o seu dispositivo já tiver volumes hierárquicos, então o espaço disponível para criar um volume fixado localmente será proporcionalmente inferior a este limite máximo. Por exemplo, se já tiverem sido aprovisionados cerca de 106 TB de volumes hierárquicos no seu dispositivo 8100 (que é metade da capacidade hierárquica), então o tamanho máximo de um volume local que pode criar no dispositivo 8100 será correspondentemente reduzido a 4 TB (cerca de metade da capacidade máxima de volume fixado localmente).
...
**Q.** Porque é que a criação de volume local mente é uma operação de longa duração?
...
**Q.** Quanto tempo demora a criar um volume local?
...
**Q.** Quero criar um volume localmente fixado. Há algumas boas práticas que eu precise de ter conhecimento?
...
**Q.** Posso criar vários volumes localmente presos ao mesmo tempo?
...
**Q.** Apaguei um volume localmente fixado. Por que não vejo o espaço recuperado refletido no espaço disponível quando tento criar um novo volume?
...
**Q.** Os volumes fixados localmente são suportados no aparelho de nuvem?
...
**Q.** Posso usar os cmdlets Azure PowerShell para criar e gerir volumes locais?
...
**Q.** As imagens locais de volumes locais são suportadas?
...
**Q.** Existem algumas diretrizes para gerir instantâneos locais para volumes locais?
...
**Q.** Recebi um alerta a dizer que as minhas fotos locais de volumes locais podem ser invalidadas. Quando é que isto pode acontecer?

**A.** Instantâneos locais frequentes ao lado de uma alta taxa de dados no volume fixado localmente podem fazer com que o espaço local no dispositivo seja consumido rapidamente. Se os níveis locais do dispositivo forem fortemente utilizados, uma paragem prolongada da nuvem pode resultar na formação do dispositivo, e a entrada de escritos para o volume pode resultar na invalidação dos instantâneos (uma vez que não existe espaço para atualizar os instantâneos para se referir aos blocos mais antigos de dados que foram sobreescritos). Numa situação destas, os escritos ao volume continuarão a ser servidos, mas os instantâneos locais podem ser inválidos. Não há impacto nas suas imagens de nuvens existentes.
...
**Q.** Estou a observar alguma lentidão no dispositivo enquanto converto um volume hierárquico num volume local. Porque é que isto está a acontecer?
...
**Q.** Quanto tempo demora a restaurar um volume local?
...
**Q.** Preciso restaurar o meu volume localmente fixado para um instantâneo mais antigo (tirado quando o volume foi niveado). Será que o volume será restaurado como niverizado neste caso?
...
**Q.** Estendei o meu volume localmente fixado recentemente, mas agora preciso de restaurar os dados numa altura em que o volume era menor em tamanho. Irá restaurar a redimensionação do volume atual e terei de alargar o tamanho do volume assim que a restauração estiver concluída?
...
**Q.** Preciso restaurar o meu volume local, mas escolhi um ponto incorreto no instantâneo do tempo. Posso cancelar a operação de restauro atual?
...
**Q.** Iniciei uma operação de restauro num dos meus volumes locais, e agora vejo uma foto no meu catálogo de atrasos que não me lembro de ter criado. Para que é usado?
...
**Q.** Posso clonar um volume local?
...
**Q.** Preciso de falhar o meu dispositivo para outro dispositivo físico. Será que os meus volumes locais serão falhados como fixados localmente ou nivilhados?
...
**Q.** Vejo o trabalho de failover concluído, como posso acompanhar o progresso do volume local mente fixado que está a ser restaurado no dispositivo-alvo?
...
**Q.** Posso alterar o tipo de volume durante a falha?
...
**Q.** Posso falhar sobre um contentor de volume com volumes fixados localmente no aparelho de nuvem?
