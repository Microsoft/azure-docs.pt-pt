---
title: Notas de lançamento storSimple Virtual Array Update 1.0
description: Descreve questões e resoluções críticas abertas para o StorSimple Virtual Array executando update 1.0.
author: alkohli
...
---
# <a name="storsimple-virtual-array-update-10-release-notes"></a>Notas de lançamento storSimple Virtual Array Update 1.0
...
> [!IMPORTANT]
> - As atualizações são disruptivas e reiniciam o seu dispositivo. Se o I/O estiver em curso, o dispositivo incorre no tempo de inatividade. Para obter instruções detalhadas sobre como aplicar a atualização, vá à [Instalação da Atualização 1.0](storsimple-virtual-array-install-update-1.md).
>
> - O Update 1 só está disponível através do portal Azure se o seu dispositivo estiver a executar o Update 0.6.

## <a name="whats-new-in-update-10"></a>Novidades na Atualização 1.0

**A atualização 1.0 contém alterações relacionadas com a autenticação do serviço StorSimple Device Manager e deve ser implementada o mais rapidamente possível.** Esta atualização contém as seguintes melhorias e correções de bugs:

 - **Utilização do Azure Ative Directory (AAD) para autenticar com o serviço StorSimple Device Manager** – A partir da Atualização 1.0, o Azure Ative Directory é utilizado para autenticar com o serviço StorSimple Device Manager. O antigo mecanismo de autenticação será depreto até dezembro de 2017. Todos os utilizadores devem incluir os novos URLs de autenticação nas suas regras de firewall. Para mais informações, vá a URLs de autenticação listados nos [requisitos de Networking para o seu StorSimple Virtual Array](storsimple-ova-system-requirements.md).
 
    Se o URL de autenticação não estiver incluído nas regras de firewall, os utilizadores verão um alerta crítico de que o seu dispositivo StorSimple não poderia autenticar com o serviço. Se os utilizadores virem este alerta, têm de incluir o novo URL de autenticação. Para mais informações, vá a alertas de [rede StorSimple](storsimple-virtual-array-manage-alerts.md).

 - **Melhoria do desempenho** - Foram feitas várias correções de bugs para melhorar as velocidades das leituras em nuvem, nível de acesso e de nível. Como resultado, tanto o desempenho de backup como de restauro melhoraram para dispositivos iSCSI e servidor de ficheiros.

 - **Melhoria** da recolha de lixo - Esta versão tem correções de bugs que melhoram o desempenho do ciclo de recolha de lixo quando o dispositivo e a conta de armazenamento estão em duas regiões distantes.

 - **Melhoria do registo** - Esta versão contém melhorias na exploração madeireira relacionada com a recolha de lixo e o caminho de I/S.
...
| --- | --- | --- |
| 1 |Autenticação baseada em AAD| Esta versão contém alterações que permitem que o AAD autentique com o Gestor de Dispositivos StorSimple.|
| 2 |Recolha de lixo| Este problema foi reportado num site de clientes onde o dispositivo e as contas de armazenamento estão em diferentes regiões e o cliente relatou erros intermitentes de rede, afetando assim a faturação. Nesta versão, esta questão foi corrigida. |
| 3 |Desempenho| Esta versão contém alterações que resultam em restaurar/cloud lê/tier in/tier out melhoria de desempenho.|
| 4 |Atualizar| Houve um problema com a atualização no lançamento anterior que resultou em falhas de backup num site de clientes. Esta questão está corrigida nesta versão.|

## <a name="known-issues-in-update-10"></a>Questões conhecidas na Atualização 1.0
