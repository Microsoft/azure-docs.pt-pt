...
O Serviço Apple Push Notification (APNS) utiliza certificados para autenticar as suas notificações push. Siga estas instruções para criar o certificado push necessário para enviar e receber notificações. Para obter mais informações sobre estes conceitos, consulte a documentação oficial do Serviço de [Notificação apple push.](https://developer.apple.com/library/archive/documentation/NetworkingInternet/Conceptual/RemoteNotificationsPG/APNSOverview.html)  
...
2. Selecione **Keychain Access**, expanda **o Certificate Assistant**e, em seguida, selecione **Solicite um Certificado a partir de uma Autoridade de Certificados**.
   ...
   :::image type="content" source="media/ios-sdk-get-started/image1.png" alt-text="Acesso ao chaveiro":::
...
3. Selecione **Choose File**, navegue no local onde guardou o ficheiro CSR a partir da primeira tarefa e, em seguida, clique duas vezes no nome do certificado para o carregar. Em seguida, **selecione Continue**.
...
No final destes passos deverá ter as seguintes informações para utilização posterior no [Configure o seu centro de notificação com informações sobre APNS:](notification-hubs-ios-push-notifications-swift-apps-get-started.md#configure-your-notification-hub-with-apns-information)
...
4. Na janela **de certificados Select,**   selecione o certificado de desenvolvimento que utiliza para a assinatura de código e selecione **Continue**. Este certificado não é o certificado de pressão que criou. Se alguém não existe, deve criá-lo. Se existir um certificado, salte para o passo seguinte. Para criar um certificado de desenvolvimento se não existir:
   ...
   2. Na secção **de Software,**   selecione **Apple Development**. Em seguida, **selecione Continue**.
...
6. Na janela **de certificados Select,**   selecione o certificado de desenvolvimento que acabou de criar. Em seguida, **selecione Continue**.
...
Nos **Serviços de Notificação**, selecione **Apple (APNS)**, siga os passos adequados com base na abordagem que escolheu anteriormente na secção [Criar um Certificado para Centros de Notificação.](notification-hubs-ios-push-notifications-swift-apps-get-started.md#creating-a-certificate-for-notification-hubs)  
...
- [Tutorial: Integrar-se com uma aplicação iOS usando Swift](notification-hubs-ios-push-notifications-swift-apps-get-started.md)
