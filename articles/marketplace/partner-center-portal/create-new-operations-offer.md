---
title: Criar uma oferta de Dinâmica 365 para Operações no Mercado Comercial
...
Este ficheiro deve estar em formato .xls ou .xlsx. Se tiver mais de um relatório, pode fazer o upload de um ficheiro .zip contendo todos os relatórios.
...
- Consulte o estado de conclusão de cada secção da oferta.
    - **Não iniciado** – A secção não foi tocada e deve estar concluída.
    - **Incompleto** – A secção tem erros que precisam de ser corrigidos ou exigem mais informações a fornecer. Volte para a secção(s) e atualize-a.
