...
Com o serviço de medição marketplace, pode criar ofertas de software-as-a-service (SaaS) que são cobradas de acordo com unidades não padrão.  Antes de publicar esta oferta, você define as dimensões de faturação tais como largura de banda, bilhetes ou e-mails processados.  Os clientes pagam então de acordo com o seu consumo destas dimensões, com o seu sistema a informar a Microsoft através do serviço de medição de marketplace API de eventos faturados à medida que ocorrem.  
...
* Cumpra todos os requisitos de oferta para uma [venda através da oferta da Microsoft,](./create-new-saas-offer.md#sell-through-microsoft) conforme delineado na Create a [SaaS.](./create-new-saas-offer.md)
...
Em seguida, a oferta SaaS pode integrar-se com o [serviço de medição de marketplace APIs](./marketplace-metering-service-apis.md) para informar a Microsoft de eventos faturantes.
...
* Além das taxas recorrentes, um plano de taxa fixa também pode incluir dimensões personalizadas opcionais usadas para cobrar aos clientes pelo uso excessivo não incluído na taxa fixa.  Cada dimensão representa uma unidade faturada que o seu serviço comunicará à Microsoft utilizando o [serviço de medição do Marketplace API](./marketplace-metering-service-apis.md).
...
Tal como os preços fixos, os preços de dimensão da faturação podem ser definidos por país ou região suportados. O editor precisa de usar a funcionalidade de importação e exportação de dados de preços no Partner Center.

1. Primeiro, defina as dimensões desejadas e marque quais os mercados que são apoiados. 
1. Em seguida, exporte estes dados em arquivo.
...
[![Dimensões do serviço de medição do mercado](./media/metering-service-dimentions.png "Clique para ver ampliada")](./media/metering-service-dimentions.png)
...
A faturação medido utilizando o serviço de medição do mercado não é compatível com a oferta de um teste gratuito.  Não é possível configurar um plano para utilizar a faturação medido e um julgamento gratuito.

### <a name="locking-behavior"></a>Comportamento de bloqueio

Uma vez que uma dimensão utilizada com o serviço de medição do Marketplace representa uma compreensão de como um cliente vai pagar pelo serviço, todos os detalhes para uma dimensão já não são editáveis uma vez que o publique.  É importante que tenha as suas dimensões totalmente definidas para um plano antes de publicar.
