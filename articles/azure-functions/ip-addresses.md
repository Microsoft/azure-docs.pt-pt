...
```
> [!NOTE]
> Quando um aplicativo de funções que é executado no [plano de consumo](functions-scale.md#consumption-plan) é dimensionado, um novo intervalo de endereços IP de saída pode ser atribuído. Ao executar no plano de consumo, talvez seja necessário colocar a data center inteira na lista de permissões.
