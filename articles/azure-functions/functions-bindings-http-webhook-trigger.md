...
Pode ligar-se a um objeto personalizado em vez de `HttpRequest`. Este objeto é criado a partir do corpo do pedido e analisado como JSON. Da mesma forma, um tipo pode ser passado para a ligação de saída de resposta HTTP e devolvido como o corpo de resposta, juntamente com um código de estado `200`.
...
Pode permitir pedidos anónimos, que não requerem chaves. Também pode exigir que a chave principal seja utilizada. Altera o nível de autorização por defeito utilizando a propriedade `authLevel` no JSON vinculativo. Para mais informações, consulte [Trigger - configuração](#configuration).
