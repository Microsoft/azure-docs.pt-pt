...
## <a name="creating-1x-apps"></a>Executar numa versão específica
...
Pode optar por migrar uma aplicação existente escrita para utilizar o tempo de execução da versão 1.x para, em vez disso, utilizar uma versão mais recente. A maioria das alterações que precisa de fazer estão C# relacionadas com alterações no tempo de execução da linguagem, tais como alterações de API entre .NET Framework 4.7 e .NET Core. Também terá de se certificar de que o seu código e bibliotecas são compatíveis com o tempo de execução da linguagem que escolher. Por fim, certifique-se de que quaisquer alterações no gatilho, encadernações e funcionalidades destacadas abaixo. Para obter os melhores resultados de migração, deverá criar uma nova aplicação de funções numa nova versão e de portar o código de função da versão 1.x existente para a nova aplicação.  

Embora seja possível fazer uma atualização "in-place" atualizando manualmente a configuração da aplicação, passar de 1.x para uma versão mais alta inclui algumas alterações de quebra. Por exemplo, C#em , o objeto de depuração é alterado de `TraceWriter` para `ILogger`. Ao criar um novo projeto versão 3.x, começa com funções atualizadas com base nos modelos mais recentes da versão 3.x.
...
Há também algumas alterações na *função.json* ou atributos da função entre versões. Por exemplo, a propriedade de `path` do Event Hub está agora `eventHubName`. Consulte a [tabela de encadernação existente](#bindings) para ligações à documentação para cada encadernação.
...
* Os aceleradores de condivisões HTTP são implementados por padrão para funções do plano de consumo, com um padrão de 100 pedidos simultâneos por exemplo. Pode alterar isto na [definição`maxConcurrentRequests`](functions-host-json.md#http) no ficheiro host.json.

* Devido às [limitações do Núcleo .NET,](https://github.com/Azure/azure-functions-host/issues/3414)o suporte para F# funções de script (.fsx) foi removido. As F# funções compiladas (.fs) ainda são suportadas.
...
* O Event Hub desencadeou funções com `dataType` binário receberá uma série de `binary` em vez de `string`.

* A carga útil do pedido HTTP já não pode ser acedida através de `context.bindingData.req`.  Ainda pode ser acedido como parâmetro de entrada, `context.req`, e em `context.bindings`.
...
A versão do tempo de funcionamento das Funções utilizada pelas aplicações publicadas no Azure é ditada pela definição de aplicação [`FUNCTIONS_EXTENSION_VERSION`.](functions-app-settings.md#functions_extension_version) Os seguintes principais valores da versão runtime são suportados:
...
Para o desenvolvimento do Código do Estúdio Visual, poderá também ser necessário atualizar a definição do utilizador para que o `azureFunctions.projectRuntime` corresponda à versão das ferramentas instaladas.  Esta definição também atualiza os modelos e idiomas utilizados durante a criação de aplicações de função.  Para criar aplicações em `~3` atualizaria a definição de `azureFunctions.projectRuntime` utilizador para `~3`.
