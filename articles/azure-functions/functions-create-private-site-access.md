...
2. No campo de `Windows Server`pesquisa, escreva e selecione **O Servidor do Windows** nos resultados da pesquisa.
...
4. No separador **Basics,** utilize as definições vm conforme especificado na tabela abaixo da imagem:
    ...
    >![Separador básico para um novo VM do Windows](./media/functions-create-private-site-access/create-vm-3.png)
    ...
    | **Portos de entrada pública** | Nenhuma | Selecione **Nenhum** para garantir que não há conectividade de entrada com o VM a partir da internet. O acesso remoto ao VM será configurado através do serviço Azure Bastion. |
...
6. Na **Rede Criar virtual,** utilize as definições na tabela abaixo da imagem:
    ...
    | **Intervalo de endereços** (sub-rede) | 10.10.1.0/24 | O tamanho da sub-rede define quantas interfaces podem ser adicionadas à sub-rede. Esta sub-rede é utilizada pelo VM. Uma `/24` sub-rede fornece 254 endereços de anfitrião. |
...
3. Selecione **Criar** para iniciar o processo de criação de um novo recurso Azure Bastion. Notará uma mensagem de erro na secção de `AzureBastionSubnet` rede **Virtual,** uma vez que ainda não existe uma sub-rede. A sub-rede é criada nos seguintes passos. Utilize as definições na tabela abaixo da imagem:

    >[!div class="mx-imgBorder"]
    >![Início da criação do Bastião Azure](./media/functions-create-private-site-access/create-bastion-basics-1.png)
    ...
    | **Sub-rede** | AzureBastionSubnet | A subrede na sua rede virtual para a qual será implantado o novo recurso de anfitrião bastião. Deve criar uma sub-rede `AzureBastionSubnet`utilizando o valor do nome . Este valor permite ao Azure saber para que sub-rede implantar os recursos da Bastião. Deve utilizar uma sub-rede `/27` de,`/27` `/26`pelo menos ou maior, e assim por diante. |
...
5. A sub-rede deve `AzureBastionSubnet` ser do nome e a `/27`prefixação da sub-rede deve ser pelo menos .  Selecione **OK** para criar a sub-rede.

    >[!div class="mx-imgBorder"]
    >![Criar subnet para anfitrião do Bastião Azure](./media/functions-create-private-site-access/create-bastion-subnet-2.png)

6. Na página **Criar um Bastião,** selecione o recém-criado `AzureBastionSubnet` a partir da lista de subnets disponíveis.

    >[!div class="mx-imgBorder"]
    >![Criar um hospedeiro Azure Bastion com subnet específico](./media/functions-create-private-site-access/create-bastion-basics-2.png)

7. Selecione **Rever & Criar**. Assim que a validação estiver concluída, selecione **Criar**. Levará alguns minutos para que o recurso Azure Bastion seja criado.
...
1. No portal, escolha **Adicionar** no topo da vista do grupo de recursos.
2. **Selecione Compute > Function App**
3. Na secção **Basics,** utilize as definições da aplicação de função conforme especificado no quadro abaixo.
    ...
    Selecione o **seguinte: Ahospedação >** botão.
...
5. A página **de Restrições** de Acesso mostra agora que existe uma nova restrição. Pode levar alguns segundos para que o `Provisioning` `Enabled`estado do **Ponto Final** mude de `Disabled` até .

    >[!IMPORTANT]
    > Cada aplicação de função tem um site advanced [Tool (Kudu)](../app-service/app-service-ip-restrictions.md#scm-site) que é usado para gerir implementações de aplicações de função. Este site é acedido a `<FUNCTION_APP_NAME>.scm.azurewebsites.net`partir de um URL como: . Como as restrições de acesso não foram ativadas neste site de implementação, ainda pode implementar o seu código de projeto a partir de uma estação de trabalho de desenvolvedores local ou construir serviço sem ter de fornecer um agente dentro da rede virtual.
...
1. Volte à aplicação de funções previamente criada.  Na secção **Visão Geral,** copie o URL.
    ...
    >[!div class="mx-imgBorder"]
    >![Obtenha o URL da aplicação função](./media/functions-create-private-site-access/access-function-overview.png)
...
1. Para testar o acesso à função, é necessário copiar o URL da função. Selecione a função implantada e, em seguida, selecione **</> Obter URL**de função . Em seguida, clique no botão **Copiar** o URL para a sua área de redação.
...
>[!div class="mx-imgBorder"]
>![Aceda à Função Azure via Bastião Azure](./media/functions-create-private-site-access/access-function-via-bastion-final.png)
