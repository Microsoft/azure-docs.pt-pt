...
| **nome** | n/d| Necessário - o nome da variável no código de função para a pedido ou corpo do pedido. |
...
| **rota** | **Encaminhar** | Define o modelo de rota, controlar a que URLs responde de sua função de pedido. O valor predefinido se não for fornecida nenhuma é `<functionname>`. Para obter mais informações, consulte [personalizar o ponto de extremidade http](#customize-the-http-endpoint). |
...
|**nome** | O nome da variável no código de função para a resposta, ou `$return` para utilizar o valor de retorno. |
