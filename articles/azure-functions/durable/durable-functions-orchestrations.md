...
* Os IDs de exemplo não devem começar com `@`.
* Os iDs por exemplo não devem conter caracteres `/`, `\`, `#`ou `?`.
...
Em cadaC#`await` ( ou `yield` (JavaScript), o Quadro de Tarefas Duráveis verifica o estado de execução da função em algum backend de armazenamento durável (armazenamento tipicamente Azure Table). Este estado é o que é referido como a história da *orquestração.*
...
3. Enfilas mensagens para o próprio orquestrador &mdash; por exemplo, mensagens temporais duráveis.
...
* **Tipo**de evento : Representa o tipo do evento. Pode ser um dos seguintes tipos:
  * **OrquestraçãoIniciada**: A função de orquestrador retomou-se de um aguardado ou está a decorrer pela primeira vez. A coluna `Timestamp` é utilizada para preencher o valor determinístico para as APIs `CurrentUtcDateTime` (.NET) e `currentUtcDateTime` (JavaScript).
  * **ExecuçãoIniciada**: A função de orquestrador começou a ser executada pela primeira vez. Este evento também contém a entrada de função na coluna `Input`.
  * **TarefaSAgendadas**: Uma função de atividade foi programada. O nome da função de atividade é capturado na coluna `Name`.
  * **TarefaConcluída**: Uma função de atividade concluída. O resultado da função está na coluna `Result`.
  * **TimerCreated**: Foi criado um temporizador durável. A coluna `FireAt` contém o tempo de UTC programado em que o temporizador expira.
  * **TimerFired:** Um temporizador durável disparado.
  * **EventRaised**: Um evento externo foi enviado para a instância de orquestração. A coluna `Name` captura o nome do evento e a coluna `Input` captura a carga útil do evento.
  * **OrchestratorCompleted**: A função de orquestrador aguardava.
  * **ContinueAsNew**: A função orquestradora completou-se e recomeçou-se com um estado novo. A coluna `Result` contém o valor, que é usado como entrada na instância reiniciada.
  * **ExecuçãoConcluída:** A função de orquestrador correu para a conclusão (ou falhou). As saídas da função ou os erros são armazenados na coluna `Result`.
...
Sempre que a função recomeça a partir de um `await` (C#) ou `yield` (JavaScript), o Quadro de Tarefas Duráveis repete a função orquestradora do zero. Em cada recandidatura, consulta o histórico de execução para determinar se a operação asincronizada atual ocorreu.  Se a operação tiver ocorrido, o quadro repete imediatamente a saída dessaC#operação e passa para a próxima `await` () ou `yield` (JavaScript). Este processo continua até que toda a história tenha sido reproduzida. Uma vez reproduzida a história atual, as variáveis locais terão sido restauradas aos seus valores anteriores.
...
As orquestrações podem programar *tempos duráveis* para implementar atrasos ou para configurar o tratamento do timeout em ações de asincronização. Utilize tempors duráveis em funções deC#orquestrador em vez de `Thread.Sleep` e `Task.Delay` ( ou `setTimeout()` e `setInterval()` (JavaScript).
...
As funções de orquestrador podem utilizar as funcionalidades de manipulação de erros da linguagem de programação. Padrões existentes como `try`/`catch` são apoiados no código de orquestração.
...
> [!NOTE]
> Se houver uma exceção não tratada numa função orquestradora, a instância de orquestração completar-se-á num estado `Failed`. Um caso de orquestração não pode ser julgado novamente uma vez que falhou.
...
Para simplificar este padrão comum, as funções orquestradoras podem usar o método `CallHttpAsync` para invocar http APIs diretamente.
