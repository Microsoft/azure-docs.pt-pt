...
* **C#**: ambas [as bibliotecas de classes pré-compiladas](../functions-dotnet-class-library.md) e [o guião C#](../functions-reference-csharp.md).
...
* **F#**: bibliotecas de classes pré-compiladas e guião F# O script F# é suportado apenas para a versão 1.x do tempo de execução das Funções Azure.
...
### <a name="pattern-1-function-chaining"></a><a name="chaining"></a>#1 de padrão: Corrente de funções
...
Neste exemplo, os `F1` `F2`valores , `F3`e `F4` são os nomes de outras funções na mesma aplicação de funções. Pode implementar o fluxo de controlo utilizando construções normais de codificação imperativas. O código executa de cima para baixo. O código pode envolver a semântica de fluxo de controlo linguístico existente, como condicionales e loops. Pode incluir a lógica `try` / `catch` / de manipulação de erros em `finally` blocos.
...
Pode utilizar `context` o parâmetro para invocar outras funções pelo nome, passar parâmetros e devolução da função. Cada vez que `await`o código chama, o quadro de Funções Duráveis verifica o progresso da atual função. Se o processo ou a máquina virtual reciclarem a meio da `await` execução, a instância de função retoma a partir da chamada anterior. Para mais informações, consulte a secção seguinte, Pattern #2: Fan out/fan in.
...
Pode utilizar `context.df` o objeto para invocar outras funções pelo nome, passar parâmetros e devolução da função. Cada vez que `yield`o código chama, o quadro de Funções Duráveis verifica o progresso da atual função. Se o processo ou a máquina virtual reciclarem a meio da `yield` execução, a instância de função retoma a partir da chamada anterior. Para mais informações, consulte a secção seguinte, Pattern #2: Fan out/fan in.

> [!NOTE]
> O `context` objeto no JavaScript representa todo o [contexto da função](../functions-reference-node.md#context-object). Aceda ao contexto das `df` Funções Duráveis utilizando a propriedade no contexto principal.

---

### <a name="pattern-2-fan-outfan-in"></a><a name="fan-in-out"></a>Padrão #2: Ventilador out/ventilador em
...
O trabalho de fan-out é distribuído `F2` em várias instâncias da função. O trabalho é acompanhado através de uma lista dinâmica de tarefas. `Task.WhenAll`é chamado para esperar que todas as funções chamadas terminem. Em seguida, as `F2` saídas de função são agregadas a partir da lista de tarefas dinâmica e passadas para a `F3` função.

O checkpoint automático que `await` ocorre `Task.WhenAll` na chamada garante que uma possível colisão ou reinicialização a meio do caminho não requer reiniciar uma tarefa já concluída.
...
O trabalho de fan-out é distribuído `F2` em várias instâncias da função. O trabalho é acompanhado através de uma lista dinâmica de tarefas. `context.df.Task.all`A API é chamada para esperar que todas as funções chamadas terminem. Em seguida, as `F2` saídas de função são agregadas a partir da lista de tarefas dinâmica e passadas para a `F3` função.

O checkpoint automático que `yield` ocorre `context.df.Task.all` na chamada garante que uma possível colisão ou reinicialização a meio do caminho não requer reiniciar uma tarefa já concluída.
...
### <a name="pattern-3-async-http-apis"></a><a name="async-http"></a>Padrão #3: Async HTTP APIs
...
As Funções Duráveis fornecem **suporte incorporado** para este padrão, simplificando ou mesmo removendo o código que precisa escrever para interagir com execuções de funções de longa duração. Por exemplo, as amostras de arranque rápido das Funções Duráveis[(C#](durable-functions-create-first-csharp.md) e [JavaScript](quickstart-js-vscode.md)) mostram um simples comando REST que pode utilizar para iniciar novas instâncias de função orquestradora. Após o início de uma instância, a extensão expõe AApIs HTTP webhook que consultam o estado da função do orquestrador. 
...
### <a name="pattern-4-monitor"></a><a name="monitoring"></a>#4 de padrão: Monitor
...
Para criar o temporizador durável, ligue. `context.CreateTimer` A notificação `context.WaitForExternalEvent`é recebida por . Em `Task.WhenAny` seguida, é chamado para decidir se a escalada (o tempo de tempo acontece primeiro) ou processar a aprovação (a aprovação é recebida antes do intervalo).
...
Para criar o temporizador durável, ligue. `context.df.createTimer` A notificação `context.df.waitForExternalEvent`é recebida por . Em `context.df.Task.any` seguida, é chamado para decidir se a escalada (o tempo de tempo acontece primeiro) ou processar a aprovação (a aprovação é recebida antes do intervalo).
...
### <a name="pattern-6-aggregator-stateful-entities"></a><a name="aggregator"></a>Padrão #6: Agregador (entidades estatais)
...
As entidades duráveis também podem ser modeladas como classes em .NET. Este modelo pode ser útil se a lista de operações for fixada e se tornar grande. O exemplo seguinte é uma `Counter` implementação equivalente da entidade utilizando classes e métodos .NET.
