...
Em seguida, configuure o portão de preparação para esperar até que não estejam a decorrer orquestrações. Para obter mais informações, consulte [o controlo de implementação de desbloqueio utilizando portões](/azure/devops/pipelines/release/approvals/gates?view=azure-devops)
