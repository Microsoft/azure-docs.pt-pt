...
- Os padrões de injeção de dependência diferem consoante as suas funções C# funcionem [no processo](functions-dotnet-class-library.md) ou [fora do processo](dotnet-isolated-process-guide.md).  

> [!IMPORTANT]
> A orientação deste artigo aplica-se apenas às [funções da biblioteca da classe C#,](functions-dotnet-class-library.md)que funcionam em processo com o tempo de funcionamento. Este modelo de injeção de dependência personalizada não se aplica a [funções isoladas .NET,](dotnet-isolated-process-guide.md)o que permite executar funções .NET 5.0 fora do processo. O modelo de processo isolado .NET baseia-se em padrões regulares de injeção de dependência ASP.NET núcleo. Para saber mais, consulte [a injeção de Dependência](dotnet-isolated-process-guide.md#dependency-injection) no guia de processos isolado .NET.
...
- **Âmbito :** A vida útil do serviço de âmbito corresponde a uma execução útil da função. Os serviços de âmbito são criados uma vez por execução de função. Os pedidos posteriores para esse serviço durante a execução reutilizam a instância de serviço existente.
...
> [!WARNING]
> - Não adicione `AddApplicationInsightsTelemetry()` à recolha de serviços, que regista serviços que entram em conflito com os serviços prestados pelo meio ambiente.
> - Não registe o seu próprio `TelemetryConfiguration` ou se estiver a utilizar a funcionalidade De Insights de `TelemetryClient` Aplicação incorporada. Se precisar de configurar a sua própria `TelemetryClient` instância, crie uma através da injeção, `TelemetryConfiguration` como mostrado na [telemetria personalizada do Log nas funções C#](functions-dotnet-class-library.md?tabs=v2%2Ccmd#log-custom-telemetry-in-c-functions).
...
Para obter mais informações sobre os níveis de registo, consulte [os níveis de registo](configure-monitoring.md#configure-log-levels)de configuração .
...
## <a name="using-aspnet-core-user-secrets"></a>Usando ASP.NET segredos do utilizador do núcleo

Ao desenvolver localmente, ASP.NET Core fornece uma [ferramenta Secret Manager](/aspnet/core/security/app-secrets#secret-manager) que lhe permite armazenar informações secretas fora da raiz do projeto. Torna menos provável que os segredos sejam acidentalmente comprometidos com o controlo de fontes. Azure Functions Core Tools (versão 3.0.3233 ou posterior) lê automaticamente segredos criados pelo ASP.NET Core Secret Manager.

Para configurar um projeto .NET Azure Functions para utilizar segredos do utilizador, executar o seguinte comando na raiz do projeto.
...
Para aceder aos valores dos segredos dos utilizadores no código da aplicação da sua função, utilize `IConfiguration` ou `IOptions` .
...
Adicione fornecedores de configuração à `ConfigurationBuilder` propriedade de `IFunctionsConfigurationBuilder` . Para obter mais informações sobre a utilização de fornecedores de configuração, consulte [Configuração no ASP.NET Core](/aspnet/core/fundamentals/configuration/#configuration-providers).
