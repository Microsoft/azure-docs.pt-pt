...
Por predefinição, a função de gatilho HTTP está configurada para aceitar qualquer método HTTP. Também pode utilizar o URL predefinido, `http://<yourapp>.azurewebsites.net/api/<funcname>?code=<functionkey>` . Nesta secção, modifica a função de responder apenas aos pedidos do GET com `/api/hello` . 

1. Navegue até à sua função no portal do Azure. Selecione **Integração** no menu esquerdo e, em seguida, selecione **HTTP (req)** sob **o Gatilho**.
...
1. Utilize as definições do gatilho HTTP conforme especificado na tabela a seguir.
    ...
    Você não incluiu o prefixo do `/api` caminho base no modelo de rota, porque é tratado por um cenário global.

1. Selecione **Guardar**.

Para obter mais informações sobre personalizar as funções HTTP, consulte as [ligações HTTP funções do Azure.](https://docs.microsoft.com/azure/azure-functions/functions-bindings-http-webhook)

### <a name="test-your-api"></a>Testar a API

Em seguida, teste a sua função para ver como funciona com a nova superfície DaPI:
1. Na página de funções, selecione **Código + Teste** a partir do menu esquerdo.
...
1. Copie o URL para um novo separador do browser ou para o seu cliente REST preferido. 
   ...
   Os navegadores usam o GET por padrão.
 
1. Adicione parâmetros à corda de consulta no seu URL. 

   Por exemplo, `/api/hello/?name=John`.
 
1. Pressione Enter para confirmar que está funcionando. Devia ver a resposta,*"Olá John.".*
...
Na próxima secção, vais emergir a tua API através de um proxy. Os Proxies de Funções do Azure permitem reencaminhar pedidos para outros recursos. Define um ponto final http como com o gatilho HTTP. No entanto, em vez de escrever código para executar quando esse ponto final é chamado, fornece um URL para uma implementação remota. Fazê-lo permite-lhe compor múltiplas fontes de API numa única superfície API, o que é fácil para os clientes consumirem, o que é útil se pretender construir a sua API como microserviços.
...
Repita os passos para [Criar uma aplicação de funções](https://docs.microsoft.com/azure/azure-functions/functions-create-first-azure-function#create-a-function-app) para criar uma nova aplicação de funções na qual vai criar o proxy. O URL desta nova aplicação serve de fachada para a nossa API, e a aplicação de funções que estava a editar anteriormente serve como um backend.
...
1. Desloque-se para as definições de **Aplicação,** onde os pares chave/valor são armazenados, e crie uma nova definição com a chave `HELLO_HOST` . Defina o respetivo valor como o sistema anfitrião da sua aplicação de funções de back-end, como, por exemplo `<YourBackendApp>.azurewebsites.net`. Este valor faz parte do URL que copiou anteriormente ao testar a sua função HTTP. Vai fazer referência a esta definição na configuração mais à frente.
...
1. Navegue de volta para a sua aplicação de função frontal no portal.

1. No menu à esquerda, selecione **Proxies,** e, em seguida, **selecione Adicionar**. 

1. Na página **New Proxy,** utilize as definições na tabela a seguir e, em seguida, selecione **Criar**.
    ...
    Funções Azure Proxies não fornece o prefixo do `/api` caminho base, que deve ser incluído no modelo de rota. A `%HELLO_HOST%` sintaxe refere a definição de aplicação que criou anteriormente. O URL resolvido apontará para a sua função original.

1. Experimente o seu novo proxy copiando o URL proxy e testando-o no navegador ou com o seu cliente HTTP favorito:
    - Para uma função anónima: `https://YOURPROXYAPP.azurewebsites.net/api/remotehello?name="Proxies"` .
    - Para uma função com utilização de autorização: `https://YOURPROXYAPP.azurewebsites.net/api/remotehello?code=YOURCODE&name="Proxies"` .

## <a name="create-a-mock-api"></a>Criar uma API fictícia

Em seguida, usará um proxy para criar uma API falsa para a sua solução. Este proxy permite que o desenvolvimento do cliente progrida, sem precisar do backend totalmente implementado. Mais tarde em desenvolvimento, pode criar uma nova app de funções, que suporta esta lógica e redirecioná-la para ela.

Para criar esta Falsa API, vamos criar um novo proxy, desta vez usando o Editor de Serviços de [Aplicações.](https://github.com/projectkudu/kudu/wiki/App-Service-Editor) Para começar, navegue para a aplicação de funções no portal. Selecione **funcionalidades da Plataforma**, e em **ferramentas** de desenvolvimento encontre **o Editor de Serviço de Aplicações.** O Editor de Serviço de Aplicações abre num novo separador.

Selecione `proxies.json` no painel de navegação esquerdo. Este ficheiro armazena a configuração de todos os seus proxies. Se utilizar um dos métodos de implementação das [Funções,](https://docs.microsoft.com/azure/azure-functions/functions-continuous-deployment)mantenha este ficheiro no controlo de origem. Para saber mais sobre este ficheiro, veja [Configuração avançada de proxies](https://docs.microsoft.com/azure/azure-functions/functions-proxies#advanced-configuration).

Se acompanhou até agora, os seus proxies.json devem ser os seguintes:
...
Em seguida, você vai adicionar a sua aPI falsa. Substitua o ficheiro proxies.json pelo seguinte código:
