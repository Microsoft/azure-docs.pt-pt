...
A autenticação do serviço de aplicativo define cabeçalhos HTTP chamados `x-ms-client-principal-id` e `x-ms-client-principal-name` que contêm a ID e o nome da entidade de serviço do usuário autenticado, respectivamente. Você pode definir a propriedade `UserId` da associação com o valor de um dos cabeçalhos usando uma [expressão de associação](./functions-bindings-expressions-patterns.md): `{headers.x-ms-client-principal-id}` ou `{headers.x-ms-client-principal-name}`. 
...
Você pode enviar uma mensagem somente para conexões que foram autenticadas para um usuário, definindo a propriedade `UserId` da mensagem do Signalr.
