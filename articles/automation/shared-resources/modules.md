...
description: Este artigo diz como usar módulos PowerShell para ativar cmdlets em livros de execução e recursos DSC em configurações DSC.
services: automation
...
ms.locfileid: "83713486"
---
# <a name="manage-modules-in-azure-automation"></a>Utilizar módulos na Automatização do Azure

A Azure Automation utiliza vários módulos PowerShell para ativar os cmdlets em livros de execução e recursos DSC nas configurações dSC. Os módulos suportados incluem:
...
>[!NOTE]
>Certifique-se de importar apenas os módulos que os seus livros de execução e configurações DSC exigem. Não recomendamos importar o módulo Az raiz. Inclui muitos outros módulos que você pode não precisar, o que pode causar problemas de desempenho. Importar módulos individuais, como a Az.Compute, em vez disso.
...
A automatização não importa o módulo Raiz Az automaticamente em quaisquer contas de Automação novas ou existentes. Para mais informações sobre o trabalho com estes módulos, consulte [os módulos Migrating para Az](#migrate-to-az-modules).

> [!NOTE]
> Não recomendamos alterar módulos e livros de execução em contas de Automação utilizadas para a implementação dos [VMs Start/Stop durante](../automation-solution-vm-management.md) a funcionalidade off-hours.
...
A Azure Automation suporta o módulo interno `Orchestrator.AssetManagement.Cmdlets` do agente Log Analytics para windows, instalado por padrão. A tabela a seguir define os cmdlets internos. Estes cmdlets são projetados para serem usados em vez de cmdlets Azure PowerShell para interagir com recursos partilhados. Podem recuperar segredos de variáveis encriptadas, credenciais e ligações encriptadas.
...
Note que as cmdlets internas diferem no nome dos cmdlets Az e AzureRM. Os nomes internos da cmdlet não contêm palavras como `Azure` ou `Az` no substantivo, mas usam a palavra `Automation` . Recomendamos a sua utilização sobre a utilização de cmdlets Az ou AzureRM durante a execução do livro de corridas numa caixa de areia Azure ou num Trabalhador do Livro híbrido do Windows. Exigem menos parâmetros e funcionam no contexto do seu trabalho que já está em execução.
...
Esta secção diz como migrar para os módulos Az em Automação. Para mais informações, consulte [Migrate Azure PowerShell de AzureRM para Az](https://docs.microsoft.com/powershell/azure/migrate-from-azurerm-to-az?view=azps-3.7.0). 
...
### <a name="test-your-runbooks-and-dsc-configurations-prior-to-module-migration"></a>Teste os seus livros de execução e configurações de DSC antes da migração do módulo
...
### <a name="import-az-modules"></a>Módulos Az importadores
...
3. Na barra de pesquisa, introduza o nome do módulo (por exemplo, `Az.Accounts` ). 
...
Inclua uma sinopse, descrição e ajude uri para cada cmdlet no seu módulo. No PowerShell, pode definir informações de ajuda para cmdlets utilizando o `Get-Help` cmdlet. O exemplo que se segue mostra como definir uma sinopse e ajudar o URI num ficheiro de módulo **.psm1.**
  ...
  Fornecer esta informação mostra texto de ajuda através do `Get-Help` cmdlet na consola PowerShell. Este texto também é apresentado no portal Azure.
...
O exemplo do livro de corridas seguinte utiliza um ativo de conexão Contoso chamado `ContosoConnection` para aceder aos recursos da Contoso e devolver dados do serviço externo. Neste exemplo, os campos são mapeados para as `UserName` propriedades e propriedades de um objeto e depois `Password` `PSCredential` passados para o cmdlet.
...
Adicione, `[OutputType([<MyOutputType>])]` onde é um tipo `MyOutputType` válido. Para saber mais sobre `OutputType` , consulte [funções OutputTypeAttribute](/powershell/module/microsoft.powershell.core/about/about_functions_outputtypeattribute). O seguinte código é um exemplo de adição `OutputType` a um cmdlet:
...
Faça todos os cmdlets no seu módulo apátridas. Vários trabalhos de livro de corridas podem simultaneamente funcionar no mesmo processo e caixa de `AppDomain` areia. Se houver algum Estado partilhado nesses níveis, os empregos podem afetar-se mutuamente. Este comportamento pode levar a problemas intermitentes e difíceis de diagnosticar. Aqui está um exemplo do que não fazer:
...
Certifique-se de que todos os ficheiros do módulo têm caminhos com menos de 140 caracteres. Quaisquer caminhos com mais de 140 caracteres de comprimento causam problemas com a importação de livros de execução. A automação não pode importar um ficheiro com tamanho de caminho superior a 140 caracteres para a sessão PowerShell com `Import-Module` .
...
Também pode utilizar o mesmo cmdlet para importar um módulo da PowerShell Gallery diretamente. Certifique-se de agarrar `ModuleName` e da `ModuleVersion` Galeria [PowerShell.](https://www.powershellgallery.com)
...
1. Vá procurar https://www.powershellgallery.com o módulo para importar.
...
* [Escrever um módulo PowerShell do Windows](https://docs.microsoft.com/powershell/scripting/developer/module/writing-a-windows-powershell-module?view=powershell-7)
