...
Pode implantar e embarcar um VM para a Configuração do Estado da Automação Azure utilizando modelos de Gestor de Recursos Azure. Consulte o [Servidor gerido pelo serviço de Configuração do Estado Desejado](https://azure.microsoft.com/resources/templates/101-automation-configuration/) para um modelo de exemplo que a bordo de um VM existente para configuração do Estado de Automação Azure. Se estiver a gerir um Conjunto de Escala VM, consulte o modelo de exemplo na Configuração do Conjunto de [Escala Seletiva VM gerida pela Automação Azure](https://azure.microsoft.com/resources/templates/201-vmss-automation-dsc/).
...
## <a name="onboarding-physicalvirtual-windows-machines-on-premises-or-in-a-cloud-other-than-azure-including-aws-ec2-instances"></a>No embarque, máquinas de janelas físicas/virtuais no local, ou numa nuvem diferente do Azure (incluindo instâncias AWS EC2)

Pode embarcar servidores do Windows que executam no local ou em outros ambientes de nuvem para a Configuração do Estado da Automação Azure. Os servidores devem ter [acesso de saída ao Azure](automation-dsc-overview.md#network-planning).
...
1. Se não conseguir aplicar as configurações do DSC PowerShell remotamente, copie a pasta de **configurações** para as máquinas que está a embarcar. Em seguida, `Set-DscLocalConfigurationManager` adicione código para ligar localmente sobre as máquinas.
...
## <a name="onboarding-physicalvirtual-linux-machines-on-premises-or-in-a-cloud-other-than-azure"></a>Onboarding máquinas linux físicas/virtuais no local, ou numa nuvem diferente de Azure
...
2. Ligar-se ao Gestor de Recursos Azure usando`Connect-AzAccount`
...
Pode voltar a registar um nó da mesma forma que registou o nó inicialmente, utilizando qualquer um dos métodos de embarque descritos neste documento. Não é necessário desregistar um nó da Configuração do Estado da Automação Azure antes de o reregistar.
