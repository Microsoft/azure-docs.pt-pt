...
* Os tipos PowerShell e os tipos powerShell Workflow não podem `Start-AzAutomationRunbook`ligar uns aos outros em linha, e devem usar .
...
> [!IMPORTANT]
> Se o seu livro de corridas `Start-AzAutomationRunbook` invocar um `Wait` livro de crianças com o cmdlet com o parâmetro e o livro de crianças produzir um resultado de objeto, a operação pode encontrar um erro. Para contornar o erro, consulte os [livros infantis com saída](troubleshoot/runbooks.md#child-runbook-object) de objetos para aprender a implementar a lógica para fazer sondagem para os resultados utilizando o cmdlet [Get-AzAutomationJobOutputRecord.](/powershell/module/az.automation/get-azautomationjoboutputrecord)
...
A saída do livro de crianças não regressa ao livro de corridas dos pais de forma fiável devido ao timing. Além disso, variáveis `$WarningPreference`como, `$VerbosePreference`e outras, podem não ser propagadas aos livros infantis. Para evitar estes problemas, pode iniciar os livros `Start-AzAutomationRunbook` de `Wait` execução para crianças como trabalhos separados de automação utilizando com o parâmetro. Esta técnica bloqueia o livro dos pais até que o livro de corridas da criança esteja completo.

Se não quiser que o livro de corridas dos pais seja bloqueado `Start-AzAutomationRunbook` à `Wait` espera, pode iniciar o livro de corridas para crianças usando sem o parâmetro. Neste caso, o seu livro de execução deve utilizar [o Get-AzAutomationJob](/powershell/module/az.automation/get-azautomationjob) para aguardar a conclusão do trabalho. Também deve utilizar [get-AzAutomationJobOutput](/powershell/module/az.automation/get-azautomationjoboutput) e [Get-AzAutomationJobOutputOutputRecord](/powershell/module/az.automation/get-azautomationjoboutputrecord) para recuperar os resultados.
