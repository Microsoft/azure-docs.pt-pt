---
title: Configuração do Estado da Automação Azure (DSC)
description: Este artigo fornece informações sobre a resolução de problemas da Configuração do Estado da Automação Azure (DSC).
services: automation
...
ms.locfileid: "81679313"
---
# <a name="troubleshoot-issues-with-azure-automation-state-configuration-dsc"></a>Problemas de resolução de problemas com configuração do Estado da Automação Azure (DSC)

Este artigo fornece informações sobre problemas de resolução de problemas que surgem durante a compilação ou implementação de configurações na Configuração do Estado da Automação Azure (DSC).
...
A Configuração do Estado da Automação Azure (DSC) é construída com base na configuração do Estado desejado pela PowerShell (DSC). Pode encontrar a documentação para a língua DSC e sintaxe nos [Docs Docs Docs PowerShell DSC](https://docs.microsoft.com/powershell/scripting/overview).
...
Pode instalar `xDscDiagnostics` o módulo na sua máquina local utilizando as instruções encontradas no [módulo de versão estável](https://github.com/PowerShell/xDscDiagnostics#install-the-stable-version-module).

Para instalar `xDscDiagnostics` o módulo na sua máquina Azure, utilize o [Invoke-AzVMRunCommand](https://docs.microsoft.com/powershell/module/az.compute/invoke-azvmruncommand?view=azps-3.7.0). Também pode utilizar a opção **de comando Run** no portal Azure seguindo os [passos encontrados nos scripts Run PowerShell no seu Windows VM com Comando de Execução](../../virtual-machines/windows/run-command.md).
...
Verifique se a sua máquina tem acesso aos pontos finais adequados para o DSC e tente novamente. Para obter uma lista de portas e endereços necessários, consulte [o planeamento](../automation-dsc-overview.md#network-planning) da rede
...
Utilize os passos abaixo para voltar a registar o nó DSC em falha.

Passo 1 - Desregistre o nó.

1. No portal Azure, navegue para Contas de**Automação** **Doméstica** -> -> (a sua conta de Automação) -> **configuração do Estado (DSC)**.
2. Selecione **Nós**e clique no nó com problemas.
3. Clique **em Não registar** para não registar o nó.

Passo 2 - Desinstale a extensão DSC do nó.

1. No portal Azure, navegue para **home** -> **virtual machine** -> (nó falhado) -> **Extensões**.
2. Selecione **Microsoft.Powershell.DSC,** a extensão PowerShell DSC.
3. Clique em **desinstalar** para desinstalar a extensão.

Passo 3 - Retire todos os certificados maus ou caducados do nó.
...
Passo 4 - Re-registe o nó em queda.

1. No portal Azure, navegue para Contas de**Automação** **Domiciliária** -> -> (a sua conta de Automação) -> **configuração do Estado (DSC)**
2. Selecione **Nóesos**.
...
* Pode atribuir uma configuração do nó a um nó utilizando o portal Azure ou com um cmdlet PowerShell.
  ...
  * No portal Azure, navegue para Contas de**Automação** **Doméstica** -> -> (a sua conta de Automação) -> **configuração do Estado (DSC)** e, em seguida, selecione um nó e clique na **configuração do nó de atribuição**.
...
* Se estiver a passar o [ConfigurationData](../automation-dsc-compile.md) ao compilar a configuração, certifique-se de que está a passar os valores que a configuração espera dos dados de configuração.
...
Siga as instruções em [questões e limitações conhecidas da DSC](https://docs.microsoft.com/powershell/scripting/wmf/known-issues/known-issues-dsc).
...
Certifique-se de que `ConfigurationData` passa `PSDscAllowPlainTextPassword` no correto para se definir fiel a cada configuração do nó que é mencionada na configuração. Consulte a compilação de [configurações dSC na configuração do Estado da Automação Azure](../automation-dsc-compile.md).

## <a name="scenario-failure-processing-extension-error-when-onboarding-from-dsc-extension"></a><a name="failure-processing-extension"></a>Cenário: Erro de "extensão de processamento de falhas" ao embarcar a partir da extensão do DSC

### <a name="issue"></a>Problema

Ao embarcar com uma extensão DSC, ocorre uma falha que contém o erro:
...
## <a name="scenario-one-or-more-errors-occurred-error-when-registering-a-node-using-powershell"></a><a name="cross-subscription"></a>Cenário: Erro de "Um ou mais erros ocorreram" ao registar um nó usando powerShell
...
* Windows - [Máquinas de Janelas Físicas/Virtuais no local, ou numa nuvem diferente do Azure/AWS](../automation-dsc-onboarding.md#onboarding-physicalvirtual-windows-machines).
...
## <a name="scenario-node-configuration-names-that-overlap-can-result-in-bad-release"></a><a name="compilation-node-name-overlap"></a>Cenário: Nomes de configuração do nó que se sobrepõem podem resultar em mau lançamento
...
A melhor sucinta é compilar localmente ou num oleoduto CI/CD e carregar os ficheiros MOF de configuração do nó diretamente para o serviço. Se a compilação no serviço for um requisito, a próxima melhor suposição é dividir os trabalhos de compilação para que não haja sobreposição de nomes.
