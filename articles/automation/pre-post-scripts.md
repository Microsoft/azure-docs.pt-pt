...
Ao clicar na execução da implantação de atualização, você receberá detalhes adicionais dos pré e pós-scripts. Um link para a origem do script no momento da execução é fornecido.
...
|SoftwareUpdateConfigurationSettings. Duration     | A duração máxima da implantação de atualização é executada como `PT[n]H[n]M[n]S` de acordo com o ISO8601, também chamado de "janela de manutenção"          |
...
Para interagir com computadores não Azure, um runbook pai é executado no contexto do Azure. Esse runbook chama um runbook filho com o cmdlet [Start-AzureRmAutomationRunbook](/powershell/module/azurerm.automation/start-azurermautomationrunbook) . Você deve especificar o parâmetro `-RunOn` e fornecer o nome do Hybrid Runbook Worker em que o script será executado. Para obter um exemplo, consulte o exemplo de runbook [Gerenciamento de atualizações-executar script localmente](https://gallery.technet.microsoft.com/Update-Management-Run-6949cc44).
...
* Você não pode passar um booliano, objetos ou matrizes para parâmetros ao usar pré e pós-scripts. O runbook falhará. Para obter uma lista completa dos tipos com suporte, consulte [parâmetros](#passing-parameters).
