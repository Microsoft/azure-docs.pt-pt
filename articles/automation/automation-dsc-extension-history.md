...
description: Este artigo partilha informações de histórico da versão para a extensão de Configuração do Estado Desejado (DSC) em Azure.
ms.date: 02/17/2021
...
A [extensão](../virtual-machines/extensions/dsc-overview.md) VM de Configuração do Estado Azure Desired (DSC) é atualizada conforme necessário para suportar melhorias e novas capacidades entregues pelo Azure, Windows Server e o Windows Management Framework (WMF) que inclui o Windows PowerShell.
...
- **Ambiente:**
  ...
  - Azure Government
- **Observações:** Esta versão inclui uma correção para binários não assinados com a extensão VM do Windows.
...
- **Ambiente:**
  ...
  - Azure China Vianet 21
  - Azure Government
- **Observações:** Esta versão utiliza o DSC como incluído no Windows Server 2016; para outros OSs do Windows, instala o Quadro de Gestão do [Windows 5.1](https://devblogs.microsoft.com/powershell/wmf-5-1-releasing-january-2017/) (instalar o WMF requer um reboot). Para o Nano Server, a função DSC é instalada no VM.
...
> [!WARNING]
> As versões 2.4 a 2.13 utilizam o WMF 5.0 Public Preview, cujos certificados de assinatura expiraram em agosto de 2016.
> Para mais informações sobre este assunto, consulte o seguinte [artigo de blog.](https://devblogs.microsoft.com/powershell/azure-dsc-extension-versions-2-4-up-to-2-13-will-retire-in-august/)
...
- **Observações:** Esta versão utiliza o DSC como incluído no Windows Server 2016; para outros OSs do Windows, instala o Quadro de Gestão do [Windows 5.1](https://devblogs.microsoft.com/powershell/wmf-5-1-releasing-january-2017/) (instalar o WMF requer um reboot). Para o Nano Server, a função DSC é instalada no VM.
- **Novas funcionalidades:**
  - Após a aplicação do GitHub do TLS 1.2, não pode embarcar numa Configuração de Estado VM para Azure Automation usando modelos de Gestor de Recursos DIY disponíveis no Azure Marketplace, ou utilizar a extensão DSC para recuperar quaisquer configurações hospedadas no GitHub. Um erro semelhante ao seguinte durante a implementação da extensão é devolvido:
  ...
  - Na nova versão de extensão, o TLS 1.2 é agora aplicado. Durante a implementação da extensão, se já estiver especificado `AutoUpgradeMinorVersion = true` no modelo de Gestor de Recursos, a extensão é auto-actualizada para 2.75. Para atualizações manuais, especifique `TypeHandlerVersion = 2.75` no seu modelo de Gestor de Recursos.
...
- **Observações:** Esta versão utiliza o DSC como incluído no Windows Server 2016; para outros OSs do Windows, instala o Quadro de Gestão do [Windows 5.1](https://devblogs.microsoft.com/powershell/wmf-5-1-releasing-january-2017/) (instalar o WMF requer um reboot). Para o Nano Server, a função DSC é instalada no VM.
- **Novas funcionalidades:**
  - As correções de & melhorias que simplificam usando a Configuração do Estado da Automação Azure no portal e com um modelo de Gestor de Recursos. Para obter mais informações, consulte o [Script de Configuração Padrão](../virtual-machines/extensions/dsc-overview.md) na documentação de extensão DSC.
...
- **Observações:** Esta versão utiliza o DSC como incluído no Windows Server 2016; para outros OSs do Windows, instala o Quadro de Gestão do [Windows 5.1](https://devblogs.microsoft.com/powershell/wmf-5-1-releasing-january-2017/) (instalar o WMF requer um reboot). Para o Nano Server, a função DSC é instalada no VM.
- **Novas funcionalidades:**
  - A extensão DSC suporta agora o WMF 5.1.
...
- **Observações:** Esta versão utiliza o DSC como incluído na pré-visualização técnica do Windows Server 2016; para outros OSs do Windows, instala o [Windows Management Framework 5.0 RTM](https://devblogs.microsoft.com/powershell/windows-management-framework-wmf-5-0-rtm-is-now-available-via-the-microsoft-update-catalog/) (instalar o WMF requer um reboot).
- **Novas funcionalidades:**
  - A extensão DSC está agora disponível no Azure China Vianet 21. Esta versão contém correções para executar a extensão no Azure China Vianet 21.
...
- **Novas funcionalidades:**
  ...
  - Corrija para o problema intermitente onde a extensão para de processar a configuração após um reboot. Isto estava a fazer com que a extensão do CDS permanecesse em estado de "transição".
  - Foram adicionadas pequenas outras correções e melhorias.
...
- **Novas funcionalidades:**
  ...
  - Nova propriedade nas definições de extensão DSC. `ForcePullAndApply` em AdvancedOptions é adicionado para ativar a extensão DSC decretar configurações DSC quando o modo de atualização é Pull (em oposição ao modo push predefinido). Para obter mais informações sobre as definições de extensão do DSC, consulte [este blog](https://devblogs.microsoft.com/powershell/arm-dsc-extension-settings/).
...
- **Novas funcionalidades:**
  ...
  - Se já instalou a versão 2.14 e está a desatenção de um dos dois problemas acima referidos, tem de realizar estes passos manualmente. Numa sessão elevada da PowerShell executam os seguintes comandos:
...
- **Novas funcionalidades:**
  - Usa a WMF RTM.
  - Permite a recolha de dados de forma a melhorar a qualidade da extensão do DSC. Para mais informações, consulte este [artigo de blog.](https://devblogs.microsoft.com/powershell/azure-dsc-extension-data-collection-2/)
  - Fornece um formato de definições atualizado para a extensão num modelo de Gestor de Recursos. Para mais informações, consulte este [artigo de blog.](https://devblogs.microsoft.com/powershell/arm-dsc-extension-settings/)
...
- Para outras funcionalidades e recursos que possa gerir com o PowerShell DSC, consulte a [galeria PowerShell](https://www.powershellgallery.com/packages?q=DscResource&x=0&y=0).
