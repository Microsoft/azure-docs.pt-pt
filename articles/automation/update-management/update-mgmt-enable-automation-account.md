...
Este artigo descreve como pode utilizar a sua conta Demômes para ativar a funcionalidade [de Gestão de Atualização](update-mgmt-overview.md) para VMs no seu ambiente. Para ativar os VMs Azure em escala, tem de ativar um VM existente utilizando a Gestão de Atualização.
...
3. Para ativar a Gestão de Atualização de todas as máquinas disponíveis, selecione **Ative em todas as máquinas disponíveis** na página 'Gerir Máquinas'. Esta ação desativa o controlo para adicionar máquinas individualmente. Esta tarefa adiciona todos os nomes das máquinas que reportam ao espaço de trabalho ao grupo de computador que guardou a consulta de pesquisa. Quando selecionado, esta ação desativa o botão **'Gerir máquinas'.**
...
5. Se necessário, pode adicionar as configurações de alcance de volta, adicionando novamente as primeiras pesquisas guardadas. Para obter mais informações, consulte [o âmbito de implementação da Gestão de Atualização de Limites](update-mgmt-scope-configuration.md).
