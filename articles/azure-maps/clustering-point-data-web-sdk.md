...
Ativar o agrupamento na `DataSource` classe definindo a `cluster` opção para `true` . `clusterRadius`Desapase para selecionar pontos próximos e combine-os num cluster. O valor dos `clusterRadius` pixéis está em pixels. Utilize `clusterMaxZoom` para especificar um nível de zoom para desativar a lógica de agrupamento. Aqui está um exemplo de como permitir o agrupamento numa fonte de dados.
