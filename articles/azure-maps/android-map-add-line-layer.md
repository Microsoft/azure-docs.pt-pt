---
title: Adicione uma camada de linha aos mapas Android | Microsoft Azure Maps
description: Saiba como adicionar linhas aos mapas. Veja exemplos que usam o Azure Maps Android SDK para adicionar camadas de linha aos mapas e para personalizar linhas com símbolos e gradientes de cores.
author: rbrundritt
ms.author: richbrun
ms.date: 2/26/2021
ms.topic: conceptual
...
ms.locfileid: "102097453"
---
# <a name="add-a-line-layer-to-the-map-android-sdk"></a>Adicione uma camada de linha ao mapa (Android SDK)
...
> [!TIP]
> As camadas de linha por padrão tornarão as coordenadas dos polígonos, bem como as linhas numa fonte de dados. Para limitar a camada de modo a que apenas torne as características de geometria lineString, deite `filter` a opção da camada para `eq(geometryType(), "LineString")` . Se também pretender incluir funcionalidades MultiLineString, deite `filter` a opção da camada para `any(eq(geometryType(), "LineString"), eq(geometryType(), "MultiLineString"))` .
...
## <a name="add-a-line-layer"></a>Adicionar uma camada de linhas

O código que se segue mostra como criar uma linha. Adicione a linha a uma fonte de dados e, em seguida, torne-a com uma camada de linha utilizando a `LineLayer` classe.
...
A imagem que se segue mostra o código acima, tornando uma linha numa camada de linha.

![Mapa com uma linha renderizada usando camada de linha](media/android-map-add-line-layer/android-line-layer.png)

## <a name="data-driven-line-style"></a>Estilo de linha orientado por dados

O código a seguir cria duas funcionalidades de linha e adiciona um valor limite de velocidade como propriedade a cada linha. Uma camada de linha usa uma cor de expressão estilo de unidade de dados as linhas com base no valor limite de velocidade. Uma vez que os dados da linha se sobrepõem ao longo das estradas, o código abaixo adiciona a camada de linha abaixo da camada de etiqueta, de modo que as etiquetas de estrada ainda podem ser claramente lidas.
...
A imagem que se segue mostra o código acima mostrando uma linha renderizada utilizando uma cor de curso de gradiente.

![Mapa com uma linha renderizada como um caminho de gradiente em uma camada de linha](media/android-map-add-line-layer/android-line-layer-gradient.jpg)

## <a name="add-symbols-along-a-line"></a>Adicione símbolos ao longo de uma linha

Esta amostra mostra como adicionar ícones de seta ao longo de uma linha no mapa. Ao utilizar uma camada de símbolo, desate a `symbolPlacement` opção para `SymbolPlacement.LINE` . Esta opção irá tornar os símbolos ao longo da linha e rodar os ícones (0 graus = à direita).
...
| ![Imagem de ícone de seta roxa](media/android-map-add-line-layer/purple-arrow-right.png)|
|:-----------------------------------------------------------------------:|
|                           `purple_arrow_right.png`                       |

A imagem abaixo mostra o código acima mostrando uma linha com ícones de seta exibidos ao longo dela.

![Mapa com linhas estilo data-drive com setas renderizadas em uma camada de linha](media/android-map-add-line-layer/android-symbols-along-line-path.png)
