...
Para aprender as portas específicas necessárias para o seu sistema, utilize o seguinte comando ``rpcinfo``. Este comando abaixo lista as portas e forma os resultados relevantes numa tabela. (Utilize o endereço IP do seu sistema no lugar do *termo <storage_IP>*
...
O ``/ifs/accounting/payroll`` de exportação é filho de ``/ifs/accounting``, e ``/ifs/accounting`` é em si uma criança de ``/ifs``.

Se adicionar o ``payroll`` exportação como um alvo de armazenamento de cache HPC, a cache realmente monta ``/ifs/`` e acessa o diretório de folha de pagamento a partir daí. Assim, o Azure HPC Cache necessita de acesso raiz a ``/ifs`` para aceder ao ``/ifs/accounting/payroll`` exportação.
...
Um sistema NAS com exportações hierárquicas pode dar diferentes cabos de ficheiro para o mesmo ficheiro se o ficheiro for recuperado de diferentes exportações. Por exemplo, um cliente poderia montar ``/ifs/accounting`` e aceder ao ficheiro ``payroll/2011.txt``. Outro cliente monta ``/ifs/accounting/payroll`` e acede ao ficheiro ``2011.txt``. Dependendo da forma como o sistema de armazenamento atribui cabos de ficheiros, estes dois clientes poderão receber o mesmo ficheiro com diferentes cabos de ficheiro (um para ``<mount2>/payroll/2011.txt`` e outro para ``<mount3>/2011.txt``).
...
* Se o seu VPN permitir comandos de ping, pode testar o envio de um pacote de tamanho completo.
  ...
  * ``-s 1472`` - Desloque o tamanho da carga útil para 1472 bytes. Esta é a carga útil de tamanho máximo para um pacote de 1500 bytes após a contabilização da sobrecarga ethernet.
