...
> [!NOTE]
> Por padrão, a Azure HPC Cache esmaga o acesso à raiz. Leia [configurar as definições adicionais de cache](configuration.md#configure-root-squash) para mais detalhes.
...
O NAS deve enumerar as suas exportações quando o Cache Azure HPC o questiona.

Na maioria dos sistemas de armazenamento NFS, pode testá-lo enviando a seguinte consulta a um cliente Linux: ``showmount -e <storage IP address>``

Utilize um cliente Linux da mesma rede virtual que o seu cache, se possível.

Se esse comando não enumerar as exportações, a cache terá problemas em ligar-se ao seu sistema de armazenamento. Trabalhe com o seu fornecedor NAS para permitir a listagem de exportação.
