...
O modelo Yeoman gera ficheiros no diretório atual. Assim, terá de criar um diretório.
...
1. Executar o seguinte comando substituindo o espaço reservado. Para este exemplo, é utilizado um nome de diretório de `GeneratorDocSample`.
...
1. Siga as instruções no ecrã para fornecer as seguintes informações:
    ...
    - Projeto do **módulo Terraform Nome** - Um valor de `doc-sample-module` é utilizado para o exemplo.

        ![Nome do projeto](media/terraform-vscode-module-generator/ymg-project-name.png)       


    - **Pretende incluir o ficheiro de imagem do Docker?** - Insira `y`. Se selecionar `n`, o código de módulo gerado suportará o funcionamento apenas no modo nativo.
...
- `outputs.tf` - Define o que o módulo é saída. Aqui, é o valor devolvido por `random_shuffle`, que é um módulo terraforme embutido.
...
- `test`
    ...
    - Os ensaios de ponta a ponta tentam fornecer todos os itens definidos no âmbito `fixture`. Os resultados do ficheiro `template_output.go` são comparados com os valores esperados pré-definidos.
...
1. A partir do pedido de comando, introduza `docker image ls` para ver o módulo criado `terra-mod-example` listado.

    ![Lista contendo o novo módulo](media/terraform-vscode-module-generator/ymg-repository-results.png)

1. Introduza `docker run -it terra-mod-example /bin/sh`. Depois de dirigir o comando `docker run`, está no ambiente do Docker. Nessa altura, pode descobrir o ficheiro utilizando o comando `ls`.
...
1. Neste ponto, a Cloud Shell já terá configurado gopath nas suas variáveis ambientais para si. Para ver o caminho, entre `go env`.

1. Crie o diretório $GOPATH, se já não existir: Enter `mkdir ~/go`.
...
1. Crie um diretório para manter o módulo Terraform substituindo o espaço reservado. Para este exemplo, é utilizado um nome de diretório de `my-module-name`.
...
1. Executar o seguinte comando e seguir as instruções. Quando lhe perguntamos se quer criar os ficheiros Does, insere-se `N`.
