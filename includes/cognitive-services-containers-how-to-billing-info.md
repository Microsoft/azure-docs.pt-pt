...
---
As consultas ao contentor são faturadas no nível de preços do recurso Azure que é usado para o `ApiKey`.
...
O comando <a href="https://docs.docker.com/engine/reference/commandline/run/" target="_blank">`docker run` <span class="docon docon-navigate-external x-hidden-focus"></span> </a> iniciará o contentor quando as três opções seguintes forem fornecidas com valores válidos:

| Opção | Descrição |
|--------|-------------|
| `ApiKey` | A chave DaPI do recurso dos Serviços Cognitivos que é usado para rastrear informações de faturação.<br/>O valor desta opção deve ser definido para uma chave API para o recurso provisionado especificado em `Billing`. |
