...
1. Definir o **data de início**. Por predefinição, uma revisão de acesso ocorre uma vez, é iniciada ao mesmo tempo, que é criado e termina dentro de um mês. Pode alterar o início e datas de término para ter um acesso rever início no futuro e pela última vez no entanto, o número de dias que pretende.
    ...
    ![Criar uma revisão de acesso - início e datas de término](./media/active-directory-privileged-identity-management-access-reviews/start-end-dates.png)
...
1. Na **utilizadores** secção, selecione uma ou mais funções que pretende rever a associação de.
    ...
    ![Criar uma revisão de acesso - os utilizadores](./media/active-directory-privileged-identity-management-access-reviews/users.png)
    ...
    ![Criar uma revisão de acesso - associação de função de revisão](./media/active-directory-privileged-identity-management-access-reviews/review-membership.png)
    ...
    ![Criar uma revisão de acesso - associação de função de revisão](./media/active-directory-privileged-identity-management-access-reviews/review-membership-azure-resource-roles.png)
...
1. Para especificar o que acontece após a conclusão de uma revisão, expanda o **nas definições de conclusão** secção.
    ...
    ![Após as definições de conclusão](./media/active-directory-privileged-identity-management-access-reviews/upon-completion-settings.png)
...
1. Para especificar definições adicionais, expanda o **definições avançadas** secção.
    ...
    ![Definições avançadas](./media/active-directory-privileged-identity-management-access-reviews/advanced-settings.png)
