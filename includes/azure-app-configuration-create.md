...
1. Na **configuração** > da aplicação**Criar** painel, introduza as seguintes definições:
    ...
    |---|---|---|
    | **Nome do recurso** | Nome globalmente exclusivo | Introduza um nome único de recurso para utilizar para o recurso da loja de configuração de aplicações. O nome deve ser uma corda entre 5 e 50 `-` caracteres e conter apenas números, letras e o personagem. O nome não pode começar `-` ou terminar com o personagem.  |
    | **Assinatura** | A sua subscrição | Selecione a subscrição Azure que pretende utilizar para testar a Configuração da Aplicação. Se a sua conta tiver apenas uma subscrição, é automaticamente selecionada e a lista **de subscrição** não é apresentada. |
    ...
    | **Localização** | *E.U.A. Central* | Utilize o **Local** para especificar a localização geográfica em que a sua loja de configuração de aplicações está hospedada. Para obter o melhor desempenho, crie o recurso na mesma região que outros componentes da sua aplicação. |
    | **Nível de preços** | *Gratuito* | Selecione o nível de preços desejado. Para mais detalhes, consulte a página de preços da Configuração da [Aplicação](https://azure.microsoft.com/pricing/details/app-configuration/).
...
1. Depois de terminar a implementação, selecione**Teclas**de acesso **definições** > . Tome nota da cadeia de ligação principal de leitura. Mais tarde, utilizará esta cadeia de ligação para configurar a sua aplicação para comunicar com a loja de configuração de aplicações que criou.
