---
author: cephalin
ms.service: app-service
ms.topic: include
ms.date: 11/03/2016
ms.author: cephalin
ms.openlocfilehash: 0046b7fcad663187f660d6a65652eab5884eee23
ms.sourcegitcommit: 877491bd46921c11dd478bd25fc718ceee2dcc08
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 07/02/2020
ms.locfileid: "67184423"
---
> [!NOTE]
> Os planos de hospedagem do Serviço de Aplicações Gratuitos e Partilhados (pré-visualização) são níveis base que funcionam nas mesmas máquinas virtuais Azure que outras aplicações do Serviço de Aplicações. Algumas aplicações podem pertencer a outros clientes. Estas camadas destinam-se a ser utilizadas apenas para efeitos de desenvolvimento e teste.
>
