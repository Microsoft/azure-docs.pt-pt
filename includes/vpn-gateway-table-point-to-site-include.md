---
title: ficheiro de inclusão
description: ficheiro de inclusão
services: vpn-gateway
author: cherylmc
ms.service: vpn-gateway
ms.topic: include
ms.date: 04/26/2019
ms.author: cherylmc
ms.custom: include file
ms.openlocfilehash: 59ba66fb013a4ea6aebd40164ca252447876053d
ms.sourcegitcommit: 877491bd46921c11dd478bd25fc718ceee2dcc08
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 07/02/2020
ms.locfileid: "67184136"
---
**Autenticação de certificado nativo Azure**

**Modelo/método de implementação** | **Portal do Azure** | **PowerShell** |
|---|---|---|
| Resource Manager | [Tutorial](../articles/vpn-gateway/vpn-gateway-howto-point-to-site-resource-manager-portal.md) | [Tutorial](../articles/vpn-gateway/vpn-gateway-howto-point-to-site-rm-ps.md)|
| Clássico | [Tutorial](../articles/vpn-gateway/vpn-gateway-howto-point-to-site-classic-azure-portal.md) | Suportado |

**Autenticação RADIUS**

**Modelo/método de implementação** | **Portal do Azure** | **PowerShell** |
|---|---|---|
| Resource Manager | Suportado | [Tutorial](../articles/vpn-gateway/point-to-site-how-to-radius-ps.md)|
| Clássico | Não suportado | Não suportado |