...
2. Navegue na pasta `%programfiles%\Microsoft Azure Active Directory Connect`.
...
1. Executar powerShell como administrador. Na PowerShell, ligue para `New-AzureADSSOAuthenticationContext`. Quando solicitado, insira as credenciais de administrador global do seu inquilino.
2. Ligue para `Get-AzureADSSOStatus`. Este comando fornece-lhe a lista de florestas de Diretório Ativo (veja a lista de "Domínios") em que esta funcionalidade foi ativada.
...
1. Ligue para `Enable-AzureADSSOForest`. Quando solicitado, introduza as credenciais do administrador de domínio para a floresta de Diretório Ativo pretendida.
...
Para ativar o recurso no seu inquilino, ligue para `Enable-AzureADSSO -Enable $true`.
