...
| A migração não é suportada para rede virtual {vnet-name} porque tem implementações paaS não gateway. | Este erro ocorre quando tem implementações paaS não gateway, tais como Gateway de aplicação ou serviços de Gestão API que estão ligados à Rede Virtual.|
