---
title: incluir ficheiro
description: incluir ficheiro
services: storage
author: normesta
ms.service: storage
ms.topic: include
ms.date: 03/13/2020
ms.author: normesta
ms.custom: include file
ms.openlocfilehash: d66c8a59747fc4837d727ed3d3c7f6b1722fa7df
ms.sourcegitcommit: 849bb1729b89d075eed579aa36395bf4d29f3bd9
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 04/28/2020
ms.locfileid: "79365372"
---
> [!NOTE]
> Esta funcionalidade ainda não é suportada em contas que tenham um espaço hierárquico (Azure Data Lake Storage Gen2). Para saber mais, consulte as funcionalidades de [armazenamento blob disponíveis no Azure Data Lake Storage Gen2](https://docs.microsoft.com/azure/storage/blobs/data-lake-storage-supported-blob-storage-features).

