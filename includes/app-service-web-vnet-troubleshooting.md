...
* Tem permissões suficientes para definir a delegação na sub-rede de integração? Durante a configuração regional de Integração VNet, a sua sub-rede de integração é delegada no Microsoft.Web. O UI de Integração VNet delega automaticamente a sub-rede para o Microsoft.Web. Se a sua conta não tiver permissões de networking suficientes para definir a delegação, precisará de alguém que possa definir atributos na sua sub-rede de integração para delegar a sub-rede. Para delegar manualmente a sub-rede de integração, vá à sub-rede Azure Virtual Network UI e deslote a delegação para Microsoft.Web.
