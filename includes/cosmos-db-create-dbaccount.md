...
1. Na página **criar conta de Azure Cosmos DB** , insira as configurações básicas para a nova conta do Azure Cosmos. 
    ...
    | API|Núcleo (SQL)|A API determina o tipo de conta a criar. O Azure Cosmos DB fornece cinco APIs: Core (SQL) e MongoDB para dados de documento, Gremlin para dados de grafo, tabela do Azure e Cassandra. Atualmente, tem de criar uma conta separada para cada API. <br><br>Selecione **núcleo (SQL)** para criar um banco de dados de documento e uma consulta usando a sintaxe SQL. <br><br>[Saiba mais sobre a API de SQL](../articles/cosmos-db/documentdb-introduction.md).|
    | Localização|Selecione a região mais próxima dos seus utilizadores|Selecione a localização geográfica para alojar a sua conta do Azure Cosmos DB. Use o local mais próximo de seus usuários para dar a eles o acesso mais rápido aos dados.|
