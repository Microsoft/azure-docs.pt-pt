---
author: stevestein
ms.service: sql-database
ms.topic: include
ms.date: 11/25/2018
ms.author: sstein
ms.openlocfilehash: 1462f0dbeb45edeed964c24585b1fbc948f74233
ms.sourcegitcommit: 877491bd46921c11dd478bd25fc718ceee2dcc08
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 07/02/2020
ms.locfileid: "84013248"
---
## <a name="additional-resources"></a>Recursos adicionais
Ainda não está a usar ferramentas elásticas de base de dados? Consulte o nosso [Guia de Início.](../articles/azure-sql/database/elastic-scale-get-started.md)  Para obter perguntas, contacte-nos no [Microsoft Q&Página de perguntas para base de dados SQL](https://docs.microsoft.com/answers/topics/azure-sql-database.html) e para pedidos de funcionalidades, por favor adicione-as ao [fórum de feedback da Base de Dados SQL](https://feedback.azure.com/forums/217321-sql-database/).
