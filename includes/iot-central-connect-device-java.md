...
Na cópia do Microsoft Azure IoT SDK para Java que descarregou anteriormente, abra o *azure-iot-sdk-java/dispositivo/iot-device-samples/pnp-device-sample/termóstato-sample/src/main/java/samples/com/microsoft/azure/sdk/iot/device/thermostat.java* file em um editor de texto.
...
O `sendTemperatureTelemetry` método mostra como o dispositivo envia a telemetria de temperatura para a IoT Central:
...
O `updateMaxTemperatureSinceLastReboot` método envia uma `maxTempSinceLastReboot` atualização de propriedade para a IoT Central:
...
A `TargetTemperatureUpdateCallback` classe contém o método para lidar com `TwinPropertyCallBack` atualizações de propriedade writable da IoT Central:
...
A `GetMaxMinReportMethodCallback` classe contém o método para lidar com `call` comandos chamados da IoT Central:
...
No Windows, navegue para a pasta raiz do repositório Java SDK que descarregou.
...
Para executar a aplicação da amostra, abra um ambiente de linha de comando e navegue para a pasta *azure-iot-sdk-java/dispositivo/iot-device-samples/pnp-device-sample/termóstato-sample* que contém o *termóstato.java* ficheiro de amostra.
