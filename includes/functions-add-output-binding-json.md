...
Para criar uma ligação, clique à direita (Ctrl+clique no macOS) o `function.json` ficheiro na sua pasta HttpTrigger e escolha Adicionar a **ligação...**. Siga as instruções para definir as seguintes propriedades vinculativas para a nova ligação:
...
| **Selecione a definição de "local.setting.json"** | `AzureWebJobsStorage` | O nome de uma definição de aplicação que contém a cadeia de ligação para a conta de armazenamento. A `AzureWebJobsStorage` definição contém a cadeia de ligação para a conta de Armazenamento que criou com a aplicação de função. |

Uma ligação é `bindings` adicionada à matriz na sua função.json, que deve parecer o seguinte:
