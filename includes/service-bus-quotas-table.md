...
| Número de mensagens por transação | aciona | Mensagens de entrada adicionais são rejeitadas e uma exceção informando "não é possível enviar mais de 100 mensagens em uma única transação" é recebida pelo código de chamada. | 100 <br /><br /> Para as operações **Send ()** e **SendAsync ()** . |
