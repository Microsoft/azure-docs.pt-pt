...
| Número de [regras DeAuthorizationSEpartes partilhadas](/dotnet/api/microsoft.servicebus.messaging.sharedaccessauthorizationrule) por espaço de nome, fila ou tópico |Entidade, espaço de nome |Os pedidos subsequentes de criação de regras adicionais são rejeitados e uma exceção é recebida pelo código de chamada. |Número máximo de regras por tipo de entidade: 12. <br /><br /> As regras configuradas num espaço de nomes de Service Bus aplicam-se a todos os tipos: filas, tópicos. |
