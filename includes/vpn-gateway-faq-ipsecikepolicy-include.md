...
> [!IMPORTANT]
> *  DHGroup2048 & PFS2048 são iguais ao Grupo Diffie-Hellman **14** em IKE e IPsec PFS. Veja os [Grupos de Diffie-Hellman](#DH) para obter os mapeamentos completos.
> * Para algoritmos GCMAES, tem de especificar o mesmo algoritmo GCMAES e o comprimento da chave de encriptação de IPsec e da Integridade.
> * A vida útil do Modo Principal IKEv2 SA é fixada a 28.800 segundos nos gateways Azure VPN.
> * As Durações de SA QM são parâmetros opcionais. Se tiver sido especificado “nenhum”, são utilizados os valores predefinidos de 27,000 segundos (7,5 horas) e de 102400000 KBytes (102 GB).
> * UsePolicyBasedTrafficSelector é um parâmetro de opção da ligação. Consulte o próximo item faq para "UsePolicyBasedTrafficSelectors".
...
N.º O SKU Básico não apoia isto.
...
N.º Uma vez criada a ligação, os protocolos IKEv1/IKEv2 não podem ser alterados. Deve eliminar e recriar uma nova ligação com o tipo de protocolo pretendido.
...
Consulte [a política de Configuração IPsec/IKE para as ligações S2S ou VNet-to-VNet](../articles/vpn-gateway/vpn-gateway-ipsecikepolicy-rm-powershell.md).