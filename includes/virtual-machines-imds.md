...
| `/metadata/loadbalancer` | Consulte [metadados do balanceador de carga de recuperação através de IMDS](#load-balancer-metadata) | 2020-10-01
...
| `userData` | O conjunto de dados especificados quando o VM foi criado para utilização durante ou após o provisionamento (Base64 codificado)  | 2021-01-01
...
### <a name="get-user-data"></a>Obtenha dados do utilizador

Ao criar um novo VM, pode especificar um conjunto de dados a utilizar durante ou após a provisão VM e recuperá-los através do IMDS. Para configurar os dados do utilizador, utilize o modelo de arranque rápido [aqui](https://aka.ms/ImdsUserDataArmTemplate). A amostra abaixo mostra como recuperar estes dados através do IMDS.

> [!NOTE]
> Esta funcionalidade é lançada com versão `2021-01-01` e depende de uma atualização para a plataforma Azure, que se encontra atualmente a ser lançada e que ainda não se encontra disponível em todas as regiões.

> [!NOTE]
> Aviso de segurança: O IMDS está aberto a todas as aplicações no VM, não devem ser colocados dados sensíveis nos dados do utilizador.
...
#### <a name="sample-3-get-vm-tags"></a>Amostra 3: Obter tags VM
...
#### <a name="sample-4-get-more-information-about-the-vm-during-support-case"></a>Amostra 4: Obter mais informações sobre o VM durante o caso de apoio
...
#### <a name="sample-5-get-the-azure-environment-where-the-vm-is-running"></a>Amostra 5: Obtenha o Ambiente Azure onde o VM está em execução
...
#### <a name="sample-6-retrieve-network-information"></a>Amostra 6: Recuperar informações sobre a rede
...
#### <a name="sample-7-retrieve-public-ip-address"></a>Amostra 7: Recuperar endereço IP público
...
## <a name="load-balancer-metadata"></a>Metadados do balançador de carga
Quando coloca a máquina virtual ou a máquina virtual atrás de um Balancer de Carga Padrão Azure, pode utilizar o IMDS para recuperar metadados relacionados com o equilibrador de carga e as instâncias. Para obter mais informações, consulte [as informações do balanceador de carga de recuperar.](../articles/load-balancer/instance-metadata-service-load-balancer.md)
...
- Estou a perceber o `400 Bad Request, Required metadata header not specified` erro. O que é que isto quer dizer?
...
- Os dados dos utilizadores são os mesmos que os dados personalizados?
  - Os dados do utilizador oferecem a mesma funcionalidade a dados personalizados, permitindo-lhe passar os seus próprios metadados para a instância VM. A diferença é que os dados do utilizador são recuperados através do IMDS, e são persistentes ao longo da vida útil da instância VM. A funcionalidade de dados personalizados existente continuará a funcionar conforme descrito [neste artigo.](https://docs.microsoft.com/azure/virtual-machines/custom-data) No entanto, só é possível obter dados personalizados através da pasta do sistema local, não através do IMDS.
...
- Porque é que não estou a ver a informação do SKU para o meu VM em `instance/compute` detalhes?
  - Para imagens personalizadas criadas a partir do Azure Marketplace, a plataforma Azure não retém as informações do SKU para a imagem personalizada e os detalhes de quaisquer VMs criados a partir da imagem personalizada. Isto é por design e, portanto, não surgiu nos detalhes do `instance/compute` VM.
...
Pode fornecer feedback e ideias de produto ao nosso canal de feedback [](https://feedback.azure.com/forums/216843-virtual-machines?category_id=394627) do utilizador no Serviço de Metadados > Exemplo de Máquinas Virtuais
